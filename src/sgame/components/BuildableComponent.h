...
class BuildableComponent : public BuildableComponentBase {
    public:
        ...
        lifecycle_t GetState() {
            return state;
        }
        void SetState(lifecycle_t state) {
            this->state = state;
        }
         ...
         */
        int  GetMarkTime() {
            return marked ? markTime : 0;
        }

        void SetDeconstructionMark() {
            marked = true;
            markTime = level.time;
        }
        void ClearDeconstructionMark() {
            marked = false;
        }
        void ToggleDeconstructionMark() {
            marked = !marked;
            if (marked) {
                markTime = level.time;
            }
