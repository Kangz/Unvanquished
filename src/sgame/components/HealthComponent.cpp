...
void HealthComponent::HandleDamage(float amount, gentity_t* source, Util::optional<Vec3> location,
    ...
    // Check for immunity.
    if (entity.oldEnt->flags & FL_GODMODE) {
        return;
    }
    if (client) {
        if (client->noclip) {
    ...
    if (!source) {
        source = &g_entities[ENTITYNUM_WORLD];
    }
    ...
    if (health <= 0) {
        ...
        // Disable knockback.
        if (client) {
            entity.oldEnt->flags |= FL_NO_KNOCKBACK;
        }

        // Call legacy die function.
        if (entity.oldEnt->die) {
            entity.oldEnt->die(entity.oldEnt, source, source, meansOfDeath);
        }
        ...
        if (!client) {
            G_EventFireEntity(entity.oldEnt, source, ON_DIE);
        }
...
void HealthComponent::SetMaxHealth(float maxHealth, bool scaleHealth) {
    ...
    if (scaleHealth) {
        SetHealth(health * (this->maxHealth / maxHealth));
    }
}
