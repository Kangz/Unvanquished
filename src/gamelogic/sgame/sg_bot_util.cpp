...
float BotGetHealScore(gentity_t* self) {
    ...
    float maxHealth =
            BG_Class((class_t) self->client->ps.stats[STAT_CLASS])->health;
    ...
    distToHealer =
            MAX(MIN(MAX_HEAL_DIST, distToHealer), MAX_HEAL_DIST * (3.0f / 4.0f));
...
bool BotCanEvolveToClass(gentity_t* self, class_t newClass) {
    return (BG_ClassCanEvolveFromTo(
                    (class_t) self->client->ps.stats[STAT_CLASS], newClass, self->client->ps.persistant[PERS_CREDIT]) >= 0);
}
...
void BotGetDesiredBuy(gentity_t* self, weapon_t* weapon, upgrade_t* upgrades, int* numUpgrades) {
    ...
    if (BG_WeaponUnlocked(WP_PAIN_SAW) && BG_UpgradeUnlocked(UP_BATTLESUIT) &&
        usableCapital >=
                (BG_Weapon(WP_PAIN_SAW)->price + BG_Upgrade(UP_BATTLESUIT)->price)) {
        upgrades[0] = UP_BATTLESUIT;
        *numUpgrades = 1;
    } else if (BG_WeaponUnlocked(WP_SHOTGUN) &&
               BG_UpgradeUnlocked(UP_MEDIUMARMOUR) &&
               BG_UpgradeUnlocked(UP_RADAR) &&
               usableCapital >= (BG_Weapon(WP_SHOTGUN)->price +
                                 BG_Upgrade(UP_MEDIUMARMOUR)->price +
                                 BG_Upgrade(UP_RADAR)->price)) {
        upgrades[0] = UP_MEDIUMARMOUR;
        upgrades[1] = UP_RADAR;
        *numUpgrades = 2;
    } else if (BG_WeaponUnlocked(WP_SHOTGUN) &&
               BG_UpgradeUnlocked(UP_LIGHTARMOUR) &&
               BG_UpgradeUnlocked(UP_RADAR) &&
               usableCapital >= (BG_Weapon(WP_SHOTGUN)->price +
        ...
        upgrades[1] = UP_RADAR;
        *numUpgrades = 2;
    } else if (BG_WeaponUnlocked(WP_PAIN_SAW) &&
               BG_UpgradeUnlocked(UP_MEDIUMARMOUR) &&
               usableCapital >= (BG_Weapon(WP_PAIN_SAW)->price +
                                 BG_Upgrade(UP_MEDIUMARMOUR)->price)) {
        upgrades[0] = UP_MEDIUMARMOUR;
        *numUpgrades = 1;
    } else if (BG_WeaponUnlocked(WP_PAIN_SAW) &&
    ...
    if (g_bot_lcannon.integer && BG_WeaponUnlocked(WP_LUCIFER_CANNON) &&
        usableCapital >= BG_Weapon(WP_LUCIFER_CANNON)->price) {
        *weapon = WP_LUCIFER_CANNON;
        ;
    } else if (g_bot_chaingun.integer && BG_WeaponUnlocked(WP_CHAINGUN) &&
               usableCapital >= BG_Weapon(WP_CHAINGUN)->price &&
               upgrades[0] == UP_BATTLESUIT) {
        *weapon = WP_CHAINGUN;
    } else if (g_bot_flamer.integer && BG_WeaponUnlocked(WP_FLAMER) &&
               usableCapital >= BG_Weapon(WP_FLAMER)->price) {
        *weapon = WP_FLAMER;
    } else if (g_bot_prifle.integer && BG_WeaponUnlocked(WP_PULSE_RIFLE) &&
               usableCapital >= BG_Weapon(WP_PULSE_RIFLE)->price) {
        *weapon = WP_PULSE_RIFLE;
    } else if (g_bot_chaingun.integer && BG_WeaponUnlocked(WP_CHAINGUN) &&
               usableCapital >= BG_Weapon(WP_CHAINGUN)->price) {
        *weapon = WP_CHAINGUN;
        ;
    } else if (g_bot_mdriver.integer && BG_WeaponUnlocked(WP_MASS_DRIVER) &&
               usableCapital >= BG_Weapon(WP_MASS_DRIVER)->price) {
        *weapon = WP_MASS_DRIVER;
    } else if (g_bot_lasgun.integer && BG_WeaponUnlocked(WP_LAS_GUN) &&
               usableCapital >= BG_Weapon(WP_LAS_GUN)->price) {
        *weapon = WP_LAS_GUN;
    } else if (g_bot_shotgun.integer && BG_WeaponUnlocked(WP_SHOTGUN) &&
               usableCapital >= BG_Weapon(WP_SHOTGUN)->price) {
        *weapon = WP_SHOTGUN;
    } else if (g_bot_painsaw.integer && BG_WeaponUnlocked(WP_PAIN_SAW) &&
               usableCapital >= BG_Weapon(WP_PAIN_SAW)->price) {
        *weapon = WP_PAIN_SAW;
    } else {
    ...
    if (BG_InventoryContainsWeapon((int) *weapon, self->client->ps.stats)) {
        ...
        for (i = 0; i < *numUpgrades; i++) {
            if (BG_InventoryContainsUpgrade((int) upgrades[i],
                                            self->client->ps.stats)) {
                numContain++;
            }
        }
...
void BotFindClosestBuildings(gentity_t* self) {
    ...
    for (testEnt = &g_entities[MAX_CLIENTS];
         testEnt < &g_entities[level.num_entities - 1];
         testEnt++) {
        float newDist;
        ...
        if (testEnt->buildableTeam == TEAM_HUMANS &&
            (!testEnt->powered || !testEnt->spawned)) {
            continue;
        }
...
void BotFindDamagedFriendlyStructure(gentity_t* self) {
    ...
    for (target = &g_entities[MAX_CLIENTS];
         target < &g_entities[level.num_entities - 1];
         target++) {
        float distSqr;
        ...
        if (target->health >=
            BG_Buildable((buildable_t) target->s.modelindex)->health) {
            continue;
        }
...
gentity_t* BotFindBestEnemy(gentity_t* self) {
    ...
    bool hasRadar =
            (team == TEAM_ALIENS) ||
            (team == TEAM_HUMANS &&
             BG_InventoryContainsUpgrade(UP_RADAR, self->client->ps.stats));

    for (target = g_entities; target < &g_entities[level.num_entities - 1];
         target++) {
        float newScore;
        ...
        if (DistanceSquared(self->s.origin, target->s.origin) >
            Square(ALIENSENSE_RANGE)) {
            continue;
        }

        if (target->s.eType == ET_PLAYER &&
            self->client->pers.team == TEAM_HUMANS &&
        ...
        if (newScore > bestVisibleEnemyScore &&
            BotEntityIsVisible(self, target, MASK_SHOT)) {
...
void BotTargetToRouteTarget(gentity_t* self, botTarget_t target, botRouteTarget_t* routeTarget) {
    ...
    if (BotTargetIsEntity(target)) {
        if (target.ent->client) {
            BG_ClassBoundingBox((class_t) target.ent->client->ps.stats[STAT_CLASS],
                                mins,
                                maxs,
                                nullptr,
                                nullptr,
    ...
    if (BotTargetIsEntity(target)) {
        if (target.ent->s.eType == ET_BUILDABLE ||
            ...
            if (target.ent->s.origin2[2] < 0.7 || target.ent->s.eType == ET_PLAYER) {
                ...
                trap_Trace(&trace, targetPos, mins, maxs, end, target.ent->s.number, CONTENTS_SOLID, MASK_ENTITY);
...
bool BotTargetInAttackRange(gentity_t* self, botTarget_t target) {
    ...
    switch (self->client->ps.weapon) {
        ...
        case WP_ABUILD2:
            range = ABUILDER_CLAW_RANGE;
            secondaryRange = 300; // An arbitrary value for the blob launcher, has
                                  // nothing to do with actual range
            width = height = ABUILDER_CLAW_WIDTH;
            break;
        case WP_ALEVEL0:
        ...
        case WP_ALEVEL3:
            ...
            secondaryRange = LEVEL3_POUNCE_JUMP_MAG; // An arbitrary value for
                                                     // pounce, has nothing to do
                                                     // with actual range
        ...
        case WP_ALEVEL3_UPG:
            ...
            secondaryRange = LEVEL3_POUNCE_JUMP_MAG_UPG; // An arbitrary value for
                                                         // pounce and barbs, has
        ...
        case WP_ALEVEL4:
            ...
            secondaryRange = 0; // Using 0 since tyrant rush is basically just
                                // movement, not a ranged attack
        ...
        case WP_FLAMER: {
            ...
            // flamer projectiles add the player's velocity scaled by FLAMER_LAG to
            // the fire direction with length FLAMER_SPEED
        ...
        case WP_SHOTGUN:
            range =
                    (50 * 8192) /
                    SHOTGUN_SPREAD; // 50 is the maximum radius we want the spread to be
            secondaryRange = 0;
            break;
        case WP_MACHINEGUN:
            range =
                    (100 * 8192) /
                    RIFLE_SPREAD; // 100 is the maximum radius we want the spread to be
            secondaryRange = 0;
            break;
        case WP_CHAINGUN:
            range =
                    (60 * 8192) /
                    CHAINGUN_SPREAD; // 60 is the maximum radius we want the spread to be
            secondaryRange = 0;
            break;
        default:
            range =
                    4098 * 4;   // large range for guns because guns have large ranges :)
    ...
    if (self->client->pers.team !=
                BotGetEntityTeam(&g_entities[trace.entityNum]) &&
...
bool BotTargetIsVisible(gentity_t* self, botTarget_t target, int mask) {
    ...
    trap_Trace(&trace, muzzle, nullptr, nullptr, targetPos, self->s.number, mask, (mask == CONTENTS_SOLID) ? MASK_ENTITY : 0);
...
void BotGetIdealAimLocation(gentity_t* self, botTarget_t target, vec3_t aimLocation) {
    ...
    if (BotGetTargetType(target) != ET_BUILDABLE && BotTargetIsEntity(target) &&
        BotGetTargetTeam(target) == TEAM_HUMANS) {
        // aim at head
        aimLocation[2] += target.ent->r.maxs[2] * 0.85;

    } else if (BotGetTargetType(target) == ET_BUILDABLE ||
               BotGetTargetTeam(target) == TEAM_ALIENS) {
        ...
        if (self->client->ps.weapon == WP_LUCIFER_CANNON &&
            ...
            VectorMA(aimLocation,
                     Distance(self->s.origin, aimLocation) / LCANNON_SPEED,
                     target.ent->s.pos.trDelta,
                     aimLocation);
        }
    }
}
...
int BotGetAimPredictionTime(gentity_t* self) {
    return (10 - self->botMind->botSkill.level) * 100 *
           MAX(((float) rand()) / RAND_MAX, 0.5f);
}
...
void BotAimAtEnemy(gentity_t* self) {
    ...
    if (self->botMind->futureAimTime < level.time) {
        int predictTime = self->botMind->futureAimTimeInterval =
                BotGetAimPredictionTime(self);
    ...
    frac = (1.0f -
            ((float) (self->botMind->futureAimTime - level.time)) /
                    self->botMind->futureAimTimeInterval);
...
bool BotTeamateHasWeapon(gentity_t* self, int weapon) {
    ...
    int i;
    int numBots =
            FindBots(botNumbers, MAX_CLIENTS, (team_t) self->client->pers.team);
    ...
    switch (self->client->ps.stats[STAT_CLASS]) {
        ...
        case PCL_ALIEN_LEVEL3_UPG:
            if (BotGetTargetType(self->botMind->goal) == ET_BUILDABLE &&
                self->client->ps.ammo > 0 && inAttackRange) {
...
float CalcAimPitch(gentity_t* self, botTarget_t target, vec_t launchSpeed) {
    ...
    distance2D = sqrt(Square(startPos[0] - targetPos[0]) +
                      Square(startPos[1] - targetPos[1]));
    ...
    // This is better than returning some failure value because it gives us the
    // best launch angle possible, even if it wont hit in the end.
    while (check < 0) {
        v += 5;
        check = Square(Square(v)) - g * (g * Square(x) + 2 * y * Square(v));
    }
...
float CalcPounceAimPitch(gentity_t* self, botTarget_t target) {
    vec_t speed = (self->client->ps.stats[STAT_CLASS] == PCL_ALIEN_LEVEL3)
                          ? LEVEL3_POUNCE_JUMP_MAG
    ...
    switch (self->s.weapon) {
        case WP_ABUILD:
            if (distance <= ABUILDER_CLAW_RANGE) {
                ...
                usercmdPressButton(botCmdBuffer->buttons,
                                   BUTTON_GESTURE); // make cute granger sounds to ward
        ...
        case WP_ALEVEL3:
            if (distance > LEVEL3_CLAW_RANGE &&
                self->client->ps.stats[STAT_MISC] < LEVEL3_POUNCE_TIME) {
                botCmdBuffer->angles[PITCH] = ANGLE2SHORT(
                        -CalcPounceAimPitch(self, self->botMind->goal)); // compute and
                                                                         // apply correct
                                                                         // aim pitch to
                                                                         // hit target
        ...
        case WP_ALEVEL3_UPG:
            if (self->client->ps.ammo > 0 && distance > LEVEL3_CLAW_UPG_RANGE) {
                botCmdBuffer->angles[PITCH] = ANGLE2SHORT(
                        -CalcBarbAimPitch(self, self->botMind->goal)); // compute and apply
                                                                       // correct aim pitch
                                                                       // to hit target
                BotFireWeapon(WPM_TERTIARY, botCmdBuffer);             // goon barb
            } else if (distance > LEVEL3_CLAW_UPG_RANGE &&
                       self->client->ps.stats[STAT_MISC] < LEVEL3_POUNCE_TIME_UPG) {
                botCmdBuffer->angles[PITCH] = ANGLE2SHORT(
                        -CalcPounceAimPitch(self, self->botMind->goal)); // compute and
                                                                         // apply correct
                                                                         // aim pitch to
        ...
        case WP_ALEVEL4:
            if (distance > LEVEL4_CLAW_RANGE &&
                self->client->ps.stats[STAT_MISC] < LEVEL4_TRAMPLE_CHARGE_MAX) {
            ...
            break;
        case WP_LUCIFER_CANNON:
            if (self->client->ps.stats[STAT_MISC] <
                LCANNON_CHARGE_TIME_MAX * Com_Clamp(0.5, 1, random())) {
...
bool BotEvolveToClass(gentity_t* ent, class_t newClass) {
    ...
    if (ent->client->pers.classSelection != PCL_NONE) {
        ...
        for (i = 0; i < num; i++) {
            ...
            if ((other->client && other->client->pers.team == TEAM_HUMANS) ||
                (other->s.eType == ET_BUILDABLE &&
                 other->buildableTeam == TEAM_HUMANS)) {
                return false;
            }
        }
        ...
        numLevels = BG_ClassCanEvolveFromTo(
                currentClass, newClass, (short) ent->client->ps.persistant[PERS_CREDIT]);

        if (G_RoomForClassChange(ent, newClass, infestOrigin)) {
            //...check we can evolve to that class
            if (numLevels >= 0 && BG_ClassUnlocked(newClass) &&
                !BG_ClassDisabled(newClass)) {
            ...
            // trap_SendServerCommand( -1, va( "print \"Not enough evos to evolve to
            // %s\n\"", classname) );
            {
                return false;
            }
...
void BotBuyWeapon(gentity_t* self, weapon_t weapon) {
    if (weapon != WP_NONE) {
        ...
        if (BG_Weapon(weapon)->slots &
            BG_SlotsForInventory(self->client->ps.stats)) {
            return;
        }
...
gentity_t* BotPopEnemy(enemyQueue_t* queue) {
    ...
    if (level.time - queue->enemys[queue->front].timeFound >=
        g_bot_reactiontime.integer) {
...
void BotPain(gentity_t* self, gentity_t* attacker, int damage) {
    if (BotGetEntityTeam(attacker) != TEAM_NONE &&
        BotGetEntityTeam(attacker) != self->client->pers.team) {
...
void BotSearchForEnemy(gentity_t* self) {
    ...
    if (self->botMind->bestEnemy.ent) {
        self->botMind->bestEnemy.distance =
                Distance(self->s.origin, self->botMind->bestEnemy.ent->s.origin);
    } else {
