...
void G_FindEntityGroups() {
    ...
    for (i = MAX_CLIENTS, masterEntity = g_entities + i; i < level.num_entities;
         i++, masterEntity++) {
        ...
        for (j = i + 1, comparedEntity = masterEntity + 1; j < level.num_entities;
             j++, comparedEntity++) {
...
void G_InitSetEntities() {
    ...
    for (i = MAX_CLIENTS, entity = g_entities + i; i < level.num_entities;
         i++, entity++) {
...
void G_RegisterCvars() {
    ...
    for (i = 0, cvarTable = gameCvarTable; i < gameCvarTableSize;
         i++, cvarTable++) {
        trap_Cvar_Register(cvarTable->vmCvar, cvarTable->cvarName, cvarTable->defaultString, cvarTable->cvarFlags);
            ...
            if (cv->modificationCount != cv->vmCvar->modificationCount) {
                ...
                if (cv->trackChange) {
                    trap_SendServerCommand(
                            -1,
                            va("print_tr %s %s %s", QQ(N_("Server: $1$ changed to $2$\n")), Quote(cv->cvarName), Quote(cv->vmCvar->string)));
                }
...
void G_InitGame(int levelTime, int randomSeed, int restart, bool inClient) {
    ...
    level.snd_fry =
            G_SoundIndex("sound/misc/fry.wav"); // FIXME standing in lava / slime
    ...
    if (g_logGameplayStatsFrequency.integer > 0) {
        ...
        Com_sprintf(logfile, sizeof(logfile), "stats/gameplay/%04i%02i%02i_%02i%02i%02i_%s.log", 1900 + qt.tm_year, qt.tm_mon + 1, qt.tm_mday, qt.tm_hour, qt.tm_min, qt.tm_sec, mapname);
    ...
    // initialise whether bot vote kicks are allowed. the map rotation may clear
    // this flag.
    trap_Cvar_Set("g_botKickVotesAllowedThisMap",
                  g_botKickVotesAllowed.integer ? "1" : "0");
    ...
    for (int team = TEAM_NONE + 1; team < NUM_TEAMS; team++) {
        int startBP = std::max(0, g_initialBuildPoints.integer - level.team[(team_t) team].layoutBuildPoints);

        G_ModifyBuildPoints((team_t) team, (float) startBP);
        G_MarkBuildPointsMined((team_t) team, (float) startBP);

        level.team[(team_t) team].mainStructAcquiredBP =
                std::max((float) startBP, FLT_EPSILON);
    }
...
void G_SendClientPmoveParams(int client) {
    trap_SendServerCommand(
            client, va("pmove_params %i %i %i %i", level.pmoveParams.synchronous, level.pmoveParams.fixed, level.pmoveParams.msec, level.pmoveParams.accurate));
}
...
void G_SpawnClients(team_t team) {
    ...
    if (G_GetSpawnQueueLength(sq) > 0 && numSpawns > 0) {
        ...
        if ((spawn = G_SelectUnvanquishedSpawnPoint(
...
void G_CalculateAvgPlayers() {
    ...
    for (team = TEAM_NONE + 1; team < NUM_TEAMS; team++) {
        ...
        if (*samples == 0) {
            ...
            *avgPlayers =
                    ((*avgPlayers * *samples) + currentPlayers) / (*samples + 1);
...
void CalculateRanks() {
    ...
    // voting code expects level.team[ TEAM_NONE ].numPlayers to be all players,
...
void G_AdminMessage(gentity_t* ent, const char* msg) {
    ...
    Com_sprintf(
            string, sizeof(string), "chat %ld %d %s", ent ? (long) (ent - g_entities) : -1, G_admin_permission(ent, ADMF_ADMINCHAT) ? SAY_ADMINS : SAY_ADMINS_PUBLIC, Quote(msg));
    ...
    G_LogPrintf(
            "%s: %d \"%s" S_COLOR_WHITE "\": " S_COLOR_MAGENTA "%s\n",
...
Calculates the average amount of spare credits as well as the value of each
teams' players.
=================
*/
static void GetAverageCredits(int teamCredits[], int teamValue[]) {
    ...
    for (team = TEAM_ALIENS; team < NUM_TEAMS; ++team) {
        teamCredits[team] =
                (teamCnt[team] == 0) ? 0 : (teamCredits[team] / teamCnt[team]);
        teamValue[team] =
                (teamCnt[team] == 0) ? 0 : (teamValue[team] / teamCnt[team]);
    }
}
...
static void G_LogGameplayStats(int state) {
    ...
    switch (state) {
        case LOG_GAMEPLAY_STATS_HEADER: {
            ...
            Com_sprintf(logline, sizeof(logline), "# "
                                                  ...
                                                  "13   14   15   16\n"
                                                  "#  T #A #H AMom HMom  LMR  AME  HME  ABP  HBP ABRV HBRV "
                                                  "ACre HCre AVal HVal\n"
                                                  "# "
                                                  "------------------------------------------------------------"
                                                  "-------------\n",
        ...
        case LOG_GAMEPLAY_STATS_BODY: {
            ...
            Com_sprintf(
                    logline, sizeof(logline), "%4i %2i %2i %4i %4i %4.1f %4i %4i %4i %4i %4i %4i %4i %4i %4i %4i\n", time, num[TEAM_ALIENS], num[TEAM_HUMANS], Mom[TEAM_ALIENS], Mom[TEAM_HUMANS], LMR, ME[TEAM_ALIENS], ME[TEAM_HUMANS], BP[TEAM_ALIENS], BP[TEAM_HUMANS], BRV[TEAM_ALIENS], BRV[TEAM_HUMANS], Cre[TEAM_ALIENS], Cre[TEAM_HUMANS], Val[TEAM_ALIENS], Val[TEAM_HUMANS]);
            break;
        }
        case LOG_GAMEPLAY_STATS_FOOTER: {
            ...
            Com_sprintf(logline, sizeof(logline), "# "
                        ...
                        level.team[TEAM_ALIENS].averageNumPlayers,
                        level.team[TEAM_ALIENS].averageNumBots,
                        level.team[TEAM_HUMANS].averageNumPlayers,
                        level.team[TEAM_HUMANS].averageNumBots);
            break;
        }
        default:
            return;
    ...
    if (state == LOG_GAMEPLAY_STATS_BODY) {
        nextCalculation =
                level.time + MAX(1, g_logGameplayStatsFrequency.integer) * 1000;
    } else {
        nextCalculation = 0;
    }
}
...
void G_SendGameStat(team_t team) {
    ...
    Com_sprintf(data, BIG_INFO_STRING, "%s %s T:%c A:%f H:%f M:%s D:%d CL:%d", Q3_VERSION, g_tag.string, teamChar, level.team[TEAM_ALIENS].averageNumClients, level.team[TEAM_HUMANS].averageNumClients, map, level.matchTime, level.numConnectedClients);
    ...
    for (i = 0; i < level.numConnectedClients; i++) {
        ...
        Com_sprintf(entry, MAX_STRING_CHARS, " \"%s\" %c %d %d %d", cl->pers.netname, teamChar, cl->ps.persistant[PERS_SCORE], ping, (level.time - cl->pers.enterTime) / 60000);
...
void G_Vote(gentity_t* ent, team_t team, bool voting) {
    ...
    // TODO maybe refactor vote no/yes in one and only one variable and divide the
    // NLOC by 2 ?
...
void G_CheckVote(team_t team) {
    ...
    if ((level.time - level.team[team].voteTime >= VOTE_TIME) ||
        ...
        pass = (level.team[team].voteYes &&
                (float) level.team[team].voteYes / ((float) level.team[team].voteYes +
                                                    (float) level.team[team].voteNo) >
                        votePassThreshold);
    } else {
        if ((float) level.team[team].voteYes >
            ...
            pass = true;
                   ...
                   (float) level.team[team].numPlayers *
                           (1.0f - votePassThreshold)) {
            return;
        }
    }
    ...
    if (!quorum) {
        cmd =
                va("print_tr %s %d %d",
                   (team == TEAM_NONE)
                           ? QQ(N_("Vote failed ($1$ of $2$; quorum not reached)\n"))
        ...
        cmd = va("print_tr %s %d %d %.0f",
                 (team == TEAM_NONE)
                         ? QQ(N_("Vote failed ($1$ — $2$; $3$% needed)\n"))
    ...
    if (level.pausedTime) {
        ...
        while (ptime3000 > 3000) {
            ...
            trap_SendServerCommand(-1,
                                   "cp \"The game has been paused. Please wait.\"");

            if (level.pausedTime >= 110000 && level.pausedTime <= 119000) {
                trap_SendServerCommand(
                        -1, va("print_tr %s %d", QQ(N_("Server: Game will auto-unpause in $1$ seconds\n")), (int) ((float) (120000 - level.pausedTime) / 1000.0f)));
            }
        }
        ...
        if (level.pausedTime > 120000) {
            trap_SendServerCommand(-1, "print_tr \"" N_("Server: The game has been unpaused "
                                                        "automatically (2 minute max)\n") "\"");
