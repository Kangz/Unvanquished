...
/**
 * searches for a the grading texture with the given name among the
 *configstrings and returns the index
 * if it wasn't found it will add the texture to the configstrings, send these
 *to the client and return the new index
 *
 * the first one at CS_GRADING_TEXTURES is always the global one, so we start
 *searching from CS_GRADING_TEXTURES+1
 */
...
void G_TeleportPlayer(gentity_t* player, vec3_t origin, vec3_t angles, float speed) {
    ...
    player->client->ps.pm_time =
            0.4f * abs(speed); // duration of loss of control
...
void G_KillBox(gentity_t* ent) {
    ...
    for (i = 0; i < num; i++) {
        ...
        // nail it
        G_Damage(hit, ent, ent, nullptr, nullptr, 100000, DAMAGE_NO_PROTECTION, MOD_TELEFRAG);
    }
}
...
void G_KillBrushModel(gentity_t* ent, gentity_t* activator) {
    ...
    for (e = &g_entities[0]; e < &g_entities[level.num_entities]; ++e) {
        if (!e->takedamage || !e->r.linked || !e->clipmask ||
            (e->client && e->client->noclip))
            continue;
        ...
        trap_Trace(&tr, e->r.currentOrigin, e->r.mins, e->r.maxs, e->r.currentOrigin, e->s.number, e->clipmask, 0);
...
Calculates loMask/hiMask as used by SVF_CLIENTMASK type events to match only the
given client.
===============
*/
...
Calculates loMask/hiMask as used by SVF_CLIENTMASK type events to match all
clients in a team.
===============
*/
...
void G_FireThink(gentity_t* self) {
    ...
    if (self->nextBurnSplashDamage < level.time) {
        ...
        hit = G_SelectiveRadiusDamage(self->s.origin, self->fireStarter, BURN_SPLDAMAGE, BURN_SPLDAMAGE_RADIUS, self, MOD_BURN, TEAM_NONE);
    ...
    if (self->nextBurnAction < level.time) {
        ...
        while ((neighbor = G_IterateEntitiesWithinRadius(neighbor, self->s.origin, BURN_STOP_RADIUS))) {
            ...
            if (neighbor->onFire && neighbor->health > 0) {
                float frac = Distance(self->s.origin, neighbor->s.origin) /
                             (float) BURN_STOP_RADIUS;
        ...
        while ((neighbor = G_IterateEntitiesWithinRadius(neighbor, self->s.origin, BURN_SPREAD_RADIUS))) {
            ...
            chance = 1.0f - (Distance(self->s.origin, neighbor->s.origin) /
                             (float) BURN_SPREAD_RADIUS);

            if (random() < chance && G_LineOfSight(self, neighbor)) {
                if (g_debugFire.integer) {
                    Com_Printf("%s has chance to ignite a neighbour of %.2f → ^2ignite^7",
                               descr,
                               chance);
                }
                ...
                Com_Printf(
                        "%s has chance to ignite a neighbour of %.2f → failed or no los",
                        descr,
                        chance);
            }
        }
    ...
    // HACK: Assume that all non-ET_FIRE entities that can catch fire will think
    // frequently enough.
...
gentity_t* G_SpawnFire(vec3_t origin, vec3_t normal, gentity_t* fireStarter) {
    ...
    // don't spawn fire on walls and ceiling since we can't display it properly
    // yet
    ...
    fire = nullptr;
    while (
            (fire = G_IterateEntitiesWithinRadius(fire, origin, FIRE_MIN_DISTANCE))) {
...
bool G_LineOfSight(const gentity_t* from, const gentity_t* to, int mask, bool useTrajBase) {
    ...
    trap_Trace(&trace, useTrajBase ? from->s.pos.trBase : from->s.origin, nullptr, nullptr, to->s.origin, from->s.number, mask, 0);

    // Also check for fraction in case the mask is chosen so that the trace skips
    // the target entity
    return (trace.entityNum == to->s.number || trace.fraction == 1.0f);
}

/**
 * @return Wheter a shot from the source's origin towards the target's origin
...
/**
 * @return Wheter a shot from the source's trajectory base towards the target's
 * origin would hit the
 *         target.
 */
...
/**
 * @brief This version of line of sight only considers map geometry, including
 * movers.
...
int G_Heal(gentity_t* self, int amount) {
    ...
    if (maxHealth) {
        if (self->health == maxHealth) {
            ...
            Com_Printf(S_WARNING
                       "G_Heal: Target has health above max health (%i/%i).\n",
                       self->health,
                       maxHealth);
            return 0;
        }
    ...
    totalCredits = 0;
    for (clientNum = 0, relevantClientNum = 0; clientNum < MAX_CLIENTS;
         clientNum++) {
