...
On each frame, the behavior tree for each bot is evaluated starting from the
root node
Each node returns either STATUS_SUCCESS, STATUS_RUNNING, or STATUS_FAILURE
depending on their logic
The return values are used in various sequences and selectors to change the
...
It starts at the first child node, and if the node did not fail, it returns its
status
...
It starts at the first previously running child node, and if the node does not
succeed, it returns its status
...
// using double because it has enough precision to exactly represent both a
// float and an int
...
bool EvaluateBinaryOp(gentity_t* self, AIExpType_t* exp) {
    ...
    switch (o->opType) {
        ...
        case OP_NEQUAL:
            return EvalValue(self, o->exp1) != EvalValue(self, o->exp2);
        case OP_AND:
            return EvalConditionExpression(self, o->exp1) &&
                   EvalConditionExpression(self, o->exp2);
        case OP_OR:
            return EvalConditionExpression(self, o->exp1) ||
                   EvalConditionExpression(self, o->exp2);
        default:
...
A behavior tree may contain multiple other behavior trees which are run in this
way
======================
*/
...
AINodeStatus_t BotEvaluateNode(gentity_t* self, AIGenericNode_t* node) {
    ...
    // reset running information on node success so sequences and selectors reset
    // their state
    ...
    if (status == STATUS_RUNNING) {
        ...
        // this insures that only 1 node in a sequence or selector has the running
        // state
...
AINodeStatus_t BotActionFireWeapon(gentity_t* self, AIGenericNode_t* node) {
    if (WeaponIsEmpty(BG_GetPlayerWeapon(&self->client->ps), &self->client->ps) &&
...
AINodeStatus_t BotActionActivateUpgrade(gentity_t* self,
...
AINodeStatus_t BotActionDeactivateUpgrade(gentity_t* self,
                                          AIGenericNode_t* node) {
    AIActionNode_t* action = (AIActionNode_t*) node;
    upgrade_t u = (upgrade_t) AIUnBoxInt(action->params[0]);
...
AINodeStatus_t BotActionAlternateStrafe(gentity_t* self,
                                        AIGenericNode_t* node) {
...
// TODO: Move decision making out of these actions and into the rest of the
// behavior tree
AINodeStatus_t BotActionFight(gentity_t* self, AIGenericNode_t* node) {
    ...
    // aliens have radar so they will always 'see' the enemy if they are in radar
    // range
    if (myTeam == TEAM_ALIENS &&
        DistanceToGoalSquared(self) <= Square(ALIENSENSE_RANGE)) {
        self->botMind->enemyLastSeen = level.time;
    }
    ...
    if (!BotTargetIsVisible(self, self->botMind->goal, CONTENTS_SOLID)) {
        ...
        if (self->botMind->bestEnemy.ent &&
            self->botMind->goal.ent != self->botMind->bestEnemy.ent &&
            BotPathIsWalkable(self, proposedTarget)) {
            return STATUS_SUCCESS;
        } else if (level.time - self->botMind->enemyLastSeen >=
                   g_bot_chasetime.integer) {
            return STATUS_SUCCESS;
        } else {
            BotMoveToGoal(self);
            return STATUS_RUNNING;
        ...
        if ((inAttackRange && myTeam == TEAM_HUMANS) ||
            self->botMind->nav.directPathToGoal) {
            BotAimAtEnemy(self);
            ...
            if (myTeam == TEAM_HUMANS) {
                if (self->botMind->botSkill.level >= 3 &&
                    DistanceToGoalSquared(self) < Square(MAX_HUMAN_DANCE_DIST) &&
                    (DistanceToGoalSquared(self) > Square(MIN_HUMAN_DANCE_DIST) ||
                     self->botMind->botSkill.level < 5) &&
                    self->client->ps.weapon != WP_PAIN_SAW) {
                    BotMoveInDir(self, MOVE_BACKWARD);
                } else if (DistanceToGoalSquared(self) <=
                           Square(
                                   MIN_HUMAN_DANCE_DIST)) // we wont hit this if skill < 5
                {
                    ...
                    BotAlternateStrafe(self);
                           ...
                           self->client->ps.weapon != WP_PAIN_SAW) {
                    if (DistanceToGoalSquared(self) - Square(MAX_HUMAN_DANCE_DIST) <
                        100) {
                        BotStandStill(self);
                    }
                ...
                if (inAttackRange &&
                    BotGetTargetType(self->botMind->goal) == ET_BUILDABLE) {
                    BotStandStill(self);
                }
...
AINodeStatus_t BotActionMoveTo(gentity_t* self, AIGenericNode_t* node) {
    ...
    if (DistanceToGoal2DSquared(self) <= Square(radius) &&
        self->botMind->nav.directPathToGoal) {
        return STATUS_SUCCESS;
    }
...
AINodeStatus_t BotActionEvolve(gentity_t* self, AIGenericNode_t* node) {
    ...
    } else if (BotCanEvolveToClass(self, PCL_ALIEN_LEVEL3_UPG) &&
               g_bot_level3upg.integer) {
        if (BotEvolveToClass(self, PCL_ALIEN_LEVEL3_UPG)) {
            status = STATUS_SUCCESS;
        }
    } else if (BotCanEvolveToClass(self, PCL_ALIEN_LEVEL3) &&
               (!BG_ClassUnlocked(PCL_ALIEN_LEVEL3_UPG) ||
                !g_bot_level2upg.integer || !g_bot_level3upg.integer) &&
               g_bot_level3.integer) {
        if (BotEvolveToClass(self, PCL_ALIEN_LEVEL3)) {
            status = STATUS_SUCCESS;
        }
    } else if (BotCanEvolveToClass(self, PCL_ALIEN_LEVEL2_UPG) &&
               g_bot_level2upg.integer) {
        if (BotEvolveToClass(self, PCL_ALIEN_LEVEL2_UPG)) {
            status = STATUS_SUCCESS;
        }
    } else if (BotCanEvolveToClass(self, PCL_ALIEN_LEVEL2) &&
               g_bot_level2.integer) {
        if (BotEvolveToClass(self, PCL_ALIEN_LEVEL2)) {
            status = STATUS_SUCCESS;
        }
    } else if (BotCanEvolveToClass(self, PCL_ALIEN_LEVEL1) &&
               g_bot_level1.integer) {
...
AINodeStatus_t BotActionHealH(gentity_t* self, AIGenericNode_t* node) {
    ...
    bool fullyHealed =
            BG_Class(self->client->ps.stats[STAT_CLASS])->health <=
                    self->client->ps.stats[STAT_HEALTH] &&
    ...
    if (self->botMind->currentNode != node) {
        ...
        if (!BotChangeGoalEntity(
                    self, self->botMind->closestBuildings[BA_H_MEDISTAT].ent)) {
            return STATUS_FAILURE;
        }
        self->botMind->currentNode = node;
    }
    ...
    if (DistanceSquared(myPos, targetPos) >
        Square(BG_BuildableModelConfig(BA_H_MEDISTAT)->mins[1])) {
        BotMoveToGoal(self);
    }
    return STATUS_RUNNING;
}
AINodeStatus_t BotActionRepair(gentity_t* self, AIGenericNode_t* node) {
    ...
    if (self->botMind->goal.ent->health >=
        BG_Buildable((buildable_t) self->botMind->goal.ent->s.modelindex)
                ->health) {
        return STATUS_SUCCESS;
    }
    ...
    if (!BotTargetIsVisible(self, self->botMind->goal, MASK_SHOT) ||
        DistanceToGoalSquared(self) > Square(100)) {
        BotMoveToGoal(self);
    } else {
...
AINodeStatus_t BotActionBuy(gentity_t* self, AIGenericNode_t* node) {
    ...
    if (buy->nparams == 0) {
        ...
        if (weapon < WP_NONE || weapon >= WP_NUM_WEAPONS) {
            BotDPrintf(S_COLOR_YELLOW
                       "WARNING: parameter 1 to action buy out of range\n");
            weapon = WP_NONE;
        }
        ...
        for (i = 1; i < buy->nparams; i++) {
            ...
            if (upgrades[numUpgrades] <= UP_NONE ||
                upgrades[numUpgrades] >= UP_NUM_UPGRADES) {
                BotDPrintf(S_COLOR_YELLOW
                           "WARNING: parameter %d to action buy out of range\n",
                           i + 1);
                continue;
            }
