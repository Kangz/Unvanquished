...
bool G_BotAdd(char* name, team_t team, int skill, const char* behavior) {
    ...
    userinfo[0] = '\0';
    Info_SetValueForKey(userinfo, "name", name ? name : "",
                        false); // allow defaulting
...
void G_BotDel(int clientNum) {
    ...
    trap_SendServerCommand(-1,
                           va("print_tr %s %s", QQ(N_("$1$^7 disconnected\n")), Quote(bot->client->pers.netname)));
...
void G_BotThink(gentity_t* self) {
    ...
    self->botMind->behaviorTree->run(
            self, (AIGenericNode_t*) self->botMind->behaviorTree);
