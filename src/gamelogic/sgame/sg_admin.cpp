...
static void G_admin_notIntermission(gentity_t* ent) {
    ...
    *command = 0;
    ...
    ADMP(va("%s %s",
            QQ(N_("^3$1$: ^7this command is not valid during intermission\n")),
            command));
}
...
static const g_admin_cmd_t g_admin_cmds[] = {
        ...
        {"adjustban", G_admin_adjustban, false, "ban", N_("change the IP address mask, duration or reason of a ban.  mask is "
                                                          "prefixed with '/'.  duration is specified as "
                                                          "numbers followed by units 'w' (weeks), 'd' (days), 'h' (hours) or "
                                                          "'m' (minutes), or seconds if no units are specified.  if the duration "
                                                          "is"
                                                          " preceded by a + or -, the ban duration will be extended or shortened "
                                                          "by"
                                                          " the specified amount"),
         N_("[^3ban#^7] (^5/mask^7) (^5duration^7) (^5reason^7)")},

        {"adminhelp", G_admin_adminhelp, true, "adminhelp", N_("display admin commands available to you or help on a specific "
                                                               "command"),
         N_("(^5command^7)")},

        {"admintest", G_admin_admintest, false, "admintest", N_("display your current admin level"), ""},

        {"allowbuild", G_admin_denybuild, false, "denybuild", N_("restore a player's ability to build"), N_("[^3name|slot#^7]")},

        {"allready", G_admin_allready, false, "allready", N_("makes everyone ready in intermission"), ""},

        {"ban", G_admin_ban, false, "ban", N_("ban a player by IP address and GUID with an optional expiration time "
                                              "and reason."
        ...
        {"bot", G_admin_bot, false, "bot", N_("Add/Del/Spec bots"), N_("[^5add|del|spec|unspec^7] [^5name|all^7] [^5aliens/humans^7] "
                                                                       "(^5skill^7)")},
        {"builder", G_admin_builder, true, "builder", N_("show who built a structure"), ""},

        {"buildlog", G_admin_buildlog, false, "buildlog", N_("show buildable log"), N_("(^5name|slot#^7) (^5id^7)")},

        {"cancelvote", G_admin_endvote, false, "cancelvote", N_("cancel a vote taking place"), "(^5a|h^7)"},

        {"changemap", G_admin_changemap, false, "changemap", N_("load a map (and optionally force layout)"), N_("[^3mapname^7] (^5layout^7)")},

        {"denybuild", G_admin_denybuild, false, "denybuild", N_("take away a player's ability to build"), N_("[^3name|slot#^7]")},

        {"flag", G_admin_flag, false, "flag", N_("add an admin flag to a player, prefix flag with '-' to disallow the "
                                                 "flag. "
                                                 "console can use this command on admin levels by prefacing a '*' to "
                                                 "the admin level value."),
         N_("[^3name|slot#|admin#|*level#^7] (^5+^7|^5-^7)[^3flag^7]")},

        {"flaglist", G_admin_flaglist, false, "flag", N_("list the flags understood by this server"), ""},

        {"kick", G_admin_kick, false, "kick", N_("kick a player with an optional reason"), N_("[^3name|slot#^7] (^5reason^7)")},

        {"l0", G_admin_l0, false, "l0", N_("remove name protection from a player "
                                           "by setting them to admin level 0"),
         N_("[^3name|slot#|admin#^7]")},

        {"l1", G_admin_l1, false, "l1", N_("give a player name protection by setting them to admin level 1"), N_("[^3name|slot#^7]")},

        {"listadmins", G_admin_listadmins, true, "listadmins", N_("display a list of all server admins and their levels"), N_("(^5name^7) (^5start admin#^7)")},

        {"listgeoip", G_admin_listgeoip, true, "listgeoip", N_("display a list of players and which countries they're from (using "
                                                               "GeoIP data)"),
         N_("(^5name^7)")},

        {"listinactive", G_admin_listinactive, true, "listadmins", N_("display a list of inactive server admins and their levels"), N_("[^5months^7] (^5start admin#^7)")},

        {"listlayouts", G_admin_listlayouts, true, "listlayouts", N_("display a list of all available layouts for a map"), N_("(^5mapname^7)")},

        {"listplayers", G_admin_listplayers, true, "listplayers", N_("display a list of players, their client numbers and their levels"), ""},

        {"lock", G_admin_lock, false, "lock", N_("lock a team to prevent anyone from joining it"), "[^3a|h^7]"},

        {"maprestarted", G_admin_maprestarted, false, "", nullptr, nullptr},

        {"mute", G_admin_mute, false, "mute", N_("mute a player"), "[^3name|slot#^7]"},

        {"namelog", G_admin_namelog, true, "namelog", N_("display a list of names used by recently connected players"), N_("(^5name|IP(/mask)^7) (start namelog#)")},

        {"nextmap", G_admin_nextmap, false, "nextmap", N_("go to the next map in the cycle"), ""},

        {"passvote", G_admin_endvote, false, "passvote", N_("pass a vote currently taking place"), "(^5a|h^7)"},

        {"pause", G_admin_pause, false, "pause", N_("Pause (or unpause) the game."), ""},

        {"putteam", G_admin_putteam, false, "putteam", N_("move a player to a specified team"), N_("[^3name|slot#^7] [^3h|a|s^7]")},

        {"readconfig", G_admin_readconfig, false, "readconfig", N_("reloads the admin config file and refreshes permission flags"), ""},

        {"register", G_admin_register, false, "register", N_("register your name to protect it from being used by other players."), ""},

        {"rename", G_admin_rename, false, "rename", N_("rename a player"), N_("[^3name|slot#^7] [^3new name^7]")},

        {"restart", G_admin_restart, false, "restart", N_("restart the current map (optionally using named layout or "
                                                          "keeping/switching teams)"),
         N_("(^5layout^7) "
            "(^5keepteams|switchteams|keepteamslock|switchteamslock^7)")},

        {"revert", G_admin_revert, false, "revert", N_("revert buildables to a given time"), N_("[^3id^7]")},

        {"setlevel", G_admin_setlevel, false, "setlevel", N_("sets the admin level of a player"), N_("[^3name|slot#|admin#^7] [^3level^7]")},

        {"showbans", G_admin_showbans, true, "showbans", N_("display a (partial) list of active bans"), N_("(^5name|IP(/mask)^7) (^5start at ban#^7)")},

        {"spec999", G_admin_spec999, false, "spec999", N_("move 999 pingers to the spectator team"), ""},

        {"speclock", G_admin_speclock, false, "kick", N_("move a player to spectators and prevent from joining a team"
                                                         " duration is specified as numbers followed by units 'h' (hours) or "
                                                         "'m' (minutes), "
                                                         "or seconds if no units are specified. End-of-game automatically "
                                                         "revokes this"),
         N_("[^3name|slot#^7] [^3duration^7]")},

        {"specunlock", G_admin_specunlock, false, "ban", N_("allow a player to join any team again"), N_("[^3name|slot#^7]")},
        ...
        {"unban", G_admin_unban, false, "ban", N_("unbans a player specified by the slot as seen in showbans"), N_("[^3ban#^7]")},

        {"unflag", G_admin_flag, false, "flag", N_("clears an admin flag from a player. "
                                                   "console can use this command on admin levels by prefacing a '*' to "
                                                   "the admin level value."),
         N_("[^3name|slot#|admin#|*level#^7] (^5+^7|^5-^7)[^3flag^7]")},

        {"unlock", G_admin_lock, false, "lock", N_("unlock a locked team"), N_("[^3a|h^7]")},

        {"unmute", G_admin_mute, false, "mute", N_("unmute a muted player"), N_("[^3name|slot#^7]")},

        {"unregister", G_admin_unregister, false, "unregister", N_("unregister your name so that it can be used by other players."), ""},

        {"warn", G_admin_warn, false, "warn", N_("warn a player about his behaviour"), N_("[^3name|slot#^7] [^3reason^7]")}};
...
#define G_ADMIN_NAME(ent)                                    \
    (ent->client->pers.admin ? ent->client->pers.admin->name \
...
void G_admin_cmdlist(gentity_t* ent) {
    ...
    for (i = 0; i < adminNumCmds; i++) {
        if (!g_admin_cmds[i].keyword ||
            !G_admin_permission(ent, g_admin_cmds[i].flag)) {
            continue;
        }
...
void G_admin_writeconfig() {
    ...
    for (a = g_admin_admins; a; a = a->next) {
        ...
        admin_writeconfig_int(a->lastSeen.tm_year * 10000 +
                                      a->lastSeen.tm_mon * 100 + a->lastSeen.tm_mday,
                              f);
        trap_FS_Write("\n", 1, f);
    }
...
static void admin_default_levels() {
    ...
    Q_strncpyz(l->flags, "listplayers admintest adminhelp time register", sizeof(l->flags));
    ...
    Q_strncpyz(l->flags,
               "listplayers admintest adminhelp time putteam spec999 register "
               "unregister",
               sizeof(l->flags));
    ...
    Q_strncpyz(l->flags,
               "listplayers admintest adminhelp time putteam spec999 warn kick "
               "mute ADMINCHAT "
...
static void admin_log_start(gentity_t* admin, const char* cmd) {
    ...
    adminLogLen = Q_snprintf(
            adminLog, sizeof(adminLog), "%d \"%s" S_COLOR_WHITE "\" \"%s" S_COLOR_WHITE "\" [%d] (%s): %s", admin ? admin->s.clientNum : -1, name, admin && admin->client->pers.admin ? admin->client->pers.admin->name : name, admin && admin->client->pers.admin ? admin->client->pers.admin->level : 0, admin ? admin->client->pers.guid : "", cmd);
}

static void admin_log(const char* str) {
    if (adminLog[0]) {
        adminLogLen += Q_snprintf(adminLog + adminLogLen,
                                  sizeof(adminLog) - adminLogLen,
                                  ": %s",
                                  str);
    }
}
...
static int admin_search(gentity_t* ent, const char* cmd, const char* noun, bool (*match)(void*, const void*), int (*out)(void*, char*), const void* list, const void* arg, const char* arglist, int start, const int offset, const int limit) {
    int i;
    int count = 0;
    int found = 0;
    int total;
    ...
    if (limit > 0) {
        ADMBP(va("^3%s: ^7showing %d of %d %s %d-%d%s%s.", cmd, count, found, noun, start + offset, end + offset, (arglist && *arglist) ? " matching " : "", arglist));

        if (next) {
            ADMBP(va("  use '%s%s%s %d' to see more", cmd, arglist ? " " : "", arglist, next + offset));
        }
    }
...
static int admin_out(void* admin, char* str) {
    ...
    for (i = 0; l && l->name[i]; i++) {
        if (Q_IsColorString(l->name + i)) {
            lncol += 2;
        } else if (l->name[i] == Q_COLOR_ESCAPE &&
                   l->name[i + 1] == Q_COLOR_ESCAPE) {
            lncol += 1;
        }
    }
    ...
    Com_sprintf(str, MAX_STRING_CHARS, "%-6d %*s^7 %s %s", a->level, admin_level_maxname + lncol, l ? l->name : "(null)", lastSeen, a->name);

    return 0;
}

static int admin_listadmins(gentity_t* ent, int start, char* search) {
    return admin_search(ent, "listadmins", "admins", admin_match, admin_out, g_admin_admins, search, search, start, MAX_CLIENTS, MAX_ADMIN_LISTITEMS);
}

static int admin_find_admin(gentity_t* ent, char* name, const char* command, gentity_t** client, g_admin_admin_t** admin) {
    ...
    if (numeric) {
        ...
        if (id >= 0 && id < level.maxclients) {
            ...
            if (!vic || !vic->client ||
                ...
                ADMP(va("%s %s %d",
                        QQ(N_("^3$1$: ^7no player connected in slot $2$\n")),
                        command,
                        id));
                return -1;
            }
        ...
        if (!vic && !a) {
            ...
            ADMP(va("%s %s %s %d %d %d",
                    QQ(N_("^3$1$: ^7$2$ not in range 0–$3$ or $4$–$5$\n")),
                    Quote(command),
                    Quote(name),
                    level.maxclients - 1,
                    MAX_CLIENTS,
        ...
        for (wa = g_admin_admins, i = MAX_CLIENTS; wa && matches < 2;
             wa = wa->next, i++) {
        ...
        if (matches == 0) {
            ADMP(va("%s %s",
                    QQ(
                            N_("^3$1$:^7 no match, use listplayers or listadmins to "
        ...
        if (matches > 1) {
            ADMP(va(
                    "%s %s",
                    QQ(N_(
                            "^3$1$:^7 more than one match, use the admin number instead:\n")),
                    command));
...
static void G_admin_ban_message(gentity_t* ent, g_admin_ban_t* ban, char* creason, int clen, char* areason, int alen) {
    if (creason) {
        ...
        G_admin_duration(ban->expires - Com_GMTime(nullptr), time, sizeof(time), duration, sizeof(duration));
        // part of this might get cut off on the connect screen
        Com_sprintf(creason, clen, "You have been banned by %s" S_COLOR_WHITE " duration: %s%s"
                                   " reason: %s",
                    ban->banner,
                    time,
                    duration,
    ...
    if (areason && ent) {
        Com_sprintf(areason, alen, S_COLOR_YELLOW "Banned player %s" S_COLOR_YELLOW " tried to connect from %s (ban #%d)", G_user_name(ent, ban->name), ent->client->pers.ip.str, ban->id);
    }
}
...
bool G_admin_ban_check(gentity_t* ent, char* reason, int rlen) {
    ...
    while ((ban = G_admin_match_ban(ent, ban))) {
        ...
        // and don't fill the console
            ...
            trap_Print(va("%s%s\n", warningMessage, ban->warnCount + 1 == 10 ? S_COLOR_WHITE " — future messages for this ban will be suppressed" : ""));
        }
...
static void llsort(struct llist** head,
...
// Uncomment this if your server lags (shouldn't happen unless you are on a
// *very* old computer)
// Will only regenrate messages when there are no active client (When they are
// all loading the map)
#if 0
	int             i;
    ...
    if (highest) {
        if (trap_RSA_GenerateMessage(highest->pubkey, highest->msg, highest->msg2)) {
            ...
            // so we clear the pubkey and ask the client for a new one when he
            // reconnects
            highest->pubkey[0] = '\0';
            highest->msg[0] = '\0';
            highest->msg2[0] = '\0';
...
bool G_admin_readconfig(gentity_t* ent) {
    ...
    if (!g_admin.string[0]) {
        ADMP(QQ(
                N_("^3readconfig: g_admin is not set, not loading configuration "
                   "from a file\n")));
        return false;
    }
    ...
    *(cnf1 + len) = '\0';
    ...
    while (1) {
        ...
        if (!Q_stricmp(t, "[level]")) {
            if (l) {
                ...
                l = g_admin_levels =
                        (g_admin_level_t*) BG_Alloc(sizeof(g_admin_level_t));
            }
            ...
            if (a) {
                ...
                a = g_admin_admins =
                        (g_admin_admin_t*) BG_Alloc(sizeof(g_admin_admin_t));
            }
            ...
            if (c) {
                ...
                c = g_admin_commands =
                        (g_admin_command_t*) BG_Alloc(sizeof(g_admin_command_t));
            }
    ...
    BG_Free(cnf2);
    ADMP(va("%s %d %d %d %d", QQ(N_("^3readconfig: ^7loaded $1$ levels, $2$ "
                                    "admins, $3$ bans, $4$ commands\n")),
            lc,
            ac,
            bc,
            cc));
    ...
    for (i = 0; i < level.maxclients; i++) {
        if (level.clients[i].pers.connected != CON_DISCONNECTED) {
            level.clients[i].pers.admin =
                    level.clients[i].pers.pubkey_authenticated
    ...
    switch (trap_Argc()) {
        case 1:
            ...
            if (level.matchTime < timelimit) {
                ...
                ADMP(va("%s %02i %02i %02i %02i %02i %i %02i",
                        QQ(
                                N_("^3time: ^7time is ^d$1$:$2$:$3$^7 UTC; game time is "
                                   "^d$4$:$5$^7, reaching time limit in ^d$6$:$7$^7\n")),
                        qt.tm_hour,
                        qt.tm_min,
                        qt.tm_sec,
                        gameMinutes,
                        ...
                        remainingMinutes,
                        remainingSeconds));
            } else // requesting time in intermission after the timelimit hit, or
                   // timelimit wasn't set
            {
                ADMP(va("%s %02i %02i %02i %02i %02i",
                        QQ(
                                N_("^3time: ^7time is ^d$1$:$2$:$3$^7 UTC; game time is "
                                   "^d$4$:$5$^7\n")),
                        qt.tm_hour,
        ...
        case 2:
            ...
            if (isdigit(tstr[0])) {
                ...
                if (timelimit != level.timelimit) {
                    AP(va("print_tr %s %d %d %s",
                          QQ(N_("^3time: ^7time limit set to $1$m from $2$m by $3$\n")),
                          timelimit,
                          level.timelimit,
                          G_quoted_admin_name(ent)));
                    level.timelimit = timelimit;
                    // reset 'time remaining' warnings
                    level.timelimitWarning =
                            (level.matchTime < (level.timelimit - 5) * 60000)
                                    ? TW_NOT
...
bool G_admin_setlevel(gentity_t* ent) {
    ...
    if (ent &&
        ...
        ADMP(QQ(
                N_("^3setlevel: ^7you may not use setlevel to set a level higher "
    ...
    if (ent && !admin_higher_admin(ent->client->pers.admin, a)) {
        ADMP(va("%s %s",
                QQ(
                        N_("^3$1$: ^7sorry, but your intended victim has a higher admin"
                           " level than you\n")),
                "setlevel"));
        return false;
    }
    ...
    AP(va("print_tr %s %s %d %s",
          QQ(N_("^3setlevel: ^7$1$^7 was given level $2$ admin rights by $3$\n")),
          Quote(a->name),
          a->level,
          G_quoted_admin_name(ent)));
...
static g_admin_ban_t* admin_create_ban_entry(gentity_t* ent, char* netname, char* guid, addr_t* ip, int seconds, const char* reason) {
    g_admin_ban_t* b = nullptr;
    ...
    Com_sprintf(b->made, sizeof(b->made), "%04i-%02i-%02i %02i:%02i:%02i", 1900 + qt.tm_year, qt.tm_mon + 1, qt.tm_mday, qt.tm_hour, qt.tm_min, qt.tm_sec);
...
static void admin_create_ban(gentity_t* ent, char* netname, char* guid, addr_t* ip, int seconds, const char* reason) {
    int i;
    char disconnect[MAX_STRING_CHARS];
    g_admin_ban_t* b =
            admin_create_ban_entry(ent, netname, guid, ip, seconds, (reason && *reason) ? reason : "banned by admin");
    ...
    for (i = 0; i < level.maxclients; i++) {
        ...
        if (G_admin_ban_matches(b, &g_entities[i])) {
            ...
            trap_DropClient(
                    i, va("has been kicked by %s^7. reason: %s", b->banner, b->reason));
        }
    }
}
...
bool G_admin_kick(gentity_t* ent) {
    ...
    admin_log(va("%d (%s) \"%s" S_COLOR_WHITE "\": \"%s" S_COLOR_WHITE "\"", pid, vic->client->pers.guid, vic->client->pers.netname, reason));
    time = G_admin_parse_time(g_adminTempBan.string);
    admin_create_ban(ent, vic->client->pers.netname, vic->client->pers.guid, &vic->client->pers.ip, MAX(1, time), (*reason) ? reason : "kicked by admin");
    G_admin_writeconfig();
...
bool G_admin_ban(gentity_t* ent) {
    ...
    if (trap_Argc() < 2) {
        ADMP(QQ(
                N_("^3ban: ^7usage: ban [name|slot|address(/mask)] [duration] "
                   "[reason]\n")));
        return false;
    }
    ...
    if (G_AddressParse(search, &ip)) {
        ...
        if (ip.mask < min || ip.mask > max) {
            ADMP(va("%s %d %d %d",
                    QQ(N_("^3ban: ^7invalid netmask ($1$ is not one of $2$–$3$)\n")),
                    ip.mask,
                    min,
                    max));
            return false;
    ...
    G_admin_duration((seconds) ? seconds : -1, time, sizeof(time), duration, sizeof(duration));

    AP(va("print_tr %s %s %s %s %s %s",
          QQ(
                  N_("^3ban:^7 $1$^7 has been banned by $2$^7; "
    ...
    if (ipmatch) {
        admin_create_ban(ent, match->slot == -1 ? match->name[match->nameOffset] : level.clients[match->slot].pers.netname, match->guid, &ip, seconds, reason);
        admin_log(va("[%s]", ip.str));
    } else {
        // ban all IP addresses used by this player
        for (i = 0; i < MAX_NAMELOG_ADDRS && match->ip[i].str[0]; i++) {
            admin_create_ban(ent, match->slot == -1 ? match->name[match->nameOffset] : level.clients[match->slot].pers.netname, match->guid, &match->ip[i], seconds, reason);
    ...
    if (!g_admin.string[0]) {
        ADMP(
                QQ(N_("^3ban: ^7WARNING g_admin not set, not saving ban to a file\n")));
    } else {
        G_admin_writeconfig();
    }
...
bool G_admin_unban(gentity_t* ent) {
    ...
    for (ban = p = g_admin_bans; ban && ban->id != bnum;
         p = ban, ban = ban->next) {
    }
    ...
    if (!G_admin_permission(ent, ADMF_CAN_PERM_BAN)) {
        ...
        if (ban->expires == 0 ||
            (maximum = G_admin_parse_time(g_adminMaxBan.string),
             ban->expires - time > MAX(1, maximum))) {
    ...
    if (expireOnly && G_ADMIN_BAN_EXPIRED(ban, time)) {
        ADMP(
                va("%s %d", QQ(N_("^3unban: ^7ban #$1$ has already expired\n")), bnum));
        return false;
    }
    ...
    admin_log(va("%d (%s) \"%s" S_COLOR_WHITE "\": \"%s" S_COLOR_WHITE "\": [%s]",
                 ban->expires ? ban->expires - time : 0,
                 ban->guid,
                 ban->name,
                 ban->reason,
                 ban->ip.str));
    ...
    if (expireOnly) {
        AP(va("print_tr %s %d %s %s",
              QQ(N_("^3unban: ^7ban #$1$ for $2$^7 has been expired by $3$\n")),
              bnum,
              Quote(ban->name),
              G_quoted_admin_name(ent)));
        ...
        AP(va("print_tr %s %d %s %s",
              QQ(N_("^3unban: ^7ban #$1$ for $2$^7 has been removed by $3$\n")),
              bnum,
              Quote(ban->name),
              G_quoted_admin_name(ent)));
...
bool G_admin_adjustban(gentity_t* ent) {
    ...
    if (trap_Argc() < 3) {
        ADMP(QQ(
                N_("^3adjustban: ^7usage: adjustban [ban#] [/mask] [duration] [reason]"
                   "\n")));
        return false;
    }
    ...
    if (secs[0] == '/') {
        ...
        if (mask < min || mask > max) {
            ADMP(va(
                    "%s %d %d %d",
                    QQ(N_(
                            "^3adjustban: ^7invalid netmask ($1$ is not one of $2$–$3$)\n")),
                    mask,
                    min,
                    max));
            return false;
    ...
    if (length < 0) {
        ...
        ban->expires = expires;
        G_admin_duration((expires) ? expires - time : -1, seconds, sizeof(seconds), duration, sizeof(duration));
    }
    ...
    admin_log(va("%d (%s) \"%s" S_COLOR_WHITE "\": \"%s" S_COLOR_WHITE "\": [%s]",
                 ...
                 ban->reason,
                 ban->ip.str));
    AP(va("print_tr %s %d %s %s %s %s %s %s %s %s %s",
          QQ(
                  N_("^3adjustban: ^7ban #$1$ for $2$^7 has been updated by $3$^7 "
                     "$4t$$5$$6$$7t$$8$$9t$$10$\n")),
          bnum,
          Quote(ban->name),
          G_quoted_admin_name(ent),
...
bool G_admin_putteam(gentity_t* ent) {
    ...
    AP(va("print_tr %s %s %s %s",
          QQ(N_("^3putteam: ^7$1$^7 put $2$^7 on to the $3$ team\n")),
          G_quoted_admin_name(ent),
...
bool G_admin_speclock(gentity_t* ent) {
    int pid, lockTime = 0;
    char name[MAX_NAME_LENGTH], lock[MAX_STRING_CHARS], err[MAX_STRING_CHARS],
            duration[MAX_DURATION_LENGTH], time[MAX_DURATION_LENGTH];
    ...
    admin_log(va("%d (%s) \"%s" S_COLOR_WHITE "\" SPECTATE %s%s", pid, vic->client->pers.guid, vic->client->pers.netname, time, duration));

    if (vic->client->pers.team != TEAM_NONE) {
        G_ChangeTeam(vic, TEAM_NONE);
        AP(va("print_tr %s %s %s %s %s",
              QQ(
                      N_("^3speclock: ^7$1$^7 put $2$^7 on to the spectators team and "
                         "blocked team-change for $3$$4t$\n")),
              G_quoted_admin_name(ent),
              Quote(vic->client->pers.netname),
              Quote(time),
              duration));
        ...
        AP(va(
                "print_tr %s %s %s %s %s",
                QQ(N_(
                        "^3speclock: ^7$1$^7 blocked team-change for $2$^7 for $3$$4t$\n")),
                G_quoted_admin_name(ent),
                Quote(vic->client->pers.netname),
                Quote(time),
                duration));
...
bool G_admin_specunlock(gentity_t* ent) {
    ...
    if (spec) {
        spec->expires = 0;
        admin_log(va("%d (%s) \"%s" S_COLOR_WHITE "\" SPECTATE -", pid, vic->client->pers.guid, vic->client->pers.netname));
        AP(va("print_tr %s %s %s",
              QQ(N_("^3specunlock: ^7$1$^7 unblocked team-change for $2$\n")),
              G_quoted_admin_name(ent),
              Quote(vic->client->pers.netname)));
    }
...
bool G_admin_changemap(gentity_t* ent) {
    ...
    if (trap_Argc() > 2) {
        ...
        if (!Q_stricmp(layout, S_BUILTIN_LAYOUT) ||
            trap_FS_FOpenFile(va("layouts/%s/%s.dat", map, layout), nullptr, FS_READ) > 0) {
            // nothing to do
        } else {
    ...
    G_MapLog_Result('M');
    AP(va("print_tr %s %s %s %s %s %s",
          QQ(N_("^3changemap: ^7map '$1$' started by $2$^7 $3t$$4$$5$\n")),
          Quote(map),
          G_quoted_admin_name(ent),
...
bool G_admin_warn(gentity_t* ent) {
    ...
    if ((found = G_ClientNumbersFromString(name, pids, MAX_CLIENTS)) != 1) {
        ...
        ADMP(va("%s %s %s %s %s", QQ("^3$1$: ^7$2t$\n $3$$4$"), "warn", (count > 0) ? Quote(Substring(err, 0, count)) : Quote(err), (count > 0) ? Quote(Substring(err, count + 1, strlen(err))) : "", (count > 0) ? "\n" : ""));
        return false;
    }
    ...
    // create a ban list entry, set warnCount to -1 to indicate that this should
    // NOT result in denying connection
    if (ent && !ent->client->pers.localClient) {
        int time = G_admin_parse_time(g_adminWarn.string);
        admin_create_ban_entry(ent, vic->client->pers.netname, vic->client->pers.guid, &vic->client->pers.ip, MAX(1, time), (*reason) ? reason : "warned by admin")
                ->warnCount = -1;
    ...
    CPx(pids[0],
        va("cp_tr " QQ(
                   N_("^1You have been warned by an administrator:\n^3$1$")) " %s",
           Quote(reason)));
    AP(va("print_tr %s %s %s %s",
          QQ(N_("^3warn: ^7$1$^7 has been warned: '$2$' by $3$\n")),
          Quote(vic->client->pers.netname),
          Quote(reason),
          G_quoted_admin_name(ent)));
...
bool G_admin_mute(gentity_t* ent) {
    ...
    if (ent &&
        !admin_higher_admin(ent->client->pers.admin, G_admin_admin(vic->guid))) {
        ADMP(va("%s %s",
                QQ(
                        N_("^3$1$: ^7sorry, but your intended victim has a higher admin"
                           " level than you\n")),
    ...
    if (vic->muted) {
        ...
        AP(va("print_tr %s %s %s",
              QQ(N_("^3unmute: ^7$1$^7 has been unmuted by $2$\n")),
        ...
        AP(va("print_tr %s %s %s",
              QQ(N_("^3mute: ^7$1$^7 has been muted by $2$\n")),
              Quote(vic->name[vic->nameOffset]),
              G_quoted_admin_name(ent)));
    }
...
bool G_admin_denybuild(gentity_t* ent) {
    ...
    if (vic->denyBuild) {
        ...
        AP(va(
                "print_tr %s %s %s",
                QQ(N_("^3allowbuild: ^7building rights for ^7$1$^7 restored by $2$\n")),
                Quote(vic->name[vic->nameOffset]),
                G_quoted_admin_name(ent)));
    } else {
        ...
        AP(va("print_tr %s %s %s",
              QQ(N_("^3denybuild: ^7building rights for ^7$1$^7 revoked by $2$\n")),
              Quote(vic->name[vic->nameOffset]),
              G_quoted_admin_name(ent)));
    }
...
static bool admin_match_inactive(void* admin, const void* match) {
    g_admin_admin_t* a = (g_admin_admin_t*) admin;
    unsigned int date = a->lastSeen.tm_year * 10000 + a->lastSeen.tm_mon * 100 +
                        a->lastSeen.tm_mday;
...
bool G_admin_listinactive(gentity_t* ent) {
    ...
    if (i > 3) {
        ADMP(QQ(
                N_("^3listinactive: ^7usage: listinactive [^5months^7] (^5start "
                   "admin#^7)\n")));
        return false;
    }
    ...
    admin_search(ent, "listinactive", "admins", admin_match_inactive, admin_out, g_admin_admins, &date, s, start, MAX_CLIENTS, MAX_ADMIN_LISTITEMS);
...
bool G_admin_listplayers(gentity_t* ent) {
    ...
    bool canseeWarn =
            G_admin_permission(ent, "warn") || G_admin_permission(ent, "ban");
    ...
    for (i = 0; i < level.maxclients; i++) {
        ...
        ADMBP(va("%2i ^%c%c^7 %-2i^2%c^7 %*s^7 ^5%c^1%c%c%s^7 %s^7 %s%s%s %s\n", i, c, t, l ? l->level : 0, hint ? '*' : ' ', admin_level_maxname + colorlen, lname, bot, muted, denied, canseeWarn ? (p->pers.hasWarnings ? S_COLOR_YELLOW "W" : " ") : "", p->pers.netname, (registeredname) ? "(a.k.a. " : "", (registeredname) ? registeredname : "", (registeredname) ? S_COLOR_WHITE ")" : "", (!authed) ? S_COLOR_RED "NOT AUTHED" : ""));
    }
...
static int ban_out(void* ban, char* str) {
    ...
    for (i = 0; b->name[i]; i++) {
        if (Q_IsColorString(&b->name[i])) {
            colorlen1 += 2;
        } else if (b->name[i] == Q_COLOR_ESCAPE &&
                   b->name[i + 1] == Q_COLOR_ESCAPE) {
            colorlen1 += 1;
        }
    }
    ...
    if (!b->expires || b->expires - t > 0) {
        G_admin_duration(b->expires ? b->expires - t : -1, time, sizeof(time), duration, sizeof(duration));
    } else {
        *time = 0;
...
bool G_admin_showbans(gentity_t* ent) {
    ...
    admin_search(ent, "showbans", "bans", ipmatch ? ban_matchip : ban_matchname, ban_out, g_admin_bans, ipmatch ? (const char*) &ip : (const char*) name_match, ipmatch ? (const char*) &ip : (const char*) name_match, start, 1, MAX_ADMIN_SHOWBANS);
    return true;
}
...
bool G_admin_adminhelp(gentity_t* ent) {
    ...
    if (trap_Argc() < 2) {
        ...
        perline = 78 / (width + 1); // allow for curses console border and at least
        ...
        ADMP(va("%s %d", QQ(N_("^3adminhelp: ^7$1$ available commands\n"
                               "run adminhelp [^3command^7] for adminhelp with a "
                               "specific command.\n")),
                count));
        ...
        ADMP(va(
                "%s %s",
                denied
                        ? QQ(N_("^3adminhelp: ^7you do not have permission to use '$1$'\n"))
...
bool G_admin_admintest(gentity_t* ent) {
    ...
    l = G_admin_level(ent->client->pers.admin ? ent->client->pers.admin->level
                                              : 0);

    AP(va("print_tr %s %s %d %s %s %s",
          QQ(N_("^3admintest: ^7$1$^7 is a level $2$ admin $3$$4$^7$5$\n")),
...
bool G_admin_allready(gentity_t* ent) {
    ...
    if (!level.intermissiontime) {
        ADMP(QQ(
                N_("^3allready: ^7this command is only valid during intermission\n")));
        return false;
    }
    ...
    AP(va("print_tr %s %s",
          QQ(N_("^3allready:^7 $1$^7 says everyone is READY now\n")),
          G_quoted_admin_name(ent)));
    return true;
}
...
bool G_admin_endvote(gentity_t* ent) {
    ...
    msg = va("print_tr %s %s %s",
             cancel ? QQ(N_("^3$1$: ^7$2$^7 decided that everyone voted No\n"))
...
bool G_admin_spec999(gentity_t* ent) {
    ...
    for (i = 0; i < level.maxclients; i++) {
        ...
        if (vic->client->ps.ping == 999) {
            G_ChangeTeam(vic, TEAM_NONE);
            AP(va("print_tr %s %s %s",
                  QQ(N_("^3spec999: ^7$1$^7 moved $2$^7 to spectators\n")),
                  G_quoted_admin_name(ent),
                  Quote(vic->client->pers.netname)));
        }
    }
...
bool G_admin_rename(gentity_t* ent) {
    ...
    if (victim->client->pers.connected != CON_CONNECTED) {
        ADMP(QQ(N_(
                "^3rename: ^7sorry, but your intended victim is still connecting\n")));
        return false;
    }
    ...
    AP(va("print_tr %s %s %s %s",
          QQ(N_("^3rename: ^7$1$^7 has been renamed to $2$^7 by $3$\n")),
...
bool G_admin_restart(gentity_t* ent) {
    ...
    if (!builtin &&
        !trap_FS_FOpenFile(va("layouts/%s/%s.dat", map, layout), nullptr, FS_READ)) {
    ...
    if (!Q_stricmp(teampref, "keepteams") ||
        !Q_stricmp(teampref, "keepteamslock") || !Q_stricmp(teampref, "kt") ||
        !Q_stricmp(teampref, "ktl")) {
        for (i = 0; i < level.maxclients; i++) {
            cl = level.clients + i;
        ...
        trap_Cvar_Set("g_mapRestarted", "yk");
               ...
               !Q_stricmp(teampref, "st") || !Q_stricmp(teampref, "stl")) {
        for (i = 0; i < level.maxclients; i++) {
            cl = level.clients + i;
    ...
    if (!Q_stricmp(teampref, "switchteamslock") ||
        !Q_stricmp(teampref, "keepteamslock") || !Q_stricmp(teampref, "ktl") ||
        !Q_stricmp(teampref, "stl")) {
    ...
    AP(va("print_tr %s %s %s %s %s %s %s %s",
          QQ(N_("^3restart: ^7map restarted by $1$ $2$$3t$$4$$5$$6t$$7$\n")),
          G_quoted_admin_name(ent),
...
bool G_admin_nextmap(gentity_t* ent) {
    ...
    AP(va("print_tr %s %s",
          QQ(N_("^3nextmap: ^7$1$^7 decided to load the next map\n")),
          G_quoted_admin_name(ent)));
...
bool G_admin_namelog(gentity_t* ent) {
    ...
    admin_search(ent, "namelog", "recent players", ipmatch ? namelog_matchip : namelog_matchname, namelog_out, level.namelogs, ipmatch ? (const char*) &ip : s2, ipmatch ? (const char*) &ip : s2, start, MAX_CLIENTS, MAX_ADMIN_LISTITEMS);
    return true;
}
...
namelog_t* G_NamelogFromString(gentity_t* ent, char* s) {
    ...
    if (found > 1) {
        admin_search(ent, "namelog", "recent players", namelog_matchname, namelog_out, level.namelogs, s2, s2, 0, MAX_CLIENTS, -1);
    }
...
bool G_admin_lock(gentity_t* ent) {
    ...
    if (fail) {
        ADMP(va("%s %s %s",
                lock ? QQ(N_("^3$1$: ^7the $2$ team is already locked\n"))
    ...
    if (lock) {
        AP(va("print_tr %s %s %s %s",
              QQ(N_("^3$1$: ^7the $2$ team has been locked by $3$\n")),
              command,
              BG_TeamName(team),
              G_quoted_admin_name(ent)));
    } else {
        AP(va("print_tr %s %s %s %s",
              QQ(N_("^3$1$: ^7the $2$ team has been unlocked by $3$\n")),
              command,
              BG_TeamName(team),
              G_quoted_admin_name(ent)));
    }
...
const char* G_admin_flag_update(char* newflags, char* oldflags, int size, const char* flag, bool add, bool permission) {
    ...
    if (add) {
        ...
        Com_sprintf(flags[count], sizeof(flags[count]), "%c%s", (permission) ? '+' : '-', flag);
        count++;
    } else if (!found) {
    ...
    for (i = 0; i < count; i++) {
        Q_strcat(newflags, size, va("%s%s", (i) ? " " : "", flags[i]));
    }
...
bool G_admin_flaglist(gentity_t* ent) {
    ...
    for (i = 0; i < adminNumFlags; i++) {
        ADMBP(va("  ^5%-20s ^7%s\n", g_admin_flags[i].flag, g_admin_flags[i].description));
    }
    ...
    ADMBP_end();
    ADMP(va("%s %d %d",
            QQ(N_("^3flaglist: ^7listed $1$ ability and $2$ command flags\n")),
            (int) adminNumFlags,
            count));
...
bool G_admin_flag(gentity_t* ent) {
    ...
    if (trap_Argc() < 2) {
        ADMP(va("%s %s", QQ(N_("^3$1$: ^7usage: $1$ [^3name|slot#|admin#|*level#^7] "
                               "(^5+^7|^5-^7)[^3flag^7]\n")),
                command));
        return false;
    }
    ...
    if (name[0] == '*') {
        if (ent) {
            ADMP(va("%s %s",
                    QQ(N_("^3$1$: only console can change admin level flags\n")),
                    command));
            return false;
        }
        ...
        if (!admin || admin->level == 0) {
            ADMP(va("%s %s",
                    QQ(N_("^3$1$:^7 your intended victim is not an admin\n")),
                    command));
            return false;
        }

        if (ent && !admin_higher_admin(ent->client->pers.admin, admin)) {
            ADMP(va("%s %s", QQ(N_("^3$1$:^7 your intended victim has a higher "
    ...
    if (trap_Argc() < 3) {
        ...
        if (level) {
            ADMP(va("%s %s %d %s",
                    QQ(N_("^3$1$:^7 admin level $2$ flags are '$3$'\n")),
                    command,
                    level->level,
                    Quote(level->flags)));
        }
    ...
    if (ent && ent->client->pers.admin == admin) {
        ADMP(va("%s %s",
                QQ(N_("^3$1$:^7 you may not change your own flags (use rcon)\n")),
                command));
        return false;
    }
    ...
    if (!G_admin_permission(ent, flag)) {
        ADMP(va("%s %s",
                QQ(N_("^3$1$:^7 you may only change flags that you also have\n")),
                command));
        return false;
    }
    ...
    if (result) {
        const char* msg = add ? QQ(
                                        N_("^3$1$: ^7an error occurred when setting "
                                           "flag '^3$2$^7' for $3$^7, $4t$\n"))
                              : QQ(
                                        N_("^3$1$: ^7an error occurred when clearing "
                                           "flag '^3$2$^7' for $3$^7, $4t$\n"));
    ...
    if (!G_admin_permission(ent, ADMF_ADMINCHAT)) {
        const char* msg[] = {QQ(N_("^3$1$: ^7flag '$2$' allowed for $3$\n")),
                             QQ(N_("^3$1$: ^7flag '$2$' cleared for $3$\n")),
                             QQ(N_("^3$1$: ^7flag '$2$' denied for $3$\n"))};
        ADMP(
                va("%s %s %s %s", msg[action], command, Quote(flag), Quote(adminname)));
    }

    {
        const char* msg[] = {"admin flag '%s' allowed for %s",
...
bool G_admin_builder(gentity_t* ent) {
    ...
    if (tr.fraction < 1.0f && (traceEnt->s.eType == ET_BUILDABLE)) {
        ...
        if (!buildlog && ent->client->pers.team != TEAM_NONE &&
        ...
        if (buildlog) {
            ...
            for (i = 0; i < level.numBuildLogs; i++) {
                ...
                if (log->fate == BF_CONSTRUCT &&
                    traceEnt->s.modelindex == log->modelindex) {
        ...
        if (buildlog && traceEnt->builtBy && i < level.numBuildLogs) {
            ADMP(va("%s %s %s %d",
                    QQ(N_("^3builder: ^7$1$ built by $2$^7, buildlog #$3$\n")),
                    Quote(buildingName),
                    Quote(builder),
...
bool G_admin_pause(gentity_t* ent) {
    if (!level.pausedTime) {
        ...
        if (level.pausedTime < 1000) {
            ADMP(QQ(N_(
                    "^3pause: ^7Unpausing so soon assumed accidental and ignored.\n")));
            return false;
        }

        AP(va("print_tr %s %s %d",
              QQ(N_("^3pause: ^7$1$^7 unpaused the game (paused for $2$ sec)\n")),
              G_quoted_admin_name(ent),
...
static const char* const fates[] = {
        N_("^2built^7"), N_("^3deconstructed^7"), N_("^7replaced^7"), N_("^5destroyed^7"), N_("^1TEAMKILLED^7"), N_("^7unpowered^7"), N_("removed")};
bool G_admin_buildlog(gentity_t* ent) {
    ...
    if (ent && ent->client->pers.team != TEAM_NONE) {
        if (team == TEAM_NONE) {
            trap_SendServerCommand(
                    -1, va("print_tr %s %s", QQ(N_("^3buildlog: ^7$1$^7 requested a log "
                                                   "of recent building activity\n")),
                           Quote(ent->client->pers.netname)));
        } else {
            // FIXME? Send only to team-mates
            trap_SendServerCommand(
                    -1, va("print_tr %s %s %s", QQ(N_("^3buildlog: ^7$1$^7 requested a log "
                                                      "of recent $2$ building activity\n")),
                           Quote(ent->client->pers.netname),
                           Quote(BG_TeamName(team))));
        }
    }
    ...
    ADMP(va(
            "%s %d %d %d %d %d %s",
            QQ(N_("^3buildlog: ^7showing $1$ build logs $2$–$3$ of $4$–$5$.  $6$\n")),
            printed,
            start + MAX_CLIENTS,
            i + MAX_CLIENTS - 1,
...
bool G_admin_revert(gentity_t* ent) {
    ...
    if (!log->actor || log->fate == BF_REPLACE || log->fate == BF_UNPOWER) {
        ...
        ADMP(QQ(
                N_("^3revert: ^7you can only revert direct player actions, "
                   "indicated by ^2* ^7in buildlog\n")));
        return false;
    }

    G_admin_duration((level.time - log->time) / 1000, time, sizeof(time), duration, sizeof(duration));
    admin_log(arg);
    AP(va("print_tr %s %s %d %s %s",
          (level.buildId - id) > 1 ? QQ(
                                             N_("^3revert: ^7$1$^7 reverted $2$ "
                                                "changes over the past $3$ $4t$\n"))
                                   : QQ(
                                             N_("^3revert: ^7$1$^7 reverted $2$ "
                                                "change over the past $3$ $4t$\n")),
          G_quoted_admin_name(ent),
          level.buildId - id,
          time,
          duration));
...
bool G_admin_l0(gentity_t* ent) {
    ...
    AP(va("print_tr %s %s %s",
          QQ(N_("^3l0: ^7name protection for $1$^7 removed by $2$\n")),
...
bool G_admin_l1(gentity_t* ent) {
    ...
    AP(va("print_tr %s %s %s",
          QQ(N_("^3l1: ^7name protection for $1$^7 enabled by $2$\n")),
          G_quoted_user_name(vic, a->name),
          G_quoted_admin_name(ent)));
    return true;
}
...
bool G_admin_register(gentity_t* ent) {
    ...
    trap_SendConsoleCommand(
            va("setlevel %d %d;", (int) (ent - g_entities), level));

    AP(va("print_tr %s %s",
          QQ(N_("^3register: ^7$1$^7 is now a protected name\n")),
          Quote(ent->client->pers.netname)));
...
bool G_admin_unregister(gentity_t* ent) {
    ...
    trap_SendConsoleCommand(va("setlevel %d 0;", (int) (ent - g_entities)));

    AP(va("print_tr %s %s",
          QQ(N_("^3unregister: ^7$1$^7 is now an unprotected name\n")),
...
void G_admin_buffer_print(gentity_t* ent, const char* m) {
    ...
    if (!m || strlen(m) + strlen(g_bfb) >= 1009) {
        trap_SendServerCommand(ent ? ent->s.number : -2,
                               va("print %s", Quote(g_bfb)));
        g_bfb[0] = '\0';
    }
...
bool G_admin_bot(gentity_t* ent) {
    ...
    static const char bot_usage[] = QQ(
            N_("^3bot: ^7usage: bot add [^5name|*^7] [^5aliens|humans^7] (^5skill^7) "
               "(^5behavior^7)\n"
    ...
    if (!Q_stricmp(arg1, "add")) {
        ...
        if (!Q_stricmp(name, "all")) {
            G_BotDelAllBots();
                   ...
                   -1) // something went wrong when finding the client Number
        {
        ...
        G_ChangeTeam(&g_entities[clientNum],
                     g_entities[clientNum].botMind->botTeam);
        ...
        if (!Q_stricmp(name, "humans") || !Q_stricmp(name, "h")) {
            ...
            if (!G_BotClearNames()) {
                ADMP(QQ(
                        N_("some automatic bot names are in use – not clearing lists\n")));
                return false;
            }
