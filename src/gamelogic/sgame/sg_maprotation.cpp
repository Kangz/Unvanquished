...
typedef struct label_s { char name[MAX_QPATH]; } mrLabel_t;
...
bool G_MapExists(const char* name) {
    // Due to filesystem changes, this is no longer the correct way to check if a
    // map exists
...
static bool G_LabelExists(int rotation, const char* name) {
    ...
    for (i = 0; i < mr->numNodes; i++) {
        ...
        if (node->type == NT_LABEL && !Q_stricmp(name, node->u.label.name)) {
            return true;
        }

        if (node->type == NT_MAP && !Q_stricmp(name, node->u.map.name)) {
            return true;
        }
    }
...
static bool G_ParseNode(mrNode_t** node, char* token, const char** text_p, bool conditional) {
    if (!Q_stricmp(token, "if")) {
        ...
        *node = condition->target;
        ...
        (*node)->type = NT_RETURN;
    } else if (!Q_stricmp(token, "goto") || !Q_stricmp(token, "resume")) {
        mrLabel_t* label;
        ...
        if (!Q_stricmp(token, "{")) {
            if (mrNameSet) {
                ...
                if (!G_ParseMapRotation(
                            &mapRotations.rotations[mapRotations.numRotations], &text_p)) {
    ...
    for (i = 0; i < mapRotations.numRotations; i++) {
        ...
        for (j = 0; j < mr->numNodes; j++) {
            ...
            if (node->type == NT_MAP && strlen(node->u.map.postCommand) > 0) {
                G_Printf(MAP_CONTROL "    command: %s",
                         node->u.map.postCommand); // assume that there's an LF
                                                   // there... if not, well...
            }
        }
...
void G_PrintCurrentRotation(gentity_t* ent) {
    ...
    mapRotation_t* mapRotation = G_MapRotationActive()
                                         ? &mapRotations.rotations[mapRotationIndex]
                                         : nullptr;
    int i = 0;
    ...
    if (mapRotation == nullptr) {
        trap_SendServerCommand(ent - g_entities,
                               "print_tr \"" N_(
                                       "^3listrotation: ^7there is no active map "
                                       "rotation on this server\n") "\"");
        return;
    }
    ...
    if (mapRotation->numNodes == 0) {
        trap_SendServerCommand(ent - g_entities,
                               "print_tr \"" N_(
                                       "^3listrotation: ^7there are no maps in the "
                                       "active map rotation\n") "\"");
        return;
    }
    ...
    while ((node = mapRotation->nodes[i++])) {
        ...
        if (node->type == NT_MAP && !G_MapExists(node->u.map.name)) {
            colour = MAP_BAD;
        } else if (G_NodeIndexAfter(i - 1, mapRotationIndex) ==
                   G_CurrentNodeIndex(mapRotationIndex)) {
            currentMap = true;
...
static void G_PushRotationStack(int rotation) {
    ...
    Com_sprintf(text, sizeof(text), "%d %s", rotation, g_mapRotationStack.string);
...
static mrNode_t* G_NodeByIndex(int index, int rotation) {
    if (rotation >= 0 && rotation < mapRotations.numRotations && index >= 0 &&
        index < mapRotations.rotations[rotation].numNodes) {
...
===============
*/
static bool G_GotoLabel(int rotation, int nodeIndex, char* name, bool reset_index, int depth) {
    mrNode_t* node;
    int i;
    ...
    while (step) {
        ...
        switch (node->type) {
            ...
            case NT_RETURN:
                ...
                if (returnRotation >= 0) {
                    G_SetCurrentNodeByIndex(G_NodeIndexAfter(nodeIndex, rotation),
                                            rotation);

                    if (G_StartMapRotation(G_RotationNameByIndex(returnRotation), true, false, false, depth)) {
                        return false;
                    }
                }
            ...
            case NT_RESUME:
                ...
                if (G_GotoLabel(rotation, nodeIndex, node->u.label.name, (node->type == NT_GOTO), depth)) {
                    return false;
                }
...
bool G_MapRotationActive() {
    return (g_currentMapRotation.integer > NOT_ROTATING &&
            g_currentMapRotation.integer <= MAX_MAP_ROTATIONS);
}
