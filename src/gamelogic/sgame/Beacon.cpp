...
// otherEntityNum2 | owner        | beacon's owner (ENTITYNUM_NONE if none,
// ENTITYNUM_WORLD if auto)
// modelindex      | type         | beacon's type (BCT_*)
// modelindex2     | data         | additional data (e.g. buildable type for
// BCT_TAG)
...
namespace Beacon // this should eventually become a class
        {
/**
 * @brief A meaningless think function for beacons (everything is now handled in
 * Beacon::Frame).
 */
...
/**
 * @brief Handles beacon expiration and tag score decay. Called every server
 * frame.
 */
void Frame() {
 ...
 */
gentity_t* New(const vec3_t origin, beaconType_t type, int data, team_t team, int owner, beaconConflictHandler_t conflictHandler) {
    gentity_t* ent;
    int decayTime;
...
void Delete(gentity_t* ent, bool verbose) {
    ...
    if (verbose) {
        ...
        if (ent->tagAttachment)
            *ent->tagAttachment = nullptr;

        // BaseClustering::Remove will be called inside G_FreeEntity since we need
        // to be sure
...
void MoveTowardsRoom(vec3_t origin) {
    static const vec3_t vecs[162] = {{0.000000, 0.000000, -1.000000},
 ...
 */
gentity_t* FindSimilar(const vec3_t origin, beaconType_t type, int data, int team, int owner, float radius, int eFlags, int eFlagsRelevant) {
 ...
 */
int RemoveSimilar(const vec3_t origin, beaconType_t type, int data, int team, int owner, float radius, int eFlags, int eFlagsRelevant) {
    gentity_t* ent;
    int numRemoved = 0;
 ...
 */
gentity_t* MoveSimilar(const vec3_t from, const vec3_t to, beaconType_t type, int data, int team, int owner, float radius, int eFlags, int eFlagsRelevant) {
    gentity_t* ent;
...
/**
 * @brief Sets the entity fields so that the beacon is visible only by its team
 * and spectators.
 */
void Propagate(gentity_t* ent) {
    ...
    else if (ent->s.modelindex == BCT_TAG &&
             !(ent->s.eFlags & EF_BC_TAG_PLAYER)) {
    } else {
        int loMask, hiMask;
 ...
 *
 * Deletion is deferred for the enemy team's buildable tags until their death
 *was confirmed.
 */
...
/**
 * @brief Immediately deletes all tags attached to an entity (skips all
 * effects).
 */
...
static inline bool CheckRefreshTag(gentity_t* ent, team_t team) {
    gentity_t* existingTag =
            (team == TEAM_ALIENS) ? ent->alienTag : ent->humanTag;
...
bool EntityTaggable(int num, team_t team, bool trace) {
    ...
    if (num == ENTITYNUM_NONE || num == ENTITYNUM_WORLD)
        return false;
    ...
    switch (ent->s.eType) {
        ...
        case ET_PLAYER:
            ...
            if (!ent->client)
                return false;
            // don't tag teammates
            if (team != TEAM_NONE && ent->client->pers.team == team)
                return false;
            return true;
 ...
 * @param refreshTagged  Refresh all already tagged entities's tags and exclude
 * these entities from further consideration.
 */
