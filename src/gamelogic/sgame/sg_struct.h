...
/**
 * in the context of a target, this describes the conditions to create or to act
 * within
 * while as part of trigger or most other types, it will be used as filtering
 * condition that needs to be fulfilled to trigger, or to act directly
 */
...
/**
 * struct containing the configuration data of a gentity opposed to its state
 * data
 */
...
struct entityClass_s {
    ...
    /**
   * default config
   * entities might fallback to their classwide config if their individual is
   * not set
   */
...
struct gentity_s {
   ...
   */
    ...
    // entity creation time, i.e. when a building was build or a missile was fired
    // (for diminishing missile damage)
    int creationTime;
   ...
   * e.g. used for buildables (e.g. medi-stations or hives can be in an active
   * state or being inactive)
   * or during executing act() in general
   */
    bool active;
    /**
   * delay being really active until this time, e.g for act() delaying or for
   * spinup for norfenturrets
   * this will most probably be set by think() before act()ing, probably by
   * using the config.delay time
   */
    int nextAct;
    /**
   ...
   * e.g. used for buildables to indicate being usable or a stationary weapon
   *being "live"
   * or for sensors to indicate being able to sense other entities and fire
   *events
   *
   * as a reasonable assumption we default to entities being enabled directly
   ...
   * currentSparePower takes temporary influences into account and sets a
   *buildables power state.
   * expectedSparePower is a prediction of the static part and is used for
   *limiting new buildables.
   *
   ...
   */
    ...
    gentity_t* target; /*< the currently selected target to aim at/for, is the
                        reverse to "tracker" */
    gentity_t*
            tracker; /*< entity that currently targets, aims for or tracks this
                  entity, is the reverse to "target" */
    int numTrackedBy;
   ...
   */
    ...
    // sound index, used by movers as well as target_speaker e.g. for looping
    // sounds
    int soundIndex;
   ...
   */
    ...
    /*
   * do not abuse this variable (again) for anything but actual representing a
   *count
   *
   ...
   */
    ...
    /*
   * handle the notification about an event or undertaken action, so each entity
   * can decide to undertake special actions as result
   */
    void (*notifyHandler)(gentity_t* self, gentityCall_t* call);

    /**
   * the entry function for calls to the entity;
   * especially previous chain members will indirectly call this when firing
   * against the given entity
   * @returns true if the call was handled by the given function and doesn't
   * need default handling anymore or false otherwise
   */
   ...
   * so now we rather name them to indicate the fact, that we cannot imply any
   *meaning by the name
   *
   * please try not to use them for new things if you can avoid it
   * but prefer them over semantically abusing other variables if you are into
   *that sort of thing
   */
    int damage;
    int customNumber;
    ...
    int nextPhysicsTime; // buildables don't need to check what they're sitting
                         // on
...
struct clientSession_s {
    ...
    int seenWelcome; // determines if the client has seen server's welcome
                     // message
};
...
struct clientPersistant_s {
    ...
    bool disableBlueprintErrors; // should the buildable blueprint never be
                                 // hidden from the players?

    class_t classSelection; // player class (copied to ent->client->ps.stats[
                            // STAT_CLASS ] once spawned)
    ...
    // These have a copy in playerState_t.persistent but we use them in GAME so
    // they don't get invalidated by
    ...
    bool useUnlagged;
    int pubkey_authenticated; // -1 = does not have pubkey, 0 = not
...
struct gclient_s {
    ...
    // sum up damage over an entire frame, so shotgun blasts give a single big
    // kick
...
struct level_locals_s {
    ...
    int framenum;
    int time;         // time the map was first started in milliseconds (map restart will
                      // update startTime)
