    ...
    for (i = PCL_NONE + 1; i < PCL_NUM_CLASSES; i++) {
        ...
        if (model->navMeshClass) {
            if (BG_ClassModelConfig(model->navMeshClass)->navMeshClass) {
                Com_Printf(S_ERROR
                           "class '%s': navmesh reference target class '%s' must have "
                           "its own navmesh\n",
                           BG_Class(i)->name,
                           BG_Class(model->navMeshClass)->name);
                return;
            }
...
float BotGetGoalRadius(gentity_t* self) {
    if (BotTargetIsEntity(self->botMind->goal)) {
        ...
        if (t->ent->s.modelindex == BA_H_MEDISTAT ||
            t->ent->s.modelindex == BA_A_BOOSTER) {
            return self->r.maxs[0] + t->ent->r.maxs[0];
        } else {
            return RadiusFromBounds2D(t->ent->r.mins, t->ent->r.maxs) +
                   RadiusFromBounds2D(self->r.mins, self->r.maxs);
        }
    } else {
...
void BotStrafeDodge(gentity_t* self) {
    ...
    if ((self->client->time1000 % 300) >= 100 &&
        (self->client->time10000 % 3000) > 2000) {
...
bool BotShouldJump(gentity_t* self, gentity_t* blocker, const vec3_t dir) {
    ...
    jumpMagnitude =
            BG_Class((class_t) self->client->ps.stats[STAT_CLASS])->jumpMagnitude;
    ...
    // note that we also test for a blocking barricade because barricades will
    // collapse to let us through
...
bool BotFindSteerTarget(gentity_t* self, vec3_t dir) {
    ...
    for (i = 0; i < 5; i++, yaw1 -= 15, yaw2 += 15) {
        ...
        // test it
        trap_Trace(&trace1, self->s.origin, playerMins, playerMaxs, testPoint1, self->s.number, MASK_SHOT, 0);
        ...
        // test it
        trap_Trace(&trace2, self->s.origin, playerMins, playerMaxs, testPoint2, self->s.number, MASK_SHOT, 0);
...
bool BotOnLadder(gentity_t* self) {
    ...
    if (!BG_ClassHasAbility((class_t) self->client->ps.stats[STAT_CLASS],
                            SCA_CANUSELADDERS)) {
        return false;
    }
...
void BotClampPos(gentity_t* self) {
    ...
    trap_Trace(&trace, self->client->ps.origin, mins, maxs, origin, self->client->ps.clientNum, MASK_PLAYERSOLID, 0);
