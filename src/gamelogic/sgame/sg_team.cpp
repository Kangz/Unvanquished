...
void G_AreaTeamCommand(gentity_t* ent, const char* cmd) {
    ...
    for (i = 0; i < num; i++) {
        if (g_entities[entityList[i]].client &&
            g_entities[entityList[i]].client->pers.connected == CON_CONNECTED) {
...
void G_LeaveTeam(gentity_t* self) {
    ...
    if (TEAM_ALIENS == team || TEAM_HUMANS == team) {
        G_RemoveFromSpawnQueue(&level.team[team].spawnQueue,
...
void G_ChangeTeam(gentity_t* ent, team_t newTeam) {
    ...
    if (oldTeam == TEAM_HUMANS && newTeam == TEAM_ALIENS) {
        ...
        ent->client->pers.credit =
                (int) (ent->client->pers.credit * ALIEN_MAX_CREDITS / HUMAN_MAX_CREDITS +
                       0.5f);
    } else if (oldTeam == TEAM_ALIENS && newTeam == TEAM_HUMANS) {
        // Convert from alien to human credits
        ent->client->pers.credit =
                (int) (ent->client->pers.credit * HUMAN_MAX_CREDITS / ALIEN_MAX_CREDITS +
                       0.5f);
    }
    ...
    // Update PERS_UNLOCKABLES in the same frame as PERS_TEAM to prevent bad
...
void TeamplayInfoMessage(gentity_t* ent) {
    ...
    if (ent->client->pers.team == TEAM_NONE) {
        ...
        team = g_entities[ent->client->sess.spectatorClient].client->pers.team;
    } else {
    ...
    for (i = 0; i < level.maxclients; i++) {
        ...
        if (ent == player || !cl || team != cl->pers.team || !player->inuse) {
            continue;
        }
        ...
        {
            Com_sprintf(entry, sizeof(entry), " %i %i %i %i %i", i, cl->pers.location, cl->ps.stats[STAT_HEALTH] < 1 ? 0 : cl->ps.stats[STAT_HEALTH], curWeaponClass, cl->pers.credit);
        } else {
            Com_sprintf(entry, sizeof(entry), " %i %i %i %i %i %i", i, cl->pers.location, cl->ps.stats[STAT_HEALTH] < 1 ? 0 : cl->ps.stats[STAT_HEALTH], curWeaponClass, cl->pers.credit, upgrade);
        }
    ...
    if (g_teamImbalanceWarnings.integer && !level.intermissiontime &&
        ...
        if (level.team[TEAM_ALIENS].numSpawns > 0 &&
            level.team[TEAM_HUMANS].numClients -
                            level.team[TEAM_ALIENS].numClients >
                    2) {
