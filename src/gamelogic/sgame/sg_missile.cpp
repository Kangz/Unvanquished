...
static float MissileTimePowerMod(gentity_t* self, missileTimePowerMod_t type, float startMod, float endMod, int startTime, int endTime) {
...
static float MissileTimeDmgMod(gentity_t* self) {
    switch (self->s.weapon) {
        case MIS_FLAMER:
            return MissileTimePowerMod(self, MTPR_LINEAR_DECREASE, 1.0f, FLAMER_DAMAGE_MAXDST_MOD, 0, FLAMER_LIFETIME);

        case MIS_LCANNON:
            return MissileTimePowerMod(self, MTPR_EXPONENTIAL_DECREASE, 1.0f, 0.0f, LCANNON_DAMAGE_FULL_TIME, LCANNON_DAMAGE_HALF_LIFE);

        case MIS_PRIFLE:
            return MissileTimePowerMod(self, MTPR_EXPONENTIAL_DECREASE, 1.0f, 0.0f, PRIFLE_DAMAGE_FULL_TIME, PRIFLE_DAMAGE_HALF_LIFE);
    }
...
static float MissileTimeSplashDmgMod(gentity_t* self) {
    switch (self->s.weapon) {
        case MIS_FLAMER:
            return MissileTimePowerMod(self, MTPR_LINEAR_INCREASE, FLAMER_SPLASH_MINDST_MOD, 1.0f, 0, FLAMER_LIFETIME);
    }
...
#define MIB_BOUNCE \
    (MIF_NO_DAMAGE | MIF_NO_EFFECT | MIF_NO_FREE) // Continue flight.
...
static int ImpactFlamer(gentity_t* ent, trace_t* trace, gentity_t* hitEnt) {
    ...
    while ((neighbor = G_IterateEntitiesWithinRadius(neighbor, trace->endpos, FLAMER_IGNITE_RADIUS))) {
        // we already handled other, since it might not always be in
        // FLAMER_IGNITE_RADIUS due to BBOX sizes
        if (neighbor == hitEnt) {
            continue;
        }
...
static int ImpactSlowblob(gentity_t* ent, trace_t* trace, gentity_t* hitEnt) {
    ...
    if (hitEnt->client && hitEnt->client->pers.team == TEAM_HUMANS) {
    ...
    } else if (hitEnt->s.eType == ET_BUILDABLE &&
               hitEnt->buildableTeam == TEAM_ALIENS) {
        hitEnt->onFire = false;
        ...
        neighbor = nullptr;
        while ((neighbor = G_IterateEntitiesWithinRadius(
                        neighbor, trace->endpos, ABUILDER_BLOB_FIRE_STOP_RANGE))) {
...
static int ImpactHive(gentity_t* ent, trace_t* trace, gentity_t* hitEnt) {
    if (hitEnt->s.eType == ET_BUILDABLE && hitEnt->s.modelindex == BA_A_HIVE) {
        if (!ent->parent) {
            G_Printf(S_WARNING
                     "Hive missile returned to hive that is not its parent.\n");
        } else {
...
static void MissileImpact(gentity_t* ent, trace_t* trace) {
    ...
    switch (ent->s.modelindex) {
        ...
        case MIS_FIREBOMB:
            ...
            break;
        case MIS_FLAMER:
            impactFunc = ImpactFlamer;
            break;
        case MIS_FIREBOMB_SUB:
            impactFunc = ImpactFirebombSub;
            break;
        case MIS_LOCKBLOB:
            impactFunc = ImpactLockblock;
            break;
        case MIS_SLOWBLOB:
            impactFunc = ImpactSlowblob;
            break;
        case MIS_HIVE:
            impactFunc = ImpactHive;
            break;
        default:
    ...
    if (!(impactFlags & MIF_NO_DAMAGE)) {
        if (ent->damage && hitEnt->takedamage) {
            ...
            int dflags = 0;
            if (!ma->doLocationalDamage)
                dflags |= DAMAGE_NO_LOCDAMAGE;
            if (ma->doKnockback)
                dflags |= DAMAGE_KNOCKBACK;

            G_Damage(hitEnt, ent, attacker, dir, trace->endpos, roundf(ent->damage * MissileTimeDmgMod(ent)), dflags, ent->methodOfDeath);
        }

        // splash damage (doesn't apply to person directly hit)
        if (ent->splashDamage) {
            G_RadiusDamage(trace->endpos, ent->parent, ent->splashDamage * MissileTimeSplashDmgMod(ent), ent->splashRadius, hitEnt, (ma->doKnockback ? DAMAGE_KNOCKBACK : 0), ent->splashMethodOfDeath);
        }
    }

    // Play hit effects and remove the missile.
    if (!(impactFlags & MIF_NO_EFFECT)) {
        // Use either the trajectory direction or the surface normal for the hit
        // event.
        ...
        if (hitEnt->takedamage &&
            (hitEnt->s.eType == ET_PLAYER || hitEnt->s.eType == ET_BUILDABLE)) {
...
void G_ExplodeMissile(gentity_t* ent) {
    ...
    // splash damage
    if (ent->splashDamage) {
        G_RadiusDamage(ent->r.currentOrigin, ent->parent, ent->splashDamage * MissileTimeSplashDmgMod(ent), ent->splashRadius, ent, (ma->doKnockback ? DAMAGE_KNOCKBACK : 0), ent->splashMethodOfDeath);
    }
...
void G_RunMissile(gentity_t* ent) {
    ...
    trap_Trace(&tr, ent->r.currentOrigin, ent->r.mins, ent->r.maxs, origin, passent, ent->clipmask, 0);
    ...
    if (tr.fraction < 1.0f) {
        if (!ent->pointAgainstWorld || (tr.contents & CONTENTS_BODY)) {
            ...
            trap_Trace(&tr, ent->r.currentOrigin, nullptr, nullptr, origin, passent, ent->clipmask, 0);
            ...
            if (tr.fraction < 1.0f) {
                ...
                if (tr.contents & CONTENTS_BODY) {
                    ...
                    trap_Trace(&tr, ent->r.currentOrigin, ent->r.mins, ent->r.maxs, origin, passent, CONTENTS_BODY, 0);
    ...
    if (impact) {
        ...
        // HACK: The missile has turned into an explosion and will free itself
        // later.
    ...
    if (ent->flightSplashDamage) {
        G_RadiusDamage(tr.endpos, ent->parent, ent->flightSplashDamage, ent->flightSplashRadius, ent->parent, 0, ent->splashMethodOfDeath);
    }
...
gentity_t* G_SpawnMissile(missile_t missile, gentity_t* parent, vec3_t start, vec3_t dir, gentity_t* target, void (*think)(gentity_t* self), int nextthink) {
    ...
    m->r.mins[0] = m->r.mins[1] = m->r.mins[2] = -ma->size;
    m->r.maxs[0] = m->r.maxs[1] = m->r.maxs[2] = ma->size;
    m->s.eFlags = ma->flags;
