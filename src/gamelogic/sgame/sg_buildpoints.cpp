...
/**
 * @brief Calculates modifier for the efficiency of one RGS when another one
 * interfers at given
 *        distance.
 */
static float RGSInterferenceMod(float distance) {
    ...
    // q is the ratio of the part of a sphere with radius RGS_RANGE that
    // intersects
...
static void RGSCalculateRate(gentity_t* self) {
    ...
    if (!self->spawned || !self->powered) {
        ...
        for (rgs = nullptr; (rgs = G_IterateEntitiesWithinRadius(
                                     rgs, self->s.origin, RGS_RANGE * 2.0f));) {
            if (rgs->s.eType == ET_BUILDABLE && (rgs->s.modelindex == BA_H_DRILL ||
                                                 rgs->s.modelindex == BA_A_LEECH) &&
...
void G_RGSThink(gentity_t* self) {
    ...
    if (active ^ lastThinkActive) {
        // If the state of this RGS has changed, adjust own rate and inform active
        // closeby RGS so
    ...
    self->s.weapon =
            std::round((255.0f * self->acquiredBuildPoints) /
...
void G_MainStructBPStorageThink(gentity_t* self) {
    self->acquiredBuildPoints =
            level.team[self->buildableTeam].mainStructAcquiredBP;
...
void G_BPStorageDie(gentity_t* self) {
    ...
    // Removes some of the owner's team's build points, proportional to the amount
    // this structure
    // acquired and the amount of health lost (before deconstruction).
    float acquiredFrac =
            self->acquiredBuildPoints / level.team[team].acquiredBuildPoints;
...
/**
 * @brief Predict the efficiecy loss of a RGS if another one is constructed at
 * given origin.
...
static float RGSPredictInterferenceLoss(gentity_t* self, vec3_t origin) {
    ...
    predictedRate =
            currentRate * RGSInterferenceMod(Distance(self->s.origin, origin));
...
/**
 * @brief Predict the total efficiency gain for a team when a RGS is constructed
 * at the given point.
...
float G_RGSPredictEfficiencyDelta(vec3_t origin, team_t team) {
    ...
    for (rgs = nullptr;
         (rgs = G_IterateEntitiesWithinRadius(rgs, origin, RGS_RANGE * 2.0f));) {
        if (rgs->s.eType == ET_BUILDABLE &&
            (rgs->s.modelindex == BA_H_DRILL || rgs->s.modelindex == BA_A_LEECH) &&
            rgs->spawned && rgs->powered && rgs->health > 0 &&
            rgs->buildableTeam == team) {
...
/**
 * @brief Calculate the level mine rate and the teams' mining efficiencies, add
 * build points.
 */
void G_MineBuildPoints() {
    ...
    if (G_ActiveReactor() &&
        (deltaEff = minEff - level.team[TEAM_HUMANS].mineEfficiency) > 0) {
        level.team[TEAM_HUMANS].mineEfficiency += deltaEff;
        level.team[TEAM_HUMANS].mainStructAcquiredBP += deltaEff * mineMod;
    }
    if (G_ActiveOvermind() &&
        (deltaEff = minEff - level.team[TEAM_ALIENS].mineEfficiency) > 0) {
    ...
    for (playerNum = 0; playerNum < level.maxclients; playerNum++) {
        ...
        if (G_IsPlayableTeam(team)) {
            ...
            // TODO: Check if this is related to efficiency display flickering for
            // spectators
...
/**
 * @brief Adds or removes build points to the virtual pool of mined build
 * points.
 */
