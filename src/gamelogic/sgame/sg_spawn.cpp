...
static const fieldDescriptor_t fields[] = {
        ...
        {"target2", FOFS(targets), F_TARGET},                             // backwardcompatibility with AMP and
                                                                          // to use the blackout map for
                                                                          // testing
        {"target3", FOFS(targets), F_TARGET},                             // backwardcompatibility with AMP and
                                                                          // to use the blackout map for
                                                                          // testing
        {"target4", FOFS(targets), F_TARGET},                             // backwardcompatibility with AMP and
                                                                          // to use the blackout map for
                                                                          // testing
        {"targetname", FOFS(names[1]), F_STRING, ENT_V_TMPNAME, "name"},  // radiants ui sadly strongly encourages the "targetname" keyword
        {"targetname2", FOFS(names[2]), F_STRING, ENT_V_RENAMED, "name"}, // backwardcompatibility with AMP and to use the blackout map for
                                                                          // testing
...
typedef enum {
    /*
   * self sufficent, it might possibly be fired at, but it can do as well on its
   * own, so won't be freed automaticly
   */
    CHAIN_AUTONOMOUS,
    /*
   * needs something to fire at, or it has no reason to exist in this <world>
   * and will be freed
   */
    CHAIN_ACTIVE,
    /*
   * needs something to fire at it in order to fullfill any task given to it, or
   * will be freed otherwise
   */
    CHAIN_PASSIV,
    /*
   * needs something to fire at it, and something to relay that fire to (under
   * whatever conditions given), or will be freed otherwise
   */
    CHAIN_RELAY,
    /*
   * will be aimed at by something, but no firing needs to be involved at all,
   * if not aimed at, it will be freed
   */
    CHAIN_TARGET,
} entityChainType_t;
...
static const entityClassDescriptor_t entityClassDescriptions[] = {
     ...
     *  These are actions and effects against the whole match, a team or a
     *player.
     */
     ...
     *	Graphic Effects
     *	====================
     *	Entities that represent some form of Graphical or visual Effect in the
     *world.
     ...
     *  Position entities
     *  =================
     *  position entities may get used by other entities or other processes as
     *provider for positional data
     *
     ...
     *  of another entity, event, or gamestate (timer and start being aware of
     * the game start).
     *  Enabling/Disabling Sensors generally changes their ability of perceiving
     * other entities.
     */
         ...
         *	Sound Effects
         *	====================
         *	Entities that represent some form of auditive effect in the
      *world.
        ...
        /*
     * former target and trigger entities, now deprecated or soon to be
     * deprecated
     */
...
bool G_HandleEntityVersions(entityClassDescriptor_t* spawnDescription,
                            gentity_t* entity) {
    if (spawnDescription->versionState ==
        ENT_V_CURRENT) // we don't need to handle anything
        return true;

    if (!spawnDescription->replacement ||
        !Q_stricmp(entity->classname, spawnDescription->replacement)) {
        if (g_debugEntities.integer > -2)
            G_Printf(S_ERROR
                     "Class %s has been marked deprecated but no replacement has "
                     "been supplied\n",
                     etos(entity));
    ...
    if (g_debugEntities.integer >=
        0) // dont't warn about anything with -1 or lower
    {
        if (spawnDescription->versionState < ENT_V_TMPORARY ||
            (g_debugEntities.integer >= 1 &&
             spawnDescription->versionState >= ENT_V_TMPORARY)) {
            G_Printf(
                    S_WARNING
                    "Entity %s uses a deprecated classtype — use the class " S_COLOR_CYAN
                    "%s" S_COLOR_WHITE " instead\n",
                    etos(entity),
                    spawnDescription->replacement);
        }
    }
    ...
    switch (entityClass->chainType) {
        case CHAIN_ACTIVE:
                                          ...
                                          // compatibility
            {
                if (g_debugEntities.integer > -2)
                    G_Printf(
                            S_WARNING
                            "Entity %s needs to call or target to something — Removing it.\n",
                            etos(entity));
                return false;
            }
            break;
        ...
        case CHAIN_PASSIV:
            if (!entity->names[0]) {
                if (g_debugEntities.integer > -2)
                    G_Printf(S_WARNING
                             "Entity %s needs a name, so other entities can target it — "
                             "Removing it.\n",
                             etos(entity));
                return false;
            }
        ...
        case CHAIN_RELAY:
            if ((!entity->callTargetCount) // check target usage for backward
                ...
                if (g_debugEntities.integer > -2)
                    G_Printf(S_WARNING
                             "Entity %s needs a name as well as a target to "
                             "conditionally relay the firing — Removing it.\n",
                             etos(entity));
                return false;
            }
            break;
...
bool G_CallSpawnFunction(gentity_t* spawnedEntity) {
    ...
    if (!spawnedEntity->classname) {
        // don't even warn about spawning-errors with -2 (maps might still work at
        // least partly if we ignore these willingly)
        if (g_debugEntities.integer > -2)
            G_Printf(S_ERROR "Entity " S_COLOR_CYAN "#%i" S_COLOR_WHITE
                             " is missing classname – we are unable to spawn it.\n",
    ...
    spawnedClass = (entityClassDescriptor_t*) bsearch(
            spawnedEntity->classname, entityClassDescriptions, ARRAY_LEN(entityClassDescriptions), sizeof(entityClassDescriptor_t), cmdcmp);

    if (spawnedClass) { // found it

        spawnedEntity->eclass =
                &entityClasses[(int) (spawnedClass - entityClassDescriptions)];
        ...
        if (g_debugEntities.integer > 2)
            G_Printf(S_DEBUG "Successfully spawned entity " S_COLOR_CYAN
                             "#%i" S_COLOR_WHITE " as " S_COLOR_YELLOW
                             "%i" S_COLOR_WHITE "th instance of " S_COLOR_CYAN "%s\n",
                     spawnedEntity->s.number,
                     spawnedEntity->eclass->instanceCounter,
                     spawnedClass->name);
    ...
    if (g_debugEntities.integer > -2) {
        if (!Q_stricmp(S_WORLDSPAWN, spawnedEntity->classname)) {
            G_Printf(S_ERROR "a " S_COLOR_CYAN S_WORLDSPAWN S_COLOR_WHITE
                             " class was misplaced into position " S_COLOR_CYAN
                             "#%i" S_COLOR_WHITE " of the spawn string – Ignoring\n",
                     spawnedEntity->s.number);
        } else {
...
=============
*/
gentityCallDefinition_t G_NewCallDefinition(char* eventKey,
                                            const char* string) {
    char* stringPointer;
    int i, stringLength;
    ...
    newCallDefinition.actionType =
            G_GetCallActionTypeFor(newCallDefinition.action);
...
void G_ParseField(const char* key, const char* rawString, gentity_t* entity) {
    ...
    fieldDescriptor = (fieldDescriptor_t*) bsearch(
            key, fields, ARRAY_LEN(fields), sizeof(fieldDescriptor_t), cmdcmp);
    ...
    switch (fieldDescriptor->type) {
        ...
        case F_TARGET:
            ...
            ((char**) entityDataField)[entity->targetCount++] =
                    G_NewString(rawString);
            break;

        case F_CALLTARGET:
            if (entity->callTargetCount >= MAX_ENTITY_CALLTARGETS)
                G_Error(
                        "Maximal number of %i calltargets reached. You can solve this by "
                        "using a Relay.",
                        MAX_ENTITY_CALLTARGETS);

            ((gentityCallDefinition_t*) entityDataField)[entity->callTargetCount++] =
                    G_NewCallDefinition(fieldDescriptor->replacement
                                                ? fieldDescriptor->replacement
        ...
        case F_SOUNDINDEX:
            if (strlen(rawString) >= MAX_QPATH) {
                G_Error(S_ERROR
                        "Sound filename %s in field %s of %s exceeds MAX_QPATH\n",
                        rawString,
                        fieldDescriptor->name,
                        etos(entity));
            }
...
void G_SpawnGEntityFromSpawnVars() {
   ...
   */
    if (level.numSpawnVars <= 1) {
        G_Printf(
                S_ERROR "encountered ghost-entity #%i with only one field: %s = %s\n",
                spawningEntity->s.number,
                level.spawnVars[0][0],
                level.spawnVars[0][1]);
        G_FreeEntity(spawningEntity);
        return;
   ...
   */
    ...
    // if we didn't get necessary fields (like the classname), don't bother
    // spawning anything
...
void G_ReorderCallTargets(gentity_t* ent) {
    ...
    for (i = 0; i < MAX_ENTITY_CALLTARGETS; ++i) {
        if (ent->calltargets[i].name) {
            ...
            ent->calltargets[j].actionType =
                    G_GetCallActionTypeFor(ent->calltargets[i].action);
            j++;
        }
    }
...
bool G_WarnAboutDeprecatedEntityField(gentity_t* entity,
                                      const char* expectedFieldname,
                                      const char* actualFieldname,
                                      const int typeOfDeprecation) {
    if (!Q_stricmp(expectedFieldname, actualFieldname) ||
        typeOfDeprecation == ENT_V_CURRENT)
        return false;
        ...
        0) // dont't warn about anything with -1 or lower
    {
        if (typeOfDeprecation < ENT_V_TMPORARY ||
            ...
            G_Printf(S_WARNING "Entity " S_COLOR_CYAN "#%i" S_COLOR_WHITE
                               " contains deprecated field " S_COLOR_CYAN
                               "%s" S_COLOR_WHITE " — use " S_COLOR_CYAN
                               "%s" S_COLOR_WHITE " instead\n",
                     entity->s.number,
                     actualFieldname,
                     expectedFieldname);
...
/**
 * Warning: The following comment contains information, that might be parsed and
 * used by radiant based mapeditors.
 */
...
; music: path/name of looping .wav file used for level's music (eg.
music/sonic5.wav).
; gravity: level gravity [g_gravity (800)]

; humanBuildPoints: maximum amount of power the humans can use.
[g_humanBuildPoints]
; humanRepeaterBuildPoints: maximum amount of power the humans can use around
each repeater. [g_humanRepeaterBuildPoints]
; alienBuildPoints: maximum amount of sentience available to the overmind.
[g_alienBuildPoints]

; disabledEquipment: A comma delimited list of human weapons or upgrades to
disable for this map. [g_disabledEquipment ()]
; disabledClasses: A comma delimited list of alien classes to disable for this
map. [g_disabledClasses ()]
; disabledBuildables: A comma delimited list of buildables to disable for this
map. [g_disabledBuildables ()]
*/
void SP_worldspawn() {
    char* s;
    ...
    if (Q_stricmp(s, S_WORLDSPAWN)) {
        G_Error(
                "SP_worldspawn: The first entry in the spawn string isn't of expected "
                "type '" S_WORLDSPAWN "'");
    }
    ...
    if (G_SpawnString("colorGrade", "", &s)) {
        Com_Printf(
                "^3Warning: ^7\"colorGrade\" deprecated. Please use "
                "\"gradingTexture\"");
        trap_SetConfigstring(CS_GRADING_TEXTURES, va("%i %f %s", 0, 0.0f, s));
    }
    ...
    if (G_SpawnString("reverbEffect", "", &s))
        trap_SetConfigstring(
                CS_REVERB_EFFECTS,
                va("%i %f %s %f", 0, 0.0f, s, Com_Clamp(0.0f, 2.0f, reverbIntensity)));
    ...
    G_SpawnStringIntoCVarIfSet("humanRepeaterBuildPoints",
...
void G_SpawnFakeEntities() {
    ...
    level.fakeLocation->s.origin[0] = level.fakeLocation->s.origin[1] =
