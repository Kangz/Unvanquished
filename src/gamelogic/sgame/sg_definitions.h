...
#define BURN_PERIODS_RAND_MOD \
    (1.0f + (random() - 0.5f) * 2.0f * BURN_PERIODS_RAND)

// resolves a variatingTime_t to a variated next level.time
#define VariatedLevelTime(variableTime) \
    level.time + (variableTime.time + variableTime.variance * crandom()) * 1000
