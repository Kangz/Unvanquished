...
gentity_t* G_NewEntity() {
    ...
    for (force = 0; force < 2; force++) {
        ...
        for (i = MAX_CLIENTS; i < level.num_entities; i++, newEntity++) {
            ...
            if (!force && newEntity->freetime > level.startTime + 2000 &&
                level.time - newEntity->freetime < 1000) {
                continue;
            }
...
char* etos(const gentity_t* entity) {
    ...
    Com_sprintf(resultString, MAX_ETOS_LENGTH, "%s%s" S_COLOR_WHITE "(" S_COLOR_CYAN "%s" S_COLOR_WHITE "|" S_COLOR_CYAN "#%i" S_COLOR_WHITE ")", entity->names[0] ? entity->names[0] : "", entity->names[0] ? " " : "", entity->classname, entity->s.number);
...
Iteration will continue to return the gentity following the "previous" parameter
that fullfill these conditions
...
gentity_t* G_IterateEntities(gentity_t* entity, const char* classname, bool skipdisabled, size_t fieldofs, const char* match) {
    ...
    if (!entity) {
        entity = g_entities;
        // start after the reserved player slots, if we are not searching for a
        // player
...
if we are not searching for player entities it is recommended to start searching
from gentities[MAX_CLIENTS - 1]
...
gentity_t* G_IterateEntitiesWithinRadius(gentity_t* entity, vec3_t origin, float radius) {
    ...
    for (; entity < &g_entities[level.num_entities]; entity++) {
        ...
        for (j = 0; j < 3; j++) {
            eorg[j] = origin[j] - (entity->r.currentOrigin[j] +
                                   (entity->r.mins[j] + entity->r.maxs[j]) * 0.5);
        }
...
gentity_t* G_PickRandomEntity(const char* classname, size_t fieldofs, const char* match) {
    ...
    if (!totalChoiceCount) {
        if (g_debugEntities.integer > -1)
            G_Printf(S_WARNING "Could not find any entity matching \"" S_COLOR_CYAN
                               "%s%s%s" S_COLOR_WHITE "\"\n",
                     classname ? classname : "",
...
#define WORLD_CALL                                                          \
    gentityCall_t {                                                         \
        nullptr, &g_entities[ENTITYNUM_WORLD], &g_entities[ENTITYNUM_WORLD] \
    }
/**
 * a non made call
 */
#define NULL_CALL                                                         \
    gentityCall_t {                                                       \
        nullptr, &g_entities[ENTITYNUM_NONE], &g_entities[ENTITYNUM_NONE] \
    }
...
gentityCallEvent_t G_GetCallEventTypeFor(const char* event) {
    ...
    foundDescription = (entityCallEventDescription_t*) bsearch(
            event, gentityEventDescriptions, ARRAY_LEN(gentityEventDescriptions), sizeof(entityCallEventDescription_t), cmdcmp);
...
gentityCallActionType_t G_GetCallActionTypeFor(const char* action) {
    ...
    foundDescription = (entityActionDescription_t*) bsearch(
            action, actionDescriptions, ARRAY_LEN(actionDescriptions), sizeof(entityActionDescription_t), cmdcmp);
...
gentity_t* G_IterateTargets(gentity_t* entity, int* targetIndex, gentity_t* self) {
    ...
    for (*targetIndex = 0; self->targets[*targetIndex]; ++(*targetIndex)) {
        if (self->targets[*targetIndex][0] == '$') {
            possibleTarget =
                    G_ResolveEntityKeyword(self, self->targets[*targetIndex]);
            if (possibleTarget && possibleTarget->enabled)
                return possibleTarget;
            return nullptr;
        }

        for (entity = &g_entities[MAX_CLIENTS];
             entity < &g_entities[level.num_entities];
             entity++) {
...
gentity_t* G_IterateCallEndpoints(gentity_t* entity, int* calltargetIndex, gentity_t* self) {
    ...
    for (*calltargetIndex = 0; self->calltargets[*calltargetIndex].name;
         ++(*calltargetIndex)) {
        if (self->calltargets[*calltargetIndex].name[0] == '$')
            return G_ResolveEntityKeyword(self,
                                          self->calltargets[*calltargetIndex].name);
...
gentity_t* G_PickRandomTargetFor(gentity_t* self) {
    ...
    // collects the targets
    while ((foundTarget = G_IterateTargets(foundTarget, &targetIndex, self)) !=
           nullptr)
    ...
    if (!totalChoiceCount) {
        if (g_debugEntities.integer > -1) {
            G_Printf(S_WARNING
                     "none of the following targets could be resolved for Entity %s:",
                     etos(self));
...
void G_FireEntityRandomly(gentity_t* entity, gentity_t* activator) {
    ...
    while ((possibleTarget = G_IterateCallEndpoints(possibleTarget, &targetIndex, entity)) != nullptr) {
        ...
        choices[totalChoiceCount].callDefinition =
                &entity->calltargets[targetIndex];
        totalChoiceCount++;
    }
...
void G_EventFireEntity(gentity_t* self, gentity_t* activator, gentityCallEvent_t eventType) {
    ...
    while ((currentTarget = G_IterateCallEndpoints(currentTarget, &targetIndex, self)) != nullptr) {
        ...
        call.caller = self; // reset the caller in case there have been nested
                            // calls
 ...
 * neither paramater may be nullptr, and the entity is required to have an act
 * function to execute
...
void G_ExecuteAct(gentity_t* entity, gentityCall_t* call) {
   ...
   */
    ...
    entity->active = true;
    /*
   * for now we use the callIn activator if its set or fallback to the old
   * solution, but we should
...
/**
 * check delayed variable and either call an entity act() directly or delay its
 * execution
 */
void G_HandleActCall(gentity_t* entity, gentityCall_t* call) {
    ...
    } else /* no time and variance set means, we can call it directly instead of
...
void G_CallEntity(gentity_t* targetedEntity, gentityCall_t* call) {
    if (g_debugEntities.integer > 1) {
        G_Printf(S_DEBUG "[%s] %s calling %s %s:%s\n", etos(call->activator), etos(call->caller), call->definition ? call->definition->event : "onUnknown", etos(targetedEntity), call->definition && call->definition->action ? call->definition->action : "default");
    }

    targetedEntity->callIn = *call;

    if ((!targetedEntity->handleCall ||
         !targetedEntity->handleCall(targetedEntity, call)) &&
        call->definition) {
        switch (call->definition->actionType) {
            ...
            case ECA_ENABLE:
                if (!targetedEntity
                             ->enabled) // only fire an event if we weren't already enabled
                {
            ...
            case ECA_DISABLE:
                if (targetedEntity
                            ->enabled) // only fire an event if we weren't already disabled
                {
            ...
            case ECA_USE:
                if (!targetedEntity->use) {
                    if (g_debugEntities.integer >= 0)
                        G_Printf(S_WARNING
                                 "calling :use on %s, which has no use function!\n",
                                 etos(targetedEntity));
                    break;
                }
                if (!call->activator || !call->activator->client) {
                    if (g_debugEntities.integer >= 0)
                        G_Printf(S_WARNING
                                 "calling %s:use, without a client as activator.\n",
                                 etos(targetedEntity));
                    break;
                }
...
bool G_IsVisible(gentity_t* start, gentity_t* end, int contents) {
    ...
    trap_Trace(&trace, start->s.pos.trBase, nullptr, nullptr, end->s.pos.trBase, start->s.number, contents, 0);
