...
/**
 * Notifies legacy stage sensors by assuming a certain amount of momentum is a
 *stage.
 *
...
static INLINE float MomentumTimeMod() {
    if (g_momentumRewardDoubleTime.value <= 0.0f) {
        ...
        // ln(2) ~= 0.6931472
        return exp(0.6931472f * ((level.matchTime / 60000.0f) /
                                 g_momentumRewardDoubleTime.value));
    }
}

/**
 * @todo Currently this function is just a guess, find out the correct mod via
 * statistics.
 */
static INLINE float MomentumPlayerCountMod() {
    int playerCount = std::max(2, level.team[TEAM_ALIENS].numClients + level.team[TEAM_HUMANS].numClients);

    // HACK: This uses the average number of players taking part in development
    // games so that the
...
static float MomentumMod(momentum_t type) {
    ...
    switch (type) {
        ...
        case CONF_DECONSTRUCTING:
            // always used on top of build mod, so neutral
            // baseMod/timeMod/playerCountMod
            baseMod = 1.0f;
    ...
    if (g_debugMomentum.integer > 1) {
        Com_Printf(
                "Momentum mod for %s: Base %.2f, Type %.2f, Time %.2f, Playercount "
                "%.2f → %.2f\n",
                MomentumTypeToReason(type),
                baseMod,
                typeMod,
                timeMod,
                playerCountMod,
