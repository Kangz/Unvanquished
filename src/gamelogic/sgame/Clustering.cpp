...
public:
    ...
    typedef
            typename std::unordered_map<Data, point_type>::const_iterator iter_type;
   ...
   */
    ...
    iter_type begin() const NOEXCEPT { return records.begin(); }

    iter_type end() const NOEXCEPT { return records.end(); }

    iter_type find(const Data& key) const { return records.find(key); }
...
/**
 * @brief A self-organizing container of clusters of objects located in
 *euclidean space.
 *
 * The clustering algorihm is based on Kruskal's algorithm for minimum spanning
 *trees.
 * In the minimum spanning tree of all edges that pass the optional visibility
 *check, delete the
 * edges that are longer than the average plus the standard deviation multiplied
 *by a "laxity"
...
public:
    ...
    /**
   * @param laxity Factor that scales the allowed deviation from the average
   * edge length.
   * @param edgeVisCallback Relation that decides whether an edge should be
   * considered.
   */
   ...
   */
    bool Remove(const Data& data) {
        // Check if we even know the object in O(n) to avoid unnecessary O(m)
        // iteration.
    ...
    /**
   * @brief Set laxity, a factor that scales the allowed deviation from the
   * average edge
   *        length when splitting the minimum spanning tree into cluster
   * spanning trees.
   */
...
private:
    /**
   * @brief Finds the minimum spanning tree in the graph defined by edges, where
   *edge
   ...
   *
   * In the minimum spanning tree, delete the edges that are longer than the
   *average plus
   * the standard deviation multiplied by a "laxity" factor. The remaining trees
   *span the
   * clusters.
   */
    void GenerateClusters() {
        ...
        // Split the MST into several trees by keeping only the edges that have a
        // length up
        // to a threshold.
        float edgeLengthThreshold =
                mstAverageDistance + mstStandardDeviation * laxity;
        ...
        // Build a cluster for each connected component, excluding isolated
        // vertices.
    ...
    /**  The edges of a non-reflexive graph of the data objects, sorted by
   * distance. */
    std::multimap<float, edge_type> edges;

    /** The edges of the minimum spanning tree in the graph defined by edges,
   * sorted by
   *  distance. Is a subset of edges. */
    std::multimap<float, edge_type> mstEdges;

    /** The edges of a forest of which each connected component spans a cluster.
   * Is a subset
   *  of mstEdges. */
    ...
    /** Whether clusters need to be rebuilt on read acces. Does not imply
   * regeneration of
   *  the minimum spanning tree. */
    bool dirtyClusters;

    /** Whether the minimum spanning tree needs to be rebuild on read access.
   * Implies
   *  regeneration of clusters. */
    bool dirtyMST;

    /** A factor that scales the allowed deviation from the average edge length
   * when
...
public:
    ...
    void Update(gentity_t* ent) { super::Update(ent, point_type(ent->s.origin)); }
 ...
 */
static inline baseClusteringLayer_t GetClusteringLayer(team_t team,
                                                       bool enemy) {
    if (team == TEAM_ALIENS)
...
static void PostChangeHook(baseClusteringLayer_t layer) {
    ...
    for (EntityClustering::cluster_type& cluster : bases[layer]) {
        ...
        float baseRadius =
                std::max(averageDistance + standardDeviation, MININUM_BASE_RADIUS);
        ...
        for (const EntityClustering::cluster_type::record_type& record : cluster) {
            ...
            if (ent->s.modelindex2 == BA_A_OVERMIND ||
                ent->s.modelindex2 == BA_H_REACTOR) {
                mainBase = true;
            }
        }

        // Trace from mean buildable's origin towards cluster center, so that the
        // beacon does
        ...
        // If a fitting beacon close to the target location already exists, move it
        // silently,
        // otherwise add a new one.
        gentity_t* beacon;
        if (!(beacon = Beacon::MoveSimilar(center.coords, tr.endpos, BCT_BASE, 0, team, 0, baseRadius, eFlags, EF_BC_BASE_RELEVANT))) {
...
void Init() {
    ...
    for (int clusteringNum = 0; clusteringNum < NUM_BC_LAYERS; clusteringNum++) {
        ...
        // Reset beacon lists
        std::map<baseClusteringLayer_t, std::unordered_set<gentity_t*>>::iterator
                layerBeacons;
...
void Update(gentity_t* beacon) {
    baseClusteringLayer_t layer = GetClusteringLayer(
            (team_t) beacon->s.generic1, (beacon->s.eFlags & EF_BC_ENEMY));
