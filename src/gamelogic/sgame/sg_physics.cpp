...
static void G_Bounce(gentity_t* ent, trace_t* trace) {
    ...
    if (VectorLength(ent->s.pos.trDelta) < 10) {
        VectorMA(trace->endpos, 0.5f, trace->plane.normal,
                 trace->endpos); // make sure it is off ground
...
void G_Physics(gentity_t* ent, int msec) {
    ...
    // if groundentity has been set to ENTITYNUM_NONE, it may have been pushed off
    // an edge
    ...
    if (ent->s.pos.trType == TR_STATIONARY) {
        ...
        if (ent->nextPhysicsTime < level.time) {
            ...
            trap_Trace(&tr, ent->r.currentOrigin, ent->r.mins, ent->r.maxs, origin, ent->s.number, ent->clipmask, 0);
