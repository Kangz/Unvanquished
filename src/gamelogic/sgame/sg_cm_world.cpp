...
bool G_CM_EntityContact(const vec3_t mins, const vec3_t maxs, const gentity_t* gEnt, traceType_t type) {
    ...
    CM_TransformedBoxTrace(&trace, vec3_origin, vec3_origin, mins, maxs, ch, MASK_ALL, 0, origin, angles, type);
...
To avoid linearly searching through lists of entities during environment
testing,
...
void G_CM_LinkEntity(gentity_t* gEnt) {
    ...
    if (gEnt->r.bmodel) {
        ...
        gEnt->r.originCluster =
                CM_LeafCluster(CM_PointLeafnum(gEnt->r.currentOrigin));
...
void G_CM_ClipToEntity(trace_t* trace, const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end, int entityNum, int contentmask, traceType_t type) {
    ...
    CM_TransformedBoxTrace(trace, (float*) start, (float*) end, (float*) mins, (float*) maxs, clipHandle, contentmask, 0, origin, angles, type);
...
void G_CM_ClipMoveToEntities(moveclip_t* clip) {
    ...
    num =
            G_CM_AreaEntities(clip->boxmins, clip->boxmaxs, touchlist, MAX_GENTITIES);
    ...
    for (i = 0; i < num; i++) {
        ...
        CM_TransformedBoxTrace(&trace, clip->start, clip->end, clip->mins, clip->maxs, clipHandle, clip->contentmask, 0, origin, angles, clip->collisionType);
...
==================
*/
void G_CM_Trace(trace_t* results, const vec3_t start, const vec3_t mins2, const vec3_t maxs2, const vec3_t end, int passEntityNum, int contentmask, int skipmask, traceType_t type) {
    moveclip_t clip;
    int i;
    ...
    clip.trace.entityNum =
            clip.trace.fraction != 1.0 ? ENTITYNUM_WORLD : ENTITYNUM_NONE;
