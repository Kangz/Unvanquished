...
/**
 * @brief Function to find who assisted most (and, in case of a tie, most
 * recently) with a kill
 * @param self
 * @param killer
 */
static const gentity_t* G_FindKillAssist(const gentity_t* self,
                                         const gentity_t* killer,
                                         team_t* team) {
    ...
    for (playerNum = 0; playerNum < level.maxclients; ++playerNum) {
        ...
        if (player == killer || player == self ||
            self->credits[playerNum].team <= TEAM_NONE) {
            continue;
        }

        if (self->credits[playerNum].value > damage ||
            (self->credits[playerNum].value == damage &&
             self->credits[playerNum].time > when)) {
            assistant = player;
...
void G_RewardAttackers(gentity_t* self) {
    ...
    for (playerNum = 0; playerNum < level.maxclients; playerNum++) {
        ...
        if (playerTeam == ownTeam || playerTeam <= TEAM_NONE ||
            playerTeam >= NUM_TEAMS) {
            continue;
        }
...
void G_PlayerDie(gentity_t* self, gentity_t* inflictor, gentity_t* attacker, int meansOfDeath) {
    ...
    if (assistant != ENTITYNUM_NONE) {
        G_LogPrintf("Die: %d %d %s %d %d: %s" S_COLOR_WHITE
                    " killed %s" S_COLOR_WHITE "; %s" S_COLOR_WHITE " assisted\n",
                    killer,
                    (int) (self - g_entities),
                    obit,
                    assistant,
    ...
    if (attacker && attacker->client) {
        if ((attacker == self || G_OnSameTeam(self, attacker))) {
            ...
            trap_SendServerCommand(
                    self - g_entities,
                    va("print_tr %s %s %3i", QQ(N_("Your killer, $1$^7, had $2$ HP.\n")), Quote(killerName), attacker->health));
        }
...

    {
        ...
        if (!(self->client->ps.persistant[PERS_STATE] & PS_NONSEGMODEL)) {
            self->client->ps.torsoAnim =
                    ((self->client->ps.torsoAnim & ANIM_TOGGLEBIT) ^ ANIM_TOGGLEBIT) |
                    anim;
        }
    ...
    for (count = 0;; count++) {
        ...
        while (1) {
            ...
            if (!Q_stricmp(token, "}")) {
                ...
                if (token[0]) {
                    Q_strncpyz(regions[count].name, token, sizeof(regions[count].name));
                }
...
float G_GetNonLocDamageMod(class_t pcl) {
    ...
    for (regionNum = 0; regionNum < g_numDamageRegions[pcl]; regionNum++) {
        ...
        if (g_debugDamage.integer > 1) {
            Com_Printf("GetNonLocDamageModifier( pcl = %s ): " S_COLOR_GREEN
                       "FOUND:" S_COLOR_WHITE " %.2f\n",
                       BG_Class(pcl)->name,
                       region->modifier);
        }
    ...
    if (g_debugDamage.integer > 1) {
        Com_Printf("GetNonLocDamageModifier( pcl = %s ): " S_COLOR_YELLOW
                   "NOT FOUND:" S_COLOR_WHITE " %.2f.\n",
                   BG_Class(pcl)->name,
                   1.0f);
    }
...
float G_GetPointDamageMod(gentity_t* target, class_t pcl, float angle, float height) {
    ...
    for (regionNum = 0; regionNum < g_numDamageRegions[pcl]; regionNum++) {
        ...
        if ((region->minAngle <= region->maxAngle &&
             (angle < region->minAngle || angle > region->maxAngle)) ||
            (region->minAngle > region->maxAngle &&
             (angle > region->maxAngle && angle < region->minAngle))) {
            continue;
        }

        if (g_debugDamage.integer > 1) {
            G_Printf(
                    "GetPointDamageModifier( pcl = %s, angle = %.2f, height = %.2f "
...
static float CalcDamageModifier(vec3_t point, gentity_t* target, class_t pcl, int damageFlags) {
    ...
    hitRotation =
            AngleNormalize360(target->client->ps.viewangles[YAW] - bulletAngle[YAW]);
...
void G_KnockbackByDir(gentity_t* target, const vec3_t direction, float strength, bool ignoreMass) {
    vec3_t dir, vel;
    int mass;
    float massMod;
    ...
    if (!target || !target->client || VectorLength(direction) == 0.0f ||
        strength == 0) {
        return;
    }
    ...
    // set pmove timer so that the client can't cancel out the movement
    // immediately
...
void G_Damage(gentity_t* target, gentity_t* inflictor, gentity_t* attacker, vec3_t dir, vec3_t point, int damage, int damageFlags, int mod) {
    gclient_t* client;
    int take, loss;
    int knockback;
    float modifier;
    ...
    if (target->s.eType == ET_MOVER && !(target->die || target->pain)) {
        ...
        if ((target->moverState == MOVER_POS1 ||
             target->moverState == ROTATOR_POS1) &&
            target->act) {
    ...
    if (client && (damageFlags & DAMAGE_KNOCKBACK) && dir) {
        ...
        if (inflictor->s.weapon != WP_NONE) {
            knockback =
                    (int) ((float) damage * BG_Weapon(inflictor->s.weapon)->knockbackScale);
        } else {
            knockback = damage;
        }
        ...
        G_KnockbackByDir(target, dir, knockback, false);
    } else {
    ...
    if (!(damageFlags & DAMAGE_NO_PROTECTION)) {
        ...
        if (target != attacker && G_OnSameTeam(target, attacker)) {
            ...
            // if dretchpunt is enabled and this is a dretch, do dretchpunt instead of
            // damage
        ...
        if (target->s.eType == ET_BUILDABLE && attacker->client &&
            mod != MOD_DECONSTRUCT && mod != MOD_SUICIDE && mod != MOD_REPLACE &&
            mod != MOD_NOCREEP) {
    ...
    if (client) {
        ...
        modifier = CalcDamageModifier(
                point, target, (class_t) client->ps.stats[STAT_CLASS], damageFlags);
    ...
    // TODO: gentity_t->nextRegenTime only affects alien clients, remove it and
    // use lastDamageTime
    // Optionally (if needed for some reason), move into client struct and add
    // "Alien" to name
    ...
    if (attacker != target) {
        ...
        if (attacker->client) {
            ...
            target->credits[attacker->client->ps.clientNum].team =
                    (team_t) attacker->client->pers.team;
...
bool G_SelectiveRadiusDamage(vec3_t origin, gentity_t* attacker, float damage, float radius, gentity_t* ignore, int mod, int ignoreTeam) {
    ...
    for (e = 0; e < numListedEntities; e++) {
        ...
        if (G_CanDamage(ent, origin) && ent->client &&
            ...
            G_Damage(ent, nullptr, attacker, nullptr, origin, (int) points, DAMAGE_RADIUS | DAMAGE_NO_LOCDAMAGE, mod);
        }
    }
...
bool G_RadiusDamage(vec3_t origin, gentity_t* attacker, float damage, float radius, gentity_t* ignore, int dflags, int mod, team_t testHit) {
    float points, dist;
    gentity_t* ent;
    int entityList[MAX_GENTITIES];
    int numListedEntities;
    ...
    for (e = 0; e < numListedEntities; e++) {
        ...
        if (G_CanDamage(ent, origin)) {
            if (testHit == TEAM_NONE) {
                ...
                VectorNormalize(dir);
                hitSomething = true;
                G_Damage(ent, nullptr, attacker, dir, origin, (int) points, (DAMAGE_RADIUS | DAMAGE_NO_LOCDAMAGE | dflags), mod);
...
void G_LogDestruction(gentity_t* self, gentity_t* actor, int mod) {
    ...
    //  when the messages appear attributed to the deconner. Just don't print
    //  them.
    ...
    if (actor->client &&
        actor->client->pers.team == BG_Buildable(self->s.modelindex)->team) {
        G_TeamCommand(
                (team_t) actor->client->pers.team,
                va("print_tr %s %s %s",
                   mod == MOD_DECONSTRUCT ? QQ(N_("$1$ ^3DECONSTRUCTED^7 by $2$\n"))
