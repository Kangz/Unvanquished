...
void G_ResetTimeField(variatingTime_t* result, variatingTime_t instanceField, variatingTime_t classField, variatingTime_t fallback) {
    ...
    if (result->variance < 0) {
        ...
        if (g_debugEntities.integer > 0) {
            G_Printf(S_WARNING
                     "limitting variance (%f) to be smaller than time (%f)\n",
                     result->variance,
                     result->time);
        }
    }
...
void SP_ConditionFields(gentity_t* self) {
    ...
    if (G_SpawnString("equipment", "", &buffer))
        BG_ParseCSVEquipmentList(buffer, self->conditions.weapons, WP_NUM_WEAPONS, self->conditions.upgrades, UP_NUM_UPGRADES);
}
