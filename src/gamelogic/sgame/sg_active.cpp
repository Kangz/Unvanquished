...
void PushBot(gentity_t* ent, gentity_t* other) {
    ...
    VectorMA(
            ...
            100 * ((level.time + (ent->s.number * 1000)) % 4000 < 2000 ? 1.0 : -1.0),
            r,
            other->client->ps.velocity);
    ...
    for (i = 0; i < pm->numtouch; i++) {
        ...
        if (other->client) {
            ...
            if ((other->r.svFlags & SVF_BOT) &&
                ent->client->pers.team == other->client->pers.team) {
                PushBot(ent, other);
            }
...
void G_TouchTriggers(gentity_t* ent) {
    ...
    BG_ClassBoundingBox(ent->client->ps.stats[STAT_CLASS], pmins, pmaxs, nullptr, nullptr, nullptr);
...
void SpectatorThink(gentity_t* ent, usercmd_t* ucmd) {
    ...
    if (team == TEAM_ALIENS || team == TEAM_HUMANS) {
        client->ps.persistant[PERS_UNLOCKABLES] =
                BG_UnlockablesMask(client->pers.team);
    ...
    if (!following) {
        ...
        if (client->ps.pm_flags & PMF_QUEUED) {
            ...
            client->ps.persistant[PERS_SPAWNQUEUE] |=
                    G_GetPosInSpawnQueue(&level.team[team].spawnQueue,
                                         client->ps.clientNum)
                    << 8;
        }
    }
...
bool ClientInactivityTimer(gentity_t* ent, bool active) {
    ...
    if (!g_inactivity.integer) {
    ...
    } else if (active || client->pers.cmd.forwardmove ||
        ...
        if (level.time > client->inactivityTime &&
            ...
            if (strchr(g_inactivity.string, 's')) {
                trap_SendServerCommand(
                        -1,
                        va("print_tr %s %s %s",
                           QQ(N_("$1$^7 moved from $2$ to spectators due to inactivity\n")),
                           Quote(client->pers.netname),
                           Quote(BG_TeamName(client->pers.team))));
        ...
        if (level.time > client->inactivityTime - 10000 &&
            !client->inactivityWarning && !G_admin_permission(ent, ADMF_ACTIVITY)) {
            client->inactivityWarning = true;
            trap_SendServerCommand(
                    client - level.clients,
                    va("cp_tr %s",
                       strchr(g_inactivity.string, 's')
                               ? N_("\"Ten seconds until inactivity spectate!\n\"")
...
static void G_ReplenishHumanHealth(gentity_t* self) {
    ...
    remainingStartupTime =
            MEDKIT_STARTUP_TIME - (level.time - client->lastMedKitTime);
    ...
    if (remainingStartupTime > 0) {
        if (level.time < client->medKitIncrementTime) {
            ...
            client->medKitIncrementTime =
                    level.time + (remainingStartupTime / MEDKIT_STARTUP_SPEED);
        }
    }
...
static void BeaconAutoTag(gentity_t* self, int timePassed) {
    ...
    traceEnt =
            Beacon::TagTrace(viewOrigin, end, self->s.number, MASK_SHOT, team, true);
    G_UnlaggedOff();

    client->ps.stats[STAT_TAGSCORE] = 0;

    for (target = nullptr; (target = G_IterateEntities(target));) {
        // Tag entity directly hit and entities in human radar range, make sure the
        // latter are also
        ...
        if ((target == traceEnt) ||
            ...
            client->ps.stats[STAT_TAGSCORE] =
                    Maths::clamp(target->tagScore, client->ps.stats[STAT_TAGSCORE], 1000);
        }
    }
}
...
void ClientTimerActions(gentity_t* ent, int msec) {
    ...
    while (client->time100 >= 100) {
        ...
        switch (weapon) {
            ...
            case WP_HBUILD:
                buildable =
                        (buildable_t)(client->ps.stats[STAT_BUILDABLE] & SB_BUILDABLE_MASK);
                ...
                if (buildable > BA_NONE) {
                    ...
                    client->ps.stats[STAT_BUILDABLE] |= SB_BUILDABLE_FROM_IBE(
                            G_CanBuild(ent, buildable, dist, dummy, dummy2, &dummy3));

                    if (buildable == BA_H_DRILL || buildable == BA_A_LEECH) {
                        client->ps.stats[STAT_PREDICTION] =
                                (int) (G_RGSPredictEfficiencyDelta(
                                               dummy, (team_t) ps->persistant[PERS_TEAM]) *
                                       100.0f);
                    }
        ...
        // refill weapon ammo
        if (ent->client->lastAmmoRefillTime + HUMAN_AMMO_REFILL_PERIOD <
                    level.time &&
            ps->weaponTime == 0) {
            G_FindAmmo(ent);
        }
        ...
        // remove orphaned tags for enemy structures when the structure's death was
        // confirmed
        {
            gentity_t* other = nullptr;
            while ((other = G_IterateEntities(other))) {
                if (other->s.eType == ET_BEACON && other->s.modelindex == BCT_TAG &&
                    (other->s.eFlags & EF_BC_ENEMY) && !other->tagAttachment &&
    ...
    while (client->time1000 >= 1000) {
        ...
        if (client->ps.stats[STAT_STATE] & SS_POISONED) {
            G_Damage(ent, client->lastPoisonClient, client->lastPoisonClient, nullptr, nullptr, ALIEN_POISON_DMG, DAMAGE_NO_LOCDAMAGE, MOD_POISON);
        }

        // turn off life support when a team admits defeat
        if (client->pers.team == TEAM_ALIENS &&
            level.surrenderTeam == TEAM_ALIENS) {
            G_Damage(ent, nullptr, nullptr, nullptr, nullptr, BG_Class(client->ps.stats[STAT_CLASS])->regenRate, DAMAGE_NO_ARMOR, MOD_SUICIDE);
    ...
    for (i = oldEventSequence; i < client->ps.eventSequence; i++) {
        ...
        switch (event) {
            ...
            case EV_FALL_FAR:
                ...
                fallDistance =
                        ((float) client->ps.stats[STAT_FALLDIST] - MIN_FALL_DISTANCE) /
...
void G_UnlaggedCalc(int time, gentity_t* rewindEnt) {
    ...
    for (i = 0; i < level.maxclients; i++) {
        ...
        VectorLerpTrem(lerp, ent->client->unlaggedHist[startIndex].mins, ent->client->unlaggedHist[stopIndex].mins, ent->client->unlaggedCalc.mins);
        VectorLerpTrem(lerp, ent->client->unlaggedHist[startIndex].maxs, ent->client->unlaggedHist[stopIndex].maxs, ent->client->unlaggedCalc.maxs);
        VectorLerpTrem(lerp, ent->client->unlaggedHist[startIndex].origin, ent->client->unlaggedHist[stopIndex].origin, ent->client->unlaggedCalc.origin);
...
static void G_UnlaggedDetectCollisions(gentity_t* ent) {
    ...
    trap_Trace(&tr, ent->client->oldOrigin, ent->r.mins, ent->r.maxs, ent->client->ps.origin, ent->s.number, MASK_PLAYERSOLID, 0);
...
static int FindAlienHealthSource(gentity_t* self) {
    ...
    for (ent = nullptr;
         (ent = G_IterateEntities(ent, nullptr, true, 0, nullptr));) {
        ...
        if (ent->client && self != ent && distance < REGEN_TEAMMATE_RANGE &&
            ...
            if (ent->s.modelindex == BA_A_BOOSTER && ent->powered &&
                ...
                // Creep healing
                ret |= SS_HEALING_4X;
            } else if ((ent->s.modelindex == BA_A_OVERMIND ||
                        ent->s.modelindex == BA_A_SPAWN) &&
    ...
    if (ret & SS_HEALING_8X)
        ret |= SS_HEALING_4X;
    if (ret & SS_HEALING_4X)
        ret |= SS_HEALING_2X;
...
static void G_ReplenishAlienHealth(gentity_t* self) {
    ...
    if (!client || client->pers.team != TEAM_ALIENS || self->health <= 0 ||
    ...
    client->ps.stats[STAT_STATE] &=
            ~(SS_HEALING_2X | SS_HEALING_4X | SS_HEALING_8X);
    ...
    if (self->nextRegenTime < level.time) {
        if (client->ps.stats[STAT_STATE] & SS_HEALING_8X) {
            ...
            if (g_alienOffCreepRegenHalfLife.value < 1) {
                ...
                // Exponentially decrease healing rate when not on creep. ln(2) ~=
                // 0.6931472
                modifier =
                        exp((0.6931472f / (1000.0f * g_alienOffCreepRegenHalfLife.value)) *
                            (self->healthSourceTime - level.time));
...
void ClientThink_real(gentity_t* self) {
    ...
    // don't think if the client is not yet connected (and thus not yet spawned
    // in)
    ...
    if (level.pmoveParams.fixed || client->pers.pmoveFixed) {
        ucmd->serverTime = ((ucmd->serverTime + level.pmoveParams.msec - 1) /
    ...
    if (BG_InventoryContainsUpgrade(UP_MEDKIT, client->ps.stats) &&
        ...
        if ((client->ps.stats[STAT_STATE] & SS_HEALING_4X) ||
            ...
            client->medKitIncrementTime =
                    level.time + (MEDKIT_STARTUP_TIME / MEDKIT_STARTUP_SPEED);
    ...
    if (client->ps.pm_type == PM_NOCLIP) {
        ...
        client->ps.speed =
                g_speed.value * BG_Class(client->ps.stats[STAT_CLASS])->speed;
    }
    ...
    switch (client->ps.weapon) {
        ...
        case WP_ALEVEL4:
            ...
            if (!(client->ps.pm_flags & PMF_CHARGE) &&
                client->trampleBuildablesHitPos) {
    ...
    // NOTE: now copy the exact origin over otherwise clients can be snapped into
    // solid
    ...
    client->ps.persistant[PERS_BP] =
            G_GetBuildPointsInt((team_t) client->pers.team);
    client->ps.persistant[PERS_MARKEDBP] =
            G_GetMarkedBuildPointsInt((team_t) client->pers.team);
...
void ClientThink(int clientNum) {
    ...
    // mark the time we got info, so we can display the phone jack if we don't get
    // any for a while
