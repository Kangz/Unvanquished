...
void InitBrushSensor(gentity_t* self) {
    ...
    self->r.contents =
            CONTENTS_SENSOR; // replaces the -1 from trap_SetBrushModel
    self->r.svFlags = SVF_NOCLIENT;
    trap_LinkEntity(self);
}

void sensor_act(gentity_t* self, gentity_t* other, gentity_t* activator) {
    // if we wanted to tell the cgame about our deactivation, this would be the
    // way to do it
    // self->s.eFlags ^= EF_NODRAW;
    // but unless we have to, we rather not share the information, so "patched"
    // clients cannot do anything with it either
...
// some old sensors/triggers used to propagate use-events, this is deprecated
// behavior
void trigger_compat_propagation_act(gentity_t* self, gentity_t* other, gentity_t* activator) {
    ...
    if (g_debugEntities.integer >=
        -1) // dont't warn about anything with -1 or lower
    {
        G_Printf(S_ERROR
                 "It appears as if %s is targeted by %s to enforce firing, which "
                 "is undefined behavior — stop doing that! This WILL break in "
                 "future releases and toggle the sensor instead.\n",
                 etos(self),
                 etos(activator));
    }
}
...
void trigger_multiple_compat_reset(gentity_t* self) {
    if (!!(self->spawnflags & 1) !=
        !!(self->spawnflags &
           2)) // if both are set or none are set we assume TEAM_ALL
    {
        if (self->spawnflags & 1)
    ...
    if (self->spawnflags &&
        g_debugEntities.integer >=
                -1) // dont't warn about anything with -1 or lower
    {
        G_Printf(S_ERROR
                 "It appears as if %s has set spawnflags that were not defined "
                 "behavior of the entities.def; this is likely to break in the "
                 "future\n",
                 etos(self));
    }
}
...
void SP_sensor_start(gentity_t* self) {
    // self->think = sensor_start_fireAndForget; //gonna reuse that later, when we
    // make sensor_start delayable again (configurable though)
}
...
void G_notify_sensor_start() {
    ...
    while ((sensor = G_IterateEntitiesOfClass(sensor, S_SENSOR_START)) !=
           nullptr) {
...
void SP_sensor_timer(gentity_t* self) {
    SP_WaitFields(self, 1.0f,
                  (self->classname[0] == 'f') ? 1.0f : 0.0f); // wait variance default only for func_timer
...
void G_notify_sensor_stage(team_t team, int previousStage, int newStage) {
    ...
    if (g_debugEntities.integer >= 2)
        G_Printf(S_DEBUG
                 "Notification of team %i changing stage from %i to %i (0-2).\n",
                 team,
                 previousStage,
                 newStage);

    if (newStage <= previousStage) // not supporting stage down yet, also no need
                                   // to fire if stage didn't change at all
        return;

    while ((entities = G_IterateEntitiesOfClass(entities, S_SENSOR_STAGE)) !=
           nullptr) {
        if (((!entities->conditions.stage ||
              newStage == entities->conditions.stage) &&
...
bool sensor_class_match(gentity_t* self, gentity_t* activator) {
    ...
    if (self->conditions.classes[i] == PCL_NONE) {
        ...
        for (i = 0; self->conditions.classes[i] != PCL_NONE; i++) {
            if (activator->client->ps.stats[STAT_CLASS] ==
                self->conditions.classes[i]) {
                return true;
            }
        }
    }
...
bool sensor_equipment_match(gentity_t* self, gentity_t* activator) {
    ...
    if (self->conditions.weapons[i] == WP_NONE &&
        self->conditions.upgrades[i] == UP_NONE) {
        // if there is no equipment list all equipment triggers for the old behavior
        // of target_equipment, but not the new or different one
        return true;
    } else {
        // otherwise check against the lists
        for (i = 0; self->conditions.weapons[i] != WP_NONE; i++) {
            if (BG_InventoryContainsWeapon(self->conditions.weapons[i],
        ...
        for (i = 0; self->conditions.upgrades[i] != UP_NONE; i++) {
            if (BG_InventoryContainsUpgrade(self->conditions.upgrades[i],
                                            activator->client->ps.stats)) {
                return true;
            }
        }
...
void sensor_player_touch(gentity_t* self, gentity_t* activator, trace_t* trace) {
    ...
    if (self->conditions.team &&
        (activator->client->pers.team != self->conditions.team))
        return;

    if ((self->conditions.upgrades[0] || self->conditions.weapons[0]) &&
        activator->client->pers.team == TEAM_HUMANS) {
        shouldFire = sensor_equipment_match(self, activator);
    } else if (self->conditions.classes[0] &&
