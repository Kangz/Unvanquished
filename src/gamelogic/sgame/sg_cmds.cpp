...
void ScoreboardMessage(gentity_t* ent) {
    ...
    for (i = 0; i < numSorted; i++) {
        ...
        Com_sprintf(entry, sizeof(entry), " %d %d %d %d %d %d", level.sortedClients[i], cl->ps.persistant[PERS_SCORE], ping, (level.time - cl->pers.enterTime) / 60000, weapon, upgrade);
    ...
    trap_SendServerCommand(ent - g_entities,
                           va("scores %i %i%s", level.team[TEAM_ALIENS].kills, level.team[TEAM_HUMANS].kills, string));
}
...
void Cmd_Give_f(gentity_t* ent) {
    ...
    if (trap_Argc() < 2) {
        ...
        ADMP(QQ(
                N_("usage: valid choices are: all, health [amount], funds [amount], "
    ...
    if (g_cheats.integer) {
        ...
        if (ent->suicideTime == 0) {
            trap_SendServerCommand(
                    ent - g_entities,
                    "print_tr \"" N_("You will suicide in 20 seconds\n") "\"");
            ent->suicideTime = level.time + 20000;
        } else if (ent->suicideTime > level.time) {
            trap_SendServerCommand(ent - g_entities,
                                   "print_tr \"" N_("Suicide cancelled\n") "\"");
            ent->suicideTime = 0;
        }
    }
}
...
void Cmd_Team_f(gentity_t* ent) {
    ...
    if (!g_cheats.integer && g_combatCooldown.integer &&
        ent->client->lastCombatTime &&
        ent->client->sess.spectatorState == SPECTATOR_NOT && ent->health > 0 &&
        ent->client->lastCombatTime + g_combatCooldown.integer * 1000 >
                level.time) {
        float remaining =
                ((ent->client->lastCombatTime + g_combatCooldown.integer * 1000) -
                 level.time) /
        ...
        trap_SendServerCommand(
                ent - g_entities,
                va("print_tr %s %i %.0f", QQ(N_("You cannot leave your team until $1$ "
                                                "after combat. Try again in $2$s.\n")),
    ...
    if (!Q_stricmp(s, "auto")) {
        if (level.team[TEAM_HUMANS].locked && level.team[TEAM_ALIENS].locked) {
            team = TEAM_NONE;
                   ...
                   players[TEAM_HUMANS] > players[TEAM_ALIENS]) {
            team = TEAM_ALIENS;
        }

        else if (level.team[TEAM_ALIENS].locked ||
                 players[TEAM_ALIENS] > players[TEAM_HUMANS]) {
            team = TEAM_HUMANS;
        } else {
    ...
    if (team != TEAM_NONE && (specOnly = G_admin_match_spec(ent))) {
        if (specOnly->expires == -1) {
            trap_SendServerCommand(
                    ent - g_entities,
                    "print_tr \"" N_(
                            "You cannot join a team until the next game.\n") "\"");
            return;
        }
        ...
        if (specOnly->expires - t > 0) {
            ...
            trap_SendServerCommand(
                    ent - g_entities,
                    va("print_tr %s %d",
                       QQ(N_("You cannot join a team for another $1$s.\n")),
                       remaining));
            return;
        }
    }
...
static bool G_SayTo(gentity_t* ent, gentity_t* other, saymode_t mode, const char* message) {
    ...
    if (mode == SAY_ALL_ADMIN) {
        trap_SendServerCommand(
                other - g_entities,
                va("achat %s %d %s", G_quoted_admin_name(ent), mode, Quote(message)));
    } else {
        trap_SendServerCommand(
                other - g_entities,
                va("chat %ld %d %s", ent ? (long) (ent - g_entities) : -1, mode, Quote(message)));
    }
...
void G_Say(gentity_t* ent, saymode_t mode, const char* chatText) {
    ...
    if ((!g_specChat.integer) && (mode != SAY_TEAM) && (ent) &&
        (ent->client->pers.team == TEAM_NONE) &&
        (!G_admin_permission(ent, ADMF_NOCENSORFLOOD))) {
        trap_SendServerCommand(ent - g_entities,
                               "print_tr \"" N_(
                                       "say: Global chatting for "
                                       "spectators has been disabled. You may only use "
                                       "team chat.\n") "\"");
        mode = SAY_TEAM;
    }
...
void Cmd_VSay_f(gentity_t* ent) {
    ...
    if (trap_Argc() < 2) {
        trap_SendServerCommand(
                ent - g_entities,
                va("print_tr %s %s", QQ(N_("usage: $1$ command [text]\n")), arg));
        return;
    }
    ...
    if (!level.voices) {
        trap_SendServerCommand(
                ent - g_entities,
                va("print_tr %s %s",
                   QQ(N_("$1$: voice system is not installed on this server\n")),
                   arg));
        return;
    }
    ...
    if (!g_enableVsays.integer) {
        trap_SendServerCommand(
                ...
                va("print_tr %s %s",
                   QQ(N_(
                           "$1$: voice system administratively disabled on this server\n")),
                   arg));
        return;
    }
    ...
    if (!voice) {
        trap_SendServerCommand(
                ent - g_entities,
                va("print_tr %s %s %s", QQ(N_("$1$: voice '$2$' not found\n")), vsay, Quote(voiceName)));
        return;
    }
    ...
    if (!cmd) {
        trap_SendServerCommand(
                ent - g_entities,
                va("print_tr %s %s %s %s",
                   QQ(N_("$1$: command '$2$' not found in voice '$3$'\n")),
                   vsay,
                   Quote(voiceCmd),
                   Quote(voiceName)));
        return;
    ...
    track = BG_VoiceTrackFind(cmd->tracks, ent->client->pers.team, ent->client->pers.classSelection, weapon, (int) ent->client->voiceEnthusiasm, &trackNum);

    if (!track) {
        trap_SendServerCommand(
                ent - g_entities,
                va("print_tr %s %s %s %d %d %d %d %s",
                   QQ(N_(
                           "$1$: no available track for command '$2$', team $3$, "
                   ...
                   weapon,
                   (int) ent->client->voiceEnthusiasm,
                   Quote(voiceName)));
        return;
    }
    ...
    switch (vchan) {
        case VOICE_CHAN_ALL:
            trap_SendServerCommand(
                    -1, va("voice %ld %d %d %d %s\n", (long) (ent - g_entities), vchan, cmdNum, trackNum, Quote(text)));
            break;

        case VOICE_CHAN_TEAM:
            G_TeamCommand((team_t) ent->client->pers.team,
                          va("voice %ld %d %d %d %s\n", (long) (ent - g_entities), vchan, cmdNum, trackNum, Quote(text)));
            break;

        case VOICE_CHAN_LOCAL:
            G_AreaTeamCommand(
                    ent, va("voice %ld %d %d %d %s\n", (long) (ent - g_entities), vchan, cmdNum, trackNum, Quote(text)));
            break;
...
static const struct {
    ...
    const vmCvar_t* reasonFlag; // where a reason requirement is configurable
                                // (reasonNeeded must be true)
} voteInfo[] = {
        // Name           Stop?   Type      Target     Immune   Quorum  Reason  Vote
        // percentage var        Extra
...
bool G_CheckStopVote(team_t team) {
    return level.team[team].voteTime &&
           voteInfo[level.team[team].voteType].stopOnIntermission;
}
...
void Cmd_CallVote_f(gentity_t* ent) {
    char cmd[MAX_TOKEN_CHARS], vote[MAX_TOKEN_CHARS], arg[MAX_TOKEN_CHARS];
    ...
    trap_Argv(0, cmd, sizeof(cmd));
    team = (team_t)((!Q_stricmp(cmd, "callteamvote")) ? ent->client->pers.team
                                                      : TEAM_NONE);
    ...
    if (!voteInfo[voteId].name) {
        ...
        trap_SendServerCommand(ent - g_entities,
                               "print_tr \"" N_("Invalid vote string\n") "\"");
        trap_SendServerCommand(
                ent - g_entities,
                va("print_tr %s", team == TEAM_NONE ? QQ(N_("Valid vote commands are: ")) : QQ(N_("Valid team-vote commands are: "))));
        cmd[0] = '\0';
        ...
        for (voteId = 0; voteInfo[voteId].name; ++voteId) {
            if ((team == TEAM_NONE && voteInfo[voteId].type != V_TEAM) ||
                (team != TEAM_NONE && voteInfo[voteId].type != V_PUBLIC)) {
                if (!voteInfo[voteId].percentage ||
                    voteInfo[voteId].percentage->integer > 0) {
    ...
    // Does not distinguish between public and team votes
    {
        ...
        while (*dv) {
            ...
            if (delim - dv == voteNameLength &&
                !Q_strnicmp(dv, vote, voteNameLength)) {
    ...
    if (g_voteLimit.integer > 0 &&
        ...
        trap_SendServerCommand(
                ent - g_entities,
                va("print_tr %s %s %d", QQ(N_("$1$: you have already called the "
                                              "maximum number of votes ($2$)\n")),
                   cmd,
                   g_voteLimit.integer));
        return;
    }
    ...
    level.team[team].voteThreshold =
            voteInfo[voteId].percentage ? voteInfo[voteId].percentage->integer : 50;
    ...
    vote_is_disabled:
        trap_SendServerCommand(
                ent - g_entities,
                va("print_tr %s %s", QQ(N_("'$1$' votes have been disabled\n")), voteInfo[voteId].name));
        return;
    }
    ...
    switch (voteInfo[voteId].special) {
        case VOTE_BEFORE:
            if (level.matchTime >= (voteInfo[voteId].specialCvar->integer * 60000)) {
                trap_SendServerCommand(
                        ent - g_entities,
                        va("print_tr %s %s %d", QQ(N_("'$1$' votes are not allowed once "
                                                      "$2$ minutes have passed\n")),
                           voteInfo[voteId].name,
                           voteInfo[voteId].specialCvar->integer));
                return;
            }
        ...
        case VOTE_AFTER:
            if (level.matchTime < (voteInfo[voteId].specialCvar->integer * 60000)) {
                trap_SendServerCommand(
                        ent - g_entities,
                        va("print_tr %s %s %d", QQ(N_("'$1$' votes are not allowed until "
                                                      "$2$ minutes have passed\n")),
                           voteInfo[voteId].name,
                           voteInfo[voteId].specialCvar->integer));
                return;
        ...
        case VOTE_REMAIN:
            if (!level.timelimit ||
                level.matchTime <
                        (level.timelimit - voteInfo[voteId].specialCvar->integer / 2) *
                                60000) {
                trap_SendServerCommand(ent - g_entities,
                                       va("print_tr %s %s %d",
                                          QQ(
                                                  N_("'$1$' votes are only allowed with "
    ...
    if (voteInfo[voteId].target == T_PLAYER) {
        ...
        if (voteInfo[voteId].adminImmune &&
            G_admin_permission(g_entities + clientNum, ADMF_IMMUNITY)) {
            trap_SendServerCommand(
                    ent - g_entities,
    ...
    switch (voteId) {
        case VOTE_KICK:
            Com_sprintf(
                    level.team[team].voteString, sizeof(level.team[team].voteString), "ban %s 1s%s %s ^7called vote kick (%s^7)", level.clients[clientNum].pers.ip.str, Quote(g_adminTempBan.string), Quote(ent->client->pers.netname), Quote(reason));
        ...
        case VOTE_SPECTATE:
            Com_sprintf(level.team[team].voteString,
                        sizeof(level.team[team].voteString),
                        "speclock %d 1s%s",
                        clientNum,
                        Quote(g_adminTempBan.string));
            Com_sprintf(level.team[team].voteDisplayString,
                        sizeof(level.team[team].voteDisplayString),
                        N_("Move player '%s' to spectators"),
                        name);
            break;
        ...
        case VOTE_BOT_SPECTATE:
            ...
            if (voteId == VOTE_BOT_KICK) {
                ...
                Com_sprintf(level.team[team].voteDisplayString,
                            sizeof(level.team[team].voteDisplayString),
                            N_("Move all bots to spectators"));
            }
            break;
        ...
        case VOTE_DENYBUILD:
            if (level.clients[clientNum].pers.namelog->denyBuild) {
                trap_SendServerCommand(
                        ent - g_entities,
                        va("print_tr %s %s",
                           QQ(N_("$1$: player already lost building rights\n")),
                           cmd));
                return;
            }
            ...
            Com_sprintf(level.team[team].voteDisplayString,
                        sizeof(level.team[team].voteDisplayString),
                        "Take away building rights from '%s'",
                        name);
            break;
        ...
        case VOTE_ALLOWBUILD:
            if (!level.clients[clientNum].pers.namelog->denyBuild) {
                trap_SendServerCommand(
                        ent - g_entities,
                        va("print_tr %s %s",
                           QQ(N_("$1$: player already has building rights\n")),
                           cmd));
                return;
            }
        ...
        case VOTE_EXTEND:
            Com_sprintf(level.team[team].voteString,
                        sizeof(level.team[team].voteString),
                        "time %i",
                        level.timelimit + g_extendVotesTime.integer);
            Com_sprintf(level.team[team].voteDisplayString,
                        sizeof(level.team[team].voteDisplayString),
        ...
        case VOTE_NEXT_MAP:
            if (G_MapExists(g_nextMap.string)) {
                trap_SendServerCommand(
                        ent - g_entities,
                        va("print_tr %s %s %s",
                           QQ(N_("$1$: the next map is already set to '$2$'\n")),
                           cmd,
            ...
            if (*reason) // layout?
            {
                Com_sprintf(level.team[team].voteString,
                            sizeof(level.team[team].voteString),
                            "set g_nextMap %s; set g_nextMapLayouts %s",
                            Quote(arg),
                            Quote(reason));
                Com_sprintf(level.team[team].voteDisplayString,
                            sizeof(level.team[team].voteDisplayString),
                            "Set the next map to '%s' layout '%s'",
                            arg,
                            reason);
            } else {
                Com_sprintf(level.team[team].voteString,
                            sizeof(level.team[team].voteString),
                            "set g_nextMap %s; set g_nextMapLayouts \"\"",
                            Quote(arg));
                Com_sprintf(level.team[team].voteDisplayString,
                            sizeof(level.team[team].voteDisplayString),
                            "Set the next map to '%s'",
                            arg);
            }
            ...
            if (level.clients[i].pers.connected == CON_CONNECTED) {
                if (level.clients[i].pers.team == team ||
                    ...
                    trap_SendServerCommand(i,
                                           va("print_tr %s %s %s",
                                              QQ(N_("$1$^7 called a team vote: $2t$\n")),
                    ...
                    trap_SendServerCommand(
                            i,
                            va("chat -1 %d " S_COLOR_YELLOW "%s\"" S_COLOR_YELLOW
                               " called a team vote (%ss): \"%s",
                               SAY_ADMINS,
                               Quote(ent->client->pers.netname),
                               BG_TeamName(team),
                               Quote(level.team[team].voteDisplayString)));
    ...
    trap_SetConfigstring(CS_VOTE_CALLER + team, caller);
...
static bool Cmd_Class_internal(gentity_t* ent, const char* s, bool report) {
    ...
    if (ent->client->sess.spectatorState != SPECTATOR_NOT) {
        ...
        if (team == TEAM_ALIENS) {
            if (newClass != PCL_ALIEN_BUILDER0 &&
                newClass != PCL_ALIEN_BUILDER0_UPG && newClass != PCL_ALIEN_LEVEL0) {
                if (report) {
    ...
    if (ent->health <= 0) {
        return true; // dead, can't evolve; no point in trying other classes (if
                     // any listed)
    }
            ...
            for (i = 0; i < num; i++) {
                ...
                if ((other->client && other->client->pers.team == TEAM_HUMANS) ||
                    (other->s.eType == ET_BUILDABLE &&
                     other->buildableTeam == TEAM_HUMANS && other->powered)) {
                    if (report) {
...
void Cmd_Class_f(gentity_t* ent) {
    ...
    for (i = 1; i <= args; ++i) {
        ...
        if (Cmd_Class_internal(ent, s, i == args))
            break;
    }
}
...
void Cmd_Deconstruct_f(gentity_t* ent) {
    ...
    if (trace.fraction >= 1.0f || buildable->s.eType != ET_BUILDABLE ||
    ...
    if (instant && buildable->deconstruct) {
        if (!g_cheats.integer) {
            ...
            switch (buildable->s.modelindex) {
                ...
                case BA_A_SPAWN:
                case BA_H_SPAWN:
                    if (level.team[ent->client->ps.persistant[PERS_TEAM]].numSpawns <=
                        1) {
            ...
            ent->client->ps.stats[STAT_MISC] +=
                    BG_Buildable(buildable->s.modelindex)->buildTime / 4;
        }
...
void Cmd_Ignite_f(gentity_t* player) {
    ...
    if (!target || target->s.eType != ET_BUILDABLE ||
...
void Cmd_ActivateItem_f(gentity_t* ent) {
    ...
    if (upgrade != UP_NONE &&
        ...
        trap_SendServerCommand(
                ent - g_entities,
                va("print_tr %s %s", QQ(N_("You don't have the $1$\n")), Quote(s)));
    }
}
...
static bool Cmd_Sell_upgradeItem(gentity_t* ent, upgrade_t item) {
    ...
    if (item == UP_LIGHTARMOUR || item == UP_MEDIUMARMOUR ||
        item == UP_BATTLESUIT) {
        vec3_t newOrigin;
...
static bool Cmd_Sell_internal(gentity_t* ent, const char* s) {
    ...
    if (weapon != WP_NONE) {
        ...
        if (!BG_Weapon(weapon)->purchasable) {
            trap_SendServerCommand(
                    ent - g_entities,
                    "print_tr \"" N_("You can't sell this weapon\n") "\"");
            return false;
        }
        ...
        if (!BG_Upgrade(upgrade)->purchasable) {
            trap_SendServerCommand(
                    ent - g_entities,
                    "print_tr \"" N_("You can't sell this item\n") "\"");
            return false;
        }
...
#define Maybe_TriggerMenu(num, reason)      \
    do {                                    \
        if (!quiet)                         \
            G_TriggerMenu((num), (reason)); \
    } while (0)
    weapon_t weapon;
    upgrade_t upgrade;
    vec3_t newOrigin;
    ...
    if (weapon != WP_NONE) {
        ...
        for (;;) {
            ...
            if (BG_Weapon(weapon)->slots &
                BG_SlotsForInventory(ent->client->ps.stats)) {
                // if requested, try to sell then recheck
                if (sellConflicting && Cmd_Sell_weapons(ent)) {
                    continue;
                }
...
cant_buy:
    trap_SendServerCommand(
            ent - g_entities,
            va("print_tr \"" N_("You can't buy this item ($1$)\n") "\" %s",
               Quote(s)));
    return false;

not_alien:
    trap_SendServerCommand(ent - g_entities,
                           "print_tr \"" N_("You can't buy alien items\n") "\"");
    return false;
}
...
void Cmd_Buy_f(gentity_t* ent) {
    ...
    for (; c < args; ++c) {
        ...
        switch (s[0]) {
            ...
            case '+':
                updated |=
                        Cmd_Buy_internal(ent, s + 1, true, false); // auto-sell if needed
                break;
            case '?':
...
void Cmd_Build_f(gentity_t* ent) {
    ...
    if (buildable != BA_NONE &&
        ...
        ent->client->ps.stats[STAT_BUILDABLE] =
                BA_NONE | SB_BUILDABLE_FROM_IBE(reason);

        // these are the errors displayed when the builder first selects something
        // to use
        switch (reason) {
...
bool G_FollowNewClient(gentity_t* ent, int dir) {
    ...
    do {
        ...
        if (ent->client->pers.team != TEAM_NONE &&
            (level.clients[clientnum].pers.team != ent->client->pers.team)) {
            continue;
        }
...
void Cmd_Follow_f(gentity_t* ent) {
    ...
    if (trap_Argc() != 2) {
        ...
        if (ent->client->pers.team != TEAM_NONE &&
            (level.clients[i].pers.team != ent->client->pers.team)) {
            return;
        }
...
static void Cmd_Ignore_f(gentity_t* ent) {
    ...
    if (trap_Argc() < 2) {
        trap_SendServerCommand(
                ent - g_entities,
                va("print_tr \"" S_SKIPNOTIFY "%s\" %s",
                   N_("usage: $1$ [clientNum | partial name match]\n"),
                   cmd));
        return;
    }
    ...
    if (matches < 1) {
        trap_SendServerCommand(
                ent - g_entities,
                va("print_tr \"" S_SKIPNOTIFY "%s\" %s %s",
                   N_("$1$: no clients match the name '$2$'\n"),
                   cmd,
                   Quote(name)));
        return;
    }
        ...
        if (ignore) {
            if (!Com_ClientListContains(&ent->client->sess.ignoreList, pids[i])) {
                ...
                trap_SendServerCommand(
                        ent - g_entities,
                        va("print_tr \"" S_SKIPNOTIFY "%s\" %s",
                           N_("ignore: added $1$^7 to your ignore list\n"),
                           Quote(level.clients[pids[i]].pers.netname)));
            } else {
                trap_SendServerCommand(
                        ent - g_entities,
                        va("print_tr \"" S_SKIPNOTIFY "%s\" %s",
                           N_("ignore: $1$^7 is already on your ignore list\n"),
                           Quote(level.clients[pids[i]].pers.netname)));
            }
        } else {
            if (Com_ClientListContains(&ent->client->sess.ignoreList, pids[i])) {
                ...
                trap_SendServerCommand(
                        ent - g_entities,
                        va("print_tr \"" S_SKIPNOTIFY "%s\" %s",
                           N_("unignore: removed $1$^7 from your ignore list\n"),
                           Quote(level.clients[pids[i]].pers.netname)));
            } else {
                trap_SendServerCommand(
                        ent - g_entities,
                        va("print_tr \"" S_SKIPNOTIFY "%s\" %s",
                           N_("unignore: $1$^7 is not on your ignore list\n"),
                           Quote(level.clients[pids[i]].pers.netname)));
            }
        }
    }
...
void Cmd_ListMaps_f(gentity_t* ent) {
    ...
    for (size_t i = 0; i < paks.size(); ++i) {
        ...
        if (Q_strncmp("map-", paks[i].name.c_str(), 4) ||
            (search[0] && !strstr(paks[i].name.c_str(), search))) {
            continue;
        }
    ...
    if (pages > 1 && page + 1 < pages) {
        ADMP(va("%s %d %d %s %s %d", QQ(N_("^3listmaps: ^7page $1$ of $2$; use "
                                           "'listmaps $3$$4$$5$' to see more")),
                page + 1,
                pages,
                Quote(search),
                (search[0]) ? " " : "",
...
void G_MapLog_NewMap() {
    ...
    trap_Cvar_Set("g_mapLog", va("%s%s%s", map, maplog[0] ? " " : "", maplog));
}
...
void Cmd_MapLog_f(gentity_t* ent) {
    ...
    while (*ptr) {
        ...
        ADMBP(va("  ^%s%-20s %6s %s^7\n", ptr == maplog ? "2" : "7", ptr, clock, result));
        ptr = end;
    }
    ADMBP_end();
}
...
void Cmd_Damage_f(gentity_t* ent) {
    ...
    point[1] += dy;
    point[2] += dz;
    G_Damage(ent, nullptr, nullptr, nullptr, point, damage, (nonloc ? DAMAGE_NO_LOCDAMAGE : 0), MOD_TARGET_LASER);
}
...
void Cmd_Beacon_f(gentity_t* ent) {
    ...
    Beacon::Propagate(
            Beacon::New(tr.endpos, type, 0, team, ent->s.number, BCH_REMOVE));
    return;
}
...
int G_FloodLimited(gentity_t* ent) {
    ...
    trap_SendServerCommand(
            ent - g_entities,
            va("print_tr %s %d", QQ(N_("You are flooding: "
...
static void Cmd_Pubkey_Identify_f(gentity_t* ent) {
    ...
    if (ent->client->pers.pubkey_authenticated != 0 || !admin->pubkey[0] ||
        admin->counter == -1) {
        return;
    }
...
void ClientCommand(int clientNum) {
    ...
    if ((command->cmdFlags & CMD_MESSAGE) &&
        (ent->client->pers.namelog->muted || G_FloodLimited(ent))) {
        return;
    }

    if ((command->cmdFlags & CMD_TEAM) && ent->client->pers.team == TEAM_NONE) {
...
void Cmd_PrivateMessage_f(gentity_t* ent) {
    ...
    for (i = 0; i < pcount; i++) {
        if (G_SayTo(ent, &g_entities[pids[i]], teamonly ? SAY_TPRIVMSG : SAY_PRIVMSG, text)) {
            count++;
            Q_strcat(
                    recipients, sizeof(recipients), va("%s" S_COLOR_WHITE ", ", level.clients[pids[i]].pers.netname));
        }
    }
    ...
    if (!count) {
        ADMP(va("%s %s",
                QQ(N_("^3No player matching ^7 '$1$^7' ^3to send message to.\n")),
                Quote(name)));
    } else {
        ...
        // FIXME PLURAL
        ADMP(va("%s %c %i %s", Quote(P_("^$1$sent to $2$ player: ^7$3$\n", "^$1$sent to $2$ players: ^7$3$\n", count)), color, count, Quote(recipients)));
...
void Cmd_AdminMessage_f(gentity_t* ent) {
    ...
    if (!G_admin_permission(ent, ADMF_ADMINCHAT)) {
        if (!g_publicAdminMessages.integer) {
            ADMP("\"" N_(
                    "Sorry, but use of /a by non-admins has been disabled.\n") "\"");
            return;
        } else {
            ADMP("\"" N_(
