...
// functions that are used to provide values to the behavior tree in condition
// nodes
...
static AIValue_t haveWeapon(gentity_t* self, const AIValue_t* params) {
    return AIBoxInt(BG_InventoryContainsWeapon(AIUnBoxInt(params[0]),
...
static AIValue_t goalDead(gentity_t* self, const AIValue_t* params) {
    ...
    if (!BotTargetIsEntity(*goal)) {
        ...
        dead = true;
    } else if (goal->ent->client &&
               goal->ent->client->sess.spectatorState != SPECTATOR_NOT) {
        dead = true;
    } else if (goal->ent->s.eType == ET_BUILDABLE &&
               goal->ent->buildableTeam == self->client->pers.team &&
               !goal->ent->powered) {
        dead = true;
    }
...
static AIValue_t percentAmmo(gentity_t* self, const AIValue_t* params) {
    return AIBoxFloat(PercentAmmoRemaining(
            BG_PrimaryWeapon(self->client->ps.stats), &self->client->ps));
}
...
static AIValue_t percentHealth(gentity_t* self, const AIValue_t* params) {
    ...
    if (et.ent) {
        ...
        if (et.ent->s.eType == ET_PLAYER) {
            maxHealth =
                    BG_Class((class_t) et.ent->client->ps.stats[STAT_CLASS])->health;
        }
    }
...
static const struct AIConditionMap_s {
    ...
    AIFunc func;
    int nparams;
} conditionFuncs[] = {{"alertedToEnemy", VALUE_INT, alertedToEnemy, 0},
...
static const struct AIOpMap_s {
    ...
    int tokenSubtype;
    AIOpType_t opType;
} conditionOps[] = {{">=", P_LOGIC_GEQ, OP_GREATERTHANEQUAL},
...
static AIValue_t* parseFunctionParameters(pc_token_list** list, int* nparams, int minparams, int maxparams) {
    ...
    if (!parenEnd) {
        BotError("could not find matching ')' for '(' on line %d",
                 parenBegin->token.line);
...
static AIExpType_t* ReadConditionExpression(pc_token_list** list,
                                            AIOpType_t op2) {
    AIExpType_t* t;
    AIOpType_t op;
...
static const struct AIDecoratorMap_s {
    ...
    int minparams;
    int maxparams;
} AIDecorators[] = {{"return", BotDecoratorReturn, 1, 1},
...
static const struct AIActionMap_s {
    ...
    int minparams;
    int maxparams;
} AIActions[] = {{"activateUpgrade", BotActionActivateUpgrade, 1, 1},
...
Parses and creates an AIGenericNode_t with the type ACTION_NODE from a token
list
The token list pointer is modified to point to the beginning of the next node
...
ReadNodeList, ReadActionNode, and ReadConditionNode depending on the first token
in the list
======================
*/
...
AIBehaviorTree_t* ReadBehaviorTree(const char* name, AITreeList_t* list) {
    ...
    if (!handle) {
        G_Printf(S_COLOR_RED
                 "ERROR: Cannot load behavior tree %s: File not found\n",
                 treefilename);
        return nullptr;
    }
...
void AddTreeToList(AITreeList_t* list, AIBehaviorTree_t* tree) {
    if (list->maxTrees == list->numTrees) {
        AIBehaviorTree_t** trees = (AIBehaviorTree_t**) BG_Alloc(
                sizeof(AIBehaviorTree_t*) * list->maxTrees);
        list->maxTrees *= 2;
