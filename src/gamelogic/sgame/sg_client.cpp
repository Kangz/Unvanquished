...
void G_AddCreditToClient(gclient_t* client, short credit, bool cap) {
    ...
    if (cap && credit > 0) {
        capAmount = client->pers.team == TEAM_ALIENS ? ALIEN_MAX_CREDITS
                                                     : HUMAN_MAX_CREDITS;
...
gentity_t* G_SelectRandomFurthestSpawnPoint(vec3_t avoidPoint, vec3_t origin, vec3_t angles) {
    ...
    while ((spot = G_IterateEntitiesOfClass(spot, S_POS_PLAYER_SPAWN)) !=
           nullptr) {
...
================
*/
static gentity_t* G_SelectSpawnBuildable(vec3_t preference,
                                         buildable_t buildable) {
    gentity_t* search = nullptr;
    gentity_t* spot = nullptr;

    while ((search = G_IterateEntitiesOfClass(
                    search, BG_Buildable(buildable)->entityName)) != nullptr) {
        if (!search->spawned) {
            continue;
        }
        ...
        if (G_CheckSpawnPoint(search->s.number, search->s.origin, search->s.origin2, buildable, nullptr) != nullptr) {
            continue;
        }

        if (!spot ||
            DistanceSquared(preference, search->s.origin) <
...
static unnamed_t G_FindFreeUnnamed(unnamed_t number) {
    ...
    do {
        for (i = 0; i < level.maxclients; ++i) {
            if (level.clients[i].pers.namelog &&
                level.clients[i].pers.namelog->unnamedNumber == number) {
...
static const char* G_UnnamedClientName(gclient_t* client) {
    ...
    if (client->pers.namelog->unnamedNumber) {
        ...
        if (g_unnamedNumbering.integer > 0) {
            ...
            trap_Cvar_Set("g_unnamedNumbering",
                          va("%d", (number + 1 < 0 ? 1 : number + 1)));
        } else {
    ...
    Com_sprintf(name, sizeof(name), "%.*s%d", (int) sizeof(name) - 11, g_unnamedNamePrefix.string[0] ? g_unnamedNamePrefix.string : UNNAMED_PLAYER, number);
...
static void G_ClientCleanName(const char* in, char* out, int outSize, gclient_t* client) {
    ...
    *p = 0;
    ...
    for (; *in; in++) {
        ...
        if (Q_IsColorString(in) ||
            (in[0] == Q_COLOR_ESCAPE && in[1] == Q_COLOR_ESCAPE)) {
            in++;
            ...
            *out++ = '^';
            len += 2;
            continue;
        } else if (!g_emoticonsAllowedInNames.integer &&
                   G_IsEmoticon(in, &escaped)) {
            // make sure room in dest for both chars
            if (len > outSize - 2) {
                break;
            }
    ...
    *out = 0;
    ...
    // don't allow names beginning with S_SKIPNOTIFY because it messes up
    // /ignore-related code
...
char* ClientUserinfoChanged(int clientNum, bool forceName) {
    ...
    // If their userinfo overflowed, tremded is in the process of disconnecting
    // them.
    // If we send our own disconnect, it won't work, so just return to prevent
    // crashes later
    ...
    if (strcmp(oldname, newname)) {
        if (!forceName && client->pers.namelog->nameChangeTime &&
            ...
            trap_SendServerCommand(
                    ent - g_entities,
                    va("print_tr %s %d", QQ(N_("Name change spam protection "
                                               "(g_minNameChangePeriod = $1$)\n")),
            ...
            trap_SendServerCommand(
                    ent - g_entities,
                    va("print_tr %s %d",
                       QQ(N_("Maximum name changes reached (g_maxNameChanges = $1$)\n")),
            ...
            trap_SendServerCommand(
                    ent - g_entities,
                    va("print_tr %s",
                       QQ(N_("You cannot change your name while you are muted\n"))));
            revertName = true;
        } else if (!G_admin_name_check(ent, newname, err, sizeof(err))) {
            trap_SendServerCommand(ent - g_entities,
                                   va("print_tr %s %s %s", QQ("$1t$ $2$\n"), Quote(err), Quote(newname)));
            revertName = true;
        }

        if (revertName) {
            Q_strncpyz(client->pers.netname,
                       *oldname ? oldname : G_UnnamedClientName(client),
            ...
            if (*oldname) {
                G_LogPrintf(
                        "ClientRename: %i [%s] (%s) \"%s^7\" -> \"%s^7\" \"%c%s%c^7\"\n",
                        clientNum,
                        client->pers.ip.str,
                        client->pers.guid,
                        oldname,
    ...
    if (client->pers.classSelection == PCL_NONE) {
        // This looks hacky and frankly it is. The clientInfo string needs to hold
        // different
        // model details to that of the spawning class or the info change will not
        // be
        // registered and an axis appears instead of the player model. There is zero
        // chance
        // the player can spawn with the battlesuit, hence this choice.
        Com_sprintf(buffer, MAX_QPATH, "%s/%s", BG_ClassModelConfig(PCL_HUMAN_BSUIT)->modelName, BG_ClassModelConfig(PCL_HUMAN_BSUIT)->skinName);
    } else {
        Com_sprintf(buffer, MAX_QPATH, "%s/%s", BG_ClassModelConfig(client->pers.classSelection)->modelName, BG_ClassModelConfig(client->pers.classSelection)->skinName);
    ...
    Com_sprintf(
            userinfo, sizeof(userinfo), "n\\%s\\t\\%i\\model\\%s\\ig\\%16s\\v\\%s", client->pers.netname, client->pers.team, model, Com_ClientListString(&client->sess.ignoreList), client->pers.voice);
...
char* ClientConnect(int clientNum, bool firstTime) {
    ...
    for (i = 0; i < level.maxclients; i++) {
        ...
        if (!(g_entities[i].r.svFlags & SVF_BOT) &&
            !Q_stricmp(client->pers.guid, level.clients[i].pers.guid)) {
    ...
    G_LogPrintf("ClientConnect: %i [%s] (%s) \"%s^7\" \"%c%s%c^7\"\n", clientNum, client->pers.ip.str[0] ? client->pers.ip.str : "127.0.0.1", client->pers.guid, client->pers.netname, DECOLOR_OFF, client->pers.netname, DECOLOR_ON);
    ...
    if (firstTime) {
        if (g_geoip.integer && country && *country) {
            trap_SendServerCommand(
                    -1, va("print_tr %s %s %s", QQ(N_("$1$^7 connected from $2$\n")), Quote(client->pers.netname), Quote(country)));
        } else {
            trap_SendServerCommand(-1,
                                   va("print_tr %s %s", QQ(N_("$1$^7 connected\n")), Quote(client->pers.netname)));
        }
    }
    ...
    // if this is after !restart keepteams or !restart switchteams, apply said
    // selection
...
char* ClientBotConnect(int clientNum, bool firstTime, team_t team) {
    ...
    G_LogPrintf("ClientConnect: %i [%s] (%s) \"%s^7\" \"%c%s%c^7\" [BOT]\n",
                clientNum,
                client->pers.ip.str[0] ? client->pers.ip.str : "127.0.0.1",
                client->pers.guid,
                client->pers.netname,
                DECOLOR_OFF,
                client->pers.netname,
...
void ClientAdminChallenge(int clientNum) {
    ...
    if (!client->pers.pubkey_authenticated && admin && admin->pubkey[0] &&
        (level.time - client->pers.pubkey_challengedAt) >= 6000) {
        trap_SendServerCommand(clientNum, va("pubkey_decrypt %s", admin->msg2));
        client->pers.pubkey_challengedAt =
                level.time ^ (5 * clientNum); // a small amount of jitter

        // copy the decrypted message because generating a new message will
        // overwrite it
        G_admin_writeconfig();
    }
}
...
void ClientBegin(int clientNum) {
    ...
    trap_SendServerCommand(
            -1, va("print_tr %s %s", QQ(N_("$1$^7 entered the game\n")), Quote(client->pers.netname)));
    ...
    if (*startMsg) {
        trap_SendServerCommand(
                ent - g_entities,
                va("cpd %d %s", g_mapStartupMessageDelay.integer, Quote(startMsg)));
    }
...
void ClientSpawn(gentity_t* ent, gentity_t* spawn, const vec3_t origin, const vec3_t angles) {
    ...
    ent->takedamage = teamLocal != TEAM_NONE &&
                      client->sess.spectatorState == SPECTATOR_NOT; // true;
    ...
    ent->health = client->ps.stats[STAT_HEALTH] =
            client->ps.stats[STAT_MAX_HEALTH]; //* 1.25;
    ...
    if (client->sess.spectatorState == SPECTATOR_NOT &&
        ...
        if (ent == spawn) {
            ...
            if (spawnPoint->s.origin2[2] > 0.0f) {
                ...
                VectorNormalize(dir);
                VectorScale(dir,
                            BG_Class(ent->client->pers.classSelection)->jumpMagnitude,
                            client->ps.velocity);
            }
