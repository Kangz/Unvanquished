...
bool G_FindAmmo(gentity_t* self) {
    ...
    while ((neighbor = G_IterateEntitiesWithinRadius(neighbor, self->s.origin, ENTITY_BUY_RANGE))) {
        ...
        if (neighbor->s.eType != ET_BUILDABLE || !G_OnSameTeam(self, neighbor) ||
            !neighbor->spawned || !neighbor->powered || neighbor->health <= 0) {
            continue;
        }
...
================
*/
static void G_WideTrace(trace_t* tr, gentity_t* ent, const float range, const float width, const float height, gentity_t** target) {
    ...
    *target = nullptr;
    ...
    // The range is reduced according to the former trace so we don't hit
    ...
    // In case we hit a different target, which can happen if two potential
    // targets are close,
...
static void SendRangedHitEvent(gentity_t* attacker, gentity_t* target, trace_t* tr) {
    ...
    if (target->takedamage &&
        (target->s.eType == ET_BUILDABLE || target->s.eType == ET_PLAYER)) {
...
static gentity_t* FireMelee(gentity_t* self, float range, float width, float height, int damage, meansOfDeath_t mod) {
    trace_t tr;
    gentity_t* traceEnt;
...
static void FireLevel1Melee(gentity_t* self) {
    ...
    target = FireMelee(self, LEVEL1_CLAW_RANGE, LEVEL1_CLAW_WIDTH, LEVEL1_CLAW_WIDTH, LEVEL1_CLAW_DMG, MOD_LEVEL1_CLAW);
...
static void FireShotgun(gentity_t* self) {
    ...
    // instead of an EV_WEAPON_HIT_* event, send this so client can generate the
    // same spread pattern
    tent = G_NewTempEntity(muzzle, EV_SHOTGUN);
    VectorScale(forward, 4096, tent->s.origin2);
    SnapVector(tent->s.origin2);
    tent->s.eventParm =
            rand() / (RAND_MAX / 0x100 + 1); // seed for spread pattern
...
static void FireMassdriver(gentity_t* self) {
    ...
    if (target->takedamage) {
        G_Damage(target, self, self, forward, tr.endpos, MDRIVER_DMG, DAMAGE_KNOCKBACK, MOD_MDRIVER);
    }
}
...
static void HiveMissileThink(gentity_t* self) {
    ...
    nearest =
            DistanceSquared(self->r.currentOrigin, self->target->r.currentOrigin);
    ...
    for (i = 0; i < MAX_CLIENTS; i++) {
        ...
        if (ent->client && ent->health > 0 &&
            ...
            nearest > (d = DistanceSquared(ent->r.currentOrigin,
                                           self->r.currentOrigin))) {
            trap_Trace(&tr, self->r.currentOrigin, self->r.mins, self->r.maxs, ent->r.currentOrigin, self->r.ownerNum, self->clipmask, 0);
...
static void FireHive(gentity_t* self) {
    ...
    m = G_SpawnMissile(MIS_HIVE, self, origin, forward, self->target, HiveMissileThink, level.time + HIVE_DIR_CHANGE_PERIOD);
...
static void RocketThink(gentity_t* self) {
    ...
    RotatePointAroundVector(
            newDir, rotAxis, currentDir, Maths::clamp(rotAngle, -ROCKET_TURN_ANGLE, ROCKET_TURN_ANGLE));

    // Check if new direction is safe. Turn anyway if old direction is unsafe,
    // too.
    ...
    VectorCopy(self->r.currentOrigin,
               self->s.pos.trBase); // TODO: Snap this, too?
    self->s.pos.trTime = level.time;
}

static void FireRocket(gentity_t* self) {
    G_SpawnMissile(MIS_ROCKET, self, muzzle, forward, self->target, RocketThink, level.time + ROCKET_TURN_PERIOD)->timestamp =
            level.time + ROCKET_LIFETIME;
}
...
bool G_RocketpodSafeShot(int passEntityNum, vec3_t origin, vec3_t dir) {
    ...
    return !G_RadiusDamage(tr.endpos, nullptr, attr->splashDamage, attr->splashRadius, nullptr, 0, MOD_ROCKETPOD, TEAM_HUMANS);
}
...
static void FirebombMissileThink(gentity_t* self) {
    ...
    while ((neighbor = G_IterateEntitiesWithinRadius(neighbor, self->s.origin, FIREBOMB_IGNITE_RANGE))) {
        if (neighbor->s.eType == ET_BUILDABLE &&
            neighbor->buildableTeam == TEAM_ALIENS &&
    ...
    // spam fire
    for (subMissileNum = 0; subMissileNum < FIREBOMB_SUBMISSILE_COUNT;
         subMissileNum++) {
...
static gentity_t* FireLcannonHelper(gentity_t* self, vec3_t start, vec3_t dir, int damage, int radius, int speed) {
...
static void FireLcannon(gentity_t* self, bool secondary) {
    if (secondary && self->client->ps.stats[STAT_MISC] <= 0) {
        FireLcannonHelper(self, muzzle, forward, LCANNON_SECONDARY_DAMAGE, LCANNON_SECONDARY_RADIUS, LCANNON_SECONDARY_SPEED);
    } else {
        FireLcannonHelper(self, muzzle, forward, self->client->ps.stats[STAT_MISC] * LCANNON_DAMAGE / LCANNON_CHARGE_TIME_MAX, LCANNON_RADIUS, LCANNON_SPEED);
    }
    ...
    if (tr.fraction < 1.0f && traceEnt->spawned && traceEnt->health > 0 &&
        ...
        if (traceEnt->health < buildable->health) {
            ...
            self->client->ps.weaponTime +=
                    BG_Weapon(self->client->ps.weapon)->repeatRate1;
        }
    }
}
...
static void CancelBuild(gentity_t* self) {
    ...
    if (self->client->ps.weapon == WP_ABUILD ||
        ...
        FireMelee(self, ABUILDER_CLAW_RANGE, ABUILDER_CLAW_WIDTH, ABUILDER_CLAW_WIDTH, ABUILDER_CLAW_DMG, MOD_ABUILDER_CLAW);
    }
}
...
static void FireBuild(gentity_t* self, dynMenu_t menu) {
    ...
    buildable =
            (buildable_t)(self->client->ps.stats[STAT_BUILDABLE] & SB_BUILDABLE_MASK);
...
static void FindZapChainTargets(zap_t* zap) {
    ...
    for (i = 0; i < num; i++) {
        ...
        if (((enemy->client && enemy->client->pers.team == TEAM_HUMANS) ||
            ...
            trap_Trace(&tr, ent->s.origin, nullptr, nullptr, enemy->s.origin, ent->s.number, CONTENTS_SOLID, 0);
...
static void UpdateZapEffect(zap_t* zap) {
    ...
    BG_PackEntityNumbers(&zap->effectChannel->s, entityNums, zap->numTargets + 1);
...
static void CreateNewZap(gentity_t* creator, gentity_t* target) {
    ...
    for (i = 0; i < MAX_ZAPS; i++) {
        ...
        if (target->health > 0) {
            G_Damage(target, creator, creator, forward, target->s.origin, LEVEL2_AREAZAP_DMG, DAMAGE_NO_LOCDAMAGE, MOD_LEVEL2_ZAP);

            FindZapChainTargets(zap);

            for (i = 1; i < zap->numTargets; i++) {
                G_Damage(
                        zap->targets[i], target, zap->creator, forward, target->s.origin, LEVEL2_AREAZAP_DMG * (1 - powf((zap->distances[i] / LEVEL2_AREAZAP_CHAIN_RANGE), LEVEL2_AREAZAP_CHAIN_FALLOFF)) + 1, DAMAGE_NO_LOCDAMAGE, MOD_LEVEL2_ZAP);
            }
        }
...
void G_UpdateZaps(int msec) {
    ...
    for (i = 0; i < MAX_ZAPS; i++) {
        ...
        // first, the disappearance of players is handled immediately in
        // G_ClearPlayerZapEffects()

        // the deconstruction or gibbing of a directly targeted buildable destroys
        // the whole zap effect
        ...
        // the deconstruction or gibbing of chained buildables destroy the
        // appropriate beams
...
void G_ClearPlayerZapEffects(gentity_t* player) {
    ...
    for (i = 0; i < MAX_ZAPS; i++) {
        ...
        // the disappearance of the creator or the first target destroys the whole
        // zap effect
...
bool G_CheckPounceAttack(gentity_t* self) {
    ...
    G_WideTrace(&tr, self, pounceRange, LEVEL3_POUNCE_WIDTH, LEVEL3_POUNCE_WIDTH, &traceEnt);
    ...
    self->client->pmext.pouncePayload = 0;
    G_Damage(traceEnt, self, self, forward, tr.endpos, damage, DAMAGE_NO_LOCDAMAGE, MOD_LEVEL3_POUNCE);
...
void G_ChargeAttack(gentity_t* self, gentity_t* victim) {
    ...
    if (!victim->client) {
        ...
        self->client->trampleBuildablesHit[self->client->trampleBuildablesHitPos++ %
                                           MAX_TRAMPLE_BUILDABLES_TRACKED] =
                victim - g_entities;
    }
    ...
    G_Damage(victim, self, self, forward, victim->s.origin, damage, DAMAGE_NO_LOCDAMAGE, MOD_LEVEL4_TRAMPLE);
...
void G_ImpactAttack(gentity_t* self, gentity_t* victim) {
    ...
    if (victim->client &&
        victim->client->ps.stats[STAT_STATE2] & SS2_JETPACK_ACTIVE &&
    ...
    impactVelocity =
            fabs(self->client->pmext.fallImpactVelocity[2]) * QU_TO_METER; // in m/s
    impactEnergy = attackerMass * impactVelocity * impactVelocity;         // in J
    impactDamage = (int) (impactEnergy * IMPACTDMG_JOULE_TO_DAMAGE);

    // deal impact damage to both clients and structures, use a threshold for
    // friendly fire
    if (impactDamage > 0) {
        ...
        G_Damage(victim, self, self, knockbackDir, victim->s.origin, impactDamage, DAMAGE_NO_LOCDAMAGE, ModWeight(self));
    }
}
...
void G_WeightAttack(gentity_t* self, gentity_t* victim) {
    ...
    if (weightDPS > WEIGHTDMG_DPS_THRESHOLD) {
        ...
        if (weightDamage > 0) {
            G_Damage(victim, self, self, nullptr, victim->s.origin, weightDamage, DAMAGE_NO_LOCDAMAGE, ModWeight(self));
        }
    }
    ...
    switch (weaponMode) {
        case WPM_PRIMARY: {
            switch (weapon) {
                ...
                case WP_ALEVEL3:
                    FireMelee(self, LEVEL3_CLAW_RANGE, LEVEL3_CLAW_WIDTH, LEVEL3_CLAW_WIDTH, LEVEL3_CLAW_DMG, MOD_LEVEL3_CLAW);
                    break;

                case WP_ALEVEL3_UPG:
                    FireMelee(self, LEVEL3_CLAW_UPG_RANGE, LEVEL3_CLAW_WIDTH, LEVEL3_CLAW_WIDTH, LEVEL3_CLAW_DMG, MOD_LEVEL3_CLAW);
                    break;

                case WP_ALEVEL2:
                    FireMelee(self, LEVEL2_CLAW_RANGE, LEVEL2_CLAW_WIDTH, LEVEL2_CLAW_WIDTH, LEVEL2_CLAW_DMG, MOD_LEVEL2_CLAW);
                    break;

                case WP_ALEVEL2_UPG:
                    FireMelee(self, LEVEL2_CLAW_U_RANGE, LEVEL2_CLAW_WIDTH, LEVEL2_CLAW_WIDTH, LEVEL2_CLAW_DMG, MOD_LEVEL2_CLAW);
                    break;

                case WP_ALEVEL4:
                    FireMelee(self, LEVEL4_CLAW_RANGE, LEVEL4_CLAW_WIDTH, LEVEL4_CLAW_HEIGHT, LEVEL4_CLAW_DMG, MOD_LEVEL4_CLAW);
                    break;
