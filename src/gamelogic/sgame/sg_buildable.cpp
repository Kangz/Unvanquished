...
static void G_WarnPrimaryUnderAttack(gentity_t* self) {
    ...
    if (fracHealth < 0.25f && fracLastHealth >= 0.25f) {
        event =
                attr->team == TEAM_ALIENS ? EV_OVERMIND_ATTACK_2 : EV_REACTOR_ATTACK_2;
    } else if (fracHealth < 0.75f && fracLastHealth >= 0.75f) {
        event =
                attr->team == TEAM_ALIENS ? EV_OVERMIND_ATTACK_1 : EV_REACTOR_ATTACK_1;
    }

    if (event &&
        (event != self->attackLastEvent || level.time > self->attackTimer)) {
...
gentity_t* G_CheckSpawnPoint(int spawnNum, const vec3_t origin, const vec3_t normal, buildable_t spawn, vec3_t spawnOrigin) {
    ...
    if (spawn == BA_A_SPAWN) {
        ...
        // Calculate the necessary displacement for a given normal so that the
        // bounding boxes
    ...
    */
        ...
        // Method 2
        // For normals with equal X and Y component these results are optimal.
        // Otherwise they are
        // still better than naive worst-case calculations.
        // HACK: This only works for normals w/ angle < 45° towards either {0,0,1}
        // or {0,0,-1}
        if (normal[2] < 0.0f) {
        ...
        // the linear interpolation of min & max distance should be an upper
        // boundary for the
        ...
        // Since humans always spawn right above the telenode center, this is the
        // optimal
...
bool G_InsideBase(gentity_t* self, bool ownBase) {
    ...
    if (Distance(self->s.origin, mainBuilding->s.origin) >=
        INSIDE_BASE_MAX_DISTANCE) {
        return false;
    }
...
bool G_FindCreep(gentity_t* self) {
    ...
    // if self does not have a parentNode or its parentNode is invalid find a new
    // one
    if (self->client || self->powerSource == nullptr ||
        !self->powerSource->inuse || self->powerSource->health <= 0) {
...
static void AGeneric_CreepSlow(gentity_t* self) {
    ...
    for (ent = nullptr;
         (ent = G_IterateEntitiesWithinRadius(ent, self->s.origin, creepSize));) {
        if (!ent->client || ent->client->pers.team != TEAM_HUMANS ||
...
hack to prevent compilers complaining about function pointer -> nullptr
conversion
================
*/
            ...
            self->s.time = -(level.time -
                             (int) ((float) CREEP_SCALEDOWN_TIME *
                                    (1.0f - ((float) (level.time - self->creationTime) /
                                             (float) BG_Buildable(self->s.modelindex)
                                                     ->buildTime))));
        }
    }
...
void AGeneric_Blast(gentity_t* self) {
    ...
    G_SelectiveRadiusDamage(self->s.pos.trBase, g_entities + self->killedBy, self->splashDamage, self->splashRadius, self, self->splashMethodOfDeath, TEAM_ALIENS);
...
void AGeneric_Die(gentity_t* self, gentity_t* inflictor, gentity_t* attacker, int mod) {
    ...
    G_SetBuildableAnim(
            self, self->powered ? BANIM_DESTROY : BANIM_DESTROY_UNPOWERED, true);
    ...
    if (self->spawned &&
        -self->health < BG_Buildable(self->s.modelindex)->health) {
...
void G_IgniteBuildable(gentity_t* self, gentity_t* fireStarter) {
    ...
    if (!self->onFire && level.time > self->fireImmunityUntil) {
        ...
        self->nextBurnDamage =
                level.time + BURN_SELFDAMAGE_PERIOD * BURN_PERIODS_RAND_MOD;
        self->nextBurnSplashDamage =
                level.time + BURN_SPLDAMAGE_PERIOD * BURN_PERIODS_RAND_MOD;
        self->nextBurnAction =
                level.time + BURN_ACTION_PERIOD * BURN_PERIODS_RAND_MOD;
    } else if (self->onFire) {
        if (g_debugFire.integer) {
            char selfDescr[64], fireStarterDescr[64];
            BG_BuildEntityDescription(selfDescr, sizeof(selfDescr), &self->s);
...
void ASpawn_Think(gentity_t* self) {
    ...
    if (self->s.groundEntityNum != ENTITYNUM_NONE) {
        if ((ent = G_CheckSpawnPoint(self->s.number, self->s.origin, self->s.origin2, BA_A_SPAWN, nullptr)) !=
            nullptr) {
            // If the thing blocking the spawn is a buildable, kill it.
            // If it's part of the map, kill self.
            if (ent->s.eType == ET_BUILDABLE) {
                if (ent->builtBy && ent->builtBy->slot >= 0) {
                    G_Damage(ent, nullptr, g_entities + ent->builtBy->slot, nullptr, nullptr, 10000, DAMAGE_NO_PROTECTION, MOD_SUICIDE);
                } else {
                    G_Damage(ent, nullptr, nullptr, nullptr, nullptr, 10000, DAMAGE_NO_PROTECTION, MOD_SUICIDE);
                }
...
static bool AOvermind_TargetValid(gentity_t* self, gentity_t* target) {
    if (!target || !target->client ||
        ...
        target->health <= 0 || target->flags & FL_NOTARGET ||
...
static void AOvermind_FindTarget(gentity_t* self) {
    ...
    while ((ent = G_IterateEntities(ent))) {
        if (AOvermind_TargetValid(self, ent) &&
            AOvermind_IsBetterTarget(self, ent)) {
            self->target = ent;
        }
    }
    ...
    self->s.otherEntityNum =
            self->target ? self->target->s.number : ENTITYNUM_NONE;
}
...
void AOvermind_Think(gentity_t* self) {
    ...
    if (self->spawned && (self->health > 0)) {
        ...
        // do some damage
        if (G_SelectiveRadiusDamage(self->s.pos.trBase, self, self->splashDamage, self->splashRadius, self, MOD_OVERMIND, TEAM_ALIENS)) {
            self->timestamp = level.time;
...
void ABarricade_Shrink(gentity_t* self, bool shrink) {
    ...
    if (shrink) {
        ...
        trap_Trace(&tr, self->s.origin, self->r.mins, self->r.maxs, self->s.origin, self->s.number, MASK_PLAYERSOLID, 0);
        ...
        if (self->spawned && self->health > 0 && anim != BANIM_CONSTRUCT &&
            anim != BANIM_POWERUP) {
...
void ABarricade_Touch(gentity_t* self, gentity_t* other, trace_t* trace) {
    ...
    min_z =
            self->s.origin[2] - 18 + (int) (self->r.maxs[2] * BARRICADE_SHRINKPROP);
...
void AAcidTube_Think(gentity_t* self) {
    ...
    for (ent = nullptr; (ent = G_IterateEntitiesWithinRadius(ent, self->s.origin, ACIDTUBE_RANGE));) {
        if ((ent->flags & FL_NOTARGET) || !ent->client ||
        ...
        G_SelectiveRadiusDamage(self->s.pos.trBase, self, ACIDTUBE_DAMAGE, ACIDTUBE_RANGE, self, MOD_ATUBE, TEAM_ALIENS);
...
bool ASpiker_Fire(gentity_t* self) {
    ...
    // calculate total perimeter of all spike rows to allow for a more even spike
    // distribution
    float totalPerimeter = 0.0f;

    for (int row = 0; row < SPIKER_MISSILEROWS; row++) {
        float altitude =
                (((float) row + SPIKER_ROWOFFSET) * M_PI_2) / (float) SPIKER_MISSILEROWS;
    ...
    for (int row = 0; row < SPIKER_MISSILEROWS; row++) {
        ...
        for (int spike = 0; spike < spikes; spike++) {
            float azimuth =
                    2.0f * M_PI * (((float) spike + 0.5f * crandom()) / (float) spikes);
            ...
            if (g_debugTurrets.integer) {
                Com_Printf(
                        ...
                        spike + 1,
                        spikes,
                        RAD2DEG(altitude + altitudeVariance),
                        RAD2DEG(azimuth),
                        dir[0],
                        dir[1],
                        dir[2]);
            ...
            G_SpawnMissile(MIS_SPIKER, self, self->s.origin, dir, nullptr, G_FreeEntity, level.time + (int) (1000.0f * SPIKER_SPIKE_RANGE / (float) BG_Missile(MIS_SPIKER)->speed));
        }
    }
...
void ASpiker_Think(gentity_t* self) {
    ...
    for (ent = nullptr; (ent = G_IterateEntitiesWithinRadius(
        ...
        switch (ent->s.eType) {
            ...
            case ET_BUILDABLE:
                if (G_OnSameTeam(self, ent)) {
                    ...
                    durability =
                            (float) BG_Buildable(ent->s.modelindex)->health; // max health
                }
                break;
        ...
        if (!durability || !G_LineOfSight(self, ent))
            continue;
        ...
        float targetArea =
                0.5f * diameter * diameter; // approx. proj. of target on effect area
    ...
    // close enough to it.
    bool senseLost =
            self->spikerLastScoring > 0.0f && self->spikerLastSensing && !sensing;

    if (scoring > 0.0f || senseLost) {
        if (g_debugTurrets.integer) {
            Com_Printf("Spiker #%i scoring %.1f - %.1f = %.1f%s%s\n", self->s.number, enemyDamage, friendlyDamage, scoring, sensing ? " (sensing)" : "", senseLost ? " (lost target)" : "");
        }
...
static bool AHive_isBetterTarget(const gentity_t* const self,
    ...
    // Tie breaker is random decision, so clients on lower slots don't get shot at
    // more often.
...
static bool AHive_TargetValid(gentity_t* self, gentity_t* target, bool ignoreDistance) {
    ...
    if (!target || !target->client || target->client->pers.team != TEAM_HUMANS ||
        target->health <= 0 || (target->flags & FL_NOTARGET)) {
        return false;
    }
    ...
    if (!ignoreDistance &&
        Distance(tipOrigin, target->s.origin) > HIVE_SENSE_RANGE) {
        return false;
    }
...
static bool AHive_FindTarget(gentity_t* self) {
    ...
    while ((ent = G_IterateEntitiesWithinRadius(ent, self->s.origin, HIVE_SENSE_RANGE))) {
        if (AHive_TargetValid(self, ent, false) &&
            AHive_isBetterTarget(self, ent)) {
...
void ABooster_Think(gentity_t* self) {
    ...
    // check if there is a closeby alien that used this booster for healing
    // recently
    for (ent = nullptr; (ent = G_IterateEntitiesWithinRadius(
                                 ent, self->s.origin, REGEN_BOOSTER_RANGE));) {
...
void ATrapper_FireOnEnemy(gentity_t* self, int firespeed, float range) {
    ...
    int lowMsec = 0;
    int highMsec =
            (int) ((((distanceToTarget * LOCKBLOB_SPEED) +
                     (distanceToTarget *
                      BG_Class(target->client->ps.stats[STAT_CLASS])->speed)) /
                    (LOCKBLOB_SPEED * LOCKBLOB_SPEED)) *
                   1000.0f);
    ...
    while (highMsec - lowMsec > TRAPPER_ACCURACY) {
        ...
        float projectileDistance =
                LOCKBLOB_SPEED * (time + MISSILE_PRESTEP_TIME / 1000.0f);
...
bool ATrapper_CheckTarget(gentity_t* self, gentity_t* target, int range) {
    ...
    trap_Trace(&trace, self->s.pos.trBase, nullptr, nullptr, target->s.pos.trBase, self->s.number, MASK_SHOT, 0);
...
Calculates the amount of power a buildable recieves from or loses to a neighbor
at given distance.
=================
*/
static float IncomingInterference(buildable_t buildable, gentity_t* neighbor, float distance, bool prediction) {
    float range, power;
    ...
    if (neighbor->s.eType == ET_BUILDABLE &&
        neighbor->buildableTeam == TEAM_HUMANS) {
        // Only take unpowered and constructing buildables in consideration when
        // predicting
...
Calculates the amount of power a buildable gives to or takes from a neighbor at
given distance.
=================
*/
...
static void CalculateSparePower(gentity_t* self) {
    ...
    while ((neighbor = G_IterateEntitiesWithinRadius(neighbor, self->s.origin, PowerRelevantRange()))) {
        ...
        self->expectedSparePower += IncomingInterference(
                (buildable_t) self->s.modelindex, neighbor, distance, true);

        if (self->spawned) {
            self->currentSparePower += IncomingInterference(
                    (buildable_t) self->s.modelindex, neighbor, distance, false);
        }
    }

    // HACK: Store total power (used + spare) in entityState_t.clientNum for
    // display
    if (self->spawned) {
        int totalPower = (int) self->currentSparePower +
                         BG_Buildable(self->s.modelindex)->powerConsumption;
...
void G_SetHumanBuildablePowerState() {
    ...
    do {
        ...
        while ((ent = G_IterateEntities(ent, nullptr, false, 0, nullptr))) {
            ...
            // never shut down the telenode, even if it was set to consume power and
            // operates below
            // its threshold
...
void HGeneric_Blast(gentity_t* self) {
    ...
    G_RadiusDamage(self->s.pos.trBase, g_entities + self->killedBy, self->splashDamage, self->splashRadius, self, DAMAGE_KNOCKBACK, self->splashMethodOfDeath);
...
#define HUMAN_DETONATION_RAND_DELAY                                       \
    ((rand() - (RAND_MAX / 2)) / (float) (RAND_MAX / 2)) *                \
...
void HGeneric_Die(gentity_t* self, gentity_t* inflictor, gentity_t* attacker, int mod) {
    ...
    if (self->s.modelindex != BA_H_REACTOR && self->powered &&
        IsWarnableMOD(mod)) {
        ...
        if (inBase) {
            // note that being inside the main base doesn't mean there always is an
            // active reactor
...
void HSpawn_Think(gentity_t* self) {
    ...
    if (self->spawned) {
        ...
        if (self->s.groundEntityNum != ENTITYNUM_NONE) {
            if ((ent = G_CheckSpawnPoint(self->s.number, self->s.origin, self->s.origin2, BA_H_SPAWN, nullptr)) !=
                nullptr) {
                // If the thing blocking the spawn is a buildable, kill it.
                // If it's part of the map, kill self.
                if (ent->s.eType == ET_BUILDABLE) {
...
void HReactor_Think(gentity_t* self) {
    ...
    for (ent = nullptr; (ent = G_IterateEntitiesWithinRadius(
                                 ent, self->s.pos.trBase, REACTOR_ATTACK_RANGE));) {
        if (!ent->client || ent->client->pers.team != TEAM_ALIENS ||
            ent->flags & FL_NOTARGET) {
            continue;
        }
    ...
    if (fire) {
        ...
        G_SelectiveRadiusDamage(self->s.pos.trBase, self, REACTOR_ATTACK_DAMAGE, REACTOR_ATTACK_RANGE, self, MOD_REACTOR, TEAM_HUMANS);
    }
...
void HMedistat_Die(gentity_t* self, gentity_t* inflictor, gentity_t* attacker, int mod) {
...
void HMedistat_Think(gentity_t* self) {
    ...
    maxs[2] +=
            32; // continue to heal jumping players but don't heal jetpack campers
    ...
    if (self->target && self->target->client) {
        ...
        // restore stamina
        if (client->ps.stats[STAT_STAMINA] + STAMINA_MEDISTAT_RESTORE >=
            STAMINA_MAX) {
            client->ps.stats[STAT_STAMINA] = STAMINA_MAX;
        } else {
...
static bool HMGTurret_IsBetterTarget(gentity_t* self, gentity_t* candidate) {
    ...
    // (For the MG turret, do so after the already-targeted check so that such a
    // lock is kept if all
    // other valid targets are being dealt with. This results in suppressive fire
    // against targets
    ...
    VectorSubtract(self->target->s.origin, self->s.pos.trBase, dir_t);
    VectorNormalize(dir_t);
    VectorSubtract(candidate->s.origin, self->s.pos.trBase, dir_c);
    VectorNormalize(dir_c);
...
static bool HTurret_TargetValid(gentity_t* self, gentity_t* target, bool newTarget, float range) {
    if (!target || !target->client ||
        target->client->sess.spectatorState != SPECTATOR_NOT ||
        target->health <= 0 || target->flags & FL_NOTARGET ||
...
static bool HTurret_FindTarget(gentity_t* self, float range, bool (*isBetterTarget)(gentity_t*, gentity_t*)) {
    ...
    while ((ent = G_IterateEntitiesWithinRadius(ent, self->s.origin, range))) {
        if (HTurret_TargetValid(self, ent, true, range) &&
            HMGTurret_IsBetterTarget(self, ent)) {
            self->target = ent;
        }
    }
...
/**
 * @brief If the turret's head should rest this needs to be called so the move
 * timer stays valid.
 */
...
static bool HTurret_MoveHeadToTarget(gentity_t* self) {
    ...
    vec3_t rotAxis, angleToTarget, relativeDirToTarget, deltaAngles,
            relativeNewDir, newDir, maxAngles;
...
static void HTurret_SetBaseDir(gentity_t* self) {
    ...
    // invert base direction if it reaches into a wall within the first damage
    // zone
...
static void HMGTurret_Shoot(gentity_t* self) {
    ...
    zone = HMGTurret_DistanceToZone(
            Distance(self->s.pos.trBase, self->target->s.pos.trBase));
...
void HMGTurret_Think(gentity_t* self) {
    ...
    if (!self->target &&
        level.time - self->turretLastTargetSearch > TURRET_SEARCH_PERIOD) {
    ...
    if (HTurret_TargetInReach(self, MGTURRET_RANGE)) {
        // If the target's origin is visible, aim for it first, otherwise pause
        // head.
...
static bool HRocketPod_EnemyClose(gentity_t* self) {
    ...
    for (neighbour = nullptr;
         (neighbour = G_IterateEntitiesWithinRadius(neighbour, self->s.origin, ROCKETPOD_RANGE));) {
        if (neighbour->client && neighbour->health > 0 &&
            !G_OnSameTeam(self, neighbour) &&
            neighbour->client->sess.spectatorState == SPECTATOR_NOT) {
            classModelConfig_t* cmc =
                    BG_ClassModelConfig(neighbour->client->pers.classSelection);
            float classRadius =
                    0.5f * (VectorLength(cmc->mins) + VectorLength(cmc->maxs));
...
void G_BuildableThink(gentity_t* ent, int msec) {
    ...
    if (ent->time1000 >= 1000) {
        ...
        if (!ent->spawned && ent->health > 0) {
            int gain = (int) ceil(maxHealth * (1.0f - BUILDABLE_START_HEALTH_FRAC) *
                                  1000.0f / buildTime);
            ...
            switch (ent->buildableTeam) {
                case TEAM_ALIENS:
                    regenWait = ALIEN_BUILDABLE_REGEN_WAIT;
                    break;
                case TEAM_HUMANS:
                    regenWait = HUMAN_BUILDABLE_REGEN_WAIT;
                    break;
                default:
                    regenWait = 0;
                    break;
...
bool G_BuildableInRange(vec3_t origin, float radius, buildable_t buildable) {
    ...
    while ((neighbor = G_IterateEntitiesWithinRadius(neighbor, origin, radius))) {
        if (neighbor->s.eType != ET_BUILDABLE || !neighbor->spawned ||
            neighbor->health <= 0 ||
...
static int CompareBuildablesForRemoval(const void* a, const void* b) {
    ...
    aMatches = BuildablesIntersect(cmpBuildable, cmpOrigin, (buildable_t) buildableA->s.modelindex, buildableA->s.origin);
    bMatches = BuildablesIntersect(cmpBuildable, cmpOrigin, (buildable_t) buildableB->s.modelindex, buildableB->s.origin);
    ...
    if (cmpBuildable == BA_A_SPAWN || cmpBuildable == BA_H_SPAWN) {
        if ((buildableA->s.modelindex == BA_A_SPAWN &&
             level.team[TEAM_ALIENS].numSpawns == 1) ||
            (buildableA->s.modelindex == BA_H_SPAWN &&
             level.team[TEAM_HUMANS].numSpawns == 1)) {
            return 1;
        }

        if ((buildableB->s.modelindex == BA_A_SPAWN &&
             level.team[TEAM_ALIENS].numSpawns == 1) ||
            (buildableB->s.modelindex == BA_H_SPAWN &&
             level.team[TEAM_HUMANS].numSpawns == 1)) {
            return -1;
        }
    }
    ...
    if (!buildableA->powered && !buildableB->powered) {
        if (Distance(cmpOrigin, buildableA->s.origin) <
            Distance(cmpOrigin, buildableB->s.origin)) {
            return -1;
        } else {
            return 1;
        }
...
void G_Deconstruct(gentity_t* self, gentity_t* deconner, meansOfDeath_t deconType) {
    ...
    self->deconHealthFrac =
            Maths::clamp(self->health / (float) attr->health, 0.0f, 1.0f);
...
===============
*/
int G_FreeMarkedBuildables(gentity_t* deconner, char* readable, int rsize, char* nums, int nsize) {
    int i;
    int bNum;
    int listItems = 0;
        ...
        if (removalCounts[i]) {
            if (listItems) {
                if (listItems == (totalListItems - 1)) {
                    Q_strcat(readable, rsize, va("%s and ", (totalListItems > 2) ? "," : ""));
                } else {
...
static itemBuildError_t BuildableReplacementChecks(buildable_t oldBuildable,
                                                   buildable_t newBuildable) {
    ...
    // TODO: don't replace an egg that is the single creep provider for a
    // buildable
...
static bool PredictBuildablePower(buildable_t buildable, vec3_t origin) {
    ...
    while ((neighbor = G_IterateEntitiesWithinRadius(neighbor, origin, PowerRelevantRange()))) {
        ...
        neighborPrediction = neighbor->expectedSparePower +
                             OutgoingInterference(buildable, neighbor, distance);

        // check power of neighbor, with regards to pending deconstruction
        if (neighborPrediction < 0.0f &&
            distance < g_powerCompetitionRange.integer) {
            buddy = nullptr;
            while ((buddy = G_IterateEntitiesWithinRadius(buddy, neighbor->s.origin, PowerRelevantRange()))) {
                if (IsSetForDeconstruction(buddy)) {
                    ...
                    neighborPrediction -= IncomingInterference(
                            (buildable_t) neighbor->s.modelindex, buddy, distance, true);
                }
            }
...
Attempts to build a set of buildables that have to be deconstructed for a new
buildable.
Takes both power consumption and build points into account.
Sets level.markedBuildables and level.numBuildablesForRemoval.
===============
*/
static itemBuildError_t PrepareBuildableReplacement(buildable_t buildable,
                                                    vec3_t origin) {
    int entNum, listLen;
        ...
        if (BuildablesIntersect(buildable, origin, (buildable_t) ent->s.modelindex, ent->s.origin)) {
            if (ent->buildableTeam == attr->team && ent->deconstruct) {
                ...
                if (!(buildable == BA_H_REACTOR && ent->s.modelindex == BA_H_REACTOR) &&
                    ...
                    if ((reason = BuildableReplacementChecks(
                                 (buildable_t) ent->s.modelindex, buildable)) != IBE_NONE) {
                        return reason;
                    }
    ...
    if (attr->team == TEAM_HUMANS) {
        ...
        while (!PredictBuildablePower(buildable, origin)) {
            ...
            for (; neighborNum < numNeighbors; neighborNum++) {
                ...
                // apply general replacement rules
                if (BuildableReplacementChecks((buildable_t) neighbor->s.modelindex,
                                               buildable) != IBE_NONE) {
                    continue;
                }
    ...
    for (entNum = MAX_CLIENTS; entNum < level.num_entities; entNum++) {
        ...
        // apply general replacement rules
        if (BuildableReplacementChecks((buildable_t) ent->s.modelindex, buildable) !=
            IBE_NONE) {
            continue;
        }
...
itemBuildError_t G_CanBuild(gentity_t* ent, buildable_t buildable, int distance, vec3_t origin, vec3_t normal, int* groundEntNum) {
    ...
    *groundEntNum = tr1.entityNum;
    ...
    if ((tempReason = PrepareBuildableReplacement(buildable, origin)) !=
        IBE_NONE) {
        reason = tempReason;
        ...
        if ((tr1.surfaceFlags & (SURF_NOALIENBUILD | SURF_NOBUILD)) ||
            (contents & (CONTENTS_NOALIENBUILD | CONTENTS_NOBUILD))) {
            reason = IBE_SURFACE;
        }
        ...
        // Check permissions
        if ((tr1.surfaceFlags & (SURF_NOHUMANBUILD | SURF_NOBUILD)) ||
            (contents & (CONTENTS_NOHUMANBUILD | CONTENTS_NOBUILD))) {
            reason = IBE_SURFACE;
        }
    ...
    if (reason == IBE_NONE) {
        ...
        if (G_CheckSpawnPoint(ENTITYNUM_NONE, origin, normal, buildable, nullptr) !=
            nullptr) {
            reason = IBE_NORMAL;
        }
...
================
*/
static gentity_t* Build(gentity_t* builder, buildable_t buildable, const vec3_t origin, const vec3_t normal, const vec3_t angles, int groundEntNum) {
    gentity_t* built;
    ...
    if (builder->client && g_cheats.integer) {
        ...
        // HACK: This causes animation issues and can result in built->creationTime
        // < 0.
    ...
    built->s.angles2[PITCH] =
            0.0f; // Neutral pitch since this is how the ghost buildable looks.
    ...
    if (builder->client) {
        ...
        G_TeamCommand(
                (team_t) builder->client->pers.team,
                va("print_tr %s %s %s %s",
                   (readable[0]) ? QQ(N_("$1$ ^2built^7 by $2$^7, ^3replacing^7 $3$\n"))
                                 : QQ(N_("$1$ ^2built^7 by $2$$3$\n")),
                   Quote(BG_Buildable(built->s.modelindex)->humanName),
                   Quote(builder->client->pers.netname),
                   Quote(readable)));
        G_LogPrintf("Construct: %d %d %s%s: %s" S_COLOR_WHITE
                    " is building "
                    "%s%s%s\n",
...
bool G_BuildIfValid(gentity_t* ent, buildable_t buildable) {
    ...
    switch (G_CanBuild(ent, buildable, dist, origin, normal, &groundEntNum)) {
        case IBE_NONE:
            ...
            G_ModifyBuildPoints(BG_Buildable(buildable)->team,
...
static gentity_t* FinishSpawningBuildable(gentity_t* ent, bool force) {
    ...
    trap_Trace(&tr, built->s.origin, built->r.mins, built->r.maxs, dest, built->s.number, built->clipmask, 0);
...
int G_LayoutList(const char* map, char* list, int len) {
    ...
    numFiles = trap_FS_GetFileList(va("layouts/%s", map), ".dat", fileList, sizeof(fileList));
    filePtr = fileList;
    ...
    if (count != numFiles) {
        G_Printf(S_WARNING
                 "layout list was truncated to %d "
...
void G_LayoutSelect() {
    ...
    if (!cnt) {
        G_Printf(
                S_ERROR
                "None of the specified layouts could be found, using map default.\n");
        return;
    }
    ...
    for (cnt = 0;*(layout = COM_ParseExt(&layoutPtr, false)) && cnt < layoutNum;
         cnt++) {
...
void G_LayoutLoad() {
    ...
    while (*layout) {
        ...
        if (*layout == '\n') {
            i = 0;
            sscanf(line, "%s %f %f %f %f %f %f %f %f %f %f %f %f\n", buildName, &origin[0], &origin[1], &origin[2], &angles[0], &angles[1], &angles[2], &origin2[0], &origin2[1], &origin2[2], &angles2[0], &angles2[1], &angles2[2]);
            ...
            if (buildable <= BA_NONE || buildable >= BA_NUM_BUILDABLES) {
                G_Printf(S_WARNING
                         "bad buildable name (%s) in layout."
                         " skipping\n",
                         buildName);
            } else {
...
void G_BuildLogRevert(int id) {
    ...
    while (level.buildId > id) {
        ...
        switch (log->fate) {
                ...
                for (team = MAX_CLIENTS; team < level.num_entities; team++) {
                    ...
                    if (((ent->s.eType == ET_BUILDABLE && ent->health > 0) ||
                        ...
                        if (VectorLengthSquared(dist) <= 2.0f) {
                            ...
                            // Revert resources
                            G_ModifyBuildPoints(ent->buildableTeam,
                                                BG_Buildable(ent->s.modelindex)->buildPoints);
            ...
            case BF_REPLACE:
                // Revert resources
                G_ModifyBuildPoints(log->buildableTeam,
            ...
            default:
                ...
                buildable->suicideTime =
                        30; // number of thinks before killing players in the way
        }
    }
