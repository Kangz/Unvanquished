    ...
    while (1) {
        ...
        if ((weapon = BG_WeaponNumberByName(token))) {
            ...
            if (!Q_stricmp(token, "crosshair")) {
                ...
                cg_weapons[weapon].crossHair =
                        trap_R_RegisterShader(token, RSF_DEFAULT);
                ...
                cg_weapons[weapon].crossHairIndicator =
                        trap_R_RegisterShader(token, RSF_DEFAULT);
...
static bool CG_ParseWeaponAnimationFile(const char* filename,
                                        weaponInfo_t* wi) {
    const char* text_p;
    int len;
    int i;
...
===============
*/
static bool CG_ParseWeaponModeSection(weaponInfoMode_t* wim,
                                      const char** text_p) {
    char* token;
    int i;
...
bool CG_RegisterWeaponAnimation(animation_t* anim, const char* filename, bool loop, bool reversed, bool clearOrigin) {
    int frameRate;
    ...
    while (1) {
        ...
        if (!Q_stricmp(token, "{")) {
            if (weaponMode == WPM_NONE) {
                CG_Printf(S_ERROR
                          "weapon mode section started without a declaration\n");
                return false;
            ...
            if (cg_highPolyWeaponModels.integer &&
                ...
                switch (weapon) {
                    ...
                    case WP_ALEVEL2_UPG:
                       ...
                       (wi->weaponModel =
                                trap_R_RegisterModel(va("%s_view.md5mesh", token2)))) {
                wi->md5 = true;

                if (!CG_RegisterWeaponAnimation(&wi->animations[WANIM_IDLE],
                                                va("%s_view_idle.md5anim", token2),
                                                true,
                                                false,
                                                false)) {
                    // CG_Error( "could not find '%s'", path );
                ...
                switch (weapon) {
                    ...
                    case WP_PULSE_RIFLE:
                        ...
                        CG_RegisterWeaponAnimation(&wi->animations[WANIM_RAISE],
                                                   va("%s_view_raise.md5anim", token2),
                                                   false,
                                                   false,
                                                   false);
                        CG_RegisterWeaponAnimation(&wi->animations[WANIM_DROP],
                                                   va("%s_view_lower.md5anim", token2),
                                                   false,
                                                   false,
                                                   false);
                        CG_RegisterWeaponAnimation(&wi->animations[WANIM_RELOAD],
                                                   va("%s_view_reload.md5anim", token2),
                                                   false,
                                                   false,
                                                   false);
                        CG_RegisterWeaponAnimation(&wi->animations[WANIM_ATTACK1],
                                                   va("%s_view_fire.md5anim", token2),
                                                   false,
                                                   false,
                                                   false);
                        break;
                    ...
                    case WP_LUCIFER_CANNON:
                        CG_RegisterWeaponAnimation(&wi->animations[WANIM_ATTACK2],
                                                   va("%s_view_fire2.md5anim", token2),
                                                   false,
                                                   false,
                                                   false);
                    case WP_BLASTER:
                    ...
                    case WP_ALEVEL4:
                        ...
                        CG_RegisterWeaponAnimation(&wi->animations[WANIM_ATTACK2],
                                                   ...
                                                   false);
                        CG_RegisterWeaponAnimation(&wi->animations[WANIM_ATTACK3],
                                                   va("%s_view_fire3.md5anim", token2),
                                                   false,
                                                   false,
                                                   false);
                        CG_RegisterWeaponAnimation(&wi->animations[WANIM_ATTACK4],
                                                   va("%s_view_fire4.md5anim", token2),
                                                   false,
                                                   false,
                                                   false);
                        CG_RegisterWeaponAnimation(&wi->animations[WANIM_ATTACK5],
                                                   va("%s_view_fire5.md5anim", token2),
                                                   false,
                                                   false,
                                                   false);
                        CG_RegisterWeaponAnimation(&wi->animations[WANIM_ATTACK6],
                                                   va("%s_view_fire6.md5anim", token2),
                                                   false,
                                                   false,
                                                   false);
                        break;
                    ...
                    case WP_ALEVEL2_UPG:
                        ...
                        CG_RegisterWeaponAnimation(&wi->animations[WANIM_ATTACK6],
                                                   ...
                                                   false);
                        CG_RegisterWeaponAnimation(&wi->animations[WANIM_ATTACK7],
                                                   va("%s_view_fire7.md5anim", token2),
                                                   false,
                                                   false,
                                                   false);
                }
            ...
            wi->weaponIcon = wi->ammoIcon = trap_R_RegisterShader(token, RSF_DEFAULT);
...
void CG_RegisterWeapon(int weaponNum) {
    ...
    if (!weaponInfo->md5) {
        CG_ParseWeaponAnimationFile(
                va("models/weapons/%s/animation.cfg", BG_Weapon(weaponNum)->name),
                weaponInfo);
    }
...
static void CG_SetWeaponLerpFrameAnimation(weapon_t weapon, lerpFrame_t* lf, int newAnimation) {
    ...
    if (&cg_weapons[weapon].md5 && !toggle && lf && lf->old_animation &&
        lf->old_animation->handle) {
        if (!trap_R_BuildSkeleton(&oldGunSkeleton, lf->old_animation->handle, lf->oldFrame, lf->frame, lf->backlerp, lf->old_animation->clearOrigin)) {
            CG_Printf(
                    "CG_SetWeaponLerpFrameAnimation: can't build old gunSkeleton\n");
            return;
        }
    }
}
...
static void CG_WeaponAnimation(centity_t* cent, int* old, int* now, float* backLerp) {
    ...
    if (es->weaponAnim != lf->animationNumber || !lf->animation ||
        (cg_weapons[es->weapon].md5 && !lf->animation->handle)) {
...
static int CG_MapTorsoToWeaponFrame(clientInfo_t* ci, int frame, int anim) {
    ...
    } else // MD5 animations all start at 0, so there is no way to differentiate
           // them with first frame alone
    {
        // change weapon
        ...
        // stand attack
        else if ((anim == TORSO_ATTACK || anim == TORSO_ATTACK_BLASTER) &&
                 frame < 6) {
            return 1 + frame;
        }
    }
...
void CG_AddPlayerWeapon(refEntity_t* parent, playerState_t* ps, centity_t* cent) {
    ...
    noGunModel =
            ((!ps || cg.renderingThirdPerson) && weapon->disableIn3rdPerson) ||
            !gun.hModel;
    ...
    if (weaponNum == WP_LUCIFER_CANNON &&
        ...
        trap_S_AddLoopingSound(
                cent->currentState.number, cent->lerpOrigin, vec3_origin, ps ? cgs.media.lCannonWarningSound : cgs.media.lCannonWarningSound2);
    }
    ...
    if (ps || cg.renderingThirdPerson ||
        ...
        if (weapon->wim[weaponMode].muzzleParticleSystem && cent->muzzlePsTrigger) {
            cent->muzzlePS = CG_SpawnNewParticleSystem(
                    weapon->wim[weaponMode].muzzleParticleSystem);
        ...
        if (weapon->wim[weaponMode].flashDlightColor[0] ||
            ...
            trap_R_AddLightToScene(flash.origin, weapon->wim[weaponMode].flashDlight, weapon->wim[weaponMode].flashDlightIntensity, weapon->wim[weaponMode].flashDlightColor[0], weapon->wim[weaponMode].flashDlightColor[1], weapon->wim[weaponMode].flashDlightColor[2], 0, 0);
        }
    }
}
...
void CG_WeaponInertia(playerState_t* ps, vec3_t origin) {
    ...
    VectorMA(origin, atan(av[0] * WI_Y_SCALE) * WI_Y_LIMIT, cg.refdef.viewaxis[2], origin);
    VectorMA(origin, atan(av[1] * WI_X_SCALE) * WI_X_LIMIT, cg.refdef.viewaxis[1], origin);
...
void CG_AddViewWeapon(playerState_t* ps) {
    ...
    if (!drawGun) {
        ...
        if (wi->wim[weaponMode].muzzleParticleSystem && cent->muzzlePsTrigger) {
            cent->muzzlePS =
                    CG_SpawnNewParticleSystem(wi->wim[weaponMode].muzzleParticleSystem);
    ...
    if (cg_gun_frame.integer) {
        ...
        hand.frame = CG_MapTorsoToWeaponFrame(
                ci, cent->pe.torso.frame, !wi->md5 ? cent->pe.torso.animationNumber & ~ANIM_TOGGLEBIT : -1);
        hand.oldframe = CG_MapTorsoToWeaponFrame(
                ci, cent->pe.torso.oldFrame, !wi->md5 ? cent->pe.torso.animationNumber & ~ANIM_TOGGLEBIT : -1);
...
void CG_DrawHumanInventory() {
    ...
    for (i = 0; i < numItems; ++i) {
        const char* rmlClass;
        const char* src = CG_GetShaderNameFromHandle(
                items[i] < 32 ? cg_weapons[items[i]].weaponIcon
                              : cg_upgrades[items[i] - 32].upgradeIcon);
...
static void DrawTracer(vec3_t source, vec3_t dest, float chance, float length, float width) {
    ...
    begin = random() *
            Q_clamp(distance - 0.25f * length, TRACER_MIN_DISTANCE, distance);
...
void CG_HandleWeaponHitEntity(entityState_t* es, vec3_t origin) {
    ...
    if (victim->currentState.eType == ET_PLAYER) {
        PlayHitSound(origin, wim->impactFleshSound);
               ...
               BG_Buildable(victim->currentState.modelindex)->team ==
                       TEAM_ALIENS) {
        PlayHitSound(origin, wim->impactFleshSound);
    } else {
    ...
    if (CalcMuzzlePoint(attackerNum, muzzle)) {
        DrawTracer(muzzle, origin, cg_tracerChance.value, cg_tracerLength.value, cg_tracerWidth.value);
    }
}
...
void CG_HandleMissileHitWall(entityState_t* es, vec3_t origin) {
    ...
    if (ma->impactMark && ma->impactMarkSize > 0.0f) {
        CG_ImpactMark(ma->impactMark, origin, normal, random() * 360, 1, 1, 1, 1, false, ma->impactMarkSize, false);
    }
}
...
float CG_ChargeProgress() {
    ...
    progress =
            ((float) cg.predictedPlayerState.stats[STAT_MISC] - min) / (max - min);
