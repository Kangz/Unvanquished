...
void CG_CheckPlayerstateEvents(playerState_t* ps, playerState_t* ops) {
    ...
    for (i = ps->eventSequence - MAX_EVENTS; i < ps->eventSequence; i++) {
        ...
        if (i >= ops->eventSequence ||
            // or the server told us to play another event instead of a predicted
            // event we already issued
            // or something the server told us changed our prediction causing a
            // different event
...
void CG_CheckChangedPredictableEvents(playerState_t* ps) {
    ...
    for (i = ps->eventSequence - MAX_EVENTS; i < ps->eventSequence; i++) {
        ...
        // if this event is not further back in than the maximum predictable events
        // we remember
        if (i > cg.eventSequence - MAX_PREDICTED_EVENTS) {
            // if the new playerstate event is different from a previously predicted
            // one
            if (ps->events[i & (MAX_EVENTS - 1)] !=
