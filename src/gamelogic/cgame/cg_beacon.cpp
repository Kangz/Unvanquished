...
#define READ_COLOR(x)                        \
        ...
        else Com_Printf("^3WARNING: bad keyword \"%s\" in \"%s\"\n", token.string, path);
    }
...
static bool LoadImplicitBeacons() {
    ...
    if (cg.predictedPlayerState.persistant[PERS_TEAM] == TEAM_ALIENS &&
        ...
        for (int clientNum = 0; clientNum < MAX_CLIENTS; clientNum++) {
            ...
            if (!(ent = &cg_entities[clientNum])->valid)
                continue;

            // Only tag enemies like this, teammates have an explicit beacon.
            if (!(client = &cgs.clientinfo[clientNum])->infoValid)
                continue;
            if (client->team != TEAM_HUMANS)
                continue;
            ...
            // Set location on exact center of target player entity.
            {
                vec3_t mins, maxs, center;
                int pClass =
                        ((es->misc >> 8) & 0xFF); // TODO: Write function for this.
            ...
            // A value of 0 means the target is out of range, don't create a beacon in
            // that case.
            ...
            if (ent->currentState.eFlags & EF_DEAD) {
                ...
                if (!beacon->old || !(beacon->oldFlags & EF_BC_DYING)) {
                    beacon->etime =
                            cg.time + 1500; // TODO: Sync delay with explicit beacons.
                }
...
static void MarkRelevantBeacons() {
    ...
    for (int beaconNum = 0, tofind = 3; beaconNum < cg.beaconCount && tofind;
         beaconNum++) {
        cbeacon_t* beacon = cg.beacons[beaconNum];

        // Only tagged buildables are relevant so far.
        if (beacon->type != BCT_TAG || (beacon->flags & EF_BC_TAG_PLAYER) ||
            (beacon->flags & EF_BC_DYING)) {
            continue;
        }
        ...
        if (tofind & 2) {
            if (team == TEAM_HUMANS && (beacon->data == BA_H_ARMOURY ||
                                        (energy && (beacon->data == BA_H_REPEATER ||
                                                    beacon->data == BA_H_REACTOR)))) {
                if (lowammo)
...
/**
 * @brief Handles beacon animations and sounds. Called every frame for every
 * beacon.
 */
static void DrawBeacon(cbeacon_t* b) {
    ...
    // check death
    if (b->old && !(b->oldFlags & EF_BC_DYING) && (b->flags & EF_BC_DYING)) {
        if (ba->outSound)
    ...
    if (b->type == BCT_TIMER) {
        ...
        if (time_in >= BEACON_TIMER_TIME + cgs.bc.fadeIn)
            ...
            if (!b->eventFired) {
                trap_S_StartLocalSound(cgs.media.timerBeaconExpiredSound,
                                       CHAN_LOCAL_SOUND);
                b->eventFired = true;
            }
        } else
    ...
    /*
          ...
          if ( b->type == BCT_TAG && ( b->flags & EF_BC_TAG_PLAYER ) )
          {
                  ...
                  CG_Trace( &tr, cg.refdef.vieworg, nullptr, nullptr, b->origin,
     ENTITYNUM_NONE, CONTENTS_SOLID, 0 );
          ...
          // Fade out when too close. Span the same distance as the size change
     but fade linearly.
          ...
          if( !( ba->flags & BCF_IMPORTANT ) )
          {
                  if( b->dist < cgs.bc.fadeMinDist )
                          ...
                          alpha *= LinearRemap( b->dist, cgs.bc.fadeMinDist,
     cgs.bc.fadeMaxDist, cgs.bc.fadeMinAlpha, cgs.bc.fadeMaxAlpha );
          }
  */
    // color
    ...
    if (!front) {
        b->pos[0] = 640.0 - b->pos[0], b->pos[1] = 480.0 - b->pos[1];
    }
    ...
    // clamp to edges
    if (!front || b->pos[0] < cgs.bc.hudRect[0][0] + b->size / 2 ||
        b->pos[0] > cgs.bc.hudRect[1][0] - b->size / 2 ||
        b->pos[1] < cgs.bc.hudRect[0][1] + b->size / 2 ||
        b->pos[1] > cgs.bc.hudRect[1][1] - b->size / 2) {
        vec2_t screen[2];
        Vector2Set(screen[0], cgs.bc.hudRect[0][0] + b->size / 2, cgs.bc.hudRect[0][1] + b->size / 2);
        Vector2Set(screen[1], cgs.bc.hudRect[1][0] - b->size / 2, cgs.bc.hudRect[1][1] - b->size / 2);
...
static void HandHLBeaconToUI() {
    ...
    bool showIcon = false, showName = false, showInfo = false,
         showDistance = false, showAge = false, showOwner = false;
    ...
    if ((beacon = cg.highlightedBeacon)) {
        ...
        if (beacon->type == BCT_TAG && (beacon->flags & EF_BC_TAG_PLAYER) &&
            ...
            Com_sprintf(br->info, sizeof(br->info), "Carrying %s", BG_Weapon(beacon->data)->humanName);
            showInfo = true;
        }

        // distance
        Com_sprintf(br->distance, sizeof(br->distance), "%im from here", (int) round(beacon->dist * QU_TO_METER));
        showDistance = true;
        ...
        if (beacon->type == BCT_TAG && !(beacon->flags & EF_BC_TAG_PLAYER) &&
            ...
            if (age < 1000)
                ...
                Com_sprintf(br->age, sizeof(br->age), "Spotted %i:%02i ago", age / 60000, (age / 1000) % 60);

            showAge = true;
        }

        // owner
        if (BG_Beacon(beacon->type)->flags & BCF_IMPORTANT && beacon->owner >= 0 &&
            beacon->owner < MAX_CLIENTS && *cgs.clientinfo[beacon->owner].name) {
            Com_sprintf(br->owner, sizeof(br->owner), "by ^7%s", cgs.clientinfo[beacon->owner].name);
            showOwner = true;
        }
    }
 ...
 */
void CG_RunBeacons() {
    // Don't load implicit beacons if there wasn't enough space for the explicit
    // ones.
...
qhandle_t CG_BeaconIcon(const cbeacon_t* b) {
    ...
    if (b->type == BCT_TAG) {
        return BG_Beacon(b->type)
                ->icon[hlFlag][b->flags & EF_BC_TAG_PLAYER ? 1 : 0];
        ...
        case BCT_TAG:
            if (b->flags & EF_BC_TAG_PLAYER) {
                if (ownTeam == TEAM_NONE || ownTeam == beaconTeam) {
                    return strncpy(out, cgs.clientinfo[b->target].name,
                                   len); // Player name
                } else if (beaconTeam == TEAM_ALIENS) {
                    return strncpy(out, BG_ClassModelConfig(b->data)->humanName,
                                   len); // Class name
