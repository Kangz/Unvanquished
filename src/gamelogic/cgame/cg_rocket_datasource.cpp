    ...
    if (netSrc != AS_FAVORITES) {
        ...
        for (i = 0; i < numServers; ++i) {
            ...
            if (ping >= 0 || !Q_stricmp(args, "favorites")) {
                ...
                AddToServerList(Info_ValueForKey(info, "hostname"),
                                Info_ValueForKey(info, "label"),
                                clients,
                                bots,
                                ping,
                                maxClients,
        ...
        for (i = 0; i < rocketInfo.data.serverCount[netSrc]; ++i) {
            ...
            Info_SetValueForKey(
                    data, "maxClients", va("%d", rocketInfo.data.servers[netSrc][i].maxClients), false);
...
static void CG_Rocket_SortServerList(const char* name, const char* sortBy) {
    ...
    for (i = 0; i < rocketInfo.data.serverCount[netSrc]; ++i) {
        ...
        Info_SetValueForKey(data, "players", va("%d", rocketInfo.data.servers[netSrc][i].clients), false);
        Info_SetValueForKey(
                data, "bots", va("%d", rocketInfo.data.servers[netSrc][i].bots), false);
        Info_SetValueForKey(
                data, "ping", va("%d", rocketInfo.data.servers[netSrc][i].ping), false);
...
static void CG_Rocket_FilterServerList(const char* table, const char* filter) {
    const char* str = (table && *table)
                              ? table
                              : CG_NetSourceToString(rocketInfo.currentNetSrc);
...
void CG_Rocket_ExecServerList(const char* table) {
    ...
    trap_SendConsoleCommand(
            va("connect %s",
               rocketInfo.data.servers[netSrc][rocketInfo.data.serverIndex[netSrc]]
                       .addr));
}
...
void CG_Rocket_BuildModList(const char* args) {
    ...
    for (i = 0; i < numdirs; i++) {
        ...
        rocketInfo.data.modList[rocketInfo.data.modCount].description =
                BG_strdup(descptr);
...
void CG_Rocket_ExecDemoList(const char* args) {
    trap_SendConsoleCommand(
            va("demo %s", rocketInfo.data.demoList[rocketInfo.data.demoIndex]));
}
...
void CG_Rocket_BuildPlayerList(const char* args) {
    ...
    for (i = 0; i < cg.numScores; ++i) {
        ...
        switch (score->team) {
            case TEAM_ALIENS:
                rocketInfo.data.playerList
                        [score->team][rocketInfo.data.playerCount[TEAM_ALIENS]++] = i;
                trap_Rocket_DSAddRow("playerList", "aliens", buf);
                break;

            case TEAM_HUMANS:
                rocketInfo.data.playerList
                        [score->team][rocketInfo.data.playerIndex[TEAM_HUMANS]++] = i;
                trap_Rocket_DSAddRow("playerList", "humans", buf);
                break;

            case TEAM_NONE:
                rocketInfo.data
                        .playerList[score->team][rocketInfo.data.playerCount[TEAM_NONE]++] =
                        i;
...
static int PlayerListCmpByScore(const void* one, const void* two) {
    ...
    if (cg.scores[*a].score > cg.scores[*b].score)
        return 1;

    if (cg.scores[*b].score > cg.scores[*a].score)
        return -1;
...
void CG_Rocket_SortPlayerList(const char* name, const char* sortBy) {
    ...
    if (!Q_stricmp("score", sortBy)) {
        qsort(rocketInfo.data.playerList[TEAM_NONE],
              rocketInfo.data.playerCount[TEAM_NONE],
              sizeof(int),
              &PlayerListCmpByScore);
        qsort(rocketInfo.data.playerList[TEAM_ALIENS],
              rocketInfo.data.playerCount[TEAM_ALIENS],
              sizeof(int),
              &PlayerListCmpByScore);
        qsort(rocketInfo.data.playerList[TEAM_HUMANS],
              rocketInfo.data.playerIndex[TEAM_HUMANS],
              sizeof(int),
              &PlayerListCmpByScore);
    }
...
void CG_Rocket_BuildTeamList(const char* args) {
    static const char* data[] = {
            ...
            "\\name\\Auto Select\\description\\Join the team with the least "
            "players.\\",
            nullptr};
...
void CG_Rocket_ExecTeamList(const char* table) {
    ...
    if (cmd) {
        ...
        trap_Rocket_DocumentAction(rocketInfo.menu[ROCKETMENU_TEAMSELECT].id,
                                   "hide");
    }
}
...
void CG_Rocket_ExecHumanSpawnItems(const char* table) {
    ...
    if (cmd) {
        trap_SendConsoleCommand(cmd);
        trap_Rocket_DocumentAction(rocketInfo.menu[ROCKETMENU_HUMANSPAWN].id,
                                   "hide");
    }
}
...
static void AddWeaponToBuyList(int i, const char* table, int tblIndex) {
    ...
    if (BG_Weapon(i)->team == TEAM_HUMANS && BG_Weapon(i)->purchasable &&
        ...
        rocketInfo.data.armouryBuyList
                [tblIndex][rocketInfo.data.armouryBuyListCount[tblIndex]++] = i;
    }
}
...
static void AddUpgradeToBuyList(int i, const char* table, int tblIndex) {
    ...
    if (BG_Upgrade(i)->team == TEAM_HUMANS && BG_Upgrade(i)->purchasable &&
        i != UP_MEDKIT) {
        Info_SetValueForKey(
                buf, "num", va("%d", tblIndex == ROCKETDS_BOTH ? i + WP_NUM_WEAPONS : i), false);
        ...
        rocketInfo.data.armouryBuyList
                [tblIndex][rocketInfo.data.armouryBuyListCount[tblIndex]++] =
                i + WP_NUM_WEAPONS;
    }
}
...
void CG_Rocket_ExecArmouryBuyList(const char* table) {
    ...
    if ((item =
                 rocketInfo.data.armouryBuyList
                         [tblIndex][rocketInfo.data.selectedArmouryBuyItem[tblIndex]]) >
        WP_NUM_WEAPONS) {
        item -= WP_NUM_WEAPONS;
        ...
        if (BG_Upgrade(item)) {
            ...
            if (BG_Upgrade(item)->slots &
                BG_SlotsForInventory(cg.predictedPlayerState.stats)) {
                int i;
        ...
        if (BG_Weapon(item)) {
            ...
            trap_SendClientCommand(
                    va("sell %s",
                       BG_Weapon(BG_GetPlayerWeapon(&cg.predictedPlayerState))->name));
        }
    }
...
void CG_Rocket_BuildArmourySellList(const char* table) {
    ...
    if (!Q_stricmp(table, "default")) {
        ...
        for (i = WP_NONE + 1; i < WP_NUM_WEAPONS; ++i) {
            if (BG_InventoryContainsWeapon(i, cg.predictedPlayerState.stats) &&
                BG_Weapon(i)->purchasable) {
                buf[0] = '\0';

                rocketInfo.data
                        .armourySellList[rocketInfo.data.armourySellListCount++] = i;
        ...
        for (i = UP_NONE + 1; i < UP_NUM_UPGRADES; ++i) {
            if (BG_InventoryContainsUpgrade(i, cg.predictedPlayerState.stats) &&
                BG_Upgrade(i)->purchasable) {
                buf[0] = '\0';

                rocketInfo.data
                        .armourySellList[rocketInfo.data.armourySellListCount++] =
                        i + WP_NUM_WEAPONS;
...
void CG_Rocket_ExecArmourySellList(const char* table) {
    ...
    if (rocketInfo.data.selectedArmourySellItem < 0 ||
        rocketInfo.data.selectedArmourySellItem >=
                rocketInfo.data.armourySellListCount) {
        return;
    }

    item =
            rocketInfo.data.armourySellList[rocketInfo.data.selectedArmourySellItem];
    ...
    if (!Q_stricmp(table, "default")) {
        ...
        for (i = 0; i < PCL_NUM_CLASSES; ++i) {
            if (BG_Class(i)->team == TEAM_ALIENS) {
                ...
                Info_SetValueForKey(
                        buf, "price", va("%d", BG_ClassCanEvolveFromTo(cg.predictedPlayerState.stats[STAT_CLASS], i, cg.predictedPlayerState.persistant[PERS_CREDIT]) / CREDITS_PER_EVO), false);

                trap_Rocket_DSAddRow("alienEvolveList", "default", buf);

                rocketInfo.data
                        .alienEvolveList[rocketInfo.data.alienEvolveListCount++] = i;
            }
        }
    }
}
...
void CG_Rocket_ExecAlienEvolveList(const char* table) {
    class_t evo =
            (class_t)
                    rocketInfo.data.alienEvolveList[rocketInfo.data.selectedAlienEvolve];

    if (BG_Class(evo) &&
        BG_ClassCanEvolveFromTo(
                cg.predictedPlayerState.stats[STAT_CLASS], evo, cg.predictedPlayerState.persistant[PERS_CREDIT]) >= 0) {
        trap_SendClientCommand(va("class %s", BG_Class(evo)->name));
        trap_Rocket_DocumentAction(rocketInfo.menu[ROCKETMENU_ALIENEVOLVE].id,
                                   "hide");
    }
}
    ...
    if (!Q_stricmp(table, "default")) {
        ...
        for (i = BA_NONE + 1; i < BA_NUM_BUILDABLES; ++i) {
            if (BG_Buildable(i)->team == TEAM_HUMANS) {
                ...
                rocketInfo.data.humanBuildList[rocketInfo.data.humanBuildListCount++] =
                        i;
            }
        }
    }
...
void CG_Rocket_ExecHumanBuildList(const char* table) {
    buildable_t build =
            (buildable_t)
                    rocketInfo.data.humanBuildList[rocketInfo.data.selectedHumanBuild];

    if (BG_Buildable(build)) {
        trap_SendClientCommand(va("build %s", BG_Buildable(build)->name));
        trap_Rocket_DocumentAction(rocketInfo.menu[ROCKETMENU_HUMANBUILD].id,
                                   "hide");
    }
}
    ...
    if (!Q_stricmp(table, "default")) {
        ...
        for (i = BA_NONE + 1; i < BA_NUM_BUILDABLES; ++i) {
            if (BG_Buildable(i)->team == TEAM_ALIENS) {
                ...
                rocketInfo.data.alienBuildList[rocketInfo.data.alienBuildListCount++] =
                        i;
            }
        }
    }
...
void CG_Rocket_ExecAlienBuildList(const char* table) {
    buildable_t build =
            (buildable_t)
                    rocketInfo.data.alienBuildList[rocketInfo.data.selectedAlienBuild];

    if (BG_Buildable(build)) {
        trap_SendClientCommand(va("build %s", BG_Buildable(build)->name));
        trap_Rocket_DocumentAction(rocketInfo.menu[ROCKETMENU_ALIENBUILD].id,
                                   "hide");
    }
}
...
void CG_Rocket_ExecAlienSpawnList(const char* table) {
    ...
    switch (rocketInfo.data.selectedAlienSpawnClass) {
        ...
        case 1:
            _class =
                    BG_ClassUnlocked(PCL_ALIEN_BUILDER0_UPG) ? "builderupg" : "builder";
            break;
    }

    if (_class) {
        trap_SendClientCommand(va("class %s", _class));
        trap_Rocket_DocumentAction(rocketInfo.menu[ROCKETMENU_ALIENSPAWN].id,
                                   "hide");
    }
}
...
void CG_Rocket_RegisterDataSources() {
    ...
    for (i = 0; i < dataSourceCmdListCount; ++i) {
        ...
        // bsearch
        if (i != 0 &&
            Q_stricmp(dataSourceCmdList[i - 1].name, dataSourceCmdList[i].name) >
                    0) {
            CG_Printf("CGame dataSourceCmdList is in the wrong order for %s and %s\n",
                      dataSourceCmdList[i - 1].name,
                      dataSourceCmdList[i].name);
        }
