...
typedef enum { JPS_INACTIVE,
               JPS_ACTIVE } jetPackState_t;
...
typedef struct {
    ...
    int animationTime; // time when the first frame of the animation will be
                       // exact
...
// attachment system
typedef enum { AT_STATIC,
               AT_TAG,
               AT_CENT,
               AT_PARTICLE } attachmentType_t;
...
#define MAX_BASEPARTICLE_EJECTORS \
    MAX_BASEPARTICLE_SYSTEMS* MAX_EJECTORS_PER_SYSTEM
...
typedef enum { PMD_LINEAR,
               PMD_POINT } pDirType_t;
...
typedef struct baseParticleEjector_s {
    ...
    pLerpValues_t
            eject; // zero period indicates creation of all particles at once
...
typedef struct baseParticleSystem_s {
    ...
    bool registered; // whether or not the assets for this particle have been
                     // loaded
...
typedef enum { TBTT_STRETCH,
               TBTT_REPEAT } trailBeamTextureType_t;
...
typedef struct {
    ...
    int target;
    float
            alphaMod; // A modifier that can be set before the drawing phase begins.
...
typedef struct weaponInfo_s {
    ...
    qhandle_t handsModel; // the hands don't actually draw, they just position
                          // the weapon
...
typedef struct { qhandle_t classIcon; } classInfo_t;
...
// action
// occurs, and they will have visible effects for #define STEP_TIME or whatever
// msec after
...
typedef struct {
    ...
    int latestSnapshotNum;  // the number of snapshots the client system has
                            // received
    int latestSnapshotTime; // the time from latestSnapshotNum, so we don't need
                            // to read the snapshot yet
    ...
    // is rendering at.
    int oldTime; // time at last frame, used for missile trails and prediction
                 // checking
...
void CG_DrawSphericalCone(const vec3_t tip, const vec3_t rotation, float radius, bool a240, int customShader, const float* shaderRGBA);
...
void CG_DrawActive();
void CG_OwnerDraw(rectDef_t* rect, float text_x, float text_y, int ownerDraw, int ownerDrawFlags, int align, int textalign, int textvalign, float borderSize, float scale, vec4_t foreColor, vec4_t backColor, qhandle_t shader, int textStyle);
...
void CG_Trace(trace_t* result, const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end, int skipNumber, int mask, int skipmask);
void CG_CapTrace(trace_t* result, const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end, int skipNumber, int mask, int skipmask);
void CG_BiSphereTrace(trace_t* result, const vec3_t start, const vec3_t end, const float startRadius, const float endRadius, int skipNumber, int mask, int skipmask);
...
void CG_SetAttachmentTag(attachment_t* a, refEntity_t* parent, qhandle_t model, const char* tagName);
