...
static particle_t* CG_SpawnNewParticle(baseParticle_t* bp,
    ...
    for (i = 0; i < MAX_PARTICLES; i++) {
        ...
        if (!p->valid && cg.clientFrame > p->frameWhenInvalidated + 1) {
            ...
            p->birthTime = cg.time;
            p->lifeTime =
                    (int) CG_RandomiseValue((float) bp->lifeTime, bp->lifeTimeRandFrac);

            p->radius.delay = (int) CG_RandomiseValue((float) bp->radius.delay,
                                                      bp->radius.delayRandFrac);
            p->radius.initial =
                    CG_RandomiseValue(bp->radius.initial, bp->radius.initialRandFrac);
            p->radius.final =
                    CG_RandomiseValue(bp->radius.final, bp->radius.finalRandFrac);

            p->radius.initial += bp->scaleWithCharge * pe->parent->charge;

            p->alpha.delay = (int) CG_RandomiseValue((float) bp->alpha.delay,
                                                     bp->alpha.delayRandFrac);
            p->alpha.initial =
                    CG_RandomiseValue(bp->alpha.initial, bp->alpha.initialRandFrac);
            p->alpha.final =
                    CG_RandomiseValue(bp->alpha.final, bp->alpha.finalRandFrac);

            p->rotation.delay = (int) CG_RandomiseValue((float) bp->rotation.delay,
                                                        bp->rotation.delayRandFrac);
            p->rotation.initial =
                    CG_RandomiseValue(bp->rotation.initial, bp->rotation.initialRandFrac);
            p->rotation.final =
                    CG_RandomiseValue(bp->rotation.final, bp->rotation.finalRandFrac);

            p->dLightRadius.delay = (int) CG_RandomiseValue(
                    (float) bp->dLightRadius.delay, bp->dLightRadius.delayRandFrac);
            p->dLightRadius.initial = CG_RandomiseValue(
                    bp->dLightRadius.initial, bp->dLightRadius.initialRandFrac);
            p->dLightRadius.final = CG_RandomiseValue(bp->dLightRadius.final,
                                                      bp->dLightRadius.finalRandFrac);

            p->colorDelay = CG_RandomiseValue(bp->colorDelay, bp->colorDelayRandFrac);

            p->bounceMarkRadius =
                    CG_RandomiseValue(bp->bounceMarkRadius, bp->bounceMarkRadiusRandFrac);
            p->bounceMarkCount = rint(CG_RandomiseValue((float) bp->bounceMarkCount,
                                                        bp->bounceMarkCountRandFrac));
            p->bounceSoundCount = rint(CG_RandomiseValue(
                    (float) bp->bounceSoundCount, bp->bounceSoundCountRandFrac));
            ...
            if (bp->numModels) {
                ...
                if (bp->modelAnimation.frameLerp < 0) {
                    bp->modelAnimation.frameLerp =
                            p->lifeTime / bp->modelAnimation.numFrames;
            ...
            VectorScale(p->velocity, CG_RandomiseValue(bp->velMoveValues.mag, bp->velMoveValues.magRandFrac), p->velocity);

            if (CG_AttachmentVelocity(&ps->attachment, attachmentVelocity)) {
                VectorMA(p->velocity,
                         CG_RandomiseValue(bp->velMoveValues.parentVelFrac,
                                           bp->velMoveValues.parentVelFracRandFrac),
                         attachmentVelocity,
                         p->velocity);
            }
            ...
            if (bp->childSystemName[0] != '\0') {
                particleSystem_t* chps =
                        CG_SpawnNewParticleSystem(bp->childSystemHandle);
...
static particleEjector_t* CG_SpawnNewParticleEjector(baseParticleEjector_t* bpe,
    ...
    for (i = 0; i < MAX_PARTICLE_EJECTORS; i++) {
        ...
        if (!pe->valid) {
            ...
            pe->nextEjectionTime =
                    cg.time + (int) CG_RandomiseValue((float) bpe->eject.delay,
                                                      bpe->eject.delayRandFrac);
            pe->count = pe->totalParticles = (int) rint(CG_RandomiseValue(
                    (float) bpe->totalParticles, bpe->totalParticlesRandFrac));
            ...
            for (j = 0; j < bps->numEjectors; j++) {
                ...
                for (l = 0; l < bpe->numParticles; l++) {
                    ...
                    for (k = 0; k < bp->numFrames; k++) {
                        bp->shaders[k] =
                                trap_R_RegisterShader(bp->shaderNames[k], RSF_SPRITE);
                    }
                    ...
                    if (bp->bounceMarkName[0] != '\0') {
                        bp->bounceMark =
                                trap_R_RegisterShader(bp->bounceMarkName, RSF_DEFAULT);
                    }
                    ...
                    if (bp->childTrailSystemName[0] != '\0') {
                        bp->childTrailSystemHandle =
                                CG_RegisterTrailSystem(bp->childTrailSystemName);
                    }
                }
            }
    ...
    while (1) {
        ...
        ///
            ...
            for (i = 0; i <= 2; i++) {
                ...
                CG_ParseValueAndVariance(token, &bp->displacement[i], &bp->randDisplacement[i], true);
            }
...
===============
*/
static bool CG_ParseParticleEjector(baseParticleEjector_t* bpe,
                                    const char** text_p) {
    char* token;
    float number;
...
===============
*/
static bool CG_ParseParticleSystem(baseParticleSystem_t* bps,
                                   const char** text_p,
                                   const char* name) {
    char* token;
    baseParticleEjector_t* bpe;
    ...
    while (1) {
        ...
        if (!Q_stricmp(token, "{")) {
            if (!CG_ParseParticleEjector(
                        &baseParticleEjectors[numBaseParticleEjectors], text_p)) {
            ...
            if (bpe->totalParticles == PARTICLES_INFINITE &&
                ...
                CG_Printf(
                        S_ERROR
                        "ejector with 'count infinite' potentially has zero period\n");
                return false;
            }
        ...
        if (!Q_stricmp(token, "{")) {
            if (psNameSet) {
                ...
                if (!CG_ParseParticleSystem(
                            &baseParticleSystems[numBaseParticleSystems], &text_p, psName)) {
...
#define MAX_ACC_RADIUS 1000.0f
    ...
    if (bp->accMoveValues.dirType == PMD_POINT) {
        ...
        VectorNormalize(acceleration);
        CG_SpreadVector(acceleration, bp->accMoveValues.dirRandAngle);
        VectorScale(acceleration, CG_RandomiseValue(bp->accMoveValues.mag, bp->accMoveValues.magRandFrac), acceleration);
    }

    // Some particles have a visual radius that differs from their collision
    // radius
    if (bp->physicsRadius) {
        radius = bp->physicsRadius;
    } else {
        radius = CG_LerpValues(
                p->radius.initial, p->radius.final, CG_CalculateTimeFrac(p->birthTime, p->lifeTime, p->radius.delay));
    }
    ...
    CG_Trace(&trace, p->origin, mins, maxs, newOrigin, CG_AttachmentCentNum(&ps->attachment), CONTENTS_SOLID, 0);
    ...
    if (bp->bounceMarkName[0] && p->bounceMarkCount > 0) {
        CG_ImpactMark(bp->bounceMark, trace.endpos, trace.plane.normal, random() * 360, 1, 1, 1, 1, true, bp->bounceMarkRadius, false);
        p->bounceMarkCount--;
    }
    ...
    if (!trace.allsolid) {
        if (CG_IsParticleSystemValid(&p->childParticleSystem))
            CG_SetParticleSystemLastNormal(p->childParticleSystem,
                                           trace.plane.normal);
    }
}
...
static void CG_RenderParticle(particle_t* p) {
    ...
    if (bp->numFrames) // shader based
    {
        ...
        if (bp->realLight) {
            ...
            VectorMA(bp->initialColor,
                     CG_CalculateTimeFrac(p->birthTime, p->lifeTime, p->colorDelay),
                     colorRange,
                     re.shaderRGBA);
        }
        ...
        re.shaderRGBA[3] =
                (byte)((float) 0xFF *
                       CG_LerpValues(p->alpha.initial, p->alpha.final, CG_CalculateTimeFrac(p->birthTime, p->lifeTime, p->alpha.delay)));

        re.radius = scale;

        re.rotation = CG_LerpValues(
                p->rotation.initial, p->rotation.final, CG_CalculateTimeFrac(p->birthTime, p->lifeTime, p->rotation.delay));
        ...
        if (bp->framerate == 0.0f) {
            ...
            // looping animation
            index =
                    (int) (bp->framerate * timeFrac * p->lifeTime * 0.001) % bp->numFrames;
            re.customShader = bp->shaders[index];
        }
    ...
    if (bp->dynamicLight && !(re.renderfx & RF_THIRD_PERSON)) {
        trap_R_AddLightToScene(
                p->origin, CG_LerpValues(p->dLightRadius.initial, p->dLightRadius.final, CG_CalculateTimeFrac(p->birthTime, p->lifeTime, p->dLightRadius.delay)), 3, (float) bp->dLightColor[0] / (float) 0xFF, (float) bp->dLightColor[1] / (float) 0xFF, (float) bp->dLightColor[2] / (float) 0xFF, 0, 0);
    }
...
void CG_ParticleSystemEntity(centity_t* cent) {
    ...
    if (!CG_IsParticleSystemValid(&cent->entityPS) && !cent->entityPSMissing) {
        cent->entityPS =
                CG_SpawnNewParticleSystem(cgs.gameParticleSystems[es->modelindex]);
