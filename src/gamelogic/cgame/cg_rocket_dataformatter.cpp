...
static void CG_Rocket_DFResolution(int handle, const char* data) {
    ...
    trap_Rocket_DataFormatterFormattedData(
            handle, va("%dx%d ( %s )", w, h, aspectRatio), false);
    BG_Free(aspectRatio);
}
...
static void CG_Rocket_DFServerPing(int handle, const char* data) {
    ...
    trap_Rocket_DataFormatterFormattedData(
            handle,
            *str && Q_isnumeric(*str) ? va("%s ms", Info_ValueForKey(data, "1")) : "",
            false);
}
...
static void CG_Rocket_DFServerPlayers(int handle, const char* data) {
    ...
    trap_Rocket_DataFormatterFormattedData(
            handle, va("%s + (%s) / %s", Info_ValueForKey(data, "1"), Info_ValueForKey(data, "2"), max), true);
}

static void CG_Rocket_DFPlayerName(int handle, const char* data) {
    trap_Rocket_DataFormatterFormattedData(
            handle, va("<div class=\"playername\">%s</div>", CG_Rocket_QuakeToRML(cgs.clientinfo[atoi(Info_ValueForKey(data, "1"))].name)), false);
}

static void CG_Rocket_DFUpgradeName(int handle, const char* data) {
    trap_Rocket_DataFormatterFormattedData(
            handle, BG_Upgrade(atoi(Info_ValueForKey(data, "1")))->humanName, true);
}

static void CG_Rocket_DFVotePlayer(int handle, const char* data) {
    trap_Rocket_DataFormatterFormattedData(
            handle, va("<button onClick=\"exec set ui_dialogCvar1 %s;exec rocket "
                       "ui/dialogs/editplayer.rml load; exec rocket editplayer "
                       "show\">vote/moderate</button>",
                       cgs.clientinfo[atoi(Info_ValueForKey(data, "1"))].name),
            false);
}
...
static void CG_Rocket_DFVoteMap(int handle, const char* data) {
    ...
    if (mapIndex < rocketInfo.data.mapCount) {
        trap_Rocket_DataFormatterFormattedData(
                handle,
                va("<button onClick=\"exec set ui_dialogCvar1 %s;hide maps;exec rocket "
                   "ui/dialogs/mapdialog.rml load; exec rocket mapdialog show\" "
                   "class=\"maps\"><div class=\"levelname\">%s</div> <img "
                   "class=\"levelshot\"src='/meta/%s/%s'/><div "
                   "class=\"hovertext\">Start Vote</div> </button>",
                   rocketInfo.data.mapList[mapIndex].mapLoadName,
                   CG_Rocket_QuakeToRML(rocketInfo.data.mapList[mapIndex].mapName),
                   rocketInfo.data.mapList[mapIndex].mapLoadName,
                   rocketInfo.data.mapList[mapIndex].mapLoadName),
                false);
    }
...
static void CG_Rocket_DFWeaponName(int handle, const char* data) {
    trap_Rocket_DataFormatterFormattedData(
            handle, BG_Weapon(atoi(Info_ValueForKey(data, "1")))->humanName, true);
}

static void CG_Rocket_DFClassName(int handle, const char* data) {
    trap_Rocket_DataFormatterFormattedData(
            handle, BG_Class(atoi(Info_ValueForKey(data, "1")))->name, true);
}
...
static void CG_Rocket_DFCMArmouryBuyWeapon(int handle, const char* data) {
    ...
    trap_Rocket_DataFormatterFormattedData(
            handle, va("<button class='armourybuy %s' onMouseover='setDS "
                       "armouryBuyList weapons %s' %s>%s<img src='/%s'/></button>",
                       Class,
                       Info_ValueForKey(data, "2"),
                       action,
                       Icon,
                       CG_GetShaderNameFromHandle(cg_weapons[weapon].ammoIcon)),
            false);
}
...
static void CG_Rocket_DFCMArmouryBuyUpgrade(int handle, const char* data) {
    ...
    trap_Rocket_DataFormatterFormattedData(
            handle, va("<button class='armourybuy %s' onMouseover='setDS "
                       "armouryBuyList upgrades %s' %s>%s<img src='/%s'/></button>",
                       Class,
                       Info_ValueForKey(data, "2"),
                       action,
                       Icon,
                       CG_GetShaderNameFromHandle(cg_upgrades[upgrade].upgradeIcon)),
            false);
}
...
static void CG_Rocket_DFGWeaponDamage(int handle, const char* data) {
    ...
    trap_Rocket_DataFormatterFormattedData(
            handle, va("<div class=\"barValue\" style=\"width:%d%%;\"></div>", width), false);
}
...
static void CG_Rocket_DFLevelShot(int handle, const char* data) {
    trap_Rocket_DataFormatterFormattedData(
            handle, va("<img class=\"levelshot\" src=\"/levelshots/%s\"/>", Info_ValueForKey(data, "1")), false);
}
...
static void CG_Rocket_DFGearOrReady(int handle, const char* data) {
    ...
    if (cg.intermissionStarted) {
        ...
        if (s && s->team == cg.predictedPlayerState.persistant[PERS_TEAM] &&
            s->weapon != WP_NONE) {
            rml = va("<img src='/%s'/>",
                     CG_GetShaderNameFromHandle(cg_weapons[s->weapon].weaponIcon));
        }

        if (s && s->team == cg.predictedPlayerState.persistant[PERS_TEAM] &&
            s->team == TEAM_HUMANS && s->upgrade != UP_NONE) {
...
static void CG_Rocket_DFCMAlienBuildables(int handle, const char* data) {
    ...
    if (BG_BuildableDisabled(buildable) || !BG_BuildableUnlocked(buildable)) {
        ...
        Class = "available";
        action = va("onClick='exec \"build %s\"; hide %s'",
                    BG_Buildable(buildable)->name,
                    rocketInfo.menu[ROCKETMENU_ALIENBUILD].id);
    }

    trap_Rocket_DataFormatterFormattedData(
            handle,
            va("<button class='%s' onMouseover='setDS alienBuildList default %s' "
               "%s>%s<img src='/%s'/></button>",
               Class,
               Info_ValueForKey(data, "2"),
               action,
               Icon,
...
static void CG_Rocket_DFCMHumanBuildables(int handle, const char* data) {
    ...
    if (BG_BuildableDisabled(buildable) || !BG_BuildableUnlocked(buildable)) {
        ...
        action = va("onClick='exec \"build %s\"; hide %s'",
                    BG_Buildable(buildable)->name,
                    rocketInfo.menu[ROCKETMENU_HUMANBUILD].id);
    }

    trap_Rocket_DataFormatterFormattedData(
            handle,
            va("<button class='%s' onMouseover='setDS humanBuildList default %s' "
               "%s>%s<img src='/%s'/></button>",
               Class,
               Info_ValueForKey(data, "2"),
               action,
...
static void CG_Rocket_DFCMAlienEvolve(int handle, const char* data) {
    ...
    const char* Icon = "";
    const char* action = "";
    int cost = BG_ClassCanEvolveFromTo(
            cg.predictedPlayerState.stats[STAT_CLASS], alienClass, cg.predictedPlayerState.persistant[PERS_CREDIT]);
    ...
    if (cg.predictedPlayerState.stats[STAT_CLASS] == alienClass) {
        ...
        Class = "available";
        action =
                va("onClick='exec \"class %s\"; hide %s'", BG_Class(alienClass)->name, rocketInfo.menu[ROCKETMENU_ALIENEVOLVE].id);
    }

    trap_Rocket_DataFormatterFormattedData(
            handle,
            va("<button class='alienevo %s' onMouseover='setDS alienEvolveList "
               "alienClasss %s' %s>%s<img src='/%s'/></button>",
               Class,
               Info_ValueForKey(data, "2"),
               action,
               Icon,
...
static void CG_Rocket_DFCMBeacons(int handle, const char* data) {
    ...
    trap_Rocket_DataFormatterFormattedData(
            handle, va("<button class='beacons' onMouseover='setDS beacons default "
                       "%s' %s><img src='/%s'/></button>",
                       Info_ValueForKey(data, "2"),
                       action,
                       icon),
...
void CG_Rocket_FormatData(int handle) {
    ...
    cmd = (dataFormatterCmd_t*) bsearch(
            name, dataFormatterCmdList, dataFormatterCmdListCount, sizeof(dataFormatterCmd_t), dataFormatterCmdCmp);
...
void CG_Rocket_RegisterDataFormatters() {
    ...
    for (i = 0; i < dataFormatterCmdListCount; i++) {
        ...
        // bsearch
        if (i != 0 &&
            Q_stricmp(dataFormatterCmdList[i - 1].name,
                      dataFormatterCmdList[i].name) > 0) {
            CG_Printf(
                    "CGame Rocket dataFormatterCmdList is in the wrong order for %s and "
                    "%s\n",
                    dataFormatterCmdList[i - 1].name,
                    dataFormatterCmdList[i].name);
        }
