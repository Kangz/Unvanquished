...
It also adds content flags to allow for more specific traces in synchronized
code.
====================
*/
void CG_BuildSolidList() {
    ...
    for (i = 0; i < snap->entities.size(); i++) {
        ...
        if (ent->eType == ET_ITEM || ent->eType == ET_PUSHER ||
            ent->eType == ET_TELEPORTER) {
...
====================
*/
static void CG_ClipMoveToEntities(const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end, int skipNumber, int mask, int skipmask, trace_t* tr, traceType_t collisionType) {
    int i, x, zd, zu;
    trace_t trace;
    entityState_t* ent;
    clipHandle_t cmodel;
    ...
    for (i = 0; i < cg_numSolidEntities; i++) {
        ...
        switch (collisionType) {
            case TT_CAPSULE:
                trap_CM_TransformedCapsuleTrace(&trace, start, end, mins, maxs, cmodel, mask, skipmask, origin, angles);
                break;

            case TT_AABB:
                trap_CM_TransformedBoxTrace(&trace, start, end, mins, maxs, cmodel, mask, skipmask, origin, angles);
                break;

            case TT_BISPHERE:
                assert(maxs != nullptr);
                assert(mins != nullptr);
                trap_CM_TransformedBiSphereTrace(&trace, start, end, mins[0], maxs[0], cmodel, mask, skipmask, origin);
                break;
...
static void CG_TouchTriggerPrediction() {
    ...
    for (i = 0; i < cg_numTriggerEntities; i++) {
        ...
        trap_CM_BoxTrace(&trace, cg.predictedPlayerState.origin, cg.predictedPlayerState.origin, cg_pmove.mins, cg_pmove.maxs, cmodel, MASK_ALL, 0);
...
static int CG_IsUnacceptableError(playerState_t* ps, playerState_t* pps) {
    ...
    if (pps->pm_type != ps->pm_type || pps->pm_flags != ps->pm_flags ||
    ...
    if (pps->weaponTime != ps->weaponTime || pps->gravity != ps->gravity ||
        pps->speed != ps->speed || pps->delta_angles[0] != ps->delta_angles[0] ||
    ...
    if (pps->clientNum != ps->clientNum || pps->weapon != ps->weapon ||
...
void CG_PredictPlayerState() {
    ...
    for (cmdNum = current - CMD_BACKUP + 1; cmdNum <= current; cmdNum++) {
        ...
        if (cg_pmove.pmove_fixed) {
            cg_pmove.cmd.serverTime =
                    ((cg_pmove.cmd.serverTime + cg.pmoveParams.msec - 1) /
                     cg.pmoveParams.msec) *
                    cg.pmoveParams.msec;
        }
    ...
    CG_AdjustPositionForMover(
            cg.predictedPlayerState.origin, cg.predictedPlayerState.groundEntityNum, cg.physicsTime, cg.time, cg.predictedPlayerState.origin, cg.predictedPlayerState.viewangles, cg.predictedPlayerState.viewangles);
