...
static float CG_Rocket_GetOverallLoadProgress() {
    return (cg.mediaFraction + cg.charModelFraction + cg.buildablesFraction) /
           3.0f;
}
...
static float CG_Rocket_GetPlayerHealthProgress() {
    ...
    return (float) ps->stats[STAT_HEALTH] /
           (float) BG_Class(ps->stats[STAT_CLASS])->health;
}
...
float CG_Rocket_DownloadProgress() {
    return trap_Cvar_VariableValue("cl_downloadCount") /
           trap_Cvar_VariableValue("cl_downloadSize");
}
...
float CG_Rocket_ProgressBarValue() {
    ...
    cmd = (progressBarCmd_t*) bsearch(
            CG_Argv(0), progressBarCmdList, progressBarCmdListCount, sizeof(progressBarCmd_t), progressBarCmdCmp);
...
float CG_Rocket_ProgressBarValueByName(const char* name) {
    ...
    // Get the progressbar command
    cmd = (progressBarCmd_t*) bsearch(
            name, progressBarCmdList, progressBarCmdListCount, sizeof(progressBarCmd_t), progressBarCmdCmp);
