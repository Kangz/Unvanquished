...
static void CG_ParseTeamInfo() {
    ...
    for (i = 1; i < count;
         ++i) // i is also incremented when writing into cgs.clientinfo
    {
...
static void CG_ConfigStringModified() {
    ...
    if (num == CS_MUSIC) {
        ...
        Q_strncpyz(cgs.voteString[num - CS_VOTE_STRING], str, sizeof(cgs.voteString[num - CS_VOTE_STRING]));
    } else if (num >= CS_VOTE_CALLER && num < CS_VOTE_CALLER + NUM_TEAMS) {
        Q_strncpyz(cgs.voteCaller[num - CS_VOTE_CALLER], str, sizeof(cgs.voteCaller[num - CS_VOTE_CALLER]));
        ...
        cgs.gameShaders[num - CS_SHADERS] = trap_R_RegisterShader(str, RSF_DEFAULT);
...
void CG_Menu(int menuType, int arg) {
    ...
    switch (menuType) {
        ...
        case MN_A_TEAMFULL:
            longMsg =
                    _("The alien team has too many players. Please wait until slots "
        ...
        case MN_H_TEAMFULL:
            longMsg =
                    _("The human team has too many players. Please wait until slots "
        ...
        case MN_A_TEAMLOCKED:
            longMsg =
                    _("The alien team is locked. You cannot join the aliens "
                      "at this time.");
        ...
        case MN_H_TEAMLOCKED:
            longMsg =
                    _("The human team is locked. You cannot join the humans "
                      "at this time.");
        ...
        case MN_PLAYERLIMIT:
            longMsg =
                    _("The maximum number of playing clients has been reached. "
        ...
        case MN_WARMUP:
            longMsg =
                    _("You must wait until the warmup time is finished "
        ...
        case MN_CMD_CHEAT_TEAM:
            shortMsg =
                    _("Cheats are not enabled on this server, so "
                      "you may not use this command while on a team");
            break;

        case MN_CMD_TEAM:
            // longMsg   = "You must be on a team to perform this action. Join the
            // alien"
        ...
        case MN_B_NOROOM:
            longMsg =
                    _("There is no room to build here. Move until the structure turns "
        ...
        case MN_B_NORMAL:
            longMsg =
                    _("Cannot build on this surface. The surface is too steep or "
        ...
        case MN_B_LASTSPAWN:
            longMsg =
                    _("This action would remove your team's last spawn point, "
        ...
        case MN_B_MAINSTRUCTURE:
            longMsg =
                    _("The main structure is protected against instant removal. "
        ...
        case MN_B_REVOKED:
            longMsg =
                    _("Your teammates have lost faith in your ability to build "
        ...
        case MN_B_SURRENDER:
            longMsg =
                    _("Your team has decided to admit defeat and concede the game: "
        ...
        case MN_H_NOBP:
            longMsg =
                    _("There are no resources remaining. Free up resources by "
        ...
        case MN_H_NOTPOWERED:
            longMsg =
                    _("This buildable is not powered. Build a Reactor and/or Repeater "
        ...
        case MN_H_NOPOWERHERE:
            longMsg =
                    _("There is not enough power in this area. Keep a distance to other "
        ...
        case MN_H_NOREACTOR:
            longMsg =
                    _("There is no reactor and the local power supply is insufficient. "
        ...
        case MN_H_ONEREACTOR:
            longMsg =
                    _("There can only be one Reactor. Mark the existing one if you "
                      "wish to move it.");
        ...
        case MN_H_NOSLOTS:
            longMsg =
                    _("You have no room to carry this. Please sell any conflicting "
        ...
        case MN_H_NOFUNDS:
            longMsg =
                    _("Insufficient funds. You do not have enough credits to perform "
                      "this action.");
        ...
        case MN_H_ITEMHELD:
            longMsg =
                    _("You already hold this item. It is not possible to carry multiple "
        ...
        case MN_H_NOARMOURYHERE:
            longMsg =
                    _("You must be near a powered Armoury in order to purchase "
        ...
        case MN_H_NOENERGYAMMOHERE:
            longMsg =
                    _("You must be near a Reactor or a powered Armoury or Repeater "
        ...
        case MN_H_ARMOURYBUILDTIMER:
            longMsg =
                    _("You are not allowed to buy or sell weapons until your "
                      "build timer has expired.");
            shortMsg =
                    _("You can not buy or sell weapons until your build timer "
                      "expires");
            break;
        ...
        case MN_A_NOCREEP:
            longMsg =
                    _("There is no creep here. You must build near existing Eggs or "
        ...
        case MN_A_NOOVMND:
            longMsg =
                    _("There is no Overmind. An Overmind must be built to control "
        ...
        case MN_A_ONEOVERMIND:
            longMsg = _(
                    "There can only be one Overmind. Deconstruct the existing one if you "
                    "wish to move it.");
        ...
        case MN_A_NOBP:
            longMsg =
                    _("The Overmind cannot control any more structures. Deconstruct "
                      "existing "
        ...
        case MN_A_NOEROOM:
            longMsg =
                    _("There is no room to evolve here. Move away from walls or other "
        ...
        case MN_A_TOOCLOSE:
            longMsg =
                    _("This location is too close to the enemy to evolve. Move away "
        ...
        case MN_A_NOOVMND_EVOLVE:
            longMsg =
                    _("There is no Overmind. An Overmind must be built to allow "
                      "you to upgrade.");
            shortMsg = _("There is no Overmind");
            break;
        ...
        case MN_A_INFEST:
            trap_Cvar_Set("ui_currentClass",
                          va("%d %d", cg.snap->ps.stats[STAT_CLASS], cg.snap->ps.persistant[PERS_CREDIT]));
...
static void CG_Say(const char* name, int clientNum, saymode_t mode, const char* text) {
    ...
    if (clientNum >= 0 && clientNum < MAX_CLIENTS) {
        ...
        if (cg_chatTeamPrefix.integer) {
            Com_sprintf(prefix, sizeof(prefix), "[%s%c" S_COLOR_WHITE "] ", tcolor, toupper(*(BG_TeamName(ci->team))));
        }
    ...
    switch (mode) {
        ...
        case SAY_ADMINS_PUBLIC:
            CG_Printf("%s%s%s%s^7: ^%c%s\n", ignore, prefix, (mode == SAY_ADMINS) ? "[ADMIN]" : "[PLAYER]", name, color, text);
            break;
        ...
        case SAY_PRIVMSG:
        case SAY_TPRIVMSG:
            CG_Printf("%s%s[%s^7 → %s^7]: ^%c%s\n", ignore, prefix, name, cgs.clientinfo[cg.clientNum].name, color, text);

            if (!ignore[0]) {
                CG_CenterPrint(va(_("^%cPrivate message from: ^7%s"), color, name), 200, GIANTCHAR_WIDTH * 4);
...
static voiceTrack_t* CG_VoiceTrack(char* voice, int cmd, int track) {
    ...
    if (!c) {
        CG_Printf(
                "[skipnotify]WARNING: could not find command %d "
                "in voice \"%s\"\n",
                cmd,
                voice);
        return nullptr;
    ...
    if (!t) {
        CG_Printf(
                "[skipnotify]WARNING: could not find track %d for command %d in "
                "voice \"%s\"\n",
                track,
                cmd,
                voice);
...
static void CG_ServerCommand() {
    ...
    command = (consoleCommand_t*) bsearch(cmd, svcommands, ARRAY_LEN(svcommands), sizeof(svcommands[0]), cmdcmp);
