...
static bool CG_ParseMinimapZone(minimapZone_t* z, const char** text) {
    ...
    while (1) {
        ...
        if (!Q_stricmp(token, "bounds")) {
            if (!ParseFloats(z->boundsMin, 3, text) ||
                !ParseFloats(z->boundsMax, 3, text)) {
            ...
            if (!*(token = COM_Parse(text))) {
                CG_Printf(S_ERROR
                          "error while parsing the image name while parsing 'image'\n");
            }

            z->image = trap_R_RegisterShader(token, RSF_DEFAULT);

            if (!ParseFloats(z->imageMin, 2, text) ||
                !ParseFloats(z->imageMax, 2, text)) {
            ...
            if (!*(token = COM_Parse(text))) {
                CG_Printf(S_ERROR
                          "error while parsing the value  while parsing 'scale'\n");
            }
...
================
*/
static void CG_SetupMinimapTransform(const rectDef_t* rect,
                                     const minimap_t* minimap,
                                     const minimapZone_t* zone) {
    ...
    scale = transformScale * MINIMAP_MAP_DISPLAY_SIZE /
            (zone->imageMax[0] - zone->imageMin[0]);
    ...
    // the minimap is shown with Z pointing to the viewer but OpenGL has Z
...
static void CG_MinimapUpdateTeammateFadingAndPos( centity_t* mate )
{
    ...
    if( state->minimapFadingOut )
    {
        ...
        {
            state->minimapFading = MAX( 0.0f, state->minimapFading -
cg.frametime * MINIMAP_FADE_TIME );
        }
    ...
    else
    {
        ...
        bool shouldStayVisible = mate->valid && ! ( mate->currentState.eFlags &
EF_DEAD );
        ...
        else
        {
            ...
            {
                state->minimapFading = MIN( 1.0f, state->minimapFading +
cg.frametime * MINIMAP_FADE_TIME );
            }

            //Copy the current state so that we can use it once the player is
out of the PVS
...
static void CG_MinimapDrawTeammates( const minimap_t* m )
{
    ...
    for ( i = 0; i < MAX_GENTITIES; i++ )
    {
        ...
        bool isTeammate = mate->currentState.eType == ET_PLAYER && clientNum >=
0 && clientNum < MAX_CLIENTS && (mate->currentState.misc & 0x00FF) == ownTeam;
        ...
        if( state->minimapFading != 0.0f )
        {
            CG_DrawMinimapObject( m->gfx.teamArrow, state->lastMinimapPos,
state->lastMinimapAngle, 1.0, MINIMAP_TEAMMATE_DISPLAY_SIZE,
state->minimapFading );
        }
    }
}
...
static void CG_MinimapDrawBeacon(const cbeacon_t* b, float size, const vec2_t center, const vec2_t* bounds) {
    ...
    if (pos2d[0] < bounds[0][0] || pos2d[0] > bounds[1][0] ||
        pos2d[1] < bounds[0][1] || pos2d[1] > bounds[1][1]) {
        clamped = true;
    ...
    if (b->flags & EF_BC_DYING)
        trap_R_DrawStretchPic(pos2d[0] - size / 2 * 0.3, pos2d[1] - size / 2 * 0.3, size * 1.3, size * 1.3, 0, 0, 1, 1, cgs.media.beaconNoTarget);
    if (clamped)
        trap_R_DrawRotatedPic(pos2d[0] - size * 0.25, pos2d[1] - size * 0.25, size * 1.5, size * 1.5, 0.0, 0.0, 1.0, 1.0, cgs.media.beaconIconArrow, 270.0 - atan2(dir[1], dir[0]) * 180 / M_PI);
}
...
void CG_InitMinimap() {
    ...
    if (!CG_ParseMinimap(m, va("minimaps/%s.minimap", cgs.mapname))) {
        m->defined = false;
        CG_Printf(S_WARNING
                  "could not parse the minimap, defaulting to no minimap.\n");
    } else if (m->nZones == 0) {
        m->defined = false;
    ...
    m->gfx.playerArrow =
            trap_R_RegisterShader("gfx/2d/player-arrow", RSF_DEFAULT);
