...
static void CG_GetBindings(team_t team) {
    ...
    std::vector<std::vector<int>> keyNums =
            trap_Key_GetKeynumForBinds(team, binds);
    ...
    for (i = 0; i < numBindings; i++) {
        if (!Q_stricmp(command, bindings[i].command)) {
            if (bindings[i].keys[0] != K_NONE) {
                trap_Key_KeynumToStringBuf(bindings[i].keys[0], keyName[0], sizeof(keyName[0]));

                if (bindings[i].keys[1] != K_NONE) {
                    trap_Key_KeynumToStringBuf(bindings[i].keys[1], keyName[1], sizeof(keyName[1]));
                    Q_snprintf(buffer[which], sizeof(buffer[0]), _("%s or %s"), Q_strupr(keyName[0]), Q_strupr(keyName[1]));
                } else {
                    Q_strncpyz(buffer[which], Q_strupr(keyName[0]), sizeof(buffer[0]));
                }
            } else {
                Com_sprintf(buffer[which], MAX_STRING_CHARS, _("\"%s\" (unbound)"), _(bindings[i].humanName));
            }
...
static void CG_BuilderText(char* text, playerState_t* ps) {
    ...
    if (buildable > BA_NONE) {
        ...
        Q_strcat(text, MAX_TUTORIAL_TEXT, va(_("Press %s to place the %s\n"), CG_KeyNameForCommand("+attack"), item));
        Q_strcat(text, MAX_TUTORIAL_TEXT, va(_("Press %s to cancel placing the %s\n"), CG_KeyNameForCommand("+attack2"), item));
    } else {
        Q_strcat(text, MAX_TUTORIAL_TEXT, va(_("Press %s to build a structure\n"), CG_KeyNameForCommand("+attack")));
    }

    if ((es = CG_BuildableInRange(ps, nullptr))) {
        const char* key = CG_KeyNameForCommand(
                "modcase alt \"/deconstruct marked\" /deconstruct");

        if (es->eFlags & EF_B_MARKED) {
            Q_strcat(
                    text, MAX_TUTORIAL_TEXT, va(_("Press %s to unmark this structure for replacement\n"), key));
        } else {
            Q_strcat(text, MAX_TUTORIAL_TEXT, va(_("Press %s to mark this structure for replacement\n"), key));
        }
    }
}
...
static void CG_AlienBuilderText(char* text, playerState_t* ps) {
    ...
    if (ps->stats[STAT_CLASS] == PCL_ALIEN_BUILDER0_UPG) {
        Q_strcat(text, MAX_TUTORIAL_TEXT, va(_("Press %s to launch a projectile\n"), CG_KeyNameForCommand("+useitem")));
...
static void CG_AlienLevel0Text(char* text, playerState_t* ps) {
    ...
    Q_strcat(text, MAX_TUTORIAL_TEXT, _("Touch humans to damage them\n"
...
static void CG_AlienLevel2Text(char* text, playerState_t* ps) {
    Q_strcat(text, MAX_TUTORIAL_TEXT, va(_("Press %s to bite\n"), CG_KeyNameForCommand("+attack")));

    if (ps->stats[STAT_CLASS] == PCL_ALIEN_LEVEL2_UPG) {
        Q_strcat(text, MAX_TUTORIAL_TEXT, va(_("Press %s to invoke an electrical attack\n"), CG_KeyNameForCommand("+attack2")));
    }

    Q_strcat(text, MAX_TUTORIAL_TEXT, va(_("Hold down %s then touch a wall to wall jump\n"), CG_KeyNameForCommand("+moveup")));
}
...
static void CG_AlienLevel3Text(char* text, playerState_t* ps) {
    ...
    Q_strcat(text, MAX_TUTORIAL_TEXT, va(_("Hold down and release %s to pounce\n"), CG_KeyNameForCommand("+attack2")));
}
...
static void CG_AlienLevel4Text(char* text, playerState_t* ps) {
    ...
    Q_strcat(text, MAX_TUTORIAL_TEXT, va(_("Hold down and release %s while moving forwards to trample\n"), CG_KeyNameForCommand("+attack2")));
}
...
static void CG_HumanText(char* text, playerState_t* ps) {
    ...
    if (!ps->ammo && !ps->clips && !BG_Weapon(ps->weapon)->infiniteAmmo) {
        ...
        switch (ps->weapon) {
            ...
            case WP_SHOTGUN:
            case WP_FLAMER:
                Q_strcat(text, MAX_TUTORIAL_TEXT, _("Find an Armoury for more ammo\n"));
                break;
            ...
            case WP_LUCIFER_CANNON:
                Q_strcat(text, MAX_TUTORIAL_TEXT, _("Find an Armoury, Reactor, or Repeater for more ammo\n"));
                break;
        ...
        switch (ps->weapon) {
            ...
            case WP_FLAMER:
                Q_strcat(
                        text, MAX_TUTORIAL_TEXT, va(_("Press %s to fire the %s\n"), CG_KeyNameForCommand("+attack"), _(BG_Weapon(ps->weapon)->humanName)));
                break;
            ...
            case WP_MASS_DRIVER:
                ...
                Q_strcat(text, MAX_TUTORIAL_TEXT, va(_("Hold %s to zoom\n"), CG_KeyNameForCommand("+attack2")));
                break;

            case WP_PAIN_SAW:
                Q_strcat(text, MAX_TUTORIAL_TEXT, va(_("Hold %s to activate the %s\n"), CG_KeyNameForCommand("+attack"), _(BG_Weapon(ps->weapon)->humanName)));
                break;

            case WP_LUCIFER_CANNON:
                Q_strcat(text, MAX_TUTORIAL_TEXT, va(_("Hold and release %s to fire a charged shot\n"), CG_KeyNameForCommand("+attack")));

                Q_strcat(
                        text, MAX_TUTORIAL_TEXT, va(_("Press %s to fire the %s\n"), CG_KeyNameForCommand("+attack2"), _(BG_Weapon(ps->weapon)->humanName)));
                break;
    ...
    if (upgrade == UP_NONE ||
        ...
        Q_strcat(text, MAX_TUTORIAL_TEXT, va(_("Press %s to use the %s\n"), CG_KeyNameForCommand("+useitem"), name));
    }

    if (ps->stats[STAT_HEALTH] <= 35 &&
        BG_InventoryContainsUpgrade(UP_MEDKIT, ps->stats)) {
        Q_strcat(text, MAX_TUTORIAL_TEXT, va(_("Press %s to use your %s\n"), CG_KeyNameForCommand("itemact medkit"), _(BG_Upgrade(UP_MEDKIT)->humanName)));
    }

    switch (cg.nearUsableBuildable) {
        case BA_H_ARMOURY:
            Q_strcat(text, MAX_TUTORIAL_TEXT, va(_("Press %s to buy equipment upgrades at the %s\n"), CG_KeyNameForCommand("+activate"), _(BG_Buildable(cg.nearUsableBuildable)->humanName)));
            break;
        ...
        default:
            Q_strcat(
                    text, MAX_TUTORIAL_TEXT, va(_("Press %s to use the %s\n"), CG_KeyNameForCommand("+activate"), _(BG_Buildable(cg.nearUsableBuildable)->humanName)));
            break;
    }

    Q_strcat(text, MAX_TUTORIAL_TEXT, va(_("Press %s and any direction to sprint\n"), CG_KeyNameForCommand("+sprint")));
...
static void CG_SpectatorText(char* text, playerState_t* ps) {
    if (cgs.clientinfo[cg.clientNum].team != TEAM_NONE) {
        if (ps->pm_flags & PMF_QUEUED) {
            Q_strcat(text, MAX_TUTORIAL_TEXT, va(_("Press %s to leave spawn queue\n"), CG_KeyNameForCommand("+attack")));
        } else {
            Q_strcat(text, MAX_TUTORIAL_TEXT, va(_("Press %s to spawn\n"), CG_KeyNameForCommand("+attack")));
        }
    } else {
        Q_strcat(text, MAX_TUTORIAL_TEXT, va(_("Press %s to join a team\n"), CG_KeyNameForCommand("+attack")));
    }

    if (ps->pm_flags & PMF_FOLLOW) {
        if (!cg.chaseFollow) {
            Q_strcat(text, MAX_TUTORIAL_TEXT, va(_("Press %s to switch to chase-cam spectator mode\n"), CG_KeyNameForCommand("+useitem")));
        } else if (cgs.clientinfo[cg.clientNum].team == TEAM_NONE) {
            Q_strcat(text, MAX_TUTORIAL_TEXT, va(_("Press %s to return to free spectator mode\n"), CG_KeyNameForCommand("+useitem")));
        } else {
            Q_strcat(text, MAX_TUTORIAL_TEXT, va(_("Press %s to stop following\n"), CG_KeyNameForCommand("+useitem")));
        }
    } else {
        Q_strcat(text, MAX_TUTORIAL_TEXT, va(_("Press %s to follow a player\n"), CG_KeyNameForCommand("+useitem")));
    }
}
        ...
        if (ps->persistant[PERS_SPECSTATE] != SPECTATOR_NOT ||
            ...
            if (ps->persistant[PERS_TEAM] == TEAM_ALIENS) {
                if (BG_AlienCanEvolve(ps->stats[STAT_CLASS],
                    ...
                    Q_strcat(
                            text, MAX_TUTORIAL_TEXT, va(_("Press %s to evolve\n"), CG_KeyNameForCommand("+activate")));
                }
            }
        }
    } else if (!cg.demoPlayback) {
        if (!CG_ClientIsReady(ps->clientNum)) {
            Q_strcat(text, MAX_TUTORIAL_TEXT, va(_("Press %s when ready to continue\n"), CG_KeyNameForCommand("+attack")));
        } else {
