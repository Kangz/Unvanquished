...
static void CG_DrawBoxFace(bool solid, vec3_t a, vec3_t b, vec3_t c, vec3_t d) {
    ...
    trap_R_AddPolyToScene(solid ? cgs.media.whiteShader : cgs.media.outlineShader,
                          4,
                          verts);
}
...
void CG_SetEntitySoundPosition(centity_t* cent) {
    if (cent->currentState.solid == SOLID_BMODEL) {
        ...
        trap_S_UpdateEntityVelocity(cent->currentState.number,
...
static void CG_EntityEffects(centity_t* cent) {
    ...
    if (cent->currentState.constantLight &&
        (cent->currentState.eType == ET_MOVER ||
         cent->currentState.eType == ET_MODELDOOR)) {
        int cl;
        int i, r, g, b;
...
static void CG_Missile(centity_t* cent) {
    ...
    // add dynamic light
    if (ma->usesDlight) {
        trap_R_AddLightToScene(cent->lerpOrigin, ma->dlight, ma->dlightIntensity, ma->dlightColor[0], ma->dlightColor[1], ma->dlightColor[2], 0, 0);
    }
    ...
    if (ma->usesSprite) {
        ...
        // apply rotation from config
        {
            ...
            QuatFromAngles(rotQuat, ma->modelRotation[PITCH], ma->modelRotation[YAW], ma->modelRotation[ROLL]);
    ...
    if (ma->trailSystem && !CG_IsTrailSystemValid(&cent->missileTS)) {
      ...
      if ( attachToTag )
      {
              CG_SetAttachmentTag( &cent->missileTS->frontAttachment, &ent,
      ent.hModel, tag );
    ...
    if (!CG_IsParticleSystemValid(&cent->entityPS)) {
        ...
        if (CG_IsParticleSystemValid(&cent->entityPS)) {
            ...
            CG_SetAttachmentPoint(&cent->entityPS->attachment,
                                  cent->currentState.origin);
            CG_AttachToPoint(&cent->entityPS->attachment);
        }
    }
}

#define SETBOUNDS(v1, v2, r) \
    ((v1)[0] = (-r / 2), (v1)[1] = (-r / 2), (v1)[2] = (-r / 2), (v2)[0] = (r / 2), (v2)[1] = (r / 2), (v2)[2] = (r / 2))
...
static void CG_LightFlare(centity_t* cent) {
    ...
    if (maxAngle > 0.0f) {
        float radiusMod =
                1.0f - (180.0f - RAD2DEG(acos(DotProduct(delta, forward)))) / maxAngle;
...
static void CG_Lev2ZapChain(centity_t* cent) {
    ...
    count =
            BG_UnpackEntityNumbers(es, entityNums, LEVEL2_AREAZAP_MAX_TARGETS + 1);
...
static void CG_CalcEntityLerpPositions(centity_t* cent) {
    ...
    if (cg_projectileNudge.integer && !cg.demoPlayback &&
    ...
    if (timeshift) {
        ...
        CG_Trace(&tr, lastOrigin, vec3_origin, vec3_origin, cent->lerpOrigin, cent->currentState.number, MASK_SHOT, 0);
 ...
 *        Performs cleanup operations, e.g. destroying attached particle
 * effects.
 * @param cent
 */
static void CG_CEntityPVSLeave(centity_t* cent) {
    ...
    // Better TODO: Make two groups cent->temporaryPS[NUM_TMPPS],
    // cent->persistentPS[NUM_PERSPS]
    //              and use an enumerator each for different types of mutually
    //              non-exclusive PS.

    // NOTE: Don't destroy cent->entityPS here since it is supposed to survive for
    // certain
