...
static void CG_DrawBeacon(cbeacon_t* b) {
    ...
    if (b->type == BCT_TAG && b->clamped &&
        !((b->flags & EF_BC_ENEMY) && (b->flags & EF_BC_TAG_PLAYER)))
        return;
    ...
    trap_R_DrawStretchPic(b->pos[0] - b->size / 2, b->pos[1] - b->size / 2, b->size, b->size, 0, 0, 1, 1, CG_BeaconIcon(b));
    ...
    if (b->clamped)
        trap_R_DrawRotatedPic(
                b->pos[0] - b->size / 2 * 1.5, b->pos[1] - b->size / 2 * 1.5, b->size * 1.5, b->size * 1.5, 0, 0, 1, 1, cgs.media.beaconIconArrow, 270.0 - (angle = atan2(b->clamp_dir[1], b->clamp_dir[0])) * 180 / M_PI);
...
static void CG_Draw2D() {
    ...
    if (cg.zoomed) {
        ...
        trap_R_DrawStretchPic(
                (cgs.glconfig.vidWidth / 2) - (cgs.glconfig.vidHeight / 2), 0, cgs.glconfig.vidHeight, cgs.glconfig.vidHeight, 0, 0, 1, 1, cgs.media.scopeShader);
        trap_R_SetColor(black);
        trap_R_DrawStretchPic(
                0, 0, (cgs.glconfig.vidWidth / 2) - (cgs.glconfig.vidHeight / 2), cgs.glconfig.vidHeight, 0, 0, 1, 1, cgs.media.whiteShader);
        trap_R_DrawStretchPic(
                cgs.glconfig.vidWidth -
                        ((cgs.glconfig.vidWidth / 2) - (cgs.glconfig.vidHeight / 2)),
                0,
                (cgs.glconfig.vidWidth / 2) - (cgs.glconfig.vidHeight / 2),
...
static void CG_DrawBinaryShadersFinalPhases() {
    ...
    char str[20];
    float f, l, u;
    polyVert_t verts[4] = {{{0, 0, 0}, {0, 0}, {255, 255, 255, 255}},
