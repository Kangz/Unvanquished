...
these should refer only to playerstates that belong to the client, not the
followed player, ui cvars will do that already
===============
*/
static void CG_SetPVars() {
    ...
    /* if we follow someone, the stats won't be about us, but the followed player
   * instead */
    ...
    switch (ps->persistant[PERS_TEAM]) {
        ...
        case TEAM_NONE:
            ...
            /*
       * if we were on a team before we would want these to be reset (or they
       * could mess with bindings)
       * the only ones, that actually are helpful to hold on to are p_score and
       * p_credits, since these actually
       * might be taken over when joining a team again, or be used to look up
       * old values before leaving the game
       */
...
static void CG_SetUIVars() {
    ...
    trap_Cvar_Set("ui_carriage", va("%d %d %d", cg.snap->ps.stats[STAT_WEAPON], cg.snap->ps.stats[STAT_ITEMS], cg.snap->ps.persistant[PERS_CREDIT]));
}
    ...
    if (cg_rangeMarkerBuildableTypes.modificationCount != btmc ||
        ...
        for (;;) {
            ...
            if (buildable != BA_NONE) {
                ...
                brmMask |=
                        (1 << BA_A_OVERMIND) | (1 << BA_A_SPAWN) | (1 << BA_A_ACIDTUBE) |
                ...
                if (pp != p && !*pp) {
                    ...
                    brmMask |= only & ((1 << BA_A_OVERMIND) | (1 << BA_A_SPAWN) |
                                       (1 << BA_A_LEECH) | (1 << BA_A_BOOSTER) |
                                       (1 << BA_H_REACTOR) | (1 << BA_H_REPEATER) |
                                       (1 << BA_H_DRILL));
                } else if (!Q_stricmp(pp, "offensive")) {
                    brmMask |= only & ((1 << BA_A_ACIDTUBE) | (1 << BA_A_TRAPPER) |
...
void CG_NotifyHooks() {
    ...
    static int lastTeam =
            INT_MIN; // to make sure we run the hook initially as well
    ...
    if (!(ps->pm_flags & PMF_FOLLOW)) {
        if (lastTeam != ps->persistant[PERS_TEAM]) {
            ...
            trap_Cvar_VariableStringBuffer(
                    va("cg_%sConfig", BG_TeamName(ps->persistant[PERS_TEAM])), config, sizeof(config));
            if (config[0]) {
...
void QDECL PRINTF_LIKE(2) NORETURN
...
static void CG_UpdateLoadingStep(cgLoadingStep_t step) {
    ...
    switch (step) {
        ...
        case LOAD_PARTICLES:
            CG_UpdateLoadingProgress(
                    LOADBAR_MEDIA, 0.05f, choose("Collecting bees for the hives", "Initialising fireworks", "Causing electrical faults", nullptr));
            break;
        case LOAD_SOUNDS:
        ...
        case LOAD_GEOMETRY:
            CG_UpdateLoadingProgress(
                    LOADBAR_MEDIA, 0.60f, choose("Hello World!", "Making a scene.", nullptr));
            break;
        case LOAD_ASSETS:
            CG_UpdateLoadingProgress(
                    LOADBAR_MEDIA, 0.63f, choose("Taking pictures of the world", "Using your laptop's camera", "Adding texture to concrete", "Drawing smiley faces", nullptr));
            break;
        case LOAD_CONFIGS:
            CG_UpdateLoadingProgress(
                    LOADBAR_MEDIA, 0.80f, choose("Reading the manual", "Looking at blueprints", nullptr));
            break;
        case LOAD_WEAPONS:
            CG_UpdateLoadingProgress(
                    LOADBAR_MEDIA, 0.90f, choose("Setting up the armoury", "Sharpening the aliens' claws", "Overloading lucifer cannons", nullptr));
            break;
        case LOAD_UPGRADES:
        case LOAD_CLASSES:
            CG_UpdateLoadingProgress(
                    LOADBAR_MEDIA, 0.95f, choose("Charging battery packs", "Replicating alien DNA", "Packing tents for jetcampers", nullptr));
            break;
        case LOAD_BUILDINGS:
            cg.mediaFraction = 1.0f;
            CG_UpdateLoadingProgress(
                    LOADBAR_BUILDABLES, 0.0f, choose("Finishing construction", "Adding turret spam", "Awakening the overmind", nullptr));
            break;
...
static void CG_RegisterSounds() {
    ...
    cgs.media.weHaveEvolved =
            trap_S_RegisterSound("sound/announcements/overmindevolved.wav", true);
    cgs.media.reinforcement =
            trap_S_RegisterSound("sound/announcements/reinforcement.wav", true);

    cgs.media.alienOvermindAttack =
            trap_S_RegisterSound("sound/announcements/overmindattack.wav", true);
    cgs.media.alienOvermindDying =
            trap_S_RegisterSound("sound/announcements/overminddying.wav", true);
    cgs.media.alienOvermindSpawns =
            trap_S_RegisterSound("sound/announcements/overmindspawns.wav", true);

    cgs.media.alienL4ChargePrepare =
            trap_S_RegisterSound("sound/player/level4/charge_prepare.wav", true);
    cgs.media.alienL4ChargeStart =
            trap_S_RegisterSound("sound/player/level4/charge_start.wav", true);

    cgs.media.selectSound =
            trap_S_RegisterSound("sound/weapons/change.wav", false);
    cgs.media.turretSpinupSound =
            trap_S_RegisterSound("sound/buildables/mgturret/spinup.wav", false);
    cgs.media.weaponEmptyClick =
            trap_S_RegisterSound("sound/weapons/click.wav", false);

    cgs.media.talkSound = trap_S_RegisterSound("sound/misc/talk.wav", false);
    cgs.media.alienTalkSound =
            trap_S_RegisterSound("sound/misc/alien_talk.wav", false);
    cgs.media.humanTalkSound =
            trap_S_RegisterSound("sound/misc/human_talk.wav", false);
    cgs.media.landSound = trap_S_RegisterSound("sound/player/land1.wav", false);

    cgs.media.watrInSound =
            trap_S_RegisterSound("sound/player/watr_in.wav", false);
    cgs.media.watrOutSound =
            trap_S_RegisterSound("sound/player/watr_out.wav", false);
    cgs.media.watrUnSound =
            trap_S_RegisterSound("sound/player/watr_un.wav", false);

    cgs.media.disconnectSound =
            trap_S_RegisterSound("sound/misc/disconnect.wav", false);

    for (i = 0; i < 4; i++) {
        Com_sprintf(name, sizeof(name), "sound/player/footsteps/step%i.wav", i + 1);
        cgs.media.footsteps[FOOTSTEP_GENERAL][i] =
                trap_S_RegisterSound(name, false);
    ...
    cgs.media.jetpackThrustLoopSound =
            trap_S_RegisterSound("sound/upgrades/jetpack/hi.wav", false);

    cgs.media.medkitUseSound =
            trap_S_RegisterSound("sound/upgrades/medkit/medkit.wav", false);

    cgs.media.alienEvolveSound =
            trap_S_RegisterSound("sound/player/alienevolve.wav", false);

    cgs.media.alienBuildableExplosion =
            trap_S_RegisterSound("sound/buildables/alien/explosion.wav", false);
    cgs.media.alienBuildablePrebuild =
            trap_S_RegisterSound("sound/buildables/alien/prebuild.wav", false);

    cgs.media.humanBuildableDying =
            trap_S_RegisterSound("sound/buildables/human/dying.wav", false);
    cgs.media.humanBuildableExplosion =
            trap_S_RegisterSound("sound/buildables/human/explosion.wav", false);
    cgs.media.humanBuildablePrebuild =
            trap_S_RegisterSound("sound/buildables/human/prebuild.wav", false);
    ...
    cgs.media.hardBounceSound1 =
            trap_S_RegisterSound("sound/misc/hard_bounce1.wav", false);
    cgs.media.hardBounceSound2 =
            trap_S_RegisterSound("sound/misc/hard_bounce2.wav", false);

    cgs.media.repeaterUseSound =
            trap_S_RegisterSound("sound/buildables/repeater/use.wav", false);

    cgs.media.buildableRepairSound =
            trap_S_RegisterSound("sound/buildables/human/repair.wav", false);
    cgs.media.buildableRepairedSound =
            trap_S_RegisterSound("sound/buildables/human/repaired.wav", false);

    cgs.media.lCannonWarningSound =
            trap_S_RegisterSound("models/weapons/lcannon/warning.wav", false);
    cgs.media.lCannonWarningSound2 =
            trap_S_RegisterSound("models/weapons/lcannon/warning2.wav", false);

    cgs.media.rocketpodLockonSound =
            trap_S_RegisterSound("sound/rocketpod/lockon.wav", false);

    cgs.media.timerBeaconExpiredSound =
            trap_S_RegisterSound("sound/feedback/beacon-timer-expired.ogg", false);
}
...
void CG_RegisterGrading(int slot, const char* str) {
    ...
    cgs.gameGradingTextures[slot] = trap_R_RegisterShader(
            texture, (RegisterShaderFlags_t)(RSF_NOMIP | RSF_NOLIGHTSCALE));
...
static void CG_RegisterGraphics() {
    ...
    static const char* const sb_nums[11] = {
            "gfx/2d/numbers/zero_32b", "gfx/2d/numbers/one_32b", "gfx/2d/numbers/two_32b", "gfx/2d/numbers/three_32b", "gfx/2d/numbers/four_32b", "gfx/2d/numbers/five_32b", "gfx/2d/numbers/six_32b", "gfx/2d/numbers/seven_32b", "gfx/2d/numbers/eight_32b", "gfx/2d/numbers/nine_32b", "gfx/2d/numbers/minus_32b",
    };
    static const char* const buildWeaponTimerPieShaders[8] = {
            "ui/assets/neutral/1_5pie", "ui/assets/neutral/3_0pie", "ui/assets/neutral/4_5pie", "ui/assets/neutral/6_0pie", "ui/assets/neutral/7_5pie", "ui/assets/neutral/9_0pie", "ui/assets/neutral/10_5pie", "ui/assets/neutral/12_0pie",
    };
    ...
    for (i = 0; i < 11; i++) {
        cgs.media.numberShaders[i] =
                trap_R_RegisterShader(sb_nums[i], (RegisterShaderFlags_t) RSF_DEFAULT);
    }

    cgs.media.viewBloodShader = trap_R_RegisterShader(
            "gfx/damage/fullscreen_painblend", (RegisterShaderFlags_t) RSF_DEFAULT);

    cgs.media.connectionShader =
            trap_R_RegisterShader("gfx/2d/net", (RegisterShaderFlags_t) RSF_DEFAULT);

    cgs.media.creepShader =
            trap_R_RegisterShader("creep", (RegisterShaderFlags_t) RSF_DEFAULT);

    cgs.media.scannerBlipShader =
            trap_R_RegisterShader("gfx/2d/blip", (RegisterShaderFlags_t) RSF_DEFAULT);

    cgs.media.scannerBlipBldgShader = trap_R_RegisterShader(
            "gfx/2d/blip_bldg", (RegisterShaderFlags_t) RSF_DEFAULT);

    cgs.media.scannerLineShader =
            trap_R_RegisterShader("gfx/2d/stalk", (RegisterShaderFlags_t) RSF_DEFAULT);

    cgs.media.teamOverlayShader = trap_R_RegisterShader(
            "gfx/2d/teamoverlay", (RegisterShaderFlags_t) RSF_DEFAULT);

    cgs.media.tracerShader = trap_R_RegisterShader(
            "gfx/misc/tracer", (RegisterShaderFlags_t) RSF_DEFAULT);

    cgs.media.backTileShader =
            trap_R_RegisterShader("console", (RegisterShaderFlags_t) RSF_DEFAULT);

    // building shaders
    cgs.media.greenBuildShader = trap_R_RegisterShader(
            "gfx/misc/greenbuild", (RegisterShaderFlags_t) RSF_DEFAULT);
    cgs.media.redBuildShader = trap_R_RegisterShader(
            "gfx/misc/redbuild", (RegisterShaderFlags_t) RSF_DEFAULT);
    cgs.media.humanSpawningShader = trap_R_RegisterShader(
            "models/buildables/humanSpawning", (RegisterShaderFlags_t) RSF_DEFAULT);

    for (i = 0; i < 8; i++) {
        cgs.media.buildWeaponTimerPie[i] = trap_R_RegisterShader(
                buildWeaponTimerPieShaders[i], (RegisterShaderFlags_t) RSF_DEFAULT);
    }

    // player health cross shaders
    cgs.media.healthCross = trap_R_RegisterShader(
            "ui/assets/neutral/cross", (RegisterShaderFlags_t) RSF_DEFAULT);
    cgs.media.healthCross2X = trap_R_RegisterShader(
            "ui/assets/neutral/cross2", (RegisterShaderFlags_t) RSF_DEFAULT);
    cgs.media.healthCross3X = trap_R_RegisterShader(
            "ui/assets/neutral/cross3", (RegisterShaderFlags_t) RSF_DEFAULT);
    cgs.media.healthCrossMedkit = trap_R_RegisterShader(
            "ui/assets/neutral/cross_medkit", (RegisterShaderFlags_t) RSF_DEFAULT);
    cgs.media.healthCrossPoisoned = trap_R_RegisterShader(
            "ui/assets/neutral/cross_poison", (RegisterShaderFlags_t) RSF_DEFAULT);

    cgs.media.desaturatedCgrade = trap_R_RegisterShader(
            "gfx/cgrading/desaturated",
            (RegisterShaderFlags_t)(RSF_NOMIP | RSF_NOLIGHTSCALE));

    cgs.media.neutralCgrade = trap_R_RegisterShader(
            "gfx/cgrading/neutral",
            (RegisterShaderFlags_t)(RSF_NOMIP | RSF_NOLIGHTSCALE));

    cgs.media.redCgrade = trap_R_RegisterShader(
            "gfx/cgrading/red-only",
            (RegisterShaderFlags_t)(RSF_NOMIP | RSF_NOLIGHTSCALE));

    cgs.media.tealCgrade = trap_R_RegisterShader(
            "gfx/cgrading/teal-only",
            (RegisterShaderFlags_t)(RSF_NOMIP | RSF_NOLIGHTSCALE));

    cgs.media.balloonShader = trap_R_RegisterShader(
            "gfx/sprites/chatballoon", (RegisterShaderFlags_t) RSF_SPRITE);

    cgs.media.disconnectPS = CG_RegisterParticleSystem("disconnectPS");

    cgs.media.scopeShader = trap_R_RegisterShader(
            "scope", (RegisterShaderFlags_t)(RSF_DEFAULT | RSF_NOMIP));
    ...
    cgs.media.shadowMarkShader = trap_R_RegisterShader(
            "gfx/marks/shadow", (RegisterShaderFlags_t) RSF_DEFAULT);
    cgs.media.wakeMarkShader = trap_R_RegisterShader(
            "gfx/marks/wake", (RegisterShaderFlags_t) RSF_DEFAULT);
    ...
    cgs.media.humanBuildableNovaPS =
            CG_RegisterParticleSystem("humanBuildableNovaPS");
    cgs.media.alienBuildableDestroyedPS =
            CG_RegisterParticleSystem("alienBuildableDestroyedPS");

    cgs.media.humanBuildableBleedPS =
            CG_RegisterParticleSystem("humanBuildableBleedPS");
    cgs.media.alienBuildableBleedPS =
            CG_RegisterParticleSystem("alienBuildableBleedPS");
    cgs.media.alienBuildableBurnPS =
            CG_RegisterParticleSystem("alienBuildableBurnPS");
    ...
    cgs.media.sphericalCone64Model =
            trap_R_RegisterModel("models/generic/sphericalCone64.md3");
    cgs.media.sphericalCone240Model =
            trap_R_RegisterModel("models/generic/sphericalCone240.md3");

    cgs.media.plainColorShader = trap_R_RegisterShader(
            "gfx/plainColor", (RegisterShaderFlags_t) RSF_DEFAULT);
    cgs.media.binaryAlpha1Shader = trap_R_RegisterShader(
            "gfx/binary/alpha1", (RegisterShaderFlags_t) RSF_DEFAULT);

    for (i = 0; i < NUM_BINARY_SHADERS; ++i) {
        cgs.media.binaryShaders[i].f1 = trap_R_RegisterShader(
                va("gfx/binary/%03i_F1", i), (RegisterShaderFlags_t) RSF_DEFAULT);
        cgs.media.binaryShaders[i].f2 = trap_R_RegisterShader(
                va("gfx/binary/%03i_F2", i), (RegisterShaderFlags_t) RSF_DEFAULT);
        cgs.media.binaryShaders[i].f3 = trap_R_RegisterShader(
                va("gfx/binary/%03i_F3", i), (RegisterShaderFlags_t) RSF_DEFAULT);
        cgs.media.binaryShaders[i].b1 = trap_R_RegisterShader(
                va("gfx/binary/%03i_B1", i), (RegisterShaderFlags_t) RSF_DEFAULT);
        cgs.media.binaryShaders[i].b2 = trap_R_RegisterShader(
                va("gfx/binary/%03i_B2", i), (RegisterShaderFlags_t) RSF_DEFAULT);
        cgs.media.binaryShaders[i].b3 = trap_R_RegisterShader(
                va("gfx/binary/%03i_B3", i), (RegisterShaderFlags_t) RSF_DEFAULT);
    }
    ...
    cgs.media.beaconIconArrow =
            trap_R_RegisterShader("gfx/2d/beacons/arrow", RSF_DEFAULT);
    cgs.media.beaconNoTarget =
            trap_R_RegisterShader("gfx/2d/beacons/no-target", RSF_DEFAULT);
    cgs.media.beaconTagScore =
            trap_R_RegisterShader("gfx/2d/beacons/tagscore", RSF_DEFAULT);
    ...
    for (i = 1; i < MAX_GAME_SHADERS; i++) {
        ...
        cgs.gameShaders[i] =
                trap_R_RegisterShader(shaderName, (RegisterShaderFlags_t) RSF_DEFAULT);
    }
...
static void CG_RegisterClients() {
    ...
    for (i = PCL_NONE + 1; i < PCL_NUM_CLASSES; i++) {
        CG_PrecacheClientInfo((class_t) i, BG_ClassModelConfig(i)->modelName, BG_ClassModelConfig(i)->skinName);
    ...
    if (!cg_highPolyPlayerModels.integer) {
        ...
        // Borrow these variables for MD5 models so we don't have to create new
        // ones.
        cgs.media.larmourHeadSkin =
                trap_R_RegisterSkin("models/players/human_base/body_helmet.skin");
        cgs.media.larmourLegsSkin =
                trap_R_RegisterSkin("models/players/human_base/body_larmour.skin");
        cgs.media.larmourTorsoSkin = trap_R_RegisterSkin(
                "models/players/human_base/body_helmetlarmour.skin");
    }

    cgs.media.jetpackModel =
            trap_R_RegisterModel("models/players/human_base/jetpack.iqm");
    cgs.media.jetpackFlashModel =
            trap_R_RegisterModel("models/players/human_base/jetpack_flash.md3");
    cgs.media.radarModel = trap_R_RegisterModel(
            "models/players/human_base/battpack.md3"); // HACK: Use old battpack

    CG_RegisterWeaponAnimation(&cgs.media.jetpackAnims[JANIM_NONE],
                               "models/players/human_base/jetpack.iqm:idle",
                               false,
                               false,
                               false);

    CG_RegisterWeaponAnimation(&cgs.media.jetpackAnims[JANIM_SLIDEOUT],
                               "models/players/human_base/jetpack.iqm:slideout",
                               false,
                               false,
                               false);

    CG_RegisterWeaponAnimation(&cgs.media.jetpackAnims[JANIM_SLIDEIN],
...
void CG_Init(int serverMessageNum, int clientNum, glconfig_t gl, GameStateCSs gameState) {
    ...
    // clear everything
    // reset cgs in-place to avoid creating a huge struct on stack (caused a stack
    // overflow)
    ...
    // Set up the pmove params with sensible default values, the server params
    // will
    ...
    // compatibility with official Alpha 34 servers, but shouldn't be necessary
    // anymore for Alpha 35
    ...
    cgs.aspectScale =
            ((640.0f * cgs.glconfig.vidHeight) / (480.0f * cgs.glconfig.vidWidth));

    // load a few needed things before we do any screen updates
    trap_R_SetAltShaderTokens("unpowered,destroyed");
    cgs.media.whiteShader =
            trap_R_RegisterShader("white", (RegisterShaderFlags_t) RSF_DEFAULT);
    cgs.media.charsetShader = trap_R_RegisterShader(
            "gfx/2d/bigchars", (RegisterShaderFlags_t) RSF_DEFAULT);
    cgs.media.outlineShader =
            trap_R_RegisterShader("outline", (RegisterShaderFlags_t) RSF_DEFAULT);
    ...
    // load configs after initializing particles and trails since it registers
    // some
...
static char* CG_VoIPString() {
    ...
    for (slen = i = 0; i < cgs.maxclients; i++) {
        ...
        nlen = Q_snprintf(&voipString[slen], sizeof(voipString) - slen, "%s%d", (slen > 0) ? "," : "", i);
