...
static const char* const cg_buildableSoundNames[MAX_BUILDABLE_ANIMATIONS] = {
        "idle1.wav", "idle2.wav", "powerdown.wav", "idle_unpowered.wav", "construct1.wav", "construct2.wav", "attack1.wav", "attack2.wav", "spawn1.wav", "spawn2.wav", "pain1.wav", "pain2.wav", "destroy1.wav", "destroy2.wav", "destroyed.wav"};
...
// Mapping of buildableAnimNumber_t numbers to animation names (through their
// shorthand) and flags.
// Every row is a buildable, every column is a buildableAnimNumber_t number.
// The pd suffix means "_POWERDOWN".
// TODO: Move this into buildable model config files (ideally after killing
// MD3).
// TODO: Make XX print an error and keep current animation, right now the model
// would become distorted.
static const struct {
    shorthand_t shorthand;
    int flags;
} anims[BA_NUM_BUILDABLES][MAX_BUILDABLE_ANIMATIONS] = {
        // NONE IDLE1  IDLE2  PWRDWN IDLEpd CNSTR  PWRUP  ATTCK1 ATTCK2 SPAWN1
        // SPAWN2 PAIN1  PAIN2  DESTR  DSTRpd DESTRD   // BA_*
        {{XX, 0},
         {XX, 0},
         {XX, 0},
         {XX, 0},
...
static const char* GetAnimationName(buildable_t buildable,
                                    buildableAnimNumber_t animNumber) {
    return shorthandToName[anims[buildable][animNumber].shorthand];
}

static int GetAnimationFlags(buildable_t buildable,
                             buildableAnimNumber_t animNumber) {
...
static bool IsReversed(buildable_t buildable,
                       buildableAnimNumber_t animNumber) {
...
static void CG_Creep(centity_t* cent) {
    ...
    const buildableAttributes_t* attr =
            BG_Buildable(cent->currentState.modelindex);
    ...
    CG_Trace(&tr, cent->lerpOrigin, nullptr, nullptr, temp, cent->currentState.number, MASK_PLAYERSOLID, 0);

    if (size > 0.0f && tr.fraction < 1.0f) {
        CG_ImpactMark(cgs.media.creepShader, tr.endpos, cent->currentState.origin2, 0.0f, 1.0f, 1.0f, 1.0f, 1.0f, false, size, true);
    }
}
...
static void CG_OnFire(centity_t* cent) {
    ...
    switch (team) {
        case TEAM_ALIENS:
            if (!CG_IsParticleSystemValid(&cent->buildableStatusPS)) {
                cent->buildableStatusPS =
                        CG_SpawnNewParticleSystem(cgs.media.alienBuildableBurnPS);
            }
            break;
...
======================
*/
static bool CG_ParseBuildableAnimationFile(const char* filename,
...
======================
*/
static bool CG_ParseBuildableSoundFile(const char* filename,
                                       buildable_t buildable) {
    const char* text_p;
    int len;
    int i;
...
static bool CG_RegisterBuildableAnimation(buildableInfo_t* ci,
    ...
    ci->animations[anim].numFrames =
            trap_R_AnimNumFrames(ci->animations[anim].handle);
...
void CG_InitBuildables() {
    ...
    for (buildable = (buildable_t)(BA_NONE + 1); buildable < BA_NUM_BUILDABLES;
        ...
        if (cg_highPolyBuildableModels.integer &&
            (bi->models[0] = trap_R_RegisterModel(va(
                     "models/buildables/%s/%s.iqm", buildableName, buildableName)))) {
            bi->md5 = true;
            iqm = true;
        } else if (cg_highPolyBuildableModels.integer &&
                   (bi->models[0] =
                            trap_R_RegisterModel(va("models/buildables/%s/%s.md5mesh",
                                                    buildableName,
                                                    buildableName)))) {
            bi->md5 = true;
        } else {
        ...
        if (bi->md5) {
            for (anim = (buildableAnimNumber_t)(BANIM_NONE + 1);
                 anim < MAX_BUILDABLE_ANIMATIONS;
                ...
                if (!CG_RegisterBuildableAnimation(
                            bi, buildableName, anim, animName, IsLooped(buildable, anim), IsReversed(buildable, anim), false, iqm)) {
                    Com_Printf(S_ERROR
                               "Failed to load animation '%s' for buildable '%s' and "
                               "animation slot #%d.",
                               animName,
                               buildableName,
                               anim);
        ...
        for (j = BANIM_NONE + 1; j < MAX_BUILDABLE_ANIMATIONS; j++) {
            ...
            if (cg_buildables[buildable].sounds[j].enabled) {
                if (trap_FS_FOpenFile(filename, &f, FS_READ) > 0) {
                    ...
                    cg_buildables[buildable].sounds[j].sound =
                            trap_S_RegisterSound(filename, false);
                } else {
        ...
        if ((buildableIcon = BG_Buildable(buildable)->icon)) {
            cg_buildables[buildable].buildableIcon =
                    trap_R_RegisterShader(buildableIcon, RSF_DEFAULT);
        }
...
================
*/
bool CG_GetBuildableRangeMarkerProperties(buildable_t bType,
...
static void CG_SetBuildableLerpFrameAnimation(buildable_t buildable,
    ...
    if (!lf->animation) {
        lf->frameTime =
                cg.time + 1000; // 1 sec delay before starting the spawn anim
    }
    ...
    if (lf->blendlerp <= 0.0f) {
        ...
        lf->blendlerp = 1.0f - lf->blendlerp; // use old blending for smooth
                                              // blending between two blended
                                              // animations
    }
}
...
static void CG_RunBuildableLerpFrame(centity_t* cent) {
    ...
    if (cg_buildables[buildable].sounds[lf->animationNumber].looped &&
        ...
        trap_S_AddLoopingSound(
                cent->currentState.number, cent->lerpOrigin, vec3_origin, cg_buildables[buildable].sounds[lf->animationNumber].sound);
    }
...
static void CG_BuildableAnimation(centity_t* cent, int* old, int* now, float* backLerp) {
    ...
    if (!(es->eFlags & EF_B_SPAWNED)) {
        animation_t* anim =
                &cg_buildables[es->modelindex].animations[BANIM_CONSTRUCT];

        // Change the animation in the lerpFrame so that md5s will use it too.
        cent->lerpFrame.animation =
                &cg_buildables[es->modelindex].animations[BANIM_CONSTRUCT];
        ...
        *backLerp = cent->lerpFrame.backlerp = 0.0f;
        ...
        if ((cent->oldBuildableAnim ^ es->legsAnim) & ANIM_TOGGLEBIT) {
            if (cg_debugAnim.integer) {
                CG_Printf("%d->%d l:%d t:%d %s(%d)\n", cent->oldBuildableAnim, cent->buildableAnim, es->legsAnim, es->torsoAnim, BG_Buildable(es->modelindex)->humanName, es->number);
            }

            if (cent->buildableAnim == es->torsoAnim ||
                es->legsAnim & ANIM_FORCEBIT) {
                cent->buildableAnim = cent->oldBuildableAnim =
                        (buildableAnimNumber_t) es->legsAnim;
                cent->buildableIdleAnim = false;
            } else {
                cent->buildableAnim = cent->oldBuildableAnim =
                        (buildableAnimNumber_t) es->torsoAnim;
                cent->buildableIdleAnim = true;
            }
...
===============
*/
static void CG_PositionAndOrientateBuildable(
        const vec3_t angles, const vec3_t inOrigin, const vec3_t normal, const int skipNumber, const vec3_t mins, const vec3_t maxs, vec3_t outAxis[3], vec3_t outOrigin) {
    vec3_t forward, end;
    trace_t tr;
    float fraction;
...
================
*/
static void CG_DrawBuildableRangeMarker(buildable_t buildable,
                                        const vec3_t origin,
                                        const vec3_t normal,
                                        float opacity) {
    rangeMarker_t rmType;
...
void CG_GhostBuildable(int buildableInfo) {
    ...
    float scale;
    buildable_t buildable =
            (buildable_t)(buildableInfo & SB_BUILDABLE_MASK); // assumed not BA_NONE
    ...
    CG_PositionAndOrientateBuildable(angles, entity_origin, tr.plane.normal, ps->clientNum, mins, maxs, ent.axis, ent.origin);

    // offset on the Z axis if required
    VectorMA(ent.origin,
             cg_highPolyBuildableModels.integer ? bmc->zOffset : bmc->oldOffset,
             tr.plane.normal,
             ent.origin);
    ...
    if (cg_buildables[buildable].md5) {
        trap_R_BuildSkeleton(
                &ent.skeleton, cg_buildables[buildable].animations[BANIM_IDLE1].handle, 0, 0, 0, false);
    ...
    // Apply rotation from config.
    {
        ...
        QuatFromAngles(rotQuat, bmc->modelRotation[PITCH], bmc->modelRotation[YAW], bmc->modelRotation[ROLL]);
...
static void CG_GhostBuildableStatus(int buildableInfo) {
    ...
    if (CG_WorldToScreen(entity_origin, &x, &y)) {
        ...
        const buildStat_t* bs =
                (team == TEAM_ALIENS) ? &cgs.alienBuildStat : &cgs.humanBuildStat;
        ...
        if (text) {
            ...
            CG_DrawPic(tx - (picM - picH) / 2,
                       ty - (picM - picH) / 4 - (ty - picY) * 2,
                       (picX - tx) * 2 + (picM - picH),
                       (ty - picY) * 2 + (picM - picH),
...
static void CG_BuildableParticleEffects(centity_t* cent) {
    ...
    if (team == TEAM_HUMANS) {
        if (healthFrac < 0.33f && !CG_IsParticleSystemValid(&cent->buildablePS)) {
            cent->buildablePS =
                    CG_SpawnNewParticleSystem(cgs.media.humanBuildableDamagedPS);
        ...
        if (healthFrac < 0.33f && !CG_IsParticleSystemValid(&cent->buildablePS)) {
            cent->buildablePS =
                    CG_SpawnNewParticleSystem(cgs.media.alienBuildableDamagedPS);
    ...
    while (1) {
        ...
        if (!Q_stricmp(token.string, "frameShader")) {
            if (PC_String_Parse(handle, &s)) {
                bs->frameShader = trap_R_RegisterShader(s, RSF_DEFAULT);
            }

            continue;
        } else if (!Q_stricmp(token.string, "overlayShader")) {
            if (PC_String_Parse(handle, &s)) {
                bs->overlayShader = trap_R_RegisterShader(s, RSF_DEFAULT);
            }

            continue;
        } else if (!Q_stricmp(token.string, "noPowerShader")) {
            if (PC_String_Parse(handle, &s)) {
                bs->noPowerShader = trap_R_RegisterShader(s, RSF_DEFAULT);
            }

            continue;
        } else if (!Q_stricmp(token.string, "markedShader")) {
            if (PC_String_Parse(handle, &s)) {
                bs->markedShader = trap_R_RegisterShader(s, RSF_DEFAULT);
            }
...
static void CG_BuildableStatusDisplay(centity_t* cent) {
    entityState_t* es = &cent->currentState;
    vec3_t origin;
    float healthFrac, powerUsageFrac = 0, totalPowerFrac = 0,
                      mineEfficiencyFrac = 0, storedBPFrac = 0;
    ...
    if (cent->buildableStatus.visible) {
        if (cent->buildableStatus.lastTime + STATUS_FADE_TIME > cg.time) {
            color[3] =
                    (float) (cg.time - cent->buildableStatus.lastTime) / STATUS_FADE_TIME;
        }
    }
    ...
    showMineEfficiency =
            (attr->number == BA_A_LEECH || attr->number == BA_H_DRILL);
    showStoredBP = showMineEfficiency || (attr->number == BA_A_OVERMIND ||
                                          attr->number == BA_H_REACTOR);
    ...
    if (CG_WorldToScreen(origin, &x, &y)) {
        ...
        if (showMineEfficiency) {
            ...
            vec4_t barColor;
            // float levelRate  = cg.predictedPlayerState.persistant[ PERS_MINERATE ]
            // / 10.0f;
            ...
            //               va("Mines %.1f BP/min", mineEfficiencyFrac * levelRate),
            //               0,
        ...
        if (showPower) {
            ...
            // draw mark
            markX =
                    barX + (picW * powerUsageFrac) - (bs->healthPadding * 2.5f * scale);
            markH = 0.5f * barH;
...
void CG_DrawBuildableStatus() {
    ...
    for (i = 0; i < cg.snap->entities.size(); i++) {
        ...
        if (es->eType == ET_BUILDABLE &&
            CG_PlayerIsBuilder((buildable_t) es->modelindex)) {
...
void CG_Buildable(centity_t* cent) {
    ...
    if (es->pos.trType == TR_STATIONARY) {
        ...
        if (VectorCompare(cent->buildableCache.cachedOrigin, cent->lerpOrigin) &&
            ...
            CG_PositionAndOrientateBuildable(angles, cent->lerpOrigin, surfNormal, es->number, mins, maxs, ent.axis, ent.origin);
    ...
    if (CG_PlayerIsBuilder((buildable_t) es->modelindex) &&
        CG_BuildableRemovalPending(es->number)) {
    ...
    if (cg_buildables[es->modelindex].md5) {
        ...
        float realScale =
                spawned ? scale : scale * (float) sin(0.5f * (cg.time - es->time) /
                                                      ba->buildTime * M_PI);
        ...
        if (es->modelindex == BA_H_MGTURRET || es->modelindex == BA_H_ROCKETPOD) {
            ...
            if (es->modelindex == BA_H_MGTURRET) {
                roll = Q_clamp((1.0f / MGTURRET_ATTACK_PERIOD) * 120.0f *
            ...
            // transform bounds so they more accurately reflect the turrets' new
            // transformation
            // TODO: Evaluate
...
#define OVERMIND_EYE_CLAMP \
    43.0f // 45° shows seams due to imperfections of the low poly version.
...
#define OVERMIND_EYE_Z_OFFSET 64.0f
        ...
        if (es->modelindex == BA_A_OVERMIND) {
            ...
            if (es->otherEntityNum == ENTITYNUM_NONE /*||
                ...
                // HACK: Mess with the seed once because the first output of Q_crandom
                // is so bad.
                Q_crandom(&randSeed);
                ...
                angles[YAW] =
                        Maths::clamp(angles[YAW], -OVERMIND_EYE_CLAMP, OVERMIND_EYE_CLAMP);
            }

            // Smooth out movement.
            ExponentialFade(&cent->overmindEyeAngle[PITCH], angles[PITCH], OVERMIND_EYE_LAMBDA, 0.001f * cg.frametime);
            ExponentialFade(&cent->overmindEyeAngle[YAW], angles[YAW], OVERMIND_EYE_LAMBDA, 0.001f * cg.frametime);
    ...
    if (es->eFlags & EF_FIRING) {
        ...
        if (wi->wim[WPM_PRIMARY].muzzleParticleSystem) {
            ...
            if (!CG_IsParticleSystemValid(&cent->muzzlePS)) {
                cent->muzzlePS = CG_SpawnNewParticleSystem(
                        wi->wim[WPM_PRIMARY].muzzleParticleSystem);
            }
                ...
                if (cg_buildables[es->modelindex].md5) {
                    switch (es->modelindex) {
                        case BA_H_ROCKETPOD: {
                            ...
                            num = (num % 3) + 1;
                            CG_SetAttachmentTag(&cent->muzzlePS->attachment, &ent, ent.hModel, va("Bone_fire_%s_%d", side, num));
                            break;
                        }
        ...
        if (cg.time - cent->muzzleFlashTime < MUZZLE_FLASH_TIME) {
            if (wi->wim[WPM_PRIMARY].flashDlight) {
                trap_R_AddLightToScene(cent->lerpOrigin,
                                       wi->wim[WPM_PRIMARY].flashDlight,
    ...
    } else // Not firing.
    {
        ...
        if (es->eFlags & EF_B_LOCKON) {
            trap_S_AddLoopingSound(es->number, cent->lerpOrigin, vec3_origin, cgs.media.rocketpodLockonSound);
        }
    }
    ...
    if (team == cg.predictedPlayerState.persistant[PERS_TEAM]) {
        ...
        if (team == TEAM_HUMANS) {
            drawRange =
                    BG_InventoryContainsWeapon(WP_HBUILD, cg.predictedPlayerState.stats);
        } else if (team == TEAM_ALIENS) {
