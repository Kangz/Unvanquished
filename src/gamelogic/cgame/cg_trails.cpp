...
static void CG_CalculateBeamNodeProperties(trailBeam_t* tb) {
    ...
    TCRange = tb->class_->backTextureCoord - tb->class_->frontTextureCoord;
    widthRange = tb->class_->backWidth - tb->class_->frontWidth;
    alphaRange = tb->class_->backAlpha - tb->class_->frontAlpha;
    VectorSubtract(tb->class_->backColor, tb->class_->frontColor, colorRange);
    ...
    for (j = 0, i = tb->nodes; i; i = i->next, j++) {
        if (tb->class_->textureType == TBTT_STRETCH) {
            ...
            if (tb->class_->clampToBack) {
                i->textureCoord = (totalDistance - position) / tb->class_->repeatLength;
            } else {
        ...
        i->halfWidth =
                (tb->class_->frontWidth + ((position / totalDistance) * widthRange)) /
                2.0f;
        i->alpha =
                (byte)((float) 0xFF * (tb->class_->frontAlpha +
                                       ((position / totalDistance) * alphaRange)) *
                       fadeAlpha);
        VectorMA(tb->class_->frontColor, (position / totalDistance), colorRange, i->color);
    ...
    while (1) {
        ...
        if (!Q_stricmp(token, "{")) {
            if (tsNameSet) {
                ...
                if (!CG_ParseTrailSystem(&baseTrailSystems[numBaseTrailSystems],
                                         &text_p,
                                         tsName)) {
    ...
    for (i = 0; i < MAX_BASETRAIL_SYSTEMS; i++) {
        ...
        if (!Q_stricmp(bts->name, name)) {
            ...
            for (j = 0; j < bts->numBeams; j++) {
                ...
                btb->shader = trap_R_RegisterShader(btb->shaderName, RSF_DEFAULT);
            }
