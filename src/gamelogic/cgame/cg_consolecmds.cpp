...
static void CG_CompleteBuy_internal(bool negatives) {
    ...
    trap_CompleteCallback(
            "grenade"); // called "gren" elsewhere, so special-case it
...
static void CG_CompleteGive() {
    unsigned i = 0;
    static const char give[][12] = {"all", "health", "funds", "stamina", "poison", "fuel", "ammo", "momentum", "bp"};
...
static void CG_CompleteTeamVote() {
    unsigned i = 0;
    static const char vote[][16] = {"kick", "spectate", "denybuild", "allowbuild", "admitdefeat", "poll"};
...
static void CG_CompleteVote() {
    ...
    static const char vote[][16] = {
            "kick", "spectate", "mute", "unmute", "sudden_death", "extend", "draw", "map_restart", "map", "layout", "nextmap", "poll"};
...
static void CG_TestCGrade_f() {
    qhandle_t shader = trap_R_RegisterShader(
            CG_Argv(1), (RegisterShaderFlags_t)(RSF_NOMIP | RSF_NOLIGHTSCALE));
...
static void CG_ToggleMenu_f() {
    trap_Rocket_DocumentAction(rocketInfo.menu[ROCKETMENU_INGAME_MENU].id,
                               "show");
}
...
static const struct {
    ...
    void (*function)();
    void (*completer)();
} commands[] = {{"+scores", CG_ShowScores_f, 0},
...
bool ConsoleCommand() {
    ...
    cmd = (consoleCommand_t*) bsearch(CG_Argv(0), commands, ARRAY_LEN(commands), sizeof(commands[0]), cmdcmp);
