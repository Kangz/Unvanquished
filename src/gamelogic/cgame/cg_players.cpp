...
static const char* const cg_customSoundNames[MAX_CUSTOM_SOUNDS] = {
        "*death1.wav", "*death2.wav", "*death3.wav", "*jump1.wav", "*pain25_1.wav", "*pain50_1.wav", "*pain75_1.wav", "*pain100_1.wav", "*falling1.wav", "*gasp.wav", "*drown.wav", "*fall1.wav", "*taunt.wav"};
...
static bool CG_RegisterPlayerAnimation(clientInfo_t* ci, const char* modelName, int anim, const char* animName, bool loop, bool reversed, bool clearOrigin, bool iqm) {
    ...
    if (iqm) {
        Com_sprintf(filename, sizeof(filename), "models/players/%s/%s.iqm:%s", newModelName, newModelName, animName);
...
static bool CG_DeriveAnimationDelta(const char* modelName, weapon_t weapon, clientInfo_t* ci, bool iqm) {
    ...
    if (iqm) {
        handle = trap_R_RegisterAnimation(va("models/players/%s/%s.iqm:%s_delta",
                                             newModelName,
                                             newModelName,
        ...
        handle =
                trap_R_RegisterAnimation(va("models/players/%s/%s_delta.md5anim",
                                            newModelName,
                                            BG_Weapon(weapon)->name));
    }
    ...
    for (i = 0; i < ci->numHandBones; i++) {
        VectorSubtract(delta.bones[ci->handBones[i]].t.trans,
                       base.bones[ci->handBones[i]].t.trans,
                       deltas[weapon][ci->handBones[i]].delta);

        QuatInverse(base.bones[ci->handBones[i]].t.rot);

        QuatMultiply1(base.bones[ci->handBones[i]].t.rot,
                      delta.bones[ci->handBones[i]].t.rot,
                      deltas[weapon][ci->handBones[i]].rot);
    }
...
static bool CG_RegisterClientModelname(clientInfo_t* ci, const char* modelName, const char* skinName) {
    ...
    if (ci->md5) {
        ...
        if (ci->gender != GENDER_NEUTER) {
            ...
            // FIXME fail register of the entire model if one of these animations is
            // missing
            ...
            for (i = TORSO_GESTURE_BLASTER, j = WP_BLASTER; i <= TORSO_GESTURE_CKIT;
                 i++, j++) {
                if (i == TORSO_GESTURE) {
                    continue;
                }
                if (i == TORSO_GESTURE_CKIT) {
                    j = WP_HBUILD;
                }
    ...
    for (i = 0; i < MAX_CUSTOM_SOUNDS; i++) {
        ...
        if (!CG_FileExists(va("sound/player/%s/%s", dir, s + 1))) {
            ...
            if (i == 11 || i == 8) // fall or falling
            {
                ...
                ci->sounds[i] =
                        trap_S_RegisterSound(va("sound/player/%s/%s", dir, s + 1), false);
            }
        } else {
            ci->sounds[i] =
                    trap_S_RegisterSound(va("sound/player/%s/%s", dir, s + 1), false);
    ...
    if (ci->footsteps == FOOTSTEP_CUSTOM) {
        for (i = 0; i < 4; i++) {
            ci->customFootsteps[i] = trap_S_RegisterSound(
                    va("sound/player/%s/step%d.wav", dir, i + 1), false);

            if (!ci->customFootsteps[i]) {
                ci->customFootsteps[i] = trap_S_RegisterSound(
                        va("sound/player/footsteps/step%d.wav", i + 1), false);
            }

            ci->customMetalFootsteps[i] = trap_S_RegisterSound(
                    va("sound/player/%s/clank%d.wav", dir, i + 1), false);

            if (!ci->customMetalFootsteps[i]) {
                ci->customMetalFootsteps[i] = trap_S_RegisterSound(
                        va("sound/player/footsteps/clank%d.wav", i + 1), false);
            }
        }
    }
...
static void CG_PlayerMD5AlienAnimation(centity_t* cent) {
    ...
    // If we are attacking/taunting, and in motion, allow blending the two
    // skeletons
...
===============
*/
static void CG_PlayerAngles(centity_t* cent, const vec3_t srcAngles, vec3_t legsAngles, vec3_t torsoAngles, vec3_t headAngles) {
    float dest;
    ...
    if (cent->currentState.eFlags & EF_DEAD) {
        ...
        // did use angles2.. now uses time2.. looks a bit funny but time2 isn't used
        // othwise
...
static void CG_PlayerAxis(centity_t* cent, const vec3_t srcAngles, vec3_t legs[3], vec3_t torso[3], vec3_t head[3]) {
...
static void CG_PlayerWWSmoothing(centity_t* cent, vec3_t in[3], vec3_t out[3]) {
    ...
    for (i = MAXSMOOTHS - 1; i >= 0; i--) {
        ...
        if (cg.time < cent->pe.sList[i].time + MODEL_WWSMOOTHTIME) {
            stLocal =
                    1.0f - (((cent->pe.sList[i].time + MODEL_WWSMOOTHTIME) - cg.time) /
                            MODEL_WWSMOOTHTIME);
...
static void CG_PlayerNonSegAxis(centity_t* cent, vec3_t srcAngles, vec3_t nonSegAxis[3]) {
    ...
    // torso
    if (cent->currentState.eFlags & EF_DEAD) {
        CG_SwingAngles(localAngles[YAW], 0, 0, cg_swingSpeed.value, &cent->pe.nonseg.yawAngle, &cent->pe.nonseg.yawing);
    } else {
        CG_SwingAngles(localAngles[YAW], 40, 90, cg_swingSpeed.value, &cent->pe.nonseg.yawAngle, &cent->pe.nonseg.yawing);
    }
...
#define radar jetpack
    ...
    if (held & (1 << UP_JETPACK)) {
        ...
        if (publicFlags & PF_JETPACK_ACTIVE) {
            ...
            if (cent->jetPackState != JPS_ACTIVE) {
                ...
                cent->jetPackPS[0] =
                        CG_SpawnNewParticleSystem(cgs.media.jetPackThrustPS);
                cent->jetPackPS[1] =
                        CG_SpawnNewParticleSystem(cgs.media.jetPackThrustPS);
            ...
            // play thrust sound
            trap_S_AddLoopingSound(cent->currentState.number, cent->lerpOrigin, vec3_origin, cgs.media.jetpackThrustLoopSound);
    ...
    if (es->eFlags & EF_BLOBLOCKED) {
        ...
        if (size > 0.0f) {
            CG_ImpactMark(cgs.media.creepShader, origin, up, 0.0f, 1.0f, 1.0f, 1.0f, 1.0f, false, size, true);
        }
    }
...
static bool CG_PlayerShadow(centity_t* cent, class_t class_) {
    ...
    if (cg_shadows.integer > SHADOWING_BLOB && cg_playerShadows.integer) {
        // add inverse shadow map
        { CG_StartShadowCaster(cent->lerpOrigin, mins, maxs); }
    }

    if (cg_shadows.integer !=
        SHADOWING_BLOB) // no mark for stencil or projection shadows
    {
        return true;
    }
    ...
    CG_ImpactMark(cgs.media.shadowMarkShader, trace.endpos, trace.plane.normal, cent->pe.legs.yawAngle, 0.0f, 0.0f, 0.0f, alpha, false, 24.0f * BG_ClassModelConfig(class_)->shadowScale, true);
...
static void CG_PlayerSplash(centity_t* cent, class_t class_) {
    ...
    trap_CM_BoxTrace(&trace, start, end, nullptr, nullptr, 0, (CONTENTS_WATER | CONTENTS_SLIME | CONTENTS_LAVA), 0);

    if (trace.fraction == 1.0f) {
        return;
    }

    CG_ImpactMark(cgs.media.wakeMarkShader, trace.endpos, trace.plane.normal, cent->pe.legs.yawAngle, 1.0f, 1.0f, 1.0f, 1.0f, false, 32.0f * BG_ClassModelConfig(class_)->shadowScale, true);
}
...
void CG_Player(centity_t* cent) {
    ...
    if (ci->md5) {
        ...
        if (es->eFlags & EF_WALLCLIMB &&
            BG_RotateAxis(es->angles2, body.axis, tempAxis, false, es->eFlags & EF_WALLCLIMBCEILING)) {
        ...
        if (es->eFlags & EF_WALLCLIMB && !(es->eFlags & EF_DEAD) &&
            ...
            VectorCopy(body.origin,
                       body.oldorigin); // don't positionally lerp at all
        } else {
            VectorCopy(cent->lerpOrigin, playerOrigin);
        ...
        if (ci->gender != GENDER_NEUTER) {
            ...
            if (torsoSkeleton.numBones != legsSkeleton.numBones) {
                ...
                CG_Printf(
                        "[skipnotify]WARNING: cent->pe.legs.skeleton.numBones != "
                        "cent->pe.torso.skeleton.numBones\n");
            }
            ...
            if (ci->weaponAdjusted & (1 << es->weapon) &&
                (cent->currentState.torsoAnim & ~ANIM_TOGGLEBIT) >= TORSO_ATTACK) {
                int j;
            ...
            if ((cent->currentState.torsoAnim & ~ANIM_TOGGLEBIT) >=
                        TORSO_GESTURE_BLASTER &&
                (cent->currentState.torsoAnim & ~ANIM_TOGGLEBIT) <=
                        TORSO_GESTURE_CKIT) {
                quat_t rot;
            ...
            // Relationships are emphirically derived. They will probably need to be
            // changed upon changes to the human model
            QuatFromAngles(
                    rotation, cent->lerpAngles[0], cent->lerpAngles[0] < 0 ? -cent->lerpAngles[0] / 9 : -cent->lerpAngles[0] / (8 - (5 * (cent->lerpAngles[0] / 90))), 0);
...
void CG_Corpse(centity_t* cent) {
    ...
    // if the model failed, allow the default nullmodel to be displayed. Also, if
    // MD5, no need to add other parts
