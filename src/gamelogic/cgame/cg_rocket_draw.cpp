...
static void CG_Rocket_DrawCrosshairIndicator() {
    ...
    if ((!cg_drawCrosshairHit.integer && !cg_drawCrosshairFriendFoe.integer) ||
        ...
        cg.snap->ps.pm_type == PM_INTERMISSION || cg.renderingThirdPerson) {
        return;
    }
    ...
    if (cg_drawCrosshairFriendFoe.integer >= CROSSHAIR_ALWAYSON ||
        (cg_drawCrosshairFriendFoe.integer >= CROSSHAIR_RANGEDONLY &&
    ...
    // add hit color
    if (cg_drawCrosshairHit.integer &&
        cg.hitTime + CROSSHAIR_INDICATOR_HITFADE > cg.time) {
        dim = ((cg.hitTime + CROSSHAIR_INDICATOR_HITFADE) - cg.time) /
              (float) CROSSHAIR_INDICATOR_HITFADE;
...
void CG_AddSpeed() {
    ...
    if ((newSpeedGteMaxSpeedInWindow =
                 (speed >= speedSamples[maxSpeedSampleInWindow]))) {
    ...
    if (!newSpeedGteMaxSpeedInWindow &&
        ...
        do {
            maxSpeedSampleInWindow =
                    (maxSpeedSampleInWindow + 1) % SPEEDOMETER_NUM_SAMPLES;
...
static void CG_Rocket_DrawSpeedGraph() {
    ...
    if (cg_drawSpeed.integer & SPEEDOMETER_DRAW_GRAPH) {
        ...
        for (i = 1; i < SPEEDOMETER_NUM_DISPLAYED_SAMPLES; i++) {
            val = speedSamples[(oldestSpeedSample + i + SPEEDOMETER_NUM_SAMPLES -
                                SPEEDOMETER_NUM_DISPLAYED_SAMPLES) %
                               SPEEDOMETER_NUM_SAMPLES];
            ...
            if (val < SPEED_MED) {
                ...
                VectorLerpTrem((val - SPEED_MED) / (SPEED_FAST - SPEED_MED), medium, fast, color);
            }
            ...
            CG_DrawPic(
                    rect.x + (i / (float) SPEEDOMETER_NUM_DISPLAYED_SAMPLES) * rect.w, top, rect.w / (float) SPEEDOMETER_NUM_DISPLAYED_SAMPLES, val * rect.h / max, cgs.media.whiteShader);
        }
    ...
    if (cg_drawSpeed.integer & SPEEDOMETER_DRAW_TEXT) {
        ...
        if (cg.predictedPlayerState.clientNum == cg.clientNum) {
            ...
            val = speedSamples[(oldestSpeedSample - 1 + SPEEDOMETER_NUM_SAMPLES) %
                               SPEEDOMETER_NUM_SAMPLES];
        }

        trap_Rocket_SetInnerRML(
                va("<span class='speed_max'>%d</span><span "
                   "class='speed_current'>%d</span>",
                   (int) speedSamples[maxSpeedSampleInWindow],
                   (int) val),
                0);
    }
}
...
static void CG_Rocket_DrawWeaponIcon() {
    ...
    if (!cg_weapons[weapon].registered) {
        Com_Printf(S_WARNING
                   "CG_DrawWeaponIcon: weapon %d (%s) "
                   "is not registered\n",
                   weapon,
    ...
    trap_Rocket_SetInnerRML(
            va("<img class='weapon_icon%s%s' src='/%s' />", rmlClass ? " " : "", rmlClass, CG_GetShaderNameFromHandle(cg_weapons[weapon].weaponIcon)),
            0);
}
...
static void CG_Rocket_DrawUsableBuildable() {
    ...
    CG_Trace(&trace, cg.refdef.vieworg, nullptr, nullptr, point, cg.predictedPlayerState.clientNum, MASK_SHOT, 0);
    ...
    if (es->eType == ET_BUILDABLE && BG_Buildable(es->modelindex)->usable &&
        ...
        // hack to prevent showing the usable buildable when you aren't carrying an
        // energy weapon
        ...
        trap_Rocket_SetInnerRML(va("<img class='usable_buildable' src='%s' />",
                                   CG_Rocket_GetAttribute("src")),
                                0);
...
static void CG_Rocket_DrawDisconnect() {
    ...
    CG_DrawPic(x, y, 48, 48, trap_R_RegisterShader("gfx/2d/net", RSF_DEFAULT));
}
...
static void CG_ScanForCrosshairEntity() {
    ...
    CG_Trace(&trace, start, vec3_origin, vec3_origin, end, cg.snap->ps.clientNum, CONTENTS_SOLID | CONTENTS_BODY, 0);
...
static void CG_Rocket_DrawCrosshairNames() {
    ...
    if (cg_drawEntityInfo.integer) {
        name = va("(" S_COLOR_CYAN "%s" S_COLOR_WHITE "|" S_COLOR_CYAN
                  "#%d" S_COLOR_WHITE ")",
                  Com_EntityTypeName(
                          cg_entities[cg.crosshairClientNum].currentState.eType),
    ...
    if (cg_teamOverlayUserinfo.integer &&
        cg.snap->ps.persistant[PERS_TEAM] != TEAM_NONE && cgs.teamInfoReceived &&
        cgs.clientinfo[cg.crosshairClientNum].health > 0) {
        name = va("%s ^7[^%c%d^7]", name, CG_GetColorCharForHealth(cg.crosshairClientNum), cgs.clientinfo[cg.crosshairClientNum].health);
    }
...
static void CG_Rocket_DrawLevelshot() {
    if ((rocketInfo.data.mapIndex < 0 ||
         rocketInfo.data.mapIndex >= rocketInfo.data.mapCount)) {
        return;
    }

    trap_Rocket_SetInnerRML(
            va("<img class='levelshot' src='/meta/%s/%s' />",
               rocketInfo.data.mapList[rocketInfo.data.mapIndex].mapLoadName,
               rocketInfo.data.mapList[rocketInfo.data.mapIndex].mapLoadName),
            0);
}
...
void CG_Rocket_DrawCenterPrint() {
    ...
    trap_Rocket_SetProperty(
            "opacity",
            va("%f", CG_FadeAlpha(cg.centerPrintTime, CENTER_PRINT_DURATION)));
}
...
static void CG_DrawStack(rectDef_t* rect, vec4_t color, float fill, int align, float val, int max) {
    int i;
    float each, frac;
    float nudge;
    ...
    if (each * (vertical ? cgs.screenYScale : cgs.screenXScale) < 4.f) {
        ...
        if (vertical) {
            CG_DrawPic(rect->x, rect->y + loff, rect->w, rect->h * val / fmax, cgs.media.whiteShader);
        }

        else {
            CG_DrawPic(rect->x + loff, rect->y, rect->w * val / fmax, rect->h, cgs.media.whiteShader);
        }
    ...
    for (i = (int) val - 1; i >= 0; i--) {
        ...
        if (vertical) {
            CG_DrawPic(rect->x, rect->y + rect->h * start, rect->w, each, cgs.media.whiteShader);
        }

        else {
            CG_DrawPic(rect->x + rect->w * start, rect->y, each, rect->h, cgs.media.whiteShader);
        }
    }
    ...
    switch (align) {
        case LALIGN_TOPLEFT:
            if (vertical) {
                CG_DrawPic(rect->x, rect->y - rect->h * (1 - frac) / fmax, rect->w, each, cgs.media.whiteShader);
            }

            else {
                CG_DrawPic(rect->x - rect->w * (1 - frac) / fmax, rect->y, each, rect->h, cgs.media.whiteShader);
            }
        ...
        case LALIGN_BOTTOMRIGHT:
            if (vertical) {
                CG_DrawPic(rect->x,
                           rect->y + rect->h * (1 + ((1 - fill) / fmax) - frac / fmax),
                           rect->w,
                           each,
                ...
                CG_DrawPic(rect->x + rect->w * (1 + ((1 - fill) / fmax) - frac / fmax),
                           rect->y,
                           each,
                           rect->h,
                           cgs.media.whiteShader);
...
void CG_Rocket_DrawConnectText() {
    ...
    if (rocketInfo.cstate.connState < CA_CONNECTED &&
        *rocketInfo.cstate.messageString) {
...
void CG_Rocket_DrawChatType() {
    ...
    if ((size_t) cg.sayType < ARRAY_LEN(sayText)) {
        ...
        if (ui_chatPromptColors.integer) {
            trap_Rocket_SetInnerRML(va("%s%s", sayText[cg.sayType].colour, prompt),
                                    RP_QUAKE);
        } else {
...
void CG_Rocket_DrawMineRate() {
    ...
    trap_Rocket_SetInnerRML(
            va(_("%.1f BP/min (%d%% × %.1f)"), rate, efficiency, levelRate), 0);
}
...
static void CG_Rocket_DrawVote_internal(team_t team) {
    ...
    if (cg_tutorial.integer) {
        Com_sprintf(
                yeskey, sizeof(yeskey), "[%s]", CG_KeyBinding(va("%svote yes", team == TEAM_NONE ? "" : "team"), team));
        Com_sprintf(
                nokey, sizeof(nokey), "[%s]", CG_KeyBinding(va("%svote no", team == TEAM_NONE ? "" : "team"), team));
    }
...
static void CG_Rocket_DrawTeamVote() {
    CG_Rocket_DrawVote_internal(
            (team_t) cg.predictedPlayerState.persistant[PERS_TEAM]);
}
...
static void CG_Rocket_DrawNumSpawns() {
    ...
    if (position < 1) {
        ...
        s = P_("There is 1 spawn remaining",
               va("There are %d spawns remaining", spawns),
               spawns);
    }
...
static void CG_Rocket_HaveJetpck() {
    bool jetpackInInventory =
            BG_InventoryContainsUpgrade(UP_JETPACK, cg.snap->ps.stats);
...
void CG_Rocket_RenderElement() {
    ...
    cmd = (elementRenderCmd_t*) bsearch(
            tag, elementRenderCmdList, elementRenderCmdListCount, sizeof(elementRenderCmd_t), elementRenderCmdCmp);
...
void CG_Rocket_RegisterElements() {
    ...
    for (i = 0; i < elementRenderCmdListCount; i++) {
        // Check that the commands are in increasing order so that it can be used by
        // bsearch
        if (i != 0 &&
            Q_stricmp(elementRenderCmdList[i - 1].name,
                      elementRenderCmdList[i].name) > 0) {
            CG_Printf(
                    "CGame elementRenderCmdList is in the wrong order for %s and %s\n",
                    elementRenderCmdList[i - 1].name,
                    elementRenderCmdList[i].name);
        }
