...
void CG_SetInitialSnapshot(snapshot_t* snap) {
    ...
    BG_PlayerStateToEntityState(
            &snap->ps, &cg_entities[snap->ps.clientNum].currentState, false);
...
static void CG_TransitionSnapshot() {
    ...
    // Need to store the previous weapon because BG_PlayerStateToEntityState might
    // change it
    // so the CG_OnPlayerWeaponChange callback is never called
    oldWeapon = oldFrame->ps.weapon;

    BG_PlayerStateToEntityState(
            &cg.snap->ps, &cg_entities[cg.snap->ps.clientNum].currentState, false);
...
static void CG_SetNextSnap(snapshot_t* snap) {
    ...
    BG_PlayerStateToEntityState(
            &snap->ps, &cg_entities[snap->ps.clientNum].nextState, false);
