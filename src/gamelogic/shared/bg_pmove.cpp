...
static void PM_StartWeaponAnim(int anim) {
    ...
    pm->ps->weaponAnim =
            ((pm->ps->weaponAnim & ANIM_TOGGLEBIT) ^ ANIM_TOGGLEBIT) | anim;
}
...
static float PM_CmdScale(usercmd_t* cmd, bool zFlight) {
    ...
    if (pm->ps->persistant[PERS_TEAM] == TEAM_HUMANS &&
        ...
        // the stamina too.
        // TODO: Try to move code upwards so sprinting isn't activated in the first
        // place.
        ...
        // TODO: Move modifer into upgrade/class config files of armour
        // items/classes
    ...
    if (pm->ps->weapon == WP_ALEVEL4 && pm->ps->pm_flags & PMF_CHARGE) {
        modifier *=
                1.0f + (pm->ps->stats[STAT_MISC] * (LEVEL4_TRAMPLE_SPEED - 1.0f) /
...
static bool PM_CheckPounce() {
    ...
    switch (pm->ps->weapon) {
        ...
        case WP_ALEVEL3_UPG:
            ...
            // Pounce if minimum charge time has passed and the charge button isn't
            // held anymore
    ...
    switch (pm->ps->weapon) {
        case WP_ALEVEL1:
            // wallwalking (ground surface normal is off more than 45° from Z
            // direction)
            if (pm->ps->groundEntityNum == ENTITYNUM_WORLD &&
                acos(pml.groundTrace.plane.normal[2]) > M_PI_4) {
                // get jump magnitude
                jumpMagnitude = LEVEL1_WALLPOUNCE_MAGNITUDE;

                // if looking in the direction of the surface, jump in opposite normal
                // direction
                if (DotProduct(pml.groundTrace.plane.normal, pml.forward) < 0.0f) {
                    VectorCopy(pml.groundTrace.plane.normal, jumpDirection);
                }
                // otherwise try to find a trajectory to the surface point the player is
                // looking at
                else {
                    trace_t trace;
                    ...
                    pm->trace(&trace, pm->ps->origin, pm->mins, pm->maxs, traceTarget, pm->ps->clientNum, MASK_SOLID, 0);

                    foundTrajectory = (trace.fraction < 1.0f);

                    // HACK: make sure we don't just tangent if we want to attach to the
                    // ceiling
                    //       this is done by moving the target point inside a ceiling's
                    //       surface
                    if (foundTrajectory) {
                        ...
                        if (trace.plane.normal[2] < 0.0f) {
                            ...
                            if (pm->debugLevel > 0) {
                                Com_Printf(
                                        "[PM_CheckPounce] Aiming at ceiling; move target into "
                                        "surface by %.2f\n",
                                        zCorrection);
                            }
                        ...
                        if (pm->debugLevel > 0) {
                            Com_Printf(
                                    "[PM_CheckPounce] Trajectory target has a distance of %.1f "
                                    "qu\n",
                                    Distance(pm->ps->origin, endpos));
                        }
                    }

                    // if there is a possible trajectoy they come in pairs, use the
                    // shorter one
                    if (foundTrajectory &&
                        BG_GetTrajectoryPitch(pm->ps->origin, endpos, jumpMagnitude, pm->ps->gravity, trajAngles, trajDir1, trajDir2)) {
                        int iter;

                        if (pm->debugLevel > 0) {
                            Com_Printf(
                                    "[PM_CheckPounce] Found trajectory angles: "
                        ...
                        // HACK: make sure we get off the ceiling if jumping to an adjacent
                        // wall
                        //       this is done by subsequently rotating the jump direction in
                        //       surface
                        //       normal direction until its angle is below a threshold
                        //       (acos(0.1) ~= 85°)
                        for (iter = 0; DotProduct(jumpDirection,
                                                  pml.groundTrace.plane.normal) <= 0.1f;
                             iter++) {
                            if (iter > 10) {
                                if (pm->debugLevel > 0) {
                                    Com_Printf(
                                            "[PM_CheckPounce] Giving up adjusting jump direction.\n");
                                }
                            ...
                            if (pm->debugLevel > 0) {
                                Com_Printf(
                                        "[PM_CheckPounce] Adjusting jump direction to get off the "
                                        "surface: "
                                        "( %.2f, %.2f, %.2f )\n",
                                        jumpDirection[0],
                                        jumpDirection[1],
            ...
            // moving foward or standing still
                ...
                pitchToGround = M_PI_2 - acos(DotProduct(pml.groundTrace.plane.normal,
                                                         jumpDirection));

                // get pitch towards XY reference plane
                pitchToRef = M_PI_2 - acos(DotProduct(up, jumpDirection));

                // use the advantageous pitch; allows using an upwards gradiant as a
                // ramp
                pitch = MIN(pitchToGround, pitchToRef);

                // pitches above 45° or below LEVEL1_POUNCE_MINPITCH will result in less
                // than the maximum jump length
                if (pitch > M_PI_4) {
                    pitch = M_PI_4;
                ...
                jumpMagnitude = (int) sqrt(LEVEL1_POUNCE_DISTANCE * pm->ps->gravity /
                                           sin(2.0f * pitch));
...
static bool PM_CheckWallJump() {
    ...
    // for a long run of wall jumps the velocity can get pretty large, this caps
    // it
...
static bool PM_CheckJetpack() {
    ...
    if (pm->cmd.upmove < 10) {
        if (pm->ps->stats[STAT_STATE2] & SS2_JETPACK_ACTIVE) {
            if (pm->debugLevel > 0 && pm->cmd.upmove < 10) {
                Com_Printf("[PM_CheckJetpack] " S_COLOR_LTORANGE
                           "Jetpack thrust stopped (jump key released)\n");
            }
    ...
    if (!(pm->ps->stats[STAT_STATE2] & SS2_JETPACK_ENABLED)) {
        if (pm->debugLevel > 0) {
            Com_Printf("[PM_CheckJetpack] " S_COLOR_RED
                       "Can't start jetpack thrust (jetpack not enabled)\n");
        }
    ...
    if (!(pm->ps->stats[STAT_STATE2] & SS2_JETPACK_WARM)) {
        ...
        // we got off ground by jumping and are not yet in free fall, where free
        // fall is defined as
        // (1) fall speed bigger than sideways speed (not strafe jumping)
        // (2) fall speed bigger than jump magnitude (not jumping up and down on
        // solid ground)
        if ((pm->ps->pm_flags & PMF_JUMPED) &&
            !(-pm->ps->velocity[2] > sideVelocity &&
              -pm->ps->velocity[2] >
    ...
    if (pm->ps->stats[STAT_FUEL] < JETPACK_FUEL_USAGE) {
        if (pm->ps->stats[STAT_STATE2] & SS2_JETPACK_ACTIVE) {
            if (pm->debugLevel > 0) {
                Com_Printf("[PM_CheckJetpack] " S_COLOR_LTORANGE
                           "Jetpack thrust stopped (out of fuel)\n");
            }
...
/**
 * @brief Disables the jetpack. Without force, the call can get ignored based on
 * previous velocity.
 */
static void PM_LandJetpack(bool force) {
    ...
    if (pm->ps->stats[STAT_STATE2] & SS2_JETPACK_ACTIVE) {
        if (pm->debugLevel > 0) {
            Com_Printf("[PM_LandJetpack] " S_COLOR_LTORANGE
                       "Jetpack thrust stopped (hit surface at %.0f°)%s\n",
                       RAD2DEG(angle),
    ...
    if (pm->ps->stats[STAT_STATE2] & SS2_JETPACK_ENABLED) {
        if (pm->debugLevel > 0) {
            Com_Printf("[PM_LandJetpack] " S_COLOR_YELLOW
                       "Jetpack disabled (hit surface at %.0f°)%s\n",
                       RAD2DEG(angle),
                       force ? S_COLOR_RED " (FORCED)" : "");
        }
...
static bool PM_CheckJump() {
    ...
    if (jetpackJump || pm->ps->stats[STAT_STATE2] & SS2_JETPACK_ACTIVE) {
        if (pm->debugLevel > 0) {
            Com_Printf(
                    "[PM_CheckJump] Using jetpack: Decreasing jump magnitude to %.0f%%\n",
...
static void PM_WalkMove() {
    ...
    // if PM_Land didn't stop the jetpack (e.g. to allow for a jump) but we didn't
    // get away
...
static void PM_CheckLadder() {
    ...
    pm->trace(&trace, pm->ps->origin, pm->mins, pm->maxs, end, pm->ps->clientNum, MASK_PLAYERSOLID, 0);
...
static void PM_Land() {
    PM_LandJetpack(
            false); // don't force a stop, sometimes we can push off with a jump
...
static void PM_GroundTraceMissed() {
    ...
    if (pm->ps->groundEntityNum != ENTITYNUM_NONE) {
        ...
        // force into it
        // if we didn't do the trace, the player would be backflipping down
        // staircases
        VectorCopy(pm->ps->origin, point);
        point[2] -= 64.0f;
...
static void PM_GroundClimbTrace() {
    ...
    static const vec3_t refNormal = {
            0.0f, 0.0f, 1.0f}; // really the floor's normal. refactor?
    static const vec3_t ceilingNormal = {0.0f, 0.0f, -1.0f};
    static const vec3_t horizontal = {
            1.0f, 0.0f, 0.0f}; // arbitrary vector orthogonal to refNormal
    ...
    for (i = 0; i <= NUM_GCT_ATP; i++) {
        switch (i) {
            ...
            case GCT_ATP_GROUND:
                ...
                // mask out CONTENTS_BODY to not hit other players and avoid the camera
                // flipping out
            ...
            case GCT_ATP_CEILING:
                // attach to the ceiling if we get close enough during a jump that goes
                // roughly upwards
        ...
        if (trace.fraction < 1.0f &&
            ...
            if (i == GCT_ATP_STEPMOVE || i == GCT_ATP_UNDERNEATH ||
                i == GCT_ATP_CEILING) {
            ...
            // if the trace result and old surface normal are different then we must
            // have transided to a new surface
            if (!VectorCompareEpsilon(trace.plane.normal, surfNormal, eps)) {
                // if the trace result or the old vector is not the floor or ceiling
                ...
                if (!VectorCompareEpsilon(trace.plane.normal, refNormal, eps) &&
                    ...
                    // calculate reference rotated through to surf plane then to trace
                    // plane
                ...
                // check whether point is on one side of the plane, if so invert the
                // correction angle
                ...
                if (VectorCompareEpsilon(trace.plane.normal, ceilingNormal, eps)) {
                    if (fabs(DotProduct(surfNormal, trace.plane.normal)) < (1.0f - eps)) {
                        // we had a smooth transition, rotate along old surface x new
                        // surface
                ...
                // we need to do some different angle correction here because
                // grapplePoint is a rotation axis
                if (VectorCompareEpsilon(surfNormal, ceilingNormal, eps)) {
                    ...
                    pm->ps->delta_angles[1] -=
                            ANGLE2SHORT(((surfAngles[1] - toAngles[1]) * 2) - 180.0f);
                }
            }
...
static void PM_CheckDuck() {
    ...
    if (pm->cmd.upmove < 0 && !VectorCompare(PCmaxs, PCcmaxs)) {
        ...
        if (pm->ps->pm_flags & PMF_DUCKED) {
            // try to stand up
            pm->maxs[2] = PCmaxs[2];
            pm->trace(&trace, pm->ps->origin, pm->mins, pm->maxs, pm->ps->origin, pm->ps->clientNum, pm->tracemask, 0);
...
static void PM_Weapon() {
    ...
    if (pm->ps->weapon == WP_LUCIFER_CANNON) {
        ...
        if (!pm->ps->weaponTime && pm->ps->weaponstate != WEAPON_NEEDS_RESET &&
            ...
            if (pm->ps->stats[STAT_MISC] >
                pm->ps->ammo * LCANNON_CHARGE_TIME_MAX / LCANNON_CHARGE_AMMO) {
                pm->ps->stats[STAT_MISC] =
                        pm->ps->ammo * LCANNON_CHARGE_TIME_MAX / LCANNON_CHARGE_AMMO;
            }
        }
        ...
        if (usercmdButtonPressed(pm->cmd.buttons, BUTTON_USE_HOLDABLE)) {
            if (!(pm->ps->pm_flags & PMF_USE_ITEM_HELD)) {
                if (pm->cmd.weapon < 32) {
                    ...
                    if (BG_InventoryContainsUpgrade(pm->cmd.weapon - 32,
    ...
    if (!pm->ps->ammo && !pm->ps->clips &&
        ...
        if (attack1 || (BG_Weapon(pm->ps->weapon)->hasAltMode && attack2) ||
    ...
    if (pm->ps->weapon == WP_CHAINGUN) {
        if (pm->ps->pm_flags & PMF_DUCKED ||
            ...
            pm->ps->delta_angles[PITCH] -=
                    ANGLE2SHORT(((random() * 8) - 2) * (30.0 / (float) addTime));
            pm->ps->delta_angles[YAW] -=
                    ANGLE2SHORT(((random() * 8) - 4) * (30.0 / (float) addTime));
        }
    }
...
void PM_UpdateViewAngles(playerState_t* ps, const usercmd_t* cmd) {
    ...
    if (!(ps->stats[STAT_STATE] & SS_WALLCLIMBING) ||
        !BG_RotateAxis(ps->grapplePoint, axis, rotaxis, false, ps->eFlags & EF_WALLCLIMBCEILING)) {
...
static void PM_HumanStaminaEffects() {
    ...
    } else if ((stats[STAT_STATE] & SS_SPEEDBOOST) && !walking &&
               !crouching) // walk/crouch overrides sprint
    {
