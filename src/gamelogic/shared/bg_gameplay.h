...
#define ABUILDER_BLOB_FIRE_IMMUNITY \
    3000 // in ms, friendly buildables gain immunity for fire on hit
#define ABUILDER_BLOB_FIRE_STOP_RANGE \
    20 // granger spit that hits a surface kills environmental fire in this range
...
#define LEVEL1_POUNCE_DISTANCE \
    300 // pitch between LEVEL1_POUNCE_MINPITCH and pi/4 results in this distance
#define LEVEL1_POUNCE_MINPITCH \
    M_PI / 12.0f // 15°, minimum pitch that will result in full pounce distance
...
#define LEVEL1_SIDEPOUNCE_DIR_Z \
    0.4f // in ]0.0f,1.0f], fixed Z-coordinate of sidepounce
...
// TODO: Increase burn damage frequencies, make sure burn functions are called
// frequently
// Average burn damage of an independent buildable is given by the limit of a
// geometric series:
// ( BURN_SELFDAMAGE / BURN_SELFDAMAGE_PERIOD ) * ( BURN_STOP_PERIOD /
// BURN_STOP_CHANCE )
...
// A burn action will first evaluate the stop chance, then a spread chance for
// each close neighbor
#define BURN_ACTION_PERIOD 2500
// The stop chance is lowered by a factor for every burning entity nearby
// The factors decrease lineraly from BURN_STOP_CHANCE to 0.0 over the given
// radius
...
#define SPIKER_SENSE_RANGE \
    200.0f // an enemy needs to be this close to consider an attack
#define SPIKER_MISSILEROWS 4
#define SPIKER_MISSILES 26 // actual value +/- SPIKER_MISSILEROWS
#define SPIKER_ROWOFFSET \
    0.5f // 0.0: Spikes are shot upwards, 1.0: Spikes are shot sideways
...
#define ALIENSENSE_BORDER_FRAC \
    0.2f // In this outer fraction of the range beacons are faded.
...
#define ALIEN_POISON_DIVIDER \
    (1.0f / 1.32f) // about 1.0/((time)th root of damage)
...
#define ALIEN_REGEN_NOCREEP_MIN \
    0.5f // minimum off creep regen when half life is active. must be > 0.

#define ALIEN_MAX_CREDITS \
    2000 // CREDITS_PER_EVO converts this to evos for display
...
#define FLAMER_DAMAGE_MAXDST_MOD \
    0.5f // damage decreases linearly from full damage to this during missile
         // lifetime
#define FLAMER_SPLASH_MINDST_MOD \
    0.5f // splash damage increases linearly from this to full damage during
         // lifetime
...
#define PRIFLE_DAMAGE_HALF_LIFE \
    0 // in ms, damage half life time after full damage period, 0 = off
...
#define LCANNON_DAMAGE_HALF_LIFE \
    0 // in ms, damage half life time after full damage period, 0 = off
...
#define TURRET_THINK_PERIOD \
    25                           // doesn't affect damage or turn speed directly, just their precision
...
#define TURRET_GIVEUP_TARGET \
    1000 // in ms, time until turret stops tracking a target after losing los

#define MGTURRET_ATTACK_PERIOD 125
#define MGTURRET_RANGE 350
#define MGTURRET_SPREAD 200
#define MGTURRET_ZONES \
    3 // range is divided into this amount of zones (disks) with equal width
#define MGTURRET_ZONE_DAMAGE \
    { 4, 3, 2 } // damage for each of the TURRET_ZONES zones
...
#define POWER_DISPLAY_MAX \
    40 // power display on human buildables is capped above this
...
#define HUMAN_AMMO_REFILL_PERIOD \
    2000 // don't refill ammo more frequently than this
...
#define JETPACK_FUEL_PER_DMG \
    300                        // per damage point received (before armor mod is applied)
#define JETPACK_FUEL_RESTORE 3 // in 1/ms
#define JETPACK_FUEL_IGNITE \
    JETPACK_FUEL_MAX / 20 // used when igniting the engine
#define JETPACK_FUEL_LOW \
    JETPACK_FUEL_MAX / 5 // jetpack doesn't start from a jump below this
#define JETPACK_FUEL_STOP \
    JETPACK_FUEL_RESTORE * 150 // jetpack doesn't activate below this
...
// fall distance
#define MIN_FALL_DISTANCE \
    30.0f // the fall distance at which fall damage kicks in
#define MAX_FALL_DISTANCE \
    120.0f // the fall distance at which maximum damage is dealt
#define AVG_FALL_DISTANCE ((MIN_FALL_DISTANCE + MAX_FALL_DISTANCE) / 2.0f)

// impact and weight damage
#define IMPACTDMG_JOULE_TO_DAMAGE 0.002f // in 1/J
#define WEIGHTDMG_DMG_MODIFIER \
    0.25f // multiply with weight difference to get DPS
#define WEIGHTDMG_DPS_THRESHOLD \
    10                       // ignore weight damage per second below this
...
// score
#define SCORE_PER_CREDIT \
    0.02f // used to convert credit rewards to score points
#define SCORE_PER_MOMENTUM \
    1.0f                          // used to convert momentum rewards to score points
...
#define PLAYER_PRICE_TO_VALUE \
    0.5f                              // fraction of upgrade price added to player value
...
#define MOMENTUM_PER_CREDIT \
    0.01f                                    // used to award momentum based on credit rewards
...
#define MAIN_STRUCTURE_MOMENTUM_VALUE \
    20 // momentum reward for destroying OM/RC
