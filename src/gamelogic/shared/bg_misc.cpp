...
void trap_FS_Seek(fileHandle_t f, long offset,
                  fsOrigin_t origin); // fsOrigin_t
...
void BG_InitBuildableAttributes() {
    ...
    for (i = 0; i < bg_numBuildables; i++) {
        ...
        BG_ParseBuildableAttributeFile(
                va("configs/buildables/%s.attr.cfg", ba->name), ba);
    }
}
...
void BG_BuildableBoundingBox(int buildable, vec3_t mins, vec3_t maxs) {
    buildableModelConfig_t* buildableModelConfig =
            BG_BuildableModelConfig(buildable);
...
void BG_InitBuildableModelConfigs() {
    ...
    for (i = BA_NONE + 1; i < BA_NUM_BUILDABLES; i++) {
        ...
        BG_ParseBuildableModelFile(
                va("configs/buildables/%s.model.cfg", BG_Buildable(i)->name), bc);
    }
}
...
==============
*/
void BG_ClassBoundingBox(int pClass, vec3_t mins, vec3_t maxs, vec3_t cmaxs, vec3_t dmins, vec3_t dmaxs) {
...
int BG_ClassCanEvolveFromTo(int from, int to, int credits) {
    ...
    if (from == to || from <= PCL_NONE || from >= PCL_NUM_CLASSES ||
...
void BG_InitClassModelConfigs() {
    ...
    for (i = PCL_NONE; i < PCL_NUM_CLASSES; i++) {
        ...
        BG_ParseClassModelFile(
                va("configs/classes/%s.model.cfg", BG_Class(i)->name), cc);

        cc->segmented = cc->modelName[0]
                                ? BG_NonSegModel(va("models/players/%s/animation.cfg",
                                                    cc->modelName))
                                : false;
    }
}
...
const weaponAttributes_t* BG_Weapon(int weapon) {
    return (weapon > WP_NONE && weapon < WP_NUM_WEAPONS) ? &bg_weapons[weapon - 1]
                                                         : &nullWeapon;
}
...
static const upgradeData_t bg_upgradesData[] = {{UP_LIGHTARMOUR, "larmour"},
...
void BG_InitUpgradeAttributes() {
    ...
    for (i = 0; i < bg_numUpgrades; i++) {
        ...
        BG_ParseUpgradeAttributeFile(va("configs/upgrades/%s.attr.cfg", ua->name),
                                     ua);
    }
}
...
void BG_InitMissileAttributes() {
    ...
    for (i = 0; i < bg_numMissiles; i++) {
        ...
        BG_ParseMissileAttributeFile(
                va("configs/missiles/%s.missile.cfg", ma->name), ma);
        BG_ParseMissileDisplayFile(va("configs/missiles/%s.missile.cfg", ma->name),
                                   ma);
    }
}
...
void BG_InitBeaconAttributes() {
    ...
    for (i = 0; i < bg_numBeacons; i++) {
        ...
        BG_ParseBeaconAttributeFile(va("configs/beacons/%s.beacon.cfg", bd->name),
                                    ba);
    }
...
static const char* const eventnames[] = {
        ...
        "EV_WARN_ATTACK", // a building has been destroyed and the destruction
                          // noticed by a nearby om/rc/rrep
...
bool BG_InventoryContainsWeapon(int weapon, const int stats[]) {
    ...
    // HACK: Determine team by checking for STAT_CLASS since we merged STAT_TEAM
    // into PERS_TEAM
    //       This hack will vanish as soon as the blast isn't the only possible
    //       sidearm weapon anymore
...
===============
*/
bool BG_RotateAxis(vec3_t surfNormal, vec3_t inAxis[3], vec3_t outAxis[3], bool inverse, bool ceiling) {
    ...
    if (ceiling) {
        ...
        // cross the reference normal and the surface normal to get the rotation
        // axis
    ...
    if (VectorLength(xNormal) != 0.0f) {
        ...
        // hmmm could get away with only one rotation and some clever stuff later...
        // but i'm lazy
...
Get the position of the client's eye, based on the client's position, the
surface's normal, and client's view height
===============
*/
...
void BG_PositionBuildableRelativeToPlayer(
        playerState_t* ps, const vec3_t mins, const vec3_t maxs, void (*trace)(trace_t*, const vec3_t, const vec3_t, const vec3_t, const vec3_t, int, int, int), vec3_t outOrigin, vec3_t outAngles, trace_t* tr) {
    ...
    buildDist =
            BG_Class(ps->stats[STAT_CLASS])->buildDist * DotProduct(forward, aimDir);
    ...
    // The mask is MASK_DEADSOLID on purpose to avoid collisions with other
    // entities
...
/**
 * @brief Calculates the "value" of a player as a base value plus a fraction of
 * the price the
...
int BG_GetValueOfPlayer(playerState_t* ps) {
    ...
    switch (ps->persistant[PERS_TEAM]) {
        case TEAM_HUMANS:
            // Add upgrade price
            for (upgradeNum = UP_NONE + 1; upgradeNum < UP_NUM_UPGRADES;
                 upgradeNum++) {
            ...
            // Add weapon price
            for (upgradeNum = WP_NONE + 1; upgradeNum < WP_NUM_WEAPONS;
                 upgradeNum++) {
...
void BG_PackEntityNumbers(entityState_t* es, const int* entityNums, unsigned int count) {
    ...
    if (count > MAX_NUM_PACKED_ENTITY_NUMS) {
        ...
        Com_Printf(S_WARNING
                   "A maximum of %d entity numbers can be "
                   "packed, but BG_PackEntityNumbers was passed %d entities\n",
                   MAX_NUM_PACKED_ENTITY_NUMS,
                   count);
    }
    ...
    for (i = 0; i < MAX_NUM_PACKED_ENTITY_NUMS; i++) {
        ...
        if (entityNum & ~GENTITYNUM_MASK) {
            Com_Error(ERR_FATAL,
                      "BG_PackEntityNumbers passed an entity number (%d) which "
                      "exceeds %d bits",
                      entityNum,
                      GENTITYNUM_BITS);
        }
...
bool BG_WeaponDisabled(int weapon) {
    ...
    for (i = 0;
         i < WP_NUM_WEAPONS && bg_disabledGameElements.weapons[i] != WP_NONE;
         i++) {
...
bool BG_UpgradeDisabled(int upgrade) {
    ...
    for (i = 0;
         i < UP_NUM_UPGRADES && bg_disabledGameElements.upgrades[i] != UP_NONE;
         i++) {
...
bool BG_ClassDisabled(int class_) {
    ...
    for (i = 0;
         i < PCL_NUM_CLASSES && bg_disabledGameElements.classes[i] != PCL_NONE;
         i++) {
...
bool BG_BuildableDisabled(int buildable) {
    ...
    for (i = 0; i < BA_NUM_BUILDABLES &&
                        bg_disabledGameElements.buildables[i] != BA_NONE;
         i++) {
...
int BG_LoadEmoticons(emoticon_t* emoticons, int num) {
    ...
    numFiles =
            trap_FS_GetFileList("emoticons", "x1.crn", fileList, sizeof(fileList));
