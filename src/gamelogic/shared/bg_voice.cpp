...
static bool BG_VoiceParseTrack(int handle, voiceTrack_t* voiceTrack) {
    ...
    while (foundToken) {
        if (token.string[0] == '}') {
            ...
            if (!found) {
                BG_VoiceParseError(handle,
                                   "BG_VoiceParseTrack(): missing \"team\" name");
            }
            ...
            while (foundToken) {
                ...
                if (!Q_stricmp(token.string, "all")) {
                    ...
                    modelno = PCL_ALIEN_CLASSES;
                                      ...
                                      "-")) // this must be outside quotation marks
                {
                    negate = true;
                    modelno = 0;
                } else {
            ...
            if (!found) {
                BG_VoiceParseError(handle,
                                   "BG_VoiceParseTrack(): missing \"class\" name");
            }
...
============
*/
voiceTrack_t* BG_VoiceTrackFind(voiceTrack_t* head, int team, int class_, int weapon, int enthusiasm, int* trackNum) {
    voiceTrack_t* vt = head;
    int highestMatch = 0;
    int matchCount = 0;
    int selectedMatch = 0;
