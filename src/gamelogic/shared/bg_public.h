...
enum {
    ...
    CS_INTERMISSION,     // when 1, timelimit has been hit and intermission will
...
player_state_t
and some other output data.  Used for local prediction on the client game and
true
...
// pmtype_t categories
#define PM_Paralyzed(x) \
    ((x) == PM_DEAD || (x) == PM_FREEZE || (x) == PM_INTERMISSION)
...
#define PMF_ALL_TIMES \
    (PMF_TIME_WATERJUMP | PMF_TIME_KNOCKBACK | PMF_TIME_WALLJUMP)
...
typedef struct pmove_s {
    ...
    /*void    (*trace)( trace_t *results, const vec3_t start, vec3_t mins, vec3_t
   * maxs, const vec3_t end, int passEntityNum, int contentMask );*/
    void (*trace)(trace_t* results, const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end, int passEntityNum, int contentMask, int skipmask);
...
#define SS_BOOSTEDWARNING \
    BIT(8) // booster poison is running out // TODO: Unnecessary, remove
...
#define SB_BUILDABLE_TO_IBE(x) \
    (((x) &SB_BUILDABLE_STATE_MASK) >> SB_BUILDABLE_STATE_SHIFT)
...
typedef enum {
    ...
    IBE_NOPOWERHERE, // not enough power in this area even though a reactor is
                     // present
...
#define EF_TELEPORT_BIT \
    0x0002                     // toggled every time the origin abruptly changes
...
#define EF_BC_TAG_RELEVANT \
    (EF_BC_ENEMY | EF_BC_TAG_PLAYER) // relevant flags for tags
#define EF_BC_BASE_RELEVANT \
    (EF_BC_ENEMY | EF_BC_BASE_OUTPOST) // relevant flags for bases
...
typedef enum {
    ...
    EV_WARN_ATTACK, // a building has been destroyed and the destruction noticed
                    // by a nearby om/rc/rrep
...
typedef enum { BCH_NONE,
               BCH_REMOVE,
               BCH_MOVE } beaconConflictHandler_t;
...
// bg_utilities.c
bool BG_GetTrajectoryPitch(vec3_t origin, vec3_t target, float v0, float g, vec2_t angles, vec3_t dir1, vec3_t dir2);
...
void BG_ParseBuildableAttributeFile(const char* filename,
                                    buildableAttributes_t* ba);
void BG_ParseBuildableModelFile(const char* filename,
...
void BG_ParseUpgradeAttributeFile(const char* filename,
                                  upgradeAttributes_t* ua);
void BG_ParseMissileAttributeFile(const char* filename,
...
// bg_teamprogress.c
#define NUM_UNLOCKABLES \
    WP_NUM_WEAPONS + UP_NUM_UPGRADES + BA_NUM_BUILDABLES + PCL_NUM_CLASSES
...
momentumThresholdIterator_t BG_IterateMomentumThresholds(
        momentumThresholdIterator_t unlockableIter, team_t team, int* threshold, bool* unlocked);
#ifdef BUILD_SGAME
...
voiceTrack_t* BG_VoiceTrackFind(voiceTrack_t* head, int team, int pClass, int weapon, int enthusiasm, int* trackNum);
...
typedef struct { const char* name; } dummyCmd_t;
