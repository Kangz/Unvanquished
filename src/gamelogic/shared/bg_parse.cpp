...
Helper function that tries to read a whole file in a buffer. Should it be in
bg_parse.c?
======================
*/
...
void BG_ParseBuildableAttributeFile(const char* filename,
    ...
    while (1) {
        ...
        } else if ((var = BG_FindConfigVar(va("b_%s_%s", ba->name, token))) !=
                   nullptr) {
...
void BG_ParseClassAttributeFile(const char* filename, classAttributes_t* ca) {
    ...
    while (1) {
        ...
        if (!Q_stricmp(token, "description")) {
            ...
            defined |= UNLOCKTHRESHOLD;
        } else if ((var = BG_FindConfigVar(va("c_%s_%s", ca->name, token))) !=
                   nullptr) {
            BG_ParseConfigVar(var, &text, filename);
...
void BG_ParseWeaponAttributeFile(const char* filename, weaponAttributes_t* wa) {
    ...
    while (1) {
        ...
        if (!Q_stricmp(token, "humanName")) {
            ...
            defined |= UNLOCKTHRESHOLD;
        } else if ((var = BG_FindConfigVar(va("w_%s_%s", wa->name, token))) !=
                   nullptr) {
            BG_ParseConfigVar(var, &text, filename);
        } else {
            Com_Printf(S_ERROR "%s: unknown token '%s'\n", filename, token);
...
void BG_ParseUpgradeAttributeFile(const char* filename,
    ...
    while (1) {
        ...
        if (!Q_stricmp(token, "humanName")) {
            ...
            defined |= UNLOCKTHRESHOLD;
        } else if ((var = BG_FindConfigVar(va("u_%s_%s", ua->name, token))) !=
                   nullptr) {
            BG_ParseConfigVar(var, &text, filename);
        } else {
            Com_Printf(S_ERROR "%s: unknown token '%s'\n", filename, token);
...
void BG_ParseMissileAttributeFile(const char* filename,
    ...
    while (1) {
        ...
        /*else if( (var = BG_FindConfigVar( va( "m_%s_%s", ma->name, token ) ) ) !=
    nullptr )
    {
            BG_ParseConfigVar( var, &text, filename );
    }*/
...
void BG_ParseMissileDisplayFile(const char* filename, missileAttributes_t* ma) {
    ...
    if (ma->usesSprite && (!(defined & SPRITE) || !(defined & SPRITE_SIZE))) {
        ma->usesSprite = false;
    ...
    if (ma->usesAnim &&
        (!(defined & ANIM_START_FRAME) || !(defined & ANIM_NUM_FRAMES) ||
         !(defined & ANIM_FRAME_RATE))) {
        ma->usesAnim = false;
    ...
    if (ma->usesDlight && (!(defined & DLIGHT) || !(defined & DLIGHT_INTENSITY) ||
                           !(defined & DLIGHT_COLOR))) {
        ma->usesDlight = false;
    ...
    if (ma->usesImpactMark &&
        (!(defined & IMPACT_MARK) || !(defined & IMPACT_MARK_SIZE))) {
