...
/**
 * @brief Given an origin, a target point, an initial velocity v0, and a gravity
 * g,
 *        calculate the two possible pitch angles and corresponding directions
 * for
 *        reaching the target on a parabolic trajectory.
 * @return true when angles contains the two possible pitch angles and dir1 and
 * dir2
 *         contain the corresponding directions. false when the target is out of
 * reach.
 */
bool BG_GetTrajectoryPitch(vec3_t origin, vec3_t target, float v0, float g, vec2_t angles, vec3_t dir1, vec3_t dir2) {
    ...
    // create a 2D representation of the problem by cutting out the plane that
    // contains
    // the trajectory
    t2[1] = t3[2];
