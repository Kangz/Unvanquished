...
#ifdef BUILD_CGAME
static void InformUnlockableStatusChange(unlockable_t* unlockable,
                                         bool unlocked) {
}
#endif // BUILD_CGAME
...
static INLINE void CheckStatusKnowledge(unlockableType_t type, int itemNum) {
    ...
    if (!unlockables[unlockablesTypeOffset[type] + itemNum].statusKnown) {
        ...
        Com_Printf(S_WARNING
                   "Asked for the status of unlockable item %s but the status is "
                   "unknown.\n",
...
// retrieve relevant settings
    ...
    // a half life time of 0 means there is no decrease, so we don't need to alter
    // thresholds
    ...
    if (lastMomentumHalfLife == momentumHalfLife &&
        lastunlockableMinTime == unlockableMinTime && lastMod > 0.0f) {
...
void BG_InitUnlockackables() {
    ...
    unlockablesTypeOffset[UNLT_BUILDABLE] =
            unlockablesTypeOffset[UNLT_UPGRADE] + UP_NUM_UPGRADES;
    unlockablesTypeOffset[UNLT_CLASS] =
            unlockablesTypeOffset[UNLT_BUILDABLE] + BA_NUM_BUILDABLES;
...
void BG_ImportUnlockablesFromMask(int team, int mask) {
    ...
    // just import if data is unavailable, cached mask is outdated or team has
    // changed
    ...
    for (unlockableNum = 0; unlockableNum < NUM_UNLOCKABLES; unlockableNum++) {
        ...
        switch (unlockableType) {
            ...
            default:
                Com_Error(ERR_FATAL,
                          "BG_ImportUnlockablesFromMask: Unknown unlockable type");
        }
...
int BG_UnlockablesMask(int team) {
    if (unlockablesTeamKnowledge != team &&
        unlockablesTeamKnowledge != TEAM_ALL) {
        Com_Error(ERR_FATAL,
                  "G_GetUnlockablesMask: Requested mask for a team with unknown "
                  "unlockable status");
    }
...
unlockableType_t BG_UnlockableType(int num) {
    return (unlockableType_t)(((unsigned) num < NUM_UNLOCKABLES)
                                      ? unlockables[num].type
...
static int MomentumNextThreshold(int threshold) {
    ...
    for (i = 0; i < NUM_UNLOCKABLES; ++i) {
        int thisThreshold = unlockables[i].unlocked
                                    ? unlockables[i].lockThreshold
                                    : unlockables[i].unlockThreshold;
...
momentumThresholdIterator_t BG_IterateMomentumThresholds(
    ...
    for (++unlockableIter.num; unlockableIter.num < NUM_UNLOCKABLES;
         unlockableIter.num++) {
        unlockable_t unlockable = unlockables[unlockableIter.num];
        int thisThreshold = unlockable.unlocked ? unlockable.lockThreshold
                                                : unlockable.unlockThreshold;

        if (unlockable.team == team && unlockable.unlockThreshold &&
            (!unlockableIter.threshold ||
             unlockableIter.threshold == thisThreshold)) {
...
static void UpdateUnlockablesMask() {
    ...
    for (unlockable = 0; unlockable < NUM_UNLOCKABLES; unlockable++) {
        if (unlockables[unlockable].unlockThreshold) {
            ...
            {
                Com_Error(ERR_FATAL,
                          "UpdateUnlockablesMask: Number of unlockable items for a "
                          "team exceeded");
            }

            if (!unlockables[unlockable].statusKnown) {
                Com_Error(ERR_FATAL,
                          "UpdateUnlockablesMask: Called before G_UpdateUnlockables");
            }
...
void G_UpdateUnlockables() {
    ...
    for (unlockableNum = 0; unlockableNum < NUM_UNLOCKABLES; unlockableNum++) {
        ...
        /*Com_Printf( "G_UpdateUnlockables: Team %s, Type %s, Item %s, Momentum %d,
       Threshold %d, "
                "Unlocked %d, Synchronize %d\n",
                BG_TeamName( team ), UnlockableTypeName( unlockable ),
       UnlockableName( unlockable ),
                momentum, unlockThreshold, unlockable->unlocked,
       unlockable->synchronize );*/
    }
...
void CG_UpdateUnlockables(playerState_t* ps) {
    BG_ImportUnlockablesFromMask(ps->persistant[PERS_TEAM],
                                 ps->persistant[PERS_UNLOCKABLES]);
}
#endif
