    ...
    while (*msg) {
        if (Q_IsColorString(msg) || *msg == '\n') {
            ...
            if (*msg == '\n') {
                ...
                Com_sprintf(buffer, sizeof(buffer), "\033[%s%d%sm", (colour & 0x30) == 0 ? "0;" : "", 30 + (colour & 15), modifier[(colour / 16) & 3]);
                fputs(buffer, stderr);
                msg += 2;
            }
        } else {
...
==================
CON_SigCont
Reinitialize console input after receiving SIGCONT, as on Linux the terminal
seems to lose all
...
void CON_Init_TTY() {
    ...
    bool stdinIsATTY = isatty(STDIN_FILENO) && isatty(STDOUT_FILENO) &&
...
char* CON_Input_TTY() {
    ...
    if (ttycon_on) {
        ...
        if (avail != -1) {
            // we have something
            // backspace?
            // NOTE TTimo testing a lot of values .. seems it's the only way to get it
            // to work everywhere
