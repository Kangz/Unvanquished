...
static void IN_PrintKey(const SDL_Keysym* keysym, keyNum_t key, bool down) {
    ...
    Com_Printf("%c 0x%02x \"%s\" Q:0x%02x(%s)\n", down ? '+' : ' ', keysym->scancode, SDL_GetKeyName(keysym->sym), key, Key_KeynumToString(key));
}
...
static bool IN_IsConsoleKey(keyNum_t key, const unsigned char character) {
    ...
    static const struct {
        ...
        int key;
            ...
            {"shift", K_SHIFT}, {"ctrl", K_CTRL}, {"alt", K_ALT}, {"super", K_SUPER},
    };
...
static int joy_keys[16] = {K_LEFTARROW, K_RIGHTARROW, K_UPARROW, K_DOWNARROW, K_JOY16, K_JOY17, K_JOY18, K_JOY19, K_JOY20, K_JOY21, K_JOY22, K_JOY23,
        ...
        switch (e.type) {
                ...
                if (!dropInput) {
                    if (mouseActive) {
...
#if defined(__linux__) || defined(__BSD__)
                        {
                            ...
                            // - without this, we get LeaveNotify, no mouse button events,
                            // EnterNotify;
            ...
            case SDL_MOUSEBUTTONUP:
                if (!dropInput) {
                    ...
                    Com_QueueEvent(0, SE_KEY, b, (e.type == SDL_MOUSEBUTTONDOWN ? true : false), 0, nullptr);
                }
                break;
            case SDL_MOUSEWHEEL:
...
void IN_Init(void* windowData) {
    ...
    in_xbox360ControllerDebug =
            Cvar_Get("in_xbox360ControllerDebug", "0", CVAR_TEMP);
    SDL_StartTextInput();
