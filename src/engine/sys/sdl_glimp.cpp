...
bool GLimp_SpawnRenderThread(void (*function)()) {
    ...
    renderThread =
            SDL_CreateThread(GLimp_RenderThreadWrapper, "render thread", nullptr);
...
static int GLimp_SetMode(int mode, bool fullscreen, bool noborder) {
    ...
    icon = SDL_CreateRGBSurfaceFrom(
            (void*) CLIENT_WINDOW_ICON.pixel_data, CLIENT_WINDOW_ICON.width, CLIENT_WINDOW_ICON.height, CLIENT_WINDOW_ICON.bytes_per_pixel * 8, CLIENT_WINDOW_ICON.bytes_per_pixel * CLIENT_WINDOW_ICON.width,
    ...
    if (SDL_GetDesktopDisplayMode(r_displayIndex->integer, &desktopMode) == 0) {
        ...
        ri.Printf(PRINT_ALL,
                  "Cannot determine display aspect (%s), assuming 1.333\n",
                  SDL_GetError());
    }
    ...
    if (glewResult != GLEW_OK) {
        ...
        ri.Error(ERR_FATAL,
                 "GLW_StartOpenGL() - could not load OpenGL subsystem: %s",
...
static bool GLimp_StartDriverAndSetMode(int mode, bool fullscreen, bool noborder) {
    ...
    if (!SDL_WasInit(SDL_INIT_VIDEO)) {
        ...
        if (SDL_Init(SDL_INIT_VIDEO | SDL_INIT_NOPARACHUTE) == -1) {
            ri.Printf(PRINT_ALL,
                      "SDL_Init(SDL_INIT_VIDEO | SDL_INIT_NOPARACHUTE) FAILED (%s)\n",
                      SDL_GetError());
            return false;
        }
...
static GLenum debugTypes[] = {
        0, GL_DEBUG_TYPE_ERROR_ARB, GL_DEBUG_TYPE_DEPRECATED_BEHAVIOR_ARB, GL_DEBUG_TYPE_UNDEFINED_BEHAVIOR_ARB, GL_DEBUG_TYPE_PORTABILITY_ARB, GL_DEBUG_TYPE_PERFORMANCE_ARB, GL_DEBUG_TYPE_OTHER_ARB};
...
#endif
static void DEBUG_CALLBACK_CALL
GLimp_DebugCallback(GLenum source, GLenum type, GLuint id, GLenum severity, GLsizei length, const GLchar* message, const void* userParam) {
...
#define LOAD_EXTENSION_WITH_CVAR(ext, cvar) \
    LoadExtWithCvar(GLEW_##ext, #ext, cvar->value)
...
static void GLimp_InitExtensions() {
    ...
    int reservedComponents =
            26 *
            10; // approximation how many uniforms we have besides the bone matrices
    glConfig2.maxVertexSkinningBones = Maths::clamp(
            (glConfig2.maxVertexUniforms - reservedComponents) / 16, 0, MAX_BONES);
    glConfig2.vboVertexSkinningAvailable =
            r_vboVertexSkinning->integer &&
            ((glConfig2.maxVertexSkinningBones >= 12) ? true : false);

    // GLSL
    REQUIRE_EXTENSION(ARB_shading_language_100);

    Q_strncpyz(glConfig2.shadingLanguageVersionString,
               (char*) glGetString(GL_SHADING_LANGUAGE_VERSION_ARB),
    ...
    if (sscanf(glConfig2.shadingLanguageVersionString, "%i.%i", &majorVersion, &minorVersion) != 2) {
        ri.Printf(PRINT_ALL,
                  "WARNING: unrecognized shading language version string format\n");
    }
    ...
    glGetIntegerv(GL_MAX_CUBE_MAP_TEXTURE_SIZE_ARB,
                  &glConfig2.maxCubeMapTextureSize);

    glConfig2.textureHalfFloatAvailable =
            LOAD_EXTENSION_WITH_CVAR(ARB_half_float_pixel, r_ext_half_float_pixel);
    glConfig2.textureFloatAvailable =
            LOAD_EXTENSION_WITH_CVAR(ARB_texture_float, r_ext_texture_float);
    glConfig2.textureRGAvailable =
            LOAD_EXTENSION_WITH_CVAR(ARB_texture_rg, r_ext_texture_rg);

    // TODO figure out what was the problem with MESA
    glConfig2.framebufferPackedDepthStencilAvailable =
            glConfig.driverType != GLDRV_MESA &&
            LOAD_EXTENSION_WITH_CVAR(EXT_packed_depth_stencil,
                                     r_ext_packed_depth_stencil);

    glConfig2.ARBTextureCompressionAvailable = LOAD_EXTENSION_WITH_CVAR(
            ARB_texture_compression, r_ext_compressed_textures);
    glConfig.textureCompression = TC_NONE;
    if (LOAD_EXTENSION_WITH_CVAR(EXT_texture_compression_s3tc,
    ...
    // Texture - others
    glConfig2.textureNPOTAvailable = LOAD_EXTENSION_WITH_CVAR(
            ARB_texture_non_power_of_two, r_ext_texture_non_power_of_two);
    glConfig2.generateMipmapAvailable =
            LOAD_EXTENSION_WITH_CVAR(SGIS_generate_mipmap, r_ext_generate_mipmap);

    glConfig2.textureAnisotropyAvailable = false;
    if (LOAD_EXTENSION_WITH_CVAR(EXT_texture_filter_anisotropic,
    ...
    if (LOAD_EXTENSION_WITH_CVAR(EXT_framebuffer_object,
    ...
    glConfig2.framebufferBlitAvailable =
            LOAD_EXTENSION_WITH_CVAR(EXT_framebuffer_blit, r_ext_framebuffer_blit);
    ...
    glConfig2.mapBufferRangeAvailable =
            LOAD_EXTENSION_WITH_CVAR(ARB_map_buffer_range, r_arb_map_buffer_range);
...
static void reportDriverType(bool force) {
    static const char* const drivers[] = {"integrated", "stand-alone", "OpenGL 3+", "Mesa"};
    if (glConfig.driverType > GLDRV_UNKNOWN &&
        (int) glConfig.driverType < ARRAY_LEN(drivers)) {
...
static void reportHardwareType(bool force) {
    ...
    if (glConfig.hardwareType > GLHW_UNKNOWN &&
        (int) glConfig.hardwareType < ARRAY_LEN(hardware)) {
        ri.Printf(PRINT_ALL, "%s graphics hardware class '%s'\n", force ? "User has forced" : "Detected", hardware[glConfig.hardwareType]);
    }
}
