...
#ifdef BUILD_SERVER
#define TITLE                                             \
    S_COLOR_GREEN "[ " S_COLOR_YELLOW CLIENT_WINDOW_TITLE \
                  " Server Console " S_COLOR_GREEN "]"
#else
#define TITLE                                             \
    S_COLOR_GREEN "[ " S_COLOR_YELLOW CLIENT_WINDOW_TITLE \
                  " Console " S_COLOR_GREEN "]"
#endif
...
static void CON_SetColor(WINDOW* win, int color) {
    ...
    if (color == CURSES_NULL_COLOR || !com_ansiColor || !com_ansiColor->integer) {
        ...
        wattrset(win, A_RGB((int) (g_color_table[color][0] * 31), (int) (g_color_table[color][1] * 31), (int) (g_color_table[color][2] * 31), 0, 0, 0));
#else
        wattrset(win,
                 COLOR_PAIR(color + 9)); // hard-wired below; see init_pair() calls
#endif
    } else {
...
static void CON_UpdateClock() {
    ...
    Com_Time(&realtime);
    werase(clockwin);
    CON_ColorPrint(clockwin,
                   va("^0[^3%02d%c%02d^0]^7 ", realtime.tm_hour, (realtime.tm_sec & 1) ? ':' : ' ', realtime.tm_min),
                   true);
...
Never exit without calling this, or your terminal will be left in a pretty bad
state
==================
*/
void CON_Shutdown() {
        ...
        switch (chr) {
            case ERR:
                if (forceRedraw) {
                    ...
                    CON_ColorPrint(inputwin,
                                   Str::UTF32To8(input_field.GetViewText()).c_str(),
                                   false);
#ifdef _WIN32
