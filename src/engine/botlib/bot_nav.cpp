...
All vectors used as inputs and outputs to functions here use the engine's
coordinate system
====================
*/
...
static bool withinRadiusOfOffMeshConnection(const Bot_t* bot, rVec pos, rVec off, dtPolyRef conPoly) {
    const dtOffMeshConnection* con =
            bot->nav->mesh->getOffMeshConnectionByRef(conPoly);
    if (!con) {
        return false;
    }
...
static bool overOffMeshConnectionStart(const Bot_t* bot, rVec pos) {
    ...
    const bool offMeshConnection =
            (bot->cornerFlags[corner] & DT_STRAIGHTPATH_OFFMESH_CONNECTION) ? true
                                                                            : false;

    if (offMeshConnection) {
        return withinRadiusOfOffMeshConnection(
                bot, pos, &bot->cornerVerts[corner * 3], bot->cornerPolys[corner]);
    }
    return false;
}
...
void BotUpdateCorridor(int botClientNum, const botRouteTarget_t* target, botNavCmd_t* cmd) {
    ...
    if (!bot->offMesh) {
        ...
        // FIXME: this must be done because of a weird bug where the target is not
        // reachable even if
        ...
        if (bot->numCorners == 0) {
            ...
            float height;
            if (dtStatusSucceed(bot->nav->query->getPolyHeight(
                        bot->corridor.getLastPoly(), cmd->tpos, &height))) {
                cmd->tpos[1] = height;
            }
...
bool BotFindRandomPointInRadius(int botClientNum, const vec3_t origin, vec3_t point, float radius) {
    ...
    dtPolyRef randRef;
    dtStatus status = bot->nav->query->findRandomPointAroundCircle(
            nearPoly, rorigin, radius, &bot->nav->filter, frand, &randRef, nearPoint);
...
void BotAddObstacle(const vec3_t mins, const vec3_t maxs, qhandle_t* obstacleHandle) {
    ...
    for (int i = 0; i < numNavData; i++) {
        ...
        // offset mins down by agent height so obstacles placed on ledges are
        // handled correctly
