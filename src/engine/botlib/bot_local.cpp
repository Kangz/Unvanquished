...
void FindWaypoints(Bot_t* bot, float* corners, unsigned char* cornerFlags, dtPolyRef* cornerPolys, int* numCorners, int maxCorners) {
    ...
    *numCorners =
            bot->corridor.findCorners(corners, cornerFlags, cornerPolys, maxCorners, bot->nav->query, &bot->nav->filter);
}
...
bool PointInPolyExtents(Bot_t* bot, dtPolyRef ref, rVec point, rVec extents) {
    ...
    if (dtStatusFailed(
                bot->nav->query->closestPointOnPolyBoundary(ref, point, closest))) {
        return false;
    }

    // use the bot's bbox as an epsilon because the navmesh is always at least
    // that far from a boundry
...
bool FindRoute(Bot_t* bot, rVec s, botRouteTargetInternal rtarget, bool allowPartial) {
    ...
    status = bot->nav->query->findNearestPoly(rtarget.pos, rtarget.polyExtents, &bot->nav->filter, &endRef, end);
    ...
    status =
            bot->nav->query->findPath(startRef, endRef, start, end, &bot->nav->filter, pathPolys, &pathNumPolys, MAX_BOT_PATH);
