...
bool GetPointPointedTo(NavData_t* nav, rVec& p) {
    ...
    CM_BoxTrace(&trace, cl.snap.ps.origin, end, nullptr, nullptr, 0, CONTENTS_SOLID | CONTENTS_PLAYERCLIP, 0, TT_AABB);
...
void BotDrawNavEdit(DebugDrawQuake* dd) {
    ...
    if (cmd.enabled && GetPointPointedTo(cmd.nav, p)) {
        ...
        if (cmd.offBegin) {
            duAppendArc(dd, cmd.pc.start[0], cmd.pc.start[1], cmd.pc.start[2], p[0], p[1], p[2], 0.25f, 0.6f, 0.6f, col);
...
void Cmd_NavTest() {
    const char usage[] =
            "Usage: navtest "
            "shownodes/hidenodes/showportals/hideportals/startpath/endpath\n";
    const char* arg = nullptr;
    int argc = Cmd_Argc();
