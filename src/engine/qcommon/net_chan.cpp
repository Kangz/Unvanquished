...
static const char* const netsrcString[2] = {"client", "server"};
...
static void NET_QueuePacket(int length, const void* data, netadr_t to, int offset) {
    ...
    newp->to = to;
    newp->release =
            Sys_Milliseconds() + (int) ((float) offset / com_timescale->value);
    newp->next = nullptr;
...
return 0 on address not found, 1 on address found with port, 2 on address found
without port.
=============
*/
