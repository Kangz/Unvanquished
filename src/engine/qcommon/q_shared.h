...
// and before including any other header in case they bring in math.h/cmath
// indirectly.
...
#ifdef _MSC_VER
// vsnprintf is non-conformant in MSVC--fails to null-terminate in case of
// overflow
#define Q_vsnprintf(dest, size, fmt, args) \
    _vsnprintf_s(dest, size, _TRUNCATE, fmt, args)
#define Q_snprintf(dest, size, fmt, ...) \
    _snprintf_s(dest, size, _TRUNCATE, fmt, __VA_ARGS__)
#elif defined(_WIN32)
...
inline bool Q_IsColorString(const char* p) {
    return (p[0] == Q_COLOR_ESCAPE &&
            (p[1] == COLOR_NULL ||
             (p[1] >= '0' && p[1] != Q_COLOR_ESCAPE && p[1] < 'p')))
                   ? true
                   : false;
}
...
Please also open a bug report at bugzilla.icculus.org
// or write a mail to the ioq3 mailing list.
#else
#	define Q_ftol(v) ((long) (v))
#	define Q_round(v) do { if((v) < 0) (v) -= 0.5f; else (v) += 0.5f; (v) =
Q_ftol((v)); } while(0)
 ...
 * Returns <0,0,0> if no intersection point could be found, otherwise returns
...
// RB: NOTE renamed MatrixMultiply to AxisMultiply because it conflicts with
// most new matrix functions
// It is important for mod developers to do this change as well or they risk a
// memory corruption by using
...
// invert any m4x4 using Kramer's rule.. return true if matrix is singular, else
// return false
...
inline __m128 sseCrossProduct(__m128 a, __m128 b) {
    ...
    __m128 c_zxy = _mm_sub_ps(_mm_mul_ps(a, b_yzx), _mm_mul_ps(a_yzx, b));
...
inline __m128 sseQuatNormalize(__m128 q) {
    ...
    __m128 t, h;
    p = _mm_add_ps(sseSwizzle(p, XXZZ), sseSwizzle(p, YYWW));
    p = _mm_add_ps(sseSwizzle(p, XXXX), sseSwizzle(p, ZZZZ));
    t = _mm_rsqrt_ps(p);
#ifdef RSQRT_PRECISE
...
inline void TransInverse(const transform_t* in, transform_t* out) {
    ...
    out->sseRot = invRot;
    out->sseTransScale =
            _mm_or_ps(_mm_and_ps(tmp, mask_XYZ0()), _mm_and_ps(invS, mask_000W()));
}
...
inline void TransAddWeight(float weight, const transform_t* a, transform_t* out) {
    ...
    out->sseTransScale =
            _mm_add_ps(out->sseTransScale, _mm_mul_ps(w, a->sseTransScale));
...
void TransCombine(const transform_t* a, const transform_t* b, transform_t* c);
...
bool SkipBracedSection_Depth(const char** program,
...
typedef enum { FS_SEEK_CUR,
               FS_SEEK_END,
               FS_SEEK_SET } fsOrigin_t;
...
#define Q_strncpyz(string1, string2, length) \
    Q_strncpyzDebug(string1, string2, length, __FILE__, __LINE__)
...
#define CVAR_SYSTEMINFO \
    BIT(3) /*< these cvars will be duplicated on all clients */
...
#define CVAR_TEMP \
    BIT(8)                /*< can be set even when cheats are disabled, but is not archived */
#define CVAR_CHEAT BIT(9) /*< can not be changed if cheats are disabled */
#define CVAR_NORESTART                                                     \
    BIT(10)                 /*< do not clear when a cvar_restart is issued \
                               */
...
// number of flags voip knows. You will have to bump protocol version number if
// you
// change this.
...
typedef struct {
    ...
    float lateralFraction; // fraction of collision tangetially to the trace
                           // direction
} trace_t;
...
//
#define MAX_CLIENTS \
    64 // JPW NERVE back to q3ta default was 128    // absolute limit

#define GENTITYNUM_BITS \
    10 // JPW NERVE put q3ta default back for testing // don't need to send any
       // more
...
#define MODELINDEX_BITS \
    9 // minimum requirement for MAX_SUBMODELS and MAX_MODELS

#define MAX_SUBMODELS \
    512 // 9 bits sent (see qcommon/msg.c); q3map2 limits to 1024 via
        // MAX_MAP_MODELS; not set to 512 to avoid overlap with fake handles
#define MAX_MODELS \
    256                // 9 bits sent (see qcommon/msg.c), but limited by game VM
...
#define REF_FORCE_DLIGHT \
    (1 << 31) // RF, passed in through overdraw parameter, force this dlight
              // under all conditions
#define REF_JUNIOR_DLIGHT \
    (1 << 30) // (SA) this dlight does not light surfaces.  it only affects
              // dynamic light grid
#define REF_DIRECTED_DLIGHT \
    (1 << 29) // ydnar: global directional light, origin should be interpreted as
              // a normal vector
#define REF_RESTRICT_DLIGHT \
    (1 << 1)                        // dlight is restricted to following entities
...
// (Gordon: unless it doesn't need transmission over the network, in which case
// it should probably go into the new pmext struct anyway)
...
typedef struct playerState_s {
    ...
    int oldEventSequence; // so we can see which events have been added since we
                          // last converted to entityState_t
    ...
    // ----------------------------------------------------------------------
    // So to use persistent variables here, which don't need to come from the
    // server,
    // we could use a marker variable, and use that to store everything after it
    // before we read in the new values for the predictedPlayerState, then restore
    // them
    // after copying the structure received from the server.

    // Arnout: use the pmoveExt_t structure in bg_public.h to store this kind of
    // data now (presistant on client, not network transmitted)
...
typedef enum {
    ...
    CA_CINEMATIC     // playing a cinematic or a static pic, not connected to a
                     // server
} connstate_t;
