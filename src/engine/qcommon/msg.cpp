...
void MSG_Copy(msg_t* buf, byte* data, int length, msg_t* src) {
    if (length < src->cursize) {
        Com_Error(ERR_DROP,
                  "MSG_Copy: can't copy %d into a smaller %d msg_t buffer",
                  src->cursize,
                  length);
    }
...
void MSG_WriteDeltaUsercmd(msg_t* msg, usercmd_t* from, usercmd_t* to) {
    ...
    if (from->angles[0] == to->angles[0] && from->angles[1] == to->angles[1] &&
        from->angles[2] == to->angles[2] &&
        from->forwardmove == to->forwardmove &&
        from->rightmove == to->rightmove && from->upmove == to->upmove &&
        !memcmp(from->buttons, to->buttons, sizeof(from->buttons)) &&
        from->weapon == to->weapon && from->flags == to->flags &&
...
static netField_t entityStateFields[] = {
        ...
        {NETF(event), 10},
        {NETF(eventParm), 8},
        {NETF(eventSequence), 8}, // warning: need to modify cg_event.c at "//
                                  // check the sequencial list" if you change this
        {NETF(events[0]), 8},
        {NETF(events[1]), 8},
        {NETF(events[2]), 8},
        {NETF(events[3]), 8},
...
// )
// the float will be sent with FLOAT_INT_BITS, otherwise all 32 bits will be
// sent
        ...
        if (field->bits == 0) {
            ...
            if (fullFloat == 0.0f) {
                ...
                if (trunc == fullFloat && trunc + FLOAT_INT_BIAS >= 0 &&
                    ...
                    //                  if ( print ) {
                    //                      Com_Printf( "%s:%f ", field->name, *(float
                    //                      *)toF );
                    //                  }
                }
            }
    ...
    /*
          ...
          if ( print ) {
                  if ( msg->bit == 0 ) {
                          ...
                          endBit = ( msg->cursize - 1 ) * 8 + msg->bit -
     GENTITYNUM_BITS;
                  }
                  Com_Printf( " (%i bits)\n", endBit - startBit  );
          }
...
If the delta removes the entity, entityState_t->number will be set to
MAX_GENTITIES-1
        ...
        if (statsbits) {
            ...
            for (i = 0; i < MAX_STATS; i++) {
                if (statsbits & (1 << i)) {
                    ...
                    MSG_WriteShort(msg, to->stats[i]); //----(SA)    back to short since
                                                       // weapon bits are handled
                                                       // elsewhere now
                }
            }
        } else {
...
void MSG_ReadDeltaPlayerstate(msg_t* msg, playerState_t* from, playerState_t* to) {
    ...
    *to = *from;
    ...
    // read the arrays
    if (MSG_ReadBits(msg, 1)) {
        // one general bit tells if any of this infrequently changing stuff has
        // changed
        // parse stats
        if (MSG_ReadBits(msg, 1)) {
            LOG("PS_STATS");
            ...
            for (i = 0; i < MAX_STATS; i++) {
                if (bits & (1 << i)) {
                    ...
                    to->stats[i] = MSG_ReadShort(msg); //----(SA)    back to short since
                                                       // weapon bits are handled
                                                       // elsewhere now
                }
            }
