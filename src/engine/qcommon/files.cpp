...
static Cvar::Cvar<std::string> fs_extrapaks(
        "fs_extrapaks",
        "space-seperated list of paks to load in addition to the base pak",
        0,
        "");
...
static std::vector<std::tuple<std::string, std::string, uint32_t>>
        fs_missingPaks;

static Cvar::Cvar<bool>
        allowRemotePakDir("client.allowRemotePakDir",
                          "Connect to servers that load game data from directories",
                          Cvar::TEMPORARY,
                          false);
...
static fileHandle_t FS_FOpenFileWrite_internal(const char* path,
                                               bool temporary) {
    fileHandle_t handle = FS_AllocHandle();
    try {
        handleTable[handle].file = FS::HomePath::OpenWrite(
                temporary ? std::string(path) + TEMP_SUFFIX : path);
    } catch (std::system_error& err) {
    ...
    switch (mode) {
        case FS_READ:
            if (FS::PakPath::FileExists(path))
                ...
                int size =
                        FS_SV_FOpenFileRead(FS::Path::Build("game", path).c_str(), handle);
                return (!handle || *handle) ? size : -1;
            }

        case FS_WRITE:
        case FS_WRITE_VIA_TEMPORARY:
            *handle = FS_FOpenFileWrite_internal(FS::Path::Build("game", path).c_str(),
                                                 mode == FS_WRITE_VIA_TEMPORARY);
            return *handle == 0 ? -1 : 0;
    ...
    if (handleTable[handle].isPakFile) {
        ...
        try {
            ...
            if (handleTable[handle].renameTo) {
                ...
                handleTable[handle].renameTo =
                        Util::nullopt; // tidy up after abusing std::move
                try {
...
int FS_Seek(fileHandle_t handle, long offset, int origin) {
    ...
    if (handleTable[handle].isPakFile) {
        switch (origin) {
            ...
            case FS_SEEK_END:
                handleTable[handle].filePos =
                        handleTable[handle].fileData.size() + offset;
                break;
...
int FS_Read(void* buffer, int len, fileHandle_t handle) {
    ...
    if (handleTable[handle].isPakFile) {
        ...
        memcpy(buffer,
               handleTable[handle].fileData.data() + handleTable[handle].filePos,
               len);
...
int FS_GetFileList(const char* path, const char* extension, char* listBuf, int bufSize) {
    ...
    try {
        for (const std::string& x :
             FS::HomePath::ListFiles(FS::Path::Build("game", path))) {
            if (extension && !Str::IsSuffix(extension, x))
                continue;
            if (dirsOnly != (x.back() == '/'))
                continue;
...
void FS_LoadAllMapMetadata() {
    ...
    for (auto& x : maps) {
        try {
            FS::PakPath::LoadPakPrefix(*FS::FindPak(x),
                                       va("meta/%s/", x.substr(4).c_str()));
        } catch (std::system_error&) {
        } // ignore and move on
    }
}
    ...
    for (auto& x : args) {
        ...
        if (!FS::ParsePakName(x.data(), x.data() + x.size(), name, version, checksum)) {
            ...
            if (isDemo || allowRemotePakDir.Get())
                continue;
            Com_Error(ERR_DROP,
                      "The server is configured to load game data from a directory "
                      "which makes it incompatible with remote clients.");
        }
    ...
    *neededpaks = '\0';
    for (auto& x : fs_missingPaks) {
        if (dlstring) {
            ...
            Q_strcat(neededpaks, len, "@");
            std::string pakName = Str::Format(
                    "pkg/%s.pk3", FS::MakePakName(std::get<0>(x), std::get<1>(x)));
            if (FS::HomePath::FileExists(pakName))
                Q_strcat(
                        neededpaks, len, va("pkg/%s.pk3", FS::MakePakName(std::get<0>(x), std::get<1>(x), std::get<2>(x)).c_str()));
            else
            ...
            if (FS::FindPak(std::get<0>(x), std::get<1>(x))) {
                ...
                if (CL_WWWBadChecksum(FS::MakePakName(std::get<0>(x), std::get<1>(x), std::get<2>(x)).c_str())) {
                    try {
                        FS::HomePath::DeleteFile(Str::Format(
                                "pkg/%s.pk3", FS::MakePakName(std::get<0>(x), std::get<1>(x))));
                    } catch (std::system_error&) {
                    }
                }
...
public:
    DirCmd()
            : Cmd::StaticCmd("dir", Cmd::SYSTEM, "list all files in a given directory with the option to "
                                                 "pass a filter") {}
    ...
    void Run(const Cmd::Args& args) const OVERRIDE {
        ...
        try {
            for (auto& filename : FS::PakPath::ListFiles(args.Argv(1))) {
                if (filename.size() &&
                    (!filter ||
                     Com_Filter(args.Argv(2).c_str(), filename.c_str(), false))) {
        ...
        Print("-----------");
        try {
            for (auto& filename : FS::RawPath::ListFiles(
                         FS::Path::Build(FS::GetHomePath(), args.Argv(1)))) {
                if (filename.size() &&
                    (!filter ||
                     Com_Filter(args.Argv(2).c_str(), filename.c_str(), false))) {
                    Print("%s", filename.c_str());
