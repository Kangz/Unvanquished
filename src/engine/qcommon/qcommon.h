...
typedef struct {
    ...
    bool overflowed;    // set to true if the buffer size failed (with allowoverflow
                        // set)
...
// if this flag is set, always attempt IPv6 connections instead of IPv4 if a v6
// address is found.
#define NET_PRIOV6 0x04
// disables IPv6 multicast support if set.
#define NET_DISABLEMCAST 0x08

// right type anyway || ( DUAL && proto enabled && ( other proto disabled ||
// appropriate IPv6 pref ) )
#define NET_IS_IPv4(type)                                              \
    ((type) == NA_IP ||                                                \
     ((type) == NA_IP_DUAL && (net_enabled->integer & NET_ENABLEV4) && \
      ((~net_enabled->integer & NET_ENABLEV6) ||                       \
       (~net_enabled->integer & NET_PRIOV6))))
#define NET_IS_IPv6(type)                                              \
    ((type) == NA_IP6 ||                                               \
     ((type) == NA_IP_DUAL && (net_enabled->integer & NET_ENABLEV6) && \
      ((~net_enabled->integer & NET_ENABLEV4) ||                       \
       (net_enabled->integer & NET_PRIOV6))))
// if NA_IP_DUAL, get the preferred type (falling back on NA_IP)
#define NET_TYPE(type)                             \
    (NET_IS_IPv4(type) ? NA_IP : NET_IS_IPv6(type) \
                                         ? NA_IP6  \
                                         : ((type) == NA_IP_DUAL) ? NA_IP : (type))
...
// RF, increased this, seems to keep causing problems when set to 64, especially
// when loading
// a savegame, which is hard to fix on that side, since we can't really spread
// out a loadgame
// among several frames
//#define   MAX_RELIABLE_COMMANDS   64          // max string commands buffered
// for restransmit
//#define   MAX_RELIABLE_COMMANDS   128         // max string commands buffered
// for restransmit
...
// maximum length of an formatted IPv6 address string including port and
// trailing '\0'
// format [%s]:%hu - 48 for %s (address), 3 for []: and 5 for %hu (port number,
// max value 65535)
...
#define PROTOCOL_MISMATCH_ERROR \
...
#define PROTOCOL_MISMATCH_ERROR_LONG \
    "ERROR: Protocol Mismatch Between Client and Server.\n\n\
...
Most commands come from either keybindings or console line input, but entire
text
files can be execed.
...
// will properly create any needed paths and deal with separator character
// issues
...
FS_FCloseFile instead of fclose, otherwise the pak FILE would be improperly
closed
It is generally safe to always set uniqueFILE to true, because the majority of
file IO goes through FS_ReadFile, which Does The Right Thing already.
*/

int FS_Delete(const char* filename); // only works inside the 'save' directory
                                     // (for deleting savegames/images)
...
typedef enum { DL_CONTINUE,
               DL_DONE,
               DL_FAILED } dlStatus_t;
...
// bitmask
typedef enum { DL_FLAG_DISCON = 0 } dlFlags_t;
...
// TTimo
// centralized and cleaned, that's the max string you can send to a Com_Printf /
// Com_DPrintf (above gets truncated)
...
int QDECL Com_VPrintf(const char* fmt, va_list argptr)
        VPRINTF_LIKE(1); // conforms to vprintf prototype for print callback passing
...
typedef enum {
    ...
    SE_JOYSTICK_AXIS, // evValue is an axis number and evValue2 is the current
                      // state (-127 to 127)
...
const char* Trans_PgettextGame(const char* ctxt, const char* msgid)
        PRINTF_TRANSLATE_ARG(2);
const char* Trans_GettextGamePlural(const char* msgid, const char* msgid_plural, int num) PRINTF_TRANSLATE_ARG(1)
        PRINTF_TRANSLATE_ARG(2);
#endif // QCOMMON_H_
