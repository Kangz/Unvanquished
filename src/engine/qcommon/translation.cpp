...
public:
    DaemonInputbuf(const std::string& filename)
            : putBack(1) {
        char* end;
...
public:
    DaemonIstream(const std::string& filename)
            : std::istream(new DaemonInputbuf(filename)) {}
    ~DaemonIstream() { delete rdbuf(); }
};
...
void Trans_SetLanguage(const char* lang) {
    ...
    if (bestScore == 0) {
        Com_Printf(S_WARNING
                   "Language \"%s\" (%s) not found. Default to \"English\" (en)\n",
                   requestLang.get_name().empty() ? "Unknown Language"
                                                  : requestLang.get_name().c_str(),
                   lang);
    }
...
void Trans_Init() {
    ...
    trans_manager.set_filesystem(
            std::unique_ptr<FileSystem>(new DaemonFileSystem));
    trans_managergame.set_filesystem(
            std::unique_ptr<FileSystem>(new DaemonFileSystem));
...
void Trans_LoadDefaultLanguage() {
    ...
    if (!language->string[0]) {
        ...
        if (!locale->lang || !locale->lang[0] || !locale->country ||
            !locale->country[0]) {
            Cvar_Set("language", "en");
        } else {
            Cvar_Set("language", va("%s%s%s", locale->lang, locale->country[0] ? "_" : "", locale->country));
        }
...
const char* Trans_Gettext_Internal(const char* msgid,
...
const char* Trans_GettextPlural_Internal(const char* msgid,
    ...
    num = (num + 1) & 3;
    gettextbuffer[num] =
            manager.get_dictionary().translate_plural(msgid, msgid_plural, number);
    return gettextbuffer[num].c_str();
}
