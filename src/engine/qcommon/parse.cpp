...
static bool Parse_AddDefineToSourceFromString(source_t* source, char* string);
...
===============
*/
static void Parse_CreatePunctuationTable(script_t* script,
    ...
    if (!script->punctuationtable) {
        script->punctuationtable =
                (punctuation_t**) Z_Malloc(256 * sizeof(punctuation_t*));
    }
...
===============
*/
static void QDECL PRINTF_LIKE(2)
        Parse_ScriptError(script_t* script, const char* str, ...) {
    char text[1024];
    va_list ap;
...
*/
static void QDECL PRINTF_LIKE(2)
        Parse_ScriptWarning(script_t* script, const char* str, ...) {
    char text[1024];
    va_list ap;
...
static void Parse_SetScriptPunctuations(script_t* script, punctuation_t* p) {
    if (p) {
        ...
        Parse_CreatePunctuationTable(script, Default_Punctuations);
    }
    ...
    switch (*script->script_p) {
        ...
        default: // NOTE: decimal ASCII code, NOT octal
        {
            if (*script->script_p < '0' || *script->script_p > '9') {
                Parse_ScriptError(script, "unknown escape char");
            }
...
===============
*/
static void Parse_NumberValue(char* string, int subtype, unsigned long int* intvalue, double* floatvalue) {
...
static int Parse_ReadNumber(script_t* script, token_t* token) {
    ...
    if (*script->script_p == '0' &&
        ...
        while ((c >= '0' && c <= '9') || (c >= 'a' && c <= 'f') ||
            ...
            if (len >= MAX_TOKEN_CHARS) {
                Parse_ScriptError(script,
                                  "hexadecimal number longer than MAX_TOKEN_CHARS = %d",
                                  MAX_TOKEN_CHARS);
                return 0;
            }
    ...
    // check for a binary number
        ...
        while (c == '0' || c == '1') {
            ...
            if (len >= MAX_TOKEN_CHARS) {
                Parse_ScriptError(script,
                                  "binary number longer than MAX_TOKEN_CHARS = %d",
                                  MAX_TOKEN_CHARS);
                return 0;
            }
...
static int Parse_ReadPunctuation(script_t* script, token_t* token) {
    ...
    for (punc = script->punctuationtable[(unsigned int) *script->script_p]; punc;
         punc = punc->next) {
        p = punc->p;
        len = strlen(p);
...
static int Parse_ReadPrimitive(script_t* script, token_t* token) {
    ...
    while (*script->script_p > ' ' && *script->script_p != ';') {
        if (len >= MAX_TOKEN_CHARS) {
            Parse_ScriptError(script,
                              "primitive token longer than MAX_TOKEN_CHARS = %d",
                              MAX_TOKEN_CHARS);
            return 0;
        }
...
*/
static void QDECL PRINTF_LIKE(2)
        Parse_SourceError(source_t* source, const char* str, ...) {
    char text[1024];
    va_list ap;
...
*/
static void QDECL PRINTF_LIKE(2)
        Parse_SourceWarning(source_t* source, const char* str, ...) {
    char text[1024];
    va_list ap;
...
===============
*/
// char primes[16] = {1, 3, 5, 7, 11, 13, 17, 19, 23, 27, 29, 31, 37, 41, 43,
// 47};
...
===============
*/
static int Parse_ExpandBuiltinDefine(source_t* source, token_t* deftoken, define_t* define, token_t** firsttoken, token_t** lasttoken) {
    token_t* token;
    time_t t;
...
===============
*/
static int Parse_ExpandDefine(source_t* source, token_t* deftoken, define_t* define, token_t** firsttoken, token_t** lasttoken) {
    ...
    for (dt = define->tokens; dt; dt = dt->next) {
        ...
        if (parmnum >= 0) {
            ...
            if (dt->string[0] == '#' && dt->string[1] == '\0') {
                ...
                if (dt->next) {
                    parmnum = Parse_FindDefineParm(define, dt->next->string);
                } else {
                    parmnum = -1;
                }
...
===============
*/
static int Parse_EvaluateTokens(source_t* source, token_t* tokens, signed long int* intvalue, double* floatvalue, int integer) {
            ...
            case TT_PUNCTUATION: {
                ...
                if (!integer) {
                    if (t->subtype == P_BIN_NOT || t->subtype == P_MOD ||
                        ...
                        t->subtype == P_BIN_XOR) {
                        Parse_SourceError(source,
                                          "illegal operator %s on floating point operands",
                                          t->string);
                        error = 1;
                        break;
                    }
...
===============
*/
static int Parse_Evaluate(source_t* source, signed long int* intvalue, double* floatvalue, int integer) {
...
===============
*/
static int Parse_DollarEvaluate(source_t* source, signed long int* intvalue, double* floatvalue, int integer) {
...
===============
*/
directive_t Directives[20] = {{"if", Parse_Directive_if},
...
static bool Parse_ReadEnumeration(source_t* source) {
    ...
    if (newtoken.type != TT_PUNCTUATION || newtoken.subtype != P_BRACEOPEN) {
        Parse_SourceError(source, "Found %s when expecting {", newtoken.string);
        return false;
    }
    ...
    for (value = 0;; value++) {
        ...
        if (name.type != TT_NAME) {
            Parse_SourceError(source, "Found %s when expecting identifier", name.string);
            return false;
        }
        ...
        if (newtoken.type != TT_PUNCTUATION) {
            Parse_SourceError(source, "Found %s when expecting , or = or }", newtoken.string);
            return false;
        }
        ...
        if (newtoken.subtype == P_ASSIGN) {
            ...
            if (newtoken.type != TT_NUMBER || !(newtoken.subtype & TT_INTEGER)) {
                Parse_SourceError(source, "Found %s when expecting integer", newtoken.string);
                return false;
            }
        ...
        if (newtoken.type != TT_PUNCTUATION ||
            (newtoken.subtype != P_COMMA && newtoken.subtype != P_BRACECLOSE)) {
            Parse_SourceError(source, "Found %s when expecting , or }", newtoken.string);
            return false;
        }

        if (!Parse_AddDefineToSourceFromString(source,
                                               va("%s %d\n", name.string, value))) {
            Parse_SourceWarning(source, "Couldn't add define to source: %s = %d", name.string, value);
            return false;
        }
...
*/
static bool Parse_AddDefineToSourceFromString(source_t* source, char* string) {
    Parse_PushScript(source,
                     Parse_LoadScriptMemory(string, strlen(string), "*extern"));
...
static source_t* Parse_LoadSourceFile(const char* filename) {
    ...
    source->definehash =
            (define_t**) Z_Malloc(DEFINEHASHSIZE * sizeof(define_t*));
