...
cvar_t* com_crashed = nullptr; // ydnar: set in case of a crash, prevents
                               // CVAR_UNSAFE variables from being set from a
                               // cfg
...
cvar_t* com_logfile; // 1 = buffer log, 2 = flush after each print, 3 = append
                     // + flush
cvar_t* com_version;
...
int QDECL VPRINTF_LIKE(1) Com_VPrintf(const char* fmt, va_list argptr) {
    ...
    // would be racy, but this gets called prior to renderer threads etc. being
    // started
    if (!lock) {
...
bool Com_SafeMode() {
    ...
    for (i = 0; i < com_numConsoleLines; i++) {
        ...
        if (line.size() > 1 && (!Q_stricmp(line[0].c_str(), "safe") ||
                                !Q_stricmp(line[0].c_str(), "cvar_restart"))) {
...
// The server gives the sv_cheats cvar to the client, on 'off' it prevents the
// user from changing Cvar::CHEAT cvars
Cvar::Callback<Cvar::Cvar<bool>>
        cvar_cheats("sv_cheats", "can cheats be used in the current game", Cvar::SYSTEMINFO | Cvar::ROM, true, SetCheatMode);
...
Goals:
        reproducable without history effects -- no out of memory errors on weird
map to map changes
        allow restarting of the client without fragmentation
        minimize total pages in use at run time
        minimize total pages needed during load time

  Single block of memory with stack allocators coming from both ends towards the
middle.
...
void Com_Meminfo_f() {
    ...
    Com_Printf("%9i bytes (%6.2f MB) high tempHighwater\n",
               ...
               hunk_high.tempHighwater / Square(1024.f));
    Com_Printf("\n");
...
void Com_InitHunkMemory() {
    ...
    if (cv->integer < MIN_COMHUNKMEGS) {
        ...
        Com_Printf("Minimum com_hunkMegs is " XSTRING(
                MIN_COMHUNKMEGS) ", allocating " XSTRING(MIN_COMHUNKMEGS) "MB.\n");
    } else {
...
int Hunk_MemoryRemaining() {
    ...
    high = hunk_high.permanent > hunk_high.temp ? hunk_high.permanent
                                                : hunk_high.temp;
...
void* Hunk_Alloc(int size, ha_pref preference) {
    ...
    // Ridah, update the com_hunkused cvar in increments, so we don't update it
    // too often, since this cvar call isn't very efficent
    if ((hunk_low.permanent + hunk_high.permanent) >
        com_hunkused->integer + 2500) {
        Cvar_Set("com_hunkused",
                 va("%i", hunk_low.permanent + hunk_high.permanent));
    }
...
void* Hunk_AllocateTempMemory(int size) {
    ...
    // loaded
    // by the file system without redundant routines in the file system utilizing
    // different
    // memory systems
    if (s_hunkData == nullptr) {
    ...
    while (1) {
        ...
        switch (ev.evType) {
            ...
            case SE_CONSOLE: {
                ...
                if (cmd[0] == '/' || cmd[0] == '\\') {
                    // make sure, explicit commands are not getting handled with
                    // com_consoleCommand
                    Cmd::BufferCommandTextAfter(cmd + 1, true);
                } else {
                    /*
         * when there was no command prefix, execute the command prefixed by
         *com_consoleCommand
         * if the cvar is empty, it will interpret the text as command
         *direclty
         * (and will so for BUILD_SERVER)
         *
         * the additional space gets trimmed by the parser
         */
                    Cmd::BufferCommandTextAfter(
                            va("%s %s", com_consoleCommand->string, cmd), true);
                }
...
void Com_SetRecommended() {
    ...
    // will use this for recommended settings as well.. do i outside the lower
    // check so it gets done even with command line stuff
...
void Com_Init(char* commandLine) {
    ...
    // we could differentiate server and client, but would change the default
    // behavior many might be used to
...
================
*/
static Cvar::Cvar<int>
        fixedtime("common.framerate.fixed",
                  "in milliseconds, forces the frame time, 0 for no effect",
                  Cvar::CHEAT,
                  0);
...
static Cvar::Cvar<int> maxfps("common.framerate.max",
                              "the max framerate, 0 for unlimited",
                              Cvar::NONE,
                              125);
static Cvar::Cvar<int> maxfpsUnfocused(
        "common.framerate.maxUnfocused",
        "the max framerate when the game is unfocused, 0 for unlimited",
        Cvar::NONE,
        0);
static Cvar::Cvar<int> maxfpsMinimized(
        "common.framerate.maxMinimized",
        "the max framerate when the game is minimized, 0 for unlimited",
        Cvar::NONE,
        0);

static Cvar::Cvar<int> watchdogThreshold("common.watchdogTime",
                                         "seconds of server running without a "
                                         "map after which common.watchdogCmd "
                                         "is executed",
                                         Cvar::NONE,
                                         60);
static Cvar::Cvar<std::string>
        watchdogCmd("common.watchdogCmd",
                    "the command triggered by the watchdog, empty for /quit",
                    Cvar::NONE,
                    "");

static Cvar::Cvar<bool>
        showTraceStats("common.showTraceStats",
                       "are physics traces stats printed each frame",
                       Cvar::CHEAT,
                       false);
...
void Com_Frame() {
    ...
    // watchdog
    //
    if (Com_IsDedicatedServer() && !Com_ServerRunning() &&
        watchdogThreshold.Get() != 0) {
        if (watchdogTime == 0) {
            watchdogTime = Sys_Milliseconds();
        } else {
            if (!watchWarn &&
                Sys_Milliseconds() - watchdogTime >
                        (watchdogThreshold.Get() - 4) * 1000) {
                Com_Log(LOG_WARN, "watchdog will trigger in 4 seconds");
                watchWarn = true;
            } else if (Sys_Milliseconds() - watchdogTime >
                       watchdogThreshold.Get() * 1000) {
    ...
    if (com_speeds->integer) {
        ...
        Com_Printf(
                "frame:%i all:%3i sv:%3i sev:%3i cev:%3i cl:%3i gm:%3i rf:%3i bk:%3i\n",
                com_frameNumber,
                all,
                sv,
                sev,
...
int Sys_Milliseconds() {
    ...
    return std::chrono::duration_cast<std::chrono::milliseconds>(
                   Sys::SteadyClock::now() - baseTime).count();
}
