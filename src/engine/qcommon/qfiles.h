...
#define SHADER_MAX_VERTEXES \
    10000 // Arnout: 1024+1 (1 buffer for RB_EndSurface overflow check) // JPW
          // NERVE was 4000, 1000 in q3ta
...
typedef struct { float st[2]; } md3St_t;
...
typedef struct { int indexes[3]; } mdsTriangle_t;
...
typedef struct {
    // float     angles[3];
    // float     ofsAngles[2];
    short angles[4];    // to be converted to axis at run-time (this is also better
                        // for lerping)
    short ofsAngles[2]; // PITCH/YAW, head in this direction from parent to go to
                        // the offset position
...
typedef struct {
    ...
    vec3_t parentOffset;               // one bone is an ascendant of all other bones, it
                                       // starts the hierachy at this position
...
#define BSP_IDENT \
    (('P' << 24) + ('S' << 16) + ('B' << 8) + 'I') // little-endian "IBSP"
...
typedef struct {
    char shader[64];
    int brushNum;
    int visibleSide; // the brush side that ray tests need to clip against (-1 ==
                     // none)
} dfog_t;
...
typedef struct {
    ...
    int firstVert;
    int numVerts; // ydnar: num verts + foliage origins (for cleaner lighting
                  // code in q3map)
...
//----(SA) added so I didn't change the dsurface_t struct (and thereby the bsp
// format) for something that doesn't need to be stored in the bsp
typedef struct { char* lighttarg; } drsurfaceInternal_t;
