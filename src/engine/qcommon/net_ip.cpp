...
// use an admin local address per default so that network admins can decide on
// how to handle quake3 traffic.
...
static void Sys_SockaddrToString(char* dest, int destlen, struct sockaddr* input) {
    ...
    if (getnameinfo(input, inputlen, dest, destlen, nullptr, 0, NI_NUMERICHOST) &&
        destlen > 0) {
        *dest = '\0';
    }
}
        ...
        if (ret == SOCKET_ERROR) {
            ...
            if (usingSocks && memcmp(&from, &socksRelayAddr, fromlen) == 0) {
                if (ret < 10 || net_message->data[0] != 0 ||
                    net_message->data[1] != 0 || net_message->data[2] != 0 ||
...
void Sys_SendPacket(int length, const void* data, netadr_t to) {
    ...
    if (to.type != NA_BROADCAST && to.type != NA_IP && to.type != NA_IP_DUAL &&
...
bool Sys_IsLANAddress(netadr_t adr) {
    ...
    for (index = 0; index < numIP; index++) {
        if (localIP[index].type == adr.type) {
            if (adr.type == NA_IP) {
                compareip = (byte*) &((struct sockaddr_in*) &localIP[index].addr)
                                    ->sin_addr.s_addr;
                comparemask = (byte*) &((struct sockaddr_in*) &localIP[index].netmask)
                                      ->sin_addr.s_addr;
                compareadr = adr.ip;
                ...
                compareip =
                        (byte*) &((struct sockaddr_in6*) &localIP[index].addr)->sin6_addr;
                comparemask = (byte*) &((struct sockaddr_in6*) &localIP[index].netmask)
                                      ->sin6_addr;
                compareadr = adr.ip6;
            ...
            for (run = 0; run < addrsize; run++) {
                if ((compareip[run] & comparemask[run]) !=
                    (compareadr[run] & comparemask[run])) {
                    differed = true;
                    break;
                }
            }
...
SOCKET NET_IPSocket(const char* net_interface, int port, struct sockaddr_in* bindto, int* err) {
    ...
    *err = 0;
    ...
    Com_Printf("Opening IP socket: %s:%s\n",
               net_interface ? net_interface : "0.0.0.0",
               port ? va("%i", port) : "*");
    ...
    if (bindto) {
        ...
        if (address.sin_port == 0) {
            ...
            if (!getsockname(newsocket, (struct sockaddr*) &addr, &addrlen) &&
                addrlen) {
...
SOCKET NET_IP6Socket(const char* net_interface, int port, struct sockaddr_in6* bindto, int* err) {
    ...
    *err = 0;
    ...
    if ((newsocket = socket(PF_INET6, SOCK_DGRAM, IPPROTO_UDP)) ==
        INVALID_SOCKET) {
        *err = socketError;
...
void NET_JoinMulticast6() {
    ...
    if (ip6_socket == INVALID_SOCKET || multicast6_socket != INVALID_SOCKET ||
        (net_enabled->integer & NET_DISABLEMCAST)) {
        return;
    }

    if (IN6_IS_ADDR_MULTICAST(&boundto.sin6_addr) ||
        IN6_IS_ADDR_UNSPECIFIED(&boundto.sin6_addr)) {
        // The way the socket was bound does not prohibit receiving multi-cast
        // packets. So we don't need to open a new one.
        multicast6_socket = ip6_socket;
    } else {
        if ((multicast6_socket =
                     NET_IP6Socket(net_mcast6addr->string, ntohs(boundto.sin6_port), nullptr, &err)) == INVALID_SOCKET) {
            // If the OS does not support binding to multicast addresses, like Windows
            // XP, at least try with a non-multicast socket.
            multicast6_socket = ip6_socket;
        }
    }
...
void NET_OpenSocks(int port) {
    ...
    if ((socks_socket = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP)) ==
        INVALID_SOCKET) {
    ...
    if (connect(socks_socket, (struct sockaddr*) &address, sizeof(address)) ==
        SOCKET_ERROR) {
...
static void NET_GetLocalAddress() {
    ...
    if (!getaddrinfo(hostname, nullptr, &hint, &res)) {
        ...
        /* On operating systems where it's more difficult to find out the configured
     * interfaces, we'll just assume a
...
static bool NET_GetCvars() {
    ...
    /* End users have it enabled so they can connect to IPv6-only hosts, but IPv4
   * will be
   * used if available due to ping */
    ...
    // Some cvars for configuring multicast options which facilitates scanning for
    // servers on local subnets.
...
void NET_Config(bool enableNetworking) {
    ...
    // if enable state is the same and no cvars were modified, we have nothing to
    // do
...
const char* NET_GeoIP_Country(const netadr_t* from) {
    switch (from->type) {
        case NA_IP:
            return geoip_data_4 ? GeoIP_country_name_by_ipnum(
                                          geoip_data_4, htonl(*(uint32_t*) from->ip))
                                : nullptr;

        case NA_IP6:
            return geoip_data_6 ? GeoIP_country_name_by_ipnum_v6(
                                          geoip_data_6, *(struct in6_addr*) from->ip6)
                                : nullptr;
