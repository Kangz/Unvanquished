 ...
 * <name>             — prints the current value of the named cvar, or says that
 *the cvar does not exist
 * (unless the name is actually the name of a command, in which case the command
 *is executed)
 * <name> <value>     — sets the value of the cvar if the cvar exists (unless,
 *see above)
 * set <name> <value> — sets the value of the cvar, but creates the cvar if it
 *does not exist yet
...
typedef struct cvar_s {
    ...
    /**
   * indicate whether the cvar won't be archived, even if it's an ARCHIVE
   * flagged cvar.
   * this allows us to keep ARCHIVE cvars unwritten to autogen until a user
   * changes them
   */
    bool transient;
...
/**
 * creates the variable if it doesn't exist, or returns the existing one if it
 * exists.
 * the value will not be changed, but flags will be ORed in that allows
 * variables
