...
typedef struct cmdContext_s {
    ...
    char tokenized[BIG_INFO_STRING +
                   MAX_STRING_TOKENS]; // will have 0 bytes inserted
    char cmd[BIG_INFO_STRING];         // the original command we received (no token
                                       // processing)
} cmdContext_t;
...
Internal.
Escape all '\' '"' '$', '/' if marking a comment, and ';' if not in quotation
marks.
Optionally wrap in ""
============
*/
...
// Is registered in the new command system for all the commands registered
// through the C interface.
class ProxyCmd : public Cmd::CmdBase {
public:
    ProxyCmd()
            : Cmd::CmdBase(Cmd::PROXY_FOR_OLD) {}
...
void Cmd_AddCommand(const char* cmd_name, xcommand_t function) {
    ...
    // VMs do not properly clean up commands so we avoid creating a command if
