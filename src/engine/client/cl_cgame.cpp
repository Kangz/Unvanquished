...
bool CL_HandleServerCommand(Str::StringRef text, std::string& newText) {
    ...
    // bcs0 to bcs2 are used by the server to send info strings that are bigger
    // than the size of a packet.
    // See also SV_UpdateConfigStrings
    // bcs0 starts a new big config string
    // bcs1 continues it
    // bcs2 finishes it and feeds it back as a new command sent by the server
    // (bcs0 makes it a cs command)
    if (cmd == "bcs0") {
        if (argc >= 3) {
    ...
    if (cmd == "pubkey_decrypt") {
        ...
        if (argc == 1) {
            Com_Printf("%s",
                       "^3Server sent a pubkey_decrypt command, but sent nothing to "
                       "decrypt!\n");
            return false;
        }
...
void CL_FillServerCommands(std::vector<std::string>& commands, int start, int end) {
    ...
    if (start <= clc.serverCommandSequence - MAX_RELIABLE_COMMANDS) {
        ...
        Com_Error(ERR_DROP,
                  "CL_FillServerCommand: a reliable command was cycled out");
    }

    if (end > clc.serverCommandSequence) {
        Com_Error(ERR_DROP,
                  "CL_FillServerCommand: requested a command not received");
    }
...
bool CL_GetSnapshot(int snapshotNumber, snapshot_t* snapshot) {
    ...
    CL_FillServerCommands(snapshot->serverCommands,
                          clc.lastExecutedServerCommand + 1,
...
static int LAN_ServerIsVisible(int source, int n) {
    switch (source) {
        case AS_LOCAL:
            ...
            if (Cmd_Argc() == 1) {
                Com_Log(LOG_ERROR,
                        "Server sent a pubkey_decrypt command, but sent nothing to "
                        "decrypt!");
                return false;
            }
...
CL_UpdateLevelHunkUsage
  ...
  This updates the "hunkusage.dat" file with the current map and its hunk usage
count

  This is used for level loading, so we can show a percentage bar dependent on
the amount
  of hunk memory allocated so far

  This will be slightly inaccurate if some settings like sound quality are
changed, but these
...
void CL_UpdateLevelHunkUsage() {
    ...
    memusage = Cvar_VariableIntegerValue("com_hunkused") +
               Cvar_VariableIntegerValue("hunk_soundadjust");
...
void CL_InitCGameCVars() { /* TODO I don't understand that
                                  vm_t *cgv_vm = VM_Create( "cgame",
                              CL_CgameSystemCalls, (vmInterpret_t)
                              Cvar_VariableIntegerValue( "vm_cgame" ) );

                                  if ( !cgv_vm )
                                  {
                                          Com_Error( ERR_DROP, "VM_Create on
                              cgame failed" );
                                  }
...
void CL_AdjustTimeDelta() {
  ...
  */
    ...
    if (deltaDelta > RESET_TIME) {
        ...
        cl.oldServerTime =
                cl.snap.serverTime; // FIXME: is this a problem for cgame?
...
void CL_FirstSnapshot() {
    ...
    if (!clc.speexInitialized) {
        ...
        clc.speexPreprocessor =
                speex_preprocess_state_init(clc.speexFrameSize, clc.speexSampleRate);
...
bool CL_GetTag(int clientNum, const char* tagname, orientation_t* orientation) {
    ...
    // the current design of CG_GET_TAG is inappropriate for modules in sandboxed
    // formats
    //  (the direct pointer method to pass the tag name would work only with
    //  modules in native format)
...
void CL_OnTeamChanged(int newTeam) {
    if (p_team->integer == newTeam &&
        p_team->modificationCount >
                0) // to make sure, we run the hook initially as well
    {
        return;
    }
   ...
   * the user can use the cvars p_team or p_teamname (if the cgame sets it)
   * within that config
   * to e.g. execute team specific configs, like cg_<team>Config did previously,
   * but with less dependency on the cgame
   */
    Cmd::BufferCommandText("exec -f " TEAMCONFIG_NAME);
}

CGameVM::CGameVM()
        : VM::VMBase("cgame"), services(nullptr), cmdBuffer("client") {
}

void CGameVM::Start() {
    services = std::unique_ptr<VM::CommonVMServices>(
            new VM::CommonVMServices(*this, "CGame", Cmd::CGAME_VM));
...
void CGameVM::QVMSyscall(int index, Util::Reader& reader, IPC::Channel& channel) {
    switch (index) {
        case CG_SENDCLIENTCOMMAND:
            IPC::HandleMsg<SendClientCommandMsg>(
                    channel, std::move(reader), [this](std::string command) {
      ...
      SCR_UpdateScreen(); });
        ...
        case CG_CM_MARKFRAGMENTS:
            ...
            IPC::HandleMsg<CMMarkFragmentsMsg>(
                    channel, std::move(reader), [this](std::vector<std::array<float, 3>> points, std::array<float, 3> projection, int maxPoints, int maxFragments, std::vector<std::array<float, 3>>& pointBuffer, std::vector<markFragment_t>& fragmentBuffer) {
          pointBuffer.resize(maxPoints);
          fragmentBuffer.resize(maxFragments);
          int numFragments = re.MarkFragments(
              points.size(), (vec3_t *)points.data(), projection.data(),
              maxPoints, (float *)pointBuffer.data(), maxFragments,
              fragmentBuffer.data());
          fragmentBuffer.resize(numFragments); });
            break;

        case CG_GETCURRENTSNAPSHOTNUMBER:
            IPC::HandleMsg<GetCurrentSnapshotNumberMsg>(
                    channel, std::move(reader), [this](int& number, int& serverTime) {
          CL_GetCurrentSnapshotNumber(&number, &serverTime); });
            break;

        case CG_GETSNAPSHOT:
            IPC::HandleMsg<GetSnapshotMsg>(
                    channel, std::move(reader), [this](int number, bool& res, snapshot_t& snapshot) {
          res = CL_GetSnapshot(number, &snapshot); });
            break;

        case CG_GETCURRENTCMDNUMBER:
            IPC::HandleMsg<GetCurrentCmdNumberMsg>(
                    channel, std::move(reader), [this](int& number) { number = CL_GetCurrentCmdNumber(); });
            break;

        case CG_GETUSERCMD:
            IPC::HandleMsg<GetUserCmdMsg>(
                    channel, std::move(reader), [this](int number, bool& res, usercmd_t& cmd) {
          res = CL_GetUserCmd(number, &cmd); });
            break;

        case CG_SETUSERCMDVALUE:
            IPC::HandleMsg<SetUserCmdValueMsg>(
                    channel, std::move(reader), [this](int stateValue, int flags, float scale) {
          CL_SetUserCmdValue(stateValue, flags, scale); });
            break;

        case CG_GET_ENTITY_TOKEN:
            IPC::HandleMsg<GetEntityTokenMsg>(
                    channel, std::move(reader), [this](int len, bool& res, std::string& token) {
        ...
        case CG_REGISTER_BUTTON_COMMANDS:
            IPC::HandleMsg<RegisterButtonCommandsMsg>(
                    channel, std::move(reader), [this](std::string commands) {
          CL_RegisterButtonCommands(commands.c_str()); });
            break;

        case CG_GETCLIPBOARDDATA:
            IPC::HandleMsg<GetClipboardDataMsg>(
                    channel, std::move(reader), [this](int len, int type, std::string& data) {
        ...
        case CG_QUOTESTRING:
            IPC::HandleMsg<QuoteStringMsg>(
                    channel, std::move(reader), [this](int len, std::string input, std::string& output) {
          std::unique_ptr<char[]> buffer(new char[len]);
        ...
        case CG_PGETTEXT:
            IPC::HandleMsg<PGettextMsg>(
                    channel, std::move(reader), [this](int len, std::string context, std::string input, std::string& output) {
          std::unique_ptr<char[]> buffer(new char[len]);
        ...
        case CG_GETTEXT_PLURAL:
            IPC::HandleMsg<GettextPluralMsg>(
                    channel, std::move(reader), [this](int len, std::string input1, std::string input2, int number, std::string& output) {
          std::unique_ptr<char[]> buffer(new char[len]);
          Q_strncpyz(buffer.get(), P__(input1.c_str(), input2.c_str(), number),
                     len);
          output.assign(buffer.get(), len); });
            break;

        case CG_NOTIFY_TEAMCHANGE:
            IPC::HandleMsg<NotifyTeamChangeMsg>(
                    channel, std::move(reader), [this](int team) { CL_OnTeamChanged(team); });
            break;

        case CG_PREPAREKEYUP:
            IPC::HandleMsg<PrepareKeyUpMsg>(channel, std::move(reader), [this] { IN_PrepareKeyUp(); });
            break;

        case CG_GETNEWS:
            IPC::HandleMsg<GetNewsMsg>(
                    channel, std::move(reader), [this](bool force, bool& res) { res = GetNews(force); });
            break;

        // All sounds

        case CG_S_REGISTERSOUND:
            IPC::HandleMsg<Audio::RegisterSoundMsg>(
                    channel, std::move(reader), [this](std::string sample, int& handle) {
        ...
        case CG_R_SETALTSHADERTOKENS:
            IPC::HandleMsg<Render::SetAltShaderTokenMsg>(
                    channel, std::move(reader), [this](std::string tokens) { re.SetAltShaderTokens(tokens.c_str()); });
            break;

        case CG_R_GETSHADERNAMEFROMHANDLE:
            IPC::HandleMsg<Render::GetShaderNameFromHandleMsg>(
                    channel, std::move(reader), [this](int handle, std::string& name) {
          name = re.ShaderNameFromHandle(handle); });
            break;

        case CG_R_INPVVS:
            IPC::HandleMsg<Render::InPVVSMsg>(
                    channel, std::move(reader), [this](std::array<float, 3> p1, std::array<float, 3> p2, bool& res) {
          res = re.inPVVS(p1.data(), p2.data()); });
            break;

        case CG_R_LOADWORLDMAP:
            IPC::HandleMsg<Render::LoadWorldMapMsg>(
                    channel, std::move(reader), [this](std::string mapName) {
          re.SetWorldVisData(CM_ClusterPVS(-1));
          re.LoadWorld(mapName.c_str()); });
            break;

        case CG_R_REGISTERMODEL:
            IPC::HandleMsg<Render::RegisterModelMsg>(
                    channel, std::move(reader), [this](std::string name, int& handle) {
          handle = re.RegisterModel(name.c_str()); });
            break;

        case CG_R_REGISTERSKIN:
            IPC::HandleMsg<Render::RegisterSkinMsg>(
                    channel, std::move(reader), [this](std::string name, int& handle) {
          handle = re.RegisterSkin(name.c_str()); });
            break;

        case CG_R_REGISTERSHADER:
            IPC::HandleMsg<Render::RegisterShaderMsg>(
                    channel, std::move(reader), [this](std::string name, int flags, int& handle) {
          handle = re.RegisterShader(name.c_str(), (RegisterShaderFlags)flags); });
            break;

        case CG_R_REGISTERFONT:
            IPC::HandleMsg<Render::RegisterFontMsg>(
                    channel, std::move(reader), [this](std::string name, std::string fallbackName, int pointSize, fontMetrics_t& font) {
          re.RegisterFontVM(name.c_str(), fallbackName.c_str(), pointSize,
                            &font); });
            break;

        case CG_R_MODELBOUNDS:
            IPC::HandleMsg<Render::ModelBoundsMsg>(
                    channel, std::move(reader), [this](int handle, std::array<float, 3>& mins, std::array<float, 3>& maxs) {
          re.ModelBounds(handle, mins.data(), maxs.data()); });
            break;

        case CG_R_LERPTAG:
            IPC::HandleMsg<Render::LerpTagMsg>(
                    channel, std::move(reader), [this](refEntity_t&& entity, std::string tagName, int startIndex, orientation_t& tag, int& res) {
          res = re.LerpTag(&tag, &entity, tagName.c_str(), startIndex); });
            break;

        case CG_R_REMAP_SHADER:
            IPC::HandleMsg<Render::RemapShaderMsg>(
                    channel, std::move(reader), [this](std::string oldShader, std::string newShader, std::string timeOffset) {
          re.RemapShader(oldShader.c_str(), newShader.c_str(),
                         timeOffset.c_str()); });
            break;

        case CG_R_INPVS:
            IPC::HandleMsg<Render::InPVSMsg>(
                    channel, std::move(reader), [this](std::array<float, 3> p1, std::array<float, 3> p2, bool& res) {
          res = re.inPVS(p1.data(), p2.data()); });
            break;

        case CG_R_LIGHTFORPOINT:
            IPC::HandleMsg<Render::LightForPointMsg>(
                    channel, std::move(reader), [this](std::array<float, 3> point, std::array<float, 3>& ambient, std::array<float, 3>& directed, std::array<float, 3>& dir, int res) {
          res = re.LightForPoint(point.data(), ambient.data(), directed.data(),
                                 dir.data()); });
            break;

        case CG_R_REGISTERANIMATION:
            IPC::HandleMsg<Render::RegisterAnimationMsg>(
                    channel, std::move(reader), [this](std::string name, int& handle) {
          handle = re.RegisterAnimation(name.c_str()); });
            break;

        case CG_R_BUILDSKELETON:
            IPC::HandleMsg<Render::BuildSkeletonMsg>(
                    channel, std::move(reader), [this](int anim, int startFrame, int endFrame, float frac, bool clearOrigin, refSkeleton_t& skel, int& res) {
          res = re.BuildSkeleton(&skel, anim, startFrame, endFrame, frac,
                                 clearOrigin); });
            break;

        case CG_R_BONEINDEX:
            IPC::HandleMsg<Render::BoneIndexMsg>(
                    channel, std::move(reader), [this](int model, std::string boneName, int& index) {
          index = re.BoneIndex(model, boneName.c_str()); });
            break;

        case CG_R_ANIMNUMFRAMES:
            IPC::HandleMsg<Render::AnimNumFramesMsg>(
                    channel, std::move(reader), [this](int anim, int& res) { res = re.AnimNumFrames(anim); });
            break;

        case CG_R_ANIMFRAMERATE:
            IPC::HandleMsg<Render::AnimFrameRateMsg>(
                    channel, std::move(reader), [this](int anim, int& res) { res = re.AnimFrameRate(anim); });
            break;

        case CG_REGISTERVISTEST:
            IPC::HandleMsg<Render::RegisterVisTestMsg>(
                    channel, std::move(reader), [this](int& handle) { handle = re.RegisterVisTest(); });
            break;

        case CG_CHECKVISIBILITY:
            IPC::HandleMsg<Render::CheckVisibilityMsg>(
                    channel, std::move(reader), [this](int handle, float& res) { res = re.CheckVisibility(handle); });
            break;

        // All keys

        case CG_KEY_GETCATCHER:
            IPC::HandleMsg<Key::GetCatcherMsg>(
                    channel, std::move(reader), [this](int& catcher) { catcher = Key_GetCatcher(); });
            break;

        case CG_KEY_SETCATCHER:
            IPC::HandleMsg<Key::SetCatcherMsg>(
                    channel, std::move(reader), [this](int catcher) { Key_SetCatcher(catcher); });
            break;

        case CG_KEY_GETKEYNUMFORBINDS:
            IPC::HandleMsg<Key::GetKeynumForBindsMsg>(
                    channel, std::move(reader), [this](int team, std::vector<std::string> binds, std::vector<std::vector<int>>& result) {
          for (auto &bind : binds) {
            result.push_back({});
        ...
        case CG_KEY_KEYNUMTOSTRINGBUF:
            IPC::HandleMsg<Key::KeyNumToStringMsg>(
                    channel, std::move(reader), [this](int keynum, int len, std::string& result) {
          std::unique_ptr<char[]> buffer(new char[len]);
        ...
        case CG_LAN_GETSERVERCOUNT:
            IPC::HandleMsg<LAN::GetServerCountMsg>(
                    channel, std::move(reader), [this](int source, int& count) { count = LAN_GetServerCount(source); });
            break;

        case CG_LAN_GETSERVERINFO:
            IPC::HandleMsg<LAN::GetServerInfoMsg>(
                    channel, std::move(reader), [this](int source, int n, int len, std::string& info) {
          std::unique_ptr<char[]> buffer(new char[len]);
        ...
        case CG_LAN_MARKSERVERVISIBLE:
            IPC::HandleMsg<LAN::MarkServerVisibleMsg>(
                    channel, std::move(reader), [this](int source, int n, bool visible) {
          LAN_MarkServerVisible(source, n, visible); });
            break;

        case CG_LAN_SERVERISVISIBLE:
            IPC::HandleMsg<LAN::ServerIsVisibleMsg>(
                    channel, std::move(reader), [this](int source, int n, bool& visible) {
          visible = LAN_ServerIsVisible(source, n); });
            break;

        case CG_LAN_UPDATEVISIBLEPINGS:
            IPC::HandleMsg<LAN::UpdateVisiblePingsMsg>(
                    channel, std::move(reader), [this](int source, bool& res) {
          res = LAN_UpdateVisiblePings(source); });
            break;

        case CG_LAN_RESETPINGS:
            IPC::HandleMsg<LAN::ResetPingsMsg>(channel, std::move(reader), [this](int n) { LAN_ResetPings(n); });
            break;

        case CG_LAN_SERVERSTATUS:
            IPC::HandleMsg<LAN::ServerStatusMsg>(
                    channel, std::move(reader), [this](std::string serverAddress, int len, std::string& status, int& res) {
          std::unique_ptr<char[]> buffer(new char[len]);
        ...
        case CG_LAN_RESETSERVERSTATUS:
            IPC::HandleMsg<LAN::ResetServerStatusMsg>(
                    channel, std::move(reader), [this] { LAN_GetServerStatus(nullptr, nullptr, 0); });
            break;

        // All rocket

        case CG_ROCKET_INIT:
            IPC::HandleMsg<Rocket::InitMsg>(channel, std::move(reader), [this] { Rocket_Init(); });
            break;

        case CG_ROCKET_SHUTDOWN:
            IPC::HandleMsg<Rocket::ShutdownMsg>(channel, std::move(reader), [this] { Rocket_Shutdown(); });
            break;

        case CG_ROCKET_LOADDOCUMENT:
            IPC::HandleMsg<Rocket::LoadDocumentMsg>(
                    channel, std::move(reader), [this](std::string path) { Rocket_LoadDocument(path.c_str()); });
            break;

        case CG_ROCKET_LOADCURSOR:
            IPC::HandleMsg<Rocket::LoadCursorMsg>(
                    channel, std::move(reader), [this](std::string path) { Rocket_LoadCursor(path.c_str()); });
            break;

        case CG_ROCKET_DOCUMENTACTION:
            IPC::HandleMsg<Rocket::DocumentActionMsg>(
                    channel, std::move(reader), [this](std::string name, std::string action) {
          Rocket_DocumentAction(name.c_str(), action.c_str()); });
            break;

        case CG_ROCKET_GETEVENT:
            IPC::HandleMsg<Rocket::GetEventMsg>(
                    channel, std::move(reader), [this](bool& got, std::string& cmdText) {
          got = Rocket_GetEvent(cmdText); });
            break;

        case CG_ROCKET_DELETEEVENT:
            IPC::HandleMsg<Rocket::DeleteEventMsg>(channel, std::move(reader), [this] { Rocket_DeleteEvent(); });
            break;

        case CG_ROCKET_REGISTERDATASOURCE:
            IPC::HandleMsg<Rocket::RegisterDataSourceMsg>(
                    channel, std::move(reader), [this](std::string name) { Rocket_RegisterDataSource(name.c_str()); });
            break;

        case CG_ROCKET_DSADDROW:
            IPC::HandleMsg<Rocket::DSAddRowMsg>(
                    channel, std::move(reader), [this](std::string name, std::string table, std::string data) {
          Rocket_DSAddRow(name.c_str(), table.c_str(), data.c_str()); });
            break;

        case CG_ROCKET_DSCLEARTABLE:
            IPC::HandleMsg<Rocket::DSClearTableMsg>(
                    channel, std::move(reader), [this](std::string name, std::string table) {
          Rocket_DSClearTable(name.c_str(), table.c_str()); });
            break;

        case CG_ROCKET_SETINNERRML:
            IPC::HandleMsg<Rocket::SetInnerRMLMsg>(
                    channel, std::move(reader), [this](std::string rml, int flags) {
          Rocket_SetInnerRML("", "", rml.c_str(), flags); });
            break;

        case CG_ROCKET_GETATTRIBUTE:
            IPC::HandleMsg<Rocket::GetAttributeMsg>(
                    channel, std::move(reader), [this](std::string attribute, int len, std::string& result) {
          std::unique_ptr<char[]> buffer(new char[len]);
        ...
        case CG_ROCKET_SETATTRIBUTE:
            IPC::HandleMsg<Rocket::SetAttributeMsg>(
                    channel, std::move(reader), [this](std::string attribute, std::string value) {
          Rocket_SetAttribute("", "", attribute.c_str(), value.c_str()); });
            break;

        case CG_ROCKET_GETPROPERTY:
            IPC::HandleMsg<Rocket::GetPropertyMsg>(
                    channel, std::move(reader), [this](std::string property, int type, int len, std::vector<char>& result) {
          std::unique_ptr<char[]> buffer(new char[len]);
          Rocket_GetProperty(property.c_str(), buffer.get(), len,
                             (rocketVarType_t)type);
          result.assign(buffer.get(), buffer.get() + len); });
            break;

        case CG_ROCKET_SETPROPERTYBYID:
            IPC::HandleMsg<Rocket::SetPropertyMsg>(
                    channel, std::move(reader), [this](std::string property, std::string value) {
          Rocket_SetPropertyById("", property.c_str(), value.c_str()); });
            break;

        case CG_ROCKET_GETEVENTPARAMETERS:
            IPC::HandleMsg<Rocket::GetEventParametersMsg>(
                    channel, std::move(reader), [this](int len, std::string& result) {
          std::unique_ptr<char[]> buffer(new char[len]);
        ...
        case CG_ROCKET_REGISTERDATAFORMATTER:
            IPC::HandleMsg<Rocket::RegisterDataFormatterMsg>(
                    channel, std::move(reader), [this](std::string name) {
          Rocket_RegisterDataFormatter(name.c_str()); });
            break;

        case CG_ROCKET_DATAFORMATTERRAWDATA:
            IPC::HandleMsg<Rocket::DataFormatterDataMsg>(
                    channel, std::move(reader), [this](int handle, int nameLength, int dataLength, std::string& name, std::string& data) {
          std::unique_ptr<char[]> nameBuffer(new char[nameLength]);
          std::unique_ptr<char[]> dataBuffer(new char[dataLength]);
          Rocket_DataFormatterRawData(handle, nameBuffer.get(), nameLength,
                                      dataBuffer.get(), dataLength);
          name.assign(nameBuffer.get(), nameLength);
          data.assign(dataBuffer.get(), dataLength); });
            break;

        case CG_ROCKET_DATAFORMATTERFORMATTEDDATA:
            IPC::HandleMsg<Rocket::DataFormatterFormattedDataMsg>(
                    channel, std::move(reader), [this](int handle, std::string data, bool parseQuake) {
          Rocket_DataFormatterFormattedData(handle, data.c_str(), parseQuake); });
            break;

        case CG_ROCKET_REGISTERELEMENT:
            IPC::HandleMsg<Rocket::RegisterElementMsg>(
                    channel, std::move(reader), [this](std::string tag) { Rocket_RegisterElement(tag.c_str()); });
            break;

        case CG_ROCKET_GETELEMENTTAG:
            IPC::HandleMsg<Rocket::GetElementTagMsg>(
                    channel, std::move(reader), [this](int len, std::string& result) {
          std::unique_ptr<char[]> buffer(new char[len]);
        ...
        case CG_ROCKET_GETELEMENTABSOLUTEOFFSET:
            IPC::HandleMsg<Rocket::GetElementAbsoluteOffsetMsg>(
                    channel, std::move(reader), [this](float& x, float& y) {
          Rocket_GetElementAbsoluteOffset(&x, &y); });
            break;

        case CG_ROCKET_QUAKETORML:
            IPC::HandleMsg<Rocket::QuakeToRMLMsg>(
                    channel, std::move(reader), [this](std::string input, int len, std::string& result) {
          std::unique_ptr<char[]> buffer(new char[len]);
        ...
        case CG_ROCKET_SETCLASS:
            IPC::HandleMsg<Rocket::SetClassMsg>(
                    channel, std::move(reader), [this](std::string Class, bool activate) {
          Rocket_SetClass(Class.c_str(), activate); });
            break;

        case CG_ROCKET_INITHUDS:
            IPC::HandleMsg<Rocket::InitHUDsMsg>(
                    channel, std::move(reader), [this](int size) { Rocket_InitializeHuds(size); });
            break;

        case CG_ROCKET_LOADUNIT:
            IPC::HandleMsg<Rocket::LoadUnitMsg>(
                    channel, std::move(reader), [this](std::string path) { Rocket_LoadUnit(path.c_str()); });
            break;

        case CG_ROCKET_ADDUNITTOHUD:
            IPC::HandleMsg<Rocket::AddUnitToHUDMsg>(
                    channel, std::move(reader), [this](int weapon, std::string id) {
          Rocket_AddUnitToHud(weapon, id.c_str()); });
            break;

        case CG_ROCKET_SHOWHUD:
            IPC::HandleMsg<Rocket::ShowHUDMsg>(
                    channel, std::move(reader), [this](int weapon) { Rocket_ShowHud(weapon); });
            break;

        case CG_ROCKET_CLEARHUD:
            IPC::HandleMsg<Rocket::ClearHUDMsg>(
                    channel, std::move(reader), [this](int weapon) { Rocket_ClearHud(weapon); });
            break;

        case CG_ROCKET_ADDTEXT:
            IPC::HandleMsg<Rocket::AddTextMsg>(
                    channel, std::move(reader), [this](std::string text, std::string Class, float x, float y) {
          Rocket_AddTextElement(text.c_str(), Class.c_str(), x, y); });
            break;

        case CG_ROCKET_CLEARTEXT:
            IPC::HandleMsg<Rocket::ClearTextMsg>(channel, std::move(reader), [this] { Rocket_ClearText(); });
            break;

        case CG_ROCKET_REGISTERPROPERTY:
            IPC::HandleMsg<Rocket::RegisterPropertyMsg>(
                    channel, std::move(reader), [this](std::string name, std::string defaultValue, bool inherited, bool forceLayout, std::string parseAs) {
          Rocket_RegisterProperty(name.c_str(), defaultValue.c_str(), inherited,
                                  forceLayout, parseAs.c_str()); });
            break;

        case CG_ROCKET_SHOWSCOREBOARD:
            IPC::HandleMsg<Rocket::ShowScoreboardMsg>(
                    channel, std::move(reader), [this](std::string name, bool show) {
          Rocket_ShowScoreboard(name.c_str(), show); });
            break;

        case CG_ROCKET_SETDATASELECTINDEX:
            IPC::HandleMsg<Rocket::SetDataSelectIndexMsg>(
                    channel, std::move(reader), [this](int index) { Rocket_SetDataSelectIndex(index); });
            break;

        case CG_ROCKET_LOADFONT:
            IPC::HandleMsg<Rocket::LoadFontMsg>(
                    channel, std::move(reader), [this](std::string font) { Rocket_LoadFont(font.c_str()); });
            break;
...
template <typename Func, typename Id, typename... MsgArgs>
void HandleMsg(IPC::Message<Id, MsgArgs...>, Util::Reader reader, Func&& func) {
    typedef IPC::Message<Id, MsgArgs...> Message;

    typename IPC::detail::MapTuple<typename Message::Inputs>::type inputs;
    reader.FillTuple<0>(Util::TypeListFromTuple<typename Message::Inputs>(),
                        inputs);
...
void HandleMsg(Util::Reader reader, Func&& func) {
    HandleMsg(Msg(), std::move(reader), std::forward<Func>(func));
}

CGameVM::CmdBuffer::CmdBuffer(std::string name)
        : IPC::CommandBufferHost(name) {
}
    ...
    if (major == VM::QVM) {
        switch (minor) {
            ...
            case CG_S_STARTSOUND:
                HandleMsg<Audio::StartSoundMsg>(
                        std::move(reader),
                        [this](bool isPositional, std::array<float, 3> origin, int entityNum, int sfx) {
            Audio::StartSound(entityNum,
                              (isPositional ? origin.data() : nullptr), sfx);
                        });
                break;

            case CG_S_STARTLOCALSOUND:
                HandleMsg<Audio::StartLocalSoundMsg>(
                        std::move(reader), [this](int sfx) { Audio::StartLocalSound(sfx); });
                break;

            case CG_S_CLEARLOOPINGSOUNDS:
                HandleMsg<Audio::ClearLoopingSoundsMsg>(
                        std::move(reader), [this] { Audio::ClearAllLoopingSounds(); });
                break;

            case CG_S_ADDLOOPINGSOUND:
                HandleMsg<Audio::AddLoopingSoundMsg>(
                        std::move(reader), [this](int entityNum, int sfx) {
            Audio::AddEntityLoopingSound(entityNum, sfx); });
                break;

            case CG_S_STOPLOOPINGSOUND:
                HandleMsg<Audio::StopLoopingSoundMsg>(
                        std::move(reader), [this](int entityNum) {
            Audio::ClearLoopingSoundsForEntity(entityNum); });
                break;

            case CG_S_UPDATEENTITYPOSITION:
                HandleMsg<Audio::UpdateEntityPositionMsg>(
                        std::move(reader),
                        [this](int entityNum, std::array<float, 3> position) {
            Audio::UpdateEntityPosition(entityNum, position.data());
                        });
                break;

            case CG_S_RESPATIALIZE:
                HandleMsg<Audio::RespatializeMsg>(
                        std::move(reader), [this](int entityNum, std::array<float, 9> axis) {
            Audio::UpdateListener(entityNum, (vec3_t *)axis.data()); });
                break;

            case CG_S_STARTBACKGROUNDTRACK:
                HandleMsg<Audio::StartBackgroundTrackMsg>(
                        std::move(reader), [this](std::string intro, std::string loop) {
            Audio::StartMusic(intro.c_str(), loop.c_str()); });
                break;

            case CG_S_STOPBACKGROUNDTRACK:
                HandleMsg<Audio::StopBackgroundTrackMsg>(std::move(reader),
                                                         [this] { Audio::StopMusic(); });
                break;

            case CG_S_UPDATEENTITYVELOCITY:
                HandleMsg<Audio::UpdateEntityVelocityMsg>(
                        std::move(reader),
                        [this](int entityNum, std::array<float, 3> velocity) {
            Audio::UpdateEntityVelocity(entityNum, velocity.data());
                        });
                break;

            case CG_S_SETREVERB:
                HandleMsg<Audio::SetReverbMsg>(
                        std::move(reader),
                        [this](int slotNum, std::string name, float ratio) {
            Audio::SetReverb(slotNum, name.c_str(), ratio);
                        });
                break;

            case CG_S_BEGINREGISTRATION:
                HandleMsg<Audio::BeginRegistrationMsg>(
                        std::move(reader), [this] { Audio::BeginRegistration(); });
                break;

            case CG_S_ENDREGISTRATION:
                HandleMsg<Audio::EndRegistrationMsg>(
                        std::move(reader), [this] { Audio::EndRegistration(); });
                break;

            // All renderer

            case CG_R_SCISSOR_ENABLE:
                HandleMsg<Render::ScissorEnableMsg>(
                        std::move(reader), [this](bool enable) { re.ScissorEnable(enable); });
                break;

            case CG_R_SCISSOR_SET:
                HandleMsg<Render::ScissorSetMsg>(
                        std::move(reader),
                        [this](int x, int y, int w, int h) { re.ScissorSet(x, y, w, h); });
                break;

            case CG_R_CLEARSCENE:
                HandleMsg<Render::ClearSceneMsg>(std::move(reader),
                                                 [this] { re.ClearScene(); });
                break;

            case CG_R_ADDREFENTITYTOSCENE:
                HandleMsg<Render::AddRefEntityToSceneMsg>(
                        std::move(reader),
                        [this](refEntity_t&& entity) { re.AddRefEntityToScene(&entity); });
                break;

            case CG_R_ADDPOLYTOSCENE:
                HandleMsg<Render::AddPolyToSceneMsg>(
                        std::move(reader), [this](int shader, std::vector<polyVert_t> verts) {
            re.AddPolyToScene(shader, verts.size(), verts.data()); });
                break;

            case CG_R_ADDPOLYSTOSCENE:
                HandleMsg<Render::AddPolysToSceneMsg>(
                        std::move(reader), [this](int shader, std::vector<polyVert_t> verts, int numVerts, int numPolys) {
            re.AddPolysToScene(shader, numVerts, verts.data(), numPolys); });
                break;

            case CG_R_ADDLIGHTTOSCENE:
                HandleMsg<Render::AddLightToSceneMsg>(
                        std::move(reader),
                        [this](std::array<float, 3> point, float radius, float intensity, float r, float g, float b, int shader, int flags) {
            re.AddLightToScene(point.data(), radius, intensity, r, g, b, shader,
                               flags);
                        });
                break;

            case CG_R_ADDADDITIVELIGHTTOSCENE:
                HandleMsg<Render::AddAdditiveLightToSceneMsg>(
                        std::move(reader), [this](std::array<float, 3> point, float intensity, float r, float g, float b) {
            re.AddAdditiveLightToScene(point.data(), intensity, r, g, b); });
                break;

            case CG_R_SETCOLOR:
                HandleMsg<Render::SetColorMsg>(
                        std::move(reader),
                        [this](std::array<float, 4> color) { re.SetColor(color.data()); });
                break;

            case CG_R_SETCLIPREGION:
                HandleMsg<Render::SetClipRegionMsg>(std::move(reader),
                                                    [this](std::array<float, 4> region) {
                                            re.SetClipRegion(region.data());
                                                    });
                break;

            case CG_R_RESETCLIPREGION:
                HandleMsg<Render::ResetClipRegionMsg>(
                        std::move(reader), [this] { re.SetClipRegion(nullptr); });
                break;

            case CG_R_DRAWSTRETCHPIC:
                HandleMsg<Render::DrawStretchPicMsg>(
                        std::move(reader),
                        [this](float x, float y, float w, float h, float s1, float t1, float s2, float t2, int shader) {
            re.DrawStretchPic(x, y, w, h, s1, t1, s2, t2, shader);
                        });
                break;

            case CG_R_DRAWROTATEDPIC:
                HandleMsg<Render::DrawRotatedPicMsg>(
                        std::move(reader),
                        [this](float x, float y, float w, float h, float s1, float t1, float s2, float t2, int shader, float angle) {
            re.DrawRotatedPic(x, y, w, h, s1, t1, s2, t2, shader, angle);
                        });
                break;

            case CG_ADDVISTESTTOSCENE:
                HandleMsg<Render::AddVisTestToSceneMsg>(
                        std::move(reader), [this](int handle, std::array<float, 3> pos, float depthAdjust, float area) {
            re.AddVisTestToScene(handle, pos.data(), depthAdjust, area); });
                break;

            case CG_UNREGISTERVISTEST:
                HandleMsg<Render::UnregisterVisTestMsg>(
                        std::move(reader),
                        [this](int handle) { re.UnregisterVisTest(handle); });
                break;

            case CG_SETCOLORGRADING:
                HandleMsg<Render::SetColorGradingMsg>(
                        std::move(reader),
                        [this](int slot, int shader) { re.SetColorGrading(slot, shader); });
                break;

            case CG_R_RENDERSCENE:
                HandleMsg<Render::RenderSceneMsg>(
                        std::move(reader), [this](refdef_t rd) { re.RenderScene(&rd); });
                break;
