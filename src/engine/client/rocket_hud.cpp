...
struct HudUnit {
    HudUnit(Rocket::Core::ElementDocument* doc)
            : unit(doc) { load = true; }
...
void Rocket_ShowHud(int weapon) {
    ...
    if (activeHud) {
        for (RocketHud::iterator it = activeHud->begin(); it != activeHud->end();
             ++it) {
            HudUnit* unit;
    ...
    for (RocketHud::iterator it = currentHud->begin(); it != currentHud->end();
         ++it) {
        if (it->load) {
            it->unit->Show();
        }
