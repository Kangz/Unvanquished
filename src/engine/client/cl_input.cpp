...
void IN_KeyDown(kbutton_t* b) {
    ...
    int k = nokey ? -1 : Key_GetKeyNumber(); // -1 if typed manually at the
                                             // console for continuous down
...
void CL_AdjustAngles() {
    ...
    if (!kb[KB_STRAFE].active) {
        cl.viewangles[YAW] -=
                speed * cl_yawspeed->value * CL_KeyState(&kb[KB_RIGHT]);
        cl.viewangles[YAW] +=
                speed * cl_yawspeed->value * CL_KeyState(&kb[KB_LEFT]);
    }

    cl.viewangles[PITCH] -=
            speed * cl_pitchspeed->value * CL_KeyState(&kb[KB_LOOKUP]);
    cl.viewangles[PITCH] +=
            speed * cl_pitchspeed->value * CL_KeyState(&kb[KB_LOOKDOWN]);
}
...
void CL_KeyMove(usercmd_t* cmd) {
    ...
    if (!cl.doubleTap.lastdoubleTap ||
        com_frameTime - cl.doubleTap.lastdoubleTap >
        ...
        for (i = 1; i < DT_NUM; i++) {
            key_down = dtmapping[i] == KB_BUTTONS || kb[dtmapping[i]].active ||
...
void CL_Xbox360ControllerMove(usercmd_t* cmd) {
    ...
    cl.viewangles[PITCH] += anglespeed * cl_pitchspeed->value *
                            (cl.joystickAxis[AXIS_PITCH] / 127.0f);
    cl.viewangles[YAW] +=
            anglespeed * cl_yawspeed->value * (cl.joystickAxis[AXIS_YAW] / 127.0f);

    cmd->rightmove = ClampChar(cmd->rightmove + cl.joystickAxis[AXIS_SIDE]);
    cmd->forwardmove =
            ClampChar(cmd->forwardmove + cl.joystickAxis[AXIS_FORWARD]);
...
void CL_MouseMove(usercmd_t* cmd) {
    ...
    if (cl_mouseAccel->value != 0.0f) {
        if (cl_mouseAccelStyle->integer == 0) {
            ...
            // cl_mouseAccelOffset is the rate for which the acceleration will have
            // doubled the non accelerated amplification
            // NOTE: decouple the config cvars for independent acceleration setup
            // along X and Y?

            rate[0] = fabs(mx) / (float) frame_msec;
            rate[1] = fabs(my) / (float) frame_msec;
            power[0] =
                    powf(rate[0] / cl_mouseAccelOffset->value, cl_mouseAccel->value);
            power[1] =
                    powf(rate[1] / cl_mouseAccelOffset->value, cl_mouseAccel->value);

            mx =
                    cl_sensitivity->value *
                    (mx + ((mx < 0) ? -power[0] : power[0]) * cl_mouseAccelOffset->value);
            my =
                    cl_sensitivity->value *
                    (my + ((my < 0) ? -power[1] : power[1]) * cl_mouseAccelOffset->value);

            /*  NERVE - SMF - this has moved to CG_CalcFov to fix zoomed-in/out
         transition movement bug
...
void CL_WritePacket() {
    ...
    // NOTE TTimo: if you verbose this, you will see that there are quite a few
    // duplicates
    ...
    // added a WARNING message, we'll see if there are legit situations where this
    // happens
...
static const struct {
    const char* name;
    int key;
} builtinButtonCommands[] = {{"moveup", KB_UP},
                             {"movedown", KB_DOWN},
                             {"left", KB_LEFT},
                             {"right", KB_RIGHT},
                             {"forward", KB_FORWARD},
...
void IN_KeysUp_f() {
    ...
    for (i = 0; i < USERCMD_BUTTONS; ++i) {
        if (kb[KB_BUTTONS + i].down[0] == key ||
            kb[KB_BUTTONS + i].down[1] == key) {
            if (first) {
    ...
    for (i = 0; builtinButtonCommands[i].name; ++i) {
        if (kb[builtinButtonCommands[i].key].down[0] == key ||
            kb[builtinButtonCommands[i].key].down[1] == key) {
            if (first) {
                Cmd::ExecuteCommand(va("setkeydata %d %d %u", check, key + 1, time));
                first = false;
            }

            Cmd::ExecuteCommand(va(
                    "-%s",
...
void CL_RegisterButtonCommands(const char* cmd_names) {
    ...
    for (i = 0; cmd_names && i < USERCMD_BUTTONS; ++i) {
        ...
        Q_snprintf(name + 1, sizeof(name) - 1, "%.*s", (int) (term ? (term - cmd_names) : sizeof(name) - 1), cmd_names);
    ...
    if (cmd_names) {
        Com_Printf(S_WARNING
                   "cgame: some button commands left unregistered (\"%s\")",
                   cmd_names);
    }
}
...
void CL_InitInput() {
    ...
    while (builtinButtonCommands[i].name != nullptr) {
        Cmd_AddCommand(va("-%s", builtinButtonCommands[i].name),
                       IN_BuiltinButtonCommand);
        Cmd_AddCommand(va("+%s", builtinButtonCommands[i].name),
                       IN_BuiltinButtonCommand);
        i++;
    }
