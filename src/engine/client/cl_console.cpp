 ...
 * 1: lock scrolling if in scrollback, but scroll down for send message/entered
 * commands
...
#define CON_LINE(line)                                    \
    (((line) % consoleState.maxScrollbackLengthInLines) * \
     consoleState.textWidthInChars)
...
void Con_Dump_f() {
    ...
    if (l == 1) {
        ...
        strftime(name, sizeof(name), "condump/%Y%m%d-%H%M%S%z.txt", localtime(&now));
    } else {
    ...
    // skip empty lines
    for (l = consoleState.currentLine - consoleState.maxScrollbackLengthInLines +
             1;
         l <= consoleState.currentLine;
         l++) {
...
void Con_Search_f() {
    ...
    // check the lines
    for (l = consoleState.scrollLineIndex - 1 + direction;
         l <= consoleState.currentLine &&
                 consoleState.currentLine - l <
...
bool Con_CheckResize() {
    ...
    if (cls.glconfig.vidWidth) {
        const int consoleVidWidth =
                cls.glconfig.vidWidth -
                2 * (consoleState.margin.sides + consoleState.padding.sides);
    ...
    } else if (textWidthInChars < 1) // video hasn't been initialized yet
    {
        ...
        consoleState.maxScrollbackLengthInLines =
                CON_TEXTSIZE / consoleState.textWidthInChars;
        Con_Clear();
        ...
        if (oldtotallines > 0 && oldwidth > 0 &&
            consoleState.usedScrollbackLengthInLines > 0) {
            numlines =
                    std::min({consoleState.maxScrollbackLengthInLines,
            ...
            for (i = 0; i < numlines; i++) {
                conChar_t* destination =
                        consoleState.text +
                        (consoleState.maxScrollbackLengthInLines - 1 - i) *
                                consoleState.textWidthInChars;
                memcpy(destination,
                       buf +
                               ((consoleState.currentLine - i - 1 + oldtotallines) %
                                oldtotallines) *
                                       oldwidth,
    ...
    if (con_prompt) {
        ...
        g_console_field_width =
                consoleState.textWidthInChars - 9 - Q_UTF8_Strlen(prompt);
        g_consoleField.SetWidth(g_console_field_width);
    }
...
void Con_Linefeed() {
    ...
    if (consoleState.usedScrollbackLengthInLines <
        consoleState.maxScrollbackLengthInLines)
        consoleState.usedScrollbackLengthInLines++;
...
#pragma optimize( \
        "g", off) // SMF - msvc totally screws this function up with optimize on
#endif
...
bool CL_InternalConsolePrint(const char* text) {
    ...
    // NERVE - SMF - work around for text that shows up in console but not in
    // notify
    ...
    while ((c = *text & 0xFF) != 0) {
        if (Q_IsColorString(text)) {
            color = (text[1] == COLOR_NULL) ? ColorIndex(CONSOLE_COLOR)
                                            : ColorIndex(text[1]);
            text += 2;
            continue;
        }
        ...
        if (!wordLen) {
            ...
            // word wrap
            if (consoleState.horizontalCharOffset + wordLen >=
                        consoleState.textWidthInChars &&
                consoleState.horizontalCharOffset > 0) {
                Con_Linefeed();
            }
        ...
        switch (c) {
            ...
            default: // display character and advance
                ...
                consoleState.text[y * consoleState.textWidthInChars +
                                  consoleState.horizontalCharOffset].ch =
                        Q_UTF8_CodePoint(text);
                consoleState.text[y * consoleState.textWidthInChars +
                                  consoleState.horizontalCharOffset].ink = color;
...
void Con_DrawBackground() {
    ...
    const int consoleWidth =
            cls.glconfig.vidWidth - 2 * consoleState.margin.sides;
    ...
    if (con_margin->integer) {
        ...
        SCR_FillRect(consoleState.margin.sides - consoleState.border.sides,
                     ...
                     consoleWidth + consoleState.border.sides,
                     consoleState.border.top,
                     color);
        // left border
        SCR_FillRect(consoleState.margin.sides - consoleState.border.sides,
                     consoleState.margin.top,
                     consoleState.border.sides,
                     consoleState.height + consoleState.border.bottom,
                     color);

        // right border
        SCR_FillRect(cls.glconfig.vidWidth - consoleState.margin.sides,
                     consoleState.margin.top - consoleState.border.top,
                     consoleState.border.sides,
                     consoleState.border.top + consoleState.height,
                     color);

        // bottom border
        SCR_FillRect(consoleState.margin.sides,
                     consoleState.height + consoleState.margin.top +
                             consoleState.border.top - consoleState.border.bottom,
                     consoleWidth + consoleState.border.sides,
                     consoleState.border.bottom,
                     color);
    } else {
        // bottom border
...
void Con_DrawInput(int linePosition, float overrideAlpha) {
    ...
    SCR_DrawSmallStringExt(consoleState.margin.sides + consoleState.padding.sides,
                           linePosition,
                           prompt,
                           color,
                           false,
    ...
    Field_Draw(g_consoleField,
               consoleState.margin.sides + consoleState.padding.sides +
                       SCR_ConsoleFontStringWidth(prompt, strlen(prompt)),
               linePosition,
               true,
               true,
               color[3]);
...
void Con_DrawRightFloatingTextLine(const int linePosition, const float* color, const char* text) {
    ...
    const int positionFromTop = consoleState.margin.top +
                                consoleState.border.top +
                                consoleState.padding.top + charHeight;
    ...
    for (x = 0; x < i; x++) {
        ...
        SCR_DrawConsoleFontUnichar(currentWidthLocation,
                                   positionFromTop + (linePosition * charHeight),
                                   ch);
...
void Con_DrawAboutText() {
    ...
    color[1] = 1.0f;
    color[2] = 1.0f;
    // ANIMATION_TYPE_FADE but also ANIMATION_TYPE_SCROLL_DOWN needs this, latter,
    // since it might otherwise scroll out the console
...
void Con_DrawConsoleScrollbackIndicator(int lineDrawPosition) {
    ...
    for (i = 0; i < consoleState.textWidthInChars; i += 4) {
        SCR_DrawConsoleFontUnichar(consoleState.margin.sides +
                                           consoleState.padding.sides +
                                           (i + 1.5) * hatWidth,
                                   lineDrawPosition,
                                   '^');
    }
...
void Con_DrawConsoleScrollbar() {
    vec4_t color;
    const int freeConsoleHeight = consoleState.height - consoleState.padding.top -
                                  consoleState.padding.bottom;
    const float scrollBarX = cls.glconfig.vidWidth - consoleState.margin.sides -
                             consoleState.padding.sides -
                             2 * consoleState.border.sides;
    const float scrollBarY = consoleState.margin.top + consoleState.border.top +
                             consoleState.padding.top + freeConsoleHeight * 0.10f;
    const float scrollBarLength = freeConsoleHeight * 0.80f;
    const float scrollBarWidth = consoleState.border.sides * 2;

    const float scrollHandleLength =
            consoleState.usedScrollbackLengthInLines
                    ? scrollBarLength *
                              std::min(1.0f, (float) consoleState.visibleAmountOfLines / consoleState.usedScrollbackLengthInLines)
                    : 0;

    const float scrollBarLengthPerLine =
            (scrollBarLength - scrollHandleLength) /
            (consoleState.usedScrollbackLengthInLines -
             consoleState.visibleAmountOfLines);
    // that may result in -NaN

    const float relativeScrollLineIndex =
            consoleState.currentLine - consoleState.usedScrollbackLengthInLines +
            std::min(consoleState.visibleAmountOfLines,
                     consoleState.usedScrollbackLengthInLines);

    const float scrollHandlePostition =
            (scrollBarLengthPerLine == scrollBarLengthPerLine)
                    ? scrollBarLengthPerLine *
                              (consoleState.bottomDisplayedLine - relativeScrollLineIndex)
    ...
    } else if (consoleState.usedScrollbackLengthInLines) // this happens when
                                                         // line appending gets
                                                         // us over the top
                                                         // position in a
                                                         // roll-lock situation
    ...
    if (con_debug->integer) {
        Con_DrawRightFloatingTextLine(
                6, nullptr, va("Scrollbar (px): Size %d HandleSize %d Position %d", (int) scrollBarLength, (int) scrollHandleLength, (int) scrollHandlePostition));
    }
}
...
void Con_DrawScrollbackMarkerline(int lineDrawPosition) {
    ...
    const int scrollBarImpliedPadding =
            2 * consoleState.padding.sides + 2 * consoleState.border.sides;
    ...
    SCR_FillRect(consoleState.margin.sides + consoleState.border.sides +
                         consoleState.padding.sides / 2,
                 lineDrawPosition + SCR_ConsoleFontCharVPadding(),
                 cls.glconfig.vidWidth - 2 * consoleState.margin.sides -
                         2 * consoleState.border.sides -
                         consoleState.padding.sides / 2 - scrollBarImpliedPadding,
                 1,
                 color);
}
...
static float Con_MarginFadeAlpha(float alpha, float lineDrawPosition, int topMargin, int bottomMargin, int charHeight) {
    if (lineDrawPosition > bottomMargin &&
        lineDrawPosition <= bottomMargin + charHeight) {
        return alpha * (bottomMargin + charHeight - lineDrawPosition) /
               (float) charHeight;
    }

    if (lineDrawPosition < topMargin ||
        lineDrawPosition >= topMargin + charHeight) {
        return alpha;
    }
...
void Con_DrawConsoleContent() {
    ...
    const int textDistanceToTop = consoleState.margin.top +
                                  ...
                                  consoleState.border.top - charPadding - 1;

    // draw from the bottom up
    lineDrawPosition = consoleState.height + consoleState.margin.top -
                       consoleState.padding.bottom - consoleState.border.top -
                       charPadding - 1;
    ...
    Con_DrawInput(lineDrawPosition,
                  Con_MarginFadeAlpha(1, lineDrawPosition, textDistanceToTop, lineDrawPosition, charHeight));
    ...
    if (con_debug->integer) {
        Con_DrawRightFloatingTextLine(
                3, nullptr, va("Buffer (lines): ScrollbackLength %d/%d  CurrentIndex %d", consoleState.usedScrollbackLengthInLines, consoleState.maxScrollbackLengthInLines, consoleState.currentLine));
        Con_DrawRightFloatingTextLine(
                4, nullptr, va("Display (lines): From %d to %d (%d a %i px)", consoleState.currentLine - consoleState.maxScrollbackLengthInLines, consoleState.scrollLineIndex, consoleState.visibleAmountOfLines, charHeight));
    }

    /*
   * if we scrolled back, give feedback,
   * unless it's the last line (which will be rendered partly transparent
   * anyway)
   ...
   */
    ...
    if (consoleState.bottomDisplayedLine -
                floor(consoleState.bottomDisplayedLine) !=
        0.0f) {
        lineDrawPosition += charHeight -
    ...
    for (; row >= 0 && lineDrawPosition > textDistanceToTop;
         lineDrawPosition -= charHeight, row--) {
        conChar_t* text;

        if (consoleState.currentLine - row >=
            consoleState.maxScrollbackLengthInLines) {
        ...
        if (row == consoleState.lastReadLineIndex - 1 &&
            ...
            consoleState.currentLine - consoleState.lastReadLineIndex <
                    consoleState.usedScrollbackLengthInLines) {
        ...
        for (x = 0; x < consoleState.textWidthInChars && text[x].ch; ++x) {
            ...
            color[3] = Con_MarginFadeAlpha(consoleState.currentAlphaFactor,
                                           lineDrawPosition,
                                           textDistanceToTop,
...
void Con_DrawAnimatedConsole() {
    ...
    contentClipping[0] =
            consoleState.margin.sides + consoleState.border.sides;          // x
    contentClipping[1] = consoleState.margin.top + consoleState.border.top; // y
    contentClipping[2] = cls.glconfig.vidWidth - consoleState.margin.sides -
                         consoleState.border.sides; // x-end
    contentClipping[3] = consoleState.margin.top + consoleState.border.top +
                         consoleState.height; // y-end
    ...
    if (con_debug->integer) {
        Con_DrawRightFloatingTextLine(
                8, nullptr, va("Animation: target %d current fraction %f alpha %f", (int) consoleState.isOpened, consoleState.currentAnimationFraction, consoleState.currentAlphaFactor));
    }
...
void Con_UpdateConsoleState() {
   ...
   * different widths for horizontal and vertical borders due to different
   * resolution-ratios
   ...
   * apply the fade animation if the type is set, otherwise remain completly
   * visible
   */
    consoleState.currentAlphaFactor =
            (con_animationType->integer & ANIMATION_TYPE_FADE)
                    ? consoleState.currentAnimationFraction
                    : 1.0f;

    /*
   * calculate current console height
   */
    consoleState.height = con_height->integer * 0.01 *
                          (cls.glconfig.vidHeight - consoleState.margin.top -
                           consoleState.margin.bottom - consoleState.border.top -
                           consoleState.border.bottom);
    ...
    consoleState.height -=
            (consoleState.height - totalVerticalPadding - charPadding) % charHeight;
    // ... and ensure that at least three lines are visible
    consoleState.height =
            std::max(3 * charHeight + totalVerticalPadding, consoleState.height);
    ...
    /*
   * calculate current amount of visible lines after we learned about the
   * current height
   */

    consoleState.visibleAmountOfLines =
            (consoleState.height - consoleState.padding.top -
             consoleState.padding.bottom) /
...
void Con_RunAnimatedConsole() {
    ...
    if (con_height->value < con_margin->value ||
        (consoleState.visibleAmountOfLines < 1 &&
...
void Con_DrawConsole() {
    // render console only if flag is set or is within an animation but also in
    // special disconnected states
...
runs each frame once independend wheter or not the console is going to be
rendered or not
==================
*/
...
void Con_RunConsole() {
    ...
    if (consoleState.isOpened < consoleState.currentAnimationFraction) {
        consoleState.currentAnimationFraction -=
                con_animationSpeed->value * cls.realFrametime * 0.001;

        if (consoleState.currentAnimationFraction <= 0 ||
            con_animationType->integer ==
                    ANIMATION_TYPE_NONE) { // we are closed, do some last onClose work
        ...
        if (consoleState.currentAnimationFraction > 1 ||
            con_animationType->integer == ANIMATION_TYPE_NONE) {
            consoleState.currentAnimationFraction = 1;
        }
    }

    if (consoleState.currentAnimationFraction > 0) {
        const float scrollDifference =
                std::max(0.5f, fabsf(consoleState.bottomDisplayedLine - consoleState.scrollLineIndex));
        if (consoleState.bottomDisplayedLine < consoleState.scrollLineIndex) {
            consoleState.bottomDisplayedLine += con_animationSpeed->value *
                                                cls.realFrametime * 0.005 *
                                                scrollDifference;
            if (consoleState.bottomDisplayedLine > consoleState.scrollLineIndex ||
                con_animationType->integer == ANIMATION_TYPE_NONE) {
                consoleState.bottomDisplayedLine = consoleState.scrollLineIndex;
            }
        } else if (consoleState.bottomDisplayedLine >
                   consoleState.scrollLineIndex) {
            consoleState.bottomDisplayedLine -= con_animationSpeed->value *
                                                cls.realFrametime * 0.005 *
                                                scrollDifference;
     ...
     * skipping it entirely would lead to retrospective/delayed scrolling on
     * opening
     */
        consoleState.bottomDisplayedLine = consoleState.scrollLineIndex;
    }
...
public:
    GraphicalTarget() { this->Register(Log::GRAPHICAL_CONSOLE); }
...
void Con_ScrollUp(int lines) {
    ...
    if (consoleState.scrollLineIndex <
        consoleState.currentLine - consoleState.usedScrollbackLengthInLines +
                std::min(consoleState.visibleAmountOfLines,
                         consoleState.usedScrollbackLengthInLines)) {
        Con_ScrollToTop();
...
void Con_ScrollToMarkerLine() {
    consoleState.scrollLineIndex =
            std::max(consoleState.lastReadLineIndex,
                     consoleState.currentLine -
                             consoleState.usedScrollbackLengthInLines) +
            std::min(consoleState.visibleAmountOfLines,
                     consoleState.usedScrollbackLengthInLines);
...
void Con_JumpUp() {
    if (consoleState.lastReadLineIndex &&
        consoleState.scrollLineIndex >
                consoleState.lastReadLineIndex +
                        std::min(consoleState.visibleAmountOfLines,
                                 consoleState.usedScrollbackLengthInLines)
        //&& consoleState.currentLine - consoleState.lastReadLineIndex >
        // consoleState.visibleAmountOfLines
...
void Con_Close() {
    ...
    // instant disappearance, if we need it for situations where this is not
    // called by the user
    consoleState.currentAnimationFraction = 0;
}
