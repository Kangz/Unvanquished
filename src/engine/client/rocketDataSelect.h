...
public:
    RocketDataSelect(const Rocket::Core::String& tag)
            : Rocket::Controls::ElementFormControlDataSelect(tag), selection(-2) {}
    ~RocketDataSelect() {}
    ...
    virtual void ProcessEvent(Rocket::Core::Event& event) {
        ...
        if (event.GetTargetElement() == owner && event == "show") {
            eventQueue.push(new RocketEvent_t(
                    this, Rocket::Core::String(1024, "setDataSelectValue %s %s", dsName.CString(), tableName.CString())));
        }
    }
    ...
    void OnUpdate() {
        ...
        if (GetSelection() != selection) {
            ...
            eventQueue.push(new RocketEvent_t(
                    Rocket::Core::String(va("setDS %s %s %d", dsName.CString(), tableName.CString(), selection))));
