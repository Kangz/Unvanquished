...
public:
    RocketKeyBinder(const Rocket::Core::String& tag)
            : Rocket::Core::Element(tag), dirty_key(false), waitingForKeypress(false),
              team(0), key(-1), cmd(""), mouse_x(0), mouse_y(0) {}
    ...
    OnAttributeChange(const Rocket::Core::AttributeNameList& changed_attributes) {
        ...
        if (changed_attributes.find("team") != changed_attributes.end()) {
            team = Key_GetTeam(
                    GetAttribute("team")->Get<Rocket::Core::String>().CString(),
                    "Rocket KeyBinder");
            dirty_key = true;
        }
    }
    ...
    void ProcessEvent(Rocket::Core::Event& event) {
        ...
        if (!waitingForKeypress && event == "mousedown" &&
            event.GetTargetElement() == this) {
        ...
        else if (waitingForKeypress && event == "mousedown" &&
                 event.GetTargetElement() == this) {
