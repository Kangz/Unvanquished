...
static void SCR_DrawUnichar(int x, int y, float size, int ch) {
    ...
    if (cls.useLegacyConsoleFace) {
        ...
        re.DrawStretchPic(ax, ay, aw, glyph->imageHeight, glyph->s, glyph->t, glyph->s2, glyph->t2, glyph->glyph);
    }
}
...
void SCR_DrawConsoleFontUnichar(float x, float y, int ch) {
    ...
    if (ch != ' ') {
        ...
        re.DrawStretchPic(x + xadj, y - yadj, glyph->imageWidth, glyph->imageHeight, glyph->s, glyph->t, glyph->s2, glyph->t2, glyph->glyph);
    }
}
...
void SCR_DrawSmallUnichar(int x, int y, int ch) {
    ...
    if (ch < 0x100 || cls.useLegacyConsoleFont) {
        ...
        // adjust for baseline
        re.DrawStretchPic(x,
                          y - (int) (SMALLCHAR_HEIGHT / (CONSOLE_FONT_VPADDING + 1)),
                          SMALLCHAR_WIDTH,
                          SMALLCHAR_HEIGHT,
                          fcol,
                          frow,
        ...
        re.DrawStretchPic(x, y, SMALLCHAR_WIDTH, glyph->imageHeight, glyph->s, glyph->t, glyph->s2, glyph->t2, glyph->glyph);
    }
}
...
#if 0 // FIXME we need to find another way to get the team CS_PLAYERS value,
       // which will continuously change independently of the client, especially
       // now with several cgames/games in development
	char string[ 256 ];
	char teamColor;
    ...
    if (cgvm.IsActive()) {
        switch (cls.state) {
            ...
            case CA_PRIMED:
                ...
                // if (!com_sv_running->value || Cvar_VariableIntegerValue("sv_cheats"))
                // // Ridah, don't draw useless text if not in dev mode
                break;
...
void SCR_UpdateScreen() {
    ...
    if (++recursive >= 2) {
        recursive = 0;
        // Gordon: i'm breaking this again, because we've removed most of our cases
        // but still have one which will not fix easily
        return;
    ...
    // If there is no VM, there are also no rendering commands issued. Stop the
    // renderer in
    // that case.
    if (cgvm.IsActive()) {
        SCR_DrawScreenField();
...
float SCR_ConsoleFontCharHeight() {
    return cls.useLegacyConsoleFont
                   ? SMALLCHAR_HEIGHT
                   : cls.consoleFont->glyphBlock[0][(unsigned) 'I'].imageHeight +
                             CONSOLE_FONT_VPADDING * cl_consoleFontSize->value;
}
...
float SCR_ConsoleFontCharVPadding() {
    return cls.useLegacyConsoleFont
                   ? 0
                   : std::max(0,
                              -cls.consoleFont->glyphBlock[0][(unsigned) 'g'].bottom >>
                                      6);
}
