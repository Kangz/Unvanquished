...
public:
    RocketConditionalElement(const Rocket::Core::String& tag)
            : Rocket::Core::Element(tag), condition(NOT_EQUAL), dirty_value(false) {
    ...
    OnAttributeChange(const Rocket::Core::AttributeNameList& changed_attributes) {
        ...
        if (changed_attributes.find("value") != changed_attributes.end()) {
            Rocket::Core::String attrib =
                    GetAttribute<Rocket::Core::String>("value", "");
            char* end = nullptr;
            // Check if float
...
#define Compare(one, two)      \
    ...
    bool IsConditionValid() {
        switch (value.GetType()) {
            ...
            default:
                Compare(Q_stricmp(Cvar_VariableString(cvar.CString()),
                                  value.Get<Rocket::Core::String>().CString()),
                        0);
        }
