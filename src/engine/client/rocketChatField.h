...
class RocketChatField : public Rocket::Core::Element,
                        Rocket::Core::EventListener {
public:
    RocketChatField(const Rocket::Core::String& tag)
            : Rocket::Core::Element(tag), cursor_timer(0), last_update_time(0),
              focus(false), cursor_character_index(0), text_element(nullptr) {
        // Spawn text container
        text_element = Rocket::Core::Factory::InstanceElement(
        ...
        if (focus) {
            ...
            // Handle key presses
                ...
                if (event.GetParameter<int>("ctrl_key", 0) == 0 &&
                    ...
                    Rocket::Core::word character =
                            event.GetParameter<Rocket::Core::word>("data", 0);
...
protected:
    void GenerateCursor() {
        ...
        cursor_size.x = 1;
        cursor_size.y =
                (float) Rocket::Core::ElementUtilities::GetLineHeight(text_element) + 2;
        Rocket::Core::GeometryUtilities::GenerateQuad(
                &vertices[0], &indices[0], Rocket::Core::Vector2f(0, 0), cursor_size, GetProperty<Rocket::Core::Colourb>("color"));
    }

    void MoveCursor(int amt) {
        cursor_character_index += amt;

        cursor_character_index = Rocket::Core::Math::Clamp<int>(
                cursor_character_index, 0, text.Length());
    }
    ...
    void UpdateCursorPosition() {
        ...
        cursor_position.x += (float) Rocket::Core::ElementUtilities::GetStringWidth(
                text_element, text.Substring(0, cursor_character_index));
    }
    ...
    void q2rml(const char* in, Rocket::Core::Element* parent) {
        ...
        for (p = in; p && *p; ++p) {
            if (*p == '<') {
                ...
                parent->AppendChild(
                        (child = Rocket::Core::Factory::InstanceElement(
                                 parent, "*", "br", Rocket::Core::XMLAttributes())));
                child->RemoveReference();
                out.Clear();
            }
            ...
            // Convert ^^ to ^
                ...
                child = Rocket::Core::Factory::InstanceElement(parent, "#text", "span", xml);
                child->SetProperty("color", va("#%02X%02X%02X", (int) (g_color_table[code][0] * 255), (int) (g_color_table[code][1] * 255), (int) (g_color_table[code][2] * 255)));
                out.Append(va("^%c", c));
                span = true;
            }
        ...
        if (span && child && !out.Empty()) {
            ...
            child = Rocket::Core::Factory::InstanceElement(
                    parent, "#text", "span", Rocket::Core::XMLAttributes());
            static_cast<Rocket::Core::ElementText*>(child)->SetText(out);
            parent->AppendChild(child);
            child->RemoveReference();
        }
