...
void CL_SystemInfoChanged() {
    ...
    // NOTE TTimo:
    // when the serverId changes, any further messages we send to the server will
    // use this new serverId
    // show_bug.cgi?id=475
    ...
    if (!com_sv_running->integer) {
        ...
        if (!FS_LoadServerPaks(Info_ValueForKey(systemInfo, "sv_paks"),
            ...
            if (!cl_allowDownload->integer) {
                Com_Error(ERR_DROP,
                          "Client is missing paks but downloads are disabled");
    ...
    // scan through all the variables in the systeminfo and locally set cvars to
    // match
    s = systemInfo;
...
void CL_ParseGamestate(msg_t* msg) {
    ...
    // This used to call CL_StartHunkUsers, but now we enter the download state
    // before loading the
    // cgame
    CL_InitDownloads();
...
void CL_ParseDownload(msg_t* msg) {
    ...
    if (block == -1) {
        if (!clc.bWWWDl) {
            ...
            // make sure the server is not trying to redirect us again on a bad
            // checksum
            ...
            if (!DL_BeginDownload(cls.downloadTempName, cls.downloadName, com_developer->integer)) {
                ...
                // not sure why, but I suspect we have to eat all remaining block -1
                // that the server has sent us
    ...
    if (!size) {
        ...
        *cls.downloadTempName = *cls.downloadName = 0;
        ...
        // send intentions now
        // We need this because without it, we would hold the last nextdl and then
        // start
        // loading right away.  If we take a while to load, the server is happily
        // trying
...
static void CL_ParseVoip(msg_t* msg) {
    ...
    } else if (seqdiff < 0) // we're ahead of the sequence?!
    {
        ...
        speex_bits_reset(&clc.speexDecoderBits[sender]);
        seqdiff = 0;
    } else if (seqdiff * clc.speexFrameSize * 2 >=
               decodedSize) // more than 2 seconds of audio dropped?
    {
        // just start over.
