...
/*
  ...
  This is a decoder for OGM, a "better" (smaller files, higher resolutions)
cinematic format than ROQ

  In this code "ogm" is only: ogg wrapper, vorbis audio, theora video
  (ogm(Ogg Media) in general is ogg wrapper with all kind of
 ...
 * VLC ogg demux (
http://trac.videolan.org/vlc/browser/trunk/modules/demux/ogg.c )
theora:
...
typedef struct {
    ...
    // ogg_stream_state os; /* take physical pages, weld into a logical stream of
    // packets */
    ...
    vorbis_info
            vi;        /* struct that stores all the static vorbis bitstream settings */
...
static bool loadAudio() {
    ...
    while (anyDataTransferred &&
           g_ogm.currentTime + MAX_AUDIO_PRELOAD >
                   (int) (g_ogm.vd.granulepos * 1000 / g_ogm.vi.rate)) {
        ...
        if ((samples = vorbis_synthesis_pcmout(&g_ogm.vd, &pcm)) > 0) {
            ...
            for (i = 0; i < samplesNeeded; ++i) {
                ptr[0] = (left[i] >= -1.0f && left[i] <= 1.0f)
                                 ? left[i] * 32767.f
                                 : 32767 * ((left[i] > 0.0f) - (left[i] < 0.0f));
                ptr[1] = (right[i] >= -1.0f && right[i] <= 1.0f)
                                 ? right[i] * 32767.f
    ...
    if (g_ogm.currentTime + MIN_AUDIO_PRELOAD >
        (int) (g_ogm.vd.granulepos * 1000 / g_ogm.vi.rate)) {
        return true;
    } else {
        return false;
...
/*
  ...
  return: 1 -> loaded a new Frame ( g_ogm.outputBuffer points to the actual
  frame )
                        0 -> no new Frame
                        <0  -> error
*/
...
static int loadVideoFrameTheora() {
    ...
    while (!r && (ogg_stream_packetout(&g_ogm.os_video, &op))) {
        ...
        if ((g_ogm.VFrameCount < th_frame && th_frame >= nextNeededVFrame()) ||
            !g_ogm.outputBuffer) {
            //          int i,j;
            ...
            if (g_ogm.outputWidht != g_ogm.th_info.width ||
            ...
            if (g_ogm.outputBufferSize < g_ogm.th_info.width * g_ogm.th_info.height) {
                ...
                g_ogm.outputBuffer =
                        (unsigned char*) malloc(g_ogm.outputBufferSize * 4);
            ...
            uvWShift =
                    findSizeShift(g_ogm.th_yuvbuffer.uv_width, g_ogm.th_info.width);
            yHShift =
                    findSizeShift(g_ogm.th_yuvbuffer.y_height, g_ogm.th_info.height);
            uvHShift =
                    findSizeShift(g_ogm.th_yuvbuffer.uv_height, g_ogm.th_info.height);
            ...
            if (yWShift < 0 || uvWShift < 0 || yHShift < 0 || uvHShift < 0) {
                ...
                Frame_yuv_to_rgb24(
                        g_ogm.th_yuvbuffer.y, g_ogm.th_yuvbuffer.u, g_ogm.th_yuvbuffer.v, g_ogm.th_info.width, g_ogm.th_info.height, g_ogm.th_yuvbuffer.y_stride, g_ogm.th_yuvbuffer.uv_stride, yWShift, uvWShift, yHShift, uvHShift, (unsigned int*) g_ogm.outputBuffer);

                /*        unsigned char*  pixelPtr = g_ogm.outputBuffer;
                                        unsigned int* pixPtr;
                                        pixPtr = (unsigned
int*)g_ogm.outputBuffer;

                                        //TODO: use one yuv->rgb function for
the whole frame (large amount of stack movement (yuv->rgb calls) can't be good
;))
...
grayscale-output ^^
                                                        pixelPtr[0] =
                                                                pixelPtr[1] =
                                                                pixelPtr[2] =
g_ogm.th_yuvbuffer.y[i+j*g_ogm.th_yuvbuffer.y_stride];
                                                        pixelPtr+=4;
...
static int loadVideoFrame() {
    ...
    // if we come to this point, there will be no codec that use the stream
    // content ...
        ...
        //      if needPacket
        {
            ...
            if (needVOutputData && (status = loadVideoFrame())) {
                ...
                if (status > 0) {
                    ...
                    anyDataTransferred = false; // error (we don't need any videodata and
                                                // we had no transferred)
                }
            }
            ...
            if (g_ogm.VFrameCount > 1) // wait some videoframes (it's better to have
                                       // some delay, than laggy sound)
            {
...
typedef struct {
    ...
    /* in reference time */ // in 10^-7 seconds (dT between frames)
...
int Cin_OGM_Init(const char* filename) {
    ...
    if (g_ogm.ogmFile) {
        Com_Printf(S_WARNING
                   "there is already an OGM running, which will be stopped before "
                   "starting %s\n",
                   filename);
        Cin_OGM_Shutdown();
    }
    ...
    // TODO: support for more than one audio stream? / detect files with one
    // stream(or without correct ones)
        ...
        if (ogg_sync_pageout(&g_ogm.oy, &og) == 1) {
            if (strstr((char*) (og.body + 1), "vorbis")) {
                ...
                if (g_ogm.os_audio.serialno) {
                    Com_Printf(S_WARNING
                               "more than one audio stream in OGM file(%s). We will stay "
                               "at the first one\n",
                               filename);
                } else {
            ...
            if (strstr((char*) (og.body + 1), "theora")) {
                if (g_ogm.os_video.serialno) {
                    Com_Printf(S_WARNING
                               "more than one video stream in OGM file(%s). We will stay "
                               "at the first one\n",
                               filename);
                } else {
    ...
    while (i < 3) {
        ...
        if (status < 0) {
            Com_Printf(S_WARNING
                       "Corrupt Ogg packet while loading Vorbis headers (%s)\n",
                       filename);
            return -8;
        }
        ...
        if (status > 0) {
            ...
            if (i == 0 && status < 0) {
                Com_Printf(
                        S_WARNING
                        "This Ogg bitstream does not contain Vorbis audio data (%s)\n",
                        filename);
                return -9;
            }
    ...
    if (g_ogm.videoStreamIsTheora) {
        ...
        while (i < 3) {
            ...
            if (status < 0) {
                Com_Printf(S_WARNING
                           "Corrupt Ogg packet while loading Theora headers (%s)\n",
                           filename);
                return -8;
            }
            ...
            if (status > 0) {
                ...
                if (i == 0 && status != 0) {
                    Com_Printf(S_WARNING
                               "This Ogg bitstream does not contain Theora data (%s)\n",
                               filename);
                    return -9;
                }
        ...
        if (!isPowerOf2(g_ogm.th_info.width)) {
            Com_Printf(S_WARNING
                       "Video width of the OGM file isn't a power of 2 (%s)\n",
                       filename);
            return -5;
        }
        ...
        if (!isPowerOf2(g_ogm.th_info.height)) {
            Com_Printf(S_WARNING
                       "Video height of the OGM file isn't a power of 2 (%s)\n",
                       filename);
            return -6;
        }

        g_ogm.Vtime_unit = ((ogg_int64_t) g_ogm.th_info.fps_denominator * 1000 *
                            10000 / g_ogm.th_info.fps_numerator);
    }
...
int Cin_OGM_Run(int time) {
    ...
    while (!g_ogm.VFrameCount ||
           time + 20 >= (int) (g_ogm.VFrameCount * g_ogm.Vtime_unit / 10000)) {
        if (loadFrame()) {
            return 1;
        }
    }
...
void Cin_OGM_Shutdown() {
    ...
    vorbis_info_clear(
            &g_ogm.vi); /* must be called last (comment from vorbis example code) */
