...
cvar_t* cl_shownet = nullptr; // NERVE - SMF - This is referenced in msg.c and
                              // we need to make sure it is nullptr
...
cvar_t* p_team; /*<team id without team semantics (to not break the relationship
                   between client and cgame)*/
    ...
    if ((*idstr >= '0') && (*idstr <= '9')) {
        ...
        if ((id >= 0) && (id < MAX_CLIENTS)) {
            ...
            CL_AddReliableCommand(
                    va("voip %s %d", ignore ? "ignore" : "unignore", id));
            Com_Printf(ignore ? "VoIP: Now ignoring player #%d\n"
                              : "VoIP: No longer ignoring player #%d\n",
                       id);
            return;
        }
    }
...
static void CL_VoipNewGeneration() {
    ...
    while (target) {
        ...
        if (isdigit(*target)) {
            ...
            if (!Q_strnicmp(target, "all", 3)) {
                ...
                const char* players =
                        (char*) VM_ExplicitArgPtr(cgvm, VM_Call(cgvm, CG_VOIP_STRING, 0));
                const char* head;
                char* p;
...
static void CL_CaptureVoip() {
    ...
    if (cl_voip->modified || cl_rate->modified) {
        if ((cl_voip->integer) && (Cvar_VariableIntegerValue("rate") < 25000)) {
            Com_Printf(S_COLOR_YELLOW "%s",
                       ...
                       "Set 'Data Rate' to 'LAN/Cable/xDSL' in "
                       "'Setup/System/Network' and restart.\n"
    ...
    if ((cl_voipSend->integer) || (finalFrame)) // user wants to capture audio?
    {
        ...
        if (samples >= (clc.speexFrameSize * mult)) {
            ...
            Audio::GetCapturedData(samples,
                                   (byte*) sampbuffer); // grab from audio card.
...
void CL_Record_f() {
    ...
    // sync 0 doesn't prevent recording, so not forcing it off .. everyone does
    ...
    if (Cmd_Argc() == 2) {
        ...
        Com_sprintf(name, sizeof(name), "demos/%s-%s_%04i-%02i-%02i_%02i%02i%02i.dm_%d", NET_AdrToString(clc.serverAddress), period, 1900 + time.tm_year, time.tm_mon + 1, time.tm_mday, time.tm_hour, time.tm_min, time.tm_sec, PROTOCOL_VERSION);
    }
...
void CL_WriteWaveClose() {
    ...
    hdr.Subchunk2Size =
            hdr.NumSamples * hdr.NumChannels * (hdr.BitsPerSample / 8);
...
public:
    DemoCmd()
            : Cmd::StaticCmd("demo", Cmd::SYSTEM, "starts playing a demo file") {}
    ...
    void Run(const Cmd::Args& args) const OVERRIDE {
        ...
        //  CL_FlushMemory();   //----(SA)  MEM NOTE: in missionpack, this is moved
        //  to CL_DownloadsComplete
    ...
    Cmd::CompletionResult Complete(int argNum, const Cmd::Args& args, Str::StringRef prefix) const OVERRIDE {
        if (argNum == 1) {
            return FS::HomePath::CompleteFilename(
                    prefix, "demos", ".dm_" XSTRING(PROTOCOL_VERSION), false, true);
        }
...
void CL_ShutdownAll() {
    ...
    // Gordon: stop recording on map change etc, demos aren't valid over map
    // changes anyway
    if (clc.demorecording) {
        CL_StopRecord_f();
    }
...
Called by CL_MapLoading, CL_Connect_f, CL_PlayDemo_f, and CL_ParseGamestate the
only
...
void CL_Disconnect(bool showMainMenu) {
    ...
    // show_bug.cgi?id=589
    // don't try a restart if rocket is nullptr, as we might be in the middle of a
    // restart already
...
void CL_Connect_f() {
    ...
    // Copy the arguments before they can be overwritten, afater that server is
    // invalid
    ...
    // we need to setup a correct default for this, otherwise the first val we set
    // might reappear
...
static void CL_LoadRSAKeys() {
    ...
    Com_Printf("^5Loading RSA keys from %s\n", fileName);

    len = FS_FOpenFileRead(fileName, &f, true);

    if (!f || len < 1) {
        Com_Printf("^2%s", "Daemon RSA public-key file not found, regenerating\n");
    ...
    Z_Free(buf);
    Com_Printf("%s", "Daemon RSA public-key found.\n");
}
...
void CL_DownloadsComplete() {
    ...
    if (cls.downloadRestart) {
        ...
        FS::PakPath::ClearPaks();
        FS_LoadServerPaks(Cvar_VariableString("sv_paks"),
                          clc.demoplaying); // We possibly downloaded a pak,
...
void CL_InitDownloads() {
    ...
    // whatever autodownload configuration, store missing files in a cvar, use
    // later in the ui maybe
    ...
    if (cl_allowDownload->integer &&
        FS_ComparePaks(clc.downloadList, sizeof(clc.downloadList), true)) {
...
void CL_DisconnectPacket(netadr_t from) {
    ...
    // if we have received packets within three seconds, ignore (it might be a
    // malicious spoof)
    // NOTE TTimo:
    // there used to be a  clc.lastPacketTime = cls.realtime; line in
    // CL_PacketEvent before calling CL_ConnectionLessPacket
    // therefore .. packets never got through this check, clients never
    // disconnected
    // switched the clc.lastPacketTime = cls.realtime to happen after the
    // connectionless packets have been processed
    // you still can't spoof disconnects, cause legal netchan packets will
    // maintain realtime - lastPacketTime below the threshold
    if (cls.realtime - clc.lastPacketTime < 3000) {
        return;
    }

    // if we are doing a disconnected download, leave the 'connecting' screen on
    // with the progress information
    if (!cls.bWWWDlDisconnected) {
        // drop the connection
...
void CL_ServerLinksResponsePacket(const netadr_t* from, msg_t* msg) {
    ...
    while (buffptr < buffend - 20 &&
           cls.numserverLinks < ARRAY_LEN(cls.serverLinks)) {
...
void CL_ServersResponsePacket(const netadr_t* from, msg_t* msg, bool extended) {
    ...
    while (buffptr + 1 < buffend) {
        ...
        if (*buffptr == '\\') {
            ...
            if (buffend - buffptr < sizeof(addresses[numservers].ip) +
            ...
            for (j = 0; j < cls.numserverLinks && !duplicate; ++j) {
                if (addresses[numservers].port == cls.serverLinks[j].port4 &&
                    !memcmp(addresses[numservers].ip, cls.serverLinks[j].ip, 4)) {
                    // found it, so look up the corresponding address
                    char s[NET_ADDR_W_PORT_STR_MAX_LEN];

                    // hax to get the IP address & port as a string (memcmp etc. SHOULD
                    // work, but...)
                    ...
                    for (i = 0; i < numservers; ++i) {
                        if (!strcmp(s, NET_AdrToStringwPort(addresses[i]))) {
                            ...
                            addresses[i].port = (addresses[i].type == NA_IP)
                                                        ? cls.serverLinks[j].port4
                                                        : cls.serverLinks[j].port6;
                            duplicate = true;
                            break;
                        }
                    }
        ...
        // IPv6 address, if it's an extended response
            ...
            if (buffend - buffptr < sizeof(addresses[numservers].ip6) +
                                            sizeof(addresses[numservers].port) + 1) {
                break;
            }
            ...
            // look up this address in the links list
            for (j = 0; j < cls.numserverLinks && !duplicate; ++j) {
                if (addresses[numservers].port == cls.serverLinks[j].port6 &&
                    !memcmp(addresses[numservers].ip6, cls.serverLinks[j].ip6, 16)) {
                    // found it, so look up the corresponding address
                    char s[NET_ADDR_W_PORT_STR_MAX_LEN];
    ...
    if (count >= MAX_GLOBAL_SERVERS &&
        ...
        for (; i < numservers && cls.numGlobalServerAddresses < MAX_GLOBAL_SERVERS;
             i++) {
...
void CL_ConnectionlessPacket(netadr_t from, msg_t* msg) {
    ...
    if (args.Argv(0) == "connectResponse") {
        ...
        if (!NET_CompareAdr(from, clc.serverAddress)) {
            ...
            Com_Printf("%s should have been %s\n", NET_AdrToString(from), NET_AdrToStringwPort(clc.serverAddress));
            return;
        }
    ...
    // list of servers with both IPv4 and IPv6 addresses; sent back by a master
    // server (extended)
...
void CL_CheckUserinfo() {
    ...
    if (cvar_modifiedFlags & CVAR_USERINFO) {
        cvar_modifiedFlags &= ~CVAR_USERINFO;
        CL_AddReliableCommand(va(
                "userinfo %s", Cmd_QuoteString(Cvar_InfoString(CVAR_USERINFO, false))));
    }
}
    ...
    if (ret == DL_DONE) {
        ...
        *cls.downloadTempName = *cls.downloadName = 0;
        ...
        if (!cls.bWWWDlDisconnected) {
            ...
            // tracking potential web redirects leading us to wrong checksum - only
        ...
        if (cls.bWWWDlDisconnected) {
            // in a connected download, we'd tell the server about failure and wait
            // for a reply
            // but in this case we can't get anything from server
            // if we just reconnect it's likely we'll get the same disconnected
            // download message, and error out again
            // this may happen for a regular dl or an auto update
            const char* error =
                    va("Download failure while getting '%s'\n",
                       cls.downloadName); // get the msg before clearing structs

            cls.bWWWDlDisconnected = false; // need clearing structs before ERR_DROP,
                                            // or it goes into endless reload
            CL_ClearStaticDownload();
...
this indicates that the redirect setup is broken, and next dl attempt should NOT
redirect
==================
*/
bool CL_WWWBadChecksum(const char* pakname) {
    if (strstr(clc.redirectedList, va("@%s@", pakname))) {
        ...
        if (strlen(clc.badChecksumList) + strlen(pakname) + 1 >=
...
void CL_Frame(int msec) {
    ...
    if (CL_VideoRecording() && cl_aviFrameRate->integer && msec) {
        ...
        if (cls.state == CA_ACTIVE || cl_forceavidemo->integer) {
            ...
            msec =
                    (int) ceil((1000.0f / cl_aviFrameRate->value) * com_timescale->value);
...
bool CL_InitRenderer() {
    ...
    // filehandle is unused but forces FS_FOpenFileRead() to heed purecheck
    // because it does not when filehandle is nullptr
...
void CL_Init() {
    ...
    cl_xbox360ControllerAvailable =
            Cvar_Get("in_xbox360ControllerAvailable", "0", CVAR_ROM);
    ...
    cl_doubletapdelay =
            Cvar_Get("cl_doubletapdelay", "250", 0); // Arnout: double tap
    ...
    cl_voipGainDuringCapture =
            Cvar_Get("cl_voipGainDuringCapture", "0.2", CVAR_ARCHIVE);
...
static void CL_SetServerInfo(serverInfo_t* server, const char* info, int ping) {
    if (server) {
        if (info) {
            ...
            server->friendlyFire =
                    atoi(Info_ValueForKey(info, "g_friendlyFire")); // NERVE - SMF
            server->needpass =
                    atoi(Info_ValueForKey(info, "g_needpass")); // NERVE - SMF
            Q_strncpyz(server->gameName, Info_ValueForKey(info, "gamename"),
                       MAX_NAME_LENGTH); // Arnout
        }
...
void CL_ServerInfoPacket(netadr_t from, msg_t* msg) {
    ...
    for (i = 0; i < MAX_PINGREQUESTS; i++) {
        if (cl_pinglist[i].adr.port && !cl_pinglist[i].time &&
            ...
            // NOTE: make sure these types are in sync with the netnames strings in
            // the UI
            switch (from.type) {
                case NA_BROADCAST:
                case NA_IP:
...
int CL_ServerStatus(const char* serverAddress, char* serverStatusString, int maxLen) {
    ...
    // if no server status string then reset the server status request for this
    // address
...
void CL_GlobalServers_f() {
    ...
    int protocol = atoi(Cmd_Argv(2)); // Do this right away, otherwise weird
                                      // things happen when you use the ingame
                                      // "Get New Servers" button.

    if ((count = Cmd_Argc()) < 2 || (masterNum = atoi(Cmd_Argv(1))) < 0 ||
        masterNum > MAX_MASTER_SERVERS - 1) {
        Cmd_PrintUsage("<master# 0-" XSTRING(MAX_MASTER_SERVERS -
                                             1) "> [<protocol>] [<keywords>]",
                       nullptr);
        return;
    }
    ...
    Com_sprintf(command, sizeof(command), "getserversExt %s %d dual", cl_gamename->string, protocol);
