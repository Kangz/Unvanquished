...
public:
    SelectableDataGrid(const Rocket::Core::String& tag)
            : Rocket::Controls::ElementDataGrid(tag), lastSelectedRow(nullptr),
    ...
    void ProcessEvent(Rocket::Core::Event& evt) {
        ...
        Rocket::Core::String dataSource =
                GetAttribute<Rocket::Core::String>("source", "");
        ...
        if (evt == "click" || evt == "dblclick") {
            ...
            Rocket::Core::String dsName =
                    dataSource.Substring(0, dataSource.Find("."));
            Rocket::Core::String tableName =
                    dataSource.Substring(dataSource.Find(".") + 1, dataSource.Length());

            // get the column index
            elem = evt.GetTargetElement();
            while (elem && elem->GetTagName() != "datagridcell" &&
                   elem->GetTagName() != "datagridcolumn") {
                elem = elem->GetParentNode();
            }
            if (elem) {
                // "datagridcolumn" points to just Element, so figure out the index by
                // iterating
                // FIXME: We could be little smarter with this and get the column
                // definition here too
                ...
                if (elem->GetTagName() == "datagridcolumn") {
                    ...
                    column = static_cast<Rocket::Controls::ElementDataGridCell*>(elem)
                                     ->GetColumn();
                }
            }
            ...
            while (elem && elem->GetTagName() != "datagridrow" &&
                   elem->GetTagName() != "datagridheader") {
                elem = elem->GetParentNode();
            }

            if (elem) {
                Rocket::Controls::ElementDataGridRow* row =
                        dynamic_cast<Rocket::Controls::ElementDataGridRow*>(elem);
                ...
                if (index >= 0) {
                    ...
                    eventQueue.push(new RocketEvent_t(
                            Rocket::Core::String(va("setDS %s %s %d", dsName.CString(), tableName.CString(), index))));
                }
            ...
            if (evt == "dblclick") {
                eventQueue.push(new RocketEvent_t(Rocket::Core::String(
                        va("execDS %s %s",
                           dataSource.Substring(0, dataSource.Find(".")).CString(),
                           tableName.CString()))));
            }
            ...
            if (lastSelectedRowIndex >= firstRowRemoved &&
                lastSelectedRowIndex < firstRowRemoved + numRowsRemoved) {
                lastSelectedRow->RemoveReference();
