...
void Rocket_ProcessKeyInput(int key, bool down) {
    ...
    if ((key == K_MWHEELDOWN || key == K_MWHEELUP)) {
        ...
        menuContext->ProcessMouseWheel(key == K_MWHEELDOWN ? MOUSEWHEEL_DELTA
                                                           : -MOUSEWHEEL_DELTA,
                                       Rocket_GetKeyModifiers());
        return;
    }
    if (down) {
        menuContext->ProcessKeyDown(Rocket_FromQuake(key),
                                    Rocket_GetKeyModifiers());
    } else {
