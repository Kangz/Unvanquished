...
/* IRC command status; used to determine if connection should be re-attempted or
 * not */
...
#define IRC_MakeEvent(type, isself) \
    (IRC_EVT_##type | ((isself) ? IRC_EVTF_SELF : 0))
...
==================
*/
static INLINE void IRC_AddHandler(const char* command,
                                  irc_handler_func_t handler) {
...
==================
*/
static void IRC_AddCTCPHandler(const char* command,
                               ctcp_handler_func_t handler) {
...
static INLINE bool IRC_CheckEventRate(int event_type) {
    if (IRC_RateLimiter[event_type] >=
        IRC_LIMIT_THRESHOLD * IRC_TIMEOUTS_PER_SEC) {
        return false;
    }
...
static int IRC_HandleMessage(bool is_channel, const char* string) {
    ...
    if (IRC_CheckEventRate(IRC_RL_MESSAGE)) {
        return IRC_Send(
                "PRIVMSG %s :Sorry, Daemon's IRC client does not support private "
                "messages\n",
...
static int IRCH_PrivMsg() {
    ...
    if (is_channel &&
        strcmp(&(IRC_String(arg_values[0])[1]), cl_IRC_channel->string)) {
        return IRC_CMD_SUCCESS;
    }

    if (IRC_Length(arg_values[1]) > 2 && IRC_String(arg_values[1])[0] == 1 &&
...
void CL_IRCSay() {
    ...
    if ((m_sendstring[0] == '/' || m_sendstring[0] == '.') &&
        !Q_strnicmp(m_sendstring + 1, "me ", 3) && m_sendstring[4] != 0) {
...
static int IRC_AttemptConnection() {
    ...
    // Attempt connection
    if ((connect(IRC_Socket, (struct sockaddr*) &address, sizeof(address))) !=
        0) {
        closesocket(IRC_Socket);
        Com_Printf("…IRC: %s\n", "connection refused");
        return IRC_CMD_RETRY;
...
static void IRC_StartThread() {
    if (IRC_ThreadHandle == nullptr) {
        IRC_ThreadHandle =
                CreateThread(nullptr, 0, IRC_SystemThreadProc, nullptr, 0, nullptr);
    }
}
