...
class RocketDataSourceSingle : public Rocket::Core::Element,
                               public Rocket::Controls::DataSourceListener,
                               public Rocket::Core::EventListener {
public:
    RocketDataSourceSingle(const Rocket::Core::String& tag)
            : Rocket::Core::Element(tag), formatter(nullptr), data_source(nullptr),
              selection(-1), targetElement(nullptr), dirty_query(false),
    ...
    OnAttributeChange(const Rocket::Core::AttributeNameList& changed_attributes) {
        ...
        if (changed_attributes.find("targetid") != changed_attributes.end() ||
            changed_attributes.find("targetdoc") != changed_attributes.end()) {
            dirty_listener = true;
        }
    }
        ...
        if (dirty_listener) {
            ...
            if (document) {
                ...
                if ((element = document->GetElementById(
                             GetAttribute<Rocket::Core::String>("targetid", "")))) {
