...
static void setupQuad(long xOff, long yOff) {
    ...
    numQuadCels =
            (cinTable[currentHandle].xsize * cinTable[currentHandle].ysize) / (16);
...
redump:
    ...
    switch (cinTable[currentHandle].roq_id) {
        ...
        case ZA_SOUND_MONO:
            if (!cinTable[currentHandle].silent) {
                ...
                ssize = RllDecodeMonoToStereo(
                        framedata, sbuf, cinTable[currentHandle].RoQFrameSize, 0, (unsigned short) cinTable[currentHandle].roq_flags);
        ...
        case ROQ_QUAD_INFO:
            if (cinTable[currentHandle].numQuads == -1) {
                ...
                setupQuad(0, 0);
                // we need to use CL_ScaledMilliseconds because of the smp mode calls
                // from the renderer
    ...
    cinTable[currentHandle].RoQFrameSize =
            framedata[2] + framedata[3] * 256 + framedata[4] * 65536;
...
static void RoQ_init() {
    // we need to use CL_ScaledMilliseconds because of the smp mode calls from the
    // renderer
    cinTable[currentHandle].startTime = cinTable[currentHandle].lastTime =
            CL_ScaledMilliseconds() * com_timescale->value;
    ...
    cinTable[currentHandle].RoQFrameSize =
            cin.file[10] + cin.file[11] * 256 + cin.file[12] * 65536;
...
// FIXME: this isn't really a "ROQ shutdown" function (it's more like a "CIN
// shutdown" function, beside the file closing)
static void RoQShutdown() {
    const char* s;

    if (!cinTable[currentHandle].buf) {
        // FIXME: there could be something that should be "shutdowned" even if we
        // don't have an output frame (at least in the ogm code)
        return;
    }
...
e_status CIN_RunCinematic(int handle) {
    ...
    if (cinTable[currentHandle].fileType == FT_OGM) {
        if (Cin_OGM_Run(cinTable[currentHandle].startTime == 0
                                ? 0
    ...
    // FIXME? CL_ScaledMilliseconds already uses com_timescale (so I can't see
    // that the com_timescale in here makes any sense at all O_o)
    // we need to use CL_ScaledMilliseconds because of the smp mode calls from the
    // renderer
    ...
    if (cinTable[currentHandle].shader &&
        ...
        cinTable[currentHandle].startTime +=
                thisTime - cinTable[currentHandle].lastTime;
    }
    ...
    while ((cinTable[currentHandle].tfps != cinTable[currentHandle].numQuads) &&
        ...
        if (start != cinTable[currentHandle].startTime) {
            // we need to use CL_ScaledMilliseconds because of the smp mode calls from
            // the renderer
            cinTable[currentHandle].tfps =
                    ((((CL_ScaledMilliseconds() * com_timescale->value) -
                       cinTable[currentHandle].startTime) *
...
int CIN_PlayCinematic(const char* arg, int x, int y, int w, int h, int systemBits) {
    ...
    */
    ...
    cinTable[currentHandle].ROQSize = FS_FOpenFileRead(
            ...
            for (iy = 0; iy < 256; iy++) {
                ...
                for (ix = 0; ix < 2048; ix += 8) {
                    for (ic = ix; ic < (ix + 4); ic++) {
                        *bc2 = (bc3[iiy + ic] + bc3[iiy + 4 + ic] + bc3[iiy + 2048 + ic] +
                                bc3[iiy + 2048 + 4 + ic]) >>
                               2;
                        bc2++;
                    }
