...
int DL_BeginDownload(const char* localName, const char* remoteName, int debug) {
    ...
    Q_strncpyz(referer + URI_SCHEME_LENGTH,
               Cvar_VariableString("cl_currentServerIP"),
               MAX_STRING_CHARS);

    dl_request = curl_easy_init();
    curl_easy_setopt(
            dl_request, CURLOPT_USERAGENT, va("%s %s", PRODUCT_NAME "/" PRODUCT_VERSION, curl_version()));
...
dlStatus_t DL_DownloadLoop() {
    ...
    if ((status = curl_multi_perform(dl_multi, &dls)) ==
    ...
    if (err) {
        Com_DPrintf(
                "DL_DownloadLoop: request terminated with failure status '%s'\n", err);
        return DL_FAILED;
    }
