...
class RocketCircleMenu : public Rocket::Core::Element,
                         public Rocket::Controls::DataSourceListener {
public:
    RocketCircleMenu(const Rocket::Core::String& tag)
            : Rocket::Core::Element(tag), dirty_query(false), dirty_layout(false),
              init(false), radius(10), formatter(nullptr), data_source(nullptr) {}
    ...
    bool GetIntrinsicDimensions(Rocket::Core::Vector2f& dimensions) {
        ...
        // Return the calculated dimensions. If this changes the size of the
        // element, it will result in
    ...
    void
    OnAttributeChange(const Rocket::Core::AttributeNameList& changed_attributes) {
        ...
        if (changed_attributes.find("formatter") != changed_attributes.end()) {
            formatter = Rocket::Controls::DataFormatter::GetDataFormatter(
                    GetAttribute("formatter")->Get<Rocket::Core::String>());
            dirty_query = true;
        }
    }

    void
    OnPropertyChange(const Rocket::Core::PropertyNameList& changed_properties) {
        Rocket::Core::Element::OnPropertyChange(changed_properties);
        if (changed_properties.find("width") != changed_properties.end() ||
            changed_properties.find("height") != changed_properties.end()) {
            dirty_layout = true;
        }
    ...
    void OnRowAdd(Rocket::Controls::DataSource* data_source,
    ...
    void OnRowChange(Rocket::Controls::DataSource* data_source,
                     const Rocket::Core::String& table,
    ...
    void OnRowRemove(Rocket::Controls::DataSource* data_source,
                     const Rocket::Core::String& table,
    ...
    virtual void ProcessEvent(Rocket::Core::Event& event) {
        ...
        if (event == "mouseover") {
            ...
            Rocket::Core::Element* button =
                    parent->GetTagName() == "button" ? parent : nullptr;
            while ((parent = parent->GetParentNode())) {
...
protected:
    void LayoutChildren() {
        ...
        child->SetProperty(
                "top", va("%fpx", offset.y + (dimensions.y / 2) - (height / 2)));
        child->SetProperty("left",
                           va("%fpx", offset.x + (dimensions.x / 2) - (width / 2)));
        ...
        for (int i = 1; i < numChildren; ++i) {
            ...
            child->SetProperty("position", "absolute");
            child->SetProperty(
                    "left", va("%fpx", (dimensions.x / 2) - (width / 2) + offset.x + x));
            child->SetProperty(
                    "top", va("%fpx", (dimensions.y / 2) - (height / 2) + offset.y + y));
        }
    }
...
private:
    void AddCancelbutton() {
        init = true;
        Rocket::Core::Factory::InstanceElementText(
                this, va("<button onClick=\"hide %s\">Cancel</button>", GetOwnerDocument()->GetId().CString()));
