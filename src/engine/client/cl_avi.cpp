...
bool CL_OpenAVIForWriting(const char* fileName) {
    ...
    if ((afd.idxF = FS_FOpenFileWrite(va("%s" INDEX_FILE_EXTENSION, fileName))) <=
        0) {
        FS_FCloseFile(afd.f);
        return false;
    }
...
#define MAX_PACK_LEN 16
    afd.cBuffer = (byte*) Z_Malloc(
            (afd.width * 3 + MAX_PACK_LEN - 1) * afd.height + MAX_PACK_LEN - 1);
    // raw avi files have pixel lines start on 4-byte boundaries
    afd.eBuffer =
            (byte*) Z_Malloc(PAD(afd.width * 3, AVI_LINE_PADDING) * afd.height);
  ...
  */
    ...
    if (afd.a.rate % afd.frameRate) {
        ...
        Com_Printf(
                S_WARNING
                "cl_aviFrameRate is not a divisor of the audio rate, suggest %d\n",
                suggestRate);
    }
    ...
    if (!Cvar_VariableIntegerValue("s_initsound")) {
        ...
        Com_Printf(S_WARNING
                   "Audio capture is not supported with OpenAL. Set s_useOpenAL to "
                   "0 for audio capture\n");
    }
...
void CL_WriteAVIAudioFrame(const byte* pcmBuffer, int size) {
    ...
    if (bytesInBuffer >=
        (int) ceil((float) afd.a.rate / (float) afd.frameRate) * afd.a.sampleSize) {
        int chunkOffset = afd.fileSize - afd.moviOffset - 8;
