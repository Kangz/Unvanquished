...
class RocketColorInput : public Rocket::Core::Element,
                         public Rocket::Core::EventListener {
public:
    RocketColorInput(const Rocket::Core::String& tag)
            : Rocket::Core::Element(tag) {
        // Initialize the input element
        Rocket::Core::XMLAttributes attribs;
        attribs.Set("type", "text");
        input =
                Rocket::Core::Factory::InstanceElement(this, "input", "input", attribs);
        color_value = Rocket::Core::Factory::InstanceElement(
    ...
    OnAttributeChange(const Rocket::Core::AttributeNameList& changed_attributes) {
        ...
        for (Rocket::Core::AttributeNameList::const_iterator it =
                     changed_attributes.begin();
             it != changed_attributes.end();
             ++it) {
...
private:
    void UpdateValue() {
        ...
        string += dynamic_cast<Rocket::Controls::ElementFormControlInput*>(input)
                          ->GetValue();

        Rocket::Core::Factory::InstanceElementText(
                color_value, Rocket_QuakeToRML(string.CString(), RP_QUAKE));
    }
