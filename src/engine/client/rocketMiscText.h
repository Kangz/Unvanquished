...
struct TextAndRect {
    TextAndRect(const char* _text, const char* _class, float _x, float _y)
            : text(_text), text_class(_class), x(_x), y(_y) {}
...
// A libRocket element to hold other misc text on the screen. It is assumed that
// it will change every frame

class RocketMiscText : public Rocket::Core::Element {
public:
    RocketMiscText(const Rocket::Core::String& tag)
            : Rocket::Core::Element(tag) {}

    static void ClearText() { strings.clear(); }
    ...
    void OnUpdate() {
        ...
        for (size_t i = 0; i < strings.size(); ++i) {
            Rocket::Core::Factory::InstanceElementText(
                    this, va("<div style='position: fixed; left: %.2f%%; top: %.2f%%;' "
                             "class='%s'>%s</div>",
                             strings[i].x * 100.0f,
                             strings[i].y * 100.0f,
                             strings[i].text_class.CString(),
                             strings[i].text.CString()));
        }
    }
