...
struct ConsoleLine {
    ...
    int time;
    ConsoleLine(Rocket::Core::String _i)
            : text(_i) { time = Sys_Milliseconds(); }
};

class RocketConsoleTextElement : public Rocket::Core::Element {
public:
    RocketConsoleTextElement(const Rocket::Core::String& tag)
            : Rocket::Core::Element(tag), numLines(0), maxLines(0), lastTime(-1),
              dirty_height(true) {}
    ...
    void OnUpdate() {
        ...
        while (HasChildNodes() &&
               atoi(GetFirstChild()->GetId().CString()) + latency < time) {
        ...
        if (!lines.empty() && lines.front().time > lastTime) {
            ...
            for (line = line - 1; line >= 0; --line, numLines++) {
                Rocket::Core::Element* child = Rocket::Core::Factory::InstanceElement(
                        this, "#text", "span", Rocket::Core::XMLAttributes());
                Rocket::Core::Factory::InstanceElementText(
                        child, Rocket_QuakeToRML(lines[line].text.CString(), RP_EMOTICONS));
        ...
        if (dirty_height && HasChildNodes()) {
            const Rocket::Core::FontFaceHandle* font =
                    GetFirstChild()->GetFontFaceHandle();
            if (font) {
                maxLines = floor(GetProperty("height")->value.Get<float>() /
                                 (font->GetBaseline() + font->GetLineHeight()));
