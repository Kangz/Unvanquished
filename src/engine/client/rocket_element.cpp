...
void Rocket_RegisterElement(const char* tag) {
    Rocket::Core::Factory::RegisterElementInstancer(
            tag, new Rocket::Core::ElementInstancerGeneric<RocketElement>())
            ->RemoveReference();
}
...
static inline void
Rocket_SetInnerRMLGuarded(Rocket::Core::Element* e,
                          const Rocket::Core::String& newRML) {
...
void Rocket_SetInnerRML(const char* name, const char* id, const char* RML, int parseFlags) {
    Rocket::Core::String newRML =
            parseFlags ? Rocket_QuakeToRML(RML, parseFlags) : RML;
...
void Rocket_SetAttribute(const char* name, const char* id, const char* attribute, const char* value) {
    if ((!*name && !*id) && activeElement) {
        ...
        Rocket::Core::ElementDocument* document =
                name[0] ? menuContext->GetDocument(name)
    ...
    if (activeElement) {
        ...
        switch (type) {
            ...
            case ROCKET_FLOAT: {
                ...
                // HACK: special case for width and height specified in non absolute
                // units

                if (!Q_stricmp("width", name) &&
                    property->unit & Rocket::Core::Property::RELATIVE_UNIT) {
                    float base_size = 0;
            ...
            case ROCKET_COLOR: {
                ...
                outColor[0] = color.red, outColor[1] = color.green,
                outColor[2] = color.blue, outColor[3] = color.alpha;
...
void Rocket_AddConsoleText(Str::StringRef text) {
    // HACK: Ugly way to force pre-engine-upgrade behavior. TODO: Make it work
    // without this hack
    ...
    RocketConsoleTextElement::lines.push_front(
            ConsoleLine(Rocket::Core::String(va("%s\n", buffer))));
}

void Rocket_RegisterProperty(const char* name, const char* defaultValue, bool inherited, bool force_layout, const char* parseAs) {
    Rocket::Core::StyleSheetSpecification::RegisterProperty(
            name, defaultValue, (bool) inherited, (bool) force_layout)
            .AddParser(parseAs);
}

void Rocket_SetDataSelectIndex(int index) {
    if (activeElement) {
        dynamic_cast<Rocket::Controls::ElementFormControlDataSelect*>(
                activeElement)->SetSelection(index);
    }
}
