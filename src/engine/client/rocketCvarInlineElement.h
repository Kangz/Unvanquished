...
public:
    RocketCvarInlineElement(const Rocket::Core::String& tag)
            : Rocket::Core::Element(tag), cvar(""), cvar_value(""),
              dirty_value(false) {}

    enum CvarType { NUMBER,
                    STRING };
    ...
    OnAttributeChange(const Rocket::Core::AttributeNameList& changed_attributes) {
        ...
        if (changed_attributes.find("type") != changed_attributes.end()) {
            Rocket::Core::String typeString =
                    GetAttribute<Rocket::Core::String>("type", "");
    ...
    virtual void OnUpdate() {
        if (dirty_value ||
            (!cvar.Empty() && cvar_value != Cvar_VariableString(cvar.CString()))) {
            Rocket::Core::String value = cvar_value =
                    Cvar_VariableString(cvar.CString());

            if (!format.Empty()) {
                if (type == NUMBER) {
                    value = Rocket::Core::String(cvar_value.Length() + format.Length(),
                                                 format.CString(),
                                                 Cvar_VariableValue(cvar.CString()));
                } else {
                    value = Rocket::Core::String(cvar_value.Length() + format.Length(),
                                                 format.CString(),
                                                 Cvar_VariableString(cvar.CString()));
