...
===================
*/
void Field_VariableSizeDraw(const Util::LineEditData& edit, int x, int y, int size, bool showCursor, bool noColorEscape, float alpha) {
    ...
    std::string text =
            Str::UTF32To8(std::u32string(edit.GetViewText(), drawWidth));
    ...
    if (showCursor) {
        ...
        if (size == SMALLCHAR_WIDTH) {
            ...
            height = key_overstrikeMode
                             ? SMALLCHAR_HEIGHT / (CONSOLE_FONT_VPADDING + 1)
                             : 2;
            width = SMALLCHAR_WIDTH;
        } else {
            xpos = x + cursorPos * size;
            height =
                    key_overstrikeMode ? BIGCHAR_HEIGHT / (CONSOLE_FONT_VPADDING + 1) : 2;
            width = BIGCHAR_WIDTH;
        }
...
Key events are used for non-printable characters, others are gotten from char
events.
=================
*/
...
void Console_Key(int key) {
    ...
    if (keys[K_ALT].down || keys[K_COMMAND].down || keys[K_MODE].down ||
        keys[K_SUPER].down) {
        return;
    }
    ...
    if (key == K_ENTER or key == K_KP_ENTER) {
        ...
        if (cls.state == CA_DISCONNECTED) {
            SCR_UpdateScreen(); // force an update, because the command may take some
                                // time
        }
        return;
    }
    ...
    if (key ==
        K_MWHEELUP) //----(SA) added some mousewheel functionality to the console
    {
        Con_PageUp();
...
int Key_GetTeam(const char* arg, const char* cmd) {
    static const struct {
        ...
        char label[11];
            ...
            {0, "spectators"}, {0, "default"}, {1, "aliens"}, {2, "humans"}};
    int t, l;
...
void Key_SetBinding(int keynum, int team, const char* binding) {
    ...
    if (binding && binding[0]) {
        ...
        Q_strlwr(lcbinding); // saves doing it on all the generateHashValues in
    ...
    while (binding && team >= 0) {
        for (i = 0; i < MAX_KEYS; i++) {
            if (keys[i].binding[team] &&
                Q_stricmp(binding, keys[i].binding[team]) == 0) {
                return i;
            }
        }
...
void Key_Bind_f() {
    ...
    if (c < 2 + teambind) {
        Cmd_PrintUsage(teambind ? "<team> <key> [<command>]" : "<key> [<command>]",
...
static void Field_TeamnameCompletion(void (*callback)(const char* s),
                                     int flags) {
...
static void Key_CompleteUnbind(char* args, int argNum) {
    if (argNum < 4) {
        ...
        if (p > args) {
            Field_CompleteKeyname(argNum > 2 ? 0 : FIELD_TEAM |
                                                           FIELD_TEAM_SPECTATORS |
                                                           FIELD_TEAM_DEFAULT);
        }
    }
}
...
static void Key_CompleteBind_Internal(char* args, int argNum, int nameArg) {
    ...
    if (argNum == nameArg) {
        ...
        Cmd::Args arg(args);
        Cmd::CompletionResult res = Cmd::CompleteArgument(
                Cmd::Args(arg.EscapedArgs(nameArg)), argNum - nameArg - 1);
...
static void Key_CompleteEditbind(char* args, int argNum) {
    if (argNum < 4) {
        Field_CompleteKeyname(argNum > 2 ? 0 : FIELD_TEAM | FIELD_TEAM_SPECTATORS |
                                                       FIELD_TEAM_DEFAULT);
    }
}
...
static const struct {
    ...
    unsigned short bit;
    unsigned int index;
} modifierKeys[] = {{"shift", 5, 1, K_SHIFT},
                    {"ctrl", 4, 2, K_CTRL},
                    {"alt", 3, 4, K_ALT},
...
void Key_ModCase_f() {
    ...
    if (argc < 3) {
        Cmd_PrintUsage(
                "<modifiers> <command> [<modifiers> <command>] … [<command>]", nullptr);
        return;
    }
    ...
    if (cls.keyCatchers & KEYCATCH_CONSOLE) {
        ...
        if (kb) {
            ...
            Cmd::BufferCommandTextAfter(
                    va("setkeydata %d %d %u\n%s", plusCommand.check, key + 1, time, kb),
                    true);
...
void CL_CharEvent(int c) {
    ...
    // the RIGHT way to do this would be to have certain keys disable the
    // equivalent SE_CHAR event
