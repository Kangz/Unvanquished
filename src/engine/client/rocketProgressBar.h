/*
 ...
 In addition, the Daemon Source Code is also subject to certain additional
 terms.
...
public:
    ...
    void SetValue(const float value) { SetAttribute("value", value); }
    ...
    void OnRender() {
        ...
        if (orientation >= UP) {
            ...
            if (orientation == DOWN) {
                re.SetColor(color);
                re.DrawStretchPic(position.x, position.y, dimensions.x, height, 0.0f, 0.0f, 1.0f, value, shader);
                re.SetColor(nullptr);
            } else {
                re.SetColor(color);
                re.DrawStretchPic(position.x, position.y - height + dimensions.y, dimensions.x, height, 0.0f, 1.0f - value, 1.0f, 1.0f, shader);
                re.SetColor(nullptr);
            }
        }
        ...
        // Horizontal meter
            ...
            if (orientation == LEFT) {
                re.SetColor(color);
                re.DrawStretchPic(position.x, position.y, width, dimensions.y, 0.0f, 0.0f, value, 1.0f, shader);
                re.SetColor(nullptr);
            } else {
                re.SetColor(color);
                re.DrawStretchPic(position.x - width + dimensions.x, position.y, width, dimensions.y, 1.0f - value, 0.0f, 1.0f, 1.0f, shader);
                re.SetColor(nullptr);
            }
        }
    }
    ...
    OnPropertyChange(const Rocket::Core::PropertyNameList& changed_properties) {
        ...
        if (changed_properties.find("color") != changed_properties.end()) {
            Rocket::Core::Colourb tempColor =
                    GetProperty("color")->Get<Rocket::Core::Colourb>();
        ...
        if (changed_properties.find("orientation") != changed_properties.end()) {
            Rocket::Core::String orientation_string =
                    GetProperty<Rocket::Core::String>("orientation");
