...
void Rocket_RegisterDataFormatter(const char* name) {
    dataFormatterList.push_back(
            new RocketDataFormatter(name, dataFormatterList.size()));
}
...
void Rocket_DataFormatterFormattedData(int handle, const char* data, bool parseQuake) {
    dataFormatterList[handle]->out = parseQuake
                                             ? Rocket_QuakeToRML(data, RP_EMOTICONS)
