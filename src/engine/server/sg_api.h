...
#define SVF_NOSERVERINFO \
    0x00001000 // only meaningful for entities numbered in [0..MAX_CLIENTS)
...
typedef struct {
    ...
    float clientRadius; // if SVF_CLIENTS_IN_RANGE, send to all clients within
                        // this range
    ...
    // current
    // time, because each entity must be moved one at a time after time is
    // advanced
    ...
    // - the owner of the given entity is the pass entity (use case: don't
    // interact with your own missiles), or
    // - the given entity and the pass entity have the same owner entity (that is
    // not none)
    ...
    //     ( ent->r.ownerNum != ENTITYNUM_NONE && ent->r.ownerNum ==
    //     entities[passEntityNum].r.ownerNum ) ) )
    int ownerNum;
    ...
    int originCluster; // Gordon: calced upon linking, for origin only bmodel vis
                       // checks
    int areanum, areanum2;
} entityShared_t;

// the server looks at a sharedEntity_t structure, which must be at the start of
// a gentity_t structure
typedef struct {
