...
// the game allocates gentities with private information after the server shared
// part
...
void SV_LocateGameData(const IPC::SharedMemory& shmRegion, int numGEntities, int sizeofGEntity_t, int sizeofGameClient) {
    if (numGEntities < 0 || sizeofGEntity_t < 0 || sizeofGameClient < 0)
        Com_Error(ERR_DROP, "SV_LocateGameData: Invalid game data parameters");
    if (shmRegion.GetSize() < numGEntities * sizeofGEntity_t +
                                      sv_maxclients->integer * sizeofGameClient)
    ...
    sv.gameClients =
            reinterpret_cast<playerState_t*>(base + MAX_GENTITIES * sizeofGEntity_t);
    ...
    if (tagFileNumber > 0 && tagFileNumber <= sv.num_tagheaders) {
        for (i = sv.tagHeadersExt[tagFileNumber - 1].start;
             i < sv.tagHeadersExt[tagFileNumber - 1].start +
                             sv.tagHeadersExt[tagFileNumber - 1].count;
             i++) {
...
GameVM::GameVM()
        : VM::VMBase("sgame"), services(nullptr) {
}

void GameVM::Start() {
    services = std::unique_ptr<VM::CommonVMServices>(
            new VM::CommonVMServices(*this, "SGame", Cmd::GAME_VM));
...
void GameVM::QVMSyscall(int index, Util::Reader& reader, IPC::Channel& channel) {
    switch (index) {
        case G_LOCATE_GAME_DATA1:
            IPC::HandleMsg<LocateGameDataMsg1>(
                    channel, std::move(reader), [this](IPC::SharedMemory shm, int numEntities, int entitySize, int playerSize) {
            shmRegion = std::move(shm);
            SV_LocateGameData(shmRegion, numEntities, entitySize, playerSize); });
            break;

        case G_LOCATE_GAME_DATA2:
            IPC::HandleMsg<LocateGameDataMsg2>(
                    channel, std::move(reader), [this](int numEntities, int entitySize, int playerSize) {
            SV_LocateGameData(shmRegion, numEntities, entitySize, playerSize); });
            break;

        case G_ADJUST_AREA_PORTAL_STATE:
            IPC::HandleMsg<AdjustAreaPortalStateMsg>(
                    channel, std::move(reader), [this](int entityNum, bool open) {
        ...
        case G_DROP_CLIENT:
            IPC::HandleMsg<DropClientMsg>(channel, std::move(reader), [this](int clientNum, std::string reason) {
                                      SV_GameDropClient(clientNum,
                                                        reason.c_str()); });
            break;

        case G_SEND_SERVER_COMMAND:
            IPC::HandleMsg<SendServerCommandMsg>(
                    channel, std::move(reader), [this](int clientNum, std::string text) {
            SV_GameSendServerCommand(clientNum, text.c_str()); });
            break;

        case G_SET_CONFIGSTRING:
            IPC::HandleMsg<SetConfigStringMsg>(
                    channel, std::move(reader), [this](int index, std::string val) {
            SV_SetConfigstring(index, val.c_str()); });
            break;

        case G_GET_CONFIGSTRING:
            IPC::HandleMsg<GetConfigStringMsg>(
                    channel, std::move(reader), [this](int index, int len, std::string& res) {
        ...
        case G_SET_CONFIGSTRING_RESTRICTIONS:
            IPC::HandleMsg<SetConfigStringRestrictionsMsg>(
                    channel, std::move(reader), [this] {
        ...
        case G_GET_SERVERINFO:
            IPC::HandleMsg<GetServerinfoMsg>(
                    channel, std::move(reader), [this](int len, std::string& res) {
            std::unique_ptr<char[]> buffer(new char[len]);
            buffer[0] = '\0';
        ...
        case G_GET_USERCMD:
            IPC::HandleMsg<GetUsercmdMsg>(
                    channel, std::move(reader), [this](int index, usercmd_t& cmd) { SV_GetUsercmd(index, &cmd); });
            break;

        case G_GET_ENTITY_TOKEN:
            IPC::HandleMsg<GetEntityTokenMsg>(
                    channel, std::move(reader), [this](bool& boolRes, std::string& res) {
            res = COM_Parse(&sv.entityParsePoint);
            boolRes = sv.entityParsePoint or res.size() > 0; });
            break;

        case G_SEND_GAME_STAT:
            IPC::HandleMsg<SendGameStatMsg>(
                    channel, std::move(reader), [this](std::string text) { SV_MasterGameStat(text.c_str()); });
            break;

        case G_GET_TAG:
            IPC::HandleMsg<GetTagMsg>(
                    channel, std::move(reader), [this](int clientNum, int tagFileNumber, std::string tagName, int& res, orientation_t& orientation) {
            res = SV_GetTag(clientNum, tagFileNumber, tagName.c_str(),
                            &orientation); });
            break;
        ...
        case G_SEND_MESSAGE:
            IPC::HandleMsg<SendMessageMsg>(
                    channel, std::move(reader), [this](int clientNum, int len, std::vector<char> message) {
        ...
        case G_RSA_GENMSG:
            IPC::HandleMsg<RSAGenMsgMsg>(
                    channel, std::move(reader), [this](std::string pubkey, int& res, std::string& cleartext, std::string& encrypted) {
            char cleartextBuffer[RSA_STRING_LENGTH];
            char encryptedBuffer[RSA_STRING_LENGTH];
            res =
                SV_RSAGenMsg(pubkey.c_str(), cleartextBuffer, encryptedBuffer);
            cleartext = cleartextBuffer;
            encrypted = encryptedBuffer; });
            break;

        case G_GEN_FINGERPRINT:
            IPC::HandleMsg<GenFingerprintMsg>(
                    channel, std::move(reader), [this](int keylen, const std::vector<char>& key, int len, std::string& res) {
            std::unique_ptr<char[]> buffer(new char[len]);
            buffer[0] = '\0';
        ...
        case G_GET_PLAYER_PUBKEY:
            IPC::HandleMsg<GetPlayerPubkeyMsg>(
                    channel, std::move(reader), [this](int clientNum, int len, std::string& pubkey) {
            std::unique_ptr<char[]> buffer(new char[len]);
            buffer[0] = '\0';
        ...
        case G_GET_TIME_STRING:
            IPC::HandleMsg<GetTimeStringMsg>(
                    channel, std::move(reader), [this](int len, std::string format, const qtime_t& time, std::string& res) {
            std::unique_ptr<char[]> buffer(new char[len]);
            buffer[0] = '\0';
        ...
        case BOT_ALLOCATE_CLIENT:
            IPC::HandleMsg<BotAllocateClientMsg>(
                    channel, std::move(reader), [this](int& output) { output = SV_BotAllocateClient(); });
            break;

        case BOT_FREE_CLIENT:
            IPC::HandleMsg<BotFreeClientMsg>(
                    channel, std::move(reader), [this](int input) { SV_BotFreeClient(input); });
            break;

        case BOT_GET_CONSOLE_MESSAGE:
            IPC::HandleMsg<BotGetConsoleMessageMsg>(
                    channel, std::move(reader), [this](int client, int len, int& res, std::string& message) {
            std::unique_ptr<char[]> buffer(new char[len]);
            buffer[0] = '\0';
        ...
        case BOT_NAV_SETUP:
            IPC::HandleMsg<BotNavSetupMsg>(
                    channel, std::move(reader), [this](botClass_t botClass, int& res, int& handle) {
        ...
        case BOT_FIND_ROUTE:
            IPC::HandleMsg<BotFindRouteMsg>(
                    channel, std::move(reader), [this](int clientNum, botRouteTarget_t target, bool allowPartial, int& res) {
            res = BotFindRouteExt(clientNum, &target, allowPartial); });
            break;

        case BOT_UPDATE_PATH:
            IPC::HandleMsg<BotUpdatePathMsg>(
                    channel, std::move(reader), [this](int clientNum, botRouteTarget_t target, botNavCmd_t& cmd) {
            BotUpdateCorridor(clientNum, &target, &cmd); });
            break;

        case BOT_NAV_RAYCAST:
            IPC::HandleMsg<BotNavRaycastMsg>(
                    channel, std::move(reader), [this](int clientNum, std::array<float, 3> start, std::array<float, 3> end, int& res, botTrace_t& botTrace) {
            res = BotNavTrace(clientNum, &botTrace, start.data(), end.data()); });
            break;

        case BOT_NAV_RANDOMPOINT:
            IPC::HandleMsg<BotNavRandomPointMsg>(
                    channel, std::move(reader), [this](int clientNum, std::array<float, 3>& point) {
            BotFindRandomPoint(clientNum, point.data()); });
            break;

        case BOT_NAV_RANDOMPOINTRADIUS:
            IPC::HandleMsg<BotNavRandomPointRadiusMsg>(
                    channel, std::move(reader), [this](int clientNum, std::array<float, 3> origin, float radius, int& res, std::array<float, 3>& point) {
            res = BotFindRandomPointInRadius(clientNum, origin.data(),
                                             point.data(), radius); });
            break;

        case BOT_ENABLE_AREA:
            IPC::HandleMsg<BotEnableAreaMsg>(
                    channel, std::move(reader), [this](std::array<float, 3> origin, std::array<float, 3> mins, std::array<float, 3> maxs) {
            BotEnableArea(origin.data(), mins.data(), maxs.data()); });
            break;

        case BOT_DISABLE_AREA:
            IPC::HandleMsg<BotDisableAreaMsg>(
                    channel, std::move(reader), [this](std::array<float, 3> origin, std::array<float, 3> mins, std::array<float, 3> maxs) {
            BotDisableArea(origin.data(), mins.data(), maxs.data()); });
            break;

        case BOT_ADD_OBSTACLE:
            IPC::HandleMsg<BotAddObstacleMsg>(
                    channel, std::move(reader), [this](std::array<float, 3> mins, std::array<float, 3> maxs, int& handle) {
            BotAddObstacle(mins.data(), maxs.data(), &handle); });
            break;

        case BOT_REMOVE_OBSTACLE:
            IPC::HandleMsg<BotRemoveObstacleMsg>(
                    channel, std::move(reader), [this](int handle) { BotRemoveObstacle(handle); });
            break;
