...
typedef struct {
    ...
    int checksumFeed;      // the feed key that we use to compute the pure checksum
                           // strings
    ...
    int gameClientSize; // will be > sizeof(playerState_t) due to game private
                        // data
...
typedef enum {
    ...
    CS_ZOMBIE,    // client has been disconnected, but don't reuse connection for a
                  // couple seconds
...
typedef struct client_s {
    ...
    int reliableSequence;    // last added reliable message, not necessarily sent or
                             // acknowledged yet
    int reliableAcknowledge; // last acknowledged reliable message
    int reliableSent;        // last sent reliable message, not necessarily acknowledged
                             // yet
    ...
    unsigned char* downloadBlocks[MAX_DOWNLOAD_WINDOW]; // the buffers for the
                                                        // download blocks
    ...
    bool bFallback;               // last www download attempt failed, fallback to regular
                                  // download
    // note: this is one-shot, multiple downloads would cause a www download to be
    // attempted again

    int deltaMessage;                       // frame last client usercmd message
    int nextReliableTime;                   // svs.time when another reliable command will be
                                            // allowed
    int lastPacketTime;                     // svs.time when packet was last received
    int lastConnectTime;                    // svs.time when connection started
    int nextSnapshotTime;                   // send another snapshot when svs.time >=
                                            // nextSnapshotTime
    bool rateDelayed;                       // true if nextSnapshotTime was set based on rate instead
                                            // of snapshotMsec
    int timeoutCount;                       // must timeout a few frames in a row so debugging doesn't
                                            // break
    clientSnapshot_t frames[PACKET_BACKUP]; // updates can be delta'd from here
    int ping;
    int rate;         // bytes / second
    int snapshotMsec; // requests a snapshot every snapshotMsec unless rate
                      // choked
    netchan_t netchan;
    // TTimo
    // queuing outgoing fragmented messages to send them properly, without udp
    // packet bursts
...
// MAX_INFO_RECEIPTS is the maximum number of getstatus+getinfo responses that
// we send
...
typedef struct {
    ...
    challenge_t challenges[MAX_CHALLENGES]; // to prevent invalid IP addresses
                                            // from connecting
...
// sv_main.c
//
void SV_FinalCommand(char* cmd, bool disconnect); // ydnar: added disconnect
                                                  // flag so map changes can
...
//#define SV_GentityNum( num ) ((sharedEntity_t *)((byte *)sv.gentities +
// sv.gentitySize*(num)))
//#define SV_GameClientNum( num ) ((playerState_t *)((byte *)sv.gameClients +
// sv.gameClientSize*(num)))
