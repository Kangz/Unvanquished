...
cvar_t* sv_lanForceRate; // TTimo - dedicated 1 (LAN) server forces local
...
// this gets you a better throughput, but you lose the ability to control the
// download usage
cvar_t* sv_wwwDlDisconnected;
cvar_t* sv_wwwFallbackURL; // URL to send to if an http/ftp fails or is refused
                           // client side
...
Cvar::Cvar<bool> isLanOnly(
        "server.lanOnly",
        "should the server stay only on LAN (vs. advertise itself on the internet)",
...
==============================================================================
*/
static struct { netadr_t ipv4, ipv6; } masterServerAddr[MAX_MASTER_SERVERS];
...
static void SV_ResolveMasterServers() {
    ...
    for (i = 0; i < MAX_MASTER_SERVERS; i++) {
        if (!sv_master[i]->string || !sv_master[i]->string[0]) {
            challenges[i].type = masterServerAddr[i].ipv4.type =
                    masterServerAddr[i].ipv6.type = NA_BAD;
            continue;
        }
        ...
        // do it when needed
        if (sv_master[i]->modified || (masterServerAddr[i].ipv4.type == NA_BAD &&
                                       masterServerAddr[i].ipv6.type == NA_BAD)) {
...
void SVC_Info(netadr_t from, const Cmd::Args& args) {
    ...
    /*
   * Check whether Cmd_Argv(1) has a sane length. This was not done in the
   * original Quake3 version which led
  ...
  */
    ...
    for (i = 0; i < MAX_MASTER_SERVERS; i++) {
        ...
        if (!NET_CompareBaseAdr(from, masterServerAddr[i].ipv4) &&
            !NET_CompareBaseAdr(from, masterServerAddr[i].ipv6)) {
            continue;
        }

        // It was - if the saved challenge is for the other protocol, send it and
    ...
    Info_SetValueForKey(
            infostring, "sv_maxclients", va("%i", sv_maxclients->integer - sv_privateClients->integer), false);
    ...
    Info_SetValueForKey(infostring, "gamename", GAMENAME_STRING,
                        false); // Arnout: to be able to filter out Quake servers
...
bool SV_CheckDRDoS(netadr_t from) {
    ...
    for (i = 0; i < MAX_INFO_RECEIPTS; i++, receipt++) {
        if (receipt->time + 2000 > svs.time) {
            if (receipt->time) {
                ...
                // from the master servers don't get ignored.  As a consequence a
                // potentially
                // unlimited number of getinfo+getstatus responses may be sent during
                // the
        ...
        MAX_INFO_RECEIPTS) // All receipts happened in last 2 seconds.
    {
        ...
        {
            Com_Printf(
                    "Detected flood of getinfo/getstatus connectionless packets\n");
    ...
    if (specificCount >=
        3) // Already sent 3 to this IP address in last 2 seconds.
    {
        if (lastSpecificLogTime + 1000 <= svs.time) // Limit one log every second.
        {
            Com_Printf(
                    "Possible DRDoS attack to address %i.%i.%i.%i, ignoring "
                    "getinfo/getstatus connectionless packet\n",
                    exactFrom.ip[0],
                    exactFrom.ip[1],
                    exactFrom.ip[2],
                    exactFrom.ip[3]);
...
public:
    RconEnvironment(netadr_t from, int bufferSize)
            : from(from), bufferSize(bufferSize){};
...
// show_bug.cgi?id=376
// if we send an OOB print message this size, 1.31 clients die in a Com_Printf
// buffer overflow
...
#define SV_OUTPUTBUF_LENGTH (256 - 16)
    ...
    if (!strlen(sv_rconPassword->string) ||
        args.Argv(1) != sv_rconPassword->string) {
        valid = false;
...
void SV_Frame(int msec) {
    ...
    // this can happen considerably earlier when lots of clients play and the map
    // doesn't change
    ...
    if (svs.currentFrameIndex == SERVER_PERFORMANCECOUNTER_FRAMES) {
        ...
        svs.sampleTimes[svs.currentSampleIndex %
                        SERVER_PERFORMANCECOUNTER_SAMPLES] = averageFrameTime;
        ...
        // Com_Printf( "serverload: %i (%i/%i)\n", svs.serverLoad, averageFrameTime,
        // frameMsec );
