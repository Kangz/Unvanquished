...
=============
*/
static void SV_EmitPacketEntities(const clientSnapshot_t* from,
                                  clientSnapshot_t* to,
                                  msg_t* msg) {
...
static void SV_AddEntitiesVisibleFromPoint(
        vec3_t origin, clientSnapshot_t* frame,
        //                                  snapshotEntityNumbers_t *eNums, bool
        //                                  portal, clientSnapshot_t *oldframe, bool
        //                                  localClient ) {
        //                                  snapshotEntityNumbers_t *eNums, bool
        //                                  portal ) {
    ...
    for (e = 0; e < sv.num_entities; e++) {
        ...
        if (ent->r.svFlags & SVF_IGNOREBMODELEXTENTS) {
            if (bitvector[ent->r.originCluster >> 3] &
                (1 << (ent->r.originCluster & 7))) {
                SV_AddEntToSnapshot(playerEnt, svEnt, ent, eNums);
            }
...
static void SV_BuildClientSnapshot(client_t* client) {
    ...
    SV_AddEntitiesVisibleFromPoint(
            org, frame, &entityNumbers /*, false, client->netchan.remoteAddress.type == NA_LOOPBACK */);
...
void SV_WriteVoipToClient(client_t* cl, msg_t* msg) {
    ...
    if (cl->queuedVoipPackets) {
        ...
        for (i = 0; i < cl->queuedVoipPackets; i++) {
            packet =
                    cl->voipPacket[(i + cl->queuedVoipIndex) % ARRAY_LEN(cl->voipPacket)];
...
TTimo - use sv_maxRate or sv_dl_maxRate depending on regular or downloading
client
====================
*/
...
static int SV_RateMsec(client_t* client, int messageSize) {
    ...
    // low watermark for sv_maxRate, never 0 < sv_maxRate < 1000 (0 is no
    // limitation)
...
void SV_SendMessageToClient(msg_t* msg, client_t* client) {
    ...
    // the update server on steroids, with this disabled and sv_fps 60, the
    // download can reach 30 kb/s
...
So we send them "idle" packets with the bare minimum required to keep them on
the server.
...
void SV_SendClientSnapshot(client_t* client) {
    ...
    // bani
    if (client->state < CS_ACTIVE) {
        // bani - #760 - zombie clients need full snaps so they can still process
        // reliable commands
...
void SV_SendClientMessages() {
    ...
    for (i = 0; i < sv_maxclients->integer; i++) {
        ...
        // RF, needed to insert this otherwise bots would cause error drops in
        // sv_net_chan.c:
        ...
        if (c->netchan.unsentFragments) {
            c->nextSnapshotTime =
                    svs.time + SV_RateMsec(c, c->netchan.unsentLength - c->netchan.unsentFragmentStart);
