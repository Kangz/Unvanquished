...
public:
    ...
    void Run(const Cmd::Args& args) const OVERRIDE {
        ...
        // Gets the layout list from the command but do not override if there is
        // nothing
    ...
    Cmd::CompletionResult Complete(int argNum, const Cmd::Args& args, Str::StringRef prefix) const OVERRIDE {
        if (argNum == 1) {
            ...
            return FS::HomePath::CompleteFilename(
                    prefix, "game/layouts/" + args.Argv(1), ".dat", false, true);
        }
...
static MapCmd DevmapCmdRegistration("devmap",
                                    "starts a new map with cheats enabled",
                                    true);
...
static void SV_MapRestart_f() {
    ...
    // TTimo - don't update restartedserverId there, otherwise we won't deal
    ...
    // Gordon: meh, this won't work here as the client doesn't know it has
    // happened
    //  SV_CreateBaseline ();
