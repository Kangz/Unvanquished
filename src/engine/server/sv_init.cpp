...
void SV_Startup() {
    ...
    svs.clients =
            (client_t*) calloc(sizeof(client_t) * sv_maxclients->integer, 1);
    ...
    // Join the IPv6 multicast group now that a map is running, so clients can
    // scan for us on the local network.
    NET_JoinMulticast6();
}
...
void SV_ChangeMaxClients() {
    ...
    // Z_Free( svs.clients );
    free(svs.clients); // RF, avoid trying to allocate large chunk on a
                       // fragmented zone
...
void SV_SpawnServer(const char* server) {
    ...
    PrintBanner("Server Initialization") Com_Printf("Server: %s\n", server);

    // clear the whole hunk because we're (re)loading the server
    Hunk_Clear();

    // if not running a dedicated server CL_MapLoading will connect the client to
    // the server
    ...
    svs.snapshotEntities = (entityState_t*) Hunk_Alloc(
    ...
    for (i = 0; i < sv_maxclients->integer; i++) {
        ...
        if (svs.clients[i].state >= CS_CONNECTED) {
            ...
            denied = gvm.GameClientConnect(reason, sizeof(reason), i, false,
                                           isBot); // firstTime = false
            ...
            if (denied) {
                ...
                if (!isBot) {
                    ...
                    client->nextSnapshotTime =
                            svs.time; // generate a snapshot immediately
...
void SV_Init() {
    ...
    sv_hostname =
            Cvar_Get("sv_hostname", "Unnamed Unvanquished Server", CVAR_SERVERINFO);
    sv_maxclients = Cvar_Get(
            "sv_maxclients", "20",
            CVAR_SERVERINFO | CVAR_LATCH); // NERVE - SMF - changed to 20 from 8
...
#else
    // Use OS shared libs for the client at startup. This prevents crashes due to
    // mismatching syscall ABIs
    // from loading outdated vms pk3s. The correct vms pk3 will be loaded upon
    // connecting to a pure server.
    ...
    sv_showAverageBPS =
            Cvar_Get("sv_showAverageBPS", "0", 0); // NERVE - SMF - net debugging

    // the download netcode tops at 18/20 kb/s, no need to make you think you can
    // go above
