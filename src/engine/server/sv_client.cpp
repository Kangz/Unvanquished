...
void SV_DirectConnect(netadr_t from, const Cmd::Args& args) {
    ...
    // NOTE TTimo: but we might need to store the protocol around for potential
    // non http/ftp clients
    ...
    for (i = 0, cl = svs.clients; i < sv_maxclients->integer; i++, cl++) {
        // DHM - Nerve :: This check was allowing clients to reconnect after
        // zombietime(2 secs)
        // if ( cl->state == CS_FREE ) {
        // continue;
        //}
    ...
    if (!NET_IsLocalAddress(from)) {
        ...
        if (country) {
            ...
            Com_Printf(
                    "Client %i connecting from somewhere unknown with %i challenge "
                    "ping\n",
                    i,
                    ping);
        }
    ...
    if (!newcl) {
        if (NET_IsLocalAddress(from)) {
            ...
            if (count >= sv_maxclients->integer - startIndex) {
                SV_DropClient(&svs.clients[sv_maxclients->integer - 1],
                              "only bots on server");
...
gotnewcl:
    ...
    *newcl = std::move(temp);
    ...
    denied = gvm.GameClientConnect(reason, sizeof(reason), clientNum, true,
                                   false); // firstTime = true
...
void SV_FreeClient(client_t* client) {
    ...
    for (index = client->queuedVoipIndex; index < client->queuedVoipPackets;
         index++) {
...
void SV_BeginDownload_f(client_t* cl, const Cmd::Args& args) {
    ...
    // cl->downloadName is non-zero now, SV_WriteDownloadToClient will see this
    // and open
    // the file itself
...
void SV_WWWDownload_f(client_t* cl, const Cmd::Args& args) {
    ...
    // only accept wwwdl commands for clients which we first flagged as wwwdl
    // ourselves
    if (!cl->bWWWDl) {
    ...
    if (!Q_stricmp(subcmd, "done")) {
        ...
        *cl->downloadName = 0;
        ...
        Com_Logf(LOG_WARN,
                 "client '%s' reports that the redirect download for '%s' had "
    ...
    Com_Printf("SV_WWWDownload: unknown wwwdl subcommand '%s' for client '%s'\n",
               ...
               cl->name);
    SV_DropClient(cl,
                  va("SV_WWWDownload: unknown wwwdl subcommand '%s'", subcmd));
}
...
sv_wwwFallbackURL can be used to redirect clients to a web URL in case direct
ftp/http didn't work (or is disabled on client's end)
return true when a redirect URL message was filled up
when the cvar is set to something, the download server will effectively never
use a legacy download strategy
==================
*/
...
Check to see if the client wants a file, open it if needed and start pumping the
client
...
void SV_WriteDownloadToClient(client_t* cl, msg_t* msg) {
    ...
    if (!cl->download) {
        ...
        if (!sv_allowDownload->integer) {
            ...
            if (sv_pure->integer) {
                Com_sprintf(errorMessage, sizeof(errorMessage), "Could not download \"%s\" because autodownloading is "
        ...
        // NOTE: this is called repeatedly while a client connects. Maybe we should
        // sort of cache the message or something
        // FIXME: we need to abstract this to an independent module for maximum
        // configuration/usability by server admins
        // FIXME: I could rework that, it's crappy
        if (sv_wwwDownload->integer) {
            std::string name, version;
            Util::optional<uint32_t> checksum;
            int downloadSize = 0;
            bool success = FS::ParsePakName(
                    cl->downloadName, cl->downloadName + strlen(cl->downloadName), name, version, checksum);
            if (success) {
                const FS::PakInfo* pak = checksum
                                                 ? FS::FindPak(name, version)
                                                 : FS::FindPak(name, version, *checksum);
                if (pak) {
                    try {
            ...
            if (!cl->bFallback) {
                if (success) {
                    Q_strncpyz(cl->downloadURL,
                               va("%s/%s", sv_wwwBaseURL->string, pakName.c_str()),
                    ...
                    // once cl->downloadName is set (and possibly we have our listening
                    // socket), let the client know
                    cl->bWWWDl = true;
                    MSG_WriteByte(msg, svc_download);
                    MSG_WriteShort(msg, -1); // block -1 means ftp/http download
                    ...
                    // that should NOT happen - even regular download would fail then
                    // anyway
                ...
                Com_Logf(LOG_ERROR,
                         "Client '%s': falling back to regular downloading for failed "
                         "file %s",
                         cl->name,
                         cl->downloadName);
            }
        ...
        std::string name, version;
        Util::optional<uint32_t> checksum;
        bool success = FS::ParsePakName(cl->downloadName,
                                        cl->downloadName + strlen(cl->downloadName),
                                        name,
                                        version,
                                        checksum);
        if (success) {
            const FS::PakInfo* pak = checksum ? FS::FindPak(name, version)
                                              : FS::FindPak(name, version, *checksum);
            if (pak) {
                try {
        ...
        cl->downloadCurrentBlock = cl->downloadClientBlock = cl->downloadXmitBlock =
                0;
        cl->downloadCount = 0;
    ...
    while (cl->downloadCurrentBlock - cl->downloadClientBlock <
                   MAX_DOWNLOAD_WINDOW &&
        ...
        try {
            cl->downloadBlockSize[curindex] = cl->download->Read(
                    cl->downloadBlocks[curindex], MAX_DOWNLOAD_BLKSIZE);
        } catch (std::system_error&) {
            // EOF right now
    ...
    // based on the rate, how many bytes can we fit in the snapMsec time of the
    // client
    rate = cl->rate;

    // show_bug.cgi?id=509
    // for autodownload, we use a separate max rate value
    // we do this everytime because the client might change its rate during the
    // download
    ...
    while (blockspersnap--) {
        // Write out the next section of the file, if we have already reached our
        // window,
...
The client is going to disconnect, so remove the connection immediately  FIXME:
move to game?
=================
*/
...
void SV_UserinfoChanged(client_t* cl) {
    ...
    if (Sys_IsLANAddress(cl->netchan.remoteAddress) && isLanOnly.Get() &&
    ...
    // may be lost when client updates its userinfo
...
static void SV_UpdateUserinfo_f(client_t* cl, const Cmd::Args& args) {
    ...
    Q_strncpyz(cl->userinfo, args.Argv(1).c_str(),
               sizeof(cl->userinfo)); // FIXME QUOTING INFO
...
static ucmd_t ucmds[] = {{"userinfo", SV_UpdateUserinfo_f, false},
...
static bool SV_ClientCommand(client_t* cl, msg_t* msg, bool premaprestart) {
    ...
    if (!com_cl_running->integer &&
        cl->state >=
                CS_ACTIVE && // (SA) this was commented out in Wolf.  Did we do that?
        sv_floodProtect->integer &&
        svs.time < cl->nextReliableTime && floodprotect) {
...
static void SV_ParseBinaryMessage(client_t* cl, msg_t* msg) {
    ...
    SV_GameBinaryMessageReceived(cl - svs.clients,
                                 (char*) &msg->data[msg->readcount],
                                 size,
...
static void SV_UserVoip(client_t* cl, msg_t* msg) {
    ...
    for (i = 0, client = svs.clients; i < sv_maxclients->integer; i++, client++) {
        ...
        client->voipPacket[(client->queuedVoipIndex + client->queuedVoipPackets) %
                           ARRAY_LEN(client->voipPacket)] = packet;
...
// it is more annoying for them to let them hanging
    ...
    // can be out of range, this could cause the server to send thousands of
    // server
    // commands which the server thinks are not yet acknowledged in
...
// it is more annoying for them to let them hanging
    ...
    //
    // show_bug.cgi?id=536
    // don't drop as long as previous command was a nextdl, after a dl is done,
    // downloadName is set back to ""
    // but we still need to read the next message to move to next download or send
    // gamestate
    // I don't like this hack though, it must have been working fine at some
    // point, suspecting the fix is somewhere else
