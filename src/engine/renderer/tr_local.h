...
static inline float halfToFloat(int16_t in) {
    ...
    floatint_t fi;
            ...
            (((unsigned int) in & 0x8000) << 16) | (((unsigned int) in & 0x7fff) << 13);
    return fi.f * scale;
}
...
#define REF_COLORGRADEMAP_STORE_SIZE \
...
typedef struct trRefLight_s {
    ...
    bool noRadiosity;   // this is a pure realtime light that was not considered by
                        // XMap2
    ...
    struct interactionCache_s*
            firstInteractionCache; // only used by static lights
...
typedef struct {
    ...
    cullResult_t cull;
    vec3_t localBounds[2];
    vec3_t worldBounds[2]; // only set when not completely culled. use them for
                           // light interactions
...
typedef struct {
    ...
    matrix_t transformMatrix; // transforms object to world: either used by
                              // camera, model or light
    matrix_t viewMatrix;      // affine inverse of transform matrix to transform other
                              // objects into this space
    matrix_t viewMatrix2;     // without quake2opengl conversion
    matrix_t modelViewMatrix; // only used by models, camera viewMatrix *
                              // transformMatrix
} orientationr_t;
...
typedef enum { FT_DEFAULT,
               FT_LINEAR,
               FT_NEAREST } filterType_t;
...
typedef struct wrapType_s {
    ...
    wrapType_s()
            : s(WT_CLAMP), t(WT_CLAMP) {}
    wrapType_s(wrapTypeEnum_t w)
            : s(w), t(w) {}
    wrapType_s(wrapTypeEnum_t s, wrapTypeEnum_t t)
            : s(s), t(t) {}
...
typedef struct image_s {
    ...
    uint16_t uploadWidth, uploadHeight; // after power of two and picmip but not
...
static const char* const attributeNames[] = {
        "attr_Position", "attr_TexCoord0", "attr_QTangent", "attr_Color", "attr_Orientation", "attr_BoneFactors", "attr_Position2", "attr_QTangent2"};
...
enum {
    ...
    ATTR_BITS = ATTR_POSITION | ATTR_TEXCOORD | ATTR_QTANGENT | ATTR_COLOR // |
...
typedef struct {
    GLint numComponents;  // how many components in a single attribute for a
                          // single vertex
    GLenum componentType; // the input type for a single component
    GLboolean normalize;  // convert signed integers to the floating point range
                          // [-1, 1], and unsigned integers to the range [0, 1]
    GLsizei stride;
    GLsizei realStride;
    GLsizei ofs;
    GLsizei frameOffset; // for vertex animation, real offset computed as ofs +
                         // frame * frameOffset
...
typedef struct IBO_s {
    ...
    uint32_t indexesVBO;
    uint32_t indexesSize; // amount of memory data allocated for all triangles in
                          // bytes
    uint32_t indexesNum;
} IBO_t;
...
typedef enum {
    ...
    SS_ENVIRONMENT_NOFOG, // Tr3B: moved skybox here so we can fog post process
                          // all SS_OPAQUE materials
...
typedef enum {
    ...
    CGEN_CUSTOM_RGB,  // like fixed color but generated dynamically, single
...
typedef struct {
    ...
    bool noFog; // used only for shaders that have fog disabled, so we can enable
                // it for individual stages
} shaderStage_t;
...
typedef enum {
    ...
    SHADER_3D_DYNAMIC, // surface material: shader is for cGen diffuseLighting
                       // lighting
...
typedef struct shader_s {
    ...
    struct {
        char* name;
        int index;
    } altShader[MAX_ALTSHADERS]; // state-based remapping; note that index 0 is
                                 // unused
...
enum {
    ...
    GLS_DSTBLEND_BITS =
            GLS_DSTBLEND_ZERO | GLS_DSTBLEND_ONE | GLS_DSTBLEND_SRC_COLOR |
            GLS_DSTBLEND_ONE_MINUS_SRC_COLOR | GLS_DSTBLEND_SRC_ALPHA |
            GLS_DSTBLEND_ONE_MINUS_SRC_ALPHA | GLS_DSTBLEND_DST_ALPHA |
    ...
    GLS_DEPTHFUNC_BITS = GLS_DEPTHFUNC_LESS | GLS_DEPTHFUNC_EQUAL,
    ...
    GLS_COLORMASK_BITS = GLS_REDMASK_FALSE | GLS_GREENMASK_FALSE |
                         GLS_BLUEMASK_FALSE | GLS_ALPHAMASK_FALSE,
...
typedef struct {
    ...
    byte* pixelTarget; // set this to Non Null to copy to a buffer after scene
                       // rendering
...
typedef struct corona_s {
    ...
    int id;
    bool visible; // still send the corona request, even if not visible, for
                  // proper fading
} corona_t;
...
typedef struct {
    ...
    frustum_t frustums[MAX_SHADOWMAPS + 1]; // first frustum is the default one
                                            // with complete zNear - zFar range
...
typedef enum {
    ...
    SF_MAX = 0x7fffffff // partially (together, fully) ensures that
                        // sizeof(surfaceType_t) == sizeof(int)
} surfaceType_t;
...
static_assert(SORT_SHADER_BITS + SORT_LIGHTMAP_BITS + SORT_ENTITYNUM_BITS +
                              SORT_FOGNUM_BITS + SORT_INDEX_BITS ==
                      64,
              "invalid number of drawSurface sort bits");

static const uint64_t SORT_FOGNUM_SHIFT = SORT_INDEX_BITS;
static const uint64_t SORT_ENTITYNUM_SHIFT =
        SORT_FOGNUM_BITS + SORT_FOGNUM_SHIFT;
static const uint64_t SORT_LIGHTMAP_SHIFT =
        SORT_ENTITYNUM_BITS + SORT_ENTITYNUM_SHIFT;
static const uint64_t SORT_SHADER_SHIFT =
        SORT_LIGHTMAP_BITS + SORT_LIGHTMAP_SHIFT;
...
static_assert(SORT_ENTITYNUM_MASK >= MAX_REF_ENTITIES,
              "not enough entity bits");
...
typedef struct drawSurf_s {
    ...
    inline int index() const { return int((sort & SORT_INDEX_MASK)); }
    ...
    inline int shaderNum() const { return int(sort >> SORT_SHADER_SHIFT); }
...
typedef struct srfVBOMesh_s : srfGeneric_s {
    ...
    int lightmapNum; // FIXME get rid of this by merging all lightmaps at level
                     // load
    int fogIndex;
...
typedef struct { vec3_t xyz; } mdvXyz_t;

typedef struct { vec3_t normal; } mdvNormal_t;

typedef struct { float st[2]; } mdvSt_t;
...
typedef ALIGNED(16, struct {
  ...
  float boneWeights[MAX_WEIGHTS]; }) md5Vertex_t;
...
typedef struct {
    ...
    uint8_t componentsBits; // e.g. (COMPONENT_BIT_TX | COMPONENT_BIT_TY |
                            // COMPONENT_BIT_TZ)
...
typedef struct {
    ...
    int c_decalProjectors, c_decalTestSurfaces, c_decalClipSurfaces,
            c_decalSurfaces, c_decalSurfacesCreated;
} frontEndCounters_t;
...
typedef struct visTestResult_s {
    ...
    bool discardExisting; // true if the currently running vis test should be
                          // discarded
...
typedef struct {
    ...
    int visCounts[MAX_VISCOUNTS]; // incremented every time a new vis cluster is
                                  // entered
    ...
    // framebuffer objects
    FBO_t* portalRenderFBO;    // holds a copy of the last currentRender that was
                               // rendered into a FBO
    ...
    int mapOverBrightBits; // r_mapOverbrightBits->integer, but can be overriden
                           // by mapper using the worldspawn
    ...
    // lower indexed sortedShaders must be rendered first (opaque surfaces before
    // translucent)
    int numShaders;
...
extern cvar_t* r_lazyShaders; // 0: build all shaders on program start 1: delay
                              // shader build until first map load 2: delay
                              // shader build until needed
...
extern cvar_t* r_shadows; // controls shadows: 0 = none, 1 = blur, 2 = black
                          // planar projection,
...
void R_RotateEntityForViewParms(const trRefEntity_t* ent,
                                const viewParms_t* viewParms,
                                orientationr_t* orien);
void R_RotateEntityForLight(const trRefEntity_t* ent, const trRefLight_t* light, orientationr_t* orien);
void R_RotateLightForViewParms(const trRefLight_t* ent,
                               const viewParms_t* viewParms,
                               orientationr_t* orien);

void R_SetupFrustum2(frustum_t frustum,
                     const matrix_t modelViewProjectionMatrix);
void R_CalcFrustumNearCorners(const vec4_t frustum[FRUSTUM_PLANES],
                              vec3_t corners[4]);
void R_CalcFrustumFarCorners(const vec4_t frustum[FRUSTUM_PLANES],
                             vec3_t corners[4]);
bool R_CompareVert(srfVert_t* v1, srfVert_t* v2, bool checkst);

/* Tangent/normal vector calculation functions */
void R_CalcFaceNormal(vec3_t normal, const vec3_t v0, const vec3_t v1, const vec3_t v2);

void R_CalcTangents(vec3_t tangent, vec3_t binormal, const vec3_t v0, const vec3_t v1, const vec3_t v2, const vec2_t t0, const vec2_t t1, const vec2_t t2);

void R_CalcTangents(vec3_t tangent, vec3_t binormal, const vec3_t v0, const vec3_t v1, const vec3_t v2, const i16vec2_t t0, const i16vec2_t t1, const i16vec2_t t2);
 ...
 *   minimal error.
 */
void R_TBNtoQtangents(const vec3_t tangent, const vec3_t binormal, const vec3_t normal, i16vec4_t qtangent);

void R_QtangentsToTBN(const i16vec4_t qtangent, vec3_t tangent, vec3_t binormal, vec3_t normal);
...
image_t* R_CreateImage(const char* name, const byte** pic, int width, int height, int bits, int numMips, filterType_t filterType, wrapType_t wrapType);
...
shader_t* R_FindShader(const char* name, shaderType_t type, RegisterShaderFlags_t flags);
...
@param tetraVerts[0..2] are the ground vertices, tetraVerts[3] is the pyramid
offset
*/
...
float R_InterpolateLightGrid(world_t* w, int from[3], int to[3], float* factors[3], vec3_t ambientLight, vec3_t directedLight, vec2_t lightDir);
...
bool R_AddLightInteraction(trRefLight_t* light, surfaceType_t* surface, shader_t* surfaceShader, byte cubeSideBits, interactionType_t iaType);
...
void Tess_AutospriteDeform(int mode, int firstVertex, int numVertexes, int firstIndex, int numIndexes);
...
typedef struct { int commandId; } swapBuffersCommand_t;
...
typedef struct { int commandId; } renderFinishCommand_t;
...
extern backEndData_t*
        backEndData[SMP_FRAMES]; // the second one may not be allocated
...
void RE_RotatedPic(float x, float y, float w, float h, float s1, float t1, float s2, float t2, qhandle_t hShader,
                   float angle); // NERVE - SMF
