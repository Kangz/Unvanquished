...
GLShader_vertexLighting_DBS_entity* gl_vertexLightingShader_DBS_entity =
        nullptr;
...
void GLShaderManager::UpdateShaderProgramUniformLocations(
        GLShader* shader, shaderProgram_t* shaderProgram) const {
    ...
    shaderProgram->uniformLocations =
            (GLint*) ri.Z_Malloc(sizeof(GLint) * numUniforms);
...
static inline void AddGLSLDefine(std::string& defines,
...
std::string GLShaderManager::BuildGPUShaderText(const char* mainShaderName,
    ...
    if (!glConfig2.textureNPOTAvailable) {
        npotWidthScale =
                (float) glConfig.vidWidth / (float) NearestPowerOfTwo(glConfig.vidWidth);
        npotHeightScale = (float) glConfig.vidHeight /
                          (float) NearestPowerOfTwo(glConfig.vidHeight);
    }
    ...
    if (r_shadows->integer >= SHADOWING_ESM16 &&
        glConfig2.textureFloatAvailable && glConfig2.framebufferObjectAvailable) {
        if (r_shadows->integer == SHADOWING_ESM16 ||
            r_shadows->integer == SHADOWING_ESM32) {
            AddGLSLDefine(bufferExtra, "ESM", 1);
        } else if (r_shadows->integer == SHADOWING_EVSM32) {
            AddGLSLDefine(bufferExtra, "EVSM", 1);
            // The exponents for the EVSM techniques should be less than
            // ln(FLT_MAX/FILTER_SIZE)/2 {ln(FLT_MAX/1)/2 ~44.3}
        ...
        if ((glConfig.hardwareType == GLHW_NV_DX10 ||
            ...
            r_shadows->integer == SHADOWING_VSM32) {
    ...
    */
    ...
    // OK we added a lot of stuff but if we do something bad in the GLSL shaders
    // then we want the proper line
...
bool GLShaderManager::buildPermutation(GLShader* shader, size_t i) {
    ...
    if (shader->GetCompileMacrosString(i, compileMacros)) {
        ...
        shaderProgram->attribs =
                shader->_vertexAttribsRequired; // | _vertexAttribsOptional;
...
void GLShaderManager::InitShader(GLShader* shader) {
    shader->_shaderPrograms =
            std::vector<shaderProgram_t>(1 << shader->_compileMacros.size());
    ...
    shader->_vertexShaderText =
            BuildGPUShaderText(shader->GetMainShaderName().c_str(),
                               vertexInlines.c_str(),
                               GL_VERTEX_SHADER);
    shader->_fragmentShaderText =
            BuildGPUShaderText(shader->GetMainShaderName().c_str(),
                               fragmentInlines.c_str(),
                               GL_FRAGMENT_SHADER);
    std::string combinedShaderText =
            shader->_vertexShaderText + shader->_fragmentShaderText;

    shader->_checkSum = Com_BlockChecksum(combinedShaderText.c_str(),
                                          combinedShaderText.length());
}
...
bool GLShaderManager::LoadShaderBinary(GLShader* shader, size_t programNum) {
    ...
    fileLength =
            ri.FS_ReadFile(va("glsl/%s/%s_%u.bin", shader->GetName().c_str(), shader->GetName().c_str(), (unsigned int) programNum),
                           &binary);
...
void GLShaderManager::SaveShaderBinary(GLShader* shader, size_t programNum) {
    ...
    ri.FS_WriteFile(va("glsl/%s/%s_%u.bin", shader->GetName().c_str(), shader->GetName().c_str(), (unsigned int) programNum),
                    binary,
                    binarySize);
...
void GLShaderManager::CompileAndLinkGPUShaderProgram(
        GLShader* shader, shaderProgram_t* program, const std::string& compileMacros) const {
    ...
    if (glConfig2.shadingLanguageVersion != 120) {
        // HACK: abuse the GLSL preprocessor to turn GLSL 1.20 shaders into 1.30
        // ones
        ...
        miscText +=
                ...
                "clamp((x - edge0) / (edge1 - edge0), 0.0, 1.0); return t * t * (3.0 - "
                "2.0 * t); }\n";
    }
    ...
    // add them
    std::string vertexShaderTextWithMacros =
            vertexHeader + macrosString + miscText + shader->_vertexShaderText;
    std::string fragmentShaderTextWithMacros =
            fragmentHeader + macrosString + miscText + shader->_fragmentShaderText;
...
void GLShaderManager::CompileGPUShader(GLuint program, const char* programName, const char* shaderText, int shaderTextSize, GLenum shaderType) const {
...
#ifdef NDEBUG
        // In a release build, GLSL shader compiliation usually means that the
        // hardware does
        // not support GLSL shaders and should rely on vanilla instead.
        ri.Cvar_Set("cl_renderer", "GL");
#endif
        ri.Error(
                ERR_DROP, "Couldn't compile %s %s", (shaderType == GL_VERTEX_SHADER ? "vertex shader" : "fragment shader"), programName);
    }
    ...
    // shader will be deleted when glDeleteProgram is called on the program it is
    // attached to
...
#ifdef GLEW_ARB_get_program_binary
    // Apparently, this is necessary to get the binary program via
    // glGetProgramBinary
    if (glConfig2.getProgramBinaryAvailable) {
...
bool GLCompileMacro_USE_VERTEX_SKINNING::HasConflictingMacros(
        size_t permutation, const std::vector<GLCompileMacro*>& macros) const {
    for (size_t i = 0; i < macros.size(); i++) {
        GLCompileMacro* macro = macros[i];

        // if(GLCompileMacro_USE_VERTEX_ANIMATION* m =
        // dynamic_cast<GLCompileMacro_USE_VERTEX_ANIMATION*>(macro))
        if ((permutation & macro->GetBit()) != 0 &&
            macro->GetType() == USE_VERTEX_ANIMATION) {
...
bool GLCompileMacro_USE_VERTEX_SKINNING::MissesRequiredMacros(
        size_t permutation, const std::vector<GLCompileMacro*>& macros) const {
    return !glConfig2.vboVertexSkinningAvailable;
}

bool GLCompileMacro_USE_VERTEX_ANIMATION::HasConflictingMacros(
        size_t permutation, const std::vector<GLCompileMacro*>& macros) const {
    for (size_t i = 0; i < macros.size(); i++) {
        GLCompileMacro* macro = macros[i];

        if ((permutation & macro->GetBit()) != 0 &&
            macro->GetType() == USE_VERTEX_SKINNING) {
            // ri.Printf(PRINT_ALL, "conflicting macro! canceling '%s' vs. '%s'\n",
            // GetName(), macro->GetName());
            return true;
        }
...
uint32_t
GLCompileMacro_USE_VERTEX_ANIMATION::GetRequiredVertexAttributes() const {
    uint32_t attribs = ATTR_POSITION2 | ATTR_QTANGENT2;

    return attribs;
}

bool GLCompileMacro_USE_VERTEX_SPRITE::HasConflictingMacros(
        size_t permutation, const std::vector<GLCompileMacro*>& macros) const {
    for (size_t i = 0; i < macros.size(); i++) {
        GLCompileMacro* macro = macros[i];
...
bool GLCompileMacro_USE_PARALLAX_MAPPING::MissesRequiredMacros(
        size_t permutation, const std::vector<GLCompileMacro*>& macros) const {
    ...
    for (size_t i = 0; i < macros.size(); i++) {
        ...
        if ((permutation & macro->GetBit()) != 0 &&
            macro->GetType() == USE_NORMAL_MAPPING) {
...
bool GLCompileMacro_USE_REFLECTIVE_SPECULAR::MissesRequiredMacros(
        size_t permutation, const std::vector<GLCompileMacro*>& macros) const {
    bool foundUSE_NORMAL_MAPPING = false;
...
bool GLShader::GetCompileMacrosString(size_t permutation,
                                      std::string& compileMacrosOut) const {
    compileMacrosOut = "";
...
void GLShader::BindProgram() {
    ...
    // program may not be loaded yet because the shader manager hasn't yet gotten
    // to it
    ...
    if (index >= _shaderPrograms.size() || !_shaderPrograms[index].program) {
        ...
        ri.Error(ERR_FATAL,
                 "Invalid shader configuration: shader = '%s', macros = '%s'",
                 _name.c_str(),
                 activeMacros.c_str());
    }
    ...
    if (r_logFile->integer) {
        ...
        GLimp_LogComment(
                va("--- GL_BindProgram( name = '%s', macros = '%s' ) ---\n",
                   this->GetName().c_str(),
                   macros.c_str()));
    }
...
void GLShader::SetRequiredVertexPointers() {
    ...
    GL_VertexAttribsState((_vertexAttribsRequired | _vertexAttribs |
                           macroVertexAttribs)); // & ~_vertexAttribsUnsupported);
}
...
GLShader_generic::GLShader_generic(GLShaderManager* manager)
        : GLShader("generic", ATTR_POSITION | ATTR_TEXCOORD | ATTR_QTANGENT, manager),
          u_ColorTextureMatrix(this), u_ViewOrigin(this), u_ViewUp(this),
          u_AlphaThreshold(this), u_ModelMatrix(this),
          u_ProjectionMatrixTranspose(this), u_ModelViewProjectionMatrix(this),
          u_ColorModulate(this), u_Color(this), u_Bones(this),
          u_VertexInterpolation(this), u_DepthScale(this), GLDeformStage(this),
...
void GLShader_generic::SetShaderProgramUniforms(
        shaderProgram_t* shaderProgram) {
...
GLShader_lightMapping::GLShader_lightMapping(GLShaderManager* manager)
        : GLShader("lightMapping",
                   ...
                   manager),
          u_DiffuseTextureMatrix(this), u_NormalTextureMatrix(this),
          u_SpecularTextureMatrix(this), u_GlowTextureMatrix(this),
          u_SpecularExponent(this), u_ColorModulate(this), u_Color(this),
          u_AlphaThreshold(this), u_ViewOrigin(this), u_ModelMatrix(this),
          u_ModelViewProjectionMatrix(this), u_DepthScale(this),
          GLDeformStage(this), GLCompileMacro_USE_NORMAL_MAPPING(this),
...
void GLShader_lightMapping::BuildShaderVertexLibNames(
        std::string& vertexInlines) {
    vertexInlines += "vertexSimple vertexSkinning vertexAnimation vertexSprite "
                     "deformVertexes ";
}
...
void GLShader_lightMapping::BuildShaderCompileMacros(
        std::string& compileMacros) {
    compileMacros += "TWOSIDED ";
}

void GLShader_lightMapping::SetShaderProgramUniforms(
        shaderProgram_t* shaderProgram) {
...
GLShader_vertexLighting_DBS_entity::GLShader_vertexLighting_DBS_entity(
        ...
        : GLShader("vertexLighting_DBS_entity",
          ...
          u_DiffuseTextureMatrix(this), u_NormalTextureMatrix(this),
          u_SpecularTextureMatrix(this), u_GlowTextureMatrix(this),
          u_SpecularExponent(this), u_AlphaThreshold(this), u_ViewOrigin(this),
          u_ModelMatrix(this), u_ModelViewProjectionMatrix(this), u_Bones(this),
          u_VertexInterpolation(this), u_DepthScale(this),
          u_EnvironmentInterpolation(this), u_LightGridOrigin(this),
          u_LightGridScale(this), GLDeformStage(this),
          GLCompileMacro_USE_VERTEX_SKINNING(this),
          GLCompileMacro_USE_VERTEX_ANIMATION(this),
...
void GLShader_vertexLighting_DBS_entity::BuildShaderVertexLibNames(
        std::string& vertexInlines) {
    vertexInlines +=
            "vertexSimple vertexSkinning vertexAnimation deformVertexes ";
}

void GLShader_vertexLighting_DBS_entity::BuildShaderFragmentLibNames(
        std::string& fragmentInlines) {
    fragmentInlines += "reliefMapping";
}

void GLShader_vertexLighting_DBS_entity::BuildShaderCompileMacros(
        std::string& compileMacros) {
    compileMacros += "TWOSIDED ";
}

void GLShader_vertexLighting_DBS_entity::SetShaderProgramUniforms(
        shaderProgram_t* shaderProgram) {
    glUniform1i(glGetUniformLocation(shaderProgram->program, "u_DiffuseMap"), 0);
    glUniform1i(glGetUniformLocation(shaderProgram->program, "u_NormalMap"), 1);
    glUniform1i(glGetUniformLocation(shaderProgram->program, "u_SpecularMap"), 2);
    glUniform1i(glGetUniformLocation(shaderProgram->program, "u_EnvironmentMap0"),
                3);
    glUniform1i(glGetUniformLocation(shaderProgram->program, "u_EnvironmentMap1"),
                4);
    glUniform1i(glGetUniformLocation(shaderProgram->program, "u_GlowMap"), 5);
...
GLShader_vertexLighting_DBS_world::GLShader_vertexLighting_DBS_world(
        ...
        : GLShader("vertexLighting_DBS_world",
          ...
          u_AlphaThreshold(this), u_ViewOrigin(this), u_ModelMatrix(this),
          u_ModelViewProjectionMatrix(this), u_DepthScale(this),
          u_LightWrapAround(this), u_LightGridOrigin(this), u_LightGridScale(this),
          GLDeformStage(this), GLCompileMacro_USE_NORMAL_MAPPING(this),
          GLCompileMacro_USE_PARALLAX_MAPPING(this),
          GLCompileMacro_USE_GLOW_MAPPING(this) //,
// GLCompileMacro_TWOSIDED(this)
...
void GLShader_vertexLighting_DBS_world::BuildShaderVertexLibNames(
        std::string& vertexInlines) {
    vertexInlines += "vertexSimple vertexSkinning vertexAnimation vertexSprite "
                     "deformVertexes ";
}
void GLShader_vertexLighting_DBS_world::BuildShaderFragmentLibNames(
        std::string& fragmentInlines) {
    fragmentInlines += "reliefMapping";
}

void GLShader_vertexLighting_DBS_world::BuildShaderCompileMacros(
        std::string& compileMacros) {
    compileMacros += "TWOSIDED ";
}

void GLShader_vertexLighting_DBS_world::SetShaderProgramUniforms(
        shaderProgram_t* shaderProgram) {
    glUniform1i(glGetUniformLocation(shaderProgram->program, "u_DiffuseMap"), 0);
    glUniform1i(glGetUniformLocation(shaderProgram->program, "u_NormalMap"), 1);
    glUniform1i(glGetUniformLocation(shaderProgram->program, "u_SpecularMap"), 2);
...
GLShader_forwardLighting_omniXYZ::GLShader_forwardLighting_omniXYZ(
        ...
        : GLShader("forwardLighting_omniXYZ", "forwardLighting", ATTR_POSITION | ATTR_TEXCOORD | ATTR_QTANGENT, manager),
          u_DiffuseTextureMatrix(this), u_NormalTextureMatrix(this),
          u_SpecularTextureMatrix(this), u_SpecularExponent(this),
          u_AlphaThreshold(this), u_ColorModulate(this), u_Color(this),
          u_ViewOrigin(this), u_LightOrigin(this), u_LightColor(this),
          u_LightRadius(this), u_LightScale(this), u_LightWrapAround(this),
          u_LightAttenuationMatrix(this), u_ShadowTexelSize(this),
          u_ShadowBlur(this), u_ModelMatrix(this),
          u_ModelViewProjectionMatrix(this), u_Bones(this),
          u_VertexInterpolation(this), u_DepthScale(this), GLDeformStage(this),
          GLCompileMacro_USE_VERTEX_SKINNING(this),
          GLCompileMacro_USE_VERTEX_ANIMATION(this),
          GLCompileMacro_USE_NORMAL_MAPPING(this),
...
void GLShader_forwardLighting_omniXYZ::BuildShaderVertexLibNames(
        std::string& vertexInlines) {
    vertexInlines +=
            "vertexSimple vertexSkinning vertexAnimation deformVertexes ";
}

void GLShader_forwardLighting_omniXYZ::BuildShaderFragmentLibNames(
        std::string& fragmentInlines) {
    fragmentInlines += "reliefMapping";
}

void GLShader_forwardLighting_omniXYZ::BuildShaderCompileMacros(
        std::string& compileMacros) {
    compileMacros += "TWOSIDED ";
}

void GLShader_forwardLighting_omniXYZ::SetShaderProgramUniforms(
        shaderProgram_t* shaderProgram) {
    glUniform1i(glGetUniformLocation(shaderProgram->program, "u_DiffuseMap"), 0);
    glUniform1i(glGetUniformLocation(shaderProgram->program, "u_NormalMap"), 1);
    glUniform1i(glGetUniformLocation(shaderProgram->program, "u_SpecularMap"), 2);
    glUniform1i(
            glGetUniformLocation(shaderProgram->program, "u_AttenuationMapXY"), 3);
    glUniform1i(glGetUniformLocation(shaderProgram->program, "u_AttenuationMapZ"),
                4);
    ...
    glUniform1i(glGetUniformLocation(shaderProgram->program, "u_ShadowClipMap"),
                7);
}
...
GLShader_forwardLighting_projXYZ::GLShader_forwardLighting_projXYZ(
        ...
        : GLShader("forwardLighting_projXYZ", "forwardLighting", ATTR_POSITION | ATTR_TEXCOORD | ATTR_QTANGENT, manager),
          ...
          u_LightRadius(this), u_LightScale(this), u_LightWrapAround(this),
          u_LightAttenuationMatrix(this), u_ShadowTexelSize(this),
          u_ShadowBlur(this), u_ShadowMatrix(this), u_ModelMatrix(this),
          u_ModelViewProjectionMatrix(this), u_Bones(this),
          u_VertexInterpolation(this), u_DepthScale(this), GLDeformStage(this),
          GLCompileMacro_USE_VERTEX_SKINNING(this),
          GLCompileMacro_USE_VERTEX_ANIMATION(this),
...
void GLShader_forwardLighting_projXYZ::BuildShaderVertexLibNames(
        std::string& vertexInlines) {
    vertexInlines +=
            "vertexSimple vertexSkinning vertexAnimation deformVertexes ";
}

void GLShader_forwardLighting_projXYZ::BuildShaderFragmentLibNames(
        std::string& fragmentInlines) {
    fragmentInlines += "reliefMapping";
}

void GLShader_forwardLighting_projXYZ::BuildShaderCompileMacros(
        std::string& compileMacros) {
    compileMacros += "LIGHT_PROJ ";
    compileMacros += "TWOSIDED ";
}

void GLShader_forwardLighting_projXYZ::SetShaderProgramUniforms(
        shaderProgram_t* shaderProgram) {
    glUniform1i(glGetUniformLocation(shaderProgram->program, "u_DiffuseMap"), 0);
    glUniform1i(glGetUniformLocation(shaderProgram->program, "u_NormalMap"), 1);
    glUniform1i(glGetUniformLocation(shaderProgram->program, "u_SpecularMap"), 2);
    ...
    glUniform1i(glGetUniformLocation(shaderProgram->program, "u_RandomMap"), 6);
    glUniform1i(glGetUniformLocation(shaderProgram->program, "u_ShadowClipMap0"),
                7);
}
...
GLShader_forwardLighting_directionalSun::
        ...
        : GLShader("forwardLighting_directionalSun", "forwardLighting", ATTR_POSITION | ATTR_TEXCOORD | ATTR_QTANGENT, manager),
          ...
          u_SpecularTextureMatrix(this), u_SpecularExponent(this),
          u_AlphaThreshold(this), u_ColorModulate(this), u_Color(this),
          u_ViewOrigin(this), u_LightDir(this), u_LightColor(this),
          u_LightRadius(this), u_LightScale(this), u_LightWrapAround(this),
          u_LightAttenuationMatrix(this), u_ShadowTexelSize(this),
          u_ShadowBlur(this), u_ShadowMatrix(this),
          u_ShadowParallelSplitDistances(this), u_ModelMatrix(this),
          u_ViewMatrix(this), u_ModelViewProjectionMatrix(this), u_Bones(this),
          u_VertexInterpolation(this), u_DepthScale(this), GLDeformStage(this),
          GLCompileMacro_USE_VERTEX_SKINNING(this),
          GLCompileMacro_USE_VERTEX_ANIMATION(this),
          GLCompileMacro_USE_NORMAL_MAPPING(this),
...
void GLShader_forwardLighting_directionalSun::BuildShaderVertexLibNames(
        std::string& vertexInlines) {
    vertexInlines +=
            "vertexSimple vertexSkinning vertexAnimation deformVertexes ";
}

void GLShader_forwardLighting_directionalSun::BuildShaderFragmentLibNames(
        std::string& fragmentInlines) {
    fragmentInlines += "reliefMapping";
}

void GLShader_forwardLighting_directionalSun::BuildShaderCompileMacros(
        std::string& compileMacros) {
    compileMacros += "LIGHT_DIRECTIONAL ";
    compileMacros += "TWOSIDED ";
}

void GLShader_forwardLighting_directionalSun::SetShaderProgramUniforms(
        shaderProgram_t* shaderProgram) {
    glUniform1i(glGetUniformLocation(shaderProgram->program, "u_DiffuseMap"), 0);
    glUniform1i(glGetUniformLocation(shaderProgram->program, "u_NormalMap"), 1);
    glUniform1i(glGetUniformLocation(shaderProgram->program, "u_SpecularMap"), 2);
    // glUniform1i(glGetUniformLocation( shaderProgram->program,
    // "u_AttenuationMapXY" ), 3);
    // glUniform1i(glGetUniformLocation( shaderProgram->program,
    // "u_AttenuationMapZ" ), 4);
    glUniform1i(glGetUniformLocation(shaderProgram->program, "u_ShadowMap0"), 5);
    ...
    glUniform1i(glGetUniformLocation(shaderProgram->program, "u_ShadowClipMap0"),
                10);
    glUniform1i(glGetUniformLocation(shaderProgram->program, "u_ShadowClipMap1"),
                11);
    glUniform1i(glGetUniformLocation(shaderProgram->program, "u_ShadowClipMap2"),
                12);
    glUniform1i(glGetUniformLocation(shaderProgram->program, "u_ShadowClipMap3"),
                13);
    glUniform1i(glGetUniformLocation(shaderProgram->program, "u_ShadowClipMap4"),
                14);
}
...
GLShader_shadowFill::GLShader_shadowFill(GLShaderManager* manager)
        : GLShader("shadowFill", ATTR_POSITION | ATTR_TEXCOORD | ATTR_QTANGENT, manager),
          u_ColorTextureMatrix(this), u_ViewOrigin(this), u_AlphaThreshold(this),
          u_LightOrigin(this), u_LightRadius(this), u_ModelMatrix(this),
          u_ModelViewProjectionMatrix(this), u_Color(this), u_Bones(this),
          u_VertexInterpolation(this), GLDeformStage(this),
          GLCompileMacro_USE_VERTEX_SKINNING(this),
          GLCompileMacro_USE_VERTEX_ANIMATION(this),
          GLCompileMacro_LIGHT_DIRECTIONAL(this) {
}

void GLShader_shadowFill::BuildShaderVertexLibNames(
        std::string& vertexInlines) {
    vertexInlines +=
            "vertexSimple vertexSkinning vertexAnimation deformVertexes ";
}

void GLShader_shadowFill::SetShaderProgramUniforms(
        shaderProgram_t* shaderProgram) {
    glUniform1i(glGetUniformLocation(shaderProgram->program, "u_ColorMap"), 0);
}
...
GLShader_reflection::GLShader_reflection(GLShaderManager* manager)
        : GLShader("reflection", "reflection_CB", ATTR_POSITION | ATTR_TEXCOORD | ATTR_QTANGENT, manager),
          u_NormalTextureMatrix(this), u_ViewOrigin(this), u_ModelMatrix(this),
          u_ModelViewProjectionMatrix(this), u_Bones(this),
          u_VertexInterpolation(this), GLDeformStage(this),
          GLCompileMacro_USE_VERTEX_SKINNING(this),
          GLCompileMacro_USE_VERTEX_ANIMATION(this),
...
void GLShader_reflection::BuildShaderVertexLibNames(
        std::string& vertexInlines) {
    vertexInlines +=
            "vertexSimple vertexSkinning vertexAnimation deformVertexes ";
}
...
void GLShader_reflection::SetShaderProgramUniforms(
        shaderProgram_t* shaderProgram) {
    glUniform1i(glGetUniformLocation(shaderProgram->program, "u_ColorMap"), 0);
    glUniform1i(glGetUniformLocation(shaderProgram->program, "u_NormalMap"), 1);
}
...
GLShader_skybox::GLShader_skybox(GLShaderManager* manager)
        : GLShader("skybox", ATTR_POSITION, manager), u_ViewOrigin(this),
          u_ModelMatrix(this), u_ModelViewProjectionMatrix(this), u_Bones(this),
          u_VertexInterpolation(this), GLDeformStage(this) {
}
...
GLShader_fogQuake3::GLShader_fogQuake3(GLShaderManager* manager)
        : GLShader("fogQuake3", ATTR_POSITION | ATTR_QTANGENT, manager),
          u_ModelMatrix(this), u_ModelViewProjectionMatrix(this), u_Color(this),
          u_Bones(this), u_VertexInterpolation(this), u_FogDistanceVector(this),
          u_FogDepthVector(this), u_FogEyeT(this), GLDeformStage(this),
          GLCompileMacro_USE_VERTEX_SKINNING(this),
          GLCompileMacro_USE_VERTEX_ANIMATION(this) {
}
...
void GLShader_fogQuake3::SetShaderProgramUniforms(
        shaderProgram_t* shaderProgram) {
    glUniform1i(glGetUniformLocation(shaderProgram->program, "u_ColorMap"), 0);
}

GLShader_fogGlobal::GLShader_fogGlobal(GLShaderManager* manager)
        : GLShader("fogGlobal", ATTR_POSITION, manager), u_ViewOrigin(this),
          u_ViewMatrix(this), u_ModelViewProjectionMatrix(this),
          u_UnprojectMatrix(this), u_Color(this), u_FogDistanceVector(this),
          u_FogDepthVector(this) {
}

void GLShader_fogGlobal::SetShaderProgramUniforms(
        shaderProgram_t* shaderProgram) {
    glUniform1i(glGetUniformLocation(shaderProgram->program, "u_ColorMap"), 0);
    glUniform1i(glGetUniformLocation(shaderProgram->program, "u_DepthMap"), 1);
}
...
GLShader_heatHaze::GLShader_heatHaze(GLShaderManager* manager)
        : GLShader("heatHaze", ATTR_POSITION | ATTR_TEXCOORD | ATTR_QTANGENT, manager),
          u_NormalTextureMatrix(this), u_ViewOrigin(this), u_ViewUp(this),
          u_DeformMagnitude(this), u_ModelMatrix(this),
          u_ModelViewProjectionMatrix(this), u_ModelViewMatrixTranspose(this),
          u_ProjectionMatrixTranspose(this), u_ColorModulate(this), u_Color(this),
          u_Bones(this), u_VertexInterpolation(this), GLDeformStage(this),
          GLCompileMacro_USE_VERTEX_SKINNING(this),
          GLCompileMacro_USE_VERTEX_ANIMATION(this),
          GLCompileMacro_USE_VERTEX_SPRITE(this) {
}
...
void GLShader_heatHaze::SetShaderProgramUniforms(
        shaderProgram_t* shaderProgram) {
...
GLShader_portal::GLShader_portal(GLShaderManager* manager)
        : GLShader("portal", ATTR_POSITION, manager), u_ModelViewMatrix(this),
          u_ModelViewProjectionMatrix(this), u_PortalRange(this) {
}
...
void GLShader_contrast::SetShaderProgramUniforms(
        shaderProgram_t* shaderProgram) {
    glUniform1i(glGetUniformLocation(shaderProgram->program, "u_ColorMap"), 0);
}
...
GLShader_cameraEffects::GLShader_cameraEffects(GLShaderManager* manager)
        : GLShader("cameraEffects", ATTR_POSITION | ATTR_TEXCOORD, manager),
          u_ColorModulate(this), u_ColorTextureMatrix(this),
          u_ModelViewProjectionMatrix(this), u_DeformMagnitude(this),
          u_InverseGamma(this) {
}

void GLShader_cameraEffects::SetShaderProgramUniforms(
        shaderProgram_t* shaderProgram) {
    glUniform1i(glGetUniformLocation(shaderProgram->program, "u_CurrentMap"), 0);
...
void GLShader_debugShadowMap::SetShaderProgramUniforms(
        shaderProgram_t* shaderProgram) {
    glUniform1i(glGetUniformLocation(shaderProgram->program, "u_CurrentMap"), 0);
}
...
void GLShader_depthToColor::BuildShaderVertexLibNames(
        std::string& vertexInlines) {
    vertexInlines += "vertexSimple vertexSkinning ";
}

GLShader_lightVolume_omni::GLShader_lightVolume_omni(GLShaderManager* manager)
        : GLShader("lightVolume_omni", ATTR_POSITION, manager), u_ViewOrigin(this),
          u_LightOrigin(this), u_LightColor(this), u_LightRadius(this),
          u_LightScale(this), u_LightAttenuationMatrix(this),
          u_ModelViewProjectionMatrix(this), u_UnprojectMatrix(this),
          GLCompileMacro_USE_SHADOWING(this) {
}

void GLShader_lightVolume_omni::SetShaderProgramUniforms(
        shaderProgram_t* shaderProgram) {
    glUniform1i(glGetUniformLocation(shaderProgram->program, "u_DepthMap"), 0);
    glUniform1i(
            glGetUniformLocation(shaderProgram->program, "u_AttenuationMapXY"), 1);
    glUniform1i(glGetUniformLocation(shaderProgram->program, "u_AttenuationMapZ"),
                2);
...
GLShader_liquid::GLShader_liquid(GLShaderManager* manager)
        : GLShader("liquid", ATTR_POSITION | ATTR_TEXCOORD | ATTR_QTANGENT, manager),
          u_NormalTextureMatrix(this), u_ViewOrigin(this), u_RefractionIndex(this),
          u_ModelMatrix(this), u_ModelViewProjectionMatrix(this),
          u_UnprojectMatrix(this), u_FresnelPower(this), u_FresnelScale(this),
          u_FresnelBias(this), u_NormalScale(this), u_FogDensity(this),
          u_FogColor(this), u_SpecularExponent(this), u_LightGridOrigin(this),
          u_LightGridScale(this), GLCompileMacro_USE_PARALLAX_MAPPING(this) {
}
...
GLShader_volumetricFog::GLShader_volumetricFog(GLShaderManager* manager)
        : GLShader("volumetricFog", ATTR_POSITION, manager), u_ViewOrigin(this),
          u_UnprojectMatrix(this), u_ModelViewMatrix(this), u_FogDensity(this),
          u_FogColor(this) {
}

void GLShader_volumetricFog::SetShaderProgramUniforms(
        shaderProgram_t* shaderProgram) {
    glUniform1i(glGetUniformLocation(shaderProgram->program, "u_DepthMap"), 0);
    glUniform1i(glGetUniformLocation(shaderProgram->program, "u_DepthMapBack"),
                1);
    glUniform1i(glGetUniformLocation(shaderProgram->program, "u_DepthMapFront"),
                2);
}

GLShader_motionblur::GLShader_motionblur(GLShaderManager* manager)
        : GLShader("motionblur", ATTR_POSITION, manager), u_blurVec(this) {
}

void GLShader_motionblur::SetShaderProgramUniforms(
        shaderProgram_t* shaderProgram) {
    glUniform1i(glGetUniformLocation(shaderProgram->program, "u_ColorMap"), 0);
    glUniform1i(glGetUniformLocation(shaderProgram->program, "u_DepthMap"), 1);
}
