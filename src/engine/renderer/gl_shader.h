...
struct GLShaderHeader {
    ...
    unsigned int macros[MAX_SHADER_MACROS]; // macros the shader uses ( may or
                                            // may not be enabled )
...
protected:
    ...
    GLShader(const std::string& name, uint32_t vertexAttribsRequired, GLShaderManager* manager)
            : _name(name), _mainShaderName(name), _activeMacros(0), _checkSum(0),
              _currentProgram(nullptr), _vertexAttribsRequired(vertexAttribsRequired),
              _vertexAttribs(0), _shaderManager(manager), _uniformStorageSize(0) {}

    GLShader(const std::string& name, const std::string& mainShaderName, uint32_t vertexAttribsRequired, GLShaderManager* manager)
            : _name(name), _mainShaderName(mainShaderName), _activeMacros(0),
              _checkSum(0), _currentProgram(nullptr),
              _vertexAttribsRequired(vertexAttribsRequired), _vertexAttribs(0),
              _shaderManager(manager), _uniformStorageSize(0) {}
...
public:
    void RegisterUniform(GLUniform* uniform) { _uniforms.push_back(uniform); }

    void RegisterCompileMacro(GLCompileMacro* compileMacro) {
        if (_compileMacros.size() >= MAX_SHADER_MACROS) {
            ri.Error(ERR_DROP,
                     "Can't register more than %u compile macros for a single shader",
                     MAX_SHADER_MACROS);
        }
    ...
    const std::string& GetMainShaderName() const { return _mainShaderName; }

protected:
    bool GetCompileMacrosString(size_t permutation,
...
public:
    ...
    bool IsMacroSet(int bit) { return (_activeMacros & bit) != 0; }

    void AddMacroBit(int bit) { _activeMacros |= bit; }

    void DelMacroBit(int bit) { _activeMacros &= ~bit; }

    bool IsVertexAtttribSet(int bit) { return (_vertexAttribs & bit) != 0; }

    void AddVertexAttribBit(int bit) { _vertexAttribs |= bit; }

    void DelVertexAttribBit(int bit) { _vertexAttribs &= ~bit; }
};
...
private:
    ...
    void CompileAndLinkGPUShaderProgram(GLShader* shader,
    ...
    void
    UpdateShaderProgramUniformLocations(GLShader* shader,
                                        shaderProgram_t* shaderProgram) const;
};
...
protected:
    ...
    GLUniform(GLShader* shader, const char* name)
            : _shader(shader), _name(name), _firewallIndex(0), _locationIndex(0) {
        _shader->RegisterUniform(this);
    }

public:
    void SetFirewallIndex(size_t offSetValue) { _firewallIndex = offSetValue; }

    void SetLocationIndex(size_t index) { _locationIndex = index; }

    const char* GetName() { return _name.c_str(); }

    void UpdateShaderProgramUniformLocation(shaderProgram_t* shaderProgram) {
        shaderProgram->uniformLocations[_locationIndex] =
                glGetUniformLocation(shaderProgram->program, GetName());
    }
...
protected:
    ...
    inline void SetValue(float value) {
        ...
        if (r_logFile->integer) {
            GLimp_LogComment(
                    va("GLSL_SetUniform1f( %s, shader: %s, value: %f ) ---\n",
                       this->GetName(),
...
public:
    size_t GetSize() { return sizeof(float); }
};
...
protected:
    ...
    inline void SetValue(int numFloats, float* f) {
        ...
        if (r_logFile->integer) {
            GLimp_LogComment(
                    va("GLSL_SetUniform1fv( %s, shader: %s, numFloats: %d ) ---\n",
                       this->GetName(),
                       _shader->GetName().c_str(),
                       numFloats));
        }
...
protected:
    ...
    inline void SetValue(const vec2_t v) {
        ...
        if (r_logFile->integer) {
            GLimp_LogComment(
                    va("GLSL_SetUniform2f( %s, shader: %s, value: [ %f, %f ] ) ---\n",
                       this->GetName(),
                       _shader->GetName().c_str(),
                       v[0],
                       v[1]));
    ...
    size_t GetSize() { return sizeof(vec2_t); }
};
...
protected:
    ...
    inline void SetValue(const vec3_t v) {
        ...
        if (r_logFile->integer) {
            GLimp_LogComment(
                    va("GLSL_SetUniform3f( %s, shader: %s, value: [ %f, %f, %f ] ) ---\n",
                       this->GetName(),
                       _shader->GetName().c_str(),
                       v[0],
                       v[1],
...
public:
    size_t GetSize() { return sizeof(vec3_t); }
};
...
protected:
    ...
    inline void SetValue(const vec4_t v) {
        ...
        if (r_logFile->integer) {
            GLimp_LogComment(va(
                    "GLSL_SetUniform4f( %s, shader: %s, value: [ %f, %f, %f, %f ] ) "
                    "---\n",
                    this->GetName(),
                    _shader->GetName().c_str(),
                    v[0],
...
public:
    size_t GetSize() { return sizeof(vec4_t); }
};
...
protected:
    ...
    inline void SetValue(int numV, vec4_t* v) {
        ...
        if (r_logFile->integer) {
            GLimp_LogComment(
                    va("GLSL_SetUniform4fv( %s, shader: %s, numV: %d ) ---\n",
                       this->GetName(),
                       _shader->GetName().c_str(),
                       numV));
        }
...
protected:
    ...
    inline void SetValue(GLboolean transpose, const matrix_t m) {
        ...
        if (r_logFile->integer) {
            GLimp_LogComment(va(
                    "GLSL_SetUniformMatrix4f( %s, shader: %s, transpose: %d, [ %f, %f, "
...
public:
    size_t GetSize() { return sizeof(matrix_t); }
};
...
protected:
    ...
    inline void SetValue(int numMatrices, GLboolean transpose, const matrix_t* m) {
        ...
        if (r_logFile->integer) {
            GLimp_LogComment(va(
                    "GLSL_SetUniformMatrix4fv( %s, shader: %s, numMatrices: %d, "
                    "transpose: %d ) ---\n",
                    this->GetName(),
                    _shader->GetName().c_str(),
                    numMatrices,
...
protected:
    ...
    inline void SetValue(int numMatrices, GLboolean transpose, const float* m) {
        ...
        if (r_logFile->integer) {
            GLimp_LogComment(va(
                    "GLSL_SetUniformMatrix34fv( %s, shader: %s, numMatrices: %d, "
                    "transpose: %d ) ---\n",
                    this->GetName(),
                    _shader->GetName().c_str(),
                    numMatrices,
...
protected:
    ...
    // RB: This is not good oo design, but it can be a workaround and its cost is
    // more or less only a virtual function call.
...
public:
    ...
    virtual bool
    HasConflictingMacros(size_t permutation,
                         const std::vector<GLCompileMacro*>& macros) const {
        return false;
    }

    virtual bool
    MissesRequiredMacros(size_t permutation,
                         const std::vector<GLCompileMacro*>& macros) const {
        return false;
    }
...
class GLCompileMacro_USE_VERTEX_SKINNING : GLCompileMacro {
public:
    ...
    bool HasConflictingMacros(size_t permutation,
                              const std::vector<GLCompileMacro*>& macros) const;
    bool MissesRequiredMacros(size_t permutation,
                              const std::vector<GLCompileMacro*>& macros) const;

    uint32_t GetRequiredVertexAttributes() const { return ATTR_BONE_FACTORS; }
...
class GLCompileMacro_USE_VERTEX_ANIMATION : GLCompileMacro {
public:
...
class GLCompileMacro_USE_VERTEX_SPRITE : GLCompileMacro {
public:
    GLCompileMacro_USE_VERTEX_SPRITE(GLShader* shader)
            : GLCompileMacro(shader) {}

    const char* GetName() const { return "USE_VERTEX_SPRITE"; }

    EGLCompileMacro GetType() const { return USE_VERTEX_SPRITE; }

    bool HasConflictingMacros(size_t permutation,
                              const std::vector<GLCompileMacro*>& macros) const;
    uint32_t GetRequiredVertexAttributes() const { return ATTR_QTANGENT; }

    void EnableVertexSprite() { EnableMacro(); }

    void DisableVertexSprite() { DisableMacro(); }
...
class GLCompileMacro_USE_TCGEN_ENVIRONMENT : GLCompileMacro {
public:
...
class GLCompileMacro_USE_TCGEN_LIGHTMAP : GLCompileMacro {
public:
    ...
    void EnableTCGenLightmap() { EnableMacro(); }

    void DisableTCGenLightmap() { DisableMacro(); }
...
class GLCompileMacro_USE_NORMAL_MAPPING : GLCompileMacro {
public:
...
class GLCompileMacro_USE_PARALLAX_MAPPING : GLCompileMacro {
public:
...
class GLCompileMacro_USE_REFLECTIVE_SPECULAR : GLCompileMacro {
public:
...
class GLCompileMacro_TWOSIDED : GLCompileMacro {
public:
...
class GLCompileMacro_LIGHT_DIRECTIONAL : GLCompileMacro {
public:
...
class GLCompileMacro_USE_SHADOWING : GLCompileMacro {
public:
...
class GLCompileMacro_USE_GLOW_MAPPING : GLCompileMacro {
public:
    GLCompileMacro_USE_GLOW_MAPPING(GLShader* shader)
            : GLCompileMacro(shader) {}

    const char* GetName() const { return "USE_GLOW_MAPPING"; }

    EGLCompileMacro GetType() const { return USE_GLOW_MAPPING; }

    void EnableMacro_USE_GLOW_MAPPING() { EnableMacro(); }

    void DisableMacro_USE_GLOW_MAPPING() { DisableMacro(); }
...
class GLCompileMacro_USE_DEPTH_FADE : GLCompileMacro {
public:
    GLCompileMacro_USE_DEPTH_FADE(GLShader* shader)
            : GLCompileMacro(shader) {}

    const char* GetName() const { return "USE_DEPTH_FADE"; }

    EGLCompileMacro GetType() const { return USE_DEPTH_FADE; }

    void EnableMacro_USE_DEPTH_FADE() { EnableMacro(); }

    void DisableMacro_USE_DEPTH_FADE() { DisableMacro(); }
...
class u_ColorTextureMatrix : GLUniformMatrix4f {
public:
...
class u_DiffuseTextureMatrix : GLUniformMatrix4f {
public:
...
class u_NormalTextureMatrix : GLUniformMatrix4f {
public:
...
class u_SpecularTextureMatrix : GLUniformMatrix4f {
public:
...
class u_GlowTextureMatrix : GLUniformMatrix4f {
public:
...
class u_AlphaThreshold : GLUniform1f {
public:
...
class u_ViewOrigin : GLUniform3f {
public:
    u_ViewOrigin(GLShader* shader)
            : GLUniform3f(shader, "u_ViewOrigin") {}

    void SetUniform_ViewOrigin(const vec3_t v) { this->SetValue(v); }
};

class u_ViewUp : GLUniform3f {
public:
    u_ViewUp(GLShader* shader)
            : GLUniform3f(shader, "u_ViewUp") {}

    void SetUniform_ViewUp(const vec3_t v) { this->SetValue(v); }
};

class u_LightDir : GLUniform3f {
public:
    u_LightDir(GLShader* shader)
            : GLUniform3f(shader, "u_LightDir") {}

    void SetUniform_LightDir(const vec3_t v) { this->SetValue(v); }
};

class u_LightOrigin : GLUniform3f {
public:
    u_LightOrigin(GLShader* shader)
            : GLUniform3f(shader, "u_LightOrigin") {}

    void SetUniform_LightOrigin(const vec3_t v) { this->SetValue(v); }
};

class u_LightColor : GLUniform3f {
public:
    u_LightColor(GLShader* shader)
            : GLUniform3f(shader, "u_LightColor") {}

    void SetUniform_LightColor(const vec3_t v) { this->SetValue(v); }
};

class u_LightRadius : GLUniform1f {
public:
    u_LightRadius(GLShader* shader)
            : GLUniform1f(shader, "u_LightRadius") {}

    void SetUniform_LightRadius(float value) { this->SetValue(value); }
};

class u_LightScale : GLUniform1f {
public:
    u_LightScale(GLShader* shader)
            : GLUniform1f(shader, "u_LightScale") {}

    void SetUniform_LightScale(float value) { this->SetValue(value); }
};

class u_LightWrapAround : GLUniform1f {
public:
    u_LightWrapAround(GLShader* shader)
            : GLUniform1f(shader, "u_LightWrapAround") {}

    void SetUniform_LightWrapAround(float value) { this->SetValue(value); }
};

class u_LightAttenuationMatrix : GLUniformMatrix4f {
public:
...
class u_LightFrustum : GLUniform4fv {
public:
...
class u_ShadowTexelSize : GLUniform1f {
public:
    u_ShadowTexelSize(GLShader* shader)
            : GLUniform1f(shader, "u_ShadowTexelSize") {}

    void SetUniform_ShadowTexelSize(float value) { this->SetValue(value); }
};

class u_ShadowBlur : GLUniform1f {
public:
    u_ShadowBlur(GLShader* shader)
            : GLUniform1f(shader, "u_ShadowBlur") {}

    void SetUniform_ShadowBlur(float value) { this->SetValue(value); }
};

class u_ShadowMatrix : GLUniformMatrix4fv {
public:
...
class u_ShadowParallelSplitDistances : GLUniform4f {
public:
...
class u_RefractionIndex : GLUniform1f {
public:
    u_RefractionIndex(GLShader* shader)
            : GLUniform1f(shader, "u_RefractionIndex") {}

    void SetUniform_RefractionIndex(float value) { this->SetValue(value); }
};

class u_FresnelPower : GLUniform1f {
public:
    u_FresnelPower(GLShader* shader)
            : GLUniform1f(shader, "u_FresnelPower") {}

    void SetUniform_FresnelPower(float value) { this->SetValue(value); }
};

class u_FresnelScale : GLUniform1f {
public:
    u_FresnelScale(GLShader* shader)
            : GLUniform1f(shader, "u_FresnelScale") {}

    void SetUniform_FresnelScale(float value) { this->SetValue(value); }
};

class u_FresnelBias : GLUniform1f {
public:
    u_FresnelBias(GLShader* shader)
            : GLUniform1f(shader, "u_FresnelBias") {}

    void SetUniform_FresnelBias(float value) { this->SetValue(value); }
};

class u_NormalScale : GLUniform1f {
public:
    u_NormalScale(GLShader* shader)
            : GLUniform1f(shader, "u_NormalScale") {}

    void SetUniform_NormalScale(float value) { this->SetValue(value); }
};

class u_FogDensity : GLUniform1f {
public:
    u_FogDensity(GLShader* shader)
            : GLUniform1f(shader, "u_FogDensity") {}

    void SetUniform_FogDensity(float value) { this->SetValue(value); }
};

class u_FogColor : GLUniform3f {
public:
    u_FogColor(GLShader* shader)
            : GLUniform3f(shader, "u_FogColor") {}

    void SetUniform_FogColor(const vec3_t v) { this->SetValue(v); }
};

class u_Color : GLUniform4f {
public:
    u_Color(GLShader* shader)
            : GLUniform4f(shader, "u_Color") {}

    void SetUniform_Color(const vec4_t v) { this->SetValue(v); }
};

class u_ModelMatrix : GLUniformMatrix4f {
public:
    u_ModelMatrix(GLShader* shader)
            : GLUniformMatrix4f(shader, "u_ModelMatrix") {}

    void SetUniform_ModelMatrix(const matrix_t m) { this->SetValue(GL_FALSE, m); }
};

class u_ViewMatrix : GLUniformMatrix4f {
public:
    u_ViewMatrix(GLShader* shader)
            : GLUniformMatrix4f(shader, "u_ViewMatrix") {}

    void SetUniform_ViewMatrix(const matrix_t m) { this->SetValue(GL_FALSE, m); }
};

class u_ModelViewMatrix : GLUniformMatrix4f {
public:
...
class u_ModelViewMatrixTranspose : GLUniformMatrix4f {
public:
...
class u_ProjectionMatrixTranspose : GLUniformMatrix4f {
public:
...
class u_ModelViewProjectionMatrix : GLUniformMatrix4f {
public:
...
class u_UnprojectMatrix : GLUniformMatrix4f {
public:
...
class u_VertexInterpolation : GLUniform1f {
public:
    u_VertexInterpolation(GLShader* shader)
            : GLUniform1f(shader, "u_VertexInterpolation") {}

    void SetUniform_VertexInterpolation(float value) { this->SetValue(value); }
};

class u_PortalRange : GLUniform1f {
public:
    u_PortalRange(GLShader* shader)
            : GLUniform1f(shader, "u_PortalRange") {}

    void SetUniform_PortalRange(float value) { this->SetValue(value); }
};

class u_DepthScale : GLUniform1f {
public:
    u_DepthScale(GLShader* shader)
            : GLUniform1f(shader, "u_DepthScale") {}

    void SetUniform_DepthScale(float value) { this->SetValue(value); }
};

class u_EnvironmentInterpolation : GLUniform1f {
public:
...
class u_DeformParms : GLUniform4fv {
public:
    u_DeformParms(GLShader* shader)
            : GLUniform4fv(shader, "u_DeformParms") {}

    void SetUniform_DeformParms(deformStage_t deforms[MAX_SHADER_DEFORMS],
                                int numDeforms) {
...
class u_Time : GLUniform1f {
public:
    u_Time(GLShader* shader)
            : GLUniform1f(shader, "u_Time") {}

    void SetUniform_Time(float value) { this->SetValue(value); }
};

class GLDeformStage : public u_DeformParms, public u_Time {
public:
    GLDeformStage(GLShader* shader)
            : u_DeformParms(shader), u_Time(shader) {}
};

class u_ColorModulate : GLUniform4f {
public:
    u_ColorModulate(GLShader* shader)
            : GLUniform4f(shader, "u_ColorModulate") {}

    void SetUniform_ColorModulate(vec4_t v) { this->SetValue(v); }
    void SetUniform_ColorModulate(colorGen_t colorGen, alphaGen_t alphaGen) {
        vec4_t v;

        if (r_logFile->integer) {
            GLimp_LogComment(
                    va("--- u_ColorModulate::SetUniform_ColorModulate( program = %s, "
                       "colorGen = %i, alphaGen = %i ) ---\n",
                       _shader->GetName().c_str(),
                       colorGen,
                       alphaGen));
        }
...
class u_FogDistanceVector : GLUniform4f {
public:
    u_FogDistanceVector(GLShader* shader)
            : GLUniform4f(shader, "u_FogDistanceVector") {}

    void SetUniform_FogDistanceVector(const vec4_t v) { this->SetValue(v); }
};

class u_FogDepthVector : GLUniform4f {
public:
    u_FogDepthVector(GLShader* shader)
            : GLUniform4f(shader, "u_FogDepthVector") {}

    void SetUniform_FogDepthVector(const vec4_t v) { this->SetValue(v); }
};

class u_FogEyeT : GLUniform1f {
public:
    u_FogEyeT(GLShader* shader)
            : GLUniform1f(shader, "u_FogEyeT") {}

    void SetUniform_FogEyeT(float value) { this->SetValue(value); }
};

class u_DeformMagnitude : GLUniform1f {
public:
    u_DeformMagnitude(GLShader* shader)
            : GLUniform1f(shader, "u_DeformMagnitude") {}

    void SetUniform_DeformMagnitude(float value) { this->SetValue(value); }
};

class u_blurVec : GLUniform3f {
public:
    u_blurVec(GLShader* shader)
            : GLUniform3f(shader, "u_blurVec") {}

    void SetUniform_blurVec(vec3_t value) { this->SetValue(value); }
};

class u_TexScale : GLUniform2f {
public:
    u_TexScale(GLShader* shader)
            : GLUniform2f(shader, "u_TexScale") {}

    void SetUniform_TexScale(vec2_t value) { this->SetValue(value); }
};

class u_SpecularExponent : GLUniform2f {
public:
    ...
    void SetUniform_SpecularExponent(float min, float max) {
        ...
        // we precompute ( max - min ) before sending the uniform to the fragment
        // shader
...
class u_InverseGamma : GLUniform1f {
public:
    u_InverseGamma(GLShader* shader)
            : GLUniform1f(shader, "u_InverseGamma") {}

    void SetUniform_InverseGamma(float value) { this->SetValue(value); }
};

class u_LightGridOrigin : GLUniform3f {
public:
    u_LightGridOrigin(GLShader* shader)
            : GLUniform3f(shader, "u_LightGridOrigin") {}

    void SetUniform_LightGridOrigin(vec3_t origin) { this->SetValue(origin); }
};

class u_LightGridScale : GLUniform3f {
public:
    u_LightGridScale(GLShader* shader)
            : GLUniform3f(shader, "u_LightGridScale") {}

    void SetUniform_LightGridScale(vec3_t scale) { this->SetValue(scale); }
};

class GLShader_generic : public GLShader,
...
class GLShader_lightMapping : public GLShader,
...
class GLShader_shadowFill : public GLShader,
                            public u_ColorTextureMatrix,
                            public u_ViewOrigin,
                            public u_AlphaThreshold,
                            public u_LightOrigin,
...
class GLShader_reflection : public GLShader,
                            public u_NormalTextureMatrix,
                            public u_ViewOrigin,
                            public u_ModelMatrix,
                            public u_ModelViewProjectionMatrix,
...
class GLShader_skybox : public GLShader,
                        public u_ViewOrigin,
                        public u_ModelMatrix,
                        public u_ModelViewProjectionMatrix,
                        public u_Bones,
...
class GLShader_fogQuake3 : public GLShader,
                           public u_ModelMatrix,
                           public u_ModelViewProjectionMatrix,
                           public u_Color,
                           public u_Bones,
...
class GLShader_fogGlobal : public GLShader,
                           public u_ViewOrigin,
                           public u_ViewMatrix,
                           public u_ModelViewProjectionMatrix,
...
class GLShader_heatHaze : public GLShader,
                          public u_NormalTextureMatrix,
                          public u_ViewOrigin,
                          public u_ViewUp,
...
class GLShader_screen : public GLShader, public u_ModelViewProjectionMatrix {
public:
    GLShader_screen(GLShaderManager* manager);
    void SetShaderProgramUniforms(shaderProgram_t* shaderProgram);
};

class GLShader_portal : public GLShader,
...
class GLShader_contrast : public GLShader, public u_ModelViewProjectionMatrix {
public:
    GLShader_contrast(GLShaderManager* manager);
    void SetShaderProgramUniforms(shaderProgram_t* shaderProgram);
};

class GLShader_cameraEffects : public GLShader,
                               public u_ColorModulate,
                               public u_ColorTextureMatrix,
                               public u_ModelViewProjectionMatrix,
                               public u_DeformMagnitude,
...
class GLShader_blurX : public GLShader,
                       public u_ModelViewProjectionMatrix,
                       public u_DeformMagnitude,
                       public u_TexScale {
public:
...
class GLShader_blurY : public GLShader,
                       public u_ModelViewProjectionMatrix,
                       public u_DeformMagnitude,
                       public u_TexScale {
public:
...
class GLShader_debugShadowMap : public GLShader,
                                public u_ModelViewProjectionMatrix {
public:
...
class GLShader_depthToColor : public GLShader,
                              public u_ModelViewProjectionMatrix,
                              public u_Bones,
                              public GLCompileMacro_USE_VERTEX_SKINNING {
public:
...
class GLShader_lightVolume_omni : public GLShader,
                                  public u_ViewOrigin,
                                  public u_LightOrigin,
                                  public u_LightColor,
                                  public u_LightRadius,
...
class GLShader_liquid : public GLShader,
                        public u_NormalTextureMatrix,
                        public u_ViewOrigin,
...
class GLShader_volumetricFog : public GLShader,
                               public u_ViewOrigin,
                               public u_UnprojectMatrix,
                               public u_ModelViewMatrix,
                               public u_FogDensity,
...
class GLShader_motionblur : public GLShader, public u_blurVec {
public:
    GLShader_motionblur(GLShaderManager* manager);
    void SetShaderProgramUniforms(shaderProgram_t* shaderProgram);
};

class GLShader_fxaa : public GLShader {
public:
...
extern GLShader_forwardLighting_directionalSun*
