...
static void R_CullMDV(mdvModel_t* model, trRefEntity_t* ent) {
    ...
    for (i = 0; i < 3; i++) {
        ent->localBounds[0][i] = oldFrame->bounds[0][i] < newFrame->bounds[0][i]
                                         ? oldFrame->bounds[0][i]
                                         : newFrame->bounds[0][i];
        ent->localBounds[1][i] = oldFrame->bounds[1][i] > newFrame->bounds[1][i]
                                         ? oldFrame->bounds[1][i]
                                         : newFrame->bounds[1][i];
    }
    ...
    if (!ent->e.nonNormalizedAxes) {
        if (ent->e.frame == ent->e.oldframe) {
            switch (
                    R_CullLocalPointAndRadius(newFrame->localOrigin, newFrame->radius)) {
                case CULL_OUT:
...
static shader_t* GetMDVSurfaceShader(const trRefEntity_t* ent,
...
void R_AddMDVSurfaces(trRefEntity_t* ent) {
    ...
    if ((ent->e.frame >= tr.currentModel->mdv[lod]->numFrames) ||
        (ent->e.frame < 0) ||
        (ent->e.oldframe >= tr.currentModel->mdv[lod]->numFrames) ||
        (ent->e.oldframe < 0)) {
        ri.Printf(PRINT_DEVELOPER,
                  ...
                  tr.currentModel->mdv[lod]->numFrames);
        ent->e.frame = 0;
        ent->e.oldframe = 0;
    }
    ...
    if (r_vboModels->integer && model->numVBOSurfaces) {
        ...
        for (i = 0, mdvSurface = model->surfaces; i < model->numSurfaces;
             i++, mdvSurface++) {
            shader = GetMDVSurfaceShader(ent, mdvSurface);
