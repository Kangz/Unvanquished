...
void R_RemapShader(const char* shaderName, const char* newShaderName, const char* timeOffset) {
    ...
    if (sh->autoSpriteMode != sh2->autoSpriteMode) {
        ri.Printf(PRINT_WARNING,
                  "WARNING: R_RemapShader: shaders %s and %s have different "
                  "autoSprite modes\n",
                  shaderName,
                  newShaderName);
        return;
...
const opstring_t opStrings[] = {{"bad", OP_BAD},
...
static char* ParseExpressionElement(const char** data_p) {
    ...
    static const char* const punctuation[] = {"&&", "||", "<=", ">=", "==", "!=", nullptr};
...
static void ParseExpression(const char** text, expression_t* exp) {
    ...
    while (1) {
        ...
        if (numInFixOps == MAX_EXPRESSION_OPS) {
            ri.Printf(
                    PRINT_ALL,
                    "WARNING: too many arithmetic expression operations in shader '%s'\n",
                    shader.name);
            SkipRestOfLine(text);
            return;
        }
        ...
        switch (op.type) {
            case OP_BAD:
                ri.Printf(PRINT_ALL,
                          "WARNING: unknown token '%s' for arithmetic expression in "
                          "shader '%s'\n",
                          token,
                          shader.name);
                break;
    ...
    for (i = 0; i < (numInFixOps - 1); i++) {
        ...
        if (op2.type == OP_SUB && op.type != OP_RPAREN && op.type != OP_TABLE &&
            !IsOperand(op.type)) {
    ...
    for (i = 0; i < numInFixOps; i++) {
        ...
        // if current operator in infix is operator
            ...
            while (true) {
                if (!numTmpOps) {
                    ri.Printf(PRINT_ALL,
                              "WARNING: invalid infix expression in shader '%s'\n",
                              shader.name);
                    return;
                } else {
                    // get top element
...
static bool ParseMap(shaderStage_t* stage, const char** text, char* buffer, int bufferSize) {
    ...
    // addnormals (textures/caves/tembrick1crum_local.tga, heightmap
    // (textures/caves/tembrick1crum_bmp.tga, 3 ))
    ...
    if (!buffer[0]) {
        ri.Printf(PRINT_WARNING,
                  "WARNING: 'map' missing parameter in shader '%s'\n",
                  shader.name);
        return false;
    }
...
static bool LoadMap(shaderStage_t* stage, const char* buffer) {
    ...
    if (!buffer || !buffer[0]) {
        ri.Printf(PRINT_WARNING,
                  "WARNING: NULL parameter for LoadMap in shader '%s'\n",
                  shader.name);
        return false;
    }
    ...
    if (stage->overrideNoPicMip || shader.noPicMip || stage->highQuality ||
        stage->forceHighQuality) {
    ...
    stage->bundle[0].image[0] =
            R_FindImageFile(buffer, imageBits, filterType, wrapType, shader.name);

    if (!stage->bundle[0].image[0]) {
        ri.Printf(
                PRINT_WARNING,
                "WARNING: R_FindImageFile could not find image '%s' in shader '%s'\n",
                buffer,
                shader.name);
        return false;
    }
...
static bool ParseStage(shaderStage_t* stage, const char** text) {
    ...
    int depthMaskBits = GLS_DEPTHMASK_TRUE, blendSrcBits = 0, blendDstBits = 0,
        atestBits = 0, depthFuncBits = 0, polyModeBits = 0;
    ...
    while (1) {
        ...
        // clampmap <name>
            ...
            stage->bundle[0].image[0] =
                    R_FindImageFile(token, imageBits, filterType, WT_CLAMP, shader.name);
        ...
        // animMap <frequency> <image1> .... <imageN>
            ...
            while (1) {
                ...
                if (num < MAX_IMAGE_ANIMATIONS) {
                    stage->bundle[0].image[num] = R_FindImageFile(
                            token, IF_NONE, FT_DEFAULT, WT_REPEAT, shader.name);
            ...
            stage->bundle[0].videoMapHandle = ri.CIN_PlayCinematic(
                    token, 0, 0, 512, 512, (CIN_loop | CIN_silent | CIN_shader));
        ...
        // cubeMap <map>
                 ...
                 !Q_stricmp(token, "cameraCubeMap")) {
            token = COM_ParseExt2(text, false);

            if (!token[0]) {
                ri.Printf(
                        PRINT_WARNING,
                        "WARNING: missing parameter for 'cubeMap' keyword in shader '%s'\n",
                        shader.name);
                return false;
            }
            ...
            if (!stage->bundle[0].image[0]) {
                ri.Printf(
                        PRINT_WARNING,
                        "WARNING: R_FindCubeImage could not find '%s' in shader '%s'\n",
                        token,
                        shader.name);
                return false;
            }
        ...
        // ET fog
            ...
            if (!token[0]) {
                ri.Printf(PRINT_WARNING,
                          "WARNING: missing parameter for 'depthFade' keyword in "
                          "shader '%s'\n",
                          shader.name);
                return false;
            }
        ...
        // or blend <diffusemap | bumpmap | specularmap>
            ...
            if (token[0] == 0) {
                ri.Printf(PRINT_WARNING,
                          "WARNING: missing parm for blend in shader '%s'\n",
                          shader.name);
                continue;
            }
            ...
            // check for other semantic meanings
                ...
                if (token[0] != ',') {
                    ri.Printf(PRINT_WARNING,
                              "WARNING: expecting ',', found '%s' instead for blend in "
                              "shader '%s'\n",
                              token,
                              shader.name);
                    continue;
        ...
        // rgbGen
            ...
            if (!Q_stricmp(token, "wave")) {
                ...
                // ri.Printf(PRINT_WARNING, "WARNING: obsolete rgbGen lightingDiffuse
                // keyword not supported in shader '%s'\n", shader.name);
                stage->type = ST_DIFFUSEMAP;
                stage->rgbGen = CGEN_IDENTITY_LIGHTING;
        ...
        // alphaGen
            ...
            if (!Q_stricmp(token, "wave")) {
                ...
                ri.Printf(PRINT_WARNING,
                          "WARNING: alphaGen lightingSpecular keyword not supported in "
                          "shader '%s'\n",
                          shader.name);
            } else if (!Q_stricmp(token, "oneMinusVertex")) {
                stage->alphaGen = AGEN_ONE_MINUS_VERTEX;
                ...
                ri.Printf(
                        PRINT_WARNING,
                        "WARNING: alphaGen portal keyword not supported in shader '%s'\n",
                        shader.name);
                // stage->type = ST_PORTALMAP;
                stage->alphaGen = AGEN_CONST;
        ...
        // tcGen <function>
            ...
            if (!Q_stricmp(token, "environment")) {
                // ri.Printf(PRINT_WARNING, "WARNING: texGen environment keyword not
                ...
                ri.Printf(PRINT_WARNING,
                          "WARNING: unknown tcGen keyword '%s' not supported in shader "
                          "'%s'\n",
                          token,
                          shader.name);
                SkipRestOfLine(text);
        ...
        // maskColor
            ...
            colorMaskBits |=
                    GLS_REDMASK_FALSE | GLS_GREENMASK_FALSE | GLS_BLUEMASK_FALSE;
        }
        // maskColorAlpha
        else if (!Q_stricmp(token, "maskColorAlpha")) {
            colorMaskBits |= GLS_REDMASK_FALSE | GLS_GREENMASK_FALSE |
        ...
        // wrapAroundLighting <arithmetic expression>
            ...
            ri.Printf(PRINT_WARNING,
                      "WARNING: unknown shader stage parameter '%s' in shader '%s'\n",
                      token,
                      shader.name);
            SkipRestOfLine(text);
            return false;
    ...
    // compute state bits
    stage->stateBits = colorMaskBits | depthMaskBits | blendSrcBits |
                       blendDstBits | atestBits | depthFuncBits | polyModeBits;
...
static void ParseDeform(const char** text) {
    ...
    if (!Q_stricmp(token, "flare")) {
        ...
        if (token[0] == 0) {
            ri.Printf(PRINT_WARNING,
                      "WARNING: missing deformVertexes flare parm in shader '%s'\n",
                      shader.name);
            return;
        }
...
static void ParseSkyParms(const char** text) {
    ...
    if (strcmp(token, "-")) {
        ...
        if (!shader.sky.outerbox) {
            ri.Printf(PRINT_WARNING,
                      "WARNING: could not find cubemap '%s' for outer skybox in "
                      "shader '%s'\n",
                      prefix,
                      shader.name);
    ...
    if (strcmp(token, "-")) {
        ...
        if (!shader.sky.innerbox) {
            ri.Printf(PRINT_WARNING,
                      "WARNING: could not find cubemap '%s' for inner skybox in "
                      "shader '%s'\n",
                      prefix,
                      shader.name);
...
static const infoParm_t infoParms[] = {
        ...
        {"slag", 1, 0, CONTENTS_SLIME}, // uses the CONTENTS_SLIME flag, but the
                                        // shader reference is changed to 'slag'
        // to idendify that this doesn't work the same as 'slime' did.

        {"lava", 1, 0, CONTENTS_LAVA}, // very damaging
        {"playerclip", 1, 0, CONTENTS_PLAYERCLIP},
        {"monsterclip", 1, 0, CONTENTS_MONSTERCLIP},
        {"nodrop", 1, 0, int(CONTENTS_NODROP)}, // don't drop items or leave bodies
                                                // (death fog, lava, etc)
...
static bool ParseShader(const char* _text) {
    ...
    while (1) {
        ...
        // sun parms
            ...
            if (!token[0]) {
                ri.Printf(
                        PRINT_WARNING,
                        "WARNING: missing parm for 'xmap_sun' keyword in shader '%s'\n",
                        shader.name);
                continue;
            }
        ...
        // fogParms
            ...
            /*
      ri.Printf(PRINT_WARNING, "WARNING: fogParms keyword not supported in
      shader '%s'\n", shader.name);
      SkipRestOfLine(text);
      ...
      */
        ...
        // ET sunshader <name>
            ...
            /*
      RB: don't call tr.sunShader = R_FindShader(token, SHADER_3D_STATIC,
      RSF_DEFAULT);
      ...
      */
        ...
        // ydnar: implicit default mapping to eliminate redundant/incorrect explicit
        // shader stages
            ...
            if (!Q_stricmp(token, "implicitBlend")) {
                implicitStateBits = GLS_DEPTHMASK_TRUE | GLS_SRCBLEND_SRC_ALPHA |
        ...
        // when <state> <shader name>
            ...
            for (i = 1, p = whenTokens; i < MAX_ALTSHADERS && *p;
                 ++i, p += strlen(p) + 1) {
            ...
            if (index == 0) {
                ri.Printf(PRINT_WARNING,
                          "WARNING: unknown parameter '%s' for 'when' in '%s'\n",
                          token,
                          shader.name);
            } else {
                int tokenLen;
                ...
                shader.altShader[index].name =
                        (char*) ri.Hunk_Alloc(sizeof(char) * tokenLen, h_low);
    ...
    if (s == 0 && !shader.forceOpaque && !shader.isSky &&
        !(shader.contentFlags & CONTENTS_FOG) && implicitMap[0] == '\0') {
        return false;
    }
...
static void CollapseStages() {
    ...
    for (j = 0; j < MAX_SHADER_STAGES; j++) {
        ...
        if (stages[j].type == ST_REFRACTIONMAP ||
            stages[j].type == ST_DISPERSIONMAP || stages[j].type == ST_SKYBOXMAP ||
            stages[j].type == ST_SCREENMAP || stages[j].type == ST_PORTALMAP ||
            stages[j].type == ST_HEATHAZEMAP || stages[j].type == ST_LIQUIDMAP ||
        ...
        for (i = 0; i < 4; i++) {
            ...
            } else if (stages[j + i].type == ST_REFLECTIONMAP &&
                       !hasReflectionStage) {
        ...
        if (hasDiffuseStage && hasNormalStage && hasSpecularStage && hasGlowStage) {
            ...
            tmpStages[numStages].specularExponentMin =
                    tmpSpecularStage.specularExponentMin;
            tmpStages[numStages].specularExponentMax =
                    tmpSpecularStage.specularExponentMax;
...
Arnout: this is a nasty issue. Shaders can be registered after drawsurfaces are
generated
but before the frame is rendered. This will, for the duration of one frame,
cause drawsurfaces
to be rendered with bad shaders. To fix this, need to go through all render
commands and fix
sortedIndex.
==============
*/
    ...
    if (cmdList) {
        ...
        while (1) {
            switch (*(const int*) curCmd) {
                ...
                case RC_STRETCH_PIC: {
                    const stretchPicCommand_t* sp_cmd =
                            (const stretchPicCommand_t*) curCmd;
                ...
                case RC_DRAW_VIEW: {
                    ...
                    for (i = 0, drawSurf = dv_cmd->viewParms.drawSurfs;
                         i < dv_cmd->viewParms.numDrawSurfs;
                         i++, drawSurf++) {
                        if (drawSurf->shaderNum() >= newShader) {
                            drawSurf->setSort(drawSurf->shaderNum() + 1,
                                              drawSurf->lightmapNum(),
                                              drawSurf->entityNum(),
                                              drawSurf->fogNum(),
                                              drawSurf->index());
                        }
                ...
                case RC_DRAW_BUFFER: {
                    const drawBufferCommand_t* db_cmd =
                            (const drawBufferCommand_t*) curCmd;
                ...
                case RC_SWAP_BUFFERS: {
                    const swapBuffersCommand_t* sb_cmd =
                            (const swapBuffersCommand_t*) curCmd;
    ...
    for (i = 0; i < newShader->numStages; i++) {
        ...
        *newShader->stages[i] = stages[i];
        ...
        for (b = 0; b < MAX_TEXTURE_BUNDLES; b++) {
            ...
            Com_Memcpy(newShader->stages[i]->bundle[b].texMods,
                       stages[i].bundle[b].texMods,
                       size);
        }
    }
...
static void GeneratePermanentShaderTable(float* values, int numValues) {
    ...
    if (tr.numTables == MAX_SHADER_TABLES) {
        ri.Printf(
                PRINT_WARNING,
                "WARNING: GeneratePermanentShaderTables - MAX_SHADER_TABLES hit\n");
        return;
    }
...
static shader_t* FinishShader() {
        ...
        switch (pStage->type) {
            ...
            default: {
                if (!pStage->bundle[0].image[0]) {
                    ri.Printf(PRINT_WARNING,
                              "Shader %s has a colormap stage with no image\n",
                              shader.name);
                    pStage->active = false;
                    continue;
                }
            ...
            case ST_DIFFUSEMAP: {
                ...
                if (!pStage->bundle[0].image[0]) {
                    ri.Printf(PRINT_WARNING,
                              "Shader %s has a diffusemap stage with no image\n",
                              shader.name);
            ...
            case ST_NORMALMAP: {
                ...
                if (!pStage->bundle[0].image[0]) {
                    ri.Printf(PRINT_WARNING,
                              "Shader %s has a normalmap stage with no image\n",
                              shader.name);
            ...
            case ST_SPECULARMAP: {
                if (!pStage->bundle[0].image[0]) {
                    ri.Printf(PRINT_WARNING,
                              "Shader %s has a specularmap stage with no image\n",
                              shader.name);
            ...
            case ST_ATTENUATIONMAP_XY: {
                if (!pStage->bundle[0].image[0]) {
                    ri.Printf(PRINT_WARNING,
                              "Shader %s has a xy attenuationmap stage with no image\n",
                              shader.name);
                    pStage->active = false;
                    continue;
                }
            ...
            case ST_ATTENUATIONMAP_Z: {
                if (!pStage->bundle[0].image[0]) {
                    ri.Printf(PRINT_WARNING,
                              "Shader %s has a z attenuationmap stage with no image\n",
                              shader.name);
                    pStage->active = false;
                    continue;
                }
    ...
    for (i = 1; i < MAX_ALTSHADERS; ++i) {
        if (ret->altShader[i].name) {
            ...
            shader_t* sh =
                    R_FindShader(ret->altShader[i].name, ret->type, (RegisterShaderFlags_t) ret->altShader[0].index);
...
shader_t* R_FindShaderByName(const char* name) {
    ...
    for (sh = shaderHashTable[hash]; sh; sh = sh->next) {
        // NOTE: if there was no shader or image available with the name
        // strippedName
        // then a default shader is created with type == SHADER_3D_DYNAMIC, so we
        // have to check all default shaders otherwise for every call to
        // R_FindShader
...
shader_t* R_FindShader(const char* name, shaderType_t type, RegisterShaderFlags_t flags) {
    ...
    for (sh = shaderHashTable[hash]; sh; sh = sh->next) {
        ...
        // R_FindShader
        // with that same strippedName a new default shader is created.
        if ((sh->type == type || sh->defaultShader) &&
    ...
    shader.altShader[0].index =
            flags; // save for later use (in case of alternativer shaders)
    ...
    if (!(bits & RSF_NOMIP)) {
        image = R_FindImageFile(fileName, bits, FT_DEFAULT, WT_REPEAT, shader.name);
    } else {
        image = R_FindImageFile(fileName, bits, FT_LINEAR, WT_CLAMP, shader.name);
    }
    ...
    switch (shader.type) {
        ...
        case SHADER_LIGHT: {
            ...
            stages[0].bundle[0].image[0] =
                    tr.noFalloffImage; // FIXME should be attenuationZImage
            stages[0].active = true;
            stages[0].rgbGen = CGEN_IDENTITY;
            stages[0].stateBits = GLS_DEFAULT;
...
qhandle_t RE_RegisterShaderFromImage(const char* name, image_t* image) {
    ...
    for (sh = shaderHashTable[hash]; sh; sh = sh->next) {
        ...
        // R_FindShader
        // with that same strippedName a new default shader is created.
        if ((sh->type == SHADER_2D || sh->defaultShader) &&
...
qhandle_t RE_RegisterShader(const char* name, RegisterShaderFlags_t flags) {
    ...
    sh = R_FindShader(
            name, (flags & RSF_LIGHT_ATTENUATION) ? SHADER_LIGHT : SHADER_2D, flags);
...
static void CreateExternalShaders() {
    ...
    tr.defaultPointLightShader =
            R_FindShader("lights/defaultPointLight", SHADER_LIGHT, RSF_DEFAULT);
    tr.defaultProjectedLightShader =
            R_FindShader("lights/defaultProjectedLight", SHADER_LIGHT, RSF_DEFAULT);
    tr.defaultDynamicLightShader =
            R_FindShader("lights/defaultDynamicLight", SHADER_LIGHT, RSF_DEFAULT);
}
...
void R_SetAltShaderTokens(const char* list) {
    ...
    Q_strncpyz(whenTokens, list,
               sizeof(whenTokens) - 1); // will have double-NUL termination
