...
bool R_LoadMD3(model_t* mod, int lod, void* buffer, int bufferSize, const char* modName) {
    ...
    mdvModel = mod->mdv[lod] =
            (mdvModel_t*) ri.Hunk_Alloc(sizeof(mdvModel_t), h_low);
    ...
    mdvModel->frames = frame =
            (mdvFrame_t*) ri.Hunk_Alloc(sizeof(*frame) * md3Model->numFrames, h_low);
    ...
    mdvModel->tags = tag = (mdvTag_t*) ri.Hunk_Alloc(
            sizeof(*tag) * (md3Model->numTags * md3Model->numFrames), h_low);
    ...
    mdvModel->tagNames = tagName = (mdvTagName_t*) ri.Hunk_Alloc(
    ...
    mdvModel->surfaces = surf = (mdvSurface_t*) ri.Hunk_Alloc(
    ...
    for (i = 0; i < md3Model->numSurfaces; i++) {
        ...
        surf->shader =
                R_FindShader(md3Shader->name, SHADER_3D_DYNAMIC, RSF_DEFAULT);

        // swap all the triangles
        surf->numTriangles = md3Surf->numTriangles;
        surf->triangles = tri = (srfTriangle_t*) ri.Hunk_Alloc(
                sizeof(*tri) * md3Surf->numTriangles, h_low);
        ...
        surf->verts = v = (mdvXyz_t*) ri.Hunk_Alloc(
                sizeof(*v) * (md3Surf->numVerts * md3Surf->numFrames), h_low);
        surf->normals = n = (mdvNormal_t*) ri.Hunk_Alloc(
                sizeof(*n) * (md3Surf->numVerts * md3Surf->numFrames), h_low);
        ...
        // swap all the ST
        surf->st = st =
                (mdvSt_t*) ri.Hunk_Alloc(sizeof(*st) * md3Surf->numVerts, h_low);
    ...
    // create VBO surfaces from md3 surfaces
    {
        ...
        for (i = 0, surf = mdvModel->surfaces; i < mdvModel->numSurfaces;
             i++, surf++) {
            // allocate temp memory for vertex data
            memset(&data, 0, sizeof(data));
            data.xyz = (vec3_t*) ri.Hunk_AllocateTempMemory(
                    sizeof(*data.xyz) * mdvModel->numFrames * surf->numVerts);
            data.qtangent = (i16vec4_t*) ri.Hunk_AllocateTempMemory(
                    sizeof(i16vec4_t) * mdvModel->numFrames * surf->numVerts);
            data.numFrames = mdvModel->numFrames;
            data.st = (i16vec2_t*) ri.Hunk_AllocateTempMemory(sizeof(i16vec2_t) *
                                                              surf->numVerts);
            ...
            for (f = 0; f < mdvModel->numFrames; f++) {
                for (j = 0; j < surf->numVerts; j++) {
                    VectorCopy(surf->verts[f * surf->numVerts + j].xyz,
                               data.xyz[f * surf->numVerts + j]);
                }
            }
            ...
            // calc and feed tangent spaces
            {
                ...
                tangents = (vec3_t*) ri.Hunk_AllocateTempMemory(surf->numVerts *
                                                                sizeof(vec3_t));
                binormals = (vec3_t*) ri.Hunk_AllocateTempMemory(surf->numVerts *
                                                                 sizeof(vec3_t));
                ...
                for (f = 0; f < mdvModel->numFrames; f++) {
                    ...
                    for (j = 0, tri = surf->triangles; j < surf->numTriangles;
                         j++, tri++) {
                    ...
                    for (j = 0; j < surf->numVerts; j++) {
                        ...
                        R_TBNtoQtangents(tangents[j], binormals[j], surf->normals[f * surf->numVerts + j].normal, data.qtangent[f * surf->numVerts + j]);
                    }
                }
            ...
            vboSurf->vbo = R_CreateStaticVBO(va("staticMD3Mesh_VBO '%s'", surf->name),
                                             data,
            ...
            indexes = (glIndex_t*) ri.Hunk_AllocateTempMemory(3 * surf->numTriangles *
                                                              sizeof(glIndex_t));
            ...
            vboSurf->ibo = R_CreateStaticIBO2(va("staticMD3Mesh_IBO %s", surf->name),
                                              surf->numTriangles,
                                              indexes);
        }
        ...
        mdvModel->vboSurfaces = (srfVBOMDVMesh_t**) ri.Hunk_Alloc(
                mdvModel->numVBOSurfaces * sizeof(*mdvModel->vboSurfaces), h_low);

        for (i = 0; i < mdvModel->numVBOSurfaces; i++) {
            mdvModel->vboSurfaces[i] =
                    (srfVBOMDVMesh_t*) Com_GrowListElement(&vboSurfaces, i);
        }
