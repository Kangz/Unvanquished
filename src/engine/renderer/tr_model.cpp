...
bool RE_BeginRegistration(glconfig_t* glconfigOut, glconfig2_t* glconfig2Out) {
    ...
    *glconfig2Out = glConfig2;
    ...
    tr.visIndex = 0;
    memset(tr.visClusters, -2,
           sizeof(tr.visClusters)); // force markleafs to regenerate
...
void R_Modellist_f() {
        ...
        if (mod->type == MOD_MESH) {
            for (j = 0; j < MD3_MAX_LODS; j++) {
                if (mod->mdv[j] && (j == 0 || mod->mdv[j] != mod->mdv[j - 1])) {
                    ...
                    for (k = 0, mdvTagName = mdvModel->tagNames; k < mdvModel->numTags;
                         k++, mdvTagName++) {
            ...
            ri.Printf(PRINT_ALL, "%d.%02d MB '%s'\n", mod->dataSize / (1024 * 1024), (mod->dataSize % (1024 * 1024)) * 100 / (1024 * 1024), mod->name);
    ...
    ri.Printf(PRINT_ALL, " %d.%02d MB total model memory\n", totalDataSize / (1024 * 1024), (totalDataSize % (1024 * 1024)) * 100 / (1024 * 1024));
...
int RE_LerpTagET(orientation_t* tag, const refEntity_t* refent, const char* tagNameIn, int startIndex) {
    ...
    if (model->type == MOD_MD5 || model->type == MOD_IQM) {
        ...
        for (i = 0; i < 3; i++) {
            ...
            tag->axis[0][i] =
                    start->axis[0][i] * backLerp + end->axis[0][i] * frontLerp;
            tag->axis[1][i] =
                    start->axis[1][i] * backLerp + end->axis[1][i] * frontLerp;
            tag->axis[2][i] =
                    start->axis[2][i] * backLerp + end->axis[2][i] * frontLerp;
        }
