...
bool R_LoadMD5(model_t* mod, void* buffer, int bufferSize, const char* modName) {
    ...
    if (version != MD5_VERSION) {
        ri.Printf(PRINT_WARNING,
                  "R_LoadMD5: %s has wrong version (%i should be %i)\n",
                  modName,
                  version,
                  MD5_VERSION);
        return false;
    ...
    if (Q_stricmp(token, "numJoints")) {
        ri.Printf(PRINT_WARNING,
                  "R_LoadMD5: expected 'numJoints' found '%s' in model '%s'\n",
                  token,
                  modName);
        return false;
    }
    ...
    if (Q_stricmp(token, "numMeshes")) {
        ri.Printf(PRINT_WARNING,
                  "R_LoadMD5: expected 'numMeshes' found '%s' in model '%s'\n",
                  token,
                  modName);
        return false;
    }
    ...
    if (Q_stricmp(token, "joints")) {
        ri.Printf(PRINT_WARNING,
                  "R_LoadMD5: expected 'joints' found '%s' in model '%s'\n",
                  token,
                  modName);
        return false;
    }
    ...
    if (Q_stricmp(token, "{")) {
        ri.Printf(PRINT_WARNING,
                  "R_LoadMD5: expected '{' found '%s' in model '%s'\n",
                  token,
                  modName);
        return false;
    }
    ...
    for (i = 0, bone = md5->bones; i < md5->numBones; i++, bone++) {
        ...
        if (bone->parentIndex >= md5->numBones) {
            ri.Error(ERR_DROP,
                     "R_LoadMD5: '%s' has bone '%s' with bad parent index %i while "
                     "numBones is %i",
                     modName,
                     bone->name,
                     bone->parentIndex,
    ...
    md5->surfaces =
            (md5Surface_t*) ri.Hunk_Alloc(sizeof(*surf) * md5->numSurfaces, h_low);
    ...
    for (i = 0, surf = md5->surfaces; i < md5->numSurfaces; i++, surf++) {
        ...
        if (Q_stricmp(token, "mesh")) {
            ri.Printf(PRINT_WARNING,
                      "R_LoadMD5: expected 'mesh' found '%s' in model '%s'\n",
                      token,
                      modName);
            return false;
        }
        ...
        if (Q_stricmp(token, "shader")) {
            ri.Printf(PRINT_WARNING,
                      "R_LoadMD5: expected 'shader' found '%s' in model '%s'\n",
                      token,
                      modName);
            return false;
        }
        ...
        if (Q_stricmp(token, "numVerts")) {
            ri.Printf(PRINT_WARNING,
                      "R_LoadMD5: expected 'numVerts' found '%s' in model '%s'\n",
                      token,
                      modName);
            return false;
        }
        ...
        if (surf->numVerts > SHADER_MAX_VERTEXES) {
            ri.Error(ERR_DROP,
                     "R_LoadMD5: '%s' has more than %i verts on a surface (%i)",
                     modName,
                     SHADER_MAX_VERTEXES,
                     surf->numVerts);
        }
        ...
        surf->verts =
                (md5Vertex_t*) ri.Hunk_Alloc(sizeof(*v) * surf->numVerts, h_low);
        ...
        for (j = 0, v = surf->verts; j < surf->numVerts; j++, v++) {
            ...
            if (Q_stricmp(token, "vert")) {
                ri.Printf(PRINT_WARNING,
                          "R_LoadMD5: expected 'vert' found '%s' in model '%s'\n",
                          token,
                          modName);
                return false;
            }
            ...
            if (v->numWeights > MAX_WEIGHTS) {
                ri.Error(ERR_DROP,
                         "R_LoadMD5: vertex %i requires more than %i weights on "
                         "surface (%i) in model '%s'",
                         j,
                         MAX_WEIGHTS,
                         i,
                         modName);
        ...
        if (Q_stricmp(token, "numTris")) {
            ri.Printf(PRINT_WARNING,
                      "R_LoadMD5: expected 'numTris' found '%s' in model '%s'\n",
                      token,
                      modName);
            return false;
        }
        ...
        if (surf->numTriangles > SHADER_MAX_TRIANGLES) {
            ri.Error(ERR_DROP,
                     "R_LoadMD5: '%s' has more than %i triangles on a surface (%i)",
                     modName,
                     SHADER_MAX_TRIANGLES,
                     surf->numTriangles);
        }

        surf->triangles = (srfTriangle_t*) ri.Hunk_Alloc(
                sizeof(*tri) * surf->numTriangles, h_low);
        ...
        for (j = 0, tri = surf->triangles; j < surf->numTriangles; j++, tri++) {
            ...
            if (Q_stricmp(token, "tri")) {
                ri.Printf(PRINT_WARNING,
                          "R_LoadMD5: expected 'tri' found '%s' in model '%s'\n",
                          token,
                          modName);
                return false;
            }
        ...
        if (Q_stricmp(token, "numWeights")) {
            ri.Printf(PRINT_WARNING,
                      "R_LoadMD5: expected 'numWeights' found '%s' in model '%s'\n",
                      token,
                      modName);
            return false;
        }
        ...
        surf->weights =
                (md5Weight_t*) ri.Hunk_Alloc(sizeof(*weight) * surf->numWeights, h_low);
        ...
        for (j = 0, weight = surf->weights; j < surf->numWeights; j++, weight++) {
            ...
            if (Q_stricmp(token, "weight")) {
                ri.Printf(PRINT_WARNING,
                          "R_LoadMD5: expected 'weight' found '%s' in model '%s'\n",
                          token,
                          modName);
                return false;
            }
        ...
        // loop through all vertices and set up the vertex weights and base
        // positions
        for (j = 0, v = surf->verts; j < surf->numVerts; j++, v++) {
    ...
    // split the surfaces into VBO surfaces by the maximum number of GPU vertex
    // skinning bones
    ...
    for (i = 0, surf = md5->surfaces; i < md5->numSurfaces; i++, surf++) {
        ...
        for (j = 0, tri = surf->triangles; j < surf->numTriangles; j++, tri++) {
            skelTriangle_t* sortTri =
                    (skelTriangle_t*) Com_Allocate(sizeof(*sortTri));
        ...
        while (numRemaining) {
            ...
            for (j = 0; j < sortedTriangles.currentElements; j++) {
                skelTriangle_t* sortTri =
                        (skelTriangle_t*) Com_GrowListElement(&sortedTriangles, j);
            ...
            if (!vboTriangles.currentElements) {
                ri.Printf(PRINT_WARNING,
                          "R_LoadMD5: could not add triangles to a remaining VBO "
                          "surfaces for model '%s'\n",
                          modName);
    ...
    md5->vboSurfaces = (srfVBOMD5Mesh_t**) ri.Hunk_Alloc(
            md5->numVBOSurfaces * sizeof(*md5->vboSurfaces), h_low);

    for (i = 0; i < md5->numVBOSurfaces; i++) {
        md5->vboSurfaces[i] =
                (srfVBOMD5Mesh_t*) Com_GrowListElement(&vboSurfaces, i);
    }
