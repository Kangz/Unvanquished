...
static void png_user_warning_fn(png_structp png_ptr,
                                png_const_charp warning_message) {
    ri.Printf(PRINT_WARNING, "libpng warning: %s\n", warning_message);
}

static void NORETURN
png_user_error_fn(png_structp png_ptr, png_const_charp error_message) {
...
void LoadPNG(const char* name, byte** pic, int* width, int* height, int* numLayers, int* numMips, int* bits, byte alphaByte) {
    ...
    if (!png) {
        ri.Printf(PRINT_WARNING,
                  "LoadPNG: png_create_write_struct() failed for (%s)\n",
                  name);
        ri.FS_FreeFile(data);
        return;
    }
    ...
    if (!info) {
        ri.Printf(PRINT_WARNING,
                  "LoadPNG: png_create_info_struct() failed for (%s)\n",
                  name);
        ri.FS_FreeFile(data);
   ...
   */
    if (setjmp(png_jmpbuf(png))) {
        ...
        ri.Printf(PRINT_WARNING,
                  "LoadPNG: first exception handler called for (%s)\n",
                  name);
        ri.FS_FreeFile(data);
    ...
    *pic = out = (byte*) ri.Z_Malloc(w * h * 4);
    ...
    if (setjmp(png_jmpbuf(png))) {
        ri.Printf(PRINT_WARNING,
                  "LoadPNG: second exception handler called for (%s)\n",
                  name);
...
void SavePNG(const char* name, const byte* pic, int width, int height, int numBytes, bool flip) {
    ...
    switch (numBytes) {
        ...
        case 3:
            ...
            break;
        case 2:
            png_set_IHDR(png, info, width, height, 8, PNG_COLOR_TYPE_GA, PNG_INTERLACE_NONE, PNG_COMPRESSION_TYPE_DEFAULT, PNG_FILTER_TYPE_DEFAULT);
            break;
        case 1:
    ...
    row_pointers =
            (png_bytep*) ri.Hunk_AllocateTempMemory(height * sizeof(png_bytep));
