...
//
//
// The font system uses FreeType 2.x to render TrueType fonts for use within the
// game.
// As of this writing ( Nov, 2000 ) Team Arena uses these fonts for all of the
// ui and
// about 90% of the cgame presentation. A few areas of the CGAME were left uses
// the old
// fonts since the code is shared with standard Q3A.
//
// If you include this font rendering code in a commercial product you MUST
// include the
// following somewhere with your product, see www.freetype.org for specifics or
// changes.
// The Freetype code also uses some hinting techniques that MIGHT infringe on
// patents
// held by apple so be aware of that also.
//
// As of Q3A 1.25+ and Team Arena, we are shipping the game with the font
// rendering code
// disabled. This removes any potential patent issues and it keeps us from
// having to
// distribute an actual TrueTrype font which is 1. expensive to do and 2. seems
// to require
// an act of god to accomplish.
//
// What we did was pre-render the fonts using FreeType ( which is why we leave
// the FreeType
// credit in the credits ) and then saved off the glyph data and then hand
// touched up the
// font bitmaps so they scale a bit better in GL.
//
// There are limitations in the way fonts are saved and reloaded in that it is
// based on
// point size and not name. So if you pre-render Helvetica in 18 point and
// Impact in 18 point
// you will end up with a single 18 point data file and image set. Typically you
// will want to
// choose 3 sizes to best approximate the scaling you will be doing in the ui
// scripting system
//
// In the UI Scripting code, a scale of 1.0 is equal to a 48 point font. In Team
// Arena, we
// use three or four scales, most of them exactly equaling the specific rendered
// size. We
// rendered three sizes in Team Arena, 12, 16, and 20.
//
// To generate new font data you need to go through the following steps.
// 1. delete the fontImage_x_xx.png files and fontImage_xx.dat files from the
// fonts path.
// 2. in a ui script, specificy a font, smallFont, and bigFont keyword with font
// name and
//    point size. the original TrueType fonts must exist in fonts at this point.
// 3. run the game. you should see things.
// 4. Exit the game and there will be three dat files and at least three PNG
// files. The
//    PNGs are in 256x256 pages so if it takes three images to render a 24 point
//    font you
//    will end up with fontImage_0_24.tga through fontImage_2_24.tga
// 5. In future runs of the game, the system looks for these images and data
// files when a
//    specific point sized font is rendered and loads them for use.
// 6. Because of the original beta nature of the FreeType code you will probably
// want to hand
...
static glyphInfo_t* RE_ConstructGlyphInfo(unsigned char* imageOut, int* xOut, int* yOut, int* maxHeight, FT_Face face, FT_Face fallback, const int c, bool calcHeight) {
    int i;
...
int readInt() {
    int i = fdFile[fdOffset] + (fdFile[fdOffset + 1] << 8) +
...
void RE_GlyphChar(fontInfo_t* font, int ch, glyphInfo_t* glyph) {
    ...
    if (ch < 0 || (ch >= 0xD800 && ch < 0xE000) || ch >= 0x110000 ||
        ch == 0xFFFD) {
        ch = 0;
    }
...
static void RE_StoreImage(fontInfo_t* font, int chunk, int page, int from, int to, const unsigned char* bitmap, int yEnd) {
    ...
    for (j = from; j < to; j++) {
        if (font->glyphBlock[chunk][j]
                    .shaderName[0]) // non-0 if we have a glyph here
        {
...
void RE_RenderChunk(fontInfo_t* font, const int chunk) {
    ...
    if (out == nullptr) {
        ri.Printf(
                PRINT_WARNING,
                "RE_RenderChunk: ri.Malloc failure during output image creation.\n");
        return;
    }
    ...
    for (i = 0; i < 256; i++) {
        rendered |= !!RE_ConstructGlyphInfo(
                             out, &xOut, &yOut, &maxHeight, (FT_Face) font->face, (FT_Face) font->fallback, (i + startGlyph) ? (i + startGlyph) : 0xFFFD, true);
    }
    ...
    glyphs = font->glyphBlock[chunk] =
            (glyphInfo_t*) ri.Z_Malloc(sizeof(glyphBlock_t));
    ...
    while (i < 256) {
        glyphInfo_t* glyph = RE_ConstructGlyphInfo(
                out, &xOut, &yOut, &maxHeight, (FT_Face) font->face, (FT_Face) font->fallback, (i + startGlyph) ? (i + startGlyph) : 0xFFFD, false);
...
static fontHandle_t RE_RegisterFont_Internal(const char* fontName,
    ...
    for (i = 0; i < MAX_FONTS; i++) {
        ...
        } else if (pointSize == registeredFont[i].pointSize &&
                   Q_stricmp(strippedName, registeredFont[i].name) == 0) {
    ...
    *font = &registeredFont[fontNo];
    ...
    if (len > 0x5004 &&
        len <= 0x5004 + MAX_QPATH) // 256 glyphs, scale info, and the bitmap name
    {
        glyphInfo_t* glyphs;
        int height = 0;
    ...
    (*font)->height =
            ceil((face->height / 64.0) * (face->size->metrics.y_scale / 65536.0) *
                 (*font)->glyphScale);
...
void RE_UnregisterFont_Internal(fontHandle_t handle) {
    ...
    for (i = 0; i < 0x1100; ++i) {
        if (registeredFont[handle].glyphBlock[i] &&
            registeredFont[handle].glyphBlock[i] != nullGlyphs) {
