...
void R_TransformClipToWindow(const vec4_t clip, const viewParms_t* view, vec4_t normalized, vec4_t window) {
    ...
    window[0] =
            view->viewportX + (0.5f * (1.0f + normalized[0]) * view->viewportWidth);
    window[1] =
            view->viewportY + (0.5f * (1.0f + normalized[1]) * view->viewportHeight);
...
void R_RotateEntityForViewParms(const trRefEntity_t* ent,
    ...
    MatrixSetupTransformFromVectorsFLU(orientation->transformMatrix,
                                       orientation->axis[0],
                                       orientation->axis[1],
                                       orientation->axis[2],
                                       orientation->origin);
    ...
    orientation->viewOrigin[0] =
            DotProduct(delta, orientation->axis[0]) * axisLength;
    orientation->viewOrigin[1] =
            DotProduct(delta, orientation->axis[1]) * axisLength;
    orientation->viewOrigin[2] =
            DotProduct(delta, orientation->axis[2]) * axisLength;
}
...
=================
*/
void R_RotateLightForViewParms(const trRefLight_t* light,
                               const viewParms_t* viewParms,
                               orientationr_t* orientation) {
    vec3_t delta;
...
void R_RotateForViewer() {
    ...
    MatrixSetupTransformFromVectorsFLU(
            transformMatrix, tr.viewParms.orientation.axis[0], tr.viewParms.orientation.axis[1], tr.viewParms.orientation.axis[2], tr.viewParms.orientation.origin);
...
=================
R_SetupUnprojection
create a matrix with similar functionality like gluUnproject, project from
window space to world space
=================
*/
...
static void R_SetupFrustum() {
    ...
    for (i = 0; i < 4; i++) {
        ...
        tr.viewParms.frustums[0][i].dist = DotProduct(
                tr.viewParms.orientation.origin, tr.viewParms.frustums[0][i].normal);
        SetPlaneSignbits(&tr.viewParms.frustums[0][i]);
    }

    // Tr3B: set extra near plane which is required by the dynamic occlusion
    // culling
    tr.viewParms.frustums[0][FRUSTUM_NEAR].type = PLANE_NON_AXIAL;
    VectorCopy(tr.viewParms.orientation.axis[0],
               tr.viewParms.frustums[0][FRUSTUM_NEAR].normal);

    VectorMA(tr.viewParms.orientation.origin, r_znear->value, tr.viewParms.frustums[0][FRUSTUM_NEAR].normal, planeOrigin);
    tr.viewParms.frustums[0][FRUSTUM_NEAR].dist =
            DotProduct(planeOrigin, tr.viewParms.frustums[0][FRUSTUM_NEAR].normal);
...
static void R_SetupSplitFrustums() {
    ...
    for (i = 1; i <= (r_parallelShadowSplits->integer + 1); i++) {
        ...
        zFar = 1.005f * lambda * (tr.viewParms.zNear * powf(ratio, si)) +
               (1 - lambda) * (tr.viewParms.zNear +
                               (tr.viewParms.zFar - tr.viewParms.zNear) * si);
        ...
        VectorNegate(tr.viewParms.orientation.axis[0],
                     tr.viewParms.frustums[i][FRUSTUM_FAR].normal);

        VectorMA(tr.viewParms.orientation.origin, zFar, tr.viewParms.orientation.axis[0], planeOrigin);
        tr.viewParms.frustums[i][FRUSTUM_FAR].dist =
                DotProduct(planeOrigin, tr.viewParms.frustums[i][FRUSTUM_FAR].normal);
        ...
        if (i <= (r_parallelShadowSplits->integer)) {
            ...
            VectorCopy(tr.viewParms.orientation.axis[0],
                       tr.viewParms.frustums[i + 1][FRUSTUM_NEAR].normal);

            VectorMA(tr.viewParms.orientation.origin, zNear, tr.viewParms.orientation.axis[0], planeOrigin);
            tr.viewParms.frustums[i + 1][FRUSTUM_NEAR].dist = DotProduct(
                    planeOrigin, tr.viewParms.frustums[i + 1][FRUSTUM_NEAR].normal);
...
=================
*/
static bool R_GetPortalOrientations(drawSurf_t* drawSurf,
    ...
    if (drawSurf->entity != &tr.worldEntity) {
        ...
        plane.dist =
                originalPlane.dist + DotProduct(plane.normal, tr.orientation.origin);

        // translate the original plane
        originalPlane.dist = originalPlane.dist + DotProduct(originalPlane.normal,
                                                             tr.orientation.origin);
    } else {
        plane = originalPlane;
    }
...
static bool SurfIsOffscreen(const drawSurf_t* drawSurf, vec4_t clipDest[128]) {
    ...
    for (i = 0; i < tess.numIndexes; i += 3) {
        ...
        R_QtangentsToTBN(tess.verts[tess.indexes[i]].qtangents, qtangent, qbinormal, qnormal);
    ...
    if (shortest >
        (tess.surfaceShader->portalRange * tess.surfaceShader->portalRange)) {
        return true;
    }
...
static bool R_MirrorViewBySurface(drawSurf_t* drawSurf) {
    ...
    newParms.portalPlane.dist =
            DotProduct(camera.origin, newParms.portalPlane.normal);
...
static void R_SortDrawSurfs() {
    ...
    std::sort(
            tr.viewParms.drawSurfs,
            tr.viewParms.drawSurfs + tr.viewParms.numDrawSurfs,
            [](const drawSurf_t& a, const drawSurf_t& b) { return a.sort < b.sort; });

    // check for any pass through drawing, which
    // may cause another view to be rendered first
    for (i = 0, drawSurf = tr.viewParms.drawSurfs; i < tr.viewParms.numDrawSurfs;
         i++, drawSurf++) {
        shader = tr.sortedShaders[drawSurf->shaderNum()];
        ...
        // if the mirror was completely clipped away, we may need to check another
        // surface
...
void R_AddEntitySurfaces() {
    ...
    for (i = 0; i < tr.refdef.numEntities; i++) {
        ...
        if ((ent->e.renderfx & RF_FIRST_PERSON) &&
            (tr.viewParms.isPortal || tr.viewParms.isMirror)) {
            continue;
        }
        ...
        switch (ent->e.reType) {
            ...
            case RT_SPRITE:
                ...
                // self blood sprites, talk balloons, etc should not be drawn in the
                // primary
...
void R_TransformShadowLight(trRefLight_t* light) {
    ...
    for (i = light->restrictInteractionFirst; i <= light->restrictInteractionLast;
         i++) {
    ...
    for (i = 0; i < tr.refdef.numLights; i++) {
        ...
        if (light->isStatic) {
            if (!r_staticLight->integer ||
                ((r_precomputedLighting->integer || r_vertexLighting->integer) &&
                 !light->noRadiosity)) {
                light->cull = CULL_OUT;
                continue;
            }
        ...
        if (light->isStatic) {
            ...
            if (!r_noLightVisCull->integer) {
                for (l = light->leafs.next; l != &light->leafs; l = l->next) {
                    if (!l || !l->data) {
                        // something odd happens with the prev/next pointers if
                        // ri.Hunk_Alloc was used
                        break;
                    }
        ...
        if (light->numInteractions &&
            light->numInteractions != light->numShadowOnlyInteractions) {
            ...
            // skip all interactions of this light because it caused only shadow
            // volumes
            // but no lighting
...
void R_RenderView(viewParms_t* parms) {
    ...
    if (tr.viewCount >= MAX_VIEWS) {
        ri.Printf(PRINT_ALL,
                  "MAX_VIEWS (%i) hit. Don't add more mirrors or portals. Skipping "
                  "view ...\n",
                  MAX_VIEWS);
        return;
    }
    ...
    // set camera frustum planes in world space again, but this time including the
    // far plane
    tr.orientation = tr.viewParms.world;
    ...
    // Transform the blur vector in view space, FIXME for some we need reason
    // invert its Z component
