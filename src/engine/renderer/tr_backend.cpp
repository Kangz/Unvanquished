...
void BindAnimatedImage(textureBundle_t* bundle) {
    ...
    index = Q_ftol(backEnd.refdef.floatTime * bundle->imageAnimationSpeed *
                   FUNCTABLE_SIZE);
...
void GL_BlendFunc(GLenum sfactor, GLenum dfactor) {
    if (glState.blendSrc != (signed) sfactor ||
        glState.blendDst != (signed) dfactor) {
...
void GL_ClearColor(GLclampf red, GLclampf green, GLclampf blue, GLclampf alpha) {
    if (glState.clearColorRed != red || glState.clearColorGreen != green ||
        glState.clearColorBlue != blue || glState.clearColorAlpha != alpha) {
...
void GL_ColorMask(GLboolean red, GLboolean green, GLboolean blue, GLboolean alpha) {
    if (glState.colorMaskRed != red || glState.colorMaskGreen != green ||
        glState.colorMaskBlue != blue || glState.colorMaskAlpha != alpha) {
...
void GL_LoadModelViewMatrix(const matrix_t m) {
    ...
    MatrixMultiply(glState.projectionMatrix[glState.stackIndex],
...
void GL_PolygonMode(GLenum face, GLenum mode) {
    if (glState.polygonFace != (signed) face ||
        glState.polygonMode != (signed) mode) {
...
void GL_Scissor(GLint x, GLint y, GLsizei width, GLsizei height) {
    if (glState.scissorX != x || glState.scissorY != y ||
        glState.scissorWidth != width || glState.scissorHeight != height) {
        glState.scissorX = x;
        glState.scissorY = y;
...
void GL_Viewport(GLint x, GLint y, GLsizei width, GLsizei height) {
    if (glState.viewportX != x || glState.viewportY != y ||
        glState.viewportWidth != width || glState.viewportHeight != height) {
        glState.viewportX = x;
        glState.viewportY = y;
...
void GL_PolygonOffset(float factor, float units) {
    if (glState.polygonOffsetFactor != factor ||
...
void GL_VertexAttribPointers(uint32_t attribBits) {
    ...
    for (i = 0; i < ATTR_INDEX_MAX; i++) {
        ...
        if ((attribBits & bit) != 0 && (!(glState.vertexAttribPointersSet & bit) ||
            ...
            glVertexAttribPointer(
                    i, layout->numComponents, layout->componentType, layout->normalize, layout->stride, BUFFER_OFFSET(layout->ofs + (frame * layout->frameOffset + base)));
...
static void SetViewportAndScissor() {
    ...
    if (backEnd.viewParms.isPortal) {
        c[0] = -DotProduct(backEnd.viewParms.portalPlane.normal,
                           backEnd.viewParms.orientation.axis[1]);
        c[1] = DotProduct(backEnd.viewParms.portalPlane.normal,
                          backEnd.viewParms.orientation.axis[2]);
        c[2] = -DotProduct(backEnd.viewParms.portalPlane.normal,
                           backEnd.viewParms.orientation.axis[0]);
        c[3] = DotProduct(backEnd.viewParms.portalPlane.normal,
    ...
    GL_Viewport(backEnd.viewParms.viewportX, backEnd.viewParms.viewportY, backEnd.viewParms.viewportWidth, backEnd.viewParms.viewportHeight);

    GL_Scissor(backEnd.viewParms.viewportX, backEnd.viewParms.viewportY, backEnd.viewParms.viewportWidth, backEnd.viewParms.viewportHeight);
}
...
static void RB_RenderDrawSurfaces(bool opaque,
    ...
    for (i = 0, drawSurf = backEnd.viewParms.drawSurfs;
         i < backEnd.viewParms.numDrawSurfs;
         i++, drawSurf++) {
        // update locals
        ...
        // an "entityMergable" shader is a shader that can have surfaces from
        // separate
        ...
        if (shader != oldShader || lightmapNum != oldLightmapNum ||
            ...
            if (oldShader != nullptr) {
                if (oldShader->autoSpriteMode &&
                    !(tess.attribsSet & ATTR_ORIENTATION)) {
                    Tess_AutospriteDeform(oldShader->autoSpriteMode, 0, tess.numVertexes, 0, tess.numIndexes);
                }
                Tess_End();
            }
 ...
 */
static int MergeInteractionBounds(const matrix_t lightViewProjectionMatrix,
                                  interaction_t* ia,
                                  int iaCount,
                                  vec3_t bounds[2],
                                  bool shadowCasters) {
    ...
    while (iaCount < backEnd.viewParms.numInteractions) {
        ...
        if (*surface == SF_FACE || *surface == SF_GRID ||
            *surface == SF_TRIANGLES) {
        ...
        for (i = 0; i < 6; i++) {
            ...
            // we can have shadow casters outside the initial computed light view
            // frustum
...
static interaction_t* IterateLights(const interaction_t* prev) {
    ...
    const interaction_t* last =
            &backEnd.viewParms.interactions[backEnd.viewParms.numInteractions - 1];
...
static void RB_SetupLightAttenuationForEntity(trRefLight_t* light,
    ...
    while ((iaFirst = IterateLights(iaFirst))) {
        ...
        for (ia = iaFirst; ia; ia = ia->next) {
            ...
            if (!shader || !shader->interactLight) {
                // skip this interaction because the surface shader has no ability to
                // interact with light
            ...
            if (!(ia->type & IA_LIGHT)) {
                // skip this interaction because the interaction is meant for shadowing
                // only
                continue;
            }
...
static void RB_SetupLightForShadowing(trRefLight_t* light, int index, bool shadowClip) {
    // HACK: bring OpenGL into a safe state or strange FBO update problems will
    // occur
    ...
    switch (light->l.rlType) {
        ...
        case RL_PROJ: {
            ...
            if (shadowClip) {
                R_AttachFBOTexture2D(
                        GL_TEXTURE_2D, tr.shadowClipMapFBOImage[light->shadowLOD]->texnum, 0);
            } else {
                R_AttachFBOTexture2D(GL_TEXTURE_2D,
                                     tr.shadowMapFBOImage[light->shadowLOD]->texnum,
                                     0);
            }
        ...
        case RL_DIRECTIONAL: {
            ...
            if (shadowClip) {
                R_AttachFBOTexture2D(
                        GL_TEXTURE_2D, tr.sunShadowClipMapFBOImage[splitFrustumIndex]->texnum, 0);
            } else if (!r_evsmPostProcess->integer) {
                R_AttachFBOTexture2D(
                        GL_TEXTURE_2D, tr.sunShadowMapFBOImage[splitFrustumIndex]->texnum, 0);
            } else {
                R_AttachFBOTextureDepth(
                        tr.sunShadowMapFBOImage[splitFrustumIndex]->texnum);
            }
            ...
            if (r_parallelShadowSplits->integer) {
                ...
                for (j = 0; j < 6; j++) {
                    VectorCopy(backEnd.viewParms.frustums[1 + splitFrustumIndex][j].normal,
                               splitFrustum[j]);
                    splitFrustum[j][3] =
                            backEnd.viewParms.frustums[1 + splitFrustumIndex][j].dist;
                }
                ...
                if (r_logFile->integer) {
                    ...
                    GLimp_LogComment(
                            va("split frustum light space clip bounds (%5.3f, %5.3f, %5.3f) "
                               "(%5.3f, %5.3f, %5.3f)\n",
                               splitFrustumClipBounds[0][0],
                               splitFrustumClipBounds[0][1],
                               splitFrustumClipBounds[0][2],
                               splitFrustumClipBounds[1][0],
                               splitFrustumClipBounds[1][1],
                               splitFrustumClipBounds[1][2]));

                    GLimp_LogComment(
                            va("shadow caster light space clip bounds (%5.3f, %5.3f, %5.3f) "
                               "(%5.3f, %5.3f, %5.3f)\n",
                               casterBounds[0][0],
                               casterBounds[0][1],
                               casterBounds[0][2],
                    ...
                    GLimp_LogComment(va(
                            "light receiver light space clip bounds (%5.3f, %5.3f, %5.3f) "
                            "(%5.3f, %5.3f, %5.3f)\n",
                            receiverBounds[0][0],
                            receiverBounds[0][1],
                            receiverBounds[0][2],
                ...
                cropBounds[0][0] =
                        std::max(std::max(casterBounds[0][0], receiverBounds[0][0]),
                                 splitFrustumClipBounds[0][0]);
                cropBounds[0][1] =
                        std::max(std::max(casterBounds[0][1], receiverBounds[0][1]),
                                 splitFrustumClipBounds[0][1]);

                cropBounds[1][0] =
                        std::min(std::min(casterBounds[1][0], receiverBounds[1][0]),
                                 splitFrustumClipBounds[1][0]);
                cropBounds[1][1] =
                        std::min(std::min(casterBounds[1][1], receiverBounds[1][1]),
                                 splitFrustumClipBounds[1][1]);

                cropBounds[0][2] =
                        std::min(casterBounds[0][2], splitFrustumClipBounds[0][2]);
                cropBounds[1][2] =
                        std::min(receiverBounds[1][2], splitFrustumClipBounds[1][2]);
                ...
                ClearBounds(splitFrustumBounds[0], splitFrustumBounds[1]);
                // BoundsAdd(splitFrustumBounds[0], splitFrustumBounds[1],
                // backEnd.viewParms.visBounds[0], backEnd.viewParms.visBounds[1]);
        ...
        case RL_DIRECTIONAL: {
            ...
            if (r_showShadowMaps->integer) {
                ...
                MatrixOrthogonalProjection(
                        ortho, backEnd.viewParms.viewportX, backEnd.viewParms.viewportX + backEnd.viewParms.viewportWidth, backEnd.viewParms.viewportY, backEnd.viewParms.viewportY + backEnd.viewParms.viewportHeight, -99999, 99999);
                GL_LoadProjectionMatrix(ortho);
                GL_LoadModelViewMatrix(matrixIdentity);

                for (frustumIndex = 0; frustumIndex <= r_parallelShadowSplits->integer;
                     frustumIndex++) {
                    GL_Cull(CT_TWO_SIDED);
                    GL_State(GLS_DEPTHTEST_DISABLE);

                    gl_debugShadowMapShader->BindProgram();
                    gl_debugShadowMapShader->SetUniform_ModelViewProjectionMatrix(
                            glState.modelViewProjectionMatrix[glState.stackIndex]);
...

                    {
                        ...
                        gl_genericShader->SetUniform_ModelViewProjectionMatrix(
                                light->shadowMatrices[frustumIndex]);
                        ...
                        for (j = 0; j < 6; j++) {
                            VectorCopy(backEnd.viewParms.frustums[1 + frustumIndex][j].normal,
                                       splitFrustum[j]);
                            splitFrustum[j][3] =
                                    backEnd.viewParms.frustums[1 + frustumIndex][j].dist;
                        }
                        ...
                        if (light->isStatic && light->frustumVBO && light->frustumIBO) {
                            gl_genericShader->SetUniform_ColorModulate(CGEN_CUSTOM_RGB,
                                                                       AGEN_CUSTOM);
...
static void RB_BlurShadowMap(const trRefLight_t* light, int i) {
    ...
    if (light->l.inverseShadows || r_shadows->integer < SHADOWING_VSM16 ||
        !r_softShadowsPP->integer) {
        return;
    }
    ...
    fbos = (light->l.rlType == RL_DIRECTIONAL) ? tr.sunShadowMapFBO
                                               : tr.shadowMapFBO;
    images = (light->l.rlType == RL_DIRECTIONAL) ? tr.sunShadowMapFBOImage
                                                 : tr.shadowMapFBOImage;
    ...
    R_AttachFBOTexture2D(images[index + MAX_SHADOWMAPS]->type,
                         images[index + MAX_SHADOWMAPS]->texnum,
                         0);
    ...
    gl_blurXShader->SetUniform_ModelViewProjectionMatrix(
            glState.modelViewProjectionMatrix[glState.stackIndex]);
    ...
    gl_blurYShader->SetUniform_ModelViewProjectionMatrix(
            glState.modelViewProjectionMatrix[glState.stackIndex]);
...
static void RB_RenderInteractionsShadowMapped() {
    ...
    int startTime = 0, endTime = 0;
    static const matrix_t bias = {0.5, 0.0, 0.0, 0.0, 0.0, 0.5, 0.0, 0.0, 0.0, 0.0, 0.5, 0.0, 0.5, 0.5, 0.5, 1.0};
    ...
    while ((iaFirst = IterateLights(iaFirst))) {
        ...
        for (int i = 0; i < numMaps; i++) {
                    ...
                    case RL_DIRECTIONAL: {
                        if (entity == oldEntity &&
                            ...
                            if (r_logFile->integer) {
                                ...
                                // a call to va() every frame!
                                GLimp_LogComment(
                                        va("----- Batching Shadow Interaction: %i -----\n",
                            ...
                            if (r_logFile->integer) {
                                ...
                                // a call to va() every frame!
                                GLimp_LogComment(
                                        va("----- Beginning Shadow Interaction: %i -----\n",
                                           (int) (ia - backEnd.viewParms.interactions)));
                            }
                ...
                if (entity != oldEntity) {
                    ...
                    if (entity != &tr.worldEntity) {
                        ...
                        if (entity->e.renderfx & RF_DEPTHHACK) {
                            // hack the depth range to prevent view model from poking into
                            // walls
                            depthRange = true;
                        }
                    } else {
                        ...
                        MatrixMultiply(light->viewMatrix,
                                       backEnd.orientation.transformMatrix,
                                       backEnd.orientation.viewMatrix);
                        MatrixCopy(backEnd.orientation.viewMatrix,
                                   backEnd.orientation.modelViewMatrix);
                    }
            ...
            if (light->l.rlType == RL_DIRECTIONAL) {
                ...
                MatrixMultiply2(light->shadowMatricesBiased[i],
...
void RB_RenderGlobalFog() {
    ...
    gl_fogGlobalShader->SetUniform_ViewOrigin(
            backEnd.viewParms.orientation.origin); // world space

    {
        ...
        if (r_logFile->integer) {
            GLimp_LogComment(
                    va("--- RB_RenderGlobalFog( fogNum = %i, originalBrushNumber = %i ) "
                       "---\n",
                       tr.world->globalFog,
    ...
    gl_fogGlobalShader->SetUniform_UnprojectMatrix(
    ...
    gl_fogGlobalShader->SetUniform_ModelViewProjectionMatrix(
            glState.modelViewProjectionMatrix[glState.stackIndex]);
...
void RB_RenderBloom() {
    ...
    if ((backEnd.refdef.rdflags & (RDF_NOWORLDMODEL | RDF_NOBLOOM)) ||
        !r_bloom->integer || backEnd.viewParms.isPortal ||
        !glConfig2.framebufferObjectAvailable) {
        return;
    }
    ...
    // FIXME
    // if(glConfig.hardwareType != GLHW_ATI && glConfig.hardwareType !=
    // GLHW_ATI_DX10)
    {
        GL_State(GLS_DEPTHTEST_DISABLE);
        GL_Cull(CT_TWO_SIDED);
        ...
        gl_contrastShader->SetUniform_ModelViewProjectionMatrix(
                glState.modelViewProjectionMatrix[glState.stackIndex]);
...
void RB_RenderMotionBlur() {
    static vec4_t quadVerts[4] = {{-1.0f, -1.0f, 0.0f, 1.0f},
                                  {1.0f, -1.0f, 0.0f, 1.0f},
                                  {1.0f, 1.0f, 0.0f, 1.0f},
                                  {-1.0f, 1.0f, 0.0f, 1.0f}};
    ...
    if ((backEnd.refdef.rdflags & RDF_NOWORLDMODEL) ||
        backEnd.viewParms.isPortal || !glConfig2.framebufferObjectAvailable) {
        return;
    }
...
void RB_CameraPostFX() {
    ...
    gl_cameraEffectsShader->SetUniform_ColorModulate(
            backEnd.viewParms.gradingWeights);
    gl_cameraEffectsShader->SetUniform_ModelViewProjectionMatrix(
            glState.modelViewProjectionMatrix[glState.stackIndex]);
...
static void RB_RenderDebugUtils() {
    ...
    if (r_showLightTransforms->integer || r_showShadowLod->integer) {
        ...
        while ((ia = IterateLights(ia))) {
            ...
            if (light->isStatic && light->frustumVBO && light->frustumIBO) {
                ...
                gl_genericShader->SetUniform_ModelViewProjectionMatrix(
                        glState.modelViewProjectionMatrix[glState.stackIndex]);
                gl_genericShader->SetUniform_ModelMatrix(
                        backEnd.orientation.transformMatrix);
    ...
    if (r_showLightInteractions->integer) {
        ...
        for (iaCount = 0, ia = &backEnd.viewParms.interactions[0];
             iaCount < backEnd.viewParms.numInteractions;
             ia++, iaCount++) {
            backEnd.currentEntity = entity = ia->entity;
            light = ia->light;
            surface = ia->surface;
        ...
        for (i = 0; i < backEnd.refdef.numEntities; i++, ent++) {
                ...
                if (model) {
                    switch (model->type) {
                        case MOD_MD5: {
                            ...
                            for (j = 0, bone = &skeleton.bones[0]; j < skeleton.numBones;
                                 j++, bone++) {
            ...
            if (skel) {
                ...
                for (j = 0; j < skel->numBones; j++) {
                    ...
                    MatrixTransformPoint(backEnd.orientation.transformMatrix,
                                         skel->bones[j].t.trans,
                                         worldOrigins[j]);
                }
        ...
        for (z = 0; z < tr.world->lightGridBounds[2]; z++) {
            for (y = 0; y < tr.world->lightGridBounds[1]; y++) {
                ...
                for (x = 0; x < tr.world->lightGridBounds[0]; x++) {
                    ...
                    if (VectorDistanceSquared(origin,
                                              backEnd.viewParms.orientation.origin) >
                        Square(1024)) {
                        continue;
                    }

                    R_LightForPoint(origin, ambientColor, directedColor, lightDir);
        ...
        for (int i = 0; i < 2; i++) {
            ...
            if (i == 1) {
...

                {
                    ...
                    for (j = 0; j < 6; j++) {
                        VectorCopy(backEnd.viewParms.frustums[0][j].normal,
                                   splitFrustum[j]);
    ...
    if (r_showDecalProjectors->integer) {
        ...
        for (i = 0, dp = backEnd.refdef.decalProjectors;
             i < backEnd.refdef.numDecalProjectors;
             i++, dp++) {
            if (VectorDistanceSquared(dp->center,
                                      backEnd.viewParms.orientation.origin) >
                Square(1024)) {
                continue;
            }
        ...
        for (i = 0, srfDecal = backEnd.refdef.decals; i < backEnd.refdef.numDecals;
             i++, srfDecal++) {
...
void DebugDrawVertex(const vec3_t pos, unsigned int color, const vec2_t uv) {
    u8vec4_t colors = {static_cast<byte>(color & 0xFF),
...
static void RB_RenderView() {
    ...
    if (r_logFile->integer) {
        // don't just call LogComment, or we will get a call to va() every frame!
        GLimp_LogComment(
                va("--- RB_RenderView( %i surfaces, %i interactions ) ---\n",
    ...
    if (tr.refdef.blurVec[0] != 0.0f || tr.refdef.blurVec[1] != 0.0f ||
    ...
    if (tr.refdef.pixelTarget != nullptr) {
        ...
        // Bugfix: drivers absolutely hate running in high res and using
        // glReadPixels near the top or bottom edge.
        // Sooo... let's do it in the middle.
        glReadPixels(glConfig.vidWidth / 2, glConfig.vidHeight / 2, tr.refdef.pixelTargetWidth, tr.refdef.pixelTargetHeight, GL_RGBA, GL_UNSIGNED_BYTE, tr.refdef.pixelTarget);

        for (i = 0; i < tr.refdef.pixelTargetWidth * tr.refdef.pixelTargetHeight;
             i++) {
...
const void* RB_SetColorGrading(const void* data) {
    ...
    if (cmd->image->width == REF_COLORGRADEMAP_SIZE) {
        glTexSubImage3D(GL_TEXTURE_3D, 0, 0, 0, cmd->slot * REF_COLORGRADEMAP_SIZE, REF_COLORGRADEMAP_SIZE, REF_COLORGRADEMAP_SIZE, REF_COLORGRADEMAP_SIZE, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);
    } else {
        int i;

        glPixelStorei(GL_UNPACK_ROW_LENGTH,
                      REF_COLORGRADEMAP_SIZE * REF_COLORGRADEMAP_SIZE);

        for (i = 0; i < 16; i++) {
            glTexSubImage3D(
                    GL_TEXTURE_3D, 0, 0, 0, i + cmd->slot * REF_COLORGRADEMAP_SIZE, REF_COLORGRADEMAP_SIZE, REF_COLORGRADEMAP_SIZE, 1, GL_RGBA, GL_UNSIGNED_BYTE, ((u8vec4_t*) nullptr) + REF_COLORGRADEMAP_SIZE);
        }
...
const void* RB_Draw2dPolys(const void* data) {
    ...
    for (i = 0; i < cmd->numverts; i++) {
        ...
        tess.verts[tess.numVertexes].texCoords[0] =
                floatToHalf(cmd->verts[i].st[0]);
        tess.verts[tess.numVertexes].texCoords[1] =
                floatToHalf(cmd->verts[i].st[1]);
...
const void* RB_Draw2dPolysIndexed(const void* data) {
    ...
    for (i = 0; i < cmd->numverts; i++) {
        tess.verts[tess.numVertexes].xyz[0] =
                cmd->verts[i].xyz[0] + cmd->translation[0];
        tess.verts[tess.numVertexes].xyz[1] =
                cmd->verts[i].xyz[1] + cmd->translation[1];
        tess.verts[tess.numVertexes].xyz[2] = 0.0f;
...
const void* RB_RunVisTests(const void* data) {
    ...
    for (i = 0; i < backEnd.refdef.numVisTests; i++) {
        ...
        visTestQueries_t* testState =
                &backEnd.visTestQueries[test->visTestHandle - 1];

        if (testState->running && !test->discardExisting) {
            GLint available;
            GLuint result, resultRef;

            glGetQueryObjectiv(testState->hQuery, GL_QUERY_RESULT_AVAILABLE, &available);
            if (!available) {
                continue;
            }

            glGetQueryObjectiv(testState->hQueryRef, GL_QUERY_RESULT_AVAILABLE, &available);
            if (!available) {
                continue;
            }

            glGetQueryObjectuiv(testState->hQueryRef, GL_QUERY_RESULT, &resultRef);
            glGetQueryObjectuiv(testState->hQuery, GL_QUERY_RESULT, &result);
        ...
        Tess_MapVBOs(false);
        VectorSubtract(backEnd.orientation.viewOrigin, test->position, diff);
        VectorNormalize(diff);
        VectorMA(test->position, test->depthAdjust, diff, center);

        VectorScale(backEnd.viewParms.orientation.axis[1], test->area, left);
        VectorScale(backEnd.viewParms.orientation.axis[2], test->area, up);
...
const void* RB_SwapBuffers(const void* data) {
    ...
    if (r_measureOverdraw->integer) {
        ...
        stencilReadback = (unsigned char*) ri.Hunk_AllocateTempMemory(
