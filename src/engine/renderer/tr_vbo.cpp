...
static void R_SetAttributeLayoutsVertexAnimation(VBO_t* vbo) {
    ...
    vbo->attribs[ATTR_INDEX_POSITION].ofs =
            offsetof(struct fmtVertexAnim1, position);
    vbo->attribs[ATTR_INDEX_POSITION].realStride = sizeVertexAnim1;
    vbo->attribs[ATTR_INDEX_POSITION].stride = sizeVertexAnim1;
    vbo->attribs[ATTR_INDEX_POSITION].frameOffset =
            sizeVertexAnim1 * vbo->vertexesNum;
    ...
    vbo->attribs[ATTR_INDEX_QTANGENT].ofs =
            offsetof(struct fmtVertexAnim1, qtangents);
    vbo->attribs[ATTR_INDEX_QTANGENT].realStride = sizeVertexAnim1;
    vbo->attribs[ATTR_INDEX_QTANGENT].stride = sizeVertexAnim1;
    vbo->attribs[ATTR_INDEX_QTANGENT].frameOffset =
            sizeVertexAnim1 * vbo->vertexesNum;
    ...
    vbo->attribs[ATTR_INDEX_TEXCOORD].ofs =
            sizePart1 + offsetof(struct fmtVertexAnim2, texcoord);
    ...
    vbo->attribs[ATTR_INDEX_COLOR].ofs =
            sizePart1 + offsetof(struct fmtVertexAnim2, colour);
...
static void R_SetAttributeLayoutsSkeletal(VBO_t* vbo) {
    ...
    vbo->attribs[ATTR_INDEX_POSITION].normalize = GL_TRUE;
    vbo->attribs[ATTR_INDEX_POSITION].ofs =
            offsetof(struct fmtSkeletal, position);
    ...
    vbo->attribs[ATTR_INDEX_TEXCOORD].normalize = GL_FALSE;
    vbo->attribs[ATTR_INDEX_TEXCOORD].ofs =
            offsetof(struct fmtSkeletal, texcoord);
    ...
    vbo->attribs[ATTR_INDEX_QTANGENT].normalize = GL_TRUE;
    vbo->attribs[ATTR_INDEX_QTANGENT].ofs =
            offsetof(struct fmtSkeletal, qtangents);
    ...
    vbo->attribs[ATTR_INDEX_BONE_FACTORS].ofs =
            offsetof(struct fmtSkeletal, boneFactors);
...
static void R_SetAttributeLayoutsStatic(VBO_t* vbo) {
    ...
    vbo->attribs[ATTR_INDEX_ORIENTATION].ofs =
            offsetof(shaderVertex_t, spriteOrientation);
...
static inline unsigned short boneFactor(int index, float weight) {
    ...
    for (v = 0; v < vbo->vertexesNum; v++) {
        if (vbo->layout == VBO_LAYOUT_SKELETAL) {
            ...
            if ((vbo->attribBits & ATTR_BONE_FACTORS)) {
                ...
                for (j = 1; j < 4; j++) {
                    ptr[v].boneFactors[j] =
                            boneFactor(inData.boneIndexes[v][j], inData.boneWeights[v][j]);
                }
            }
        } else if (vbo->layout == VBO_LAYOUT_STATIC) {
            ...
            struct fmtVertexAnim2* ptr =
                    (struct fmtVertexAnim2*) (outData +
                                              (vbo->framesNum * vbo->vertexesNum) *
                                                      sizeVertexAnim1);
...
============
*/
static void R_InitRingbuffer(GLenum target, GLsizei elementSize, GLsizei segmentElements, glRingbuffer_t* rb) {
    GLsizei totalSize = elementSize * segmentElements * DYN_BUFFER_SEGMENTS;
    int i;

    glBufferStorage(target, totalSize, nullptr, GL_MAP_WRITE_BIT | GL_MAP_PERSISTENT_BIT);
    rb->baseAddr = glMapBufferRange(target, 0, totalSize, GL_MAP_WRITE_BIT | GL_MAP_PERSISTENT_BIT | GL_MAP_FLUSH_EXPLICIT_BIT);
...
static GLsizei R_RotateRingbuffer(glRingbuffer_t* rb) {
    ...
    while (glClientWaitSync(rb->syncs[rb->activeSegment],
...
#if defined(GLEW_ARB_buffer_storage) && defined(GLEW_ARB_sync)
    if (glConfig2.bufferStorageAvailable && glConfig2.syncAvailable) {
...
#ifdef GLEW_ARB_buffer_storage
    if (glConfig2.bufferStorageAvailable) {
        glBufferStorage(GL_ARRAY_BUFFER, vbo->vertexesSize, verts, 0);
    } else
#endif
    {
        glBufferData(GL_ARRAY_BUFFER, vbo->vertexesSize, verts, vbo->usage);
    }
...
static void R_InitUnitCubeVBO() {
    ...
    data.xyz = (vec3_t*) ri.Hunk_AllocateTempMemory(tess.numVertexes *
                                                    sizeof(*data.xyz));
    ...
    tr.unitCubeIBO =
            R_CreateStaticIBO("unitCube_IBO", tess.indexes, tess.numIndexes);
...
void R_InitVBOs() {
    ...
    tess.vertsBuffer = (shaderVertex_t*) Com_Allocate_Aligned(
            64, SHADER_MAX_VERTEXES * sizeof(shaderVertex_t));
    tess.indexesBuffer = (glIndex_t*) Com_Allocate_Aligned(
            64, SHADER_MAX_INDEXES * sizeof(glIndex_t));
...
#if defined(GLEW_ARB_buffer_storage) && defined(GLEW_ARB_sync)
    else if (glConfig2.bufferStorageAvailable && glConfig2.syncAvailable) {
        R_BindVBO(tess.vbo);
...
void Tess_MapVBOs(bool forceCPU) {
    ...
    if (tess.verts == nullptr) {
        ...
        if (glConfig2.bufferStorageAvailable && glConfig2.syncAvailable) {
            GLsizei segmentEnd =
                    (tess.vertexRB.activeSegment + 1) * tess.vertexRB.segmentElements;
...
#endif
        {
            ...
            tess.verts = (shaderVertex_t*) glMapBufferRange(
                    GL_ARRAY_BUFFER, tess.vertsWritten * sizeof(shaderVertex_t), SHADER_MAX_VERTEXES * sizeof(shaderVertex_t), GL_MAP_WRITE_BIT | GL_MAP_INVALIDATE_RANGE_BIT | GL_MAP_UNSYNCHRONIZED_BIT | GL_MAP_FLUSH_EXPLICIT_BIT);
        }
    }
    ...
    if (tess.indexes == nullptr) {
        ...
        if (glConfig2.bufferStorageAvailable && glConfig2.syncAvailable) {
            GLsizei segmentEnd =
                    (tess.indexRB.activeSegment + 1) * tess.indexRB.segmentElements;
...
#endif
        {
            ...
            tess.indexes = (glIndex_t*) glMapBufferRange(
                    GL_ELEMENT_ARRAY_BUFFER, tess.indexesWritten * sizeof(glIndex_t), SHADER_MAX_INDEXES * sizeof(glIndex_t), GL_MAP_WRITE_BIT | GL_MAP_INVALIDATE_RANGE_BIT | GL_MAP_UNSYNCHRONIZED_BIT | GL_MAP_FLUSH_EXPLICIT_BIT);
        }
    }
}
...
void Tess_UpdateVBOs() {
    ...
    if (tess.numIndexes > 0 && tess.numIndexes <= SHADER_MAX_INDEXES) {
        ...
        if (!glConfig2.mapBufferRangeAvailable) {
            R_BindIBO(tess.ibo);
            glBufferSubData(GL_ELEMENT_ARRAY_BUFFER, 0, size, tess.indexes);
        } else {
            R_BindIBO(tess.ibo);
            ...
            if (glConfig2.bufferStorageAvailable && glConfig2.syncAvailable) {
                ...
                glFlushMappedBufferRange(GL_ELEMENT_ARRAY_BUFFER, offset, size);
            } else {
                glFlushMappedBufferRange(GL_ELEMENT_ARRAY_BUFFER, 0, size);
                glUnmapBuffer(GL_ELEMENT_ARRAY_BUFFER);
            }
