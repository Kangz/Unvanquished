...
void Tess_EndBegin() {
    Tess_End();
    Tess_Begin(tess.stageIteratorFunc, tess.stageIteratorFunc2, tess.surfaceShader, tess.lightShader, tess.skipTangentSpaces, tess.skipVBO, tess.lightmapNum, tess.fogNum);
}
...
static void Tess_CheckVBOAndIBO(VBO_t* vbo, IBO_t* ibo) {
    if (glState.currentVBO != vbo || glState.currentIBO != ibo ||
...
void Tess_CheckOverflow(int verts, int indexes) {
    ...
    if (r_logFile->integer) {
        ...
        GLimp_LogComment(
                va("--- Tess_CheckOverflow(%i + %i vertices, %i + %i triangles ) ---\n",
                   tess.numVertexes,
                   verts,
                   (tess.numIndexes / 3),
                   indexes));
...
==============
*/
static void Tess_SurfaceVertsAndTris(const srfVert_t* verts,
                                     const srfTriangle_t* triangles,
                                     int numVerts,
                                     int numTriangles) {
    int i;
    ...
    for (i = 0; i < numVerts; i++, vert++) {
        ...
        tess.verts[tess.numVertexes + i].texCoords[2] =
                floatToHalf(vert->lightmap[0]);
        tess.verts[tess.numVertexes + i].texCoords[3] =
                floatToHalf(vert->lightmap[1]);
...
static bool Tess_SurfaceVBO(VBO_t* vbo, IBO_t* ibo, int numVerts, int numIndexes, int firstIndex) {
    ...
    glIndex_t* firstIndexOffset =
            (glIndex_t*) BUFFER_OFFSET(firstIndex * sizeof(glIndex_t));
    ...
    if (tess.multiDrawPrimitives > 0) {
        ...
        glIndex_t* prevLastIndexOffset = tess.multiDrawIndexes[lastPrimitive] +
                                         tess.multiDrawCounts[lastPrimitive];
...
void Tess_AddQuadStampExt2(vec4_t quadVerts[4], const vec4_t color, float s1, float t1, float s2, float t2, bool calcNormals) {
    ...
    R_CalcTangents(tangent, binormal, quadVerts[0], quadVerts[1], quadVerts[2], st[0], st[1], st[2]);
    // if ( !calcNormals )
    //{
...
void Tess_AddSprite(const vec3_t center, const u8vec4_t color, float radius, float rotation) {
    ...
    for (i = 0; i < 4; i++) {
        ...
        Vector4Set(texCoord, 0.5f * (i & 2), 0.5f * ((i + 1) & 2), 0.5f * (i & 2), 0.5f * ((i + 1) & 2));
    ...
    tess.attribsSet |=
            ATTR_POSITION | ATTR_COLOR | ATTR_TEXCOORD | ATTR_ORIENTATION;
}
...
static void Tess_SurfaceSprite() {
    ...
    if (tess.surfaceShader->autoSpriteMode == 1) {
        ...
        Tess_AddSprite(backEnd.currentEntity->e.origin,
                       ...
                       radius,
                       backEnd.currentEntity->e.rotation);
        return;
    }
...
static void Tess_SurfacePolychain(srfPoly_t* p) {
    ...
    for (i = 0; i < p->numVerts; i++) {
        ...
        tess.verts[tess.numVertexes + i].texCoords[0] =
                floatToHalf(p->verts[i].st[0]);
        tess.verts[tess.numVertexes + i].texCoords[1] =
                floatToHalf(p->verts[i].st[1]);
    ...
    if (tess.surfaceShader->interactLight && !tess.skipTangentSpaces) {
        ...
        tangents =
                (vec3_t*) ri.Hunk_AllocateTempMemory(numVertexes * sizeof(vec3_t));
        binormals =
                (vec3_t*) ri.Hunk_AllocateTempMemory(numVertexes * sizeof(vec3_t));
        normals =
                (vec3_t*) ri.Hunk_AllocateTempMemory(numVertexes * sizeof(vec3_t));
        ...
        for (i = 0, indices = tess.indexes + tess.numIndexes; i < numIndexes;
             i += 3, indices += 3) {
        ...
        for (i = 0; i < numVertexes; i++) {
            ...
            R_TBNtoQtangents(tangents[i], binormals[i], normals[i], tess.verts[tess.numVertexes + i].qtangents);
        }
...
void Tess_SurfacePolybuffer(srfPolyBuffer_t* surf) {
    ...
    Tess_CheckOverflow(surf->pPolyBuffer->numVerts,
                       surf->pPolyBuffer->numIndicies);
...
void Tess_SurfaceDecal(srfDecal_t* srf) {
    ...
    for (i = 0; i < srf->numVerts; i++) {
        ...
        tess.verts[tess.numVertexes + i].texCoords[0] =
                floatToHalf(srf->verts[i].st[0]);
        tess.verts[tess.numVertexes + i].texCoords[1] =
                floatToHalf(srf->verts[i].st[1]);
...
static void Tess_SurfaceFace(srfSurfaceFace_t* srf) {
    ...
    if (!r_vboFaces->integer ||
        !Tess_SurfaceVBO(srf->vbo, srf->ibo, srf->numVerts, srf->numTriangles * 3, srf->firstTriangle * 3)) {
...
static void Tess_SurfaceMDV(mdvSurface_t* srf) {
    ...
    for (i = 0, tri = srf->triangles; i < srf->numTriangles; i++, tri++) {
        tess.indexes[tess.numIndexes + i * 3 + 0] =
                tess.numVertexes + tri->indexes[0];
        tess.indexes[tess.numIndexes + i * 3 + 1] =
                tess.numVertexes + tri->indexes[1];
        tess.indexes[tess.numIndexes + i * 3 + 2] =
                tess.numVertexes + tri->indexes[2];
    }
    ...
    oldNormal =
            srf->normals + (backEnd.currentEntity->e.oldframe * srf->numVerts);
    st = srf->st;
    ...
    if (!tess.skipTangentSpaces) {
        ...
        for (i = 0; i < numVertexes; i++) {
            R_TBNtoQtangents(tangents[i], binormals[i], normals[i], tess.verts[numVertexes + i].qtangents);
        }
    ...
    if (tess.skipTangentSpaces) {
        ...
        for (j = 0, v = srf->verts; j < numVertexes; j++, v++) {
            ...
            for (k = 0; k < v->numWeights; k++) {
                TransformPoint(&bones[v->boneIndexes[k]], v->position, tmp);
                VectorMA(tess.verts[tess.numVertexes + j].xyz, v->boneWeights[k], tmp, tess.verts[tess.numVertexes + j].xyz);
            }

            tess.verts[tess.numVertexes + j].texCoords[0] =
                    floatToHalf(v->texCoords[0]);
            tess.verts[tess.numVertexes + j].texCoords[1] =
                    floatToHalf(v->texCoords[1]);
        }
    } else {
        tess.attribsSet |= ATTR_QTANGENT;
        ...
        for (j = 0, v = srf->verts; j < numVertexes; j++, v++) {
            ...
            for (k = 0; k < v->numWeights; k++) {
                ...
                TransformNormalVector(&bones[v->boneIndexes[k]], v->normal, tmp);
                VectorMA(normal, v->boneWeights[k], tmp, normal);

                TransformNormalVector(&bones[v->boneIndexes[k]], v->tangent, tmp);
                VectorMA(tangent, v->boneWeights[k], tmp, tangent);

                TransformNormalVector(&bones[v->boneIndexes[k]], v->binormal, tmp);
...
void Tess_SurfaceIQM(srfIQModel_t* surf) {
    ...
    if (surf->vbo && surf->ibo) {
        if (model->num_joints > 0) {
            ...
            TransInitScale(model->internalScale *
        ...
        tess.multiDrawIndexes[tess.multiDrawPrimitives] =
                ((glIndex_t*) nullptr) + surf->first_triangle * 3;
    ...
    for (i = 0; i < surf->num_triangles; i++) {
        tess.indexes[tess.numIndexes + i * 3 + 0] =
                offset + model->triangles[3 * (surf->first_triangle + i) + 0];
        tess.indexes[tess.numIndexes + i * 3 + 1] =
                offset + model->triangles[3 * (surf->first_triangle + i) + 1];
        tess.indexes[tess.numIndexes + i * 3 + 2] =
                offset + model->triangles[3 * (surf->first_triangle + i) + 2];
    }
    ...
    if (model->num_joints > 0 && model->blendWeights && model->blendIndexes) {
        ...
        for (i = 0; i < surf->num_vertexes; i++) {
            ...
            for (j = 0; j < 4; j++) {
                ...
                TransformPoint(&bones[bone], &model->positions[3 * idxIn], tmp);
                VectorMA(tess.verts[idxOut].xyz, weight, tmp, tess.verts[idxOut].xyz);

                TransformNormalVector(&bones[bone], &model->normals[3 * idxIn], tmp);
                VectorMA(normal, weight, tmp, normal);
                TransformNormalVector(&bones[bone], &model->tangents[3 * idxIn], tmp);
                VectorMA(tangent, weight, tmp, tangent);
                TransformNormalVector(&bones[bone], &model->bitangents[3 * idxIn], tmp);
        ...
        for (i = 0; i < surf->num_vertexes; i++) {
            ...
            int idxOut = tess.numVertexes + i;
            float scale =
                    model->internalScale * backEnd.currentEntity->e.skeleton.scale;

            VectorScale(&model->positions[3 * idxIn], scale, tess.verts[idxOut].xyz);
            R_TBNtoQtangents(
                    &model->tangents[3 * idxIn], &model->bitangents[3 * idxIn], &model->normals[3 * idxIn], tess.verts[idxOut].qtangents);
...
static void Tess_SurfaceVBOMD5Mesh(srfVBOMD5Mesh_t* srf) {
    ...
    for (i = 0; i < srf->numBoneRemap; i++) {
        refBone_t* bone =
                &backEnd.currentEntity->e.skeleton.bones[srf->boneRemapInverse[i]];

        if (backEnd.currentEntity->e.skeleton.type == SK_ABSOLUTE) {
            TransInitRotationQuat(model->bones[srf->boneRemapInverse[i]].rotation,
                                  &tess.bones[i]);
            TransAddTranslation(model->bones[srf->boneRemapInverse[i]].origin,
                                &tess.bones[i]);
