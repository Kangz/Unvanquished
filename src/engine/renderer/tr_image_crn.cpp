...
void LoadCRN(const char* name, byte** data, int* width, int* height, int* numLayers, int* numMips, int* bits, byte alphaByte) {
    byte* buff;
    ...
    *numLayers = ti.m_faces == 6 ? 6 : 0;
    ...
    for (i = 0; i < ti.m_levels; i++) {
        crnd::crnd_unpack_level(ctx, (void**) &data[i * ti.m_faces], data[i * ti.m_faces + 1] - data[i * ti.m_faces], 0, i);
    }
