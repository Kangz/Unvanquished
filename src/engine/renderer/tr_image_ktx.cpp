...
static const byte KTX_identifier[12] = {0xAB, 0x4B, 0x54, 0x58, 0x20, 0x31, 0x31, 0xBB, 0x0D, 0x0A, 0x1A, 0x0A};
static const uint32_t KTX_endianness = 0x04030201;
static const uint32_t KTX_endianness_reverse = 0x01020304;

void LoadKTX(const char* name, byte** data, int* width, int* height, int* numLayers, int* numMips, int* bits, byte alphaByte) {
    KTX_header_t* hdr;
    byte* ptr;
    size_t bufLen, size;
    uint32_t imageSize;
    ...
    *numLayers = 0;
    ...
    if (hdr->numberOfArrayElements != 0 ||
        ...
        hdr->pixelWidth == 0 || hdr->pixelHeight == 0 || hdr->pixelDepth != 0) {
        ri.FS_FreeFile(hdr);
        return;
    }
...
void SaveImageKTX(const char* path, image_t* img) {
    ...
    glGetTexLevelParameteriv(target, 0, GL_TEXTURE_INTERNAL_FORMAT, (GLint*) &hdr.glInternalFormat);
    switch (hdr.glInternalFormat) {
        case GL_ALPHA4:
        case GL_ALPHA8:
    ...
    glGetTexLevelParameteriv(target, 0, GL_TEXTURE_WIDTH, (GLint*) &hdr.pixelWidth);
    ...
    if (img->type == GL_TEXTURE_CUBE_MAP) {
        ...
        glGetTexLevelParameteriv(target, 0, GL_TEXTURE_HEIGHT, (GLint*) &hdr.pixelHeight);
        if (img->type == GL_TEXTURE_2D) {
            hdr.pixelDepth = 0;
        } else {
            glGetTexLevelParameteriv(target, 0, GL_TEXTURE_DEPTH, (GLint*) &hdr.pixelDepth);
        }
    }

    hdr.numberOfMipmapLevels = 1;
    glGetTexParameteriv(target, GL_TEXTURE_MIN_FILTER, (GLint*) &i);
    ...
    for (i = size = 0; i < hdr.numberOfMipmapLevels; i++) {
        size += 4;
        if (!hdr.glFormat) {
            glGetTexLevelParameteriv(target, i, GL_TEXTURE_COMPRESSED_IMAGE_SIZE, &mipSize);
        } else {
        ...
        *(int32_t*) ptr = PAD(mipSize, 4);
        ...
        for (j = 0; j < hdr.numberOfFaces; j++) {
            if (!hdr.glFormat) {
                ...
                glGetTexImage(target + j, i, hdr.glFormat, hdr.glType, ptr);
            }
            ptr += PAD(mipSize, 4);
        }
