...
void LoadTGA(const char* name, byte** pic, int* width, int* height, int* numLayers, int* numMips, int* bits, byte alphaByte) {
    ...
    *pic = nullptr;
    ...
    if (targa_header.image_type != 2 && targa_header.image_type != 10 &&
        ...
        ri.Error(ERR_DROP,
                 "LoadTGA: Only type 2 (RGB), 3 (gray), and 10 (RGB) TGA images "
                 "supported (%s)",
                 name);
    }
    ...
    if ((targa_header.pixel_size != 32 && targa_header.pixel_size != 24) &&
        ...
        ri.FS_FreeFile(buffer);
        ri.Error(ERR_DROP,
                 "LoadTGA: Only 32 or 24 bit images supported (no colormaps) (%s)",
                 name);
    }
    ...
    *pic = targa_rgba;
    ...
    if (targa_header.attributes & 0x20) {
        ...
        // ri.Printf(PRINT_WARNING, "WARNING: '%s' TGA file header declares top-down
        // image, flipping\n", name);
