...
constant, irrespective of distance, but the intensity should be proportional to
the
...
// flare states maintain visibility over multiple frames for fading layers:
// view, mirror, menu
...
void RB_AddFlare(void* surface, int fogNum, vec3_t point, vec3_t color, vec3_t normal) {
    ...
    if (window[0] < 0 || window[0] >= backEnd.viewParms.viewportWidth ||
        ...
        return; // shouldn't happen, since we check the clip[] above, except for FP
                // rounding
    }
