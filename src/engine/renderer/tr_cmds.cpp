...
void R_PerformanceCounters() {
    ...
    if (r_speeds->integer == RSPEEDS_GENERAL) {
        ri.Printf(
                PRINT_ALL,
                "%i views %i portals %i batches %i surfs %i leafs %i verts %i tris\n",
                backEnd.pc.c_views,
        ...
        ri.Printf(PRINT_ALL, "%i lights %i bout %i pvsout %i interactions\n", tr.pc.c_dlights + tr.pc.c_slights, tr.pc.c_box_cull_light_out, tr.pc.c_pvs_cull_light_out, tr.pc.c_dlightInteractions + tr.pc.c_slightInteractions);

        ri.Printf(PRINT_ALL, "%i draws %i vbos %i ibos %i verts %i tris\n", backEnd.pc.c_drawElements, backEnd.pc.c_vboVertexBuffers, backEnd.pc.c_vboIndexBuffers, backEnd.pc.c_vboVertexes, backEnd.pc.c_vboIndexes / 3);
        ...
        ri.Printf(PRINT_ALL, "(gen) %i pin %i pout %i bin %i bclip %i bout\n", tr.pc.c_plane_cull_in, tr.pc.c_plane_cull_out, tr.pc.c_box_cull_in, tr.pc.c_box_cull_clip, tr.pc.c_box_cull_out);

        ri.Printf(PRINT_ALL,
                  "(mdv) %i sin %i sclip %i sout %i bin %i bclip %i bout\n",
        ...
        ri.Printf(PRINT_ALL,
                  "decal projectors: %d test surfs: %d clip surfs: %d decal surfs: "
                  "%d created: %d\n",
                  tr.pc.c_decalProjectors,
...
void* R_GetCommandBuffer(int bytes) {
    ...
    if (cmdList->used + bytes + (sizeof(swapBuffersCommand_t) + sizeof(int)) >
        MAX_RENDER_COMMANDS) {
        if (bytes >
            MAX_RENDER_COMMANDS - (sizeof(swapBuffersCommand_t) + sizeof(int))) {
...
static bool R_ClipRegion(float* x, float* y, float* w, float* h, float* s1, float* t1, float* s2, float* t2) {
    ...
    if (right <= clipLeft || left >= clipRight || bottom <= clipTop ||
        top >= clipBottom) {
        return true;
    }
...
void RE_BeginFrame() {
    ...
    if (!r_ignoreGLErrors->integer) {
        ...
        if ((err = glGetError()) != GL_NO_ERROR) {
            ...
            // ri.Error(ERR_FATAL, "caught OpenGL error: %s in file %s line %i", s,
            // filename, line);
