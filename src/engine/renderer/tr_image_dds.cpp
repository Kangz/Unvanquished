...
typedef struct {
    ...
    unsigned int
            dwPitchOrLinearSize; // Formless late-allocated optimized surface size
...
void R_LoadDDSImageData(void* pImageData, const char* name, byte** data, int* width, int* height, int* numLayers, int* numMips, int* bits) {
    byte* buff;
...
// beware: we ignore some of the shorts.
    ...
    if (ddsd->dwSize != sizeof(DDSHEADER_t) ||
        ddsd->ddpfPixelFormat.dwSize != sizeof(DDS_PIXELFORMAT_t)) {
        ri.Printf(PRINT_WARNING, "R_LoadDDSImage: invalid dds header \"%s\"\n", name);
        return;
    }

    *numMips = ((ddsd->dwFlags & DDSD_MIPMAPCOUNT) && (ddsd->dwMipMapCount > 1))
                       ? ddsd->dwMipMapCount
                       : 1;

    if (*numMips > MAX_TEXTURE_MIPS) {
        ri.Printf(PRINT_WARNING,
                  "R_LoadDDSImage: dds image has too many mip levels \"%s\"\n",
                  name);
        return;
    }
    ...
    if (ddsd->ddsCaps.dwCaps2 & DDSCAPS2_CUBEMAP) {
        ...
        *numLayers = 6;
        ...
        if (*width & (*width - 1)) {
            ...
            ri.Printf(PRINT_WARNING,
                      "R_LoadDDSImage: cube images must be power of two \"%s\"\n",
                      name);
            return;
        }
    } else if ((ddsd->ddsCaps.dwCaps2 & DDSCAPS2_VOLUME) &&
        ...
        *numLayers = ddsd->dwDepth;
        ...
        if (*numLayers > MAX_TEXTURE_LAYERS) {
            ri.Printf(PRINT_WARNING,
                      "R_LoadDDSImage: dds image has too many layers \"%s\"\n",
                      name);
            return;
        }
        ...
        if (*width & (*width - 1) || *height & (*height - 1) ||
            *numLayers & (*numLayers - 1)) {
            ri.Printf(PRINT_WARNING,
                      "R_LoadDDSImage: volume images must be power of two \"%s\"\n",
                      name);
            return;
        }
    } else {
        ...
        *numLayers = 0;
        ...
        if (compressed && (*width & (*width - 1) || *height & (*height - 1))) {
            ri.Printf(PRINT_WARNING,
                      "R_LoadDDSImage: compressed texture images must be power of "
                      "two \"%s\"\n",
                      name);
            return;
        }
    ...
    if (compressed) {
        ...
        if (*numLayers != 0) {
            ri.Printf(PRINT_WARNING,
                      "R_LoadDDSImage: compressed volume textures are not supported "
                      "\"%s\"\n",
                      name);
            return;
        }
        ...
        switch (ddsd->ddpfPixelFormat.dwFourCC) {
            ...
            default:
                ri.Printf(PRINT_WARNING,
                          "R_LoadDDSImage: unsupported FOURCC 0x%08x, \"%s\"\n",
        ...
        if (ddsd->ddpfPixelFormat.dwFlags & DDPF_RGB) {
            switch (ddsd->ddpfPixelFormat.dwRGBBitCount) {
                ...
                default:
                    ri.Printf(PRINT_WARNING,
                              "R_LoadDDSImage: unsupported RGB bit depth \"%s\"\n",
                              name);
                    return;
            }
        } else {
            ri.Printf(PRINT_WARNING,
                      "R_LoadDDSImage: unsupported DDS image type \"%s\"\n",
                      name);
            return;
        }
    }
...
void LoadDDS(const char* name, byte** data, int* width, int* height, int* numLayers, int* numMips, int* bits, byte alphaByte) {
    byte* buff;
    ...
    R_LoadDDSImageData(buff, name, data, width, height, numLayers, numMips, bits);
