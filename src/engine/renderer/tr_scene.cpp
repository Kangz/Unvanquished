...
static void R_AddPolysToScene(qhandle_t hShader, int numVerts, const polyVert_t* verts, int numPolys) {
    ...
    for (j = 0; j < numPolys; j++) {
       ...
       */
            ri.Printf(PRINT_DEVELOPER,
                      "WARNING: RE_AddPolyToScene: r_max_polys or r_max_polyverts "
                      "reached\n");
            return;
        }
...
void RE_AddDynamicLightToSceneET(const vec3_t org, float radius, float intensity, float r, float g, float b, qhandle_t hShader, int flags) {
    ...
    light->l.noShadows =
            !r_dynamicLightCastShadows->integer && !light->l.inverseShadows;
...
void RE_RenderScene(const refdef_t* fd) {
    ...
    if (!(tr.refdef.rdflags & RDF_NOWORLDMODEL) &&
        !((tr.refdef.rdflags & RDF_SKYBOXPORTAL) && tr.world->numSkyNodes > 0)) {
        int areaDiff;
        int i;
    ...
    tr.refdef.polybuffers =
            &backEndData[tr.smpFrame]->polybuffers[r_firstScenePolybuffer];

    tr.refdef.numDecalProjectors =
            r_numDecalProjectors - r_firstSceneDecalProjector;
    tr.refdef.decalProjectors =
            &backEndData[tr.smpFrame]->decalProjectors[r_firstSceneDecalProjector];
    ...
    if (tr.refdef.pixelTarget == nullptr) {
        ...
        // Driver bug, if we try and do pixel target work along the top edge of a
        // window
        // we can end up capturing part of the status bar. (see screenshot
        // corruption..)
...
void R_UpdateVisTests() {
    ...
    for (i = 0; i < numVisTests; i++) {
        ...
        if (VectorCompare(test->position, res->position) &&
            test->area == res->area && test->depthAdjust == res->depthAdjust) {
...
qhandle_t RE_RegisterVisTest() {
    ...
    if (tr.numVisTests >= MAX_VISTESTS) {
        ri.Printf(PRINT_WARNING,
                  "WARNING: RE_RegisterVisTest - MAX_VISTESTS hit\n");
    }
...
void RE_AddVisTestToScene(qhandle_t hTest, vec3_t pos, float depthAdjust, float area) {
    ...
    if (r_numVisTests == MAX_VISTESTS) {
        ri.Printf(PRINT_WARNING,
                  "WARNING: RE_AddVisTestToScene - MAX_VISTESTS hit\n");
        return;
    }
