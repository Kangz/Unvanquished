...
float R_InterpolateLightGrid(world_t* w, int from[3], int to[3], float* factors[3], vec3_t ambientLight, vec3_t directedLight, vec2_t lightDir) {
    ...
    for (x = from[0], xFactor = factors[0]; x <= to[0]; x++, xFactor++) {
        if (x < 0 || x > w->lightGridBounds[0])
            continue;

        for (y = from[1], yFactor = factors[1]; y <= to[1]; y++, yFactor++) {
            if (y < 0 || y > w->lightGridBounds[1])
                continue;

            for (z = from[2], zFactor = factors[2]; z <= to[2]; z++, zFactor++) {
                if (z < 0 || z > w->lightGridBounds[2])
                    continue;

                gp1 = w->lightGridData1 + x * gridStep[0] + y * gridStep[1] +
                      z * gridStep[2];
                gp2 = w->lightGridData2 + x * gridStep[0] + y * gridStep[1] +
                      z * gridStep[2];
...
int R_LightForPoint(vec3_t point, vec3_t ambientLight, vec3_t directedLight, vec3_t lightDir) {
    ...
    totalFactor = R_InterpolateLightGrid(tr.world, from, to, factors, ambientLight, directedLight, lightDir);
...
static void R_SetupEntityLightingGrid(trRefEntity_t* ent, vec3_t forcedOrigin) {
    ...
    R_LightForPoint(lightOrigin, ent->ambientLight, ent->directedLight, ent->lightDir);
}
...
void R_SetupEntityLighting(const trRefdef_t* refdef, trRefEntity_t* ent, vec3_t forcedOrigin) {
    ...
    if ((ent->e.renderfx &
         RF_MINLIGHT)) // && VectorLength(ent->ambientLight) <= 0)
    {
...
void R_SetupLightLocalBounds(trRefLight_t* light) {
    ...
    light->sphereRadius =
            RadiusFromBounds(light->localBounds[0], light->localBounds[1]);
}
...
void R_SetupLightFrustum(trRefLight_t* light) {
    ...
    if (light->isStatic) {
        ...
        light->frustumVBO =
                R_CreateStaticVBO("staticLightFrustum_VBO", data, VBO_LAYOUT_POSITION);
        light->frustumIBO = R_CreateStaticIBO("staticLightFrustum_IBO",
                                              tess.indexes,
                                              tess.numIndexes);
...
void R_SetupLightProjection(trRefLight_t* light) {
    switch (light->l.rlType) {
        ...
        case RL_PROJ: {
            ...
            // TODO: I don't like hacking the matrix like this, but all attempts to
            // use
...
bool R_AddLightInteraction(trRefLight_t* light, surfaceType_t* surface, shader_t* surfaceShader, byte cubeSideBits, interactionType_t iaType) {
    ...
    if (surfaceShader) {
        if (surfaceShader->isSky ||
            (!surfaceShader->interactLight && surfaceShader->noShadows)) {
            return false;
        }
    } else {
        return false;
...
static int InteractionCompare(const void* a, const void* b) {
    ...
    // then entity
    if (((interaction_t*) a)->entity == &tr.worldEntity &&
        ((interaction_t*) b)->entity != &tr.worldEntity) {
        return -1;
    }

    else if (((interaction_t*) a)->entity != &tr.worldEntity &&
             ((interaction_t*) b)->entity == &tr.worldEntity) {
        return 1;
    }
...
static void R_IntersectRayPlane(const vec3_t v1, const vec3_t v2, cplane_t* plane, vec3_t res) {
    ...
    sect = -(DotProduct(plane->normal, v1) - plane->dist) /
...
Recturns the screen space rectangle taken by the box.
        (Clips the box to the near plane to have correct results even if the box
...
void R_SetupLightScissor(trRefLight_t* light) {
    ...
    light->scissor.coords[2] =
            tr.viewParms.viewportX + tr.viewParms.viewportWidth;
    light->scissor.coords[3] =
            tr.viewParms.viewportY + tr.viewParms.viewportHeight;
    ...
    light->scissor.coords[0] =
            Maths::clamp(light->scissor.coords[0], tr.viewParms.viewportX, tr.viewParms.viewportX + tr.viewParms.viewportWidth);
    light->scissor.coords[2] =
            Maths::clamp(light->scissor.coords[2], tr.viewParms.viewportX, tr.viewParms.viewportX + tr.viewParms.viewportWidth);

    light->scissor.coords[1] =
            Maths::clamp(light->scissor.coords[1], tr.viewParms.viewportY, tr.viewParms.viewportY + tr.viewParms.viewportHeight);
    light->scissor.coords[3] =
            Maths::clamp(light->scissor.coords[3], tr.viewParms.viewportY, tr.viewParms.viewportY + tr.viewParms.viewportHeight);
}
...
byte R_CalcLightCubeSideBits(trRefLight_t* light, vec3_t worldBounds[2]) {
    ...
    float fovX, fovY;
    vec3_t angles;
    matrix_t tmpMatrix, rotationMatrix, transformMatrix, viewMatrix,
    ...
    if (light->l.rlType != RL_OMNI || r_shadows->integer < SHADOWING_ESM16 ||
        r_noShadowPyramids->integer) {
