...
static bool R_CullSurface(surfaceType_t* surface, shader_t* shader, int planeBits) {
    ...
    // ydnar: made surface culling generic, inline with q3map2 surface
    // classification
    if (*surface == SF_GRID && r_nocurves->integer) {
        return true;
    }

    if (*surface != SF_FACE && *surface != SF_TRIANGLES &&
        *surface != SF_VBO_MESH && *surface != SF_GRID) {
        return true;
    }
    ...
    if (*surface == SF_FACE && r_facePlaneCull->integer) {
        ...
        d = DotProduct(tr.orientation.viewOrigin, srf->plane.normal) -
            srf->plane.dist;
...
static void R_RecursiveWorldNode(bspNode_t* node, int planeBits, int decalBits) {
    do {
        ...
        backEndData[tr.smpFrame]
                ->traversalList[backEndData[tr.smpFrame]->traversalLength++] = node;
...
static void R_RecursiveInteractionNode(bspNode_t* node, trRefLight_t* light, int planeBits, int interactionBits) {
    ...
    do {
        ...
        // Tr3B - even surfaces that belong to nodes that are outside of the view
        // frustum
...
static const byte* R_ClusterPVS(int cluster) {
    if (!tr.world || !tr.world->vis || cluster < 0 ||
...
static void R_MarkLeaves() {
    ...
    for (i = 0, leaf = tr.world->nodes; i < tr.world->numnodes; i++, leaf++) {
        ...
        if (tr.refdef.rdflags & RDF_SKYBOXPORTAL) {
            ...
            // this also means the skybox portal cannot move, as this list is
            // calculated once and never again
...
void R_AddWorldSurfaces() {
    ...
    if (tr.refdef.rdflags & RDF_SKYBOXPORTAL && tr.world->numSkyNodes > 0) {
        ...
        for (i = 0, node = tr.world->skyNodes; i < tr.world->numSkyNodes;
             i++, node++) {
...
void R_AddPrecachedWorldInteractions(trRefLight_t* light) {
    ...
    if ((r_vboShadows->integer || r_vboLighting->integer)) {
        ...
        for (iaCache = light->firstInteractionCache; iaCache;
            ...
            // Tr3B - this surface is maybe not in this view but it may still cast a
            // shadow
            // into this view
