...
#define WAVEVALUE(table, base, amplitude, phase, freq)             \
    ((base) +                                                      \
     table[Q_ftol((((phase) + backEnd.refdef.floatTime * (freq)) * \
                   FUNCTABLE_SIZE)) &                              \
           FUNCTABLE_MASK] *                                       \
             (amplitude))
    ...
    for (i = 0; i < exp->numOps; i++) {
        ...
        switch (op.type) {
            ...
            case OP_NEG: {
                if (numOps < 1) {
                    ri.Printf(PRINT_ALL,
                              "WARNING: shader %s has numOps < 1 for unary - operator\n",
            ...
            case OP_TABLE: {
                ...
                if (numOps < 1) {
                    ri.Printf(PRINT_ALL,
                              "WARNING: shader %s has numOps < 1 for table operator\n",
                              tess.surfaceShader->name);
                    return defaultValue;
                }
                ...
                if (table->snap) {
                    ...
                    // lerp value
                    value = table->values[oldIndex] +
                            ((table->values[newIndex] - table->values[oldIndex]) * lerp);
                }
            ...
            default: {
                if (numOps < 2) {
                    ri.Printf(PRINT_ALL,
                              "WARNING: shader %s has numOps < 2 for binary operator %s\n",
                              tess.surfaceShader->name,
                              GetOpName(op.type));
                    return defaultValue;
                }
...
=====================
*/
static void ReorderQuadVerts(shaderVertex_t* vert, glIndex_t* index, glIndex_t minIndex) {
    int newIndex[4];
    ...
    for (i = 0; i < 4; i++) {
        Com_Memcpy(&vert[newIndex[i]], &vertexCopy[i], sizeof(shaderVertex_t));
    }
    for (i = 0; i < 6; i++) {
...
static void AutospriteDeform(int firstVertex, int numVertexes, int firstIndex, int numIndexes) {
    ...
    for (i = 0; i < numVertexes; i += 4) {
        ...
        for (j = 0; j < 4; j++) {
            ...
            Vector4Set(v[j].spriteOrientation, 0, 0, 0, floatToHalf(radius));
        }
    }
}
