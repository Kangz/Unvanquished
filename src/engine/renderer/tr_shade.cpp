...
static void BindLightMap(int tmu) {
    ...
    } else if (tess.lightmapNum >= 0 &&
               tess.lightmapNum < tr.lightmaps.currentElements) {
...
static void BindDeluxeMap(int tmu) {
    ...
    if (tess.lightmapNum >= 0 &&
        tess.lightmapNum < tr.deluxemaps.currentElements) {
        deluxemap =
                (image_t*) Com_GrowListElement(&tr.deluxemaps, tess.lightmapNum);
    } else {
        deluxemap = nullptr;
    }
...
static void DrawTris() {
    ...
    gl_genericShader->SetVertexSkinning(glConfig2.vboVertexSkinningAvailable &&
                                        tess.vboVertexSkinning);
    ...
    // u_DeformGen
    gl_genericShader->SetUniform_DeformParms(tess.surfaceShader->deforms,
    ...
    gl_genericShader->SetUniform_ColorTextureMatrix(
            tess.svars.texMatrices[TB_COLORMAP]);
...
*/
// *INDENT-OFF*
void Tess_Begin(void (*stageIteratorFunc)(), void (*stageIteratorFunc2)(), shader_t* surfaceShader, shader_t* lightShader, bool skipTangentSpaces, bool skipVBO, int lightmapNum, int fogNum) {
    shader_t* state;
    ...
    if (surfaceShader != nullptr) {
        state = (surfaceShader->remappedShader) ? surfaceShader->remappedShader
                                                : surfaceShader;
    ...
    if (r_logFile->integer) {
        ...
        GLimp_LogComment(
                va("--- Tess_Begin( surfaceShader = %s, lightShader = %s, "
                   "skipTangentSpaces = %i, lightmapNum = %i, fogNum = %i) ---\n",
                   tess.surfaceShader->name,
                   tess.lightShader ? tess.lightShader->name : nullptr,
                   tess.skipTangentSpaces,
                   tess.lightmapNum,
                   tess.fogNum));
    }
}
...
static void Render_generic(int stage) {
    ...
    if (tess.vboVertexAnimation) {
        gl_genericShader->SetUniform_VertexInterpolation(
...
static void Render_vertexLighting_DBS_entity(int stage) {
    ...
    bool normalMapping = r_normalMapping->integer &&
                         (pStage->bundle[TB_NORMALMAP].image[0] != nullptr);
    bool glowMapping = (pStage->bundle[TB_GLOWMAP].image[0] != nullptr);

    // choose right shader program ----------------------------------
    gl_vertexLightingShader_DBS_entity->SetVertexSkinning(
            glConfig2.vboVertexSkinningAvailable && tess.vboVertexSkinning);
    gl_vertexLightingShader_DBS_entity->SetVertexAnimation(
            tess.vboVertexAnimation);

    tess.vboVertexSprite = false;

    gl_vertexLightingShader_DBS_entity->SetNormalMapping(normalMapping);
    gl_vertexLightingShader_DBS_entity->SetParallaxMapping(
            normalMapping && r_parallaxMapping->integer &&
            tess.surfaceShader->parallax);

    gl_vertexLightingShader_DBS_entity->SetReflectiveSpecular(
            normalMapping && tr.cubeHashTable != nullptr);
    ...
    if (glConfig2.vboVertexSkinningAvailable && tess.vboVertexSkinning) {
        gl_vertexLightingShader_DBS_entity->SetUniform_Bones(tess.numBones,
                                                             tess.bones);
    }

    // set uniforms
    VectorCopy(backEnd.viewParms.orientation.origin,
               viewOrigin); // in world space
    ...
    gl_vertexLightingShader_DBS_entity->SetUniform_ModelMatrix(
            backEnd.orientation.transformMatrix);
    gl_vertexLightingShader_DBS_entity->SetUniform_ModelViewProjectionMatrix(
            glState.modelViewProjectionMatrix[glState.stackIndex]);

    if (tr.world) {
        gl_vertexLightingShader_DBS_entity->SetUniform_LightGridOrigin(
                tr.world->lightGridGLOrigin);
        gl_vertexLightingShader_DBS_entity->SetUniform_LightGridScale(
                tr.world->lightGridGLScale);
    }

    // u_VertexInterpolation
    if (tess.vboVertexAnimation) {
        gl_vertexLightingShader_DBS_entity->SetUniform_VertexInterpolation(
                glState.vertexAttribsInterpolation);
    }

    // u_DeformGen
    gl_vertexLightingShader_DBS_entity->SetUniform_DeformParms(
            tess.surfaceShader->deforms, tess.surfaceShader->numDeforms);
    gl_vertexLightingShader_DBS_entity->SetUniform_Time(
    ...
    if (r_parallaxMapping->integer && tess.surfaceShader->parallax) {
        ...
        depthScale =
                RB_EvalExpression(&pStage->depthScaleExp, r_parallaxDepthScale->value);
        gl_vertexLightingShader_DBS_entity->SetUniform_DepthScale(depthScale);
    }

    // bind u_DiffuseMap
    GL_BindToTMU(0, pStage->bundle[TB_DIFFUSEMAP].image[0]);
    gl_vertexLightingShader_DBS_entity->SetUniform_DiffuseTextureMatrix(
    ...
    if (normalMapping) {
        ...
        gl_vertexLightingShader_DBS_entity->SetUniform_NormalTextureMatrix(
        ...
        gl_vertexLightingShader_DBS_entity->SetUniform_SpecularTextureMatrix(
                tess.svars.texMatrices[TB_SPECULARMAP]);

        float minSpec = RB_EvalExpression(&pStage->specularExponentMin,
                                          r_specularExponentMin->value);
        float maxSpec = RB_EvalExpression(&pStage->specularExponentMax,
                                          r_specularExponentMax->value);

        gl_vertexLightingShader_DBS_entity->SetUniform_SpecularExponent(minSpec,
                                                                        maxSpec);
        ...
        if (tr.cubeHashTable != nullptr) {
            ...
            if (backEnd.currentEntity && (backEnd.currentEntity != &tr.worldEntity)) {
                R_FindTwoNearestCubeMaps(backEnd.currentEntity->e.origin,
                                         &cubeProbeNearest,
                                         &cubeProbeSecondNearest);
            } else {
                // FIXME position
                R_FindTwoNearestCubeMaps(backEnd.viewParms.orientation.origin,
                                         &cubeProbeNearest,
                                         &cubeProbeSecondNearest);
            }
            ...
            if (cubeProbeNearest == nullptr && cubeProbeSecondNearest == nullptr) {
                ...
                if (backEnd.currentEntity &&
                    ...
                    cubeProbeNearestDistance = Distance(backEnd.currentEntity->e.origin,
                                                        cubeProbeNearest->origin);
                    cubeProbeSecondNearestDistance = Distance(
                            backEnd.currentEntity->e.origin, cubeProbeSecondNearest->origin);
                } else {
                    // FIXME position
                    cubeProbeNearestDistance = Distance(
                            backEnd.viewParms.orientation.origin, cubeProbeNearest->origin);
                    cubeProbeSecondNearestDistance =
                            Distance(backEnd.viewParms.orientation.origin,
                                     cubeProbeSecondNearest->origin);
                }

                float interpolate =
                        cubeProbeNearestDistance /
                        (cubeProbeNearestDistance + cubeProbeSecondNearestDistance);

                if (r_logFile->integer) {
                    GLimp_LogComment(
                            va("cubeProbeNearestDistance = %f, "
                               "cubeProbeSecondNearestDistance = %f, interpolation = %f\n",
                               cubeProbeNearestDistance,
                               cubeProbeSecondNearestDistance,
                               interpolate));
                }
    ...
    if (glowMapping) {
        ...
        gl_vertexLightingShader_DBS_entity->SetUniform_GlowTextureMatrix(
                tess.svars.texMatrices[TB_GLOWMAP]);
    }
...
static void Render_vertexLighting_DBS_world(int stage) {
    ...
    gl_vertexLightingShader_DBS_world->SetParallaxMapping(
            normalMapping && r_parallaxMapping->integer &&
            tess.surfaceShader->parallax);
    ...
    // u_DeformGen
    gl_vertexLightingShader_DBS_world->SetUniform_DeformParms(
            tess.surfaceShader->deforms, tess.surfaceShader->numDeforms);
    gl_vertexLightingShader_DBS_world->SetUniform_Time(
            backEnd.refdef.floatTime - backEnd.currentEntity->e.shaderTime);
    ...
    gl_vertexLightingShader_DBS_world->SetUniform_ColorModulate(colorGen,
                                                                alphaGen);

    // u_Color
    { gl_vertexLightingShader_DBS_world->SetUniform_Color(tess.svars.color); }

    gl_vertexLightingShader_DBS_world->SetUniform_LightWrapAround(
            RB_EvalExpression(&pStage->wrapAroundLightingExp, 0));

    gl_vertexLightingShader_DBS_world->SetUniform_ViewOrigin(viewOrigin);
    gl_vertexLightingShader_DBS_world->SetUniform_ModelViewProjectionMatrix(
            glState.modelViewProjectionMatrix[glState.stackIndex]);
    ...
    if (tr.world) {
        gl_vertexLightingShader_DBS_world->SetUniform_LightGridOrigin(
                tr.world->lightGridGLOrigin);
        gl_vertexLightingShader_DBS_world->SetUniform_LightGridScale(
                tr.world->lightGridGLScale);
    }

    // bind u_DiffuseMap
    GL_BindToTMU(0, pStage->bundle[TB_DIFFUSEMAP].image[0]);
    gl_vertexLightingShader_DBS_world->SetUniform_DiffuseTextureMatrix(
            tess.svars.texMatrices[TB_DIFFUSEMAP]);
    ...
    if (normalMapping) {
        ...
        gl_vertexLightingShader_DBS_world->SetUniform_NormalTextureMatrix(
                tess.svars.texMatrices[TB_NORMALMAP]);
        ...
        gl_vertexLightingShader_DBS_world->SetUniform_SpecularTextureMatrix(
                tess.svars.texMatrices[TB_SPECULARMAP]);
        ...
        gl_vertexLightingShader_DBS_world->SetUniform_SpecularExponent(minSpec,
                                                                       maxSpec);
    }
    ...
    if (glowMapping) {
        ...
        gl_vertexLightingShader_DBS_world->SetUniform_GlowTextureMatrix(
                tess.svars.texMatrices[TB_GLOWMAP]);
    }
...
static void Render_lightMapping(int stage, bool asColorMap, bool normalMapping) {
    ...
    gl_lightMappingShader->SetParallaxMapping(normalMapping &&
    ...
    // u_DeformGen
    gl_lightMappingShader->SetUniform_DeformParms(tess.surfaceShader->deforms,
                                                  tess.surfaceShader->numDeforms);
    gl_lightMappingShader->SetUniform_Time(backEnd.refdef.floatTime -
                                           backEnd.currentEntity->e.shaderTime);

    gl_lightMappingShader->SetUniform_ViewOrigin(
            backEnd.viewParms.orientation.origin); // in world space

    gl_lightMappingShader->SetUniform_ModelMatrix(
            backEnd.orientation.transformMatrix);
    gl_lightMappingShader->SetUniform_ModelViewProjectionMatrix(
            glState.modelViewProjectionMatrix[glState.stackIndex]);
    ...
    if (asColorMap) {
        ...
        GL_BindToTMU(0, pStage->bundle[TB_DIFFUSEMAP].image[0]);
        gl_lightMappingShader->SetUniform_DiffuseTextureMatrix(
                tess.svars.texMatrices[TB_DIFFUSEMAP]);
    }
    ...
    if (normalMapping) {
        ...
        gl_lightMappingShader->SetUniform_NormalTextureMatrix(
                tess.svars.texMatrices[TB_NORMALMAP]);
        ...
        gl_lightMappingShader->SetUniform_SpecularTextureMatrix(
                tess.svars.texMatrices[TB_SPECULARMAP]);

        float specExpMin = RB_EvalExpression(&pStage->specularExponentMin,
                                             r_specularExponentMin->value);
        float specExpMax = RB_EvalExpression(&pStage->specularExponentMax,
                                             r_specularExponentMax->value);
    ...
    if (glowMapping) {
        ...
        gl_lightMappingShader->SetUniform_GlowTextureMatrix(
                tess.svars.texMatrices[TB_GLOWMAP]);
    }
...
static void Render_shadowFill(int stage) {
    ...
    gl_shadowFillShader->SetVertexSkinning(glConfig2.vboVertexSkinningAvailable &&
                                           tess.vboVertexSkinning);
    gl_shadowFillShader->SetVertexAnimation(glState.vertexAttribsInterpolation >
                                            0);

    gl_shadowFillShader->SetMacro_LIGHT_DIRECTIONAL(
            backEnd.currentLight->l.rlType == RL_DIRECTIONAL);
    ...
    if (backEnd.currentLight->l.rlType != RL_DIRECTIONAL) {
        ...
        gl_shadowFillShader->SetUniform_LightRadius(
                backEnd.currentLight->sphereRadius);
    }

    gl_shadowFillShader->SetUniform_ModelMatrix(
            backEnd.orientation.transformMatrix);
    gl_shadowFillShader->SetUniform_ModelViewProjectionMatrix(
            glState.modelViewProjectionMatrix[glState.stackIndex]);
    ...
    // u_VertexInterpolation
    if (tess.vboVertexAnimation) {
        gl_shadowFillShader->SetUniform_VertexInterpolation(
                glState.vertexAttribsInterpolation);
    }

    // u_DeformGen
    gl_shadowFillShader->SetUniform_DeformParms(tess.surfaceShader->deforms,
                                                tess.surfaceShader->numDeforms);
    ...
    if ((pStage->stateBits & GLS_ATEST_BITS) != 0) {
        ...
        gl_shadowFillShader->SetUniform_ColorTextureMatrix(
                tess.svars.texMatrices[TB_COLORMAP]);
    } else {
        GL_BindToTMU(0, tr.whiteImage);
    }
...
static void Render_forwardLighting_DBS_omni(shaderStage_t* diffuseStage,
    ...
    bool normalMapping = r_normalMapping->integer &&
                         (diffuseStage->bundle[TB_NORMALMAP].image[0] != nullptr);

    bool shadowCompare = (r_shadows->integer >= SHADOWING_ESM16 &&
                          !light->l.noShadows && light->shadowLOD >= 0);

    backEnd.depthRenderImageValid = false;

    // choose right shader program ----------------------------------
    gl_forwardLightingShader_omniXYZ->SetVertexSkinning(
            glConfig2.vboVertexSkinningAvailable && tess.vboVertexSkinning);
    gl_forwardLightingShader_omniXYZ->SetVertexAnimation(
            glState.vertexAttribsInterpolation > 0);

    gl_forwardLightingShader_omniXYZ->SetNormalMapping(normalMapping);
    gl_forwardLightingShader_omniXYZ->SetParallaxMapping(
            normalMapping && r_parallaxMapping->integer &&
            tess.surfaceShader->parallax);
    ...
    gl_forwardLightingShader_omniXYZ->SetUniform_ColorModulate(colorGen,
                                                               alphaGen);

    // u_Color
    gl_forwardLightingShader_omniXYZ->SetUniform_Color(tess.svars.color);

    gl_forwardLightingShader_omniXYZ->SetUniform_AlphaTest(
            diffuseStage->stateBits);

    if (r_parallaxMapping->integer) {
        float depthScale;

        depthScale = RB_EvalExpression(&diffuseStage->depthScaleExp,
                                       r_parallaxDepthScale->value);
    ...
    gl_forwardLightingShader_omniXYZ->SetUniform_LightWrapAround(
            RB_EvalExpression(&diffuseStage->wrapAroundLightingExp, 0));
    gl_forwardLightingShader_omniXYZ->SetUniform_LightAttenuationMatrix(
            light->attenuationMatrix2);

    GL_CheckErrors();

    if (shadowCompare) {
        gl_forwardLightingShader_omniXYZ->SetUniform_ShadowTexelSize(
                shadowTexelSize);
        gl_forwardLightingShader_omniXYZ->SetUniform_ShadowBlur(
                r_shadowBlur->value);
    }

    GL_CheckErrors();

    gl_forwardLightingShader_omniXYZ->SetUniform_ModelMatrix(
            backEnd.orientation.transformMatrix);
    gl_forwardLightingShader_omniXYZ->SetUniform_ModelViewProjectionMatrix(
            glState.modelViewProjectionMatrix[glState.stackIndex]);

    // u_Bones
    if (glConfig2.vboVertexSkinningAvailable && tess.vboVertexSkinning) {
        gl_forwardLightingShader_omniXYZ->SetUniform_Bones(tess.numBones,
                                                           tess.bones);
    }

    // u_VertexInterpolation
    if (tess.vboVertexAnimation) {
        gl_forwardLightingShader_omniXYZ->SetUniform_VertexInterpolation(
                glState.vertexAttribsInterpolation);
    }

    gl_forwardLightingShader_omniXYZ->SetUniform_DeformParms(
            tess.surfaceShader->deforms, tess.surfaceShader->numDeforms);
    gl_forwardLightingShader_omniXYZ->SetUniform_Time(
            backEnd.refdef.floatTime - backEnd.currentEntity->e.shaderTime);
    ...
    gl_forwardLightingShader_omniXYZ->SetUniform_DiffuseTextureMatrix(
            tess.svars.texMatrices[TB_DIFFUSEMAP]);
    ...
    if (normalMapping) {
        ...
        gl_forwardLightingShader_omniXYZ->SetUniform_NormalTextureMatrix(
                tess.svars.texMatrices[TB_NORMALMAP]);
        ...
        gl_forwardLightingShader_omniXYZ->SetUniform_SpecularTextureMatrix(
                tess.svars.texMatrices[TB_SPECULARMAP]);

        float minSpec = RB_EvalExpression(&diffuseStage->specularExponentMin,
                                          r_specularExponentMin->value);
        float maxSpec = RB_EvalExpression(&diffuseStage->specularExponentMax,
                                          r_specularExponentMax->value);

        gl_forwardLightingShader_omniXYZ->SetUniform_SpecularExponent(minSpec,
                                                                      maxSpec);
    }
...
static void Render_forwardLighting_DBS_proj(shaderStage_t* diffuseStage,
    ...
    // choose right shader program ----------------------------------
    gl_forwardLightingShader_projXYZ->SetVertexSkinning(
            glConfig2.vboVertexSkinningAvailable && tess.vboVertexSkinning);
    gl_forwardLightingShader_projXYZ->SetVertexAnimation(
            glState.vertexAttribsInterpolation > 0);

    gl_forwardLightingShader_projXYZ->SetNormalMapping(normalMapping);
    gl_forwardLightingShader_projXYZ->SetParallaxMapping(
            normalMapping && r_parallaxMapping->integer &&
            tess.surfaceShader->parallax);
    ...
    gl_forwardLightingShader_projXYZ->SetUniform_ColorModulate(colorGen,
                                                               alphaGen);

    // u_Color
    gl_forwardLightingShader_projXYZ->SetUniform_Color(tess.svars.color);

    gl_forwardLightingShader_projXYZ->SetUniform_AlphaTest(
            diffuseStage->stateBits);
    ...
    gl_forwardLightingShader_projXYZ->SetUniform_LightWrapAround(
            RB_EvalExpression(&diffuseStage->wrapAroundLightingExp, 0));
    gl_forwardLightingShader_projXYZ->SetUniform_LightAttenuationMatrix(
            light->attenuationMatrix2);

    GL_CheckErrors();

    if (shadowCompare) {
        gl_forwardLightingShader_projXYZ->SetUniform_ShadowTexelSize(
                shadowTexelSize);
        gl_forwardLightingShader_projXYZ->SetUniform_ShadowBlur(
                r_shadowBlur->value);
        gl_forwardLightingShader_projXYZ->SetUniform_ShadowMatrix(
                light->shadowMatrices);
    }

    GL_CheckErrors();

    gl_forwardLightingShader_projXYZ->SetUniform_ModelMatrix(
            backEnd.orientation.transformMatrix);
    gl_forwardLightingShader_projXYZ->SetUniform_ModelViewProjectionMatrix(
            glState.modelViewProjectionMatrix[glState.stackIndex]);

    // u_Bones
    if (glConfig2.vboVertexSkinningAvailable && tess.vboVertexSkinning) {
        gl_forwardLightingShader_projXYZ->SetUniform_Bones(tess.numBones,
                                                           tess.bones);
    }

    // u_VertexInterpolation
    if (tess.vboVertexAnimation) {
        gl_forwardLightingShader_projXYZ->SetUniform_VertexInterpolation(
                glState.vertexAttribsInterpolation);
    }

    gl_forwardLightingShader_projXYZ->SetUniform_DeformParms(
            tess.surfaceShader->deforms, tess.surfaceShader->numDeforms);
    gl_forwardLightingShader_projXYZ->SetUniform_Time(
            backEnd.refdef.floatTime - backEnd.currentEntity->e.shaderTime);
    ...
    // bind u_DiffuseMap
    GL_BindToTMU(0, diffuseStage->bundle[TB_DIFFUSEMAP].image[0]);
    gl_forwardLightingShader_projXYZ->SetUniform_DiffuseTextureMatrix(
            tess.svars.texMatrices[TB_DIFFUSEMAP]);
    ...
    if (normalMapping) {
        ...
        gl_forwardLightingShader_projXYZ->SetUniform_NormalTextureMatrix(
                tess.svars.texMatrices[TB_NORMALMAP]);
        ...
        gl_forwardLightingShader_projXYZ->SetUniform_SpecularTextureMatrix(
                tess.svars.texMatrices[TB_SPECULARMAP]);
        ...
        gl_forwardLightingShader_projXYZ->SetUniform_SpecularExponent(minSpec,
                                                                      maxSpec);
    }
...
static void Render_forwardLighting_DBS_directional(
        shaderStage_t* diffuseStage, shaderStage_t* attenuationXYStage, shaderStage_t* attenuationZStage, trRefLight_t* light) {
    vec3_t viewOrigin;
    ...
    // choose right shader program ----------------------------------
    gl_forwardLightingShader_directionalSun->SetVertexSkinning(
            glConfig2.vboVertexSkinningAvailable && tess.vboVertexSkinning);
    gl_forwardLightingShader_directionalSun->SetVertexAnimation(
            glState.vertexAttribsInterpolation > 0);

    gl_forwardLightingShader_directionalSun->SetNormalMapping(normalMapping);
    gl_forwardLightingShader_directionalSun->SetParallaxMapping(
            normalMapping && r_parallaxMapping->integer &&
            tess.surfaceShader->parallax);
    ...
    gl_forwardLightingShader_directionalSun->SetUniform_ColorModulate(colorGen,
                                                                      alphaGen);

    // u_Color
    gl_forwardLightingShader_directionalSun->SetUniform_Color(tess.svars.color);

    gl_forwardLightingShader_directionalSun->SetUniform_AlphaTest(
            diffuseStage->stateBits);
    ...
    gl_forwardLightingShader_directionalSun->SetUniform_LightRadius(
            light->sphereRadius);
    gl_forwardLightingShader_directionalSun->SetUniform_LightScale(
            light->l.scale);
    gl_forwardLightingShader_directionalSun->SetUniform_LightWrapAround(
            RB_EvalExpression(&diffuseStage->wrapAroundLightingExp, 0));
    gl_forwardLightingShader_directionalSun->SetUniform_LightAttenuationMatrix(
            light->attenuationMatrix2);

    GL_CheckErrors();

    if (shadowCompare) {
        gl_forwardLightingShader_directionalSun->SetUniform_ShadowMatrix(
                light->shadowMatricesBiased);
        gl_forwardLightingShader_directionalSun
                ->SetUniform_ShadowParallelSplitDistances(
                        backEnd.viewParms.parallelSplitDistances);
        gl_forwardLightingShader_directionalSun->SetUniform_ShadowTexelSize(
                shadowTexelSize);
        gl_forwardLightingShader_directionalSun->SetUniform_ShadowBlur(
                r_shadowBlur->value);
    }

    GL_CheckErrors();

    gl_forwardLightingShader_directionalSun->SetUniform_ModelMatrix(
            backEnd.orientation.transformMatrix);
    gl_forwardLightingShader_directionalSun->SetUniform_ViewMatrix(
            backEnd.viewParms.world.viewMatrix);
    gl_forwardLightingShader_directionalSun->SetUniform_ModelViewProjectionMatrix(
            glState.modelViewProjectionMatrix[glState.stackIndex]);

    // u_Bones
    if (glConfig2.vboVertexSkinningAvailable && tess.vboVertexSkinning) {
        gl_forwardLightingShader_directionalSun->SetUniform_Bones(tess.numBones,
                                                                  tess.bones);
    }

    // u_VertexInterpolation
    if (tess.vboVertexAnimation) {
        gl_forwardLightingShader_directionalSun->SetUniform_VertexInterpolation(
                glState.vertexAttribsInterpolation);
    }

    gl_forwardLightingShader_directionalSun->SetUniform_DeformParms(
            tess.surfaceShader->deforms, tess.surfaceShader->numDeforms);
    gl_forwardLightingShader_directionalSun->SetUniform_Time(
            backEnd.refdef.floatTime - backEnd.currentEntity->e.shaderTime);
    ...
    // bind u_DiffuseMap
    GL_BindToTMU(0, diffuseStage->bundle[TB_DIFFUSEMAP].image[0]);
    gl_forwardLightingShader_directionalSun->SetUniform_DiffuseTextureMatrix(
            tess.svars.texMatrices[TB_DIFFUSEMAP]);
    ...
    if (normalMapping) {
        ...
        gl_forwardLightingShader_directionalSun->SetUniform_NormalTextureMatrix(
                tess.svars.texMatrices[TB_NORMALMAP]);
        ...
        gl_forwardLightingShader_directionalSun->SetUniform_SpecularTextureMatrix(
                tess.svars.texMatrices[TB_SPECULARMAP]);
        ...
        gl_forwardLightingShader_directionalSun->SetUniform_SpecularExponent(
                minSpec, maxSpec);
    }
...
static void Render_reflection_CB(int stage) {
    ...
    // choose right shader program ----------------------------------
    gl_reflectionShader->SetVertexSkinning(glConfig2.vboVertexSkinningAvailable &&
                                           tess.vboVertexSkinning);
    gl_reflectionShader->SetVertexAnimation(glState.vertexAttribsInterpolation >
                                            0);
    ...
    gl_reflectionShader->SetUniform_ViewOrigin(
            backEnd.viewParms.orientation.origin); // in world space

    gl_reflectionShader->SetUniform_ModelMatrix(
            backEnd.orientation.transformMatrix);
    gl_reflectionShader->SetUniform_ModelViewProjectionMatrix(
            glState.modelViewProjectionMatrix[glState.stackIndex]);
    ...
    // u_VertexInterpolation
    if (tess.vboVertexAnimation) {
        gl_reflectionShader->SetUniform_VertexInterpolation(
                glState.vertexAttribsInterpolation);
    }
    ...
    if (normalMapping) {
        GL_BindToTMU(1, pStage->bundle[TB_NORMALMAP].image[0]);
        gl_reflectionShader->SetUniform_NormalTextureMatrix(
                tess.svars.texMatrices[TB_NORMALMAP]);
    }
...
static void Render_skybox(int stage) {
    ...
    gl_skyboxShader->SetUniform_ViewOrigin(
            backEnd.viewParms.orientation.origin); // in world space

    gl_skyboxShader->SetUniform_ModelMatrix(backEnd.orientation.transformMatrix);
    gl_skyboxShader->SetUniform_ModelViewProjectionMatrix(
            glState.modelViewProjectionMatrix[glState.stackIndex]);
...
static void Render_screen(int stage) {
    ...
    gl_screenShader->SetUniform_ModelViewProjectionMatrix(
            glState.modelViewProjectionMatrix[glState.stackIndex]);
...
static void Render_portal(int stage) {
    ...
    gl_portalShader->SetUniform_ModelViewMatrix(
            glState.modelViewMatrix[glState.stackIndex]);
    gl_portalShader->SetUniform_ModelViewProjectionMatrix(
            glState.modelViewProjectionMatrix[glState.stackIndex]);
...
static void Render_heatHaze(int stage) {
    ...
    // choose right shader program ----------------------------------
    gl_heatHazeShader->SetVertexSkinning(glConfig2.vboVertexSkinningAvailable &&
                                         tess.vboVertexSkinning);
    ...
    gl_heatHazeShader->SetUniform_ModelViewMatrixTranspose(
            glState.modelViewMatrix[glState.stackIndex]);
    gl_heatHazeShader->SetUniform_ProjectionMatrixTranspose(
            glState.projectionMatrix[glState.stackIndex]);
    gl_heatHazeShader->SetUniform_ModelViewProjectionMatrix(
            glState.modelViewProjectionMatrix[glState.stackIndex]);
    ...
    // u_VertexInterpolation
    if (tess.vboVertexAnimation) {
        gl_heatHazeShader->SetUniform_VertexInterpolation(
                glState.vertexAttribsInterpolation);
    }

    // u_DeformGen
    if (tess.surfaceShader->numDeforms) {
        gl_heatHazeShader->SetUniform_DeformParms(tess.surfaceShader->deforms,
                                                  tess.surfaceShader->numDeforms);
    ...
    // bind u_NormalMap
    GL_BindToTMU(0, pStage->bundle[TB_COLORMAP].image[0]);
    gl_heatHazeShader->SetUniform_NormalTextureMatrix(
            tess.svars.texMatrices[TB_COLORMAP]);
...
static void Render_liquid(int stage) {
    ...
    GL_State(pStage->stateBits &
             ~(GLS_SRCBLEND_BITS | GLS_DSTBLEND_BITS | GLS_DEPTHMASK_TRUE));

    // choose right shader program
    gl_liquidShader->SetParallaxMapping(r_parallaxMapping->integer &&
                                        tess.surfaceShader->parallax);
    ...
    gl_liquidShader->SetUniform_RefractionIndex(
            RB_EvalExpression(&pStage->refractionIndexExp, 1.0));
    gl_liquidShader->SetUniform_FresnelPower(
            RB_EvalExpression(&pStage->fresnelPowerExp, 2.0));
    gl_liquidShader->SetUniform_FresnelScale(
            RB_EvalExpression(&pStage->fresnelScaleExp, 1.0));
    gl_liquidShader->SetUniform_FresnelBias(
            RB_EvalExpression(&pStage->fresnelBiasExp, 0.05));
    gl_liquidShader->SetUniform_NormalScale(
            RB_EvalExpression(&pStage->normalScaleExp, 0.05));
    gl_liquidShader->SetUniform_FogDensity(fogDensity);
    gl_liquidShader->SetUniform_FogColor(fogColor);

    gl_liquidShader->SetUniform_UnprojectMatrix(
            backEnd.viewParms.unprojectionMatrix);
    gl_liquidShader->SetUniform_ModelMatrix(backEnd.orientation.transformMatrix);
    gl_liquidShader->SetUniform_ModelViewProjectionMatrix(
            glState.modelViewProjectionMatrix[glState.stackIndex]);

    float specMin = RB_EvalExpression(&pStage->specularExponentMin,
                                      r_specularExponentMin->value);
    float specMax = RB_EvalExpression(&pStage->specularExponentMax,
                                      r_specularExponentMax->value);
    ...
    gl_liquidShader->SetUniform_NormalTextureMatrix(
            tess.svars.texMatrices[TB_COLORMAP]);
...
static void Render_fog() {
    ...
    if (r_logFile->integer) {
        GLimp_LogComment(
                va("--- Render_fog( fogNum = %i, originalBrushNumber = %i ) ---\n",
                   tess.fogNum,
    ...
    fogDistanceVector[3] =
            DotProduct(local, backEnd.viewParms.orientation.axis[0]);
    ...
    if (fog->hasSurface) {
        ...
        fogDepthVector[3] =
                -fog->surface[3] + DotProduct(backEnd.orientation.origin, fog->surface);
    ...
    gl_fogQuake3Shader->SetVertexSkinning(glConfig2.vboVertexSkinningAvailable &&
                                          tess.vboVertexSkinning);
    gl_fogQuake3Shader->SetVertexAnimation(glState.vertexAttribsInterpolation >
                                           0);
    ...
    gl_fogQuake3Shader->SetUniform_ModelMatrix(
            backEnd.orientation.transformMatrix);
    gl_fogQuake3Shader->SetUniform_ModelViewProjectionMatrix(
            glState.modelViewProjectionMatrix[glState.stackIndex]);
    ...
    // u_VertexInterpolation
    if (tess.vboVertexAnimation) {
        gl_fogQuake3Shader->SetUniform_VertexInterpolation(
                glState.vertexAttribsInterpolation);
    }

    gl_fogQuake3Shader->SetUniform_DeformParms(tess.surfaceShader->deforms,
                                               tess.surfaceShader->numDeforms);
    ...
    switch (pStage->rgbGen) {
        ...
        case CGEN_ENTITY: {
            if (backEnd.currentLight) {
                tess.svars.color[0] =
                        Maths::clampFraction(backEnd.currentLight->l.color[0]);
                tess.svars.color[1] =
                        Maths::clampFraction(backEnd.currentLight->l.color[1]);
                tess.svars.color[2] =
                        Maths::clampFraction(backEnd.currentLight->l.color[2]);
                tess.svars.color[3] = 1.0;
            } else if (backEnd.currentEntity) {
                tess.svars.color[0] = Maths::clampFraction(
                        backEnd.currentEntity->e.shaderRGBA[0] * (1.0 / 255.0));
                tess.svars.color[1] = Maths::clampFraction(
                        backEnd.currentEntity->e.shaderRGBA[1] * (1.0 / 255.0));
                tess.svars.color[2] = Maths::clampFraction(
                        backEnd.currentEntity->e.shaderRGBA[2] * (1.0 / 255.0));
                tess.svars.color[3] = Maths::clampFraction(
        ...
        case CGEN_ONE_MINUS_ENTITY: {
            if (backEnd.currentLight) {
                tess.svars.color[0] =
                        1.0 - Maths::clampFraction(backEnd.currentLight->l.color[0]);
                tess.svars.color[1] =
                        1.0 - Maths::clampFraction(backEnd.currentLight->l.color[1]);
                tess.svars.color[2] =
                        1.0 - Maths::clampFraction(backEnd.currentLight->l.color[2]);
                tess.svars.color[3] = 0.0; // FIXME
            } else if (backEnd.currentEntity) {
                tess.svars.color[0] =
                        1.0 - Maths::clampFraction(backEnd.currentEntity->e.shaderRGBA[0] *
                                                   (1.0 / 255.0));
                tess.svars.color[1] =
                        1.0 - Maths::clampFraction(backEnd.currentEntity->e.shaderRGBA[1] *
                                                   (1.0 / 255.0));
                tess.svars.color[2] =
                        1.0 - Maths::clampFraction(backEnd.currentEntity->e.shaderRGBA[2] *
                                                   (1.0 / 255.0));
                tess.svars.color[3] =
                        1.0 - Maths::clampFraction(backEnd.currentEntity->e.shaderRGBA[3] *
                                                   (1.0 / 255.0));
            } else {
                tess.svars.color[0] = 0.0;
                tess.svars.color[1] = 0.0;
        ...
        case CGEN_WAVEFORM: {
            ...
            if (wf->func == GF_NOISE) {
                glow = wf->base +
                       R_NoiseGet4f(0, 0, 0, (backEnd.refdef.floatTime + wf->phase) * wf->frequency) *
                               wf->amplitude;
            } else {
        ...
        case CGEN_CUSTOM_RGBs: {
            if (backEnd.currentLight) {
                red = Maths::clampFraction(
                        RB_EvalExpression(&pStage->redExp, backEnd.currentLight->l.color[0]));
                green = Maths::clampFraction(RB_EvalExpression(
                        &pStage->greenExp, backEnd.currentLight->l.color[1]));
                blue = Maths::clampFraction(RB_EvalExpression(
                        &pStage->blueExp, backEnd.currentLight->l.color[2]));
            } else if (backEnd.currentEntity) {
                red = Maths::clampFraction(RB_EvalExpression(
                        &pStage->redExp,
                        backEnd.currentEntity->e.shaderRGBA[0] * (1.0 / 255.0)));
                green = Maths::clampFraction(RB_EvalExpression(
                        &pStage->greenExp,
...
void Tess_StageIteratorDebug() {
    ...
    if (r_logFile->integer) {
        ...
        // a call to va() every frame!
        GLimp_LogComment(
                va("--- Tess_StageIteratorDebug( %i vertices, %i triangles ) ---\n",
                   tess.numVertexes,
                   tess.numIndexes / 3));
    }
    ...
    if (!glState.currentVBO || !glState.currentIBO ||
        glState.currentVBO == tess.vbo || glState.currentIBO == tess.ibo) {
        Tess_UpdateVBOs();
            ...
            case ST_COLLAPSE_lighting_DBS: {
                {
                    if (r_precomputedLighting->integer || r_vertexLighting->integer) {
                        if (!r_vertexLighting->integer && tess.lightmapNum >= 0 &&
                            tess.lightmapNum < tr.lightmaps.currentElements) {
...
void Tess_StageIteratorDepthFill() {
    ...
    if (r_logFile->integer) {
        ...
        // a call to va() every frame!
        GLimp_LogComment(
                va("--- Tess_StageIteratorDepthFill( %s, %i vertices, %i triangles ) "
                   "---\n",
                   tess.surfaceShader->name,
                   tess.numVertexes,
                   tess.numIndexes / 3));
...
void Tess_StageIteratorShadowFill() {
    ...
    if (r_logFile->integer) {
        ...
        // a call to va() every frame!
        GLimp_LogComment(
                va("--- Tess_StageIteratorShadowFill( %s, %i vertices, %i triangles ) "
                   "---\n",
                   tess.surfaceShader->name,
                   tess.numVertexes,
                   tess.numIndexes / 3));
...
void Tess_StageIteratorLighting() {
    ...
    if (r_logFile->integer) {
        ...
        // a call to va() every frame!
        GLimp_LogComment(
                va("--- Tess_StageIteratorLighting( %s, %s, %i vertices, %i triangles "
                   ") ---\n",
                   tess.surfaceShader->name,
                   tess.lightShader->name,
                   tess.numVertexes,
                   tess.numIndexes / 3));
    }
                    ...
                    if (light->l.rlType == RL_OMNI) {
                    ...
                    } else if (light->l.rlType == RL_DIRECTIONAL) {
                        {
                            Render_forwardLighting_DBS_directional(
                                    diffuseStage, attenuationXYStage, attenuationZStage, light);
                        }
                    }
...
void Tess_End() {
    ...
    if ((tess.stageIteratorFunc != Tess_StageIteratorShadowFill) &&
        ...
        if (r_showTris->integer || r_showBatches->integer ||
            (r_showLightBatches->integer &&
             (tess.stageIteratorFunc == Tess_StageIteratorLighting))) {
            DrawTris();
        }
    }
