...
static bool IQM_CheckRange(iqmHeader_t* header, int offset, int count, int size, const char* mod_name, const char* section) {
    int section_end;
    ...
    if (count <= 0) {
        ri.Printf(PRINT_WARNING,
                  "R_LoadIQM: %s has a negative count in the %s section (%d).\n",
                  mod_name,
                  section,
                  count);
        return true;
    ...
    if (offset < 0) {
        ri.Printf(PRINT_WARNING,
                  "R_LoadIQM: %s has a negative offset to the %s section (%d).\n",
                  mod_name,
                  section,
                  offset);
        return true;
    ...
    if (offset > header->filesize) {
        ri.Printf(PRINT_WARNING,
                  "R_LoadIQM: %s has an offset behind the end of file to the %s "
                  "section (%d).\n",
                  mod_name,
                  section,
                  offset);
    ...
    if (section_end > header->filesize || section_end < 0) {
        ri.Printf(
                PRINT_WARNING,
                "R_LoadIQM: %s has the section %s exceeding the end of file (%d).\n",
                mod_name,
                section,
                section_end);
        return true;
...
static bool LoadIQMFile(void* buffer, int filesize, const char* mod_name, size_t* len_names) {
    iqmHeader_t* header;
    ...
    if (Q_strncmp(header->magic, IQM_MAGIC, sizeof(header->magic))) {
        ri.Printf(PRINT_WARNING,
                  "R_LoadIQModel: file %s doesn't contain an IQM header.\n",
                  mod_name);
        return false;
    }
    ...
    if (header->version != IQM_VERSION) {
        ri.Printf(PRINT_WARNING,
                  "R_LoadIQM: %s is a unsupported IQM version (%d), only version "
                  "%d is supported.\n",
                  mod_name,
                  header->version,
                  IQM_VERSION);
    ...
    if (IQM_CheckRange(header, header->ofs_vertexarrays, header->num_vertexarrays, sizeof(iqmVertexArray_t), mod_name, "vertexarray")) {
        return false;
    }
    ...
    for (i = 0; i < header->num_vertexarrays; i++, vertexarray++) {
        ...
        if (vertexarray->size <= 0 || vertexarray->size > 4) {
            ri.Printf(PRINT_WARNING,
                      "R_LoadIQM: %s contains an invalid vertexarray size.\n",
                      mod_name);
            return false;
        }
        ...
        switch (vertexarray->format) {
            ...
            case IQM_UBYTE:
                ...
                if (IQM_CheckRange(header, vertexarray->offset, n, sizeof(byte), mod_name, "vertexarray")) {
                    return false;
                }
                break;
            case IQM_INT:
            ...
            case IQM_FLOAT:
                // 4-byte swap
                if (IQM_CheckRange(header, vertexarray->offset, n, sizeof(float), mod_name, "vertexarray")) {
                    return false;
                }
            ...
            default:
                // not supported
                ri.Printf(PRINT_WARNING,
                          "R_LoadIQM: file %s uses an unsupported vertex format.\n",
                          mod_name);
                return false;
                break;
        }
    ...
    if (IQM_CheckRange(header, header->ofs_triangles, header->num_triangles, sizeof(iqmTriangle_t), mod_name, "triangle")) {
        return false;
    }
    ...
    *len_names = 0;
    ...
    if (IQM_CheckRange(header, header->ofs_meshes, header->num_meshes, sizeof(iqmMesh_t), mod_name, "mesh")) {
        return false;
    }
    ...
    for (i = 0; i < header->num_meshes; i++, mesh++) {
        ...
        *len_names +=
                strlen((char*) IQMPtr(header, header->ofs_text + mesh->name)) + 1;
    }

    // check and swap joints
    if (header->num_joints != 0 &&
        IQM_CheckRange(header, header->ofs_joints, header->num_joints, sizeof(iqmJoint_t), mod_name, "joint")) {
        return false;
    }
    ...
    for (i = 0; i < header->num_joints; i++, joint++) {
        ...
        if (joint->parent < -1 || joint->parent >= (int) header->num_joints ||
            joint->name >= header->num_text) {
            ri.Printf(PRINT_WARNING,
                      "R_LoadIQM: file %s contains an invalid joint.\n",
                      mod_name);
            return false;
        }
        if (joint->scale[0] < 0.0f || (int) (joint->scale[0] - joint->scale[1]) ||
            (int) (joint->scale[1] - joint->scale[2])) {
            ri.Printf(PRINT_WARNING,
                      "R_LoadIQM: file %s contains an invalid scale.\n%f %f %f",
                      mod_name,
                      joint->scale[0],
                      joint->scale[1],
                      joint->scale[2]);
        ...
        *len_names +=
                strlen((char*) IQMPtr(header, header->ofs_text + joint->name)) + 1;
    }

    // check and swap poses
    if (header->ofs_poses > 0) {
        if (IQM_CheckRange(header, header->ofs_poses, header->num_poses, sizeof(iqmPose_t), mod_name, "pose")) {
            return false;
        }
    ...
    if (header->ofs_bounds) {
        ...
        if (IQM_CheckRange(header, header->ofs_bounds, header->num_frames, sizeof(*bounds), mod_name, "bounds")) {
            return false;
        }
    ...
    if (header->ofs_anims) {
        if (IQM_CheckRange(header, header->ofs_anims, header->num_anims, sizeof(iqmAnim_t), mod_name, "animation")) {
            return false;
        }
        ...
        for (i = 0; i < header->num_anims; i++, anim++) {
            ...
            *len_names +=
                    strlen((char*) IQMPtr(header, header->ofs_text + anim->name)) + 1;
        }
    }
...
=================
*/
void BuildTangents(int n, float* input, float* normals, float* tangents, float* bitangents) {
    int i;
    vec3_t crossProd;
...
=================
*/
bool R_LoadIQModel(model_t* mod, void* buffer, int filesize, const char* mod_name) {
    iqmHeader_t* header;
    iqmVertexArray_t* vertexarray;
    iqmTriangle_t* triangle;
    iqmMesh_t* mesh;
    ...
    for (i = 0; i < header->num_joints; i++, joint++, trans++) {
        if (joint->parent >= i) {
            ri.Printf(
                    PRINT_WARNING,
                    "R_LoadIQModel: file %s contains an invalid parent joint number.\n",
                    mod_name);
            return false;
        }
        ...
        if (joint->parent >= 0) {
            TransCombine(trans, &IQModel->joints[joint->parent], trans);
        }
    ...
    for (i = 0; i < header->num_frames; i++) {
        ...
        for (j = 0; j < header->num_poses; j++, pose++, trans++) {
            ...
            if (scale[0] < 0.0f || (int) (scale[0] - scale[1]) ||
                (int) (scale[1] - scale[2])) {
                ri.Printf(PRINT_WARNING,
                          "R_LoadIQM: file %s contains an invalid scale.",
                          mod_name);
    ...
    for (i = 0; i < header->num_vertexarrays; i++, vertexarray++) {
        ...
        switch (vertexarray->type) {
            case IQM_POSITION:
                ...
                Com_Memcpy(IQModel->positions, IQMPtr(header, vertexarray->offset), n * sizeof(float));
                for (j = 0; j < n; j += vertexarray->size) {
                    AddPointToBounds(&IQModel->positions[j], IQModel->bounds[0], IQModel->bounds[1]);
                }
                IQModel->internalScale =
                        BoundsMaxExtent(IQModel->bounds[0], IQModel->bounds[1]);
                ...
                break;
            case IQM_NORMAL:
                Com_Memcpy(IQModel->normals, IQMPtr(header, vertexarray->offset), n * sizeof(float));
                break;
            case IQM_TANGENT:
            ...
            case IQM_TEXCOORD:
                for (j = 0; j < n; j++) {
                    IQModel->texcoords[j] =
                            floatToHalf(((float*) IQMPtr(header, vertexarray->offset))[j]);
                }
                break;
            case IQM_BLENDINDEXES:
                Com_Memcpy(IQModel->blendIndexes, IQMPtr(header, vertexarray->offset), n * sizeof(byte));
                break;
            case IQM_BLENDWEIGHTS:
                weights = (u8vec4_t*) IQMPtr(header, vertexarray->offset);
                for (j = 0; j < header->num_vertexes; j++) {
                    IQModel->blendWeights[4 * j + 0] =
                            255 - weights[j][1] - weights[j][2] - weights[j][3];
                ...
                break;
            case IQM_COLOR:
                Com_Memcpy(IQModel->colors, IQMPtr(header, vertexarray->offset), n * sizeof(byte));
                break;
        }
    }
    ...
    if (r_vboModels->integer && glConfig2.vboVertexSkinningAvailable &&
        ...
        if (IQModel->blendIndexes) {
            indexbuf = (int*) ri.Hunk_AllocateTempMemory(sizeof(int[4]) *
                                                         IQModel->num_vertexes);
        ...
        if (IQModel->blendWeights) {
            ...
            weightbuf = (float*) ri.Hunk_AllocateTempMemory(sizeof(vec4_t) *
                                                            IQModel->num_vertexes);
            for (i = 0; i < IQModel->num_vertexes; i++) {
        ...
        qtangentbuf = (i16vec4_t*) ri.Hunk_AllocateTempMemory(
                sizeof(i16vec4_t) * IQModel->num_vertexes);

        for (i = 0; i < IQModel->num_vertexes; i++) {
            R_TBNtoQtangents(&IQModel->tangents[3 * i], &IQModel->bitangents[3 * i], &IQModel->normals[3 * i], qtangentbuf[i]);
        }
        ...
        vbo = R_CreateStaticVBO("IQM surface VBO", vboData, VBO_LAYOUT_SKELETAL);
    ...
    for (i = 0; i < header->num_meshes; i++, mesh++, surface++) {
        ...
        surface->shader =
                R_FindShader((char*) IQMPtr(header, header->ofs_text + mesh->material),
                             SHADER_3D_DYNAMIC,
                             RSF_DEFAULT);
...
static int R_CullIQM(trRefEntity_t* ent) {
    ...
    if (anim && (bounds = anim->bounds)) {
        ...
        BoundsAdd(localBounds[0], localBounds[1], bounds, bounds + 3);
    }

    // merge bounding box provided by skeleton
    BoundsAdd(localBounds[0], localBounds[1], ent->e.skeleton.bounds[0], ent->e.skeleton.bounds[1]);
...
int R_ComputeIQMFogNum(trRefEntity_t* ent) {
    ...
    if (ent->e.skeleton.type == SK_INVALID) {
        // no properly set skeleton so use the bounding box by the model instead by
        // the animations
        IQModel_t* model = tr.currentModel->iqm;
        IQAnim_t* anim = model->anims;
        float* bounds;
...
void R_AddIQMSurfaces(trRefEntity_t* ent) {
    ...
    for (i = 0; i < IQModel->num_surfaces; i++) {
        if (ent->e.customShader)
            ...
            if (shader == tr.defaultShader && i >= 0 && i < skin->numSurfaces &&
                skin->surfaces[i]) {
            ...
            if (ent->e.altShaderIndex > 0 && ent->e.altShaderIndex < MAX_ALTSHADERS &&
                ...
                shader =
                        R_GetShaderByHandle(shader->altShader[ent->e.altShaderIndex].index);
            }
        }
