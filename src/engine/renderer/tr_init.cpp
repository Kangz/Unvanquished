...
static byte* RB_ReadPixels(int x, int y, int width, int height, size_t offset) {
    ...
    buffer = (byte*) ri.Hunk_AllocateTempMemory(offset + paddedLineLen * height +
                                                packAlign - 1);
...
void R_TakeScreenshot(const char* name, ssFormat_t format) {
    static char fileName[MAX_OSPATH]; // bad things may happen if two screenshots
                                      // per frame are taken.
    ...
    if (ri.Cmd_Argc() == 2) {
        ...
        for (lastNumber = 0; lastNumber <= 999; lastNumber++) {
            Com_sprintf(fileName, sizeof(fileName), "screenshots/" PRODUCT_NAME_LOWER "_%04d-%02d-%02d_%02d%02d%02d_%03d.%s", 1900 + t.tm_year, t.tm_mon + 1, t.tm_mday, t.tm_hour, t.tm_min, t.tm_sec, lastNumber, name);
...
const void* RB_TakeVideoFrameCmd(const void* data) {
    ...
    // RB: it is possible to we still have a videoFrameCommand_t but we already
    // stopped
    // video recording
    if (ri.CL_VideoRecording()) {
        ...
        if (cmd->motionJpeg) {
            ...
            outputSize =
                    SaveJPGToBuffer(cmd->encodeBuffer, 3 * cmd->width * cmd->height, 90, cmd->width, cmd->height, cmd->captureBuffer);
            ...
            for (i = 0; i < cmd->height; ++i) {
                for (j = 0; j < lineLen; j += 3) {
                    cmd->encodeBuffer[i * aviLineLen + j + 0] =
                            pixels[i * captureLineLen + j + 2];
                    cmd->encodeBuffer[i * aviLineLen + j + 1] =
                            pixels[i * captureLineLen + j + 1];
                    cmd->encodeBuffer[i * aviLineLen + j + 2] =
                            pixels[i * captureLineLen + j + 0];
                }
...
*/
void GfxInfo_f() {
    static const char fsstrings[][16] = {"windowed", "fullscreen"};
    ...
    if (glConfig.driverType == GLDRV_OPENGL3) {
        ...
        if (profile == GL_CONTEXT_CORE_PROFILE_BIT) {
            ...
            ri.Printf(PRINT_DEVELOPER,
                      S_COLOR_RED "Having a compatibility profile\n");
        }
        ...
        if (contextFlags & GL_CONTEXT_FLAG_FORWARD_COMPATIBLE_BIT) {
            ri.Printf(PRINT_DEVELOPER,
                      S_COLOR_GREEN "Context is forward compatible\n");
        } else {
            ri.Printf(PRINT_DEVELOPER,
                      S_COLOR_RED "Context is NOT forward compatible\n");
        }
    }
    ...
    if (glConfig2.vboVertexSkinningAvailable) {
        ri.Printf(PRINT_ALL,
                  "Using GPU vertex skinning with max %i bones in a single pass\n",
...
void R_Register() {
    ...
    r_ext_compressed_textures =
            ri.Cvar_Get("r_ext_compressed_textures", "0", CVAR_LATCH | CVAR_ARCHIVE);
    r_ext_occlusion_query =
            ri.Cvar_Get("r_ext_occlusion_query", "1", CVAR_CHEAT | CVAR_LATCH);
    r_ext_texture_non_power_of_two = ri.Cvar_Get("r_ext_texture_non_power_of_two",
                                                 "1",
                                                 CVAR_CHEAT | CVAR_LATCH);
    r_ext_draw_buffers =
            ri.Cvar_Get("r_ext_draw_buffers", "1", CVAR_CHEAT | CVAR_LATCH);
    r_ext_vertex_array_object =
            ri.Cvar_Get("r_ext_vertex_array_object", "1", CVAR_CHEAT | CVAR_LATCH);
    r_ext_half_float_pixel =
            ri.Cvar_Get("r_ext_half_float_pixel", "1", CVAR_CHEAT | CVAR_LATCH);
    r_ext_texture_float =
            ri.Cvar_Get("r_ext_texture_float", "1", CVAR_CHEAT | CVAR_LATCH);
    r_ext_texture_rg =
            ri.Cvar_Get("r_ext_texture_rg", "1", CVAR_CHEAT | CVAR_LATCH);
    r_ext_texture_filter_anisotropic = ri.Cvar_Get(
            "r_ext_texture_filter_anisotropic", "4", CVAR_LATCH | CVAR_ARCHIVE);
    r_ext_framebuffer_object =
            ri.Cvar_Get("r_ext_framebuffer_object", "1", CVAR_LATCH);
    r_ext_packed_depth_stencil =
            ri.Cvar_Get("r_ext_packed_depth_stencil", "1", CVAR_CHEAT | CVAR_LATCH);
    r_ext_framebuffer_blit =
            ri.Cvar_Get("r_ext_framebuffer_blit", "1", CVAR_CHEAT | CVAR_LATCH);
    r_extx_framebuffer_mixed_formats =
            ri.Cvar_Get("r_extx_framebuffer_mixed_formats", "1", CVAR_LATCH);
    r_ext_generate_mipmap =
            ri.Cvar_Get("r_ext_generate_mipmap", "1", CVAR_CHEAT | CVAR_LATCH);
    r_arb_buffer_storage =
            ri.Cvar_Get("r_arb_buffer_storage", "1", CVAR_CHEAT | CVAR_LATCH);
    r_arb_map_buffer_range =
            ri.Cvar_Get("r_arb_map_buffer_range", "1", CVAR_CHEAT | CVAR_LATCH);
    r_arb_sync = ri.Cvar_Get("r_arb_sync", "1", CVAR_CHEAT | CVAR_LATCH);

    r_collapseStages =
            ri.Cvar_Get("r_collapseStages", "1", CVAR_LATCH | CVAR_CHEAT);
    ...
    r_ext_multisample =
            ri.Cvar_Get("r_ext_multisample", "0", CVAR_LATCH | CVAR_ARCHIVE);
    r_mode = ri.Cvar_Get("r_mode", "-2", CVAR_LATCH | CVAR_SHADER | CVAR_ARCHIVE);
    r_fullscreen = ri.Cvar_Get("r_fullscreen", "1", CVAR_ARCHIVE);
    r_customwidth =
            ri.Cvar_Get("r_customwidth", "1600", CVAR_LATCH | CVAR_ARCHIVE);
    r_customheight =
            ri.Cvar_Get("r_customheight", "1024", CVAR_LATCH | CVAR_ARCHIVE);
    ...
    r_precomputedLighting =
            ri.Cvar_Get("r_precomputedLighting", "1", CVAR_SHADER);
    r_vertexLighting =
            ri.Cvar_Get("r_vertexLighting", "0", CVAR_LATCH | CVAR_ARCHIVE);
    r_compressDiffuseMaps = ri.Cvar_Get("r_compressDiffuseMaps", "1", CVAR_LATCH);
    r_compressSpecularMaps =
            ri.Cvar_Get("r_compressSpecularMaps", "1", CVAR_LATCH);
    ...
    r_highQualityNormalMapping =
            ri.Cvar_Get("r_highQualityNormalMapping", "0", CVAR_LATCH);
    ...
    r_debugShadowMaps =
            ri.Cvar_Get("r_debugShadowMaps", "0", CVAR_CHEAT | CVAR_SHADER);
    r_shadowMapLinearFilter =
            ri.Cvar_Get("r_shadowMapLinearFilter", "1", CVAR_CHEAT | CVAR_LATCH);
    r_lightBleedReduction =
            ri.Cvar_Get("r_lightBleedReduction", "0", CVAR_CHEAT | CVAR_SHADER);
    r_overDarkeningFactor =
            ri.Cvar_Get("r_overDarkeningFactor", "30.0", CVAR_CHEAT | CVAR_SHADER);
    r_shadowMapDepthScale =
            ri.Cvar_Get("r_shadowMapDepthScale", "1.41", CVAR_CHEAT | CVAR_SHADER);

    r_parallelShadowSplitWeight =
            ri.Cvar_Get("r_parallelShadowSplitWeight", "0.9", CVAR_CHEAT);
    r_parallelShadowSplits =
            ri.Cvar_Get("r_parallelShadowSplits", "2", CVAR_CHEAT | CVAR_SHADER);
    ...
    r_textureMode =
            ri.Cvar_Get("r_textureMode", "GL_LINEAR_MIPMAP_LINEAR", CVAR_ARCHIVE);
    ...
    r_ambientScale =
            ri.Cvar_Get("r_ambientScale", "1.0", CVAR_CHEAT | CVAR_SHADER);
    ...
    r_specularExponentMax =
            ri.Cvar_Get("r_specularExponentMax", "16", CVAR_CHEAT);
    r_specularScale =
            ri.Cvar_Get("r_specularScale", "1.0", CVAR_CHEAT | CVAR_SHADER);
    r_normalScale = ri.Cvar_Get("r_normalScale", "1.0", CVAR_SHADER);
    r_normalMapping = ri.Cvar_Get("r_normalMapping", "1", CVAR_ARCHIVE);
    r_parallaxDepthScale =
            ri.Cvar_Get("r_parallaxDepthScale", "0.03", CVAR_CHEAT);

    r_wrapAroundLighting =
            ri.Cvar_Get("r_wrapAroundLighting", "0.7", CVAR_CHEAT | CVAR_SHADER);
    r_halfLambertLighting =
            ri.Cvar_Get("r_halfLambertLighting", "1", CVAR_CHEAT | CVAR_SHADER);
    ...
    r_shadowMapSizeUltra =
            ri.Cvar_Get("r_shadowMapSizeUltra", "1024", CVAR_LATCH);
    AssertCvarRange(r_shadowMapSizeUltra, 32, 2048, true);

    r_shadowMapSizeVeryHigh =
            ri.Cvar_Get("r_shadowMapSizeVeryHigh", "512", CVAR_LATCH);
    ...
    r_shadowMapSizeMedium =
            ri.Cvar_Get("r_shadowMapSizeMedium", "128", CVAR_LATCH);
    ...
    r_shadowMapSizeSunUltra =
            ri.Cvar_Get("r_shadowMapSizeSunUltra", "1024", CVAR_LATCH);
    AssertCvarRange(r_shadowMapSizeSunUltra, 32, 2048, true);

    r_shadowMapSizeSunVeryHigh =
            ri.Cvar_Get("r_shadowMapSizeSunVeryHigh", "1024", CVAR_LATCH);
    AssertCvarRange(r_shadowMapSizeSunVeryHigh, 512, 2048, true);

    r_shadowMapSizeSunHigh =
            ri.Cvar_Get("r_shadowMapSizeSunHigh", "1024", CVAR_LATCH);
    AssertCvarRange(r_shadowMapSizeSunHigh, 512, 2048, true);

    r_shadowMapSizeSunMedium =
            ri.Cvar_Get("r_shadowMapSizeSunMedium", "1024", CVAR_LATCH);
    AssertCvarRange(r_shadowMapSizeSunMedium, 512, 2048, true);

    r_shadowMapSizeSunLow =
            ri.Cvar_Get("r_shadowMapSizeSunLow", "1024", CVAR_LATCH);
    ...
    r_cullShadowPyramidFaces =
            ri.Cvar_Get("r_cullShadowPyramidFaces", "0", CVAR_CHEAT);
    r_cullShadowPyramidCurves =
            ri.Cvar_Get("r_cullShadowPyramidCurves", "1", CVAR_CHEAT);
    r_cullShadowPyramidTriangles =
            ri.Cvar_Get("r_cullShadowPyramidTriangles", "1", CVAR_CHEAT);
    ...
    r_maxPolyVerts =
            ri.Cvar_Get("r_maxpolyverts", "100000", 0); // 3000 in vanilla Q3A
    ...
    r_showEntityTransforms =
            ri.Cvar_Get("r_showEntityTransforms", "0", CVAR_CHEAT);
    r_showLightTransforms = ri.Cvar_Get("r_showLightTransforms", "0", CVAR_CHEAT);
    r_showLightInteractions =
            ri.Cvar_Get("r_showLightInteractions", "0", CVAR_CHEAT);
    ...
    r_showLightMaps =
            ri.Cvar_Get("r_showLightMaps", "0", CVAR_CHEAT | CVAR_SHADER);
    r_showDeluxeMaps =
            ri.Cvar_Get("r_showDeluxeMaps", "0", CVAR_CHEAT | CVAR_SHADER);
    r_showEntityNormals =
            ri.Cvar_Get("r_showEntityNormals", "0", CVAR_CHEAT | CVAR_SHADER);
    ...
    r_showParallelShadowSplits =
            ri.Cvar_Get("r_showParallelShadowSplits", "0", CVAR_CHEAT | CVAR_SHADER);
...
bool R_Init() {
    ...
    backEndData[0] =
            (backEndData_t*) ri.Hunk_Alloc(sizeof(*backEndData[0]), h_low);
    backEndData[0]->polys = (srfPoly_t*) ri.Hunk_Alloc(
            r_maxPolys->integer * sizeof(srfPoly_t), h_low);
    backEndData[0]->polyVerts = (polyVert_t*) ri.Hunk_Alloc(
            r_maxPolyVerts->integer * sizeof(polyVert_t), h_low);
    backEndData[0]->polyIndexes =
            (int*) ri.Hunk_Alloc(r_maxPolyVerts->integer * sizeof(int), h_low);
    backEndData[0]->polybuffers = (srfPolyBuffer_t*) ri.Hunk_Alloc(
            r_maxPolys->integer * sizeof(srfPolyBuffer_t), h_low);

    if (r_smp->integer) {
        backEndData[1] =
                (backEndData_t*) ri.Hunk_Alloc(sizeof(*backEndData[1]), h_low);
        backEndData[1]->polys = (srfPoly_t*) ri.Hunk_Alloc(
                r_maxPolys->integer * sizeof(srfPoly_t), h_low);
        backEndData[1]->polyVerts = (polyVert_t*) ri.Hunk_Alloc(
                r_maxPolyVerts->integer * sizeof(polyVert_t), h_low);
        backEndData[1]->polyIndexes =
                (int*) ri.Hunk_Alloc(r_maxPolyVerts->integer * sizeof(int), h_low);
        backEndData[1]->polybuffers = (srfPolyBuffer_t*) ri.Hunk_Alloc(
                r_maxPolys->integer * sizeof(srfPolyBuffer_t), h_low);
    } else {
