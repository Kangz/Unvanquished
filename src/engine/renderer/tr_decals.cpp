...
static bool MakeTextureMatrix(vec4_t texMat[2], vec4_t projection, decalVert_t* a, decalVert_t* b, decalVert_t* c) {
    ...
    bb = (b->st[0] - a->st[0]) * (c->st[1] - a->st[1]) -
         (c->st[0] - a->st[0]) * (b->st[1] - a->st[1]);
    ...
    t = 0.0f;
    bary[0] =
            ((b->st[0] - s) * (c->st[1] - t) - (c->st[0] - s) * (b->st[1] - t)) / bb;
    bary[1] =
            ((c->st[0] - s) * (a->st[1] - t) - (a->st[0] - s) * (c->st[1] - t)) / bb;
    bary[2] =
            ((a->st[0] - s) * (b->st[1] - t) - (b->st[0] - s) * (a->st[1] - t)) / bb;
...
if a single point is passed in (numPoints == 1) then the decal will be
omnidirectional
...
void RE_ProjectDecal(qhandle_t hShader, int numPoints, vec3_t* points, vec4_t projection, vec4_t color, int lifeTime, int fadeTime) {
    ...
    /* set up decal texcoords (fixme: support arbitrary projector st coordinates
   * in trapcall) */
    dv[0].st[0] = 0.0f;
    dv[0].st[1] = 0.0f;
    dv[1].st[0] = 0.0f;
...
bool R_TestDecalBoundingBox(decalProjector_t* dp, vec3_t mins, vec3_t maxs) {
    if (mins[0] >= (dp->center[0] + dp->radius) ||
        maxs[0] <= (dp->center[0] - dp->radius) ||
        mins[1] >= (dp->center[1] + dp->radius) ||
        maxs[1] <= (dp->center[1] - dp->radius) ||
        mins[2] >= (dp->center[2] + dp->radius) ||
        maxs[2] <= (dp->center[2] - dp->radius)) {
        return false;
    }
...
static void ChopWindingBehindPlane(int numInPoints,
                                   vec3_t inPoints[MAX_DECAL_VERTS],
                                   int* numOutPoints,
                                   vec3_t outPoints[MAX_DECAL_VERTS],
                                   vec4_t plane,
                                   vec_t epsilon) {
    int i, j;
    float dot;
...
static void ProjectDecalOntoWinding(decalProjector_t* dp, int numPoints, vec3_t points[2][MAX_DECAL_VERTS], bspSurface_t* surf, bspModel_t* bmodel) {
    ...
    for (i = 0; i < numPoints; i++, vert++) {
        ...
        /* set st */
        vert->st[0] =
                DotProduct(vert->xyz, dp->texMat[axis][0]) + dp->texMat[axis][0][3];
        vert->st[1] =
                DotProduct(vert->xyz, dp->texMat[axis][1]) + dp->texMat[axis][1][3];
...
projects a decal onto a triangle surface (brush faces, misc_models,
metasurfaces)
*/
...
void R_ProjectDecalOntoSurface(decalProjector_t* dp, bspSurface_t* surf, bspModel_t* bmodel) {
    ...
    if (gen->surfaceType != SF_FACE && gen->surfaceType != SF_TRIANGLES &&
...
void R_AddDecalSurface(decal_t* decal) {
    ...
    /* early outs */
    if (decal->shader == nullptr ||
        decal->parent->viewCount != tr.viewCountNoReset ||
        tr.refdef.numDecals >= MAX_DECALS) {
        return;
    }
    ...
    /* fade colors */
    if (decal->fadeStartTime < tr.refdef.time &&
        decal->fadeStartTime < decal->fadeEndTime) {
...
void R_CullDecalProjectors() {
    ...
    for (i = 0, dp = tr.refdef.decalProjectors; i < tr.refdef.numDecalProjectors;
         i++, dp++) {
