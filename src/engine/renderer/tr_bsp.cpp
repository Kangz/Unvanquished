...
static void R_ColorShiftLightingBytesCompressed(byte in[8], byte out[8]) {
    ...
    rgb565 =
            ((rgba[0] >> 3) << 11) | ((rgba[1] >> 2) << 5) | ((rgba[2] >> 3) << 0);
    out[0] = rgb565 & 0xff;
    out[1] = rgb565 >> 8;
                ...
                if (!Q_stricmp(token, "32")) {
                    ...
                    if (!Q_stricmp(token, "-")) {
                        ...
                        if (!Q_stricmp(token, "bit_rle_rgbe")) {
                            ...
                            ri.Printf(
                                    PRINT_ALL,
                                    "LoadRGBE: Expected 'bit_rle_rgbe' found instead '%s'\n",
                                    token);
                        }
                    } else {
    ...
    *pic = (float*) Com_Allocate(w * h * 3 * sizeof(float));
    ...
    for (i = 0; i < (w * h); i++) {
        for (j = 0; j < 3; j++) {
            ...
            *floatbuf++ =
                    sample.f / 255.0f; // FIXME XMap2's output is 255 times too high
        }
    }
...
static void R_LoadLightmaps(lump_t* l, const char* bspName) {
    ...
    if (!len) {
        ...
        if (tr.worldHDR_RGBE) {
            ...
            for (i = 0; i < numLightmaps; i++) {
                ri.Printf(PRINT_DEVELOPER,
                          "...loading external lightmap as RGB8 LDR '%s/%s'\n",
                          mapName,
                          lightmapFiles[i]);
                ...
                image = R_CreateImage(va("%s/%s", mapName, lightmapFiles[i]),
                                      (const byte**) &ldrImage,
                                      width,
                                      height,
                                      1,
                                      IF_NOPICMIP | IF_LIGHTMAP | IF_NOCOMPRESSION,
                                      FT_DEFAULT,
                                      WT_CLAMP);
            ...
            if (tr.worldDeluxeMapping) {
                ...
                for (i = 0; i < numLightmaps; i++) {
                    ...
                    image = R_FindImageFile(va("%s/%s", mapName, lightmapFiles[i]),
                                            IF_NORMALMAP | IF_NOCOMPRESSION,
                                            FT_DEFAULT,
                                            WT_CLAMP,
                                            nullptr);
        ...
        if (numLightmaps == 1) {
            // FIXME: HACK: maps with only one lightmap turn up fullbright for some
            // reason.
        ...
        fatbuffer = (byte*) ri.Hunk_AllocateTempMemory(
                sizeof(byte) * tr.fatLightmapSize * tr.fatLightmapSize * 4);
        ...
        for (i = 0; i < numLightmaps; i++) {
            ...
            for (y = 0; y < LIGHTMAP_SIZE; y++) {
                for (x = 0; x < LIGHTMAP_SIZE; x++) {
                    int index = (x + (y * tr.fatLightmapSize)) +
                                ((xoff * LIGHTMAP_SIZE) +
                                 (yoff * tr.fatLightmapSize * LIGHTMAP_SIZE));
                    fatbuffer[(index * 4) + 0] =
                            buf_p[((x + (y * LIGHTMAP_SIZE)) * 3) + 0];
                    fatbuffer[(index * 4) + 1] =
                            buf_p[((x + (y * LIGHTMAP_SIZE)) * 3) + 1];
                    fatbuffer[(index * 4) + 2] =
                            buf_p[((x + (y * LIGHTMAP_SIZE)) * 3) + 2];
                    fatbuffer[(index * 4) + 3] = 255;

                    R_ColorShiftLightingBytes(&fatbuffer[(index * 4) + 0],
                                              &fatbuffer[(index * 4) + 0]);
                }
            }
        }
        ...
        tr.fatLightmap =
                R_CreateImage(va("_fatlightmap%d", 0), (const byte**) &fatbuffer, tr.fatLightmapSize, tr.fatLightmapSize, 1, IF_LIGHTMAP | IF_NOCOMPRESSION, FT_DEFAULT, WT_CLAMP);
...
static float FatPackU(float input, int lightmapnum) {
    if (tr.fatLightmapSize > 0) {
        ...
        return (input / ((float) tr.fatLightmapStep)) +
               ((1.0 / ((float) tr.fatLightmapStep)) * (float) x);
    }
...
static float FatPackV(float input, int lightmapnum) {
    if (tr.fatLightmapSize > 0) {
        ...
        return (input / ((float) tr.fatLightmapStep)) +
               ((1.0 / ((float) tr.fatLightmapStep)) * (float) y);
    }
    ...
    for (i = 0; i < s_worldData.numClusters; i++) {
        ...
        for (j = 0; j < s_worldData.clusterBytes; j++) {
            ...
            for (k = 0; k < 8; k++) {
                ...
                // check if this cluster ( k = ( cluster & 7 ) ) is visible from the
                // current cluster
                if (!(bitbyte & (1 << k))) {
                    continue;
                }
...
static void ParseFace(dsurface_t* ds, drawVert_t* verts, bspSurface_t* surf, int* indexes) {
    ...
    cv->triangles = (srfTriangle_t*) ri.Hunk_Alloc(
            numTriangles * sizeof(cv->triangles[0]), h_low);

    cv->numVerts = numVerts;
    cv->verts =
            (srfVert_t*) ri.Hunk_Alloc(numVerts * sizeof(cv->verts[0]), h_low);
    ...
    components = (struct vertexComponent_t*) ri.Hunk_AllocateTempMemory(
            numVerts * sizeof(struct vertexComponent_t));
        ...
        for (i = 0, tri = cv->triangles; i < numTriangles; i++, tri++) {
            ...
            for (j = 0; j < 3; j++) {
                ...
                if (components[vertex].minVertex != minVertex) {
                    ...
                    components[minVertex].stBounds[0][0] =
                            MIN(components[minVertex].stBounds[0][0],
                                components[vertex].stBounds[0][0]);
                    components[minVertex].stBounds[0][1] =
                            MIN(components[minVertex].stBounds[0][1],
                                components[vertex].stBounds[0][1]);
                    components[minVertex].stBounds[1][0] =
                            MAX(components[minVertex].stBounds[1][0],
                                components[vertex].stBounds[1][0]);
                    components[minVertex].stBounds[1][1] =
                            MAX(components[minVertex].stBounds[1][1],
        ...
        if (components[i].minVertex == i) {
            for (j = 0; j < 2; j++) {
                components[i].stBounds[0][j] =
                        rintf(0.5f * (components[i].stBounds[1][j] +
                                      components[i].stBounds[0][j]));
            }
        }
...

    {
        ...
        for (i = 0, tri = cv->triangles; i < numTriangles; i++, tri++) {
            ...
            R_CalcTangents(tangent, binormal, dv0->xyz, dv1->xyz, dv2->xyz, dv0->st, dv1->st, dv2->st);
...
static void ParseTriSurf(dsurface_t* ds, drawVert_t* verts, bspSurface_t* surf, int* indexes) {
    ...
    // calc bounding box
    // HACK: don't loop only through the vertices because they can contain bad
    // data with .lwo models ...
    ClearBounds(cv->bounds[0], cv->bounds[1]);
...
int R_MergedWidthPoints(srfGridMesh_t* grid, int offset) {
    ...
    for (i = 1; i < grid->width - 1; i++) {
        for (j = i + 1; j < grid->width - 1; j++) {
            if (fabs(grid->verts[i + offset].xyz[0] -
                     grid->verts[j + offset].xyz[0]) > .1) {
                continue;
            }

            if (fabs(grid->verts[i + offset].xyz[1] -
                     grid->verts[j + offset].xyz[1]) > .1) {
                continue;
            }

            if (fabs(grid->verts[i + offset].xyz[2] -
                     grid->verts[j + offset].xyz[2]) > .1) {
                continue;
            }
...
FIXME: write generalized version that also avoids cracks between a patch and one
that meets half way?
=================
*/
            ...
            for (k = 1; k < grid1->width - 1; k++) {
                for (m = 0; m < 2; m++) {
                    ...
                    for (l = 1; l < grid2->width - 1; l++) {
                        //
                        if (fabs(grid1->verts[k + offset1].xyz[0] -
                                 grid2->verts[l + offset2].xyz[0]) > .1) {
                            continue;
                        }

                        if (fabs(grid1->verts[k + offset1].xyz[1] -
                                 grid2->verts[l + offset2].xyz[1]) > .1) {
                            continue;
                        }

                        if (fabs(grid1->verts[k + offset1].xyz[2] -
...
This function assumes that all patches in one group are nicely stitched together
for the highest LoD.
If this is not the case this function will still do its job but won't fix the
highest LoD cracks.
=================
*/
        ...
        for (k = 0; k < grid1->width - 2; k += 2) {
            for (m = 0; m < 2; m++) {
                ...
                for (l = 0; l < grid2->width - 1; l++) {
                    ...
                    if (fabs(v1[0] - v2[0]) < .01 && fabs(v1[1] - v2[1]) < .01 &&
                        fabs(v1[2] - v2[2]) < .01) {
                        continue;
                    }
            ...
            for (m = 0; m < 2; m++) {
                ...
                for (l = 0; l < grid2->width - 1; l++) {
                    ...
                    grid2 = R_GridInsertColumn(
                            grid2, l + 1, row, grid1->verts[grid1->width * (k + 1) + offset1].xyz, grid1->heightLodError[k + 1]);
                    grid2->lodStitched = false;
                    s_worldData.surfaces[grid2num].data = (surfaceType_t*) grid2;
                    return true;
                }
            ...
            for (m = 0; m < 2; m++) {
                ...
                for (l = 0; l < grid2->height - 1; l++) {
                    ...
                    grid2 = R_GridInsertRow(
                            grid2, l + 1, column, grid1->verts[grid1->width * (k + 1) + offset1].xyz, grid1->heightLodError[k + 1]);
                    grid2->lodStitched = false;
                    s_worldData.surfaces[grid2num].data = (surfaceType_t*) grid2;
                    return true;
                }
...
This function will try to stitch patches in the same LoD group together for the
highest LoD.

Only single missing vertice cracks will be fixed.

Vertices will be joined at the patch side a crack is first found, at the other
side
of the patch (on the same row or column) the vertices will not be joined and
cracks
...
void R_MovePatchSurfacesToHunk() {
    ...
    for (i = 0; i < s_worldData.numSurfaces; i++) {
        ...
        hunkgrid->triangles = (srfTriangle_t*) ri.Hunk_Alloc(
                grid->numTriangles * sizeof(srfTriangle_t), h_low);
        Com_Memcpy(hunkgrid->triangles, grid->triangles, grid->numTriangles * sizeof(srfTriangle_t));

        hunkgrid->numVerts = grid->numVerts;
        hunkgrid->verts =
                (srfVert_t*) ri.Hunk_Alloc(grid->numVerts * sizeof(srfVert_t), h_low);
...
static void R_CreateClusters() {
    ...
    for (i = 0, surface = s_worldData.surfaces; i < s_worldData.numSurfaces;
         i++, surface++) {
        surface->viewCount = -1;
    }
...
static void R_CreateWorldVBO() {
    ...
    for (i = 0; i < s_worldData.numnodes - s_worldData.numDecisionNodes; i++) {
        ...
        for (j = 0; j < leaf->numMarkSurfaces; j++) {
            ...
            if (*surf1->data != SF_GRID && *surf1->data != SF_TRIANGLES &&
                *surf1->data != SF_FACE) {
                continue;
            }
            ...
            for (k = j + 1; k < leaf->numMarkSurfaces; k++) {
                ...
                if (*surf2->data != SF_GRID && *surf2->data != SF_TRIANGLES &&
                    *surf2->data != SF_FACE) {
                    continue;
                }
                ...
                if (shader1 != shader2 || fogIndex1 != fogIndex2 ||
                    lightMapNum1 != lightMapNum2) {
                    continue;
                }
    ...
    surfaces = (bspSurface_t**) ri.Hunk_AllocateTempMemory(sizeof(*surfaces) *
                                                           numSurfaces);
    ...
    for (k = 0; k < s_worldData.numSurfaces; k++) {
        ...
        if (*surface->data == SF_FACE || *surface->data == SF_GRID ||
    ...
    s_worldData.verts = verts =
            (srfVert_t*) ri.Hunk_Alloc(numVerts * sizeof(srfVert_t), h_low);

    s_worldData.numTriangles = numTriangles;
    s_worldData.triangles = triangles = (srfTriangle_t*) ri.Hunk_Alloc(
            numTriangles * sizeof(srfTriangle_t), h_low);

    vboVerts = (shaderVertex_t*) ri.Hunk_AllocateTempMemory(
            numVerts * sizeof(shaderVertex_t));
    vboIdxs = (glIndex_t*) ri.Hunk_AllocateTempMemory(3 * numTriangles *
                                                      sizeof(glIndex_t));
        ...
        if (*surface->data == SF_FACE) {
            ...
            if (srf->numTriangles) {
                ...
                for (i = 0, tri = srf->triangles; i < srf->numTriangles; i++, tri++) {
                    for (j = 0; j < 3; j++) {
                        triangles[numTriangles + i].indexes[j] =
                                srf->firstVert + tri->indexes[j];
                    }
                }
            ...
            Tess_AutospriteDeform(surface->shader->autoSpriteMode, srf->firstVert, srf->numVerts, 3 * srf->firstTriangle, 3 * srf->numTriangles);
        } else if (*surface->data == SF_GRID) {
    ...
    s_worldData.vbo = R_CreateStaticVBO2(
            va("staticWorld_VBO %i", 0), numVerts, vboVerts, ATTR_POSITION | ATTR_TEXCOORD | ATTR_QTANGENT | ATTR_COLOR);
    s_worldData.ibo =
            R_CreateStaticIBO2(va("staticWorld_IBO %i", 0), numTriangles, vboIdxs);
    ...
    if (r_mergeLeafSurfaces->integer) {
        ...
        s_worldData.mergedSurfaces = (bspSurface_t*) ri.Hunk_Alloc(
                sizeof(*s_worldData.mergedSurfaces) * numMergedSurfaces, h_low);
        ...
        ri.Printf(PRINT_DEVELOPER,
                  "Processed %d surfaces into %d merged, %d unmerged\n",
                  numSurfaces,
                  numMergedSurfaces,
                  numUnmergedSurfaces);
    }
...
static void R_LoadSurfaces(lump_t* surfs, lump_t* verts, lump_t* indexLump) {
    ...
    ri.Printf(
            PRINT_DEVELOPER,
            "...loaded %d faces, %i meshes, %i trisurfs, %i flares %i foliages\n",
            numFaces,
            numMeshes,
            numTriSurfs,
            numFlares,
...
static void R_LoadSubmodels(lump_t* l) {
    ...
    s_worldData.models = out =
            (bspModel_t*) ri.Hunk_Alloc(count * sizeof(*out), h_low);
...
static void R_LoadNodesAndLeafs(lump_t* nodeLump, lump_t* leafLump) {
    ...
    s_worldData.skyNodes = (bspNode_t**) ri.Hunk_Alloc(
            WORLD_MAX_SKY_NODES * sizeof(*s_worldData.skyNodes), h_low);
    ...
    backEndData[0]->traversalList = (bspNode_t**) ri.Hunk_Alloc(
            sizeof(bspNode_t*) * s_worldData.numnodes, h_low);
    backEndData[0]->traversalLength = 0;

    if (r_smp->integer) {
        backEndData[1]->traversalList = (bspNode_t**) ri.Hunk_Alloc(
                sizeof(bspNode_t*) * s_worldData.numnodes, h_low);
...
static void R_LoadMarksurfaces(lump_t* l) {
    ...
    s_worldData.viewSurfaces =
            (bspSurface_t**) ri.Hunk_Alloc(count * sizeof(*out), h_low);
...
static void R_LoadFogs(lump_t* l, lump_t* brushesLump, lump_t* sidesLump) {
    ...
    s_worldData.fogs =
            (fog_t*) ri.Hunk_Alloc(s_worldData.numFogs * sizeof(*out), h_low);
...
void R_LoadLightGrid(lump_t* l) {
    ...
    VectorMA(w->lightGridOrigin, -0.5f, w->lightGridSize, w->lightGridGLOrigin);
    ...
    ri.Printf(PRINT_DEVELOPER, "grid size (%i %i %i)\n", (int) w->lightGridSize[0], (int) w->lightGridSize[1], (int) w->lightGridSize[2]);
    ...
    for (i = 0; i < w->numLightGridPoints;
        ...
        // RB: having a look in NormalToLatLong used by q3map2 shows the order of
        // latLong
        ...
        // First the vector is projected onto a unit octahedron, that means |x| +
        // |y| + |z| = 1,
        // then it is projected onto the x/y plane. The magnitude of z can be
        // reconstructed by
        // the above identity, but not the sign.
        // Fortunately the identity implies |x| + |y| <= 1, so all vectors fall
        // within a diamond
        ...
        //           +-----+-----+
        //
        // If z >= 0, we keep just the x,y coordinates in the diamond, otherwise the
        // point
        // is flipped across the nearest diamond edge into one of the outer
        // triangles.

        // The interpolation in this format behaves quite good except when
        // interpolating
    ...
    for (k = 0; k < w->lightGridBounds[2]; k++) {
        ...
        for (j = 0; j < w->lightGridBounds[1]; j++) {
            ...
            for (i = 0; i < w->lightGridBounds[0]; i++, gridPoint1++, gridPoint2++) {
                from[0] = i - 1;
                to[0] = i + 1;

                if (gridPoint1->ambient[0] || gridPoint1->ambient[1] ||
                    gridPoint1->ambient[2])
                    continue;

                scale = R_InterpolateLightGrid(w, from, to, factors, ambientColor, directedColor, direction);
                if (scale > 0.0f) {
                    scale = 1.0f / scale;
    ...
    tr.lightGrid1Image = R_Create3DImage(
            "<lightGrid1>", (const byte*) w->lightGridData1, w->lightGridBounds[0], w->lightGridBounds[1], w->lightGridBounds[2], IF_NOPICMIP | IF_NOLIGHTSCALE | IF_NOCOMPRESSION, FT_LINEAR, WT_EDGE_CLAMP);
    tr.lightGrid2Image = R_Create3DImage(
            "<lightGrid2>", (const byte*) w->lightGridData2, w->lightGridBounds[0], w->lightGridBounds[1], w->lightGridBounds[2], IF_NOPICMIP | IF_NOLIGHTSCALE | IF_NOCOMPRESSION, FT_LINEAR, WT_EDGE_CLAMP);
...
void R_LoadEntities(lump_t* l) {
    ...
    while (1) {
        ...
        if (!*token) {
            ri.Printf(PRINT_WARNING,
                      "WARNING: unexpected end of entities string while parsing "
                      "worldspawn\n");
            break;
        }
        ...
        // check for ambient color
                 ...
                 !Q_stricmp(keyname, "ambientColor")) {
    ...
    s_worldData.lights = (trRefLight_t*) ri.Hunk_Alloc(
            (s_worldData.numLights + 1) * sizeof(trRefLight_t), h_low);

    // basic light setup
    for (i = 0, light = s_worldData.lights; i < s_worldData.numLights;
         i++, light++) {
    ...
    while (1) {
        ...
        while (1) {
            ...
            // check for origin
                     ...
                     !Q_stricmp(keyname, "light_origin")) {
            ...
            // check for light shader
                ...
                light->l.attenuationShader =
                        RE_RegisterShader(value, RSF_LIGHT_ATTENUATION);
            }
            // check for rotation
            else if (!Q_stricmp(keyname, "rotation") ||
                     !Q_stricmp(keyname, "light_rotation")) {
                matrix_t rotation;
        ...
        if (!isLight) {
            ...
            if ((numOmniLights + numProjLights + numParallelLights) <
...
================
*/
static bool R_PrecacheGenericSurfInteraction(srfGeneric_t* face,
                                             trRefLight_t* light) {
...
======================
*/
static void R_PrecacheInteractionSurface(bspSurface_t* surf,
                                         trRefLight_t* light) {
    bool intersects;
    ...
    if (surf->shader->isSky ||
        (!surf->shader->interactLight && surf->shader->noShadows)) {
        return;
    }
    ...
    switch (*surf->data) {
        ...
        case SF_TRIANGLES:
            intersects =
                    R_PrecacheGenericSurfInteraction((srfGeneric_t*) surf->data, light);
            break;
        default:
            intersects = false;
            break;
...
================
*/
static void R_RecursivePrecacheInteractionNode(bspNode_t* node,
                                               trRefLight_t* light) {
    int r;
...

    {
        ...
        if (node->numMarkSurfaces > 0 &&
            R_CullLightWorldBounds(light, worldBounds) != CULL_OUT) {
            link_t* l;
    ...
    for (i = 0, tri = triangles; i < numTriangles; i++, tri++) {
        ...
        if (PlaneFromPoints(triPlane, pos[0], pos[1], pos[2])) {
            if (light->l.rlType == RL_DIRECTIONAL) {
                ...
                tri->facingLight = surfaceShader->cullType == CT_TWO_SIDED ||
                                   (d > 0 && surfaceShader->cullType != CT_BACK_SIDED);
            } else {
...
static void R_CreateVBOLightMeshes(trRefLight_t* light) {
    ...
    for (iaCache = light->firstInteractionCache; iaCache;
         iaCache = iaCache->next) {
    ...
    iaCachesSorted = (interactionCache_t**) ri.Hunk_AllocateTempMemory(
            numCaches * sizeof(iaCachesSorted[0]));
        ...
        if (shader != oldShader) {
            ...
            for (l = k; l < numCaches; l++) {
                ...
                if (*surface->data == SF_FACE) {
                    ...
                    if (srf->numTriangles) {
                        numLitTriangles =
                                UpdateLightTriangles(s_worldData.verts, srf->numTriangles, s_worldData.triangles + srf->firstTriangle, surface->shader, light);
            ...
            // create arrays
            indexes = (glIndex_t*) ri.Hunk_AllocateTempMemory(3 * numTriangles *
                                                              sizeof(glIndex_t));
            numTriangles = 0;
            ...
            for (l = k; l < numCaches; l++) {
                ...
                if (*surface->data == SF_FACE) {
                    ...
                    for (i = 0, tri = s_worldData.triangles + srf->firstTriangle;
                         i < srf->numTriangles;
                         i++, tri++) {
                        if (tri->facingLight) {
                            for (j = 0; j < 3; j++) {
            ...
            vboSurf->vbo = s_worldData.vbo;
            vboSurf->ibo =
                    R_CreateStaticIBO(va("staticLightMesh_IBO %i", c_vboLightSurfaces),
                                      indexes,
                                      3 * numTriangles);
...
static void R_CreateVBOShadowMeshes(trRefLight_t* light) {
    ...
    for (k = 0; k < numCaches; k++) {
        ...
        if (alphaTest ? shader != oldShader : alphaTest != oldAlphaTest) {
            ...
            vboSurf->vbo = s_worldData.vbo;
            vboSurf->ibo =
                    R_CreateStaticIBO(va("staticShadowMesh_IBO %i", c_vboLightSurfaces),
                                      indexes,
                                      3 * numTriangles);
            ...
            if (alphaTest ? shader != oldShader : alphaTest != oldAlphaTest) {
                ...
                for (l = k; l < numCaches; l++) {
                    ...
                    if (*surface->data == SF_FACE) {
                        ...
                        if (srf->numTriangles) {
                            numTriangles += UpdateLightTriangles(
                                    s_worldData.verts, srf->numTriangles, s_worldData.triangles + srf->firstTriangle, surface->shader, light);
                        }
                ...
                vboSurf->vbo = s_worldData.vbo;
                vboSurf->ibo = R_CreateStaticIBO(
                        va("staticShadowPyramidMesh_IBO %i", c_vboShadowSurfaces), indexes, numTriangles);
...
void R_PrecacheInteractions() {
    ...
    for (i = 0; i < s_worldData.numLights; i++) {
        ...
        // create a static VBO surface for each light geometry batch inside a
        // cubemap pyramid
    ...
    s_worldData.interactions = (interactionCache_t**) ri.Hunk_Alloc(
            s_worldData.numInteractions * sizeof(*s_worldData.interactions), h_low);

    for (i = 0; i < s_worldData.numInteractions; i++) {
        s_worldData.interactions[i] =
                (interactionCache_t*) Com_GrowListElement(&s_interactions, i);
    }
...
vertexHash_t** NewVertexHashTable() {
    vertexHash_t** hashTable =
            (vertexHash_t**) Com_Allocate(HASHTABLE_SIZE * sizeof(vertexHash_t*));
...
vertexHash_t* FindVertexInHashTable(vertexHash_t** hashTable, const vec3_t xyz, float distance) {
    ...
    for (vertexHash = hashTable[hash]; vertexHash;
         vertexHash = vertexHash->next) {
#ifndef HASH_USE_EPSILON
...
void GL_BindNearestCubeMap(const vec3_t xyz) {
    ...
    for (vertexHash = tr.cubeHashTable[hash]; vertexHash;
         vertexHash = vertexHash->next) {
...
void R_FindTwoNearestCubeMaps(const vec3_t position,
    ...
    *cubeProbeSecondNearest = nullptr;
    ...
    for (j = 0, vertexHash = tr.cubeHashTable[hash]; vertexHash;
         vertexHash = vertexHash->next, j++) {
        cubeProbe = (cubemapProbe_t*) vertexHash->data;
...
void R_BuildCubeMaps() {
    ...
    for (i = 0; i < 6; i++) {
        tr.cubeTemp[i] =
                (byte*) ri.Z_Malloc(REF_CUBEMAP_SIZE * REF_CUBEMAP_SIZE * 4);
    }
    ...
    for (j = 0; j < tr.cubeProbes.currentElements; j++) {
        ...
        if ((j + 1) >= nextTicCount) {
            size_t ticsNeeded =
                    (size_t)(((double) (j + 1) / tr.cubeProbes.currentElements) * 50.0);
        ...
        for (i = 0; i < 6; i++) {
            ...
            // encode the pixel intensity into the alpha channel, saves work in the
            // shader
            byte r, g, b, best;
    ...
    endTime = ri.Milliseconds();
    ri.Printf(PRINT_ALL,
              "cubemap probes pre-rendering time of %i cubes = %5.2f seconds\n",
              tr.cubeProbes.currentElements,
              (endTime - startTime) / 1000.0);
}
...
void RE_LoadWorldMap(const char* name) {
    ...
    if (i != BSP_VERSION && i != BSP_VERSION_Q3) {
        ri.FS_FreeFile(buffer);
        ri.Error(ERR_DROP,
                 "RE_LoadWorldMap: %s has wrong version number (%i should be %i "
                 "for ET or %i for Q3)",
                 name,
                 i,
                 BSP_VERSION,
    ...
    if (tr.sunShaderName) {
        tr.sunShader =
                R_FindShader(tr.sunShaderName, SHADER_3D_STATIC, RSF_DEFAULT);
    }
