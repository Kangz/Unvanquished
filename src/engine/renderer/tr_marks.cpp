...
=============
*/
static void R_ChopPolyBehindPlane(int numInPoints,
...
void R_BoxSurfaces_r(bspNode_t* node, vec3_t mins, vec3_t maxs, surfaceType_t** list, int listsize, int* listlength, vec3_t dir) {
    ...
    while (c--) {
        ...
        // extra check for surfaces to avoid list overflows
            ...
            if (s == 1 || s == 2) {
                surf->viewCount = tr.viewCountNoReset;
            } else if (DotProduct(((srfSurfaceFace_t*) surf->data)->plane.normal,
                                  dir) > -0.5) {
...
int R_MarkFragments(int numPoints, const vec3_t* points, const vec3_t projection, int maxPoints, vec3_t pointBuffer, int maxFragments, markFragment_t* fragmentBuffer) {
    ...
    for (i = 0; i < numPoints; i++) {
        ...
        // make sure we get all the leafs (also the one(s) in front of the hit
        // surface)
    ...
    for (i = 0; i < numsurfaces; i++) {
        if (*surfaces[i] == SF_GRID) {
            ...
            for (k = 0, tri = trisurf->triangles; k < trisurf->numTriangles;
                 k++, tri++) {
                for (j = 0; j < 3; j++) {
