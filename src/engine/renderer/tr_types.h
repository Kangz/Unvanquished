...
#define MAX_REF_ENTITIES \
    1023 // can't be increased without changing drawsurf bit packing
#define MAX_BONES 256
#define MAX_WEIGHTS \
    4 // GPU vertex skinning limit, never change this without rewriting many GLSL
// shaders
// XreaL END

#define MAX_CORONAS \
    32 //----(SA)  not really a reason to limit this other than trying to keep a
// reasonable count
#define MAX_DLIGHTS \
...
#define RDF_NOSHADOWS \
    (1 << 1)                    // force renderer to use faster lighting only path
...
#define RDF_UNDERWATER \
    (1 << 4) // so the renderer knows to use underwater fog when the player is
             // underwater
...
#define MAX_ALTSHADERS \
    64 // alternative shaders ('when <condition> <shader>') – selection
       // controlled from cgame
...
// RB: defining any of the following macros would break the compatibility to old
// ET mods
...
typedef enum { SK_INVALID,
               SK_RELATIVE,
               SK_ABSOLUTE } refSkeletonType_t;
...
typedef ALIGNED(16, struct {
  ...
  refBone_t bones[MAX_BONES]; }) refSkeleton_t;
...
typedef struct {
    ...
    int entityNum; // currentState.number, so we can attach rendering effects to
                   // specific entities (Zombie)
...
*/
typedef enum { TC_NONE,
               TC_S3TC,
               TC_EXT_COMP_S3TC } textureCompression_t;
