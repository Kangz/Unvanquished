...
void Tess_ClipSkyPolygons() {
    ...
    for (i = 0; i < tess.numIndexes; i += 3) {
        for (j = 0; j < 3; j++) {
            VectorSubtract(tess.verts[tess.indexes[i + j]].xyz,
...
static void FillCloudySkySide(const int mins[2], const int maxs[2], bool addIndexes) {
    ...
    // only add indexes for one pass, otherwise it would draw multiple times for
    // each pass
    if (addIndexes) {
...
static void DrawSkyBox(shader_t* shader) {
    ...
    for (i = 0; i < 6; i++) {
        ...
        sky_mins[0][i] =
                floor(sky_mins[0][i] * HALF_SKY_SUBDIVISIONS) / HALF_SKY_SUBDIVISIONS;
        sky_mins[1][i] =
                floor(sky_mins[1][i] * HALF_SKY_SUBDIVISIONS) / HALF_SKY_SUBDIVISIONS;
        sky_maxs[0][i] =
                ceil(sky_maxs[0][i] * HALF_SKY_SUBDIVISIONS) / HALF_SKY_SUBDIVISIONS;
        sky_maxs[1][i] =
                ceil(sky_maxs[1][i] * HALF_SKY_SUBDIVISIONS) / HALF_SKY_SUBDIVISIONS;
...
void Tess_StageIteratorSky() {
    ...
    if (r_logFile->integer) {
        ...
        GLimp_LogComment(
                va("--- Tess_StageIteratorSky( %s, %i vertices, %i triangles ) ---\n",
    ...
    if (tess.stageIteratorFunc2 == &Tess_StageIteratorDepthFill) {
        ...
        if (tess.surfaceShader->sky.outerbox &&
            tess.surfaceShader->sky.outerbox != tr.blackCubeImage) {
            R_BindVBO(tess.vbo);
            R_BindIBO(tess.ibo);
            ...
            gl_skyboxShader->SetUniform_ModelMatrix(
