...
static Cvar::Cvar<bool>
        cm_noAreas(VM_STRING_PREFIX "cm_noAreas",
                   "Ignore the areas (ie make them all connected)",
                   Cvar::CHEAT,
                   false);
...
int CM_PointContents(const vec3_t p, clipHandle_t model) {
    ...
    if (leaf->area == -1) {
        ...
        // p is in the void and we should return solid so particles can be removed
        // from the void
