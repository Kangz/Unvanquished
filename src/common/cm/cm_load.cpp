...
Cvar::Cvar<bool> cm_forceTriangles(VM_STRING_PREFIX "cm_forceTriangles",
                                   "Convert all patches into triangles?",
                                   Cvar::CHEAT | Cvar::ROM,
                                   false);
...
void CMod_LoadBrushes(lump_t* l) {
    ...
    cm.brushes =
            (cbrush_t*) CM_Alloc((BOX_BRUSHES + count) * sizeof(*cm.brushes));
    cm.numBrushes = count;
...
void CMod_LoadLeafs(lump_t* l) {
    ...
    cm.areaPortals =
            (int*) CM_Alloc(cm.numAreas * cm.numAreas * sizeof(*cm.areaPortals));
}
...
void CMod_LoadLeafBrushes(lump_t* l) {
    ...
    cm.leafbrushes =
            (int*) CM_Alloc((BOX_LEAF_BRUSHES + count) * sizeof(*cm.leafbrushes));
...
void CMod_LoadBrushSides(lump_t* l) {
    ...
    cm.brushsides =
            (cbrushside_t*) CM_Alloc((BOX_SIDES + count) * sizeof(*cm.brushsides));
...
void CMod_LoadSurfaces(lump_t* surfs, lump_t* verts, lump_t* indexesLump) {
    ...
    cm.surfaces =
            (cSurface_t**) CM_Alloc(cm.numSurfaces * sizeof(cm.surfaces[0]));
    ...
    for (i = 0; i < count; i++, in++) {
        if (LittleLong(in->surfaceType) == MST_PATCH) {
            ...
            surface->sc = CM_GeneratePatchCollide(width, height, vertexes);
                   ...
                   (cm.perPolyCollision || cm_forceTriangles.Get())) {
...
void CM_LoadMap(Str::StringRef name) {
    ...
    if (header.version != BSP_VERSION && header.version != BSP_VERSION_Q3) {
        Sys::Drop(
                "CM_LoadMap: %s has wrong version number (%i should be %i for ET or %i "
                "for Q3)",
                name.c_str(),
                header.version,
                BSP_VERSION,
