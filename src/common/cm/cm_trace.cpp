...
// always use capsule vs. capsule collision and never capsule vs. bbox or vice
// versa
...
CM_TracePointThroughSurfaceCollide
  ...
  special case for point traces because the surface collide "brushes" have no
volume
====================
*/
...
void CM_TraceThroughSphere(traceWork_t* tw, vec3_t origin, float radius, vec3_t start, vec3_t end) {
    ...
    // if no intersection with the sphere and the end point is at least an epsilon
    // away
    ...
    //  b = 2 * (dir[0] * (start[0] - origin[0]) + dir[1] * (start[1] - origin[1])
    //  + dir[2] * (start[2] - origin[2]));
    //  c = (start[0] - origin[0])^2 + (start[1] - origin[1])^2 + (start[2] -
    //  origin[2])^2 - radius^2;
    //
...
void CM_TraceThroughVerticalCylinder(traceWork_t* tw, vec3_t origin, float radius, float halfheight, vec3_t start, vec3_t end) {
    ...
    //
    //
    // (start[0] - origin[0] - t * dir[0]) ^ 2 + (start[1] - origin[1] - t *
    // dir[1]) ^ 2 = radius ^ 2
    // (v1[0] + t * dir[0]) ^ 2 + (v1[1] + t * dir[1]) ^ 2 = radius ^ 2;
    // v1[0] ^ 2 + 2 * v1[0] * t * dir[0] + (t * dir[0]) ^ 2 +
    //                      v1[1] ^ 2 + 2 * v1[1] * t * dir[1] + (t * dir[1]) ^ 2
    //                      = radius ^ 2
...
void CM_TraceCapsuleThroughCapsule(traceWork_t* tw, clipHandle_t model) {
    ...
    if (tw->start[0] != tw->end[0] || tw->start[1] != tw->end[1]) {
        // height of the expanded cylinder is the height of both cylinders minus the
...
==================
*/
static void CM_Trace(trace_t* results, const vec3_t start, const vec3_t end, vec3_t mins, vec3_t maxs, clipHandle_t model, const vec3_t origin, int brushmask, int skipmask, traceType_t type, sphere_t* sphere) {
    int i;
    traceWork_t tw;
    vec3_t offset;
    cmodel_t* cmod;
    ...
    if (sphere) {
        ...
        tw.sphere.radius =
                (tw.size[1][0] > tw.size[1][2]) ? tw.size[1][2] : tw.size[1][0];
    ...
    if (tw.type == TT_CAPSULE) {
        for (i = 0; i < 3; i++) {
            if (tw.start[i] < tw.end[i]) {
                tw.bounds[0][i] =
                        tw.start[i] - fabs(tw.sphere.offset[i]) - tw.sphere.radius;
                tw.bounds[1][i] =
                        tw.end[i] + fabs(tw.sphere.offset[i]) + tw.sphere.radius;
            } else {
                tw.bounds[0][i] =
                        tw.end[i] - fabs(tw.sphere.offset[i]) - tw.sphere.radius;
                tw.bounds[1][i] =
    ...
    // If allsolid is set (was entirely inside something solid), the plane is not
    // valid.
    // If fraction == 1.0, we never hit anything, and thus the plane is not valid.
    // Otherwise, the normal on the plane should have unit length

    // Tr3B: these asserts don't make sense as it is the task of the gamecode to
    // check if the trace was successful or not
...
==================
*/
void CM_BoxTrace(trace_t* results, const vec3_t start, const vec3_t end, vec3_t mins, vec3_t maxs, clipHandle_t model, int brushmask, int skipmask, traceType_t type) {
...
==================
*/
void CM_TransformedBoxTrace(trace_t* results, const vec3_t start, const vec3_t end, const vec3_t mins, const vec3_t maxs, clipHandle_t model, int brushmask, int skipmask, const vec3_t origin, const vec3_t angles, traceType_t type) {
    trace_t trace;
    ...
    if (rotated) {
        ...
        //       rotate
        //       the bounding box or the bmodel because that would make all the
        //       brush
        //       bevels invalid.
        //       However this is correct for capsules since a capsule itself is
        //       rotated too.
    ...
    CM_Trace(&trace, start_l, end_l, symetricSize[0], symetricSize[1], model, origin, brushmask, skipmask, type, &sphere);
...
==================
*/
void CM_BiSphereTrace(trace_t* results, const vec3_t start, const vec3_t end, float startRad, float endRad, clipHandle_t model, int mask, int skipmask) {
    int i;
    traceWork_t tw;
...
==================
*/
void CM_TransformedBiSphereTrace(trace_t* results, const vec3_t start, const vec3_t end, float startRad, float endRad, clipHandle_t model, int mask, int skipmask, const vec3_t origin) {
    trace_t trace;
    vec3_t start_l, end_l;
